...
| `DiTing_AGENT_BRIDGE_ENDPOINT` | 平台默认值 | Agent bridge broker endpoint。Windows 默认 `tcp://127.0.0.1:18765`；macOS/Linux 默认 `ipc:///tmp/DiTing-agent-bridge.sock`。 |
...
| `DiTing_AGENT_BRIDGE_WORKER_PORT_BASE` | `18780` | TCP worker endpoint 起始端口。 |
...
| `DiTing_WEB_UI_BACKEND_PORT` | `8648` | Vite dev proxy 使用的后端端口。 |
