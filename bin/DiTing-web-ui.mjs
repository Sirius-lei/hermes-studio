...
const WEB_UI_HOME = process.env.DiTing_WEB_UI_HOME?.trim()
  ? resolve(process.env.DiTing_WEB_UI_HOME.trim())
  : resolve(homedir(), '.diting-web-ui')
...
const WEB_UI_DB_FILE = join(WEB_UI_HOME, 'diting-web-ui.db')
const DEFAULT_PORT = 18648
...
function getDaemonStopGraceMs(options = {}) {
  ...
  if (restart && shouldPreserveBridgeOnShutdown()) {
    return envPositiveInt('DiTing_WEB_UI_RESTART_GRACE_MS') ?? DEFAULT_RESTART_GRACE_MS
  }
  if (restart) {
    return envPositiveInt('DiTing_WEB_UI_RESTART_GRACE_MS')
      ?? envPositiveInt('DiTing_WEB_UI_STOP_GRACE_MS')
      ?? DEFAULT_STOP_GRACE_MS
  }
  return envPositiveInt('DiTing_WEB_UI_STOP_GRACE_MS') ?? DEFAULT_STOP_GRACE_MS
}
...
function getGlobalCliBin() {
  ...
  return process.platform === 'win32'
    ? join(prefix, 'diting-web-ui.cmd')
    : join(prefix, 'bin', 'diting-web-ui')
}
...
function enableClientMode() {
  process.env.DiTing_WEB_UI_DISABLE_GATEWAY_AUTOSTART = '1'
...
function startDaemon(port) {
  ...
  if (existing && isRunning(existing)) {
    console.log(`  ✗ diting-web-ui is already running (PID: ${existing})`)
    console.log(`    Use "diting-web-ui stop" to stop it first`)
    process.exit(1)
  }
  removePid()
  ...
  console.log(`  ⏳ Starting diting-web-ui (PID: ${child.pid}, port: ${port})...`)

  function poll() {
    waited += interval
    if (!isRunning(child.pid)) {
      console.log('  ✗ Failed to start diting-web-ui')
    ...
    fetch(healthUrl).then(res => {
      if (res.ok) {
        ...
        console.log(`  ✓ diting-web-ui started`)
        console.log(`    ${url}`)
...
function stopDaemon(options = {}) {
  ...
  if (pidFromFile && !isRunning(pidFromFile)) {
    removePid()
    console.log(`  ✓ diting-web-ui was not running (cleaned stale PID: ${pidFromFile})`)
    pidFromFile = null
  ...
  if (!pid) {
    ...
    if (stoppedPreviewPids) {
      console.log(`  ✓ diting-web-ui preview stopped`)
      return
    }
    console.log('  ✗ diting-web-ui is not running')
    process.exit(1)
  }

  if (!isRunning(pid)) {
    removePid()
    console.log(`  ✓ diting-web-ui was not running (cleaned stale PID)`)
    return
  }
  ...
  try {
    try {
      ...
      // still shuts down the bridge; set DiTing_AGENT_BRIDGE_STOP_ON_SHUTDOWN=0
    ...
    removePid()
    console.log(`  ✓ diting-web-ui stopped (PID: ${pid})`)
  } catch (err) {
...
function showStatus() {
  ...
  if (pid && isRunning(pid)) {
    console.log(`  ✓ diting-web-ui is running (PID: ${pid})`)
...
function clearLoginLocks(options = {}) {
  ...
  if (!silent && serverRunning) {
    console.log('  ⚠ diting-web-ui is running; restart it to clear in-memory login locks.')
    console.log('    Run: diting-web-ui restart')
  }
...
async function main() {
  ...
  if (['-v', '--version', 'version'].includes(command)) {
    console.log(`diting-web-ui v${VERSION}`)
    process.exit(0)
  }

  if (['-h', '--help', 'help'].includes(command)) {
    console.log(`
diting-web-ui v${VERSION}

Usage: diting-web-ui <command> [options]
...
function doUpdate() {
  console.log('  ⬆ Updating diting-web-ui...')
...
function runUpdateInstall(npm) {
  const child = spawnCli(npm, ['install', '-g', 'diting-web-ui@latest'], {
    stdio: 'inherit',
    windowsHide: true,
    env: getCurrentNodeEnv(),
  })
