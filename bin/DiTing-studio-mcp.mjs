...
const DEFAULT_PORT = process.env.DiTing_WEB_UI_PORT || process.env.PORT || '18648'
const DEFAULT_BASE_URL = `http://127.0.0.1:${DEFAULT_PORT}`
const DISPLAY_COMMAND = 'diting-studio-mcp'
const SERVER_NAME = process.env.DiTing_MCP_SERVER_NAME || DISPLAY_COMMAND
...
DiTing Studio MCP stdio server.
...
Environment:
  DiTing_WEB_UI_URL       Web UI base URL. Default: ${DEFAULT_BASE_URL}
  DiTing_WEB_UI_HOME      Web UI state directory. Default: ~/.diting-web-ui
  DiTing_WEBUI_STATE_DIR  Fallback Web UI state directory.
  DiTing_WEB_UI_PROFILE   Default DiTing profile when a tool call omits profile.
  DiTing_WEB_UI_TOKEN     Optional explicit API token.
  AUTH_TOKEN              Optional explicit API token fallback.
  DiTing_MCP_TOOLSET      Tool category to expose: api, devices, or use. Default: api.
...
const requestedToolset = String(positionalArgs[0] || process.env.DiTing_MCP_TOOLSET || 'api').trim().toLowerCase()
...
function appHome() {
  return process.env.DiTing_WEB_UI_HOME ||
    process.env.DiTing_WEBUI_STATE_DIR ||
    join(homedir(), '.diting-web-ui')
}
...
function readToken(tokenOverride, allowTokenFile = true, profile = '') {
  const explicit = tokenOverride || process.env.DiTing_WEB_UI_TOKEN || process.env.AUTH_TOKEN
...
function defaultProfile() {
  return String(
    process.env.DiTing_WEB_UI_PROFILE ||
    process.env.DiTing_PROFILE ||
    process.env.PROFILE ||
    '',
  ).trim()
}
...
function authHint() {
  return `Web UI token was not accepted. Pass the current DiTing profile argument so this MCP server can read its temporary token, pass an explicit token argument, or set DiTing_WEB_UI_TOKEN.`
}

function baseUrl() {
  return (process.env.DiTing_WEB_UI_URL || DEFAULT_BASE_URL).replace(/\/$/, '')
}
...
function appendQuery(path, query) {
  ...
  const parsed = new URL(path, 'http://diting-web-ui.local')
...
async function requestEnvelope(path, options = {}) {
  ...
  const headers = {
    ...
    ...(profile ? { 'X-DiTing-Profile': profile } : {}),
  }
...
function normalizeApiPath(path) {
  ...
  const parsed = new URL(raw, 'http://diting-web-ui.local')
...
function pathWithoutQuery(path) {
  return new URL(path, 'http://diting-web-ui.local').pathname
}
...
async function validateApiRequest(method, path, args) {
  if (pathWithoutQuery(path) === '/api/openapi.json' || pathWithoutQuery(path) === '/api/DiTing/openapi.json') return null
...
const moduleHints = {
  ...
  Config: {
    purpose: 'Read and update DiTing Web UI configuration.',
  ...
  Files: {
    purpose: 'Browse and operate files exposed through the DiTing file browser.',
  ...
  Models: {
    purpose: 'Inspect and configure model ids available to DiTing.',
    keywords: ['models', 'model id', 'llm'],
  },
  Profiles: {
    purpose: 'Manage DiTing profiles and profile-scoped runtime state.',
  ...
  Skills: {
    purpose: 'Browse and manage skills available to DiTing agents.',
  ...
  'Write Gate': {
    purpose: 'Review and approve DiTing Agent write operations.',
...
function compactOpenApiDocument(openapi, args = {}) {
  ...
  return {
    title: openapi?.info?.title || 'DiTing Studio API',
    version: openapi?.info?.version || '',
    usage: hasFilters
      ? 'Use the selected operation details to call DiTing_studio_api_request with method, path, query, and body. Auth and profile are handled by the MCP server.'
      : 'This catalog is large. First read module purpose and keywords to choose the right module, then call DiTing_studio_api_openapi_get again with tag, path, or method filters to fetch endpoint details on demand.',
...
const tools = [
  {
    name: 'DiTing_studio_api_openapi_get',
    toolset: 'api',
    description: 'Return DiTing Studio API documentation as compact JSON. When the user asks to read/check the operation manual, API docs, endpoint docs, 接口文档, 接口手册, or 操作手册, call this tool without filters first to get the outline/module index. Without filters, returns only module purpose, keywords, and operation counts because the full API catalog is large. For endpoint details, call again with tag, path, or method filters, then use DiTing_studio_api_request.',
  ...
  {
    name: 'DiTing_studio_api_request',
    toolset: 'api',
    description: 'Execute a DiTing Studio operation by calling an endpoint path. Use DiTing_studio_api_openapi_get first as the operation manual to inspect method, parameters, requestBody, and responses.',
    inputSchema: inputSchema({
        method: {
          type: 'string',
          enum: ['GET', 'POST', 'PUT', 'PATCH', 'DELETE', 'HEAD'],
        ...
        path: {
          type: 'string',
          description: 'Relative DiTing Studio endpoint path from the operation manual, for example /api/DiTing/sessions?limit=20. Full URLs and // paths are rejected.',
        },
        body: {
        ...
        headers: {
          type: 'object',
          description: 'Optional request headers. Allowed names: accept, accept-language, content-type, x-request-id. Authorization and X-DiTing-Profile are filled from token/profile.',
  ...
  {
    name: 'DiTing_studio_use_chat_run',
    toolset: 'use',
    description: 'Start one DiTing Studio chat or coding-agent run through the HTTP bridge and wait for completion.',
    inputSchema: inputSchema({
        input: {
          oneOf: [
            { type: 'string' },
  ...
  {
    name: 'DiTing_studio_use_sessions_list',
    toolset: 'use',
    description: 'List DiTing Studio chat sessions.',
    inputSchema: inputSchema({
        limit: {
          type: 'number',
  ...
  {
    name: 'DiTing_studio_use_sessions_count',
    toolset: 'use',
    description: 'Count DiTing Studio chat sessions without returning the session list.',
    inputSchema: inputSchema({
        source: {
          type: 'string',
          description: 'Optional session source filter.',
  ...
  {
    name: 'DiTing_studio_use_usage_stats',
    toolset: 'use',
    description: 'Query DiTing Studio usage totals, cost estimate, model breakdown, and daily trend for the selected profile.',
    inputSchema: inputSchema({
        days: {
          type: 'number',
  ...
  {
    name: 'DiTing_studio_use_session_get',
    toolset: 'use',
    description: 'Get one DiTing Studio session by id.',
    inputSchema: inputSchema({
        session_id: {
          type: 'string',
  ...
  {
    name: 'DiTing_studio_use_session_messages',
    toolset: 'use',
    description: 'Get messages for one DiTing Studio conversation. By default returns user and assistant messages only.',
    inputSchema: inputSchema({
        session_id: {
          type: 'string',
  ...
  {
    name: 'DiTing_studio_use_session_context',
    toolset: 'use',
  ...
  {
    name: 'DiTing_studio_use_session_delete',
    toolset: 'use',
    description: 'Delete one DiTing Studio session by id.',
    inputSchema: inputSchema({
        session_id: {
          type: 'string',
  ...
  {
    name: 'DiTing_studio_use_session_rename',
    toolset: 'use',
    description: 'Rename one DiTing Studio session title.',
    inputSchema: inputSchema({
        session_id: {
          type: 'string',
  ...
  {
    name: 'DiTing_studio_use_profiles_list',
    toolset: 'use',
    description: 'List DiTing Studio profiles.',
    inputSchema: inputSchema(),
  },
  {
    name: 'DiTing_studio_use_available_models',
    toolset: 'use',
    description: 'List available DiTing Studio models for the selected profile.',
    inputSchema: inputSchema(),
  },
  {
    name: 'DiTing_studio_use_model_provider_get',
    toolset: 'use',
  ...
  {
    name: 'DiTing_studio_use_provider_add',
    toolset: 'use',
    description: 'Add or update a DiTing Studio model provider for the selected profile, then make it the active default provider/model.',
    inputSchema: inputSchema({
        name: {
          type: 'string',
  ...
  {
    name: 'DiTing_studio_use_provider_delete',
    toolset: 'use',
    description: 'Delete a DiTing Studio model provider or clear a built-in provider credential for the selected profile.',
    inputSchema: inputSchema({
        pool_key: {
          type: 'string',
  ...
  {
    name: 'DiTing_studio_use_worker_status',
    toolset: 'use',
    description: 'Summarize current DiTing worker state, including worker count, completed interactions, and sessions still interacting.',
    inputSchema: inputSchema(),
  },
  {
...
const TOOL_ALIASES = new Map([
  ['DiTing_api_openapi_get', 'DiTing_studio_api_openapi_get'],
  ['DiTing_api_request', 'DiTing_studio_api_request'],
  ['DiTing_lan_devices_list', 'DiTing_studio_lan_devices_list'],
  ['DiTing_lan_devices_scan', 'DiTing_studio_lan_devices_scan'],
  ['DiTing_lan_peer_connect', 'DiTing_studio_lan_peer_connect'],
  ['DiTing_lan_peer_connections', 'DiTing_studio_lan_peer_connections'],
  ['DiTing_lan_peer_disconnect', 'DiTing_studio_lan_peer_disconnect'],
  ['DiTing_lan_terminal_create', 'DiTing_studio_lan_terminal_create'],
  ['DiTing_lan_terminal_list', 'DiTing_studio_lan_terminal_list'],
  ['DiTing_lan_terminal_input', 'DiTing_studio_lan_terminal_input'],
  ['DiTing_lan_terminal_read', 'DiTing_studio_lan_terminal_read'],
  ['DiTing_lan_terminal_resize', 'DiTing_studio_lan_terminal_resize'],
  ['DiTing_lan_terminal_close', 'DiTing_studio_lan_terminal_close'],
  ['DiTing_lan_command_exec', 'DiTing_studio_lan_command_exec'],
  ['DiTing_lan_file_download', 'DiTing_studio_lan_file_download'],
  ['DiTing_lan_file_upload', 'DiTing_studio_lan_file_upload'],
])
...
async function callTool(name, args = {}) {
  ...
  switch (resolveToolName(name)) {
    ...
    case 'DiTing_studio_api_request': {
      ...
      if (!path) return errorText('Invalid path. Use a relative /api/... or /health path from DiTing_studio_api_openapi_get; full URLs are not allowed.')
      const validationError = await validateApiRequest(method, path, args)
      if (validationError) return errorText(`Invalid API request for ${method} ${pathWithoutQuery(path)}: ${validationError}. Use DiTing_studio_api_openapi_get to inspect required parameters and requestBody.`)
    ...
    case 'DiTing_studio_use_chat_run':
    ...
    case 'DiTing_studio_use_sessions_list':
      return jsonText(await request('/api/DiTing/sessions', withAuthArgs(args, {
        query: pickDefined(args, ['limit', 'source']),
      })))
    case 'DiTing_studio_use_sessions_count':
      return jsonText(await request('/api/DiTing/sessions/count', withAuthArgs(args, {
        query: pickDefined(args, ['source']),
      })))
    case 'DiTing_studio_use_usage_stats':
      return jsonText(await request('/api/DiTing/usage/stats', withAuthArgs(args, {
        query: pickDefined(args, ['days']),
      })))
    case 'DiTing_studio_use_session_get':
      return jsonText(await request(`/api/DiTing/sessions/${encodeURIComponent(args.session_id)}`, withAuthArgs(args)))
    case 'DiTing_studio_use_session_messages':
      return jsonText(await request(`/api/DiTing/sessions/conversations/${encodeURIComponent(args.session_id)}/messages`, withAuthArgs(args, {
        query: args.include_internal ? { humanOnly: '0' } : undefined,
      })))
    case 'DiTing_studio_use_session_context':
      return jsonText(cleanSessionContextPayload(
        await request(`/api/DiTing/sessions/${encodeURIComponent(args.session_id)}/context`, withAuthArgs(args)),
        args,
      ))
    case 'DiTing_studio_use_session_delete':
      return jsonText(await request(`/api/DiTing/sessions/${encodeURIComponent(args.session_id)}`, withAuthArgs(args, {
        method: 'DELETE',
      })))
    case 'DiTing_studio_use_session_rename':
      return jsonText(await request(`/api/DiTing/sessions/${encodeURIComponent(args.session_id)}/rename`, withAuthArgs(args, {
        method: 'POST',
        body: { title: args.title },
      })))
    case 'DiTing_studio_use_profiles_list':
      return jsonText(await request('/api/DiTing/profiles', withAuthArgs(args)))
    case 'DiTing_studio_use_available_models':
      return jsonText(await request('/api/DiTing/available-models', withAuthArgs(args)))
    case 'DiTing_studio_use_model_provider_get':
      return jsonText(findProvidersForModel(
        await request('/api/DiTing/available-models', withAuthArgs(args)),
        args.model,
      ))
    case 'DiTing_studio_use_provider_add':
      return jsonText(await request('/api/DiTing/config/providers', withAuthArgs(args, {
        method: 'POST',
        body: pickDefined(args, [
          'name',
          'base_url',
    ...
    case 'DiTing_studio_use_provider_delete':
      return jsonText(await request(`/api/DiTing/config/providers/${encodeURIComponent(args.pool_key)}`, withAuthArgs(args, {
        method: 'DELETE',
        query: pickDefined(args, ['source', 'providerKey']),
      })))
    case 'DiTing_studio_use_worker_status':
      return jsonText(summarizeWorkerRuntime(
        await request('/api/DiTing/performance/runtime', withAuthArgs(args)),
      ))
    case 'DiTing_studio_lan_devices_list':
      return jsonText(await request('/api/devices', withAuthArgs(args)))
    case 'DiTing_studio_lan_devices_scan':
      return jsonText(await request('/api/devices/scan', withAuthArgs(args, { method: 'POST' })))
    case 'DiTing_studio_lan_peer_connect':
      return jsonText(await request(`/api/devices/${encodeURIComponent(args.device_id)}/connect`, withAuthArgs(args, { method: 'POST' })))
    case 'DiTing_studio_lan_peer_connections':
      return jsonText(await request('/api/devices/peer-connections', withAuthArgs(args)))
    case 'DiTing_studio_lan_peer_disconnect':
    ...
    case 'DiTing_studio_lan_terminal_create':
    ...
    case 'DiTing_studio_lan_terminal_list':
    ...
    case 'DiTing_studio_lan_terminal_input':
    ...
    case 'DiTing_studio_lan_terminal_read':
    ...
    case 'DiTing_studio_lan_terminal_resize':
    ...
    case 'DiTing_studio_lan_terminal_close':
    ...
    case 'DiTing_studio_lan_command_exec':
    ...
    case 'DiTing_studio_lan_file_download':
    ...
    case 'DiTing_studio_lan_file_upload':
