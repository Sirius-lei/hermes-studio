...
DiTing Agent added a `/learn` slash command that turns a user-described source
or recent workflow into a reusable DiTing skill. The command does not use a
...
- Let users type `/learn <source or workflow>` in a normal DiTing chat session.
- Show `/learn` in the chat input slash-command picker.
- Run the prompt generated by DiTing Agent's `build_learn_prompt()` as the real
  agent input.
...
- Fail clearly when the bundled/runtime DiTing Agent version does not provide
  `agent.learn_prompt`.
...
- Do not duplicate DiTing Agent's skill-authoring prompt in TypeScript.
...
- Extend `CommandName` in
  `packages/server/src/services/DiTing/run-chat/session-command.ts`.
...
Add support in
`packages/server/src/services/DiTing/agent-bridge/python/bridge_pool.py`:
...
Update `packages/client/src/utils/DiTing/bridge-session-commands.ts`:
...
The feature depends on DiTing Agent containing `agent/learn_prompt.py`. Web UI
...
```text
/learn requires a newer DiTing Agent runtime with agent.learn_prompt.
```
...
2. Update bundled/runtime DiTing Agent to a revision containing
