...
impact: Bridge and coding-agent runs now add a one-hour user JWT to run instructions for MCP/Web UI calls, scoped Claude/Codex launches include the bundled DiTing MCP server, Codex receives DiTing media/file instructions per run instead of through AGENTS.md, and LAN MCP device endpoints require super admin JWT authorization instead of accepting the long-lived server .token.
---

Authenticated bridge runs include the current DiTing profile and a short-lived
model-run token in run instructions so MCP tools can pass them as arguments.
Scoped Claude Code and Codex launches include the bundled DiTing MCP server in
their generated config, and coding-agent inputs receive the same per-run
DiTing system prompt plus per-run profile/token prompt before being sent to
the agent. Claude Code receives the DiTing media/file prompt on every run via
`--append-system-prompt`. Codex receives the DiTing media/file prompt and the
...
runs also receive Codex-specific developer instructions that DiTing LAN
discovery is exposed as MCP tools rather than MCP resources/templates, naming
`mcp__DiTing-studio-devices__DiTing_studio_lan_devices_scan` and
`mcp__DiTing-studio-devices__DiTing_studio_lan_devices_list` as the tools to call with the
current `profile` and `token`. Managed Claude Code
`CLAUDE.md` blocks preserve existing user content and replace only the marked
DiTing Web UI prompt block. This keeps media, image, video, file, and
