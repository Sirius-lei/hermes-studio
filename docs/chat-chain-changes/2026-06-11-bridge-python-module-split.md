...
The Python bridge entrypoint moved to `packages/server/src/services/DiTing/agent-bridge/python/DiTing_bridge.py`. The implementation is split across `bridge_*.py` modules for runtime helpers, session pool, worker server, socket transport, and broker routing.
