# DiTing Agent Write Gate Notes

DiTing Agent v0.16.0 adds a write approval gate for persistent self-improvement
writes. The gate covers DiTing Agent memory and skill writes, not ordinary Web
UI state or arbitrary file edits.

## What It Controls

Write Gate protects two long-lived DiTing Agent stores:
...
The switches live in the active DiTing Agent profile's `config.yaml`.
...
```bash
DiTing config set memory.write_approval true
DiTing config set skills.write_approval true
```

Disable them with:

```bash
DiTing config set memory.write_approval false
DiTing config set skills.write_approval false
```

For multi-profile installs, the relevant file is the `config.yaml` under the
profile directory selected by `DiTing_HOME`.
...
```text
<DiTing_HOME>/pending/memory/*.json
<DiTing_HOME>/pending/skills/*.json
```
...
DiTing Agent exposes review commands through its CLI and gateway slash-command
surfaces:
...
DiTing Web UI already handles dangerous command approvals through
`approval.requested` / `approval.resolved` events and renders an approval bar in
chat and group chat.

Write Gate is different for Web UI sessions. DiTing Agent does not always emit a
...
- Reviewing those pending writes currently depends on DiTing Agent's
...
## DiTing Agent Code References

The upstream implementation is in DiTing Agent:
...
- `diting_cli/write_approval_commands.py`: shared `/memory` and `/skills`
