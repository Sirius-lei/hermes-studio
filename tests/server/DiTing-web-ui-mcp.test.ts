...
describe('DiTing-web-ui MCP server', () => {
  ...
  it('exposes a public Web UI API requester tool', async () => {
    const home = mkdtempSync(join(tmpdir(), 'DiTing-web-ui-mcp-'))
    homes.push(home)
    ...
    const server = createServer((req, res) => {
      if (req.url === '/api/openapi.json') {
        ...
        res.end(JSON.stringify({
          ...
          profile: req.headers['x-DiTing-profile'] || '',
        }))
        return
      }
    ...
    child = spawn(process.execPath, ['bin/DiTing-studio-mcp.mjs'], {
      cwd: process.cwd(),
      env: {
        ...process.env,
        DiTing_WEB_UI_URL: `http://127.0.0.1:${address.port}`,
        DiTing_WEB_UI_HOME: home,
        DiTing_WEB_UI_PROFILE: 'research',
      },
    })
    ...
    expect(initialized.result.serverInfo).toMatchObject({
      name: 'DiTing-studio-mcp',
      version: pkg.version,
    })
    expect(initialized.result.capabilities).toEqual({ tools: {} })

    const list = await waitForRpc(responses, 2)
    expect(list.result.tools.some((tool: any) => tool.name === 'DiTing_studio_api_request')).toBe(true)
    expect(list.result.tools.some((tool: any) => tool.name === 'DiTing_studio_lan_devices_list')).toBe(false)
  ...
  it.each([
    'bin/DiTing-studio-mcp.mjs',
    'bin/DiTing-web-ui-mcp.mjs',
    ...
    expect(code).toBe(0)
    expect(stdout.trim()).toBe(`DiTing-studio-mcp v${pkg.version}`)
  })

  it('exposes curated DiTing Studio use tools in the use toolset', async () => {
    const server = createServer((req, res) => {
      res.setHeader('content-type', 'application/json')
      if (req.url === '/api/chat-run/runs') {
        let raw = ''
      ...
      if (req.url === '/api/DiTing/sessions?limit=2&source=coding_agent') {
        res.end(JSON.stringify({ sessions: [{ id: 'session-1' }] }))
        return
      }
      if (req.url === '/api/DiTing/sessions/count?source=coding_agent') {
        res.end(JSON.stringify({ count: 7 }))
        return
      }
      if (req.url === '/api/DiTing/usage/stats?days=7') {
        res.end(JSON.stringify({
      ...
      if (req.url === '/api/DiTing/sessions/session-1' && req.method === 'GET') {
        res.end(JSON.stringify({ id: 'session-1', title: 'Session 1' }))
        return
      }
      if (req.url === '/api/DiTing/sessions/session-1/context') {
        res.end(JSON.stringify({
          session_id: 'session-1',
          title: 'Session 1',
          messages: [
      ...
      if (req.url === '/api/DiTing/sessions/session-1' && req.method === 'DELETE') {
        res.end(JSON.stringify({ ok: true, deleted: true }))
        return
      }
      if (req.url === '/api/DiTing/sessions/conversations/session-1/messages?humanOnly=0') {
        res.end(JSON.stringify({ messages: [{ role: 'system', content: 'internal' }] }))
        return
      }
      if (req.url === '/api/DiTing/sessions/session-1/rename' && req.method === 'POST') {
        let raw = ''
        req.on('data', chunk => { raw += chunk })
        req.on('end', () => {
      ...
      if (req.url === '/api/DiTing/profiles') {
        res.end(JSON.stringify({ profiles: ['default'] }))
        return
      }
      if (req.url === '/api/DiTing/available-models') {
        res.end(JSON.stringify({
          default: 'gpt-5.1',
      ...
      if (req.url === '/api/DiTing/config/providers' && req.method === 'POST') {
        let raw = ''
        req.on('data', chunk => { raw += chunk })
        req.on('end', () => {
          res.end(JSON.stringify({
      ...
      if (req.url === '/api/DiTing/config/providers/custom%3Aedge-router?source=providers&providerKey=edge-router' && req.method === 'DELETE') {
        res.end(JSON.stringify({ success: true, deleted: 'custom:edge-router' }))
        return
      }
      if (req.url === '/api/DiTing/performance/runtime') {
        res.end(JSON.stringify({
          timestamp: 123456,
          bridge: {
            reachable: true,
    ...
    const responses = new Map<number, any>()
    child = spawn(process.execPath, ['bin/DiTing-studio-mcp.mjs', 'use'], {
      cwd: process.cwd(),
      env: {
        ...process.env,
        DiTing_WEB_UI_URL: `http://127.0.0.1:${address.port}`,
    ...
    const list = await waitForRpc(responses, 2)
    expect(list.result.tools.some((tool: any) => tool.name === 'DiTing_studio_use_chat_run')).toBe(true)
    expect(list.result.tools.some((tool: any) => tool.name === 'DiTing_studio_use_sessions_count')).toBe(true)
    expect(list.result.tools.some((tool: any) => tool.name === 'DiTing_studio_use_usage_stats')).toBe(true)
    expect(list.result.tools.some((tool: any) => tool.name === 'DiTing_studio_use_session_context')).toBe(true)
    expect(list.result.tools.some((tool: any) => tool.name === 'DiTing_studio_use_session_delete')).toBe(true)
    expect(list.result.tools.some((tool: any) => tool.name === 'DiTing_studio_use_session_rename')).toBe(true)
    expect(list.result.tools.some((tool: any) => tool.name === 'DiTing_studio_use_model_provider_get')).toBe(true)
    expect(list.result.tools.some((tool: any) => tool.name === 'DiTing_studio_use_provider_add')).toBe(true)
    expect(list.result.tools.some((tool: any) => tool.name === 'DiTing_studio_use_provider_delete')).toBe(true)
    expect(list.result.tools.some((tool: any) => tool.name === 'DiTing_studio_use_worker_status')).toBe(true)
    expect(list.result.tools.some((tool: any) => tool.name === 'DiTing_studio_api_request')).toBe(false)
    expect(list.result.tools.some((tool: any) => tool.name === 'DiTing_studio_lan_devices_list')).toBe(false)
  ...
  it('filters exposed tools by MCP toolset argument', async () => {
    ...
    const responses = new Map<number, any>()
    child = spawn(process.execPath, ['bin/DiTing-studio-mcp.mjs', 'devices'], {
      cwd: process.cwd(),
      env: {
        ...process.env,
        DiTing_WEB_UI_URL: `http://127.0.0.1:${address.port}`,
    ...
    const list = await waitForRpc(responses, 2)
    expect(list.result.tools.some((tool: any) => tool.name === 'DiTing_studio_lan_devices_list')).toBe(true)
    expect(list.result.tools.some((tool: any) => tool.name === 'DiTing_studio_api_request')).toBe(false)
