...
const mockSpawnDiTing = vi.hoisted(() => vi.fn())
const mockLoggerError = vi.hoisted(() => vi.fn())

vi.mock('../../packages/server/src/services/DiTing/DiTing-process', () => ({
  execDiTing: (args: string[], options: unknown) => mockExecFileAsync('DiTing', args, options),
  spawnDiTing: mockSpawnDiTing,
}))
...
import * as service from '../../packages/server/src/services/DiTing/DiTing-kanban'

describe('DiTing kanban service', () => {
  beforeEach(() => {
    vi.clearAllMocks()
  })
