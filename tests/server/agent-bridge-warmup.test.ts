...
describe('agent bridge warmup', () => {
  ...
  it('defaults to the active profile when env is unset', async () => {
    const mod = await import('../../packages/server/src/services/hermes/agent-bridge/warmup')
    expect(mod.resolveWarmProfiles(undefined, 'research')).toEqual(['research'])
  })
