...
describe('DiTing schema initialization', () => {
  let db: any = null
  ...
  it('initializes all tables with correct schemas', async () => {
    const { initAllDiTingTables, USAGE_TABLE, SESSIONS_TABLE, MESSAGES_TABLE, GC_ROOMS_TABLE, USERS_TABLE, USER_PROFILES_TABLE, DEVICES_TABLE } =
      await import('../../packages/server/src/db/DiTing/schemas')

    expect(() => initAllDiTingTables()).not.toThrow()
  ...
  it('preserves existing data when adding safe schema columns', async () => {
    const { initAllDiTingTables, USAGE_TABLE, USAGE_SCHEMA } =
      await import('../../packages/server/src/db/DiTing/schemas')
  ...
  it('handles single-column primary key tables correctly', async () => {
    const { initAllDiTingTables, GC_ROOM_AGENTS_TABLE } =
      await import('../../packages/server/src/db/DiTing/schemas')
