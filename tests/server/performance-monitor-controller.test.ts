...
vi.mock('../../packages/server/src/services/DiTing/ops-monitor', () => ({
...
describe('performance monitor controller', () => {
  ...
  it('returns the runtime snapshot from the performance service', async () => {
    ...
    const { runtime } = await import('../../packages/server/src/controllers/DiTing/performance-monitor')
    await runtime(ctx)
  ...
  it('requires super admin on the runtime route', async () => {
    const { performanceMonitorRoutes } = await import('../../packages/server/src/routes/DiTing/performance-monitor')
    const layer = performanceMonitorRoutes.stack.find((entry: any) => entry.path === '/api/DiTing/performance/runtime')
