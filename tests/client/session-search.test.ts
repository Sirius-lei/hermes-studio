...
vi.mock('@/stores/DiTing/chat', () => ({
  useChatStore: () => chatStoreMock,
}))

const routerCurrentRoute = { value: { name: 'DiTing.logs' } }
...
describe('session search modal', () => {
  beforeEach(() => {
    ...
    routerCurrentRoute.value = { name: 'DiTing.logs' }
  })
  ...
  it('searches by content and opens the matched session', async () => {
    ...
    expect(apiMocks.routerPushMock).toHaveBeenCalledWith({ name: 'DiTing.session', params: { sessionId: 'match-1' } })
  })
})
