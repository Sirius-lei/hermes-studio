...
| `DiTing_AGENT_BRIDGE_ENDPOINT` | platform default | Agent bridge broker endpoint. Windows defaults to `tcp://127.0.0.1:28765`; macOS/Linux defaults to `ipc:///tmp/diting-agent-bridge.sock`. |
...
| `DiTing_AGENT_BRIDGE_WORKER_PORT_BASE` | `28780` | Base port for TCP worker endpoints. |
...
| `DiTing_WEB_UI_BACKEND_PORT` | `18647` | Backend port used by the Vite dev proxy. |
...
```
Browser → BFF (Koa, :18648) → Socket.IO /chat-run
                ↓
