...
`mcp_tool.py`, and `diting_cli/mcp_config.py`. This module is the ONLY place
...
# ---------------------------------------------------------------------------
# DiTingMCPOAuthProvider — OAuthClientProvider subclass with disk-watch
# ---------------------------------------------------------------------------


def _make_diting_provider_class() -> Optional[type]:
    ...
    class DiTingMCPOAuthProvider(OAuthClientProvider):
        ...
        def __init__(
            ...
            **kwargs: Any,
            ...
            self._diting_server_name = server_name
            ...
            self._diting_preregistered = preregistered
        ...
        async def _initialize(self) -> None:
            ...
            Paired with :class:`DiTingTokenStorage` persisting an absolute
            ...
            if (
                isinstance(storage, DiTingTokenStorage)
            ...
            async with httpx.AsyncClient(timeout=10.0) as client:
                ...
                for url in build_protected_resource_metadata_discovery_urls(
                    ...
                    try:
                        ...
                        logger.debug(
                            ...
                            self._diting_server_name, url, exc,
                        )
                        continue
                ...
                for url in build_oauth_authorization_server_metadata_discovery_urls(
                    ...
                    if asm:
                        ...
                        storage = self.context.storage
                        from tools.mcp_oauth import DiTingTokenStorage
                        if isinstance(storage, DiTingTokenStorage):
                            storage.save_oauth_metadata(asm)
                        logger.debug(
                            "MCP OAuth '%s': pre-flight ASM discovered "
                            "token_endpoint=%s",
                            self._diting_server_name, asm.token_endpoint,
                        )
                        break
        ...
        def _persist_oauth_metadata_if_changed(self) -> None:
            ...
            storage = self.context.storage
            from tools.mcp_oauth import DiTingTokenStorage
            if not isinstance(storage, DiTingTokenStorage):
                return
        ...
        async def _maybe_flag_poisoned_client(self, response: Any) -> None:
            ...
            and is handled by ``diting mcp reauth``.
            ...
            back to ``diting mcp reauth``.
            """
            try:
                if self._diting_preregistered:
                    return
    ...
    return DiTingMCPOAuthProvider


# Cached at import time. Tested and used by :class:`MCPOAuthManager`.
_DiTing_PROVIDER_CLS: Optional[type] = _make_diting_provider_class()
...
class MCPOAuthManager:
    ...
    def _build_provider(
        ...
        Constructs :class:`DiTingMCPOAuthProvider` directly using the helpers
        ...
        Returns None if the MCP SDK's OAuth support is unavailable.
        """
        if _DiTing_PROVIDER_CLS is None:
            logger.warning(
        ...
        if not _is_interactive() and not storage.has_cached_tokens():
            raise OAuthNonInteractiveError(
                ...
                "cached tokens found. Run `diting mcp login "
    ...
    def remove(self, server_name: str) -> None:
        ...
        Called by ``diting mcp remove <name>`` and (indirectly) by
        ``diting mcp login <name>`` during forced re-auth.
        """
        with self._entries_lock:
