...
2. **Per-result persistence** (maybe_persist_tool_result): After a tool
   ...
   SANDBOX temp dir (for example /tmp/diting-results/{tool_use_id}.txt on
   standard Linux, or $TMPDIR/diting-results/{tool_use_id}.txt on Termux)
...
STORAGE_DIR = "/tmp/diting-results"
HEREDOC_MARKER = "DiTing_PERSIST_EOF"
        ...
        if callable(get_temp_dir):
            try:
                ...
                if temp_dir:
                    ...
                    return f"{temp_dir}/diting-results"
    return STORAGE_DIR
...
def _heredoc_marker(content: str) -> str:
    ...
    return f"DiTing_PERSIST_{uuid.uuid4().hex[:8]}"
