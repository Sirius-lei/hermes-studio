...
* Never raises. Any I/O error (disk full, permission denied, missing
  DiTing_HOME, etc.) falls back to a byte-length truncation with an
...
def _resolve_spill_dir(directory_override: Optional[str], session_id: Optional[str]) -> Path:
    ...
    if directory_override:
        ...
        try:
            ...
            base = Path(get_diting_home()) / "hook_outputs"
        except Exception:
            # Last-resort fallback: DiTing_HOME env var, then ~/.diting
            home = os.environ.get("DiTing_HOME") or os.path.expanduser("~/.diting")
