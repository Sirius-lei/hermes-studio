...
<url>``) is spawned as a direct child of the DiTing process. DiTing's own
...
DiTing process dies hard — ``kill -9``, an OS-level crash, a force-quit of
...
parent dies, so nothing reaps these until the next DiTing startup's opt-in
