...
def auth_json_path():
    """Return the DiTing auth store path, respecting DiTing_HOME overrides."""
    return get_diting_home() / "auth.json"
...
def peek_nous_access_token() -> Optional[str]:
    ...
    Availability scans (`diting tools`, banner/status paint, provider
