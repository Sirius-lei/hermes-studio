...
`diting skills publish` for sharing. No new source type, no new store, no new
...
@dataclass
class BlueprintSpec:
    """Parsed ``metadata.diting.blueprint`` automation spec for a skill."""
...
def parse_blueprint(skill_md_text: str) -> Optional[BlueprintSpec]:
    ...
    A skill is a blueprint iff ``metadata.diting.blueprint`` is a mapping containing
    ...
    diting = meta.get("diting") if isinstance(meta, dict) else None
    blueprint = diting.get("blueprint") if isinstance(diting, dict) else None
    if blueprint is None:
        return None
    if not isinstance(blueprint, dict):
        raise BlueprintError("metadata.diting.blueprint must be a mapping")
...
def export_blueprint(job: Dict[str, Any], body: str, *, blueprint_name: Optional[str] = None) -> str:
    ...
    and emit a SKILL.md (with a ``metadata.diting.blueprint`` block) they can hand
    to ``diting skills publish`` to share. ``body`` is the plain-language
