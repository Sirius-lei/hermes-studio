...
One ``CDPSupervisor`` runs per DiTing ``task_id`` that has a reachable CDP
...
def _redact_cdp_error_text(exc: object) -> str:
    ...
    those credentials never reach DiTing logs or tracebacks. Falls back to a
...
DIALOG_BRIDGE_HOST = "diting-dialog-bridge.invalid"
...
(() => {
  if (window.__ditingDialogBridgeInstalled) return;
  window.__ditingDialogBridgeInstalled = true;
  const ENDPOINT = "http://diting-dialog-bridge.invalid/";
