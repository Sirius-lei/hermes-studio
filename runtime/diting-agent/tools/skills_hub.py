#!/usr/bin/env python3
"""
Skills Hub — Source adapters and hub state management for the DiTing Skills Hub.
...
Used by diting_cli/skills_hub.py for CLI commands and the /skills slash command.
"""
...
def _skills_dir() -> Path:
    ...
    return Path(forced) if forced is not None else _diting_home() / "skills"
...
class GitHubSource(SkillSource):
    ...
    def inspect(self, identifier: str) -> Optional[SkillMeta]:
        ...
        if isinstance(metadata, dict):
            diting_meta = metadata.get("diting", {})
            if isinstance(diting_meta, dict):
                tags = diting_meta.get("tags", [])
        if not tags:
...
class OptionalSkillSource(SkillSource):
    ...
    OFFICIAL_REPO = "NousResearch/diting-agent"
    ...
    def _scan_all(self) -> List[SkillMeta]:
        ...
        for skill_md in sorted(self._optional_dir.rglob("SKILL.md")):
            ...
            if isinstance(meta_block, dict):
                diting_meta = meta_block.get("diting", {})
                if isinstance(diting_meta, dict):
                    tags = diting_meta.get("tags", [])
...
# ---------------------------------------------------------------------------
# DiTing centralized index source
# ---------------------------------------------------------------------------

DiTing_INDEX_URL = "https://diting-agent.nousresearch.com/docs/api/skills-index.json"
DiTing_INDEX_TTL = 6 * 3600  # 6 hours


def _diting_index_cache_file() -> Path:
    return _index_cache_dir() / "diting-index.json"


def _load_diting_index() -> Optional[dict]:
    """Fetch the centralized skills index, with local cache.

    The index is a JSON file hosted on the docs site, rebuilt daily by CI.
    We cache it locally for DiTing_INDEX_TTL seconds to avoid repeated
    downloads within a session.
    """
    # Check local cache
    diting_index_cache_file = _diting_index_cache_file()
    if diting_index_cache_file.exists():
        try:
            age = time.time() - diting_index_cache_file.stat().st_mtime
            if age < DiTing_INDEX_TTL:
                return json.loads(diting_index_cache_file.read_text())
        except (OSError, json.JSONDecodeError):
            pass
    ...
    for accept_encoding in ("gzip, deflate", "identity"):
        try:
            ...
            if resp.status_code != 200:
                logger.debug("DiTing index fetch returned %d", resp.status_code)
            ...
            logger.debug(
                "DiTing index decode failed (Accept-Encoding=%s): %s",
                accept_encoding,
                e,
            )
            continue
        except (httpx.HTTPError, json.JSONDecodeError) as e:
            logger.debug("DiTing index fetch failed: %s", e)
            return _load_stale_index_cache()
    ...
    # Cache locally
    try:
        diting_index_cache_file.parent.mkdir(parents=True, exist_ok=True)
        diting_index_cache_file.write_text(json.dumps(data))
    except OSError:
        pass
...
class DiTingIndexSource(SkillSource):
    """Skill source backed by the centralized DiTing Skills Index.
    ...
    def _ensure_loaded(self) -> dict:
        if not self._loaded:
            self._index = _load_diting_index()
            self._loaded = True
    ...
    def fetch(self, identifier: str) -> Optional[SkillBundle]:
        ...
        if resolved:
            ...
            if bundle:
                bundle.source = entry.get("source", "diting-index")
    ...
    def _to_meta(entry: dict) -> SkillMeta:
        return SkillMeta(
            name=entry.get("name", ""),
            description=entry.get("description", ""),
            source=entry.get("source", "diting-index"),
            identifier=entry.get("identifier", ""),
...
def create_source_router(auth: Optional[GitHubAuth] = None) -> List[SkillSource]:
    ...
    sources: List[SkillSource] = [
        ...
        DiTingIndexSource(auth=auth), # Centralized index (search + resolved install paths)
