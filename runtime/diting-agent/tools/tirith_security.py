...
it is automatically downloaded from GitHub releases to $DiTing_HOME/bin/tirith.
...
def _get_diting_home() -> str:
    """Return the DiTing home directory, respecting DiTing_HOME env var."""
    return str(get_diting_home())


def _failure_marker_path() -> str:
    """Return the path to the install-failure marker file."""
    return os.path.join(_get_diting_home(), ".tirith-install-failed")
...
def _diting_bin_dir() -> str:
    """Return $DiTing_HOME/bin, creating it if needed."""
    d = os.path.join(_get_diting_home(), "bin")
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $DiTing_HOME/bin/tirith.
    ...
    try:
        ...
        dest = os.path.join(_diting_bin_dir(), "tirith")
        try:
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    2. $DiTing_HOME/bin/tirith (previously auto-installed)
    3. Auto-install from GitHub releases → $DiTing_HOME/bin/tirith
    ...
    diting_bin = os.path.join(_diting_bin_dir(), "tirith")
    if os.path.isfile(diting_bin) and os.access(diting_bin, os.X_OK):
        _resolved_path = diting_bin
        _install_failure_reason = ""
        _clear_install_failed()
        return diting_bin
