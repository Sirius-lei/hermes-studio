...
# Must match diting_cli.runtime_provider.RUNTIME_PROVIDER_TYPE_CUSTOM.
...
def _expand_parent_toolsets(parent_toolsets: set) -> set:
    ...
    When a parent uses a composite toolset like ``diting-cli`` (which bundles
...
def _build_child_agent(
    ...
    if toolsets:
        ...
        # Expand composite toolsets (e.g. diting-cli) so that individual
...
def _dump_subagent_timeout_diagnostic(
    *,
    ...
    writes a dedicated log under ``~/.diting/logs/subagent-<sid>-<ts>.log``
...
def _spill_summary_to_file(task_index: int, summary: str) -> Optional[str]:
    ...
    try:
        ...
        cache_dir = get_diting_dir("cache/delegation", "delegation_cache")
...
def delegate_task(
    ...
    if background:
        ...
        try:
            ...
            _source = get_session_env("DiTing_SESSION_SOURCE", "")
            _origin_ui_session_id = get_session_env("DiTing_UI_SESSION_ID", "")
...
def _resolve_delegation_credentials(cfg: dict, parent_agent) -> dict:
    ...
    if not api_key:
        raise ValueError(
            ...
            f"Set the appropriate environment variable or run 'diting auth'."
        )
...
def _load_config() -> dict:
    """Load delegation config from the active DiTing config.

    Prefer the shared persistent loader because it follows the active
    DiTing_HOME/profile. ``cli.CLI_CONFIG`` is a legacy fallback for entry
    ...
    ``DiTing_IGNORE_USER_CONFIG=1`` (``diting chat --ignore-user-config``) is
    ...
    """
    prefer_legacy = os.environ.get("DiTing_IGNORE_USER_CONFIG") == "1"
