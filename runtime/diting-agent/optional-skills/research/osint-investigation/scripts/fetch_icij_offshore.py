...
$DiTing_OSINT_CACHE/icij/ (default: ~/.cache/diting-osint/icij/) and search
...
def _cache_dir() -> Path:
    base = os.environ.get("DiTing_OSINT_CACHE")
    if base:
        return Path(base) / "icij"
    return Path.home() / ".cache" / "diting-osint" / "icij"
...
def _download(dest: Path, force: bool = False) -> Path:
    ...
    req = urllib.request.Request(
        BULK_URL,
        headers={"User-Agent": "diting-agent osint-investigation skill"},
    )
...
def main() -> int:
    ...
    p.add_argument(
        ...
        type=Path,
        default=None,
        help="Override cache directory (default: $DiTing_OSINT_CACHE/icij or ~/.cache/diting-osint/icij)",
    )
    p.add_argument(
        "--force-refresh",
        action="store_true",
