...
## 5. GODMODE FAST — DiTing 4 405B (Zero Refusal)

**Model:** `nousresearch/diting-4-405b`
**Strategy:** DiTing models are already uncensored, so this combo is about speed — instant token streaming with zero refusal checking needed. Uses the same Pliny Love divider format as a formality.
...
## Using with DiTing Agent

### As ephemeral system prompt (config.yaml)

Pick a template above and set it in `~/.diting/config.yaml`:
