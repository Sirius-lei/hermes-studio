...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("DiTing_HOME", os.path.expanduser("~/.diting")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("DiTing_HOME", Path.home() / ".diting")) / "skills" / "red-teaming" / "godmode"
...
DiTing_HOME = Path(os.getenv("DiTing_HOME", Path.home() / ".diting"))
CONFIG_PATH = DiTing_HOME / "config.yaml"
PREFILL_PATH = DiTing_HOME / "prefill.json"
...
MODEL_STRATEGIES = {
    ...
    # Nous/DiTing models — already uncensored, just needs clean prompt
    "diting": {
        "order": ["prefill_only"],
        "system_templates": {},
    },
...
def _detect_model_family(model: str) -> str:
    ...
    if "grok" in model_lower or "x-ai" in model_lower:
        return "grok"
    if "diting" in model_lower or "nous" in model_lower:
        return "diting"
...
def _get_current_model() -> tuple:
    """Read current model and provider from DiTing config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.diting/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart DiTing for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart DiTing for changes to take effect.")
