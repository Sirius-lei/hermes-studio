...
The design is **DiTing-native**: a small deterministic Python CLI (`scripts/pdd.py`) owns the state
...
```bash
diting skills install official/security/unbroker
```

Then start a new DiTing session and drive it (below). The skill works zero-config; a few optional
...
Drive it from a DiTing session:
...
- **PII handling.** Dossiers live under the DiTing home directory (`0600`, optionally
