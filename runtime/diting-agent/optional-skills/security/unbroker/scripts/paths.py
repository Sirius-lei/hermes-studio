...
All per-subject data lives under PDD_DATA_DIR (default: $DiTing_HOME/unbroker),
which is the same trust boundary DiTing uses for .env and OAuth tokens.
"""
...
def diting_home() -> Path:
    return Path(os.environ.get("DiTing_HOME") or (Path.home() / ".diting"))


def data_dir() -> Path:
    override = os.environ.get("PDD_DATA_DIR")
    return Path(override) if override else diting_home() / "unbroker"
