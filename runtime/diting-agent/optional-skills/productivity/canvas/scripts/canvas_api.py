#!/usr/bin/env python3
"""Canvas LMS API CLI for DiTing Agent.
...
def _check_config():
    ...
    if missing:
        diting_env = os.path.join(
            os.environ.get("DiTing_HOME", os.path.expanduser("~/.diting")), ".env"
        )
        print(
            f"Missing required environment variables: {', '.join(missing)}\n"
            f"Set them in {diting_env} or export them in your shell.\n"
...
def main():
    parser = argparse.ArgumentParser(
        description="Canvas LMS API CLI for DiTing Agent"
    )
