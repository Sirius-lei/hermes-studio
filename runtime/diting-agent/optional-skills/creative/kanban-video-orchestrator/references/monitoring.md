...
diting kanban watch --tenant <project-slug>

# Snapshot of the board
diting kanban list --tenant <project-slug>
diting kanban list --tenant <project-slug> --json     # machine-readable

# Per-status counts + oldest-ready age
diting kanban stats --tenant <project-slug>

# Visual dashboard (browser)
diting dashboard

# Inspect a specific task (includes comments + events)
diting kanban show <task-id>

# Follow a single task's event stream
diting kanban tail <task-id>
```

Verify available subcommands with `diting kanban --help` — the kanban CLI
...
|---------|--------------|--------|
| Task RUNNING but no heartbeat in 2+ min | Worker stuck, infinite loop, blocked on input | `diting kanban show <id>` — read the worker's last events. The dispatcher SIGTERMs tasks that exceed their `max-runtime`; if you need to stop one earlier, `diting kanban block <id>` then `diting kanban archive <id>`, and create a re-run task. |
| Same task retried 2+ times | Reproducible failure (missing key, bad spec, broken tool) | `diting kanban show <id>` to read failure events. Fix root cause before re-running. |
| RUNNING longer than max_runtime | Task is slow but progressing OR genuinely stuck | Check heartbeats with `diting kanban tail <id>`. If progressing, the dispatcher will SIGTERM eventually anyway — raise `max-runtime` on a re-created task. |
...
| Tasks created but never picked up | Profile not running, or tenant mismatch, or dispatcher not running | Check `diting profile list` (profile exists?), `diting status` (gateway/dispatcher up?), and verify tenant. |
...
diting kanban comment <renderer-task-id> "Scene 3 looks too sparse \
— increase visual density. Tighten color palette to brand spec."

# 2. Create a re-render task with the original as parent
diting kanban create "Scene 3 — re-render with feedback" \
...
NEW_TASK_ID=$(diting kanban create "Generate SRT captions from voiceover" \
    --assignee captioner \
    --workspace dir:"$HOME/projects/video-pipeline/<slug>" \
    --tenant <slug> \
...
diting kanban link "$NEW_TASK_ID" <editor-task-id>
```
...
diting kanban block <task-id>
diting kanban archive <task-id>

# Diagnose what happened
diting kanban show <task-id>      # task body, comments, recent events
diting kanban tail <task-id>      # follow the live event stream
diting kanban log <task-id>       # worker process log
```
...
3. Re-fire the initial `diting kanban create` for the director
...
while true; do
    clear
    diting kanban list --tenant <slug>
    echo "---"
    diting kanban stats --tenant <slug>
    sleep 30
done
```

For a live event feed, run `diting kanban watch --tenant <slug>` in a
...
- **Profile process not running.** Tasks queue indefinitely in READY if no
  worker for that profile is online. Check `diting profile list` and start
