...
the actual `setup.sh` that creates the project workspace, configures DiTing
...
2. **Create profiles** — `diting profile create <name> --clone`
3. **Configure profiles** — patch each profile's
   `~/.diting/profiles/<name>/config.yaml` to set toolsets, always_load skills,
   and `cwd`
4. **Write SOUL.md per profile** — the personality + role definition
5. **Copy any provided assets + write `brief.md`, `TEAM.md`, and `taste/`**
6. **Fire the initial kanban task** — `diting kanban create` assigned to the director
...
```bash
diting profile create director --clone 2>/dev/null || true
```
...
Each profile has a YAML config at `~/.diting/profiles/<name>/config.yaml`. The
...
p = os.path.expanduser(f"~/.diting/profiles/{profile}/config.yaml")
with open(p) as f:
...
PyYAML must be installed in the user's Python (it ships with most DiTing
...
Each profile gets a `SOUL.md` at `~/.diting/profiles/<name>/SOUL.md` that
...
```bash
diting kanban create "Direct production of <video title>" \
    --assignee director \
...
the DiTing `.env` (`${DiTing_HOME:-$HOME/.diting}/.env`) and macOS Keychain
(if on macOS):
...
check_key() {
    ...
    local kc_account="$2"
    local kc_service="$3"
    local _diting_env="${DiTing_HOME:-$HOME/.diting}/.env"
    if grep -q "^${var}=" "$_diting_env" 2>/dev/null && \
       [ -n "$(grep "^${var}=" "$_diting_env" | cut -d= -f2-)" ]; then
        return 0
    fi
    ...
    echo "ERROR: ${var} not set in ${_diting_env} or Keychain (${kc_account}/${kc_service})"
    return 1
}

check_key ELEVENLABS_API_KEY diting ELEVENLABS_API_KEY || exit 1
check_key OPENROUTER_API_KEY diting OPENROUTER_API_KEY || exit 1
# ...
```
