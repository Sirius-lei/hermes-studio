...
Maps each role archetype to the DiTing skills it should `always_load` and the
toolsets it needs. Only references skills that ship in the public diting-agent
...
## DiTing skills relevant to video production

### Visual / rendering skills (`diting-agent/skills/creative/`)
...
### Design / pre-production skills (`diting-agent/skills/creative/`)
...
### Audio / media skills (`diting-agent/skills/creative/` + `skills/media/`)
...
These are **not** DiTing skills but external CLIs / APIs that profiles invoke.
...
## Built-in DiTing tools for media review

These are native DiTing tools — not invoked via terminal but through their own
...
key is present in `${DiTing_HOME:-~/.diting}/.env` (or macOS Keychain) before firing the kanban.
...
| Anthropic | `ANTHROPIC_API_KEY` | every DiTing profile (Claude) |

If a key is missing, prompt the user to add it. Storage methods, in order of
preference: macOS Keychain → `${DiTing_HOME:-~/.diting}/.env` → environment variable.
...
When a new DiTing-public video skill ships:
