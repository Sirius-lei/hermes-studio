...
Usage (via cron with --no-agent):
    ...
    diting cron create diting-issues \\
      --schedule "*/5 * * * *" --no-agent \\
      --script "$DiTing_HOME/skills/devops/watchers/scripts/watch_github.py" \\
      --script-args "--name diting-issues --repo NousResearch/diting-agent --scope issues"

Set GITHUB_TOKEN (or GH_TOKEN) in the DiTing .env file
(``${DiTing_HOME:-~/.diting}/.env``) to avoid the 60 req/hr
