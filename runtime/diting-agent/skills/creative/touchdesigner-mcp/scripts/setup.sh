...
DiTing_HOME_DIR="${DiTing_HOME:-$HOME/.diting}"
DiTing_CFG="${DiTing_HOME_DIR}/config.yaml"
MCP_PORT=40404
...
# ── 3. Ensure DiTing config has twozero_td MCP entry ──
if [[ ! -f "$DiTing_CFG" ]]; then
    echo -e " ${FAIL} DiTing config not found at ${DiTing_CFG}"
    manual_steps+=("Create ${DiTing_CFG} with twozero_td MCP server entry")
elif grep -q 'twozero_td' "$DiTing_CFG" 2>/dev/null; then
    echo -e " ${OK} twozero_td MCP entry exists in DiTing config"
else
    echo -e " ${WARN} Adding twozero_td MCP entry to DiTing config..."
    python3 -c "
...
" 2>/dev/null && echo -e " ${OK} twozero_td MCP entry added to config" \
              || { echo -e " ${FAIL} Could not update config (is PyYAML installed?)"; \
                   manual_steps+=("Add twozero_td MCP entry to ${DiTing_CFG} manually"); }
    manual_steps+=("Restart DiTing session to pick up config change")
fi
