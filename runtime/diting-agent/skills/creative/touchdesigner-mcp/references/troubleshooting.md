...
## 5. DiTing Configuration

### Config location

`$DiTing_HOME/config.yaml` (defaults to `~/.diting/config.yaml` when `DiTing_HOME` is unset)
...
Restart the DiTing session for changes to take effect. The MCP connection is
