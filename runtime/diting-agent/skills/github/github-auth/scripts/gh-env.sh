#!/usr/bin/env bash
# GitHub environment detection helper for DiTing Agent skills.
#
...
if command -v gh &>/dev/null && gh auth status &>/dev/null 2>&1; then
    ...
    GH_AUTH_METHOD="curl"
elif _diting_env="${DiTing_HOME:-$HOME/.diting}/.env"; [ -f "$_diting_env" ] && grep -q "^GITHUB_TOKEN=" "$_diting_env" 2>/dev/null; then
