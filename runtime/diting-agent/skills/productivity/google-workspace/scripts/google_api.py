#!/usr/bin/env python3
"""Google Workspace API CLI for DiTing Agent.

Uses the Google Workspace CLI (`gws`) when available, but preserves the
existing DiTing-facing JSON contract and falls back to the Python client
...
from _diting_home import get_diting_home
...
def _gws_binary() -> str | None:
    override = os.getenv("DiTing_GWS_BIN")
    if override:
        return override
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace API for DiTing Agent")
