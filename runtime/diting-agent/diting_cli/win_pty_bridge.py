"""Windows ConPTY bridge for the `diting dashboard` chat tab.

Drop-in counterpart to ``diting_cli.pty_bridge.PtyBridge`` for native
Windows. Mirrors the exact public surface the ``/api/pty`` WebSocket
handler in ``diting_cli.web_server`` consumes: ``spawn``, ``read``,
