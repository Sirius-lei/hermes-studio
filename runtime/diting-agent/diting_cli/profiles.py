"""
Profile management for multiple isolated DiTing instances.

Each profile is a fully independent DiTing_HOME directory with its own
config.yaml, .env, memory, sessions, skills, gateway, cron, and logs.
Profiles live under ``~/.diting/profiles/<name>/`` by default.

The "default" profile is ``~/.diting`` itself — backward compatible,
zero migration needed.

Usage::

    diting profile create coder          # fresh profile + bundled skills
    diting profile create coder --clone  # also copy config, .env, SOUL.md, skills
    diting profile create coder --clone-all  # full copy of source profile
    coder chat                           # use via wrapper alias
    diting -p coder chat                 # or via flag
    diting profile use coder             # set as sticky default
    diting profile delete coder          # remove profile + alias + service
"""
...
_PROFILE_DIRS = [
    ...
    # See diting_constants.get_subprocess_home().
    "home",
]
...
# default profile (``~/.diting``).  Named profiles never contain these
# directories at root, so the exclusion is gated to avoid silently dropping
# user data from a named-profile source.
#
# Rationale per item:
#   diting-agent  — git repo checkout (~84 MB source + ~3 GB venv)
...
#   backups             — `diting backup` archives
...
# Marker file written by `diting profile create --no-skills`.  When present in
# a profile's root, callers of seed_profile_skills() (fresh-create, `diting
...
# `diting skills install` or drop SKILL.md files into the profile's skills/.
...
def _clone_all_copytree_ignore(source_dir: Path):
    ...
    Three categories:
      ...
      2. Root-level entries in ``_CLONE_ALL_DEFAULT_EXCLUDE_ROOT`` — known
         DiTing infrastructure directories that only the default profile
         (``~/.diting``) ever contains.  Gated on ``source_dir`` actually
    ...
    is_default_source = source_resolved == _get_default_diting_home().resolve()
...
# Directories/files to exclude when exporting the default (~/.diting) profile.
# The default profile contains infrastructure (repo checkout, worktrees, DBs,
# caches, binaries) that named profiles don't have.  We exclude those so the
# export is a portable, reasonable-size archive of actual profile data.
_DEFAULT_EXPORT_EXCLUDE_ROOT = frozenset({
    # Infrastructure
    "diting-agent",         # repo checkout (multi-GB)
    ".worktrees",           # git worktrees
...
# Allow-list for ``export_profile("default")``: when DiTing_HOME equals the
...
# into the export. The set below identifies the *known DiTing profile
# artifacts* at the root of DiTing_HOME; everything else is excluded.
...
# (#58394). Add new artifacts here when introduced in ``diting_constants``.
...
_RESERVED_NAMES = frozenset({
    "diting", "default", "test", "tmp", "root", "sudo",
})

# DiTing subcommands that cannot be used as profile names/aliases
_DiTing_SUBCOMMANDS = frozenset({
...
def _get_profiles_root() -> Path:
    ...
    Anchored to the diting root, NOT to the current DiTing_HOME
    (which may itself be a profile).  This ensures ``coder profile list``
    can see all profiles.

    In Docker/custom deployments where DiTing_HOME points outside
    ``~/.diting``, profiles live under ``DiTing_HOME/profiles/`` so
    they persist on the mounted volume.
    """
    return _get_default_diting_home() / "profiles"


def _get_default_diting_home() -> Path:
    """Return the default (pre-profile) DiTing_HOME path.

    In standard deployments this is ``~/.diting``.
    In Docker/custom deployments where DiTing_HOME is outside ``~/.diting``
    (e.g. ``/opt/data``), returns DiTing_HOME directly.
    """
...
def _get_active_profile_path() -> Path:
    ...
    return _get_default_diting_home() / "active_profile"
...
def validate_profile_name(name: str) -> None:
    ...
    if name in _RESERVED_NAMES:
        raise ValueError(
            ...
            f"the DiTing installation itself or a common system binary.  "
...
def get_profile_dir(name: str) -> Path:
    """Resolve a profile name to its DiTing_HOME directory."""
    canon = normalize_profile_name(name)
    if canon == "default":
        return _get_default_diting_home()
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    Checks: alias-name validity, reserved names, diting subcommands, existing
    binaries in PATH.
    """
    canon = normalize_profile_name(name)
    try:
    ...
    if canon in _DiTing_SUBCOMMANDS:
        return f"'{canon}' conflicts with a diting subcommand"
...
def create_wrapper_script(name: str, target: Optional[str] = None) -> Optional[Path]:
    ...
    if is_windows:
        ...
        try:
            diting_exe = shutil.which("diting") or "diting"
            wrapper_path.write_text(f'#!/bin/sh\nexec {shlex.quote(diting_exe)} -p {profile} "$@"\n')
...
def _migrate_profile_config_if_outdated(profile_dir: Path) -> None:
    ...
    ``_config_version``) or that is simply older than the running DiTing. If we
    ...
    try:
        ...
        from diting_cli.config import check_config_version, migrate_config
        ...
        # not be migrated. The next `diting doctor --fix` can still surface the
...
def find_alias_for_profile(profile_name: str) -> Optional[str]:
    ...
    alias whose body invokes ``diting -p <profile>``. When the alias name equals
    the profile name this is trivial, but a custom alias (``diting profile alias
...
# Real wrappers are a few hundred bytes of shell; the needle (``diting -p X``)
...
def _read_distribution_meta(profile_dir: Path) -> tuple:
    ...
    never break ``diting profile list`` for an unrelated profile.
    """
...
def _check_gateway_running(profile_dir: Path) -> bool:
    ...
    *this* reader — which is exactly the case for a dashboard process that is
    ...
    agree.  Parameterized by ``profile_dir`` so it never mutates ``DiTing_HOME``.
    """
    try:
...
# ``config.yaml``. ``config.yaml`` is the user-facing DiTing config
...
def list_profiles() -> List[ProfileInfo]:
    ...
    # Default profile
    default_home = _get_default_diting_home()
...
def profiles_to_serve(multiplex: bool) -> List[Tuple[str, Path]]:
    """Return the ``(profile_name, diting_home)`` pairs a gateway should serve.
    ...
    - ``multiplex=True``: returns the default profile plus every valid named
      profile under ``profiles/``, each paired with its own DiTing_HOME.
    ...
    The returned ``diting_home`` is the path to pass to
    ``set_diting_home_override`` when scoping a turn to that profile.
    """
    active = get_active_profile_name() or "default"
    if not multiplex:
        return [(active, get_profile_dir(active))]

    serve: List[Tuple[str, Path]] = [("default", _get_default_diting_home())]
...
def create_profile(
    ...
    no_skills:
        ...
        a marker file so ``diting update`` skips re-seeding this profile's
    ...
    if canon == "default":
        raise ValueError(
            "Cannot create a profile named 'default' — it is the built-in profile (~/.diting)."
        )
    ...
    if clone_all and source_dir:
        # Full copy of source profile (exclude sibling ~/.diting/profiles/)
        shutil.copytree(
            source_dir,
            profile_dir,
            symlinks=True,
    ...
    # Keys pages, `diting -p <name> auth add`) had no file until first
    ...
    if not env_path.exists():
        try:
            env_path.write_text(
                "# Per-profile secrets for this DiTing profile.\n"
    ...
    if not soul_path.exists():
        try:
            from diting_cli.default_soul import DEFAULT_SOUL_MD
            soul_path.write_text(DEFAULT_SOUL_MD, encoding="utf-8")
        except Exception:
            pass  # best-effort — don't fail profile creation over this

    # Write the opt-out marker so seed_profile_skills() and `diting update`'s
    ...
    if no_skills:
        try:
            (profile_dir / NO_BUNDLED_SKILLS_MARKER).write_text(
                ...
                "(`diting profile create --no-skills`).\n"
    ...
    # Cloned configs can be older than the running DiTing (or predate schema
    ...
    if description and description.strip():
        try:
            ...
            pass  # non-fatal — user can describe later with `diting profile describe`

    # Phase 4: when running inside a container under s6, register the
    # new profile's gateway as a runtime s6 service so
    # `diting -p <profile> gateway start` can supervise it via
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Uses subprocess because sync_skills() caches DiTing_HOME at module level.
    Returns the sync result dict, or None on failure.

    Profiles that opted out of bundled skills (via ``diting profile create
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "DiTing_HOME": str(profile_dir)},
...
def backfill_profile_envs(quiet: bool = False) -> List[str]:
    ...
    default_env = _get_default_diting_home() / ".env"
...
def _profile_bound_backend_pids(canon: str, profile_dir: Path) -> list[int]:
    """PIDs of running DiTing *backends* bound to this profile.
    ...
    names it, so find it by inspection: a DiTing backend subcommand
    (``serve``/``dashboard``/``gateway``) that is bound to *this* profile either
    by a ``--profile <canon>`` / ``-p <canon>`` selector or by a ``DiTing_HOME``
    ...
    # Never terminate ourselves or a parent (e.g. `diting -p <canon> profile
    ...
    diting_markers = ("diting_cli.main", "diting-gateway", "tui_gateway")
    pids: list[int] = []
    ...
    for proc in psutil.process_iter(["pid", "name", "username", "cmdline"]):
        try:
            ...
            # Must be a DiTing process: either an entrypoint marker in argv, or
            # a resolved executable named `diting`.
            joined = " ".join(argv)
            exe_name = os.path.basename(argv[0]).lower()
            is_diting = (
                any(marker in joined for marker in diting_markers)
                or exe_name == "diting"
                or exe_name.startswith("diting")
            )
            if not is_diting:
                continue
            ...
            # ...or by DiTing_HOME env pointing at this profile dir.
            if not bound:
                try:
                    env_home = (proc.environ() or {}).get("DiTing_HOME", "")
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if canon == "default":
        raise ValueError(
            "Cannot delete the default profile (~/.diting).\n"
            "To remove everything, use: diting uninstall"
        )
    ...
    # the rmtree below fail with ENOTEMPTY and — before the ensure_diting_home
...
def _maybe_register_gateway_service(profile_name: str) -> None:
    ...
    ``DiTing_HOME`` and binds the port resolved by ``gateway/config.py``
    from that profile's environment — ``API_SERVER_PORT`` (or
    ``platforms.api_server.extra.port`` in the profile's
    ``config.yaml``), defaulting to 18642. There is no ``[gateway] port``
    ...
    bind 18642 — give each profile a distinct ``API_SERVER_PORT`` in its
    ``.env``.
    ...
    try:
        ...
        from diting_cli.service_manager import get_service_manager
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # Temporarily set DiTing_HOME so _profile_suffix resolves correctly
    old_home = os.environ.get("DiTing_HOME")
    try:
        os.environ["DiTing_HOME"] = str(profile_dir)
        from diting_cli.gateway import get_service_name, get_launchd_plist_path
        ...
        if old_home is not None:
            os.environ["DiTing_HOME"] = old_home
        elif "DiTing_HOME" in os.environ:
            del os.environ["DiTing_HOME"]
...
def set_active_profile(name: str) -> None:
    ...
    Writes to ``~/.diting/active_profile``. Use ``"default"`` to clear.
    """
    canon = normalize_profile_name(name)
    validate_profile_name(canon)
    if canon != "default" and not profile_exists(canon):
        raise FileNotFoundError(
            ...
            f"Create it with: diting profile create {canon}"
        )
...
def get_active_profile_name() -> str:
    """Infer the current profile name from DiTing_HOME.

    Returns ``"default"`` if DiTing_HOME is not set or points to ``~/.diting``.
    Returns the profile name if DiTing_HOME points into ``~/.diting/profiles/<name>``.
    Returns ``"custom"`` if DiTing_HOME is set to an unrecognized path.
    """
    from diting_constants import get_diting_home
    diting_home = get_diting_home()
    resolved = diting_home.resolve()

    default_resolved = _get_default_diting_home().resolve()
...
def _default_export_ignore(root_dir: Path):
    ...
    * **Root-level allow-list** — only entries whose name appears in
      ...
      DiTing_HOME equals the cwd) is excluded. Blacklisting was tried
      first and proved unable to anticipate every non-DiTing file the
      user may have lying alongside DiTing_HOME (#58394).
...
def export_profile(name: str, output_path: str) -> Path:
    ...
    if canon == "default":
        # The default profile IS ~/.diting itself — its parent is ~/ and its
        # directory name is ".diting", not "default".  We stage a clean copy
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    if not inferred_name:
        raise ValueError(
            ...
            "Specify it explicitly: diting profile import <archive> --name <name>"
        )
    ...
    # dir.  Importing as "default" would target ~/.diting itself — disallow
    # that and guide the user toward a named profile.
    canon = normalize_profile_name(inferred_name)
    validate_profile_name(canon)
    if canon == "default":
        raise ValueError(
            "Cannot import as 'default' — that is the built-in root profile (~/.diting). "
            "Specify a different name: diting profile import <archive> --name <name>"
        )
    ...
    with tempfile.TemporaryDirectory(prefix="diting_profile_import_") as tmpdir:
...
def _migrate_honcho_profile_host(old_name: str, new_name: str, new_dir: Path) -> None:
    ...
    old_host = f"diting_{old_name}"
    legacy_old_host = f"diting.{old_name}"
    new_host = f"diting_{new_name}"

    candidates = [
        new_dir / "honcho.json",
        _get_default_diting_home() / "honcho.json",
    ...
    for path in candidates:
        ...
        if isinstance(block, dict) and "aiPeer" not in block:
            if source_host.startswith("diting_"):
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a DiTing_HOME path string.

    Called early in the CLI entry point, before any diting modules
    are imported, to set the DiTing_HOME environment variable.
    """
