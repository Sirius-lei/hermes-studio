...
Profile-aware location: ``$DiTing_HOME/logs/dashboard-auth.log``.
...
from ``diting_constants`` or other diting_cli modules — so it can be
...
def _resolve_log_path() -> Path:
    """``$DiTing_HOME/logs/dashboard-auth.log`` with the standard fallback.

    Mirrors ``diting_constants.get_diting_home`` semantics: env var wins,
    else ``~/.diting``. A local copy avoids an import cycle with the
    middleware which lives below ``diting_cli``.
    """
    home = os.environ.get("DiTing_HOME") or str(Path.home() / ".diting")
