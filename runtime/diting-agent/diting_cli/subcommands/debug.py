"""``diting debug`` subcommand parser.
...
def build_debug_parser(subparsers, *, cmd_debug: Callable) -> None:
    ...
    debug_parser = subparsers.add_parser(
        ...
        description="Debug utilities for DiTing Agent. Use 'diting debug share' to "
        "upload a debug report (system info + recent logs) to a paste "
        "service and get a shareable URL.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    diting debug share              Upload debug report (asks for confirmation)
    diting debug share --yes        Skip confirmation (for scripts/CI)
    diting debug share --lines 500  Include more log lines
    diting debug share --expire 30  Keep paste for 30 days
    diting debug share --local      Print report locally (no upload)
    diting debug share --no-redact  Disable upload-time secret redaction
    diting debug share --nous       Upload to Nous-internal storage (private)
    diting debug delete <url>       Delete a previously uploaded paste
""",
    )
    ...
    delete_parser = debug_sub.add_parser(
        "delete",
        help="Delete a paste uploaded by 'diting debug share'",
    )
