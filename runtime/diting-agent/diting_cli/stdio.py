...
on Japanese locales, etc.).  DiTing's banners, tool output feed, and slash
...
Entry points (``cli.py`` ``main``, ``diting_cli/main.py`` CLI dispatch,
...
def _flip_console_code_page_to_utf8() -> None:
    ...
    is silent — if there's no attached console (e.g. DiTing is running
...
def configure_windows_stdio() -> bool:
    ...
    Set ``DiTing_DISABLE_WINDOWS_UTF8=1`` in the environment to opt out
    ...
    if os.environ.get("DiTing_DISABLE_WINDOWS_UTF8") in {"1", "true", "True", "yes"}:
        _CONFIGURED = True
        return False
    ...
    # Augment PATH with the DiTing-managed Git install directories so
    ...
    # already-running shells don't see the change — which means diting
    ...
    # install without DiTing-managed PortableGit).
    ...
    # stdin is re-configured for completeness; DiTing's interactive
...
def _default_windows_editor() -> str:
    ...
    The prompt_toolkit buffer's ``open_in_editor`` and DiTing's
    ``diting config edit`` both honour ``$EDITOR``.  Users who prefer a
    ...
    Set this before launching DiTing (User env var in Windows Settings, or
    export in a PowerShell profile) and DiTing picks it up automatically.
    """
    import shutil
...
def _augment_path_with_known_tools() -> None:
    """Prepend well-known DiTing-managed tool directories to os.environ['PATH'].
    ...
    ``%LOCALAPPDATA%\\diting\\git\\bin`` to the User PATH via
    ...
    *spawned* processes only — already-running shells (including the one the
    user invokes ``diting`` from right after install) retain their old PATH.

    Any subprocess DiTing spawns — bash, ``rg``, ``grep``, ``npm`` — inherits
    ...
    Patch-up strategy: add the known DiTing-managed tool directories to our
    ...
    # see on next launch.
    candidate_dirs = [
        os.path.join(local_appdata, "diting", "git", "cmd"),
        os.path.join(local_appdata, "diting", "git", "bin"),
        os.path.join(local_appdata, "diting", "git", "usr", "bin"),
        # DiTing venv Scripts directory — host of the diting.exe shim itself,
        # also where any pip-installed console scripts land.  Usually already
        # on PATH when the user invokes diting, but harmless to include.
        os.path.join(local_appdata, "diting", "diting-agent", "venv", "Scripts"),
