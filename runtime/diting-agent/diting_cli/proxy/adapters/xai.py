...
# xAI's public API is OpenAI-compatible for the endpoints DiTing commonly
# uses. The Responses endpoint is included because DiTing' native xAI runtime
...
class XAIGrokAdapter(UpstreamAdapter):
    """Proxy upstream for xAI Grok via DiTing-managed OAuth credentials."""

    auth_hint = "diting auth add xai-oauth --type oauth"
        ...
        with self._lock:
            ...
            if pool is None or not pool.has_credentials():
                raise RuntimeError(
                    ...
                    "`diting auth add xai-oauth --type oauth` first."
                )
            ...
            if entry is None:
                raise RuntimeError(
                    ...
                    "`diting auth reset xai-oauth` or re-authenticate with "
                    "`diting auth add xai-oauth --type oauth`."
                )
    ...
    def _credential_from_entry(self, entry: PooledCredential) -> UpstreamCredential:
        ...
        if not bearer:
            raise RuntimeError(
                ...
                "Re-authenticate with `diting auth add xai-oauth --type oauth`."
            )
