"""diting hooks — inspect and manage shell-script hooks.

Usage::

    diting hooks list
    diting hooks test <event> [--for-tool X] [--payload-file F]
    diting hooks revoke <command>
    diting hooks doctor

Consent records live under ``~/.diting/shell-hooks-allowlist.json`` and
hook definitions come from the ``hooks:`` block in ``~/.diting/config.yaml``
...
def hooks_command(args) -> None:
    """Entry point for ``diting hooks`` — dispatches to the requested action."""
    sub = getattr(args, "hooks_action", None)

    if not sub:
        print("Usage: diting hooks {list|test|revoke|doctor}")
        print("Run 'diting hooks --help' for details.")
        return
...
def _cmd_list(_args) -> None:
    ...
    if not specs:
        print("No shell hooks configured in ~/.diting/config.yaml.")
        print("See `diting hooks --help` or")
                ...
                if entry and entry.get("approved_at"):
                    ...
                    if mtime_now and mtime_at and mtime_now > mtime_at:
                        print(
                            ...
                            f"run `diting hooks doctor` to re-validate"
                        )
        print()
...
# stdin a script sees under `diting hooks test` and `diting hooks doctor`
...
def _print_run_result(result: Dict[str, Any]) -> None:
    ...
    if parsed:
        print(f"      parsed (DiTing wire shape): {json.dumps(parsed)}")
    else:
...
def _doctor_one(spec, shell_hooks) -> int:
    ...
    if entry and entry.get("script_mtime_at_approval"):
        ...
        if mtime_now and mtime_at and mtime_now > mtime_at:
            ...
            print(f"      ⚠ script modified since approval "
                  ...
                  f"then `diting hooks revoke` + re-approve to refresh")
    ...
    # is already allowlisted.  Otherwise `diting hooks doctor` would execute
    ...
    if not entry:
        print("      ℹ skipped JSON smoke test — not allowlisted yet. "
              ...
              "then re-run `diting hooks doctor`.")
