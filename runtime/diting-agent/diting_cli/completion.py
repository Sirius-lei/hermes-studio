"""Shell completion script generation for diting CLI.
...
def generate_bash(parser: argparse.ArgumentParser) -> str:
    ...
    for cmd in sorted(tree["subcommands"]):
        ...
        if cmd == "profile" and info["subcommands"]:
            ...
            cases.append(
                ...
                f"                    COMPREPLY=($(compgen -W \"$(_diting_profiles)\" -- \"$cur\"))\n"
                f"                    return\n"
                f"                    ;;\n"
                f"            esac\n"
                f"            ;;"
    ...
    return f"""# DiTing Agent bash completion
# Add to ~/.bashrc:
#   eval "$(diting completion bash)"

_diting_profiles() {{
    local profiles_dir="$HOME/.diting/profiles"
...
_diting_completion() {{
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_diting_profiles)" -- "$cur"))
        return
    fi
...
complete -F _diting_completion diting
"""
...
def generate_zsh(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""#compdef diting
# DiTing Agent zsh completion
# Add to ~/.zshrc:
...
_diting_profiles() {{
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.diting/profiles" ]]; then
        profiles+=($HOME/.diting/profiles/*(N/:t))
    fi
...
_diting() {{
    ...
    _arguments -C \\
        ...
        '(-)'{{-p,--profile}}'[Profile name]:profile:_diting_profiles' \\
        '1:command:->commands' \\
        '*::arg:->args'
...
{top_cmds_str}
            ...
            _describe 'diting command' subcmds
            ;;
        args)
            case ${{line[1]}} in
{sub_cases_str}
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        "# DiTing Agent fish completion",
        "# Add to your config:",
        "#   diting completion fish | source",
        "",
        "# Helper: list available profiles",
        "function __diting_profiles",
        "    echo default",
        "    if test -d $HOME/.diting/profiles",
        "        for d in $HOME/.diting/profiles/*/",
        "            basename $d",
        "        end",
        "    end",
        "end",
        ...
        "",
        "# Complete profile names after -p / --profile",
        "complete -c diting -f -s p -l profile"
        " -d 'Profile name' -xa '(__diting_profiles)'",
        "",
        ...
        if cmd == "profile":
            for action in sorted(profile_name_actions):
                lines.append(
                    ...
                    f"-a '(__diting_profiles)' -d 'Profile name'"
                )
