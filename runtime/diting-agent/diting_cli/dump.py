"""
Dump command for diting CLI.

Outputs a compact, plain-text summary of the user's DiTing setup
...
from diting_cli.config import get_diting_home, get_env_path, get_project_root, load_config
...
def _dotenv_key_names() -> set[str]:
    """Return the set of env-var names assigned a non-empty value in ~/.diting/.env.
    ...
    exports. ``diting debug share`` runs in a terminal, so ``os.getenv`` reflects
...
def _get_git_commit(project_root: Path) -> str:
    ...
    to ``<project_root>/.diting_build_sha`` by the Dockerfile's
    ``DiTing_GIT_SHA`` build-arg (see ``diting_cli/build_info.py``).
...
def _redact(value: str) -> str:
    ...
    ``diting dump`` formats empty values as blank, not as ``"(not set)"``).
    """
...
def _gateway_status() -> str:
    ...
    try:
        from diting_cli.gateway import get_gateway_runtime_snapshot
...
def _count_skills(diting_home: Path) -> int:
...
def _cron_summary(diting_home: Path) -> str:
    """Return cron jobs summary."""
    jobs_file = diting_home / "cron" / "jobs.json"
...
def run_dump(args):
    ...
    lines.append(f"diting_home:      {display_diting_home()}")
    ...
    for env_var, label in api_keys:
        ...
        # Set in this (shell) process but absent from ~/.diting/.env: a managed
        ...
        # A credential added via `diting auth add openrouter` lives in the
        # credential pool, not as an env var — surface it so the dump doesn't
        # misleadingly read "not set" while `diting auth list` shows it (#42130).
    ...
    toolsets = config.get("toolsets", ["diting-cli"])
    ...
    lines.append(f"  cron_jobs:          {_cron_summary(diting_home)}")
    lines.append(f"  skills:             {_count_skills(diting_home)}")
