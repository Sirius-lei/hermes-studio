"""
DiTing Desktop (Chat GUI) uninstaller.
...
the Python agent or the user's config/data:
  ...
  1. Source-built GUI (``diting desktop`` / ``diting gui``)
     Built inside the agent checkout under ``$DiTing_HOME/diting-agent/``:
       ...
       - ``$DiTing_HOME/desktop-build-stamp.json`` (the build freshness stamp)
  ...
  2. Packaged distributable (DMG / NSIS / AppImage / deb / rpm)
     ...
     own bundled Electron + a per-user Electron ``userData`` directory:
       - macOS:   ``/Applications/DiTing.app`` or ``~/Applications/DiTing.app``
       - Windows: ``%LOCALAPPDATA%\\Programs\\DiTing`` (NSIS per-user)
...
the app name ("DiTing"), separate from ``$DiTing_HOME``:
  - macOS:   ``~/Library/Application Support/DiTing``
  - Windows: ``%APPDATA%\\DiTing``
  - Linux:   ``$XDG_CONFIG_HOME/DiTing`` (default ``~/.config/DiTing``)
...
``diting uninstall --gui`` (and friends) without paying for the full CLI.
"""
...
def _agent_root(diting_home: Path) -> Path:
    """The agent checkout root — same layout install.sh / install.ps1 use."""
    return diting_home / "diting-agent"


def desktop_userdata_dir() -> Path:
    """Return the Electron ``userData`` directory for the desktop app.

    Mirrors Electron's ``app.getPath('userData')`` for an app named "DiTing"
    ...
    if sys.platform == "darwin":
        return home / "Library" / "Application Support" / "DiTing"
...
def source_built_gui_artifacts(diting_home: Path) -> "list[Path]":
    """GUI build artifacts produced by ``diting desktop`` inside the checkout.

    These are removable on a GUI uninstall without harming the agent: the
    Python agent runs from ``diting-agent/`` source + ``venv/`` and never
    needs the Electron build output or node_modules.
    """
    agent_root = _agent_root(diting_home)
    ...
    return [
        ...
        diting_home / "desktop-build-stamp.json",
    ]
...
def packaged_gui_app_paths() -> "list[Path]":
    ...
    electron-builder output locations for the "DiTing" product.
    """
    home = Path.home()
    paths: list[Path] = []
    if sys.platform == "darwin":
        paths += [
            Path("/Applications/DiTing.app"),
            home / "Applications" / "DiTing.app",
        ]
    elif sys.platform == "win32":
        local = os.environ.get("LOCALAPPDATA")
        local_base = Path(local) if local else (home / "AppData" / "Local")
        paths += [
            # NSIS per-user install (perMachine=false → Programs\DiTing).
            local_base / "Programs" / "DiTing",
            # Older / alternate layout some builds used.
            local_base / "diting-desktop",
        ]
        program_files = os.environ.get("ProgramFiles")
        if program_files:
            # NSIS per-machine fallback (needs admin to remove).
            paths.append(Path(program_files) / "DiTing")
    else:
        ...
        paths += [
            data_base / "applications" / "diting.desktop",
            data_base / "applications" / "DiTing.desktop",
        ]
    return paths


def agent_is_installed(diting_home: Path) -> bool:
    """Return True when a usable Python agent install exists under DiTing_HOME.
    ...
    if (agent_root / "diting_cli").is_dir():
        return True
...
def gui_is_installed(diting_home: Path) -> bool:
    """Return True when any desktop GUI artifact exists (built or packaged)."""
    for p in source_built_gui_artifacts(diting_home):
        if p.exists():
            return True
...
def gui_install_summary(diting_home: "Path | None" = None) -> dict:
    ...
    """
    home: Path = diting_home if diting_home is not None else get_diting_home()
...
def uninstall_gui(diting_home: "Path | None" = None, *, remove_userdata: bool = True) -> "list[Path]":
    ...
    Never touches ``diting-agent/diting_cli`` (agent source), ``venv/``, or any
    config / sessions / .env under ``$DiTing_HOME``.
    ...
    if sys.platform.startswith("linux"):
        log_info(
            ...
            "with your package manager (e.g. 'sudo apt remove diting' or "
            "'sudo dnf remove diting'). AppImage builds are a single file you "
