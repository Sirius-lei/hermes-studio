...
Used by ``diting profile describe <name> --auto`` and the dashboard's
...
Design notes
------------
- Mirrors the shape of ``diting_cli/kanban_specify.py``: lazy aux
...
_SYSTEM_PROMPT = """You are a profile-describer for the DiTing Agent kanban board.
...
Rules:
  ...
  - Never write "DiTing Agent profile" or other meta-narration.
    ...
    *,
    ...
    try:
        if canon == "default":
            ...
            profile_dir = Path(get_diting_home())
        else:
