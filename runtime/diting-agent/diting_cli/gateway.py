"""
Gateway subcommand for diting CLI.

Handles: diting gateway [run|start|stop|restart|status|install|uninstall|setup]
"""
...
# display_diting_home is imported lazily at call sites to avoid ImportError
# when diting_constants is cached from a pre-update version during `diting update`.
...
def _get_parent_pid(pid: int) -> int | None:
    ...
    "diting CLI that invoked this scan" from "real gateway process".
    """
    if pid <= 1:
        return None
    try:
...
def _get_ancestor_pids() -> set[int]:
    ...
    ``diting gateway status`` from falsely counting the ``diting`` CLI that
...
def _scan_gateway_pids(
    ...
    # scan (e.g. ``diting gateway status``) is never mistaken for a running
    # gateway.  See #13242.
    ...
    current_home = str(get_diting_home().resolve())
    ...
    def _matches_current_profile(command: str) -> bool:
        ...
        if current_profile_name:
            return (
                ...
                or f"diting_home={current_home_lc}" in command_lc
            )

        # Default-profile case: no profile flag in argv. Accept as long as
        # the command doesn't advertise *some other* profile. DiTing_HOME
        ...
        # DiTing_HOME= in argv is.
        if "--profile " in command_lc or " -p " in command_lc:
            return False
        if (
            "diting_home=" in command_lc
            and f"diting_home={current_home_lc}" not in command_lc
        ):
            return False
        return True
...
def find_gateway_pids(
    ...
    Args:
        ...
        all_profiles: When ``True``, return gateway PIDs across **all**
            profiles (the pre-7923 global behaviour).  ``diting update``
            needs this because a code update affects every profile.
            When ``False`` (default), only PIDs belonging to the current
            DiTing profile are returned.
    """
...
def find_profile_gateway_processes(
    ...
    """Return running gateway PIDs mapped to DiTing profiles via PID files."""
    _exclude = set(exclude_pids or set())
    processes: list[ProfileGatewayProcess] = []
    try:
        from gateway.status import get_running_pid
        from diting_cli.profiles import list_profiles
    except Exception:
        return processes
...
def _gateway_run_args_for_profile(profile: str) -> list[str]:
    args = [get_python_path(), "-m", "diting_cli.main"]
    if profile != "default":
...
def _capture_gateway_argv(pid: int) -> list[str] | None:
    ...
    Windows Scheduled Task running ``pythonw.exe -m diting_cli.main gateway
...
def launch_detached_gateway_restart_by_cmdline(
    ...
    ``gateway run`` whose DiTing_HOME or argv doesn't match a known profile).
...
def _spawn_gateway_restart_watcher(old_pid: int, run_argv: list[str]) -> bool:
    ...
    # when the user closes the terminal, leaving ``diting update`` users
    # with no running gateway until they re-invoke ``diting gateway``
    ...
    from diting_cli._subprocess_compat import (
    ...
    if sys.platform == "win32":
        try:
            from diting_cli.gateway_windows import (
    ...
    watcher = textwrap.dedent(
        ...
        # the env (VIRTUAL_ENV / PYTHONPATH / DiTing_HOME) the windowless
        # base interpreter needs to import diting_cli.  Empty on POSIX, where
...
def _diting_home_from_systemd_unit_file(system: bool = False) -> str | None:
    """Read ``DiTing_HOME`` from the on-disk unit file (not ``systemctl show``).
    ...
    for line in text.splitlines():
        ...
        if body.startswith("DiTing_HOME="):
            value = body.split("=", 1)[1].strip().strip('"')
            return value or None
    return None


def _sync_diting_home_from_systemd_unit(system: bool) -> None:
    """When acting on a system-scope unit, adopt its ``DiTing_HOME``.

    Under ``sudo``, ``DiTing_HOME`` is stripped and ``HOME=/root``, so
    :func:`get_diting_home` falls back to ``/root/.diting`` — the wrong
    profile. The unit file pins ``DiTing_HOME`` for the actual gateway
    ...
    unit_home = (_diting_home_from_systemd_unit_file(system=True) or "").strip()
    if not unit_home:
        unit_home = _read_systemd_unit_environment(system=True).get("DiTing_HOME", "").strip()
    if not unit_home:
        return
    current = os.environ.get("DiTing_HOME", "").strip()
    if current == unit_home:
        return
    os.environ["DiTing_HOME"] = unit_home
...
def _wait_for_systemd_service_restart(
    *,
    ...
    print(
        ...
        f"  Check status: {'sudo ' if system else ''}diting gateway status\n"
...
def _print_systemd_start_limit_wait(system: bool = False) -> None:
    ...
    print(
        f"  Wait for the start-limit window to expire, then run: {'sudo ' if system else ''}diting gateway restart{scope_flag}"
    )
...
def get_gateway_runtime_snapshot(system: bool = False) -> GatewayRuntimeSnapshot:
    ...
    if is_linux() and is_container():
        ...
        try:
            from diting_cli.service_manager import detect_service_manager, get_service_manager
...
def _print_gateway_process_mismatch(snapshot: GatewayRuntimeSnapshot) -> None:
    ...
    if _launchd_unsupported_marker_exists():
        ...
        print("  Stop it with: diting gateway stop")
    else:
        print(
            "⚠ Gateway process is running for this profile, but the service is not active"
        )
        print(f"  PID(s): {_format_gateway_pids(snapshot.gateway_pids, limit=None)}")
        print("  This is usually a manual foreground/tmux/nohup run, so `diting gateway`")
...
def _print_other_profiles_gateway_status() -> None:
    ...
    Shown at the bottom of ``diting gateway status`` output so users with
...
def _gateway_list() -> None:
    ...
    """
    try:
        from diting_cli.profiles import list_profiles, get_active_profile_name
    except Exception:
...
def _reap_unsupervised_gateway_orphans() -> bool:
    ...
    in-process under a ``gateway restart`` argv (diting_cli/gateway.py restart
...
def stop_profile_gateway() -> bool:
    """Stop only the gateway for the current profile (DiTing_HOME-scoped).
...
from diting_constants import is_container, is_termux, is_wsl
...
def _container_systemd_operational() -> bool:
    ...
    This is NOT our DiTing Docker image — that one runs s6-overlay as
...
def _windows_gateway_should_absorb_console_controls() -> bool:
    ...
    Foreground ``diting gateway run`` must remain interruptible from
    PowerShell/CMD. Detached service-style launches opt in via
    ``DiTing_GATEWAY_DETACHED=1``; older wrappers without the env marker are
    treated as detached when no interactive stdin is attached.
    """
    if not is_windows():
        return False

    detached = os.getenv("DiTing_GATEWAY_DETACHED", "").strip().lower()
...
_SERVICE_BASE = "diting-gateway"
SERVICE_DESCRIPTION = "DiTing Agent Gateway - Messaging Platform Integration"


def _profile_suffix() -> str:
    """Derive a service-name suffix from the current DiTing_HOME.

    Returns ``""`` for the default root, the profile name for
    ``<root>/profiles/<name>``, or a short hash for any other path.
    Works correctly in Docker (DiTing_HOME=/opt/data) and standard deployments.
    """
    import hashlib
    import re
    from diting_constants import get_default_diting_root

    home = get_diting_home().resolve()
    default = get_default_diting_root().resolve()
    if home == default:
        return ""
    ...
    try:
        ...
        pass
    # Fallback: short hash for arbitrary DiTing_HOME paths
    return hashlib.sha256(str(home).encode()).hexdigest()[:8]


def _profile_arg(diting_home: str | None = None, default_root: str | Path | None = None) -> str:
    """Return ``--profile <name>`` only when DiTing_HOME is a named profile.

    For ``~/.diting/profiles/<name>``, returns ``"--profile <name>"``.
    For the default profile or hash-based custom paths, returns the empty string.

    Args:
        diting_home: Optional explicit DiTing_HOME path. Defaults to the current
            ``get_diting_home()`` value. Should be passed when generating a
            service definition for a different user (e.g. system service).
        default_root: Optional DiTing root to compare against. Used when
            generating a system service for another user from a sudo/root
            process, where ``Path.home()`` and ``get_default_diting_root()``
            refer to root but the target profile lives under the service user.
    """
    import re
    from diting_constants import get_default_diting_root

    home = Path(diting_home or str(get_diting_home())).resolve()
    default = Path(default_root).resolve() if default_root else get_default_diting_root().resolve()
    if home == default:
        return ""
    profiles_root = (default / "profiles").resolve()
    try:
...
def _profile_arg_for_target_user(diting_home: str, target_home_dir: str) -> str:
    """Return the profile arg for a system service running as another user."""
    target_root = Path(target_home_dir) / ".diting"
    try:
        Path(diting_home).resolve().relative_to(target_root.resolve())
        return _profile_arg(diting_home, default_root=target_root)
    except ValueError:
        return _profile_arg(diting_home)


def get_service_name() -> str:
    """Derive a systemd service name scoped to this DiTing_HOME.

    Default ``~/.diting`` returns ``diting-gateway`` (backward compatible).
    Profile ``~/.diting/profiles/coder`` returns ``diting-gateway-coder``.
    Any other DiTing_HOME appends a short hash for uniqueness.
    """
...
# Legacy service names from older DiTing installs that predate the
# diting-gateway rename. Kept as an explicit allowlist (NOT a glob) so
# profile units (diting-gateway-*.service) and unrelated third-party
# "diting" units are never matched.
_LEGACY_SERVICE_NAMES: tuple[str, ...] = ("diting.service",)

# ExecStart content markers that identify a unit as running our gateway.
# A legacy unit is only flagged when its file contains one of these.
_LEGACY_UNIT_EXECSTART_MARKERS: tuple[str, ...] = (
    "diting_cli.main gateway",
    "diting_cli/main.py gateway",
    "gateway/run.py",
    " diting gateway ",
    "/diting gateway ",
)
...
def _find_legacy_diting_units() -> list[tuple[str, Path, bool]]:
    """Return ``[(unit_name, unit_path, is_system)]`` for legacy DiTing gateway units.

    Detects unit files installed by older DiTing versions that used a
    different service name (e.g. ``diting.service`` before the rename to
    ``diting-gateway.service``). When both a legacy unit and the current
    ``diting-gateway.service`` are active, they fight over the same bot
    ...
    * Explicit allowlist of legacy names (no globbing). Profile units such
      as ``diting-gateway-coder.service`` and unrelated third-party
      ``diting-*`` services are never matched.
    * ExecStart content check — only flag units that invoke our gateway
      entrypoint. A user-created ``diting.service`` running an unrelated
...
def has_legacy_diting_units() -> bool:
    """Return True when any legacy DiTing gateway unit files exist."""
    return bool(_find_legacy_diting_units())


def print_legacy_unit_warning() -> None:
    """Warn about legacy DiTing gateway unit files if any are installed.

    Idempotent: prints nothing when no legacy units are detected. Safe to
    call from any status/install/setup path.
    """
    legacy = _find_legacy_diting_units()
    if not legacy:
        return
    print_warning("Legacy DiTing gateway unit(s) detected from an older install:")
    for name, path, is_system in legacy:
        scope = "system" if is_system else "user"
        print_info(f"    {path}  ({scope} scope)")
    print_info("  These run alongside the current diting-gateway service and")
    print_info("  cause SIGTERM flap loops — both try to use the same bot token.")
    print_info("  Remove them with:")
    print_info("    diting gateway migrate-legacy")


def remove_legacy_diting_units(
    interactive: bool = True,
    dry_run: bool = False,
) -> tuple[int, list[Path]]:
    """Stop, disable, and remove legacy DiTing gateway unit files.

    Iterates over whatever ``_find_legacy_diting_units()`` returns — which is
    ...
    legacy = _find_legacy_diting_units()
    if not legacy:
        print("No legacy DiTing gateway units found.")
        return 0, []
    ...
    print()
    print("Legacy DiTing gateway unit(s) found:")
    for name, path, is_system in legacy:
        scope = "system" if is_system else "user"
    ...
    if interactive and not prompt_yes_no("Remove these legacy units?", True):
        print("Skipped. Run again with: diting gateway migrate-legacy")
        return 0, [p for _, p, _ in legacy]
    ...
    if system_units:
        if os.geteuid() != 0:  # windows-footgun: ok — Linux systemd removal path, guarded by `if system == "Linux"` / systemd-only branch
            ...
            print_info("  Re-run with: sudo diting gateway migrate-legacy")
...
def print_systemd_scope_conflict_warning() -> None:
    ...
    print_info("    diting gateway uninstall")
    print_info("    sudo diting gateway uninstall --system")
    ...
    if scope == "system":
        ...
        if os.geteuid() != 0:  # windows-footgun: ok — Linux systemd install wizard, never invoked on Windows
            ...
            print_warning(
                ...
                "root shell, or install a user service instead: diting gateway install"
            )
            return scope, False
...
def _launchd_user_home() -> Path:
    ...
    Profile-mode DiTing often sets ``HOME`` to a profile-scoped directory, but
...
def get_launchd_plist_path() -> Path:
    ...
    Default ``~/.diting`` → ``ai.diting.gateway.plist`` (backward compatible).
    Profile ``~/.diting/profiles/coder`` → ``ai.diting.gateway-coder.plist``.
    """
    suffix = _profile_suffix()
    name = f"ai.diting.gateway-{suffix}" if suffix else "ai.diting.gateway"
...
def _remap_path_for_user(path: str, target_home_dir: str) -> str:
    ...
    to *target_home_dir*; otherwise the path is returned unchanged.
      ...
      /root/.diting/diting-agent  -> /home/alice/.diting/diting-agent
      /opt/diting                 -> /opt/diting  (kept as-is)
...
def _diting_home_for_target_user(target_home_dir: str) -> str:
    """Remap the current DiTing_HOME to the equivalent under a target user's home.

    When installing a system service via sudo, get_diting_home() resolves to
    root's home.  This translates it to the target user's equivalent path:
      /root/.diting                    → /home/alice/.diting
      /root/.diting/profiles/coder     → /home/alice/.diting/profiles/coder
      /opt/custom-diting               → /opt/custom-diting  (kept as-is)
    """
    current_diting = get_diting_home().resolve()
    current_default = (Path.home() / ".diting").resolve()
    target_default = Path(target_home_dir) / ".diting"

    # Default ~/.diting → remap to target user's default
    if current_diting == current_default:
        return str(target_default)

    # Profile or subdir of ~/.diting → preserve the relative structure
    try:
        relative = current_diting.relative_to(current_default)
        return str(target_default / relative)
    except ValueError:
        # Completely custom path (not under ~/.diting) — keep as-is
        return str(current_diting)
...
def _build_service_path_dirs(project_root: Path | None = None) -> list[str]:
    ...
    diting_node = diting_home / "node" / "bin"
    if _is_dir(diting_node):
        candidates.append(str(diting_node))
    diting_nm = diting_home / "node_modules" / ".bin"
    if _is_dir(diting_nm):
        candidates.append(str(diting_nm))
...
def _stable_service_working_dir() -> str:
    ...
    uses an absolute python interpreter and ``-m diting_cli.main``, so module
    ...
    dir, or a clone that ``diting update`` later relocates/removes — the path
    ...
    ``DiTing_HOME`` is the stable anchor: it is where config/state/logs live,
    it never moves, and it is guaranteed to exist whenever the gateway is
    meaningfully installed. Fall back to ``PROJECT_ROOT`` only if DiTing_HOME
...
def generate_systemd_unit(system: bool = False, run_as_user: str | None = None) -> str:
    ...
    if system:
        ...
        diting_home = _diting_home_for_target_user(home_dir)
        profile_arg = _profile_arg_for_target_user(diting_home, home_dir)
        # Remap all paths that may resolve under the calling user's home
        # (e.g. /root/) to the target user's home so the service can
        # actually access them.
        python_path = _remap_path_for_user(python_path, home_dir)
        # Anchor cwd to the target user's DiTing_HOME (stable, always exists)
        # rather than a remapped source-checkout path that can rot. See
        # _stable_service_working_dir() for the full rationale.
        working_dir = str(diting_home) if diting_home else _remap_path_for_user(working_dir, home_dir)
...
User={username}
Group={group_name}
ExecStart={python_path} -m diting_cli.main{f" {profile_arg}" if profile_arg else ""} gateway run
...
Environment="DiTing_HOME={diting_home}"
Restart=always
RestartSec=5
...
"""
    ...
    diting_home = str(get_diting_home().resolve())
    profile_arg = _profile_arg(diting_home)
...
def systemd_unit_is_current(system: bool = False) -> bool:
    ...
    # pinned DiTing_HOME is adopted before any compare/regenerate" at a single
    # site, so a future callsite cannot regress it by forgetting to pre-sync.
    #
    # Under ``sudo diting gateway … --system``, DiTing_HOME is often stripped
    # and falls back to ``/root/.diting``. Adopting the unit's pinned home
    # first makes TimeoutStopSec / WorkingDirectory / DiTing_HOME comparisons
    ...
    _sync_diting_home_from_systemd_unit(system=system)
...
def _temp_home_in_service_definition(definition: str) -> str | None:
    """Return the temp-dir DiTing_HOME baked into a service definition, or None.

    A generated systemd unit / launchd plist carries the resolved DiTing_HOME
    ...
    exported a throwaway ``DiTing_HOME=/tmp/...`` — writing it to the real
    ...
    Seen live 2026-06-11: an E2E guard probe ran ``diting gateway restart``
    with ``DiTing_HOME=/tmp/diting-e2e-<pr>`` exported; the restart path's
    ...
    Matches both systemd ``Environment="DiTing_HOME=..."`` lines and launchd
    ``<key>DiTing_HOME</key><string>...</string>`` pairs.
    """
    import re
    import tempfile

    candidates = re.findall(r'DiTing_HOME=([^"\n]+)', definition)
    candidates += re.findall(
        r"<key>DiTing_HOME</key>\s*<string>(.*?)</string>", definition, flags=re.S
    )
    temp_roots = {
...
def _refuse_temp_home_service_write(definition: str, kind: str) -> bool:
    """Refuse (with guidance) when a service definition carries a temp DiTing_HOME."""
    temp_home = _temp_home_in_service_definition(definition)
    if temp_home is None:
        return False
    print(
        f"✗ Refusing to write the gateway {kind}: DiTing_HOME resolves to a "
        f"temporary directory ({temp_home})."
    )
    print(
        "  This usually means a test/E2E environment exported DiTing_HOME. "
...
def refresh_systemd_unit_if_needed(system: bool = False) -> bool:
    ...
    # single DiTing_HOME-sync chokepoint (adopts the unit's pinned home before
    ...
    # sandboxed by the test conftest (only DiTing_HOME is). If a test
    # exercises ``run_gateway()`` with a pytest-tmp DiTing_HOME, the freshly
    # generated unit bakes that ``/tmp/pytest-of-.../diting_test`` path into
    # ``Environment="DiTing_HOME=..."``. Writing that to the developer's
    ...
    if not system and (
        ...
        or '/diting_test"' in new_unit
        or "/diting_test/" in new_unit
    ):
        return False

    # Structural variant of the same belt: refuse to bake ANY temp-dir
    # DiTing_HOME into the unit (manual E2E homes like /tmp/diting-e2e-NNN
    ...
    print(
        f"↻ Updated gateway {_service_scope_label(system)} service definition to match the current DiTing install"
    )
    return True
...
def _print_system_scope_remediation(action: str) -> None:
    ...
    print_info("         sudo diting gateway uninstall --system")
    print_info("         diting gateway install")
    print_info("         diting gateway start")


def _get_restart_drain_timeout() -> float:
    """Return the configured gateway restart drain timeout in seconds."""
    raw = os.getenv("DiTing_RESTART_DRAIN_TIMEOUT", "").strip()
    if not raw:
        cfg = read_raw_config()
...
def systemd_install(
    ...
    # Offer to remove legacy units (diting.service from pre-rename installs)
    # before installing the new diting-gateway.service. If both remain, they
    ...
    if has_legacy_diting_units():
        print()
        print_legacy_unit_warning()
        print()
        if non_interactive or prompt_yes_no("Remove the legacy unit(s) before installing?", True):
            remove_legacy_diting_units(interactive=False)
            print()

    unit_path = get_systemd_unit_path(system=system)
    scope_flag = " --system" if system else ""

    # Existing system units already pin DiTing_HOME; adopt it before any
    ...
    # ``sudo diting gateway install --system --force`` would bake /root/.diting
    ...
    print("Next steps:")
    print(
        f"  {'sudo ' if system else ''}diting gateway start{scope_flag}              # Start the service"
    )
    print(
        f"  {'sudo ' if system else ''}diting gateway status{scope_flag}             # Check status"
    )
    print(
...
def _require_service_installed(action: str, system: bool = False) -> None:
    ...
    if not unit_path.exists():
        ...
        print(f"  Run: {'sudo ' if system else ''}diting gateway install{scope_flag}")
        sys.exit(1)
...
def systemd_start(system: bool = False):
    ...
    # DiTing_HOME sync happens inside refresh_systemd_unit_if_needed's
...
def systemd_stop(system: bool = False):
    ...
    try:
        ...
        print(
            ...
            "check `diting gateway status` or logs for final shutdown state."
        )
        return
    ...
    if pid is not None:
        ...
        try:
            ...
            print(
                ...
                "check `diting gateway status` or logs for final state."
            )
            return
...
def systemd_status(deep: bool = False, system: bool = False, full: bool = False):
    ...
    if not systemd_unit_is_current(system=system):
        ...
        print(
            f"  Run: {'sudo ' if system else ''}diting gateway restart{scope_flag}  # auto-refreshes the unit"
        )
        print()
    ...
    if status == "active":
        ...
        print(f"  Run: {'sudo ' if system else ''}diting gateway start{scope_flag}")
    ...
    if active_state == "activating" and sub_state == "auto-restart":
        ...
        print(
            f"  Run after the start-limit window expires: {'sudo ' if system else ''}diting gateway restart{scope_flag}"
        )
        print(
        ...
        print(
            f"  Run: systemctl {'--user ' if not system else ''}reset-failed {get_service_name()} && {'sudo ' if system else ''}diting gateway start{scope_flag}"
        )
...
def get_launchd_label() -> str:
    ...
    suffix = _profile_suffix()
    return f"ai.diting.gateway-{suffix}" if suffix else "ai.diting.gateway"


# Cached launchd domain result — probing is cheap but should only run once per
# process invocation (each ``diting gateway start/stop/status`` call).
...
#      background process (the `nohup diting gateway run` workaround). See #23387.
...
def _launchd_reload_log_path() -> Path:
    ...
    return get_diting_home() / "logs" / "launchd-reload.log"
...
def _launchd_unsupported_marker_path() -> Path:
    return get_diting_home() / ".gateway-launchd-unsupported"
...
def _gateway_run_command() -> list[str]:
    """Build the `python -m diting_cli.main [--profile X] gateway run --replace` argv.

    Profile-aware: honors the active DiTing_HOME via `_profile_arg()` so the
    detached fallback launches into the same profile as the CLI invocation.
    """
    cmd = [get_python_path(), "-m", "diting_cli.main"]
...
def _spawn_detached_gateway() -> bool:
    ...
    macOS 26+ (issue #23387). Mirrors the `nohup diting gateway run --replace`
    ...
    """
    from diting_cli._subprocess_compat import windows_detach_popen_kwargs

    log_dir = get_diting_home() / "logs"
...
def _launchd_fallback_to_detached(reason: str, *, exit_on_failure: bool = True) -> bool:
    ...
    print(
        f"  Try manually: nohup diting gateway run --replace "
...
def refresh_launchd_plist_if_needed() -> bool:
    ...
    if (
        ...
        # to ~/.diting/logs/launchd-reload.log, which the health watchdog
        # can tail to detect a persistent orphan. See diting-restart
        # rootcause handoff (2026-06-26 incident).
        reload_log_path = get_diting_home() / "logs" / "launchd-reload.log"
        try:
    ...
    print(
        "↻ Updated gateway launchd service definition to match the current DiTing install"
    )
    return True
...
def launchd_install(force: bool = False):
    ...
    print()
    print("Next steps:")
    print("  diting gateway status             # Check status")
    from diting_constants import display_diting_home as _dhh
...
def launchd_stop():
    ...
    # `diting gateway start` re-bootstraps when it detects the job is unloaded.
    try:
...
def launchd_status(deep: bool = False):
    ...
    # DiTing PID tracking — may be a detached fallback process spawned when
    ...
    # launchd PID and the DiTing PID file).
    ...
    if launchd_plist_is_current():
        print("✓ Service definition matches the current DiTing install")
    else:
        print("⚠ Service definition is stale relative to the current DiTing install")
        print("  Run: diting gateway start")
    ...
    if service_listed:
        if launchd_pid is not None:
            ...
            if fallback_pid:
                ...
                print("  Cron jobs will fire. Stop with: diting gateway stop")
            else:
    ...
    if deep:
        log_file = get_diting_home() / "logs" / "gateway.log"
        if log_file.exists():
            print()
            print("Recent logs:")
...
def _running_under_gateway_supervisor() -> bool:
    ...
    a reliable marker into the child's environment:
      ...
      - the s6-overlay container longrun exports ``DiTing_S6_SUPERVISED_CHILD``.
    """
    if os.environ.get("INVOCATION_ID"):
        return True
    if os.environ.get("DiTing_S6_SUPERVISED_CHILD"):
        return True
...
def _guard_named_profile_under_multiplexer(force: bool = False) -> None:
    ...
    named-profile ``diting gateway run`` is always a misconfiguration, so we
    ...
    try:
        from diting_constants import get_default_diting_root
        default_root = get_default_diting_root()
...
def _guard_supervised_gateway_conflict(force: bool = False) -> None:
    ...
    Running ``diting gateway run [--replace]`` (or the manual-restart fallback)
...
def _guard_existing_gateway_process_conflict(replace: bool = False) -> None:
    ...
    ``diting gateway run`` can burn memory/CPU just to fail with "already
    ...
    unrelated gateway processes from other DiTing_HOME roots.
    """
    ...
    print("  Use 'diting gateway restart' to replace it,")
    print("  or 'diting gateway stop' first.")
    print("  Or use 'diting gateway run --replace' to auto-replace.")
    sys.exit(1)
...
def _guard_official_docker_root_gateway() -> None:
    ...
    if not hasattr(os, "geteuid") or os.geteuid() != 0:
        return
    if _truthy_env(os.getenv("DiTing_ALLOW_ROOT_GATEWAY")):
        return
    if not _is_official_docker_checkout():
        return

    print_error(
        "Refusing to run the DiTing gateway as root inside the official Docker image."
    )
    print(
        "  The image entrypoint normally drops privileges to the 'diting' user. "
        "If you override entrypoint in Docker Compose, include "
        "/opt/diting/docker/entrypoint.sh before the DiTing command."
    )
    print(
        "  Running the gateway as root can leave root-owned files in "
        "$DiTing_HOME and break later non-root dashboard/gateway runs."
    )
    print(
        "  Set DiTing_ALLOW_ROOT_GATEWAY=1 only if you intentionally accept this risk."
    )
    sys.exit(1)
...
def run_gateway(verbose: int = 0, quiet: bool = False, replace: bool = False, force: bool = False):
    ...
    # from sibling CLI processes, but foreground `diting gateway run` still
    # needs to obey the banner's "Press Ctrl+C to stop" contract.
    # Service-style launchers set DiTing_GATEWAY_DETACHED=1; older wrappers
    ...
    # /restart) rather than through `diting gateway restart` which already
    ...
    # `diting gateway start/restart` — leaving the gateway vulnerable to
    ...
    # DiTing_GATEWAY_EXIT_DIAG env var (default: on while we're still
    ...
    def _exit_diag(tag: str, **extra: object) -> None:
        if os.environ.get("DiTing_GATEWAY_EXIT_DIAG", "1") != "1":
            return
        try:
            from diting_constants import get_diting_home as _ghh
...
_PLATFORMS = [
    ...
    # plugins/platforms/matrix/adapter.py::register(). #41112.
    {
        ...
        "setup_instructions": [
            ...
            "2. Give it a username (e.g. diting) and copy the bot token",
        ...
        "vars": [
            ...
            },
            {
                ...
                "password": False,
                "help": "Channel ID where DiTing delivers cron results and notifications.",
            },
            {
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "5. DiTing connects via the BlueBubbles REST API and receives",
            "   incoming messages via a local webhook",
            "6. To authorize users, use DM pairing: diting pairing generate bluebubbles",
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "4. Enter them below and DiTing will connect automatically over WebSocket",
        ],
        "vars": [
            {
                "name": "YUANBAO_APP_ID",
...
def _all_platforms() -> list[dict]:
    ...
    ``diting setup gateway`` without needing the gateway to be running.
    ...
    every host. Currently:
      - Matrix is hidden on Windows. The [matrix] extra pulls
        ...
        picker. Users who want Matrix on Windows can run diting under
        WSL.
    """
    ...
    # User-installed platform plugins under ~/.diting/plugins/ still require
...
def _platform_status(platform: dict) -> str:
    ...
    if token_var == "WHATSAPP_ENABLED":
        if val and val.lower() == "true":
            session_file = get_diting_home() / "whatsapp" / "session" / "creds.json"
...
def _setup_standard_platform(platform: dict):
    ...
    if platform.get("key") == "telegram":
        ...
        if choice.strip() == "1":
            try:
                from diting_cli.telegram_managed_bot import (
            ...
            if value:
                ...
                if is_email:
                    ...
                    access_choices = [
                        ...
                        "Use DM pairing (unknown users request access, you approve with 'diting pairing approve')",
                ...
                if access_idx == 0:
                    ...
                    print_info(
                        "  Approve with: diting pairing approve <platform> <code>"
                    )
                elif is_email:
                    print_success("  Unknown email senders will be ignored.")
                else:
                    print_info(
                        "  Skipped — configure later with 'diting gateway setup'"
                    )
            continue
...
def _setup_weixin():
    ...
    print()
    print_info("  1. DiTing will open Tencent iLink QR login in this terminal.")
    print_info("  2. Use WeChat to scan and confirm the QR code.")
    print_info(
        "  3. DiTing will store the returned account_id/token in ~/.diting/.env."
    )
    print_info(
    ...
    if not check_weixin_requirements():
        ...
        print_info("  Install them, then rerun `diting gateway setup`.")
        return
    ...
    try:
        credentials = asyncio.run(qr_login(str(get_diting_home())))
    except KeyboardInterrupt:
        print()
    ...
    if access_idx == 0:
        ...
        print_info(
            "  Unknown DM users can request access and you approve them with `diting pairing approve`."
        )
    elif access_idx == 1:
...
def _setup_qqbot():
    ...
    if access_idx == 0:
        ...
        print_success("  DM pairing enabled.")
        print_info(
            "  Unknown users can request access; approve with `diting pairing approve`."
        )
    elif access_idx == 1:
...
def _setup_signal():
    ...
    if shutil.which("signal-cli"):
        ...
        print_info('    signal-cli link -n "DiTingAgent"')
...
def _builtin_setup_fn(key: str):
    ...
    Late-bound to avoid a circular import with ``diting_cli.setup`` (which
    imports from this module for the remaining bespoke flows).
    """
    from diting_cli import setup as _s
...
def _configure_platform(platform: dict) -> None:
    ...
    is needed here. User-installed platform plugins under ~/.diting/plugins/
    ...
    if required:
        print_info(f"  Set these env vars in ~/.diting/.env: {', '.join(required)}")
    else:
        print_info(
...
def gateway_setup():
    ...
    if any_configured:
            ...
            if supports_systemd_services() and _system_scope_wizard_would_need_root():
                ...
                try:
                    if supports_systemd_services():
                        ...
                        print_info("Start manually: diting gateway")
                except UserSystemdUnavailableError as e:
            ...
            if supports_systemd_services() or is_macos() or is_windows():
                ...
                if start_now or start_on_login:
                    try:
                        ...
                        print_info("  You can try manually: diting gateway install")
                else:
                    print_info("  Skipped start and auto-start setup.")
                    print_info("  You can install later: diting gateway install")
                    if supports_systemd_services():
                        print_info(
                            "  Or as a boot-time service: sudo diting gateway install --system"
                        )
                    print_info("  Or run in foreground:  diting gateway run")
            elif is_wsl():
                print_info("  WSL detected but systemd is not running.")
                print_info("  Run in foreground: diting gateway run")
                print_info(
                    "  For persistence:   tmux new -s diting 'diting gateway run'"
                )
                print_info(
                ...
                print_info("  Run in foreground: diting gateway run")
                print_info(
                    f"  Or start it manually in the background (best effort): nohup diting gateway run >{_dhh()}/logs/gateway.log 2>&1 &"
                )
            else:
        ...
        print()
        print_info("No platforms configured. Run 'diting gateway setup' when ready.")
...
def _dispatch_via_service_manager_if_s6(
    ...
    :mod:`diting_cli.service_manager` are caught and surfaced as
    actionable CLI messages (no raw ``CalledProcessError`` traceback).
    """
    from diting_cli.service_manager import (
    ...
    if profile is None:
        ...
        # DiTing_HOME=<root>/profiles/<name>, "" for the default root,
...
def _dispatch_all_via_service_manager_if_s6(action: str) -> bool:
    ...
    Without this, ``diting gateway stop --all`` and ``... restart --all``
...
def gateway_command(args):
    ...
    try:
        ...
        # The direct ``diting gateway install|uninstall|start|stop|restart``
...
def _maybe_redirect_run_to_s6_supervision(args) -> bool:
    ...
    Three gates make this a no-op outside the intended scope:
      ...
      1. ``_dispatch_via_service_manager_if_s6`` returns False unless
         ...
         ``diting gateway run`` are unaffected.
      2. ``DiTing_S6_SUPERVISED_CHILD`` is exported by
         ``S6ServiceManager._render_run_script`` for the supervised
         process itself — i.e. when s6-supervise execs ``diting gateway
         ...
         → ...).
      3. ``--no-supervise`` (or ``DiTing_GATEWAY_NO_SUPERVISE=1``) opts
    ...
    no_supervise = getattr(args, "no_supervise", False) or \
        os.environ.get("DiTing_GATEWAY_NO_SUPERVISE", "").lower() in ("1", "true", "yes")
    if no_supervise:
        return False
    if os.environ.get("DiTing_S6_SUPERVISED_CHILD"):
    ...
    # `docker logs` and ${DiTing_HOME}/logs/gateways/<profile>/current,
    ...
    print(
        ...
        "  dashboard supervised alongside if DiTing_DASHBOARD is set).\n"
        "  This is the recommended setup for the s6 container image — the\n"
        "  gateway will keep running even if it crashes.\n"
        "  Use `--no-supervise` (or DiTing_GATEWAY_NO_SUPERVISE=1) to opt out\n"
    ...
    #
    # Prefer `sleep infinity` (matches the static main-diting service's
    # pattern in docker/s6-rc.d/main-diting/run, and frees the Python
...
def _gateway_command_inner(args):
    ...
    if subcmd == "install":
        ...
        if is_termux():
            ...
            print("Run manually: diting gateway")
            sys.exit(1)
        if supports_systemd_services():
            if is_wsl():
                print_warning(
                ...
                print_info(
                    "  Consider running in foreground instead: diting gateway run"
                )
                print_info(
                    "  Or use tmux/screen for persistence: tmux new -s diting 'diting gateway run'"
                )
                print()
            ...
            print()
            print(
                "  diting gateway run                              # direct foreground"
            )
            print(
                "  tmux new -s diting 'diting gateway run'         # persistent via tmux"
            )
            print(
                "  nohup diting gateway run > ~/.diting/logs/gateway.log 2>&1 &  # background"
            )
            sys.exit(1)
        elif is_container():
            ...
            from diting_cli.service_manager import detect_service_manager
            if detect_service_manager() == "s6":
                print("Per-profile gateways are auto-registered when you create a profile.")
                print()
                print("  diting profile create <name>     # creates the s6 service slot")
                print("  diting -p <name> gateway start   # bring it up via s6")
                print("  diting status                    # see currently-supervised gateways")
                return
            ...
            print()
            print("To run the gateway: diting gateway run")
            sys.exit(0)
        else:
            print("Service installation not supported on this platform.")
            print("Run manually: diting gateway run")
            sys.exit(1)
        ...
        if is_termux():
            ...
            print("Stop manual runs with: diting gateway stop")
            sys.exit(1)
        if supports_systemd_services():
            ...
            if detect_service_manager() == "s6":
                ...
                print()
                print("  diting profile delete <name>     # tears down the s6 service slot")
                print("  diting -p <name> gateway stop    # stop without deleting the profile")
                return
        ...
        # slot — start them individually via `diting -p <name> gateway
        ...
        if supports_systemd_services():
            ...
            print()
            print("Or run the gateway directly: diting gateway run")
            sys.exit(0)
        else:
            print("Not supported on this platform.")
            sys.exit(1)
        ...
        if os.getenv("_DiTing_GATEWAY") == "1":
            print_error(
                "Refusing to stop the gateway from inside the gateway process.\n"
                "This command was blocked to prevent restart loops.\n"
                "Use `diting gateway stop` from a shell outside the running gateway."
            )
            sys.exit(1)
        ...
        if os.getenv("_DiTing_GATEWAY") == "1":
            print_error(
                ...
                "This command was blocked to prevent restart loops.\n"
                "Use `diting gateway restart` from a shell outside the running gateway."
            )
            sys.exit(1)
        ...
        if not service_available:
            ...
            if service_configured:
                ...
                print("  Fix the service, then retry: diting gateway start")
                sys.exit(1)
        ...
        if supports_systemd_services() and (
            ...
            if pids:
                ...
                if is_termux():
                    ...
                    print("  diting gateway install")
                else:
                    print("To install as a service:")
                    print("  diting gateway install")
                    print("  sudo diting gateway install --system")
            else:
                ...
                print()
                print("To start:")
                print("  diting gateway run      # Run in foreground")
                if is_termux():
                    print(
                        "  nohup diting gateway run > ~/.diting/logs/gateway.log 2>&1 &  # Best-effort background start"
                    )
                elif is_wsl():
                    print(
                        "  tmux new -s diting 'diting gateway run'         # persistent via tmux"
                    ...
                    print(
                        "  diting gateway install  # Install as Windows Scheduled Task (auto-start on login)"
                    )
                else:
                    print("  diting gateway install  # Install as user service")
                    print(
                        "  sudo diting gateway install --system  # Install as boot-time system service"
                    )
        ...
        # Stop, disable, and remove legacy DiTing gateway unit files from
        # pre-rename installs (e.g. diting.service). Profile units and
        # unrelated third-party services are never touched.
        ...
        if not supports_systemd_services() and not is_macos():
            ...
            return
        remove_legacy_diting_units(interactive=not yes, dry_run=dry_run)
