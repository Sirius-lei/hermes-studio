"""
DiTing Agent Uninstaller.

Provides options for:
- Full uninstall: Remove everything including configs and data
- Keep data: Remove code but keep ~/.diting/ (configs, sessions, logs)
"""
...
def remove_path_from_shell_configs():
    """Remove DiTing PATH entries from shell configuration files."""
    ...
    for config_path in configs:
        try:
            ...
            # Remove lines containing diting-agent or diting PATH entries
            new_lines = []
            skip_next = False
            
            for line in content.split('\n'):
                # Skip the "# DiTing Agent" comment and following line
                if '# DiTing Agent' in line or '# diting-agent' in line:
                    skip_next = True
                    continue
                if skip_next and ('diting' in line.lower() and 'PATH' in line):
                    skip_next = False
                    continue
                skip_next = False
                
                # Remove any PATH line containing diting
                if 'diting' in line.lower() and ('PATH=' in line or 'path=' in line.lower()):
                    continue
...
def remove_wrapper_script():
    """Remove the diting wrapper script if it exists."""
    wrapper_paths = [
        Path.home() / ".local" / "bin" / "diting",
        Path("/usr/local/bin/diting"),
    ]
    ...
    for wrapper in wrapper_paths:
        if wrapper.exists():
            try:
                # Check if it's our wrapper (contains diting_cli reference)
                content = wrapper.read_text()
                if 'diting_cli' in content or 'diting-agent' in content:
                    wrapper.unlink()
...
def remove_node_symlinks(diting_home: Path) -> list:
    ...
    symlinks node/npm/npx into the same directory as the ``diting`` command:
    ...
    the FHS fix).  Only symlinks that resolve into this DiTing home's ``node``
    directory are removed — links the user has repointed elsewhere (nvm, fnm,
    etc.) are left untouched.
    """
    node_dir = (diting_home / "node").resolve()
    removed = []
...
def uninstall_gateway_service():
    ...
    - All platforms: standalone ``diting gateway run`` processes
    ...
    try:
        from diting_cli.gateway import kill_gateway_processes, find_gateway_pids
    ...
    # 3. macOS: uninstall launchd plist
        ...
        try:
            from diting_cli.gateway import get_launchd_plist_path
...
#
#   1. Sets User-scope env vars ``DiTing_HOME`` and ``DiTing_GIT_BASH_PATH``
...
#      like ``%LOCALAPPDATA%\diting\git\cmd``, ``%LOCALAPPDATA%\diting\git\bin``,
#      ``%LOCALAPPDATA%\diting\git\usr\bin``, ``%LOCALAPPDATA%\diting\node``.
#      Again not in any rc file — only accessible via the registry or the
#      .NET [Environment] API.
#   3. Downloads PortableGit to ``%LOCALAPPDATA%\diting\git\`` and Node to
#      ``%LOCALAPPDATA%\diting\node\`` as user-scoped, isolated copies.
#      These are ~200MB combined and serve no purpose after uninstall.
#   4. On the ``diting dashboard`` + gateway paths, drops files into
#      ``%LOCALAPPDATA%\diting\gateway-service\`` and sometimes
...
def _diting_path_markers(diting_home: Path) -> list[str]:
    """Path-entry substrings that identify DiTing-owned User-PATH entries."""
    root = str(diting_home).rstrip("\\/")
    # Match on prefix so sub-entries (git\cmd, git\bin, git\usr\bin, node, etc.)
    # all get swept.  Also match the bare diting-agent install dir.
    markers = [root + "\\diting-agent", root + "\\git", root + "\\node", root + "\\venv"]
    # Also match if DiTing_HOME was customised to somewhere else — find-and-nuke
    # any entry whose path component contains "diting".  We don't want to catch
    # unrelated entries like "cditing-foo" or "ephermeral", so we look for
    # backslash-diting as a word-ish boundary.
    return markers


def remove_path_from_windows_registry(diting_home: Path) -> list[str]:
    """Strip DiTing-owned entries from User-scope PATH in the registry.
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, key_path, 0,
            ...
            markers = _diting_path_markers(diting_home)
            kept: list[str] = []
...
def remove_diting_env_vars_windows() -> list[str]:
    """Delete DiTing_HOME and DiTing_GIT_BASH_PATH from User-scope env vars."""
    try:
        import winreg
    except ImportError:
        return []
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, "Environment", 0,
                            winreg.KEY_READ | winreg.KEY_WRITE) as key:
            for name in ("DiTing_HOME", "DiTing_GIT_BASH_PATH"):
                try:
...
def _is_default_diting_home(diting_home: Path) -> bool:
    """Return True when ``diting_home`` points at the default (non-profile) root."""
    try:
        from diting_constants import get_default_diting_root
        return diting_home.resolve() == get_default_diting_root().resolve()
    except Exception:
        return False
...
def _uninstall_profile(profile) -> None:
    ...
    remove its alias wrapper, and wipe its DiTing_HOME directory.

    We shell out to ``diting -p <name> gateway stop|uninstall`` because
    service names, unit paths, and plist paths are all derived from the
    current DiTing_HOME and can't be easily switched in-process.
    """
    import sys as _sys
    name = profile.name
    ...
    #    Use `python -m diting_cli.main` so we don't depend on a `diting`
    #    wrapper that may be half-removed mid-uninstall.
    diting_invocation = [_sys.executable, "-m", "diting_cli.main", "--profile", name]
    for subcmd in ("stop", "uninstall"):
        try:
            subprocess.run(
                diting_invocation + ["gateway", subcmd],
                capture_output=True,
                text=True,
                timeout=60,
                check=False,
    ...
    # 3. Wipe the profile's DiTing_HOME directory.
    try:
...
def run_gui_uninstall(args):
    ...
    Mirrors ``diting uninstall --gui``. Removes the desktop app's built
    artifacts, the packaged app bundle (best-effort), and the Electron
    userData dir — nothing under ``$DiTing_HOME`` config/sessions/.env, and
    never the Python agent or its venv.
    """
    from diting_cli.gui_uninstall import (
        agent_is_installed,
        gui_install_summary,
        uninstall_gui,
    )
    ...
    summary = gui_install_summary(diting_home)
    skip_confirm = bool(getattr(args, "yes", False))

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│         ⚕ DiTing Chat GUI Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()

    if not summary["gui_installed"]:
        print("No DiTing Chat GUI installation was found.")
        print(f"  Checked: {diting_home}, and the standard app locations for this OS.")
        return

    print(color("This removes the Chat GUI only. The DiTing agent stays installed.", Colors.CYAN))
    print()
    ...
    print()
    if agent_is_installed(diting_home):
        print(color("Kept intact:", Colors.GREEN, Colors.BOLD))
        print(f"  • The DiTing agent at {diting_home / 'diting-agent'}")
        print(f"  • Your config, sessions, and secrets under {diting_home}")
        print()
    ...
    print(color("└─────────────────────────────────────────────────────────┘", Colors.GREEN, Colors.BOLD))
    print()
    print("The DiTing agent is still installed. Run 'diting' to use the CLI,")
    print("or 'diting uninstall' to remove the agent too.")
    print()
...
def run_uninstall(args):
    ...
    Options:
    - Full uninstall: removes code + ~/.diting/ (configs, data, logs)
    - Keep data: removes code but keeps ~/.diting/ for future reinstall
    """
    ...
    # offer to clean them up too instead of leaving zombie DiTing_HOMEs
    # and systemd units behind.
    is_default_profile = _is_default_diting_home(diting_home)
    named_profiles = _discover_named_profiles() if is_default_profile else []

    # Non-interactive fast path (``--yes``): no prompts. ``--full`` selects a
    # full wipe (code + ~/.diting data); otherwise keep-data. Named profiles
    ...
    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│            ⚕ DiTing Agent Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()
    ...
    print(f"  Config:  {diting_home / 'config.yaml'}")
    print(f"  Secrets: {diting_home / '.env'}")
    print(f"  Data:    {diting_home / 'cron/'}, {diting_home / 'sessions/'}, {diting_home / 'logs/'}")
    print()
    ...
    # their alias wrappers, and wiping their DiTing_HOME dirs. Otherwise
    ...
    print()
    if full_uninstall:
        print(color("⚠️  WARNING: This will permanently delete ALL DiTing data!", Colors.RED, Colors.BOLD))
        ...
        print("This will remove the DiTing code but keep your configuration and data.")
...
def _print_uninstall_dry_run(*, project_root: Path, diting_home: Path, full_uninstall: bool) -> None:
    ...
    print("  • DiTing PATH entries from shell configs / Windows User PATH")
    print("  • DiTing wrapper scripts and DiTing-managed node/npm/npx symlinks")
    print("  • Desktop Chat GUI artifacts")
    print(f"  • Code checkout: {project_root}")
    if full_uninstall:
        print(f"  • DiTing config/data: {diting_home}")
        if _is_default_diting_home(diting_home):
        ...
        print(f"  • Keep DiTing config/data: {diting_home}")
    print()
...
def _perform_uninstall(
    *,
    ...
    ``diting`` wrapper + node symlinks → remove the desktop Chat GUI artifacts →
    delete the code checkout → (Windows) remove PortableGit/Node → optionally
    wipe ``$DiTing_HOME`` data and named profiles on full uninstall.
    """
    print()
    ...
    if _is_windows():
        ...
        # Expand %LOCALAPPDATA% etc. in diting_home so the marker matching is
        # against fully resolved paths — installer writes literal strings
        # like C:\Users\<u>\AppData\Local\diting\git\cmd, not %LOCALAPPDATA%.
        removed_path_entries = remove_path_from_windows_registry(Path(os.path.expandvars(str(diting_home))))
        if removed_path_entries:
            for entry in removed_path_entries:
                log_success(f"Removed from User PATH: {entry}")
        else:
            log_info("No DiTing-owned PATH entries in User environment")

        log_info("Removing DiTing_HOME / DiTing_GIT_BASH_PATH User env vars...")
        removed_env = remove_diting_env_vars_windows()
        if removed_env:
            for name in removed_env:
                log_success(f"Removed User env var: {name}")
        else:
            log_info("No DiTing-set User env vars to remove")
    
    # 3. Remove wrapper script
    log_info("Removing diting command...")
    ...
    #     (only when they still point into this DiTing home's node dir, so we
    #     never clobber an existing nvm / user-managed Node).
    log_info("Removing DiTing-managed node/npm/npx symlinks...")
    removed_node_links = remove_node_symlinks(diting_home)
    if removed_node_links:
        for link in removed_node_links:
            log_success(f"Removed {link}")
    else:
        log_info("No DiTing-managed node/npm/npx symlinks found")
    ...
    #     step-5 rmtree(diting_home) would sweep the in-tree artifacts anyway,
    #     but the packaged app + Electron userData live OUTSIDE DiTing_HOME and
    #     must be cleaned explicitly here.
    log_info("Removing desktop Chat GUI artifacts...")
    try:
        from diting_cli.gui_uninstall import uninstall_gui
        gui_removed = uninstall_gui(diting_home)
        if not gui_removed:
    ...
    try:
        if project_root.exists():
            # If the install is inside ~/.diting/, just remove the diting-agent subdir
            if diting_home in project_root.parents or project_root.parent == diting_home:
    ...
    #     under DiTing_HOME but they're install tooling, not config — safe to
    #     remove even in "keep data" mode.  If we're doing a full uninstall
    #     the step-5 rmtree(diting_home) would sweep them anyway; calling
    ...
    if _is_windows():
        ...
        removed_artifacts = remove_portable_tooling_windows(diting_home)
    ...
    # 5. Optionally remove ~/.diting/ data directory (and named profiles)
    if full_uninstall:
        # 5a. Stop and remove each named profile's gateway service and
        #     alias wrapper. The profile DiTing_HOME dirs live under
        ...
        try:
            if diting_home.exists():
                shutil.rmtree(diting_home)
                log_success(f"Removed {diting_home}")
        except Exception as e:
            log_warn(f"Could not fully remove {diting_home}: {e}")
            log_info("You may need to manually remove it")
    else:
        log_info(f"Keeping configuration and data in {diting_home}")
    ...
    if not full_uninstall:
        ...
        if _is_windows():
            print(color("  iex (irm https://diting-agent.nousresearch.com/install.ps1)", Colors.DIM))
        else:
            print(color("  curl -fsSL https://diting-agent.nousresearch.com/install.sh | bash", Colors.DIM))
        print()
    ...
    print()
    print("Thank you for using DiTing Agent! ⚕")
    print()
...
def main(argv=None) -> int:
    """Module entrypoint: ``python -m diting_cli.uninstall --mode <gui|lite|full>``.
    ...
    so ``import diting_cli`` resolves from source while the venv is torn down.

    This module imports only stdlib + ``diting_constants`` + ``diting_cli.colors``
    (and lazily ``diting_cli.gui_uninstall``), so it runs fine under a bare
    system Python with no site-packages from the venv.
    """
    import argparse

    parser = argparse.ArgumentParser(prog="python -m diting_cli.uninstall")
    parser.add_argument(
        "--mode",
