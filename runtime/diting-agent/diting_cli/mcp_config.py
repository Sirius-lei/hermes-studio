"""
MCP Server Management CLI — ``diting mcp`` subcommand.

Implements ``diting mcp add/remove/list/test/configure`` for interactive
MCP server lifecycle management (issue #690 Phase 2).

Relies on tools/mcp_tool.py for connection/discovery and keeps
configuration in ~/.diting/config.yaml under the ``mcp_servers`` key.
"""
...
from diting_cli.config import (
    ...
    get_env_value,
    save_env_value,
    get_diting_home,  # noqa: F401 — used by test mocks
)
...
def _prompt(question: str, *, password: bool = False, default: str = "") -> str:
    from diting_cli.cli_output import prompt as _shared_prompt
...
def _resolve_mcp_server_config(config: dict) -> dict:
    ...
    ``~/.diting/.env`` into ``os.environ`` and recursively interpolate any
...
def _oauth_tokens_present(name: str) -> bool:
    ...
    Used after ``diting mcp login`` to distinguish a genuine authentication
    ...
    """
    try:
        from tools.mcp_oauth import DiTingTokenStorage
        return DiTingTokenStorage(name).has_cached_tokens()
...
def cmd_mcp_add(args):
    ...
    # diting_cli/main.py for why the dest is renamed.
    ...
    if not url and not command:
        ...
        _info("Examples:")
        _info('  diting mcp add ink --url "https://mcp.ml.ink/mcp"')
        _info('  diting mcp add github --command npx --args @modelcontextprotocol/server-github')
        _info('  diting mcp add myserver --preset mypreset')
        return
            ...
            if auth_type == "header" or not auth_type:
                ...
                if existing_key:
                    ...
                    if api_key:
                        ...
                        _success(f"Saved to {display_diting_home()}/.env as {env_key}")
    ...
    try:
        ...
        if _confirm("Save config anyway (you can test later)?", default=False):
            ...
            if _save_mcp_server(name, server_config):
                ...
                _info("Fix the issue, then: diting mcp test " + name)
        return
    ...
    if _save_mcp_server(name, server_config):
        print()
        _success(f"Saved '{name}' to {display_diting_home()}/config.yaml ({tool_count}/{total} tools enabled)")
...
def cmd_mcp_remove(args):
    ...
    # earlier `diting mcp test` in the same session) is evicted too.
    try:
        from tools.mcp_oauth_manager import get_manager
...
def cmd_mcp_list(args=None):
    ...
    if not servers:
        ...
        print()
        _info("Add one with:")
        _info('  diting mcp add <name> --url <endpoint>')
        _info('  diting mcp add <name> --command <cmd> --args <args...>')
        print()
        return
...
def _reauth_oauth_server(name: str, server_config: dict) -> bool:
    ...
    landed before reporting success. Shared by ``diting mcp login`` and
    ``diting mcp reauth`` so both behave identically for a single server.
    """
    ...
    if server_config.get("auth") != "oauth":
        ...
        _info("Use `diting mcp remove` + `diting mcp add` to reconfigure auth.")
        return False
    ...
    try:
        ...
        if not _oauth_tokens_present(name):
            ...
            print()
            _info("Then re-run `diting mcp login " + name + "`.")
            return False
        if tools:
...
def cmd_mcp_reauth(args):
    ...
    ``diting mcp reauth <name>`` re-auths a single server (same as ``login``).
    ``diting mcp reauth --all`` discovers every ``auth: oauth`` server in
    ...
    if not name:
        ...
        _info("Usage: diting mcp reauth <name>   |   diting mcp reauth --all")
        return
    if name not in servers:
        _error(f"Server '{name}' not found in config.")
        if servers:
...
# ─── diting mcp configure ────────────────────────────────────────────────────

def cmd_mcp_configure(args):
    """Reconfigure which tools are enabled for an existing MCP server."""
    import sys as _sys
    if not _sys.stdin.isatty():
        print("Error: 'diting mcp configure' requires an interactive terminal.", file=_sys.stderr)
        _sys.exit(1)
    name = args.name
    servers = _get_mcp_servers()
...
def mcp_command(args):
    """Main dispatcher for ``diting mcp`` subcommands."""
    ...
    if action == "picker":
        from diting_cli.mcp_picker import run_picker
        run_picker()
        return
    if action == "catalog":
        from diting_cli.mcp_picker import show_catalog
        show_catalog()
        return
    if action == "install":
        from diting_cli.mcp_picker import install_by_name
        import sys as _sys
    ...
    if handler:
        ...
        # "try enabling and it flows you into setup" UX matching `diting plugin`.
        from diting_cli.mcp_picker import run_picker
        run_picker()
        print(color("  Commands:", Colors.CYAN))
        _info("diting mcp                                    Open the catalog picker (default)")
        _info("diting mcp catalog                            List Nous-approved MCPs")
        _info("diting mcp install <name>                     Install a catalog MCP")
        _info("diting mcp serve                              Run as MCP server")
        _info("diting mcp add <name> --url <endpoint>        Add a custom MCP server")
        _info("diting mcp add <name> --command <cmd>         Add a stdio server")
        _info("diting mcp add <name> --preset <preset>       Add from a known preset")
        _info("diting mcp remove <name>                      Remove a server")
        _info("diting mcp list                               List configured servers")
        _info("diting mcp test <name>                        Test connection")
        _info("diting mcp configure <name>                   Toggle tools")
        _info("diting mcp login <name>                       Re-authenticate OAuth")
        _info("diting mcp reauth <name> | --all              Re-auth one or all OAuth servers")
        print()
