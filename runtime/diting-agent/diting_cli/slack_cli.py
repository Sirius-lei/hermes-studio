"""``diting slack ...`` CLI subcommands.

Today only ``diting slack manifest`` is implemented — it generates the
...
Typical workflow::
    ...
    $ diting slack manifest > slack-manifest.json
    # or:
...
def _build_full_manifest(
    ...
    it stays in sync with the rest of DiTing. Other manifest sections
    (display info, OAuth scopes, socket mode) are set to sensible defaults
    for a DiTing deployment — users can tweak them in the Slack UI after
    pasting.

    By default, this keeps DiTing on Slack's older Assistant messaging
    ...
    """
    from diting_cli.commands import slack_app_manifest
    ...
    if messaging_experience == "assistant":
        features["assistant_view"] = {
            "assistant_description": "Chat with DiTing in threads and DMs.",
        }
        ...
        features["agent_view"] = {
            "agent_description": "Chat with DiTing in Slack Messages.",
        }
        bot_scopes.append("assistant:write")
    ...
    return {
        ...
        "display_information": {
            "name": bot_name[:35],
            "description": (bot_description or "Your DiTing agent on Slack")[:140],
...
def slack_manifest_command(args) -> int:
    ...
    Flags (all parsed in ``diting_cli/main.py``):
      --write [PATH]  Write to file instead of stdout (default path:
                      ``$DiTing_HOME/slack-manifest.json``)
      --name NAME     Override the bot display name (default: "DiTing")
    ...
    """
    name = getattr(args, "name", None) or "DiTing"
    description = getattr(args, "description", None) or "Your DiTing agent on Slack"
    ...
    if write_target is not None:
        if isinstance(write_target, bool) and write_target:
            ...
            try:
                ...
                target = Path(os.environ.get("DiTing_HOME") or str(Path.home() / ".diting")) / "slack-manifest.json"
        else:
        ...
        print(
            "\nNext steps:\n"
            "  1. Open https://api.slack.com/apps and pick your DiTing app\n"
