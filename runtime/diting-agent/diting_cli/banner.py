...
Pure display functions with no DiTingCLI state dependency.
"""
import json
import logging
import os
...
# (e.g. nix-built diting — no local git history to count against).
UPDATE_AVAILABLE_NO_COUNT = -1

_UPSTREAM_REPO_URL = "https://github.com/NousResearch/diting-agent.git"
_OFFICIAL_REPO_CANONICAL = "github.com/nousresearch/diting-agent"
...
def _fetch_pypi_latest(package: str = "diting-agent") -> Optional[str]:
...
def check_for_updates() -> Optional[int]:
    """Check whether a DiTing update is available.

    Two paths: if ``DiTing_REVISION`` is set (nix builds embed it), compare
    ...
    # DiTing_REVISION (that's nix-only). Without this guard the checks below
    ...
    # and `diting update` correctly refuses to run in-place inside the
    # container anyway. The dashboard's REST `/api/diting/update/check`
    ...
    try:
        from diting_cli.config import detect_install_method, get_project_root
    ...
    if embedded_rev:
        ...
        # $DiTing_HOME/diting-agent/ may be a stale copy from --clone-all;
        # Path(__file__) always resolves to the actual installed checkout.
        repo_dir = Path(__file__).parent.parent.resolve()
        if not (repo_dir / ".git").exists():
            repo_dir = diting_home / "diting-agent"
        if not (repo_dir / ".git").exists():
...
def _resolve_repo_dir() -> Optional[Path]:
    """Return the active DiTing git checkout, or None if this isn't a git install.

    Prefers the running code's location over the profile-scoped path
    because ``$DiTing_HOME/diting-agent/`` may be a stale copy carried
    over by ``--clone-all``.
    """
    repo_dir = Path(__file__).parent.parent.resolve()
    if not (repo_dir / ".git").exists():
...
def get_git_banner_state(repo_dir: Optional[Path] = None) -> Optional[dict]:
    ...
    ``diting_cli/build_info.py``) and return it as a frozen
    ...
    if repo_dir is None:
        ...
        try:
            from diting_cli.build_info import get_build_sha
...
_RELEASE_URL_BASE = "https://github.com/NousResearch/diting-agent/releases/tag"
...
def get_latest_release_tag(repo_dir: Optional[Path] = None) -> Optional[tuple]:
    ...
    DiTing checkout. Cached per-process. Release URL always points at the
    canonical NousResearch/diting-agent repo (forks don't get a link).
    """
...
def format_banner_version_label() -> str:
    ...
    base = f"DiTing Agent v{VERSION} ({RELEASE_DATE})"
...
def build_welcome_banner(console: "Console", model: str, cwd: str,
    ...
    try:
        ...
        _hero = _bskin.banner_hero if hasattr(_bskin, 'banner_hero') and _bskin.banner_hero else DiTing_CADUCEUS
    except Exception:
        _bskin = None
        _hero = DiTing_CADUCEUS
    left_lines = ["", _hero, ""]
    ...
    try:
        from diting_cli.codex_runtime_switch import get_current_runtime
    ...
    try:
        ...
        if behind is not None and behind != 0:
            from diting_cli.config import get_managed_update_command, recommended_update_command
            if behind > 0:
                ...
                # UPDATE_AVAILABLE_NO_COUNT: nix-built diting; we know an update
    ...
    if term_width >= 95:
        _logo = _bskin.banner_logo if _bskin and hasattr(_bskin, 'banner_logo') and _bskin.banner_logo else DiTing_AGENT_LOGO
        console.print(_logo)
        console.print()
