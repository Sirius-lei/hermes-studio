...
Usage:
    diting pairing list              # Show all pending + approved users
    diting pairing approve <platform> <code>  # Approve a pairing code
    diting pairing revoke <platform> <user_id> # Revoke user access
    diting pairing clear-pending     # Clear all expired/pending codes
"""

def pairing_command(args):
    """Handle diting pairing subcommands."""
    ...
    if action == "list":
        ...
        print("Usage: diting pairing {list|approve|revoke|clear-pending}")
        print("Run 'diting pairing --help' for details.")
...
def _cmd_approve(store, platform: str, code: str):
    ...
    if result:
        ...
        print(
            ...
            "~/.diting/platforms/pairing/_rate_limits.json\n".format(platform)
        )
    else:
        print(f"\n  Code '{code}' not found or expired for platform '{platform}'.")
        print("  Run 'diting pairing list' to see pending codes.\n")
