"""Safe DiTing Console command engine.

This module backs ``diting console`` and is intentionally narrower than the
full DiTing CLI. It exposes a curated set of native adapters that can later be
...
class ConsoleCommand:
    ...
    usage: str
    summary: str
    handler: Callable[["DiTingConsoleEngine", list[str]], str]
    mutating: bool = False
    confirmation: str = ""
...
def _strip_console_status_footer(text: str) -> str:
    ...
    if not (
        prev.startswith("Run 'diting doctor'")
        and last.startswith("Run 'diting setup'")
    ):
        return text.rstrip()
...
def _parser_root() -> tuple[_ArgumentParser, argparse._SubParsersAction]:
    parser = _ArgumentParser(prog="diting", add_help=False)
...
def _clean_summary(text: str | None) -> str:
    ...
    if not summary:
        return ""
    if summary.startswith("Run `diting "):
        return ""
    return summary
...
# dashboard opens a fresh DiTingConsoleEngine per /api/console connection, so
...
def _dispatch_extracted_subcommand(
    *,
    ...
    parser, subparsers = _parser_root()
    module = importlib.import_module(module_name)
    main_module = importlib.import_module("diting_cli.main")
    builder = getattr(module, builder_name)
...
def _extracted_handler(
    ...
    main_handler_name: str,
    namespace_update: Callable[[argparse.Namespace, ConsoleContext], None] | None = None,
) -> Callable[["DiTingConsoleEngine", list[str]], str]:
    def handler(_engine: DiTingConsoleEngine, args: list[str]) -> str:
...
def _register_command_family(
    ...
    *,
    ...
    handler_factory: Callable[[Sequence[str]], Callable[["DiTingConsoleEngine", list[str]], str]],
    ...
    for child_path in paths:
        ...
        command_summary = summary or (summaries or {}).get(full_path) or f"Run `diting {usage}`."
        engine.register(
            full_path,
            usage,
            command_summary,
            ...
            confirmation=confirmation or f"Run `diting {usage}`?",
            contexts=ALL_CONTEXTS if child_key in hosted_paths else LOCAL_CONTEXTS,
        )


class DiTingConsoleEngine:
    """Curated line-command executor for DiTing Console."""
        ...
        try:
            ...
            if _contains_shell_syntax(raw_line, tokens):
                raise ConsoleCommandError(
                    "DiTing Console does not run shell syntax. Use one supported "
                    "DiTing command at a time."
                )
    ...
    def _register_defaults(self) -> None:
        self.register(("status",), "status", "Show DiTing component status.", _status, contexts=ALL_CONTEXTS)
        self.register(("doctor",), "doctor", "Run diagnostics without auto-fix.", _doctor, contexts=ALL_CONTEXTS)
        self.register(("logs",), "logs [name] [-n N]", "Show recent DiTing logs.", _logs, contexts=ALL_CONTEXTS)
        ...
        self.register(
            ...
            _config_set,
            mutating=True,
            confirmation="Update DiTing configuration?",
    ...
    def _register_broad_cli_surface(self) -> None:
        """Register non-admin CLI commands that are safe for DiTing Console."""

        extracted = {
            "version": (
                "diting_cli.subcommands.version",
            ...
            "prompt-size": (
                "diting_cli.subcommands.prompt_size",
            ...
            "insights": (
                "diting_cli.subcommands.insights",
            ...
            "security": (
                "diting_cli.subcommands.security",
            ...
            "import": (
                "diting_cli.subcommands.import_cmd",
            ...
            "plugins": (
                "diting_cli.subcommands.plugins",
            ...
            "pairing": (
                "diting_cli.subcommands.pairing",
            ...
            "webhook": (
                "diting_cli.subcommands.webhook",
            ...
            "profile": (
                "diting_cli.subcommands.profile",
        ...
        self.register(
            ...
            _config_migrate,
            mutating=True,
            confirmation="Update DiTing configuration with missing defaults?",
        )
        self.register(
            ("sessions", "export"),
        ...
        self.register(
            ...
            _adder_handler("send", (), "diting_cli.send_cmd", "register_send_subparser"),
            mutating=True,
        ...
        _register_command_family(
            ...
            root="portal",
            paths=portal_paths,
            summaries=_adder_summaries("diting_cli.portal_cli", "add_parser"),
            handler_factory=lambda fixed: _adder_handler(
                "portal",
                fixed,
                "diting_cli.portal_cli",
                "add_parser",
            ),
        )
        ...
        _register_command_family(
            ...
            summaries=_builder_summaries("diting_cli.projects_cmd", "build_parser"),
            mutating=[
                ("create",),
                ("add-folder",),
                ("remove-folder",),
        ...
        _register_command_family(
            ...
            summaries=_builder_summaries("diting_cli.kanban", "build_parser"),
            mutating=[
                ("init",),
                ("boards", "create"),
                ("boards", "rm"),
    ...
    def _resolve_command(self, tokens: Sequence[str]) -> tuple[ConsoleCommand, list[str]]:
        ...
        for size in range(min(len(tokens), 3), 0, -1):
            ...
            if command:
                if self.context not in command.contexts:
                    raise ConsoleCommandError(
                        f"`diting {command.usage}` is not available in "
                        f"{self.context} DiTing Console."
                    )
        ...
        raise ConsoleCommandError(f"Unsupported DiTing Console command: {probe}.{suffix}")
    ...
    def _rejection_for(self, tokens: Sequence[str]) -> str:
        first = tokens[0]
        if first.startswith("-"):
            return f"{first} is not available in DiTing Console."
        blocked_top = {
            "acp",
            "chat",
            "claw",
        ...
        if first in blocked_top:
            return f"`diting {first}` is not available in DiTing Console."
        blocked_pairs = {
            ("config", "edit"): "`config edit` opens an editor and is not available in DiTing Console.",
            ("mcp", "serve"): "`mcp serve` starts a server and is not available in DiTing Console.",
            ("profile", "alias"): "`profile alias` creates shell wrappers and is not available in DiTing Console.",
            ("skills", "config"): "`skills config` is interactive and is not available in DiTing Console.",
            ("skills", "publish"): "`skills publish` is not available in DiTing Console.",
            ("portal", "login"): "`portal login` is interactive and is not available in DiTing Console.",
            ("portal", "open"): "`portal open` opens a browser and is not available in DiTing Console.",
            ("kanban", "tail"): "`kanban tail` streams output and is not available in DiTing Console.",
            ("kanban", "watch"): "`kanban watch` streams output and is not available in DiTing Console.",
            ("kanban", "daemon"): "`kanban daemon` starts a service and is not available in DiTing Console.",
            ("kanban", "dispatcher"): "`kanban dispatcher` starts a worker and is not available in DiTing Console.",
            ("kanban", "swarm"): "`kanban swarm` starts agent work and is not available in DiTing Console.",
            ("kanban", "decompose"): "`kanban decompose` starts agent work and is not available in DiTing Console.",
            ("kanban", "specify"): "`kanban specify` starts agent work and is not available in DiTing Console.",
            ("kanban", "gc"): "`kanban gc` is not available in DiTing Console.",
        }
        if len(tokens) >= 2:
        ...
        if tuple(tokens[:2]) in {("sessions", "delete"), ("sessions", "prune")}:
            return "`sessions delete` and `sessions prune` are not available in DiTing Console."
        return ""
...
def _enforce_hosted_line_policy(path: tuple[str, ...], args: Sequence[str]) -> None:
    if path == ("config", "set"):
        ...
        if key and not _hosted_config_key_allowed(key):
            raise ConsoleCommandError(
                f"`config set {key}` is not available in hosted DiTing Console. "
    ...
    if path == ("mcp", "add"):
        if _flag_present(args, "--command") or _flag_present(args, "--args"):
            raise ConsoleCommandError(
                "Hosted DiTing Console does not add stdio MCP servers. "
                "Use catalog install or an HTTP/SSE URL."
            )
        if _flag_present(args, "--preset"):
            raise ConsoleCommandError(
                "Hosted DiTing Console does not add MCP presets directly. "
                "Use `mcp install <catalog-name>`."
            )
        url = _flag_value(args, "--url")
        if not url:
            raise ConsoleCommandError(
                "Hosted DiTing Console requires `mcp add` to use --url with "
                "an HTTP/SSE endpoint."
            )
        scheme = urlparse(url).scheme.lower()
        if scheme not in {"http", "https"}:
            raise ConsoleCommandError(
                "Hosted DiTing Console only accepts http:// or https:// MCP URLs."
            )
        return
...
def _apply_confirmed_defaults(args: argparse.Namespace, context: ConsoleContext) -> None:
    ...
    if getattr(args, "auth_action", None) == "add":
        ...
        if auth_type in {"api-key", "api_key"} and not getattr(args, "api_key", None):
            raise ConsoleCommandError("auth add --type api-key requires --api-key in DiTing Console.")
...
def _status(_engine: DiTingConsoleEngine, args: list[str]) -> str:
...
def _doctor(_engine: DiTingConsoleEngine, args: list[str]) -> str:
...
def _logs(_engine: DiTingConsoleEngine, args: list[str]) -> str:
    if "-f" in args or "--follow" in args:
        raise ConsoleCommandError("`logs -f` is not available in DiTing Console.")
    ...
    from diting_cli.logs import list_logs, tail_log
...
def _sessions_list(_engine: DiTingConsoleEngine, args: list[str]) -> str:
...
def _sessions_stats(_engine: DiTingConsoleEngine, args: list[str]) -> str:
...
def _config_show(_engine: DiTingConsoleEngine, args: list[str]) -> str:
    _expect_no_args(args, "config show")
    from diting_cli.config import show_config

    return _capture_output(show_config)


def _config_path(_engine: DiTingConsoleEngine, args: list[str]) -> str:
    _expect_no_args(args, "config path")
    from diting_cli.config import get_config_path

    return str(get_config_path())


def _config_set(_engine: DiTingConsoleEngine, args: list[str]) -> str:
    if len(args) < 2:
        raise ConsoleCommandError("Usage: config set <key> <value>")
    key = args[0]
    value = " ".join(args[1:])
    from diting_cli.config import set_config_value

    return _capture_output(lambda: set_config_value(key, value))


def _config_migrate(_engine: DiTingConsoleEngine, args: list[str]) -> str:
    _expect_no_args(args, "config migrate")

    def _run() -> None:
        from diting_cli.config import migrate_config
...
def _sessions_export(_engine: DiTingConsoleEngine, args: list[str]) -> str:
...
def _sessions_rename(_engine: DiTingConsoleEngine, args: list[str]) -> str:
...
def _sessions_optimize(_engine: DiTingConsoleEngine, args: list[str]) -> str:
...
def _sessions_repair(_engine: DiTingConsoleEngine, args: list[str]) -> str:
    ...
    def _run() -> None:
        from diting_state import DEFAULT_DB_PATH, _db_opens_cleanly, repair_state_db_schema
...
def _profile_status(_engine: DiTingConsoleEngine, args: list[str]) -> str:
    _expect_no_args(args, "profile")
    return _dispatch_extracted_subcommand(
        root="profile",
        fixed=(),
        args=(),
        module_name="diting_cli.subcommands.profile",
...
def _cron_list(_engine: DiTingConsoleEngine, args: list[str]) -> str:
    parser = _ArgumentParser(prog="cron list", add_help=False)
    parser.add_argument("--all", action="store_true")
    ns = parser.parse_args(args)
    from diting_cli.cron import cron_list

    return _capture_output(lambda: cron_list(show_all=ns.all))


def _cron_status(_engine: DiTingConsoleEngine, args: list[str]) -> str:
    _expect_no_args(args, "cron status")
    from diting_cli.cron import cron_status

    return _capture_output(cron_status)


def _cron_pause(_engine: DiTingConsoleEngine, args: list[str]) -> str:
    if len(args) != 1:
        raise ConsoleCommandError("Usage: cron pause <job>")
    from cron.jobs import AmbiguousJobReference, pause_job

    try:
        job = pause_job(args[0], reason="paused from diting console")
...
def _cron_resume(_engine: DiTingConsoleEngine, args: list[str]) -> str:
    if len(args) != 1:
...
def _cron_run(_engine: DiTingConsoleEngine, args: list[str]) -> str:
    if len(args) != 1:
...
def run_console_repl(
    *,
    ...
    """Run the local ``diting console`` REPL."""
    ...
    engine = DiTingConsoleEngine()
    if interactive:
        print("DiTing Console. Type `help` for commands, `exit` to quit.", file=stdout)

    while True:
        if interactive:
            print("diting> ", end="", file=stdout, flush=True)
        line = stdin.readline()
        if line == "":
            if interactive:
                print(file=stdout)
