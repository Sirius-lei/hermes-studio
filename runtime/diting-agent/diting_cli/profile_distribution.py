"""Profile distributions — shareable, packaged DiTing profiles via git.

A distribution is a DiTing profile published as a git repository (or
...
* ``diting profile export/import`` — local backup / restore for a profile
  on your own machine. NOT a distribution format. Stays as-is.
* ``diting skills install <url>`` — the URL install pattern we're mirroring,
  but at the profile granularity.

Subcommands (all live under ``diting profile``, not a parallel tree):

    diting profile install <source> [--name N] [--alias] [--force] [--yes]
...
USER_OWNED_EXCLUDE: frozenset = frozenset({
    ...
    "backups", "cache",
    # Infrastructure
    "diting-agent", ".worktrees", "profiles", "bin", "node_modules",
...
class DistributionManifest:
    ...
    def from_dict(cls, data: Any) -> "DistributionManifest":
        ...
        return cls(
            ...
            description=str(data.get("description") or ""),
            diting_requires=str(data.get("diting_requires") or ""),
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.diting_requires:
            out["diting_requires"] = self.diting_requires
        if self.author:
...
def check_diting_requires(spec: str, current_version: str) -> None:
    ...
    if not ok:
        raise DistributionError(
            f"This distribution requires DiTing {op}{target}, "
...
def _stage_source(source: str, workdir: Path) -> Tuple[Path, str]:
    ...
    installed manifest's ``source:`` field so ``diting profile update`` can
    ...
    if _looks_like_git_url(src_str):
        ...
        if not (cloned / MANIFEST_FILENAME).is_file():
            raise DistributionError(
                ...
                "This repository is not a DiTing profile distribution."
            )
...
def plan_install(
    ...
    if manifest is None:
        raise DistributionError(
            ...
            "this source is not a DiTing distribution."
        )

    # Version check up-front so we fail fast
    check_diting_requires(manifest.diting_requires, diting_version)
    ...
    if canon == "default":
        raise DistributionError(
            ...
            "root profile (~/.diting).  Pass --name <name> to install under a "
            "new profile."
        )
    manifest.name = canon
...
def install_distribution(
    ...
    with tempfile.TemporaryDirectory(prefix="diting_dist_install_") as tmp:
        ...
        if plan.existing and not force:
            raise DistributionError(
                ...
                "Use `diting profile update` to upgrade in place, "
...
def update_distribution(
    ...
    if not existing_manifest.source:
        raise DistributionError(
            ...
            "`diting profile install <source> --name {canon} --force`."
        )

    with tempfile.TemporaryDirectory(prefix="diting_dist_update_") as tmp:
        plan = plan_install(
