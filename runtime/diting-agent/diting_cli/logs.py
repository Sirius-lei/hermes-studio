"""``diting logs`` — view and filter DiTing log files.
...
Usage examples::
    ...
    diting logs                    # last 50 lines of agent.log
    diting logs -f                 # follow agent.log in real time
    diting logs errors             # last 50 lines of errors.log
    diting logs gateway -n 100    # last 100 lines of gateway.log
    diting logs gui -f            # follow gui.log (dashboard/pty/ws)
    diting logs desktop -f        # follow desktop.log (Electron app boot/backend)
    diting logs --level WARNING    # only WARNING+ lines
    diting logs --session abc123   # filter by session ID substring
    diting logs --component tools  # only tool-related lines
    diting logs --since 1h         # lines from the last hour
    diting logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    log_path = get_diting_home() / "logs" / filename
    if not log_path.exists():
        print(f"Log file not found: {log_path}")
        print("(Logs are created when DiTing runs — try 'diting chat' first)")
        sys.exit(1)
    ...
    if follow:
        print(f"--- {display_diting_home()}/logs/{filename}{filter_desc} (Ctrl+C to stop) ---")
    else:
        print(f"--- {display_diting_home()}/logs/{filename}{filter_desc} (last {num_lines}) ---")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_diting_home()}/logs/")
        return

    print(f"Log files in {display_diting_home()}/logs/:\n")
    found = False
    ...
    if not found:
        print("  (no log files yet — run 'diting chat' to generate logs)")
