"""`diting checkpoints` CLI subcommand.

Gives users direct visibility and control over the filesystem checkpoint
store at ``~/.diting/checkpoints/``.  Actions:

    diting checkpoints               # same as `status`
    diting checkpoints status        # total size, project count, breakdown
    diting checkpoints list          # per-project checkpoint counts + workdir
    diting checkpoints prune [opts]  # force a sweep (ignores the 24h marker)
    diting checkpoints clear [-f]    # nuke the entire base (asks first)
    diting checkpoints clear-legacy  # delete just the legacy-* archives

Examples::

    diting checkpoints
    diting checkpoints prune --retention-days 3 --max-size-mb 200
    diting checkpoints clear -f
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if legacy:
        ...
        print()
        print("Clear with: diting checkpoints clear-legacy")
    return 0
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire subcommands onto the ``diting checkpoints`` parser."""
    parser.set_defaults(func=cmd_status)  # bare `diting checkpoints` → status
