...
the tool registry or launching DiTing (mirrors the decoupled-helper pattern used
elsewhere in the CLI).

Motivated by #38798: a config migration silently rewrote the valid toolset name
``diting-cli`` to the non-existent ``diting``. ``resolve_toolset('diting')``
...
def validate_platform_toolsets(
    ...
    1. A toolset name that ``is_valid_toolset`` rejects — usually a corrupted or
       renamed entry. When ``diting-<platform>`` would have been valid (the exact
       #38798 shape, where ``cli`` held ``diting`` instead of ``diting-cli``),
    ...
    if valid_count == 0:
        warnings.append(
            ...
            "have no tools. Run `diting tools` to reconfigure."
        )
    return warnings
