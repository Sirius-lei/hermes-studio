"""MCP picker — interactive `diting mcp picker` (also the default `diting mcp`).

Lists every catalog entry plus any custom MCP servers the user has added via
``diting mcp add``, lets them pick one, and routes to install / enable /
disable / uninstall / configure-tools flows.

Mirrors the `diting plugin` picker UX: arrow keys to navigate, ENTER on a row
...
from diting_cli.cli_output import prompt_yes_no
from diting_cli.curses_ui import curses_single_select
from diting_cli.mcp_catalog import (
    CatalogEntry,
    CatalogError,
    catalog_diagnostics,
    install_entry,
...
def _enable_disable(name: str, *, enable: bool) -> None:
    ...
    print(color(
        ...
        "Start a new DiTing session for changes to take effect.",
        Colors.GREEN,
    ))
...
def _configure_tools(name: str) -> None:
    ...
    """
    import argparse
    from diting_cli.mcp_config import cmd_mcp_configure
...
def _print_rows_text(rows: List[_Row]) -> None:
    ...
    as the default output of `diting mcp catalog`."""
    if not rows:
        print()
    ...
    print()
    print(color(
        "  Install: diting mcp install <name>    Picker: diting mcp",
        Colors.DIM,
    ))

    # Surface manifest-version warnings so users know when their DiTing is
    ...
    if future:
        ...
        for name, _, msg in future:
            print(color(
                f"  ⚠ '{name}' requires a newer DiTing — run `diting update` "
...
def show_catalog() -> None:
    """`diting mcp catalog` — print the curated list + custom servers, no interaction."""
    _print_rows_text(_build_rows())


def run_picker() -> None:
    """`diting mcp picker` (and default `diting mcp`) — interactive selector.
...
def install_by_name(identifier: str) -> int:
    """`diting mcp install <name>` — non-interactive entry-point.
    ...
    if entry is None:
        print(color(
            ...
            "Run `diting mcp catalog` to see available entries.",
            Colors.RED,
        ))
        return 1
    try:
