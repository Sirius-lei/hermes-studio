...
Defenses 1 and 2 (the ``_DiTing_GATEWAY`` guard on ``diting gateway
...
raw ``terminal("launchctl kickstart -k gui/<uid>/ai.diting.gateway")``,
...
State lives in ``<DiTing_HOME>/gateway/restart_loop.json`` so it is
...
def _state_path():
    return get_diting_home() / "gateway" / "restart_loop.json"
