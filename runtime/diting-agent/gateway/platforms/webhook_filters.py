...
def _resolve_profile_path(path_value: Any) -> Optional[Path]:
    """Resolve a user path, mapping ~/.diting to the active profile home."""
    ...
    if raw == "~/.diting":
        return diting_home
    if raw.startswith("~/.diting/"):
        return diting_home / raw.removeprefix("~/.diting/")
    path = Path(raw).expanduser()
    if path.is_absolute():
        return path
    return diting_home / path


def _resolve_script_path(script_value: Any) -> tuple[Optional[Path], Optional[str]]:
    """Resolve a route script under DiTing_HOME/scripts."""
    if not isinstance(script_value, str) or not script_value.strip():
        return None, "script path is empty"
    from diting_constants import get_diting_home

    scripts_root = (get_diting_home() / "scripts").resolve()
    raw_text = os.path.expandvars(script_value.strip())
    if raw_text == "~/.diting" or raw_text.startswith("~/.diting/"):
...
class WebhookRouteProcessor:
    ...
    def run_route_script(self, script_value: Any, payload: dict) -> tuple[bool, Optional[dict]]:
        ...
        if (
            ...
            or transformed.get("__diting_ignore__") is True
        ):
            return False, None
