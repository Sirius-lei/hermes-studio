...
class WhatsAppBehaviorMixin:
    ...
    DEFAULT_REPLY_PREFIX: str = "⚕ *DiTing Agent*\n────────────\n"
...
def resolve_whatsapp_bridge_dir() -> Path:
    """Resolve the WhatsApp bridge directory, mirroring to DiTing_HOME if needed.

    When the install tree is read-only (e.g., Docker /opt/diting), this function
    mirrors the bridge source to a writable DiTing_HOME location and returns that
    ...
    # Try DiTing_HOME location first
    diting_home = get_diting_home()
    diting_home_bridge = diting_home / "scripts" / "whatsapp-bridge"
    ...
    # Install dir is read-only, mirror to DiTing_HOME if needed
    if diting_home_bridge.exists():
        return diting_home_bridge

    # Mirror the bridge source to DiTing_HOME
    try:
        diting_home_bridge.parent.mkdir(parents=True, exist_ok=True)
        shutil.copytree(
            install_bridge,
            diting_home_bridge,
            dirs_exist_ok=False,
