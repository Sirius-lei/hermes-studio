...
def _get_diting_version() -> str:
    """Return the diting-agent package version, or 'dev' if unavailable."""
    try:
...
def build_user_agent() -> str:
    ...
    Format::
        ...
        QQBotAdapter/<qqbot_version> (Python/<py_version>; <os>; DiTing/<diting_version>)

    Example::

        QQBotAdapter/1.0.0 (Python/3.11.15; darwin; DiTing/0.9.0)
    """
    py_version = f"{sys.version_info.major}.{sys.version_info.minor}.{sys.version_info.micro}"
    os_name = platform.system().lower()
    diting_version = _get_diting_version()
    return f"QQBotAdapter/{QQBOT_VERSION} (Python/{py_version}; {os_name}; DiTing/{diting_version})"
