...
There are two ways to add a platform to the DiTing gateway:

## Plugin Path (Recommended for Community/Third-Party)

Create a plugin directory in `~/.diting/plugins/` (or under `plugins/platforms/`
...
**zero changes to core DiTing code**.
...
- `env_enablement_fn: () -> Optional[dict]` — seeds `PlatformConfig.extra`
  ...
  `diting gateway status` or `get_connected_platforms()` until the SDK
  instantiates.
...
- `plugin.yaml` `requires_env` / `optional_env` rich-dict entries —
  auto-populate `OPTIONAL_ENV_VARS` in `diting_cli/config.py` so the setup
...
Checklist for integrating a platform directly into the DiTing core.
...
And add it to the `diting-gateway` composite:

```python
"diting-gateway": {
    "includes": [..., "diting-your-platform"]
}
```
...
## 12. Status Display (`diting_cli/status.py`)
...
## 13. Gateway Setup Wizard (`diting_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ diting_cli/ toolsets.py \
  --include="*.py" -l | sort -u
