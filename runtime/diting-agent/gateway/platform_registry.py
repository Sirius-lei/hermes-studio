...
class PlatformEntry:
    ...
    # Env vars this platform needs (for ``diting setup`` display).
    ...
    # built-ins).  Used by ``diting gateway setup`` to auto-enable the
...
class PlatformRegistry:
    ...
    def __init__(self) -> None:
        ...
        # plugin-discovery time added several seconds to *every* `diting`
        # invocation -- including plain `diting chat`, which never touches any
        ...
        # `diting setup`/`gateway status`, send_message).
    ...
    def _resolve_all(self) -> None:
        ...
        gateway startup, ``diting setup``/``gateway status``, channel
