...
# 5. Symlinks the 'diting' CLI command into a user-facing bin dir
...
echo ""
echo -e "${CYAN}⚕ DiTing Agent Setup${NC}"
echo ""
...
if is_termux; then
    ...
    if [ -n "$UV_CMD" ]; then
        ...
        _uv_log="$(mktemp 2>/dev/null || echo "/tmp/diting-uv-install.$$.log")"
...
# ============================================================================
# PATH setup — symlink diting into a user-facing bin dir
# ============================================================================

echo -e "${CYAN}→${NC} Setting up diting command..."

DiTing_BIN="$SCRIPT_DIR/venv/bin/diting"
COMMAND_LINK_DIR="$(get_command_link_dir)"
COMMAND_LINK_DISPLAY_DIR="$(get_command_link_display_dir)"
mkdir -p "$COMMAND_LINK_DIR"
ln -sf "$DiTing_BIN" "$COMMAND_LINK_DIR/diting"
echo -e "${GREEN}✓${NC} Symlinked diting → $COMMAND_LINK_DISPLAY_DIR/diting"
...
# ============================================================================
# Seed bundled skills into ~/.diting/skills/
# ============================================================================

DiTing_SKILLS_DIR="${DiTing_HOME:-$HOME/.diting}/skills"
mkdir -p "$DiTing_SKILLS_DIR"

echo ""
echo "Syncing bundled skills to ~/.diting/skills/ ..."
if "$SCRIPT_DIR/venv/bin/python" "$SCRIPT_DIR/tools/skills_sync.py" 2>/dev/null; then
    echo -e "${GREEN}✓${NC} Skills synced"
else
    # Fallback: copy if sync script fails (missing deps, etc.)
    if [ -d "$SCRIPT_DIR/skills" ]; then
        cp -rn "$SCRIPT_DIR/skills/"* "$DiTing_SKILLS_DIR/" 2>/dev/null || true
...
echo "Other commands:"
echo "  diting status        # Check configuration"
if is_termux; then
    echo "  diting gateway       # Run gateway in foreground"
else
    echo "  diting gateway install # Install gateway service (messaging + cron)"
fi
echo "  diting cron list     # View scheduled jobs"
echo "  diting doctor        # Diagnose issues"
echo ""
...
if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
    ...
    "$SCRIPT_DIR/venv/bin/python" -m diting_cli.main setup
fi
