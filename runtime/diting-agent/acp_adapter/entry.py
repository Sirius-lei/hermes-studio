"""CLI entry point for the diting-agent ACP adapter.

Loads environment variables from ``~/.diting/.env``, configures logging
...
try:
    ...
    # shadowing DiTing's own modules — ``diting acp`` can be started from any
    # cwd, including a project that has same-named packages on its path.
    diting_bootstrap.harden_import_path()
...
def _load_env() -> None:
    """Load .env from DiTing_HOME (default ``~/.diting``)."""
    from diting_cli.env_loader import load_diting_dotenv

    diting_home = get_diting_home()
    loaded = load_diting_dotenv(diting_home=diting_home)
    if loaded:
        for env_file in loaded:
            logging.getLogger(__name__).info("Loaded env from %s", env_file)
    else:
        logging.getLogger(__name__).info(
            "No .env found at %s, using system env", diting_home / ".env"
        )
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        prog="diting-acp",
        description="Run DiTing Agent as an ACP stdio server.",
    )
    parser.add_argument("--version", action="store_true", help="Print DiTing version and exit")
    parser.add_argument(
        "--check",
        action="store_true",
    ...
    parser.add_argument(
        "--setup",
        action="store_true",
        help="Run interactive DiTing provider/model setup for ACP terminal auth",
    )
    parser.add_argument(
        "--setup-browser",
        action="store_true",
        help="Install agent-browser + Playwright Chromium into ~/.diting/node/ "
...
def _print_version() -> None:
    ...
    print(diting_version)


def _run_check() -> None:
    import acp  # noqa: F401
    from acp_adapter.server import DiTingACPAgent  # noqa: F401

    print("DiTing ACP check OK")


def _run_setup() -> None:
    from diting_cli.main import main as diting_main

    old_argv = sys.argv[:]
    try:
        sys.argv = [old_argv[0] if old_argv else "diting", "model"]
        diting_main()
    finally:
        sys.argv = old_argv
...
def _run_setup_browser(assume_yes: bool = False) -> int:
    ...
    with ``diting postinstall`` and the runtime lazy installer.
...
def main(argv: list[str] | None = None) -> None:
    ...
    logger.info("Starting diting-agent ACP adapter")
    ...
    import acp
    from .server import DiTingACPAgent
