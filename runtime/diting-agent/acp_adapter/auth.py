"""ACP auth helpers — detect and advertise DiTing authentication methods."""
...
TERMINAL_SETUP_AUTH_METHOD_ID = "diting-setup"


def detect_provider() -> Optional[str]:
    """Resolve the active DiTing runtime provider, or None if unavailable.
...
def has_provider() -> bool:
    """Return True if DiTing can resolve any runtime provider credentials."""
    return detect_provider() is not None


def build_auth_methods() -> list[Any]:
    """Return registry-compatible ACP auth methods for DiTing.
    ...
    not have DiTing provider credentials configured yet, so DiTing always
    ...
    if provider:
        methods.append(
            AuthMethodAgent(
                ...
                description=(
                    "Authenticate DiTing using the currently configured "
