"""ACP session manager — maps ACP sessions to DiTing AIAgent instances.

Sessions are persisted to the shared SessionDB (``~/.diting/state.db``) so they
...
def _translate_acp_cwd(cwd: str) -> str:
    """Translate Windows ACP cwd values when DiTing itself is running in WSL.

    Windows ACP clients can launch ``diting acp`` inside WSL while still sending
...
def _normalize_cwd_for_compare(cwd: str | None) -> str:
    ...
    from diting_constants import windows_path_to_wsl
...
def _register_task_cwd(task_id: str, cwd: str) -> None:
    ...
    Zed can launch DiTing from a Windows workspace while the ACP process runs
...
def _expand_acp_enabled_toolsets(
    ...
    expanded: List[str] = []
    for name in list(toolsets or ["diting-acp"]):
...
@dataclass
class SessionState:
    """Tracks per-session state for an ACP-managed DiTing agent."""
...
class SessionManager:
    """Thread-safe manager for ACP sessions backed by DiTing AIAgent instances.
    ...
    def __init__(self, agent_factory=None, db=None):
        ...
        Args:
            agent_factory: Optional callable that creates an AIAgent-like object.
                           ...
                           using the current DiTing runtime provider configuration.
            db:            Optional SessionDB instance. When omitted, the default
                           SessionDB (``~/.diting/state.db``) is lazily created.
        """
    ...
    def _get_db(self):
        ...
        Note: we resolve ``DiTing_HOME`` dynamically rather than relying on
        ...
        later (e.g. by the test fixture ``_isolate_diting_home``).
        """
        ...
        try:
            ...
            self._db_instance = SessionDB(db_path=diting_home / "state.db")
            return self._db_instance
        except Exception:
    ...
    def _make_agent(
        ...
        *,
        ...
        # editor/session cwd instead of the DiTing daemon's process cwd.
