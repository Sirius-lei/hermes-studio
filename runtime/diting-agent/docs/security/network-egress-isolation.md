...
When running DiTing inside Docker, the default `network_mode: host` gives the
...
The DiTing [SECURITY.md](../../SECURITY.md) §2 defines the trust model. The
...
#
# Usage:
#   DiTing_UID=$(id -u) DiTing_GID=$(id -g) docker compose up -d
#
...
services:
  gateway:
    ...
    environment:
      ...
      - NO_PROXY=diting,diting-dashboard,localhost
...
docker compose exec gateway \
  curl -sf --max-time 5 http://diting-dashboard:9119/health && echo "OK: internal reachable" || echo "FAIL"
...
- [SECURITY.md](../../SECURITY.md) — DiTing trust model and vulnerability reporting
