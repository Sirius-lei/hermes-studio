...
This document is the formal interface between the **DiTing gateway** (Python,
...
- **Registration.** The gateway registers a **wake URL** at enroll/provision —
  ...
  a dashboard host). Self-hosted: `diting gateway enroll --wake-url <url>` (or
...
**per-gateway secret** (`diting gateway enroll` → connector `/relay/enroll`, or
...
driven by the managed Portal / `diting` CLI.
