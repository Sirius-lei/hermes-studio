...
The canonical transcript store is SQLite via `SessionDB` (from `diting_state`). The
...
Counts consecutive restarts via a JSON file (`{DiTing_HOME}/restart_counts.json`). If a
...
This prevents unwanted auto-resets after `diting update`, `diting gateway restart`,
or `/restart`.
