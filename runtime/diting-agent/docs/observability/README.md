# DiTing Observer Hooks

DiTing observer hooks are the read-only telemetry contract for plugins that
...
They do not replace DiTing' planner, model providers, memory, tool registry,
...
Hook callbacks are fail-open. DiTing catches callback exceptions, logs a
...
payload construction is gated behind `has_hook(...)`, so DiTing only builds
