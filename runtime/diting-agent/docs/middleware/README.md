...
DiTing middleware is the behavior-changing companion to observer hooks.
...
selection, or handoff to runtimes such as NeMo Relay without changing DiTing'
planner, model provider adapters, tool registry, memory, or CLI UX.

With middleware enabled, plugins can:

- Rewrite LLM provider request kwargs before DiTing calls the provider.
- Rewrite tool arguments before guardrails, approval checks, hooks, and tool
  execution see them.
- Wrap the actual LLM execution callback while preserving DiTing retry,
  streaming, interrupt, and hook behavior.
- Wrap the actual tool execution callback while preserving DiTing guardrails,
...
- `telemetry_schema_version`: currently `diting.observer.v1`
- `middleware_schema_version`: currently `diting.middleware.v1`
...
DiTing stores those trace entries in later observer hook payloads as
`middleware_trace`.
...
If multiple plugins register the same execution middleware kind, DiTing runs
them as a nested chain in registration order. Middleware failures are fail-open:
DiTing logs a warning and continues with the next middleware or the base
runtime path.
...
For each provider request, DiTing applies middleware in this order:
...
For each tool call, DiTing applies middleware in this order:

1. Parse and coerce model-provided tool arguments.
2. Apply `tool_request` middleware.
3. Run the normal DiTing pre-execution path against the effective arguments:
...
```bash
diting plugins enable <plugin-name>
```

For isolated local testing, use one `DiTing_HOME` for plugin enablement and the
agent run:

```bash
export DiTing_HOME=/tmp/diting-middleware-test
mkdir -p "$DiTing_HOME"
diting plugins enable <plugin-name>
diting chat --query 'Reply exactly ok'
```
...
```bash
uv sync
uv run diting plugins enable <plugin-name>
uv run diting chat --query 'Reply exactly ok'
```
...
def tag_llm_request(**kwargs):
    ...
    extra_body.setdefault("metadata", {})["diting_middleware_demo"] = True
...
def normalize_terminal_workdir(**kwargs):
    ...
    args.setdefault("workdir", "/tmp/diting-middleware-demo")
    return {
        "args": args,
        "source": "middleware-demo",
...
Return the same response shape DiTing expects from the provider adapter. Do not
...
- If execution middleware raises before calling `next_call(...)`, DiTing treats
...
- If execution middleware calls `next_call(...)` successfully and then raises
  during post-processing, DiTing preserves the downstream result and does not
  run the provider or tool a second time.
- If downstream provider or tool execution fails, middleware may let that error
  propagate or translate it deliberately. DiTing does not convert downstream
