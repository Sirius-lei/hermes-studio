...
DiTing supports multiple gateway processes running concurrently — one per profile
...
needed. On every other profile gateway, add to `~/.diting/config.yaml`:
...
Or set the env var: `DiTing_KANBAN_DISPATCH_IN_GATEWAY=false`
