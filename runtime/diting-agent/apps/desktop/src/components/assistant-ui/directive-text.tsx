...
const DiTing_REF_TYPES = ['file', 'folder', 'url', 'image', 'tool', 'line', 'terminal', 'session'] as const
type DiTingRefType = (typeof DiTing_REF_TYPES)[number]
...
const ICON_PATHS: Record<DiTingRefType, string[]> = {
  file: [
...
const iconPathsFor = (type: string) => ICON_PATHS[type as DiTingRefType] ?? ICON_FALLBACK
...
const DiTing_DIRECTIVE_RE = new RegExp(
...
export const ditingDirectiveFormatter: Unstable_DirectiveFormatter = {
...
function parseDirectiveText(text: string): Unstable_DirectiveSegment[] {
  const matches = [
    ...
    ...Array.from(text.matchAll(DiTing_DIRECTIVE_RE)).map(match => {
      ...
      return {
        ...
        type: match[1] || 'file',
        label: shortLabel(match[1] as DiTingRefType, id),
        id
      }
    })
  ]
...
function shortLabel(type: DiTingRefType, id: string): string {
  if (type === 'terminal') {
    return id || 'terminal'
  }
...
function safeDirectiveSegments(text: string): Unstable_DirectiveSegment[] {
  try {
    return [...DiTingDirectiveFormatter.parse(text)]
  } catch {
...
/**
 * Renders text containing DiTing directives (`@file:...`, `@image:...`) as
...
const DirectiveImage: FC<{ id: string; label: string }> = ({ id, label }) => {
  ...
  useEffect(() => {
    ...
    const load =
      window.DiTingDesktop && isRemoteGateway() ? gatewayMediaDataUrl(id) : window.DiTingDesktop?.readFileDataUrl(id)
