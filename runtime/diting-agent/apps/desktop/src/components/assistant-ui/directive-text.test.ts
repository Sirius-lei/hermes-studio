...
import { formatRefValue, ditingDirectiveFormatter } from './directive-text'
...
describe('ditingDirectiveFormatter.parse', () => {
  it('keeps quoted file paths whole when parsing', () => {
    const segments = ditingDirectiveFormatter.parse('see @image:`apple-touch-icon (1).png` for the icon')
  ...
  it('still parses unquoted paths', () => {
    const segments = ditingDirectiveFormatter.parse('@file:src/main.tsx the entry point')
