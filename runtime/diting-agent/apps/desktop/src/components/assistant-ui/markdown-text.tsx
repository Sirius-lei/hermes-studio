...
async function mediaSrc(path: string): Promise<string> {
  ...
  if (window.DiTingDesktop && ['audio', 'video'].includes(mediaKind(path))) {
  ...
  if (window.DiTingDesktop && isRemoteGateway()) {
    return gatewayMediaDataUrl(path)
  }

  if (!window.DiTingDesktop?.readFileDataUrl) {
    return mediaExternalUrl(path)
  }

  return window.DiTingDesktop.readFileDataUrl(filePathFromMediaPath(path))
}
...
function MarkdownLink({ children, className, href, ...props }: ComponentProps<'a'>) {
  ...
  if (window.DiTingDesktop && text && normalizeExternalUrl(text) === target) {
