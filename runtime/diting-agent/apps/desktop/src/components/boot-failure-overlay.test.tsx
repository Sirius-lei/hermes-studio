...
function failBoot() {
  $desktopBoot.set({
    error: 'Could not connect to DiTing gateway',
    fakeMode: false,
    message: 'boot failed',
...
function stubDesktop(config: Record<string, unknown>) {
  const original = window.DiTingDesktop
  Object.defineProperty(window, 'ditingDesktop', {
    configurable: true,
    value: { getRecentLogs: async () => ({ lines: [] }), getConnectionConfig: async () => config }
  })

  return () => Object.defineProperty(window, 'ditingDesktop', { configurable: true, value: original })
}
