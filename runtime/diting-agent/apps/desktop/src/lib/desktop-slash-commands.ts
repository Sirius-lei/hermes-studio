...
/** Why a known DiTing command has no desktop UI surface. */
...
const DESKTOP_COMMAND_SPECS: readonly DesktopCommandSpec[] = [
  ...
  { name: '/profile', description: 'Switch the active DiTing profile', surface: action('profile') },
  ...
  { name: '/version', description: 'Show DiTing Agent version', surface: exec() },
...
function isKnownDiTingSlashCommand(command: string): boolean {
  const normalized = normalizeCommand(command)
 ...
 * DiTing' built-in slash commands — i.e. skill commands (`/gif-search`,
...
export function isDesktopSlashExtensionCommand(command: string): boolean {
  ...
  return !isKnownDiTingSlashCommand(normalized)
}
