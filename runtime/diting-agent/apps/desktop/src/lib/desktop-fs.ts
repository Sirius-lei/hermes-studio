import type {
  DiTingConnection,
  DiTingReadDirResult,
  DiTingReadFileTextResult,
  DiTingSelectPathsOptions
} from '@/global'
...
function connectionCacheKey(connection: DiTingConnection | null) {
  if (!connection) {
    return 'local:'
  }
...
export async function readDesktopDir(path: string): Promise<DiTingReadDirResult> {
  if (!isDesktopFsRemoteMode()) {
    return bridge().readDir(path)
  }

  return remoteFsApi<DiTingReadDirResult>(fsPath('list', path))
}

export async function readDesktopFileText(path: string): Promise<DiTingReadFileTextResult> {
  if (!isDesktopFsRemoteMode()) {
    return bridge().readFileText(path)
  }

  return remoteFsApi<DiTingReadFileTextResult>(fsPath('read-text', path))
}
...
export async function selectDesktopPaths(options?: DiTingSelectPathsOptions): Promise<string[]> {
  const desktop = bridge()
