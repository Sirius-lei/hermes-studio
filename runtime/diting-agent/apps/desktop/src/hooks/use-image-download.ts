...
function isMissingIpcHandler(error: unknown): boolean {
  ...
  return message.includes("No handler registered for 'diting:saveImageFromUrl'")
}
...
export function useImageDownload(src?: string) {
  ...
  const download = useCallback(async () => {
    ...
    try {
      if (window.DiTingDesktop?.saveImageFromUrl) {
        if (await window.DiTingDesktop.saveImageFromUrl(src)) {
