...
declare global {
  interface Window {
    ditingDesktop: {
      ...
      getConnection: (profile?: string | null) => Promise<DiTingConnection>
      ...
      // DiTing Cloud: one portal login powers discovery + silent per-agent
      ...
      profile: {
        ...
        // backend under the new DiTing_HOME (reloads the window). Pass null to
        // clear the preference.
        set: (name: string | null) => Promise<DesktopActiveProfile>
      }
      api: <T>(request: DiTingApiRequest) => Promise<T>
      notify: (payload: DiTingNotification) => Promise<boolean>
      requestMicrophoneAccess: () => Promise<boolean>
      readFileDataUrl: (filePath: string) => Promise<string>
      readFileText: (filePath: string) => Promise<DiTingReadFileTextResult>
      selectPaths: (options?: DiTingSelectPathsOptions) => Promise<string[]>
      writeClipboard: (text: string) => Promise<boolean>
      saveImageFromUrl: (url: string) => Promise<boolean>
      saveImageBuffer: (data: ArrayBuffer | Uint8Array, ext: string) => Promise<string>
      saveClipboardImage: () => Promise<string>
      getPathForFile: (file: File) => string
      normalizePreviewTarget: (target: string, baseDir?: string) => Promise<DiTingPreviewTarget | null>
      watchPreviewFile: (url: string) => Promise<DiTingPreviewWatch>
      stopPreviewFileWatch: (id: string) => Promise<boolean>
      setTitleBarTheme?: (payload: DiTingTitleBarTheme) => void
      ...
      readDir: (path: string) => Promise<DiTingReadDirResult>
      ...
      git?: {
        worktreeList: (repoPath: string) => Promise<DiTingGitWorktree[]>
        worktreeAdd: (
          repoPath: string,
        ...
        branchList: (repoPath: string) => Promise<DiTingGitBranch[]>
        ...
        baseBranchList: (repoPath: string) => Promise<DiTingGitBaseBranch[]>
        ...
        repoStatus: (repoPath: string) => Promise<DiTingRepoStatus | null>
        ...
        review: {
          list: (repoPath: string, scope: DiTingReviewScope, baseRef?: null | string) => Promise<DiTingReviewList>
          diff: (
            repoPath: string,
            filePath: string,
            scope: DiTingReviewScope,
            baseRef?: null | string,
            staged?: boolean
          ) => Promise<string>
          ...
          shipInfo: (repoPath: string) => Promise<DiTingReviewShipInfo>
      ...
      terminal: {
        ...
        onExit: (id: string, callback: (payload: DiTingTerminalExit) => void) => () => void
        resize: (id: string, size: { cols: number; rows: number }) => Promise<boolean>
        start: (options?: { cols?: number; cwd?: string; rows?: number }) => Promise<DiTingTerminalSession>
      ...
      onWindowStateChanged?: (callback: (payload: DiTingWindowState) => void) => () => void
      onFocusSession?: (callback: (sessionId: string) => void) => () => void
      onNotificationAction?: (callback: (payload: { actionId: string; sessionId?: string }) => void) => () => void
      onPreviewFileChanged: (callback: (payload: DiTingPreviewFileChanged) => void) => () => void
...
export interface DiTingTerminalSession {
  cwd: string
  id: string
  shell: string
}

export interface DiTingTerminalExit {
  code: number | null
  signal: string | null
}
...
export interface DesktopUpdateApplyResult {
   ...
   *  `diting update` themselves. `command` is the exact line to run. */
  manual?: boolean
  command?: string
  ditingRoot?: string
...
export interface DiTingConnection {
  baseUrl: string
...
export interface DiTingTitleBarTheme {
  background: string
  foreground: string
}

export interface DiTingWindowState {
  isFullscreen: boolean
  nativeOverlayWidth: number
  windowButtonPosition: { x: number; y: number } | null
}
...
export interface DesktopConnectionConfig {
  envOverride: boolean
  // The saved connection mode. 'cloud' is a DiTing Cloud connection: it carries
  ...
  remoteUrl: string
  // For a 'cloud' connection: the persisted DiTing Cloud org (slug or id) the
...
export interface DesktopConnectionConfigInput {
  ...
  remoteToken?: string
  remoteUrl?: string
  // For a 'cloud' connection: the selected DiTing Cloud org (slug or id) to
...
export interface DesktopCloudStatus {
  ...
  // the diting gateway session cookies. See cookiesHavePrivySession.
  signedIn: boolean
}

// A discovered DiTing Cloud agent — the trimmed DTO from NAS GET /api/agents.
...
export interface DiTingApiRequest {
  path: string
  method?: string
  body?: unknown
  timeoutMs?: number
...
export interface DiTingNotification {
  title?: string
  body?: string
  silent?: boolean
  kind?: string
...
export interface DiTingPreviewTarget {
  binary?: boolean
  byteSize?: number
  kind: 'file' | 'url'
  label: string
...
export interface DiTingReadFileTextResult {
  binary?: boolean
  byteSize?: number
  language?: string
  mimeType?: string
...
export interface DiTingPreviewWatch {
  id: string
  path: string
}
...
export interface DiTingGitWorktree {
  path: string
  branch: null | string
  isMain: boolean
  detached: boolean
...
export interface DiTingGitBranch {
  name: string
  checkedOut: boolean
  isDefault: boolean
...
// preselect it.
export interface DiTingGitBaseBranch {
  name: string
  isRemote: boolean
  isDefault: boolean
}
...
export interface DiTingRepoStatusFile {
  path: string
  staged: boolean
  unstaged: boolean
  untracked: boolean
...
export interface DiTingRepoStatus {
  branch: null | string
...
// turn began.
export type DiTingReviewScope = 'branch' | 'lastTurn' | 'uncommitted'

// One changed file in the review pane (status letter, +/- lines, staged flag).
export interface DiTingReviewFile {
  path: string
  added: number
  removed: number
...
export interface DiTingReviewList {
  files: DiTingReviewFile[]
...
export interface DiTingReviewPr {
  url: string
  state: string
  number: number
}
...
export interface DiTingReviewShipInfo {
  ghReady: boolean
  pr: DiTingReviewPr | null
}

export interface DiTingReadDirEntry {
  name: string
  path: string
  isDirectory: boolean
}

export interface DiTingReadDirResult {
  entries: DiTingReadDirEntry[]
  error?: string
}

export interface DiTingPreviewFileChanged {
  id: string
  path: string
  url: string
}

export interface DiTingSelectPathsOptions {
  title?: string
  defaultPath?: string
  directories?: boolean
  multiple?: boolean
