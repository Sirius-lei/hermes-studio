...
import { getDiTingConfig, getDiTingConfigDefaults } from '@/diting'
...
interface DiTingConfigOptions {
  activeSessionIdRef: MutableRefObject<string | null>
  refreshProjectBranch: (cwd: string) => Promise<void>
}

export function useDiTingConfig({ activeSessionIdRef, refreshProjectBranch }: DiTingConfigOptions) {
  const [voiceMaxRecordingSeconds, setVoiceMaxRecordingSeconds] = useState(DEFAULT_VOICE_SECONDS)
  const [sttEnabled, setSttEnabled] = useState(true)

  const refreshDiTingConfig = useCallback(async () => {
    try {
      const [config, defaults] = await Promise.all([getDiTingConfig(), getDiTingConfigDefaults().catch(() => ({}))])
  ...
  return { refreshDiTingConfig, sttEnabled, voiceMaxRecordingSeconds }
}
