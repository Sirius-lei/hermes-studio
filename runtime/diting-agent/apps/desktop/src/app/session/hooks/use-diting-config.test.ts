...
import { getDiTingConfig } from '@/diting'
import { persistString } from '@/lib/storage'
import { $currentCwd, setCurrentCwd } from '@/store/session'

import { useDiTingConfig } from './use-diting-config'

vi.mock('@/diting', () => ({
  getDiTingConfig: vi.fn(),
  getDiTingConfigDefaults: vi.fn().mockResolvedValue({})
}))

const WORKSPACE_CWD_KEY = 'diting.desktop.workspace-cwd'

const mockConfig = (config: Record<string, unknown>) =>
  vi.mocked(getDiTingConfig).mockResolvedValue(config as Awaited<ReturnType<typeof getDiTingConfig>>)

describe('useDiTingConfig refreshDiTingConfig', () => {
  beforeEach(() => {
  ...
  it('applies terminal.cwd from config even when localStorage has a stale value', async () => {
    ...
    await act(async () => {
      await result.current.refreshDiTingConfig()
    })
