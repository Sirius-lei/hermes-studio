...
import { getSessionMessages, type SessionMessage, triggerCronJob } from '../diting'
...
import { useDiTingConfig } from './session/hooks/use-diting-config'
...
export function DesktopController() {
  ...
  useEffect(() => {
    window.DiTingDesktop?.setPreviewShortcutActive?.(Boolean(chatOpen && (filePreviewTarget || previewTarget)))
  }, [chatOpen, filePreviewTarget, previewTarget])

  useEffect(() => {
    startUpdatePoller()
    const unsubscribe = window.DiTingDesktop?.onOpenUpdatesRequested?.(() => openUpdatesWindow())
  ...
  useEffect(() => {
    const unsubscribe = window.DiTingDesktop?.onFocusSession?.(sessionId => {
      if (sessionId) {
  ...
  useEffect(() => {
    const unsubscribe = window.DiTingDesktop?.onNotificationAction?.(({ actionId, sessionId }) => {
  ...
  // diting:// deep links (e.g. a docs "Send to App" button for an automation blueprint).
  ...
  useEffect(() => {
    const unsubscribe = window.DiTingDesktop?.onDeepLink?.(payload => {
    ...
    void window.DiTingDesktop?.signalDeepLinkReady?.()
  ...
  useEffect(() => {
    ...
    const unsubscribe = window.DiTingDesktop?.onClosePreviewRequested?.(closeActiveRightRailTab)
  ...
  const { refreshDiTingConfig, sttEnabled, voiceMaxRecordingSeconds } = useDiTingConfig({
    activeSessionIdRef,
  ...
  useEffect(() => {
    if (gatewayState === 'open' && !activeSessionId && freshDraftReady) {
      void refreshCurrentModel()
      void refreshDiTingConfig()
    }
  }, [activeSessionId, freshDraftReady, gatewayState, refreshCurrentModel, refreshDiTingConfig])
