import { isGatewayReauthRequired, resolveGatewayWsUrl } from '@diting/shared'
import { useEffect, useRef } from 'react'

import type { DiTingConnection } from '@/global'
import { DiTingGateway } from '@/diting'
...
import type { RpcEvent } from '@/types/diting'
...
interface GatewayBootOptions {
  ...
  onConnectionReady: (
    connection: Awaited<ReturnType<NonNullable<typeof window.DiTingDesktop>['getConnection']>> | null
  ) => void
  onGatewayReady: (gateway: DiTingGateway | null) => void
  refreshDiTingConfig: () => Promise<void>
...
export function useGatewayBoot({
  ...
  useEffect(() => {
    ...
    const publish = (next: DiTingConnection | null) => {
    ...
    // composer stayed disabled forever on "Starting DiTing...". Once the
    ...
    const attemptReconnect = async () => {
      ...
      try {
        ...
        // "Starting DiTing…". The probe is a no-op for a healthy or local backend.
        ...
        // as an opaque "Could not connect to DiTing gateway". resolveGatewayWsUrl
        ...
        await callbacksRef.current.refreshDiTingConfig().catch(() => undefined)
    ...
    const offBootProgress = desktop.onBootProgress(payload => {
      // Soft switch / post-boot startDiTing re-emits progress — ignore so the
    ...
    const gateway = new DiTingGateway()
    ...
    async function boot() {
      try {
        ...
        await callbacksRef.current.refreshDiTingConfig()
