...
// bug that drives the REAL useGatewayBoot hook + REAL DiTingGateway through a
...
function Harness() {
  useGatewayBoot({
    ...
    refreshDiTingConfig: async () => undefined,
...
beforeEach(() => {
  ...
  ;(window as { ditingDesktop?: unknown }).DiTingDesktop = fakeDesktop()
...
afterEach(() => {
  ...
  delete (window as { ditingDesktop?: unknown }).DiTingDesktop
})
...
describe('useGatewayBoot remote reconnect loop (real hook, fake socket)', () => {
  it('INITIAL boot against a dead VPS: getConnection hangs (waitForDiTing) → app sits in the connecting combo, then fails', async () => {
    // The report's actual path: a fresh launch pointed at an unreachable VPS.
    // startDiTing()'s remote branch awaits waitForDiTing() for 45s before it
    ...
    ;(window as { ditingDesktop?: unknown }).DiTingDesktop = desktop
    ...
    // After ~45s waitForDiTing gives up and getConnection rejects → boot()
    // catch → failDesktopBoot → the BootFailureOverlay recovery surface.
    await act(async () => {
      rejectConn(new Error('DiTing backend did not become ready: timeout'))
      await vi.advanceTimersByTimeAsync(0)
    })
