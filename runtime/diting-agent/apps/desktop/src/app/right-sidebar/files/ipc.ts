...
import type { DiTingReadDirEntry, DiTingReadDirResult } from '@/global'
...
export type ProjectTreeEntry = DiTingReadDirEntry
...
function ignoredBy(rules: GitignoreRule[], entry: DiTingReadDirEntry) {
...
async function filterIgnored(entries: DiTingReadDirEntry[], rootPath: string, dirPath: string) {
...
export async function readProjectDir(dirPath: string, rootPath = dirPath): Promise<DiTingReadDirResult> {
  if (!window.DiTingDesktop) {
