...
import type { DiTingReadDirResult } from '@/global'
import { $connection } from '@/store/session'

import { clearProjectDirCache, readProjectDir } from './ipc'
import { resetProjectTreeState, useProjectTree } from './use-project-tree'

const readDir = vi.fn<(path: string) => Promise<DiTingReadDirResult>>()

beforeEach(() => {
  $connection.set(null)
  resetProjectTreeState()
  readDir.mockReset()
  ;(window as unknown as { ditingDesktop: { readDir: typeof readDir } }).DiTingDesktop = { readDir }
})
...
afterEach(() => {
  ...
  $connection.set(null)
  resetProjectTreeState()
  delete (window as unknown as { ditingDesktop?: unknown }).DiTingDesktop
})

function ok(entries: { name: string; path: string; isDirectory: boolean }[]): DiTingReadDirResult {
  return { entries }
}
...
describe('useProjectTree', () => {
  ...
  it('reads gitignore from the real path while caching per connection', async () => {
    ...
    ;(window as unknown as { ditingDesktop: unknown }).DiTingDesktop = { gitRoot, readDir, readFileDataUrl }
  ...
  it('dedupes concurrent loadChildren calls for the same id', async () => {
    ...
    let resolveChildren: ((value: DiTingReadDirResult) => void) | undefined
    readDir.mockImplementationOnce(
      () =>
        new Promise<DiTingReadDirResult>(resolve => {
  ...
  it('falls back to the sanitized workspace dir when the session cwd is gone', async () => {
    ...
    ;(window as unknown as { ditingDesktop: unknown }).DiTingDesktop = { readDir, sanitizeWorkspaceCwd }
  ...
  it('returns no-bridge gracefully when window.DiTingDesktop is missing', async () => {
    delete (window as unknown as { ditingDesktop?: unknown }).DiTingDesktop
