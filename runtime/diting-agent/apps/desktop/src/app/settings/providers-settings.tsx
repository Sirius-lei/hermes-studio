...
import { disconnectOAuthProvider, listOAuthProviders } from '@/diting'
...
import type { EnvVarInfo, OAuthProvider } from '@/types/diting'
...
const canRunInTerminal = () => typeof window !== 'undefined' && Boolean(window.DiTingDesktop?.terminal)
...
//      in diting_cli/provider_catalog.py) — the SAME provider identity
//      `diting model` uses. This is authoritative: a provider tagged by the
...
function ConnectedProviderRow({
  ...
  // DiTing can clear this provider's creds via the API.
  const canDisconnect = provider.disconnectable ?? provider.flow !== 'external'
  // External (CLI-managed) provider DiTing can't clear via the API, but ships a
...
export function ProvidersSettings({ onClose, onViewChange, view }: ProvidersSettingsProps) {
  ...
  // DiTing never deletes creds another tool owns behind a silent API call.
