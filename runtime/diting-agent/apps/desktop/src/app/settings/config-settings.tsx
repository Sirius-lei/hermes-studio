...
import { getElevenLabsVoices, getDiTingConfigSchema, saveDiTingConfig } from '@/diting'
import { useI18n } from '@/i18n'
import { cn } from '@/lib/utils'
import { notify, notifyError } from '@/store/notifications'
import type { ConfigFieldSchema, DiTingConfigRecord } from '@/types/diting'

import { setDiTingConfigCache, useDiTingConfigRecord } from '../hooks/use-config-record'
...
export function voiceFieldVisible(key: string, config: DiTingConfigRecord): boolean {
...
export function ConfigSettings({
  ...
  const [config, setConfig] = useState<DiTingConfigRecord | null>(null)
  const { data: loadedConfig, isError: configLoadFailed, refetch: refetchConfig } = useDiTingConfigRecord()
  ...
  const {
    ...
    queryKey: ['diting-config-schema'],
    queryFn: getDiTingConfigSchema,
    staleTime: 5 * 60 * 1000
  })
  ...
  const updateConfig = (next: DiTingConfigRecord) => {
