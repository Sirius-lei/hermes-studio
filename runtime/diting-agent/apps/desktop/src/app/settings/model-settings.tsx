...
import { invalidateDiTingConfig, setDiTingConfigCache, useDiTingConfigRecord } from '../hooks/use-config-record'
...
// DiTing' reasoning levels (VALID_REASONING_EFFORTS); `none` = thinking off.
// Empty config = DiTing default (medium), shown as Medium.
...
// backend now surfaces the full `diting model` universe (every canonical
...
// Mirrors `_AUX_TASK_SLOTS` in diting_cli/web_server.py. Friendly labels and
...
export function ModelSettings({ onMainModelChanged }: ModelSettingsProps) {
  ...
  const { data: config } = useDiTingConfigRecord()
  const setConfig = setDiTingConfigCache
  ...
  const activateApiKeyProvider = useCallback(async () => {
    ...
    try {
      ...
      // `diting model` curation). Best-effort — fall through to the refreshed
        ...
        {needsSetup && !setupIsApiKey && (
          <p className="mt-2 text-xs text-muted-foreground">
            {selectedProviderRow?.auth_type === 'api_key'
              ...
              : `${selectedProviderRow?.name} signs in through your browser — DiTing runs the flow for you.`}
          </p>
        )}
