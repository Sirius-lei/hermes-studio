...
import type { DiTingGitBranch } from '@/global'
...
const branchActionLabel = (branch: DiTingGitBranch, copy: BranchActionCopy) => {
...
// inside it. Naming is explicit — no auto-generated `diting/work-<ts>` trees.
...
export function StartWorkButton({ repoPath, onStarted }: { repoPath: string; onStarted: (path: string) => void }) {
  ...
  const [branches, setBranches] = useState<DiTingGitBranch[]>([])
  ...
  const convert = async (branch: DiTingGitBranch) => {
    if (pending || !repoPath || !branch) {
      return
    }
