        ...
        <>
          <a
            ...
            onClick={event => {
              ...
              void window.DiTingDesktop?.openExternal(error.url)
            }}
          >
...
export function PreviewPane({
  ...
  const restartServer = useCallback(async () => {
    ...
    // "Ask DiTing to restart the server" looked like it did nothing —
  ...
  useEffect(() => {
    if (
      ...
      !window.DiTingDesktop?.watchPreviewFile ||
      !window.DiTingDesktop?.onPreviewFileChanged
    ) {
      return
    }
    ...
    const unsubscribe = window.DiTingDesktop.onPreviewFileChanged(payload => {
    ...
    void window.DiTingDesktop
      ...
      .then(watch => {
        if (!active) {
          void window.DiTingDesktop?.stopPreviewFileWatch?.(watch.id)
    ...
    return () => {
      ...
      if (watchId) {
        void window.DiTingDesktop?.stopPreviewFileWatch?.(watchId)
      }
    }
  ...
  useEffect(() => {
    ...
    webview.setAttribute('partition', 'persist:diting-preview')
