...
import type { DiTingReviewFile, DiTingReviewShipInfo } from '@/global'
...
//
// Scope is always "uncommitted" — DiTing' flow is agent edits you review BEFORE
...
const OPEN_KEY = 'diting.desktop.reviewOpen'
const COMMIT_DEFAULT_KEY = 'diting.desktop.reviewCommitDefault'
const TREE_MODE_KEY = 'diting.desktop.reviewTreeMode'
const SELECTED_KEY = 'diting.desktop.reviewSelectedPath'
...
export const $reviewFiles = atom<DiTingReviewFile[]>([])
...
export const $reviewShipInfo = atom<DiTingReviewShipInfo>({ ghReady: false, pr: null })
...
type ReviewBridge = NonNullable<NonNullable<NonNullable<Window['ditingDesktop']>['git']>['review']>
...
export async function selectReviewFile(file: DiTingReviewFile): Promise<void> {
...
export async function createOrOpenPr(): Promise<void> {
  ...
  if (existing?.url) {
    void window.DiTingDesktop?.openExternal?.(existing.url)
  ...
  await runShip(async () => {
    ...
    if (url) {
      void window.DiTingDesktop?.openExternal?.(url)
    }
