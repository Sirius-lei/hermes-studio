...
import type { DiTingRepoStatus } from '@/global'

import { $repoStatus, refreshRepoStatus } from './coding-status'
import { $currentCwd } from './session'

const sampleStatus: DiTingRepoStatus = {
...
function stubProbe(impl: (cwd: string) => Promise<DiTingRepoStatus | null>) {
  ;(window as unknown as { ditingDesktop?: unknown }).DiTingDesktop = { git: { repoStatus: impl } }
}
