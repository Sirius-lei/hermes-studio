...
import { getProfiles, setApiRequestProfile, STARTUP_REQUEST_TIMEOUT_MS } from '@/diting'
...
import type { ProfileInfo } from '@/types/diting'
...
// /api/profiles/active `current`). "default" is the root ~/.diting. This is the
...
const PROFILE_ORDER_STORAGE_KEY = 'diting.desktop.profileOrder'
...
const PROFILE_COLORS_STORAGE_KEY = 'diting.desktop.profileColors'
...
export async function refreshActiveProfile(): Promise<void> {
  try {
    const res = await window.DiTingDesktop.api<ActiveProfileResponse>({
...
// Persist the choice and relaunch the backend under the new DiTing_HOME. The
...
export async function switchProfile(name: string): Promise<void> {
  ...
  await window.DiTingDesktop.profile.set(name)
}
...
async function syncConnectionToActiveProfile(profile: string): Promise<void> {
  const getConnection = window.DiTingDesktop?.getConnection
...
const SHOW_ALL_PROFILES_STORAGE_KEY = 'diting.desktop.showAllProfiles'
...
// Switch to the default (root ~/.diting) profile — bound to ⌘1.
...
export function touchActiveGatewayBackend(): void {
  ...
  void window.DiTingDesktop?.touchBackend?.(target).catch(() => undefined)
}
