...
import { getDiTingConfigRecord, saveDiTingConfig } from '@/diting'
...
export async function setAutoSpeakReplies(enabled: boolean): Promise<void> {
  ...
  try {
    const record = await getDiTingConfigRecord()
    const voice = record.voice && typeof record.voice === 'object' ? (record.voice as Record<string, unknown>) : {}

    await saveDiTingConfig({ ...record, voice: { ...voice, auto_tts: enabled } })
  } catch (error) {
