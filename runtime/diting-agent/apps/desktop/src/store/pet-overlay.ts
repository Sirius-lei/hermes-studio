 ...
 * app's bounds and stays visible while DiTing is minimized. That window carries
...
const OVERLAY_ACTIVE_KEY = 'diting.desktop.pet-overlay-active.v1'
const OVERLAY_BOUNDS_KEY = 'diting.desktop.pet-overlay-bounds.v1'
...
function pushNow(): void {
  window.DiTingDesktop?.petOverlay?.pushState(currentPayload())
}
...
function openOverlay(request: PetOverlayOpenRequest): void {
  const api = window.DiTingDesktop?.petOverlay
...
export function restorePetOverlay(): void {
  if (!window.DiTingDesktop?.petOverlay || !$petOverlayActive.get() || stateUnsubs.length) {
    return
  }
...
export function popInPet(): void {
  ...
  stateUnsubs = []
  $petOverlayActive.set(false)
  void window.DiTingDesktop?.petOverlay?.close()
}
