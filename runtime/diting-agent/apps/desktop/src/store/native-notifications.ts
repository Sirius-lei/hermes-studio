...
const STORAGE_KEY = 'diting:native-notifications'
...
// "Backgrounded" = the user isn't on DiTing. `document.hidden` only flips when
...
export function dispatchNativeNotification(input: NativeNotificationInput): void {
  ...
  void window.DiTingDesktop?.notify({
...
export async function sendTestNativeNotification(title: string, body: string): Promise<boolean> {
  const bridge = window.DiTingDesktop
