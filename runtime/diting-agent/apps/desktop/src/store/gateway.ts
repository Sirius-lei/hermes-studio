import { type ConnectionState, type GatewayEvent, resolveGatewayWsUrl } from '@diting/shared'
...
const isOpen = (gateway: DiTingGateway | null): boolean => gateway?.connectionState === 'open'
...
export const $gateway = atom<DiTingGateway | null>(null)
...
let primaryGateway: DiTingGateway | null = null
let primaryProfile = 'default'

export function setPrimaryGateway(gateway: DiTingGateway | null, profile = 'default'): void {
...
export function activeGateway(): DiTingGateway | null {
...
export async function ensureActiveGatewayOpen(): Promise<DiTingGateway | null> {
  if (activeKey === primaryProfile) {
    return primaryGateway
  }
