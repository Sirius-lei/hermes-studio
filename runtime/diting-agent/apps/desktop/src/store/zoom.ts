...
export function setZoomPercent(percent: number): void {
  window.DiTingDesktop?.zoom?.setPercent(percent)
}

if (typeof window !== 'undefined' && window.DiTingDesktop?.zoom) {
  void window.DiTingDesktop.zoom.get().then(({ percent }) => $zoomPercent.set(percent))
  window.DiTingDesktop.zoom.onChanged(({ percent }) => $zoomPercent.set(percent))
}
