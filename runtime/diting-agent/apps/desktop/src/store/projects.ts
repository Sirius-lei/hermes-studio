...
import type { DiTingGitBaseBranch, DiTingGitBranch } from '@/global'
...
import type { ProjectInfo, ProjectsPayload } from '@/types/diting'
...
const PROJECT_SCOPE_KEY = 'diting.desktop.projectScope'
...
async function gatewayRequest<T>(method: string, params: Record<string, unknown> = {}): Promise<T> {
  ...
  if (!gateway) {
    throw new Error('DiTing gateway is not connected')
  }
...
// repo, returning where DiTing should start working. Git is the source of
...
export async function listRepoBranches(repoPath: string): Promise<DiTingGitBranch[]> {
  const git = desktopGit()
...
export async function listBaseBranches(repoPath: string): Promise<DiTingGitBaseBranch[]> {
  const git = desktopGit()
...
export async function revealPath(path: null | string): Promise<void> {
  if (path) {
    await window.DiTingDesktop?.revealPath?.(path)
  }
}
...
export async function copyPath(path: null | string): Promise<void> {
  if (path) {
    await window.DiTingDesktop?.writeClipboard?.(path)
  }
}
