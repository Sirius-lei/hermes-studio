...
import type { OAuthProvider } from '@/types/diting'
...
describe('refreshOnboarding', () => {
  ...
  it('does not downgrade configured=true on fallback-only readiness failures', async () => {
    ...
    window.localStorage.setItem('diting-desktop-onboarded-v1', '1')
    ...
    expect(window.localStorage.getItem('diting-desktop-onboarded-v1')).toBe('1')
  })
