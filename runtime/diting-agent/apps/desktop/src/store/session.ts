...
import type { SessionInfo, UsageStats } from '@/types/diting'

type Updater<T> = T | ((current: T) => T)

const WORKSPACE_CWD_KEY = 'diting.desktop.workspace-cwd'
...
const COMPOSER_MODEL_KEY = 'diting.desktop.composer.model'
const COMPOSER_PROVIDER_KEY = 'diting.desktop.composer.provider'
const COMPOSER_EFFORT_KEY = 'diting.desktop.composer.reasoning-effort'
const COMPOSER_FAST_KEY = 'diting.desktop.composer.fast'
...
const LAST_SESSION_KEY = 'diting.desktop.lastSessionId'
...
function workspaceCwdKey(connection: DiTingConnection | null = $connection.get()): string {
...
export async function syncConfiguredDefaultProjectDir(): Promise<string> {
  const settings = window.DiTingDesktop?.settings?.getDefaultProjectDir
...
export async function ensureDefaultWorkspaceCwd(): Promise<void> {
  const sanitize = window.DiTingDesktop?.sanitizeWorkspaceCwd
...
export const $connection = atom<DiTingConnection | null>(null)
...
export const setConnection = (next: Updater<DiTingConnection | null>) => updateAtom($connection, next)
