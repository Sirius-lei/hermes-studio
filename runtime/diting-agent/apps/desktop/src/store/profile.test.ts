...
const { getProfiles } = await import('@/diting')
...
const profile = (name: string, isDefault = false): ProfileInfo => ({
  ...
  model: null,
  name,
  path: `/tmp/diting/${name}`,
  provider: null,
  skill_count: 0
})

const remoteConn = (over: Partial<DiTingConnection> = {}): DiTingConnection =>
  ({ baseUrl: 'https://diting-roy.tail.ts.net', mode: 'remote', profile: 'vps-remote', ...over }) as DiTingConnection

const localConn = (over: Partial<DiTingConnection> = {}): DiTingConnection =>
  ({ baseUrl: '', mode: 'local', profile: 'default', ...over }) as DiTingConnection

const getConnection = vi.fn<(profile?: string | null) => Promise<DiTingConnection>>()
...
beforeEach(() => {
  ...
  $profiles.set([])
  vi.stubGlobal('window', { ditingDesktop: { getConnection } })
