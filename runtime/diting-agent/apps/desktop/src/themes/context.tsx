...
const SKIN_KEY = 'diting-desktop-theme-v2'
const MODE_KEY = 'diting-desktop-mode-v1'
...
const PROFILE_SKINS_KEY = 'diting-desktop-profile-themes-v1'
const PROFILE_MODES_KEY = 'diting-desktop-profile-modes-v1'
// Last active profile, recorded so the boot-time paint can pick that profile's
// theme before the gateway reports which profile actually launched.
const LAST_PROFILE_KEY = 'diting-desktop-active-profile-v1'
...
function applyTheme(theme: DesktopTheme, mode: 'light' | 'dark') {
  ...
  root.dataset.DiTingTheme = skinName
  root.dataset.DiTingMode = rendered
  ...
  window.DiTingDesktop?.setTitleBarTheme?.({
    background: chromeBg,
  ...
  try {
    window.localStorage.setItem('diting-boot-background', chromeBg)
    window.localStorage.setItem('diting-boot-color-scheme', rendered)
  } catch {
  ...
  if (typo.fontUrl && !INJECTED_FONT_URLS.has(typo.fontUrl)) {
    ...
    link.dataset.DiTingThemeFont = 'true'
...
const syncNativeTheme = (pref: ThemeMode, rendered: 'light' | 'dark') =>
  window.DiTingDesktop?.setNativeTheme?.(pref === 'system' ? 'system' : rendered)
...
/** Sync the desktop skin with the active DiTing backend theme on connect. */
