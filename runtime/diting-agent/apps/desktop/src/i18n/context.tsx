...
import { getDiTingConfigRecord, type DiTingConfigRecord, saveDiTingConfig } from '@/diting'
...
export interface I18nConfigClient {
  getConfig: () => Promise<DiTingConfigRecord>
  saveConfig: (config: DiTingConfigRecord) => Promise<{ ok: boolean }>
}

const defaultConfigClient: I18nConfigClient = {
  getConfig: () => {
    if (typeof window === 'undefined' || !window.DiTingDesktop?.api) {
      return Promise.resolve({})
    }

    return getDiTingConfigRecord()
  },
  saveConfig: config => {
    if (typeof window === 'undefined' || !window.DiTingDesktop?.api) {
    ...
    return saveDiTingConfig(config)
  }
}
...
export function getConfigDisplayLanguage(config: DiTingConfigRecord): unknown {
  return isRecord(config.display) ? config.display.language : undefined
}

export function withConfigDisplayLanguage(config: DiTingConfigRecord, locale: Locale): DiTingConfigRecord {
