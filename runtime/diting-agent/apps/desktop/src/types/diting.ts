...
export interface OAuthProvider {
   ...
   *  embedded terminal. Null when DiTing doesn't know how to remove it. */
...
export interface EnvVarInfo {
  ...
  // in diting_cli/provider_catalog.py). When present, the Keys tab groups by
  // this provider identity — the SAME one `diting model` uses — instead of
...
export type DiTingConfigRecord = Record<string, unknown>
...
export interface ModelOptionProvider {
   ...
   *  the `diting model` CLI / onboarding OAuth flow. */
  auth_type?: string
...
// folders. Mirrors diting_cli/projects_db.Project.to_dict().
...
export interface SkillInfo {
  ...
  usage?: number
  /** 'agent' = learned/local (editable), 'bundled' = ships with DiTing, 'hub' = installed. */
...
export interface ToolProvider {
   ...
   *  CLI `diting tools` active-provider detection). */
  is_active: boolean
}
 ...
 *  not DiTing); elsewhere, driver health from `cua-driver doctor`. `null`
...
/** Shape of `GET /api/diting/update/check` — the backend's own update state.
