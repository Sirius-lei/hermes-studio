 ...
 * product name (`DiTing`). If a PREVIOUS `npm run pack` was interrupted
 ...
 *   '.../release/linux-unpacked/electron' -> '.../release/linux-unpacked/DiTing'
 *
 * This is a hard failure with no obvious cause for the user — `diting desktop`
 ...
 * (the mac-unpacked DiTing.app bundle) and Windows (win-unpacked), so we
