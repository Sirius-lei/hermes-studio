...
// no longer bundles the DiTing Agent Python payload (that's fetched at first
// launch via install.ps1 / install.sh, per the Phase 1 thin-installer flow).
const APP = (() => {
  if (PLATFORM === 'darwin') {
    const appPath = path.join(RELEASE_ROOT, `mac-${ARCH}`, 'DiTing.app')
    return {
      appPath,
      binary: path.join(appPath, 'Contents', 'MacOS', 'DiTing'),
  ...
  if (PLATFORM === 'win32') {
    ...
    return {
      appPath: unpacked,
      binary: path.join(unpacked, 'DiTing.exe'),
  ...
  return {
    appPath: unpacked,
    binary: path.join(unpacked, 'DiTing'),
    resourcesPath: path.join(unpacked, 'resources'),
    asarPath: path.join(unpacked, 'resources', 'app.asar'),
    unpackedDistIndex: path.join(unpacked, 'resources', 'app.asar.unpacked', 'dist', 'index.html')
  }
...
// Default DiTing_HOME for non-sandboxed runs -- matches main.ts's
// resolveDiTingHome(). On Windows it's %LOCALAPPDATA%\diting; elsewhere
// it's ~/.diting. The fresh-install sandbox launchFresh() sets its own
// DiTing_HOME and never touches this.
const DEFAULT_DiTing_HOME = (() => {
  if (PLATFORM === 'win32' && process.env.LOCALAPPDATA) {
    return path.join(process.env.LOCALAPPDATA, 'diting')
  }
  return path.join(os.homedir(), '.diting')
})()
const VENV_ROOT = path.join(DEFAULT_DiTing_HOME, 'diting-agent', 'venv')
const FRESH_SANDBOX_ROOT = path.join(os.tmpdir(), 'diting-desktop-fresh-install')
...
function ensurePackagedApp() {
  if (process.env.DiTing_DESKTOP_SKIP_BUILD === '1' && exists(APP.binary)) {
    return
  }
...
function resolveDmgPath() {
  if (!exists(RELEASE_ROOT)) {
    return path.join(RELEASE_ROOT, `DiTing-${PACKAGE_JSON.version}-${ARCH}.dmg`)
  }

  const prefix = `DiTing-${PACKAGE_JSON.version}`
  const candidates = fs
  ...
  return candidates.length > 0
    ...
    : path.join(RELEASE_ROOT, `DiTing-${PACKAGE_JSON.version}-${ARCH}.dmg`)
}

function resolveNsisPath() {
  // electron-builder NSIS artifactName template is 'DiTing-${version}-${os}-${arch}.${ext}'
...
function ensureDmg() {
  ...
  if (process.env.DiTing_DESKTOP_SKIP_BUILD === '1' && exists(resolveDmgPath())) {
    return
  }
...
function ensureNsis() {
  ...
  if (process.env.DiTing_DESKTOP_SKIP_BUILD === '1' && resolveNsisPath()) {
    return
  }
...
function launchFresh() {
  ...
  const ditingHome = path.join(sandbox, 'diting-home')
  const cwd = path.join(sandbox, 'workspace')

  fs.mkdirSync(userDataDir, { recursive: true })
  fs.mkdirSync(ditingHome, { recursive: true })
  ...
  env.DiTing_DESKTOP_CWD = cwd
  env.DiTing_DESKTOP_IGNORE_EXISTING = '1'
  env.DiTing_DESKTOP_TEST_MODE = 'fresh-install'
  env.DiTing_DESKTOP_USER_DATA_DIR = userDataDir
  env.DiTing_HOME = ditingHome
  delete env.DiTing_DESKTOP_DiTing
  delete env.DiTing_DESKTOP_DiTing_ROOT
  ...
  console.log(`  DiTing_HOME: ${ditingHome}`)
  console.log(`  cwd: ${cwd}`)

  return { runtimeRoot: path.join(ditingHome, 'diting-agent', 'venv') }
}

// Validate the packaged bundle matches the thin-installer architecture:
//   - The DiTing Agent Python payload is NOT shipped (it's fetched at first
...
function validateBundle() {
  ...
  // present anymore. If a stray ship of diting_cli sneaks back in we want
  // to fail loudly rather than re-introduce the 400MB delta we just removed.
  const staleFactoryMarker = path.join(APP.resourcesPath, 'diting-agent', 'diting_cli', 'main.py')
...
function help() {
  console.log(`Usage:
  npm run test:desktop:existing  # build packaged app, launch with normal PATH/existing DiTing
  npm run test:desktop:fresh     # build packaged app, launch with temp userData + DiTing_HOME
...
Fast rerun (skip rebuild if the packaged app already exists):
  DiTing_DESKTOP_SKIP_BUILD=1 npm run test:desktop:fresh
`)
}
