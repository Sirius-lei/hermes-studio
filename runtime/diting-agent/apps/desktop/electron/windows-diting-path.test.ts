// Unit tests for the pure Windows `diting` resolution helpers extracted from
// main.ts's findOnPath(), handOffWindowsBootstrapRecovery(), and
// unwrapWindowsVenvDiTingCommand(). These pin the two Windows resolution bugs
// that caused desktop reinstall loops:
//   1. buildPathExtCandidates() — PATHEXT extensions must be tried BEFORE the
//      empty extension, or an extensionless Git-Bash `diting` shim shadows
//      the real diting.cmd/diting.exe.
//   2. chooseUpdaterArgs() — must gate on haveRealInstall (any real-install
//      signal), not just the diting.exe console-script shim, or healthy
//      installs get forced into a destructive --repair.
//   3. resolveVenvDiTingCommand() — must probe the venv python via
//      canImportDiTingCli() before trusting it, or a broken venv gets
...
function makeDeps(overrides: Partial<Parameters<typeof resolveVenvDiTingCommand>[2]> = {}) {
  return {
    isWindows: true,
    isCommandScript: () => false,
    fileExists: () => true,
    directoryExists: () => false,
    canImportDiTingCli: () => true,
    getVenvPython: (venvRoot: string) => `${venvRoot}/Scripts/python.exe`,
    getVenvSitePackagesEntries: () => [],
    buildDesktopBackendEnv: () => ({ FAKE_ENV: '1' }),
    ditingHome: '/fake/diting-home',
...
test('resolveVenvDiTingCommand: returns null off Windows', () => {
  const deps = makeDeps({ isWindows: false })

  assert.equal(resolveVenvDiTingCommand('/root/venv/Scripts/diting.exe', [], deps), null)
})

test('resolveVenvDiTingCommand: returns null for a .cmd/.bat script command', () => {
  const deps = makeDeps({ isCommandScript: () => true })

  assert.equal(resolveVenvDiTingCommand('/root/venv/Scripts/diting.cmd', [], deps), null)
})

test('resolveVenvDiTingCommand: returns null when the basename is not diting/diting.exe', () => {
  const deps = makeDeps()

  assert.equal(resolveVenvDiTingCommand('/root/venv/Scripts/python.exe', [], deps), null)
})

test('resolveVenvDiTingCommand: returns null when the parent dir is not Scripts', () => {
  const deps = makeDeps()

  assert.equal(resolveVenvDiTingCommand('/root/venv/bin/diting.exe', [], deps), null)
})

test('resolveVenvDiTingCommand: returns null when the venv python does not exist on disk', () => {
  const deps = makeDeps({ fileExists: () => false })

  assert.equal(resolveVenvDiTingCommand('/root/venv/Scripts/diting.exe', [], deps), null)
})

test('resolveVenvDiTingCommand: probes the venv python before trusting it (returns null on failed probe)', () => {
  let probed = false

  const deps = makeDeps({
    canImportDiTingCli: (python: string) => {
      probed = true
  ...
  const result = resolveVenvDiTingCommand('/root/venv/Scripts/diting.exe', ['serve'], deps)
...
test('resolveVenvDiTingCommand: returns the resolved python backend descriptor when the probe passes', () => {
  const deps = makeDeps()
  const result = resolveVenvDiTingCommand('/root/venv/Scripts/diting.exe', ['serve', '--port', '0'], deps)
  ...
  assert.deepEqual(result.args, ['-m', 'diting_cli.main', 'serve', '--port', '0'])
...
test('resolveVenvDiTingCommand: is case-insensitive on diting.exe and the Scripts dir name', () => {
  const deps = makeDeps()

  assert.ok(resolveVenvDiTingCommand('/root/venv/Scripts/DiTing.EXE', [], deps))
  assert.ok(resolveVenvDiTingCommand('/root/venv/SCRIPTS/diting.exe', [], deps))
})
