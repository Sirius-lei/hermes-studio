...
function mkTmpHome() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'diting-bootstrap-test-'))
}
...
test('runBootstrap bails immediately when the signal is already aborted', async () => {
  ...
  const result = await runBootstrap({
    installStamp: null,
    activeRoot: '/tmp/diting-runner-test',
    sourceRepoRoot: null,
    ditingHome: '/tmp/diting-runner-test',
    logRoot: '/tmp/diting-runner-test',
...
test('installedAgentInstallScript resolves the installer in the agent checkout', () => {
  ...
  try {
    ...
    const scriptsDir = path.join(home, 'diting-agent', 'scripts')
...
test('existing checkout detection requires git metadata', () => {
  ...
  try {
    const activeRoot = path.join(home, 'diting-agent')
...
test('fresh bootstrap args include the packaged commit pin', () => {
  ...
  assert.deepEqual(
    buildPosixPinArgs({
      installStamp,
      activeRoot: '/tmp/diting-agent',
      ditingHome: '/tmp/diting'
    }),
    [
      '--dir',
      '/tmp/diting-agent',
...
test('existing-checkout bootstrap args keep branch but skip the packaged commit pin', () => {
  ...
  assert.deepEqual(
    ...
    ['--dir', '/tmp/diting-agent', '--diting-home', '/tmp/diting', '--branch', 'main']
  )
})
