// Profile-delete routing logic for the `diting:api` IPC handler.
//
...
// pool backend -- spawning a fresh one would call ensure_diting_home() and
...
/**
 * Parse a `diting:api` request into the profile name a DELETE targets, or
...
/**
 * Route the next `diting:api` request away from the primary/window backend
 ...
 * ensure_diting_home() recreates the directory the delete just removed.
 */
