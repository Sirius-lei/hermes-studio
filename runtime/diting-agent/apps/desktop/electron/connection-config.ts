 ...
 *     `X-DiTing-Session-Token` header; WS uses `?token=`.
...
// diting_cli/dashboard_auth/cookies.py.
//
// Two cookies are in play (see that module):
//   - diting_session_at: the OAuth access token. Short-lived (~15 min); its
//     Max-Age tracks the access-token TTL, so the cookie jar drops it the
//     instant the AT expires.
//   - diting_session_rt: the OAuth refresh token. Long-lived (24h rotating,
//     reuse-detected — Portal NAS #293 / diting #37247). When the AT cookie
...
const AT_COOKIE_VARIANTS = ['__Host-diting_session_at', '__Secure-diting_session_at', 'diting_session_at']
const RT_COOKIE_VARIANTS = ['__Host-diting_session_rt', '__Secure-diting_session_rt', 'diting_session_rt']

// The Nous portal (NAS) does NOT use DiTing gateway session cookies — it is a
...
// DiTing Cloud connection (cloud-auto-discovery Q3/Q6): it carries a
...
function pathWithGlobalRemoteProfile(path, profile, opts: any = {}) {
  ...
  try {
    parsed = new URL(rawPath, 'http://diting.local')
  } catch {
    return path
  }
...
/**
 * True if any cookie in `cookies` is a diting session ACCESS-token cookie
 ...
 * the DiTing gateway session cookies, so cloud sign-in / discovery liveness
