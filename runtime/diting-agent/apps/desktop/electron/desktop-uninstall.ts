 ...
 * the three user-facing uninstall modes to the `diting uninstall` CLI flags,
...
/**
 * Map an uninstall mode to the `python -m diting_cli.uninstall` argv (after the
 * python executable). Uses the dedicated lightweight module entrypoint (not
 * `diting_cli.main`) so it can run under a system Python OUTSIDE the venv that
...
function uninstallArgsForMode(mode) {
  ...
  return ['-m', 'diting_cli.uninstall', '--mode', mode]
}
 ...
 *
 *   macOS:   …/DiTing.app/Contents/MacOS/DiTing  → …/DiTing.app
 *   Windows: …\DiTing\DiTing.exe                 → …\DiTing  (install dir)
...
function resolveRemovableAppPath(execPath, platform, env: any = {}) {
  ...
  if (platform === 'darwin') {
    // …/DiTing.app/Contents/MacOS/DiTing → strip 3 segments to the .app
    const macOsDir = p.dirname(exe) // …/Contents/MacOS
    const contents = p.dirname(macOsDir) // …/Contents
    const appBundle = p.dirname(contents) // …/DiTing.app
  ...
  if (platform === 'win32') {
    // NSIS per-user installs DiTing.exe directly in the install dir.
    const dir = p.dirname(exe)

    if (/[\\/]DiTing$/i.test(dir) || /[\\/]diting-desktop$/i.test(dir)) {
      return dir
    }
  ...
  // Unpacked electron-builder tree: …/linux-unpacked/diting
  const dir = p.dirname(exe)
 ...
 * being deleted); `pythonPath` is prepended to PYTHONPATH so `import diting_cli`
 ...
 */
function buildPosixCleanupScript({ desktopPid, pythonExe, pythonPath, agentRoot, uninstallArgs, appPath, ditingHome }) {
  ...
  const lines = [
    ...
    '  done',
    'fi',
    `export DiTing_HOME=${q(ditingHome)}`
  ]
 ...
 * modes + `pythonPath`=agentRoot so `import diting_cli` resolves from source
...
function buildWindowsCleanupScript({
  ...
  // a problem, but DiTing install paths don't use them.
  const q = s => `"${String(s).replace(/"/g, '')}"`

  const lines = [
    '@echo off',
    'setlocal enableextensions',
    `set "DiTing_HOME=${String(ditingHome).replace(/"/g, '')}"`,
    `set "PID=${pid}"`
  ]
