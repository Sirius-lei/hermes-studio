// Backend subcommand routing for the desktop-managed DiTing process.
//
// The desktop app launches its own headless backend via `diting serve` — it
...
// app hasn't updated yet, or an older `diting` resolved from PATH) only knows
 ...
 * @param {string} [profile] optional DiTing profile to pin via `--profile`.
 */
 ...
 * `-m diting_cli.main` and any `--profile <name>`). Returns a copy; if there is
...
/**
 * True when a runtime's `diting_cli/subcommands/dashboard.py` source registers
