/**
 * windows-diting-path.ts
 *
 * Pure, dependency-injected pieces of Windows `diting` resolution pulled out
 * of main.ts's findOnPath(), handOffWindowsBootstrapRecovery(), and
 * unwrapWindowsVenvDiTingCommand(). Each of the three functions here pins one
 ...
 *      FIRST, so an extensionless Git-Bash `diting` shim shadowed the real
 *      diting.cmd/diting.exe; the shim then failed the --version probe and
 ...
 *      venv\Scripts\diting.exe (the console-script shim, written at the END
 ...
 *   3. resolveVenvDiTingCommand() — unwrapWindowsVenvDiTingCommand() returned
 ...
 * extensionless file (e.g. a Git-Bash shell-script shim named `diting`) must
 * not shadow `diting.cmd`/`diting.exe`. The empty entry is kept LAST so
 ...
 * signals (venv python interpreter, venv diting shim, bootstrap-complete
 * marker) — gating on just the diting.exe console-script shim alone is the
...
export interface ResolveVenvDiTingCommandDeps {
  isWindows: boolean
  isCommandScript: (command: string) => boolean
  fileExists: (filePath: string) => boolean
  directoryExists: (filePath: string) => boolean
  canImportDiTingCli: (python: string, opts?: { env?: Record<string, string> }) => boolean
...
/**
 * If `command` is a Windows venv `diting`/`diting.exe` console-script shim
 * (i.e. `<venvRoot>/Scripts/diting(.exe)`), resolve it to the underlying
 * venv python invoked as `python -m diting_cli.main <backendArgs>` — but
 * ONLY after smoke-testing that interpreter with canImportDiTingCli(). A
 * venv whose update died mid-`pip install` still has python.exe + diting.exe
 ...
 *
 * Returns null when `command` is not a venv diting shim, the underlying
 * python doesn't exist, or the import probe fails. Otherwise returns the
 * resolved backend descriptor.
 */
export function resolveVenvDiTingCommand(
  command: string,
  backendArgs: string[],
  deps: ResolveVenvDiTingCommandDeps
): {
  label: string
  command: string
  args: string[]
  ...
  if (!/^diting(?:\.exe)?$/i.test(basename(resolved))) {
    return null
  }
  ...
  if (
    ...
    rememberLog?.(
      `Ignoring venv DiTing at ${python}: runtime import probe failed (broken/partial venv); falling through to bootstrap.`
    )
  ...
  return {
    label: `existing DiTing Python at ${python}`,
    command: python,
