...
export const ZOOM_STORAGE_KEY = 'diting:desktop:zoomLevel'
 ...
 * but forgot to emit 'diting:zoom:changed', leaving the control stuck at 100%.
...
export function applyZoomLevel(webContents, level) {
  ...
  webContents.send('diting:zoom:changed', { level: clamped, percent: zoomLevelToPercent(clamped) })
