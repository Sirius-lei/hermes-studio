 ...
 * Background
 * ----------
 * After `diting update` + `diting desktop --build-only`, the freshly-rebuilt
...
// backend/profile/root it talks to). Anything DiTing_DESKTOP_* is preserved
// plus DiTing_HOME. We snapshot the values, not the live env, so the new
...
const PRESERVED_ENV_KEYS = ['DiTing_HOME', 'ELECTRON_DISABLE_SANDBOX']
const PRESERVED_ENV_PREFIXES = ['DiTing_DESKTOP_']
