 ...
 * bootstrap-runner.ts
 *
 * Drives apps/desktop's first-launch install of DiTing Agent by spawning
...
function bootstrapCacheDir(ditingHome) {
  return path.join(ditingHome, 'bootstrap-cache')
}

// The install.sh / install.ps1 that ships inside the already-installed agent
// checkout under ~/.diting/diting-agent. Used as a last-resort fallback when
// the pinned commit can't be fetched from GitHub (e.g. a locally-built desktop
// app stamped to an unpushed HEAD).
function installedAgentInstallScript(ditingHome) {
  if (!ditingHome) {
    return null
  }

  const candidate = path.join(ditingHome, 'diting-agent', 'scripts', installScriptName())
...
function cachedScriptPath(ditingHome, commit) {
  return path.join(bootstrapCacheDir(ditingHome), `install-${commit}.${process.platform === 'win32' ? 'ps1' : 'sh'}`)
}
...
function downloadInstallScript(commit, destPath) {
  ...
  const url = `https://raw.githubusercontent.com/NousResearch/diting-agent/${commit}/scripts/${scriptName}`
...
async function resolveInstallScript({
  ...
  const cached = cachedScriptPath(ditingHome, installStamp.commit)
  ...
  try {
    ...
    const installed = installedAgentInstallScript(ditingHome)
...
function spawnPowerShell(scriptPath, args, { emit, stageName, abortSignal, ditingHome }: any = {}) {
  return new Promise<any>((resolve, reject) => {
    ...
    const child = spawn(
      ...
      hiddenWindowsChildOptions({
        ...
        env: {
          ...process.env,
          // Pass DiTing_HOME through so install.ps1 respects the caller's
          // choice rather than re-computing the default.
          DiTing_HOME: ditingHome || process.env.DiTing_HOME || ''
        }
      })
    )
...
function spawnBash(scriptPath, args, { emit, stageName, abortSignal, ditingHome }: any = {}) {
  return new Promise<any>((resolve, reject) => {
...
function buildPosixPinArgs({ installStamp, activeRoot, ditingHome, pinCommit = true }) {
  const args = ['--dir', activeRoot, '--diting-home', ditingHome]
...
async function fetchManifest({
  ...
  const args = isPosix
    ? ['--manifest', ...buildPosixPinArgs({ installStamp, activeRoot, ditingHome, pinCommit })]
...
async function runStage({
  ...
  const args = isPosix
    ? [
        ...
        '--non-interactive',
        '--json',
        ...buildPosixPinArgs({ installStamp, activeRoot, ditingHome, pinCommit })
      ]
...
async function runBootstrap(opts) {
  ...
  const runLog = openRunLog(logRoot || path.join(ditingHome, 'logs'))
  ...
  try {
    ...
    const scriptInfo = await resolveInstallScript({ installStamp, sourceRepoRoot, ditingHome, emit })
