...
export function markerPath(ditingHome) {
  return path.join(ditingHome, '.diting-update-in-progress')
}
...
export function readLiveUpdateMarker(
  ...
  const file = markerPath(ditingHome)
  let raw
 ...
 *
 * The Tauri-based diting-setup.exe takes several seconds to initialise its
 ...
 */
export function writeUpdateMarker(ditingHome, pid, { now = Date.now } = {}) {
  const file = markerPath(ditingHome)
