...
import { buildDesktopBackendEnv, normalizeDiTingHomeRoot } from './backend-env'
import { canImportDiTingCli, verifyDiTingCli } from './backend-probes'
...
import { buildPathExtCandidates, chooseUpdaterArgs, getVenvSitePackagesEntries, resolveVenvDiTingCommand } from './windows-diting-path'
import { readWindowsUserEnvVar } from './windows-user-env'
import { isPackagedInstallPath as isPackagedInstallPathUnderRoots } from './workspace-cwd'
import { readWslWindowsClipboardImage } from './wsl-clipboard-image'

const USER_DATA_OVERRIDE = process.env.DiTing_DESKTOP_USER_DATA_DIR
...
const DEV_SERVER = process.env.DiTing_DESKTOP_DEV_SERVER
const IS_PACKAGED = app.isPackaged || Boolean(process.env.DiTing_DESKTOP_IS_PACKAGED)
...
// switches only apply pre-launch. Override with DiTing_DESKTOP_DISABLE_GPU
...
if (REMOTE_DISPLAY_REASON) {
  ...
  console.log(
    `[diting] remote display detected (${REMOTE_DISPLAY_REASON}); disabling GPU hardware acceleration to prevent flicker`
  )
}
...
if (IS_WSL && !REMOTE_DISPLAY_REASON && fs.existsSync('/dev/dxg')) {
  ...
  console.log('[diting] WSL GPU passthrough (/dev/dxg) detected; enabling GPU acceleration')
}

ipcMain.handle('diting:get-remote-display-reason', () => REMOTE_DISPLAY_REASON)
    ...
    try {
      ...
      if (parsed && typeof parsed === 'object' && typeof parsed.commit === 'string' && parsed.commit.length >= 7) {
        if (parsed.schemaVersion !== INSTALL_STAMP_SCHEMA_VERSION) {
          console.warn(
            `[diting] install-stamp.json schemaVersion ${parsed.schemaVersion} != expected ${INSTALL_STAMP_SCHEMA_VERSION}; ignoring`
          )
      ...
      console.warn(`[diting] install-stamp.json found at ${p} , but parsing failed with ${e}`)
...
if (INSTALL_STAMP) {
  console.log(
    `[diting] install stamp: ${INSTALL_STAMP.commit.slice(0, 12)}${INSTALL_STAMP.branch ? ` (${INSTALL_STAMP.branch})` : ''}${INSTALL_STAMP.dirty ? ' [DIRTY]' : ''} from ${INSTALL_STAMP.source || 'unknown'}`
  )
} else if (IS_PACKAGED) {
  ...
  console.error(
    '[diting] WARNING: no install-stamp.json found in packaged build. First-launch bootstrap will not have a pinned ref to install.'
  )
}

// DiTing_HOME — the user-facing root for everything DiTing-related. Mirrors
// scripts/install.ps1's $DiTingHome and scripts/install.sh's $DiTing_HOME.
//
// Defaults:
//   Windows: %LOCALAPPDATA%\diting (matches install.ps1)
//   macOS / Linux: ~/.diting (matches install.sh)
//
// Special case for Windows: if the user has a legacy ~/.diting directory
// (e.g., from a prior pip install or a manual setup) AND no
// %LOCALAPPDATA%\diting yet, prefer the legacy path so we don't orphan their
// existing config / sessions / .env. New installs go to %LOCALAPPDATA%.
//
// DiTing_DESKTOP_USER_DATA_DIR (used by test:desktop:fresh) puts the sandbox
// DiTing_HOME beneath the throwaway userData dir so a fresh-install run never
// touches the user's real ~/.diting / %LOCALAPPDATA%\diting.
function resolveDiTingHome() {
  if (process.env.DiTing_HOME) {
    return normalizeDiTingHomeRoot(process.env.DiTing_HOME)
  }

  if (USER_DATA_OVERRIDE) {
    return path.join(path.resolve(USER_DATA_OVERRIDE), 'diting-home')
  }

  if (IS_WINDOWS) {
    // A GUI app launched from Explorer inherits the environment block captured
    // at login, so a DiTing_HOME set via `setx` AFTER login is invisible in
    // process.env even though the CLI (a fresh shell) sees it. Without this the
    // backend silently falls back to %LOCALAPPDATA%\diting and reports "No
    // inference provider configured" despite a valid configured home (#45471).
    // Consult the live User-scoped registry value before the default below.
    const fromRegistry = readWindowsUserEnvVar('DiTing_HOME')

    if (fromRegistry) {
      return normalizeDiTingHomeRoot(fromRegistry)
    }
  }

  if (IS_WINDOWS && process.env.LOCALAPPDATA) {
    const localappdata = path.join(process.env.LOCALAPPDATA, 'diting')
    const legacy = path.join(app.getPath('home'), '.diting')

    // Migrate transparently to LOCALAPPDATA, but honour an existing legacy
    // ~/.diting setup (no LOCALAPPDATA install yet) so users don't lose state.
  ...
  return path.join(app.getPath('home'), '.diting')
}

const DiTing_HOME = resolveDiTingHome()

function ditingManagedNodePathEntries() {
  // NOTE: keep this ordering in sync with iter_diting_node_dirs() in
  // diting_constants.py — this Node main process cannot import the Python
  // module, so the platform-ordering rule is mirrored here.
  const root = path.join(DiTing_HOME, 'node')
...
function pathWithDiTingManagedNode(...entries) {
  return [...DiTingManagedNodePathEntries(), ...entries, process.env.PATH].filter(Boolean).join(path.delimiter)
}

// ACTIVE_DiTing_ROOT — the canonical mutable DiTing install. Same path
// install.ps1 / install.sh use, so a desktop-only user and a CLI-only user end
// up with identical layouts and can share one install.
const ACTIVE_DiTing_ROOT = path.join(DiTing_HOME, 'diting-agent')
// VENV_ROOT — venv lives inside the repo, exactly like install.ps1 does it.
const VENV_ROOT = path.join(ACTIVE_DiTing_ROOT, 'venv')
...
// boots, going straight to `resolveDiTingBackend()`. Missing or stale marker
...
//
// We deliberately put the marker INSIDE ACTIVE_DiTing_ROOT (not alongside)
...
const BOOTSTRAP_COMPLETE_MARKER = path.join(ACTIVE_DiTing_ROOT, '.diting-bootstrap-complete')
const BOOTSTRAP_MARKER_SCHEMA_VERSION = 1

const DESKTOP_CONNECTION_CONFIG_PATH = path.join(app.getPath('userData'), 'connection.json')
const DESKTOP_UPDATE_CONFIG_PATH = path.join(app.getPath('userData'), 'updates.json')
const DESKTOP_WINDOW_STATE_PATH = path.join(app.getPath('userData'), 'window-state.json')
// active-profile.json records which DiTing profile the desktop launches its
// local backend as. When set, startDiTing() passes `diting --profile <name>
// dashboard …`, which deterministically pins DiTing_HOME (see
// _apply_profile_override in diting_cli/main.py) and bypasses the sticky
// ~/.diting/active_profile file. Unset (null) preserves the legacy behavior:
// no --profile flag, so the backend honors active_profile / default.
const DESKTOP_PROFILE_CONFIG_PATH = path.join(app.getPath('userData'), 'active-profile.json')
// Mirrors diting_cli.profiles._PROFILE_ID_RE so we never hand the backend a
// value its profile resolver would reject and exit on.
const PROFILE_NAME_RE = /^[a-z0-9][a-z0-9_-]{0,63}$/
// Branch we track for self-update. The GUI work has merged to main, so this
// tracks main. User can also override at runtime via
// ditingDesktop.updates.setBranch().
const DEFAULT_UPDATE_BRANCH = 'main'
// desktop.log lives under DiTing_HOME/logs/ so it sits next to agent.log,
// errors.log, gateway.log produced by diting_logging.setup_logging — one log
// directory per user, regardless of which UI surface produced the line.
const DESKTOP_LOG_PATH = path.join(DiTing_HOME, 'logs', 'desktop.log')
...
//
// Mirror the Python logs (diting_logging.py RotatingFileHandler, maxBytes x
...
const BOOT_FAKE_MODE = process.env.DiTing_DESKTOP_BOOT_FAKE === '1'

const BOOT_FAKE_STEP_MS = (() => {
  const raw = Number.parseInt(String(process.env.DiTing_DESKTOP_BOOT_FAKE_STEP_MS || ''), 10)
...
const APP_NAME = process.env.DiTing_DESKTOP_APP_NAME || 'DiTing'
...
// 'diting:native-theme' ('dark' | 'light' | 'system'); we pin
...
// electron-builder derives from the build `appId` (com.nousresearch.diting) —
// keep this string in sync with package.json `build.appId`. macOS/Linux don't
// need this, so gate it on Windows. (Fixes: desktop approval/turn notifications
// never firing on Windows.)
if (IS_WINDOWS) {
  app.setAppUserModelId('com.nousresearch.diting')
}

// Seed the native About panel with the live DiTing version. This is refreshed
// on every open via the explicit "About" menu handler (refreshAboutPanel), so
// an in-place `diting update` mid-session is reflected without an app restart;
// the seed here just covers the first open and any non-menu invocation path.
app.setAboutPanelOptions({
  applicationName: APP_NAME,
  applicationVersion: resolveDiTingVersion(),
...
const MEDIA_PROTOCOL = 'diting-media'

// Only audio/video may be streamed. Without this the handler would read any
// non-blocklisted local file (no size cap) for any `fetch(diting-media://…)`.
...
let mainWindow = null
let ditingProcess = null
let connectionPromise = null
// True while connection-config:apply soft-rehomes the primary — suppresses the
// backend-exit toast so an intentional kill doesn't look like a crash.
let softRehomeInProgress = false
// Additional per-profile backends, keyed by profile name. The PRIMARY backend
// (the desktop's launch profile) stays managed by ditingProcess +
// connectionPromise + startDiTing(); this pool only holds EXTRA profile
...
const POOL_MAX_BACKENDS = Math.max(1, Number(process.env.DiTing_DESKTOP_POOL_MAX) || 3)
const POOL_IDLE_MS = Math.max(60_000, Number(process.env.DiTing_DESKTOP_POOL_IDLE_MS) || 10 * 60_000)
...
// onto the error so subsequent startDiTing() calls (e.g. the renderer's
// ensureGatewayOpen retrying after the WS won't open) return the same error
// instead of re-running install.ps1 in a hot loop. Cleared explicitly by
// the renderer's "Reload and retry" path or by quitting the app.
let bootstrapFailure = null
// Latched non-bootstrap backend spawn failure — stops getConnection() from
// respawning diting serve backend children in a tight loop while boot is broken.
...
let bootProgressState = {
  ...
  message: 'Waiting to start DiTing backend',
  phase: 'idle',
  progress: 0,
  running: false,
  timestamp: Date.now()
...
function rememberLog(chunk) {
  ...
  const lines = text.split(/\r?\n/).map(line => `[diting] ${line}`)
  ditingLog.push(...lines)

  if (ditingLog.length > 300) {
    ditingLog.splice(0, ditingLog.length - 300)
  }
...
function ensureWslWindowsFonts() {
  ...
  try {
    ...
    const confPath = path.join(confDir, '99-diting-wsl-windows-fonts.conf')
    let existing = ''
...
function broadcastBootProgress() {
  ...
  webContents.send('diting:boot-progress', bootProgressState)
}
...
//
// The snapshot is queryable via the diting:bootstrap:get IPC handler so a
...
function broadcastBootstrapEvent(ev) {
  ...
  webContents.send('diting:bootstrap:event', ev)
}
...
// The Tauri updater writes DiTing_HOME/.diting-update-in-progress for the whole
...
// (`force_kill_other_diting`, taskkill /IM diting.exe) kills it, the launch
...
async function waitForUpdateToFinish() {
  let marker = readLiveUpdateMarker(DiTing_HOME)
  ...
  while (marker && Date.now() < deadline) {
    await advanceBootProgress(
      'backend.update-wait',
      'An update is finishing — DiTing will start automatically when it completes…',
      12
    )
    await new Promise(r => setTimeout(r, UPDATE_WAIT_POLL_MS))
    marker = readLiveUpdateMarker(DiTing_HOME)
  }
...
function findOnPath(command) {
  ...
  // shell-script shim named `diting` — must not shadow `diting.cmd`/`diting.exe`.
...
function unwrapWindowsVenvDiTingCommand(command, backendArgs) {
  return resolveVenvDiTingCommand(command, backendArgs, {
...
// spawns `diting serve`; runtimes older than serve only have `dashboard`. We
...
// (covers a bare `diting` resolved from PATH with no known source root). Result
...
function backendSupportsServe(backend) {
  ...
  if (backend.root) {
    try {
      const src = fs.readFileSync(path.join(backend.root, 'diting_cli', 'subcommands', 'dashboard.py'), 'utf8')
  ...
  if (supported === null) {
    try {
      ...
      execFileSync(backend.command, [...prefix, 'serve', '--help'], {
        ...
        env: { ...process.env, DiTing_HOME, ...(backend.env || {}) },
        timeout: 15000,
        stdio: 'ignore',
        windowsHide: true
      })
...
function isDiTingSourceRoot(root) {
  return directoryExists(root) && fileExists(path.join(root, 'diting_cli', 'main.py'))
}

function findPythonForRoot(root) {
  const override = process.env.DiTing_DESKTOP_PYTHON
...
function findSystemPython() {
  ...
  // CPython but several DiTing deps (notably pywinpty's Rust-built
...
// findGitBash — locate bash.exe on Windows. DiTing' terminal tool requires
...
function findGitBash() {
  ...
  // install.ps1 drops PortableGit at %LOCALAPPDATA%\diting\git\... — checked
  ...
  if (localAppData) {
    candidates.push(path.join(localAppData, 'diting', 'git', 'bin', 'bash.exe'))
    candidates.push(path.join(localAppData, 'diting', 'git', 'usr', 'bin', 'bash.exe'))
  }
...
// normal DiTing_DASHBOARD_READY stdout line and no ready-file side channel is
// needed.
...
// install only has PortableGit under %LOCALAPPDATA%\diting\git (never on
...
function resolveGitBinary() {
  ...
  if (localAppData) {
    candidates.push(path.join(localAppData, 'diting', 'git', 'cmd', 'git.exe'))
    candidates.push(path.join(localAppData, 'diting', 'git', 'bin', 'git.exe'))
  }
...
function recentDiTingLog() {
  return ditingLog.slice(-20).join('\n')
}

// ─── Self-update (git-pull against the running backend's diting root) ──────
...
// Dev → SOURCE_REPO_ROOT. Packaged/CLI install → ACTIVE_DiTing_ROOT.
// DiTing_DESKTOP_DiTing_ROOT always wins so devs can pin a worktree.
function resolveUpdateRoot() {
  const candidates = [
    process.env.DiTing_DESKTOP_DiTing_ROOT && path.resolve(process.env.DiTing_DESKTOP_DiTing_ROOT),
    !IS_PACKAGED && isDiTingSourceRoot(SOURCE_REPO_ROOT) ? SOURCE_REPO_ROOT : null,
    isDiTingSourceRoot(ACTIVE_DiTing_ROOT) ? ACTIVE_DiTing_ROOT : null
  ].filter(Boolean)

  return candidates.find(c => directoryExists(path.join(c, '.git'))) || candidates[0] || ACTIVE_DiTing_ROOT
}
...
function emitUpdateProgress(payload) {
  ...
  for (const window of BrowserWindow.getAllWindows()) {
    window.webContents.send('diting:updates:progress', merged)
  }
}
...
// DiTing_HOME/diting-setup.exe on a successful install (see
// apps/bootstrap-installer paths::copy_self_to_diting_home). That binary owns
// ALL repo mutation — running `diting update` + rebuilding the desktop — so
...
function resolveUpdaterBinary() {
  const name = IS_WINDOWS ? 'diting-setup.exe' : 'diting-setup'
  const candidate = path.join(DiTing_HOME, name)
...
// Path to the venv shim whose lock decides whether `diting update` can write
// fresh entry points. On Windows this is the file the running backend
// `diting.exe` holds open; on POSIX it's never mandatory-locked.
function venvDiTingShimPath(updateRoot) {
  return IS_WINDOWS
    ? path.join(updateRoot, 'venv', 'Scripts', 'diting.exe')
    : path.join(updateRoot, 'venv', 'bin', 'diting')
}
...
// only signals the direct child, so on Windows a backend `diting.exe` that
// spawned its own grandchildren (a `diting` REPL, a pty terminal session, the
...
// `ditingProcess.kill('SIGTERM')` + `app.quit()` fire-and-forget: SIGTERM on
// Windows doesn't reap the backend's grandchildren, and quit didn't wait for
// teardown, so the updater raced a still-locked `diting.exe`, the quarantine
...
// problem: the desktop's backend (and the grandchildren IT spawned — a diting
// REPL, a pty terminal, the gateway) keep `diting.exe` and other files in the
...
async function releaseBackendLock(updateRoot, tag) {
  ...
  if (ditingProcess && Number.isInteger(ditingProcess.pid)) {
    pids.push(ditingProcess.pid)
  }
  ...
  if (ditingProcess && !ditingProcess.killed) {
    try {
      ditingProcess.kill('SIGTERM')
    } catch {
      void 0
    }
  }
  ...
  const shim = venvDiTingShimPath(updateRoot)
  ...
  while (Date.now() < deadlineMs) {
    ...
    if (ditingProcess && Number.isInteger(ditingProcess.pid)) {
      stragglers.push(ditingProcess.pid)
    }
...
// `diting update`). Instead we spawn the staged DiTing-Setup binary with
// --update and quit, so it can run `diting update` (which refuses while we
...
async function applyUpdates(opts = {}) {
  ...
  try {
    ...
    if (!updater && !IS_WINDOWS) {
      // macOS/Linux drag-install: no staged Tauri diting-setup. Unlike Windows
      ...
      // whole update itself: `diting update` (backend) + `diting desktop
    ...
    if (!updater) {
      ...
      // `diting desktop`, never the Tauri installer that self-copies
      // diting-setup.exe into DiTing_HOME). They DO have a working `diting`
      ...
      // checkout they're on — bare `diting update` defaults to main and would
      ...
      try {
        ...
        if (head.code === 0 && current && current !== 'HEAD') {
          ...
          if (branch !== 'main') {
            command = `diting update --branch ${branch}`
          }
        }
      } catch {
        // Best-effort: fall back to bare `diting update` if branch detection fails.
      }

      rememberLog(`[updates] no staged updater; surfacing manual \`${command}\` for CLI install at ${updateRoot}`)
      emitUpdateProgress({ stage: 'manual', message: command, percent: null })

      return { ok: true, manual: true, command, ditingRoot: updateRoot }
    }

    emitUpdateProgress({
      stage: 'restart',
      message:
        'Updating DiTing — this window will close and the updater will open. Don’t reopen DiTing yourself; it restarts automatically when the update finishes.',
      percent: 100
    })
    ...
    // diting.exe (held by the backend child / its grandchildren) and the update
    ...
    if (!lock.unlocked) {
      ...
      // terminal running diting, an unkillable child). Handing off anyway
      ...
      const message =
        'Update aborted: another process is holding the DiTing install open ' +
        '(a second DiTing window or a terminal running diting?). Close it and retry.'
    ...
    // `diting update` will run (the venv shim is locked while we live).
    const child = spawn(updater, updaterArgs, {
      cwd: DiTing_HOME,
      env: {
        ...process.env,
        DiTing_HOME,
        PATH: pathWithDiTingManagedNode(venvBin)
      },
      detached: true,
      stdio: 'ignore',
      windowsHide: false
    ...
    if (Number.isInteger(child.pid)) {
      writeUpdateMarker(DiTing_HOME, child.pid)
    }
...
async function handOffWindowsBootstrapRecovery(reason) {
  ...
  const venvBin = path.join(updateRoot, 'venv', IS_WINDOWS ? 'Scripts' : 'bin')
  const venvDiTing = path.join(venvBin, IS_WINDOWS ? 'diting.exe' : 'diting')
  ...
  // not just the `diting.exe` console-script shim. That shim is generated at the
  ...
  const haveRealInstall =
    fileExists(venvPython) || fileExists(venvDiTing) || fileExists(path.join(updateRoot, '.diting-bootstrap-complete'))
...
// Resolve the diting CLI to drive an in-app update: prefer the venv shim in
// the install we're updating, fall back to `diting` on PATH.
function resolveDiTingCliBinary(updateRoot) {
  const venvDiTing = path.join(updateRoot, 'venv', 'bin', 'diting')

  if (fileExists(venvDiTing)) {
    return venvDiTing
  }

  return findOnPath('diting') || null
}
...
// macOS/Linux in-app update: backend (`diting update`) + OS-aware GUI rebuild
// (`diting desktop --build-only`), then atomically swap the running .app bundle
...
async function applyUpdatesPosixInApp(opts: any) {
  const updateRoot = resolveUpdateRoot()
  const diting = resolveDiTingCliBinary(updateRoot)

  if (!diting) {
    emitUpdateProgress({ stage: 'manual', message: 'diting update', percent: null })

    return { ok: true, manual: true, command: 'diting update', ditingRoot: updateRoot }
  }

  // Put the DiTing-managed Node and the venv on PATH so `diting desktop`'s
  // npm build can find them on a machine with no system Node. Windows portable
  // Node lives directly under %LOCALAPPDATA%\diting\node, not node\bin.
  const env: Record<string, string> = {
    DiTing_HOME,
    PATH: pathWithDiTingManagedNode(path.join(updateRoot, 'venv', 'bin'))
  }

  // `diting update` reaps stale `diting serve` backends (a code update
  ...
  // _kill_stale_dashboard_processes reads DiTing_DESKTOP_CHILD_PID and excludes
  ...
  if (ditingProcess && Number.isInteger(ditingProcess.pid)) {
    desktopChildPids.push(ditingProcess.pid)
  }
  ...
  if (desktopChildPids.length) {
    env.DiTing_DESKTOP_CHILD_PID = desktopChildPids.join(',')
  }
  ...
  emitUpdateProgress({ stage: 'update', message: 'Updating DiTing (git + dependencies)…', percent: 10 })

  const updated = (await runStreamedUpdate(diting, ['update', '--yes', ...branchArgs], {
    cwd: updateRoot,
    env,
    stage: 'update'
  })) as any
  ...
  if (updated.code !== 0) {
    emitUpdateProgress({ stage: 'error', message: 'diting update failed.', error: updated.error || 'update-failed' })

    return { ok: false, error: 'diting update failed' }
  }
  ...
  const rebuilt = await runRebuildWithRetry(attempt => {
    ...
    return runStreamedUpdate(diting, ['desktop', '--build-only'], { cwd: updateRoot, env, stage: 'rebuild' })
  })

  if (rebuilt.code !== 0) {
    emitUpdateProgress({
      stage: 'error',
      message: 'Backend updated, but the desktop rebuild failed. Restart DiTing to retry.',
  ...
  // Linux in-app update terminal state (#45205). `diting desktop --build-only`
  ...
  if (!IS_MAC) {
    ...
    if (outcome === 'relaunch') {
      emitUpdateProgress({ stage: 'restart', message: 'Restarting DiTing…', percent: 100 })
      ...
      const scriptPath = path.join(app.getPath('temp'), `diting-desktop-update-${Date.now()}.sh`)
      ...
      try {
        ...
        return {
          ...
          guiUpdated: false,
          manualRestart: true,
          message: 'Backend updated. Quit and reopen DiTing to load the new version.'
        }
      }
    }
    ...
    if (outcome === 'guiSkew') {
      emitUpdateProgress({
        ...
        message:
          ...
          'Update or reinstall the DiTing desktop app to match.',
        percent: 100
      })
      rememberLog(
    ...
    return {
      ...
      message:
        ...
        '(sandbox helper needs root). Quit and reopen DiTing to finish.'
    }
  }

  const rebuiltApp = [
    path.join(updateRoot, 'apps', 'desktop', 'release', 'mac-arm64', 'DiTing.app'),
    path.join(updateRoot, 'apps', 'desktop', 'release', 'mac', 'DiTing.app')
  ...
  if (!rebuiltApp || !targetApp) {
    emitUpdateProgress({
      stage: 'done',
      message: 'Backend updated. Restart DiTing to load the new version.',
      percent: 100
    })
...
if [ "$SRC" != "$DST" ]; then
  if /usr/bin/ditto "$SRC" "$DST.diting-update-new"; then
    rm -rf "$DST.diting-update-old" 2>/dev/null || true
    mv "$DST" "$DST.diting-update-old" 2>/dev/null || rm -rf "$DST"
    mv "$DST.diting-update-new" "$DST"
    rm -rf "$DST.diting-update-old" 2>/dev/null || true
  fi
fi
...
`
  ...
  try {
    ...
    emitUpdateProgress({
      stage: 'done',
      message: 'Backend + app updated. Restart DiTing to load the new version.',
      percent: 100
    })
...
// Marker-independent: is the canonical install at ACTIVE_DiTing_ROOT actually
...
function isActiveRuntimeUsable() {
  ...
  return (
    isDiTingSourceRoot(ACTIVE_DiTing_ROOT) &&
    fileExists(venvPython) &&
    canImportDiTingCli(venvPython, {
      env: {
        PYTHONPATH: [ACTIVE_DiTing_ROOT, process.env.PYTHONPATH].filter(Boolean).join(path.delimiter)
      }
    })
  )
}
...
function isBootstrapComplete() {
  ...
  // pinned commit -- users update via the in-app update path or `diting
...
function resolveWebDist() {
  const override = process.env.DiTing_DESKTOP_WEB_DIST
  ...
  if (IS_PACKAGED && /app\.asar(?=$|[\\/])/.test(fallback) && !directoryExists(fallback)) {
    rememberLog(
      `[web-dist] dashboard frontend dir resolved to an asar-internal path that ` +
        ...
        `Ensure dist/** is unpacked (asarUnpack) or set DiTing_DESKTOP_WEB_DIST.`
    )
  }
...
function resolveRendererIndex() {
  ...
  rememberLog(
    `[renderer] index.html not found — the desktop app was packaged without a ` +
      ...
      `Rebuild with: diting desktop --force-build`
  )
...
function resolveDiTingCwd() {
  // In a packaged build, `process.cwd()` resolves to the install root (e.g.
  // `…/win-unpacked` on Windows or `/Applications/DiTing.app/Contents/...`
  ...
  const candidates = [
    ...
    process.env.DiTing_DESKTOP_CWD,
...
function sanitizeWorkspaceCwd(cwd) {
  ...
  if (!trimmed || isPackagedInstallPath(trimmed)) {
    return { cwd: resolveDiTingCwd(), sanitized: Boolean(trimmed) }
  }
...
function createPythonBackend(root, label, backendArgs, options: any = {}) {
  ...
  return {
    ...
    label,
    command,
    args: ['-m', 'diting_cli.main', ...backendArgs],
...
// createActiveBackend — build a backend pointing at ACTIVE_DiTing_ROOT, the
...
function createActiveBackend(backendArgs) {
  ...
  return {
    kind: 'python',
    label: `DiTing at ${ACTIVE_DiTing_ROOT}`,
    command,
    args: ['-m', 'diting_cli.main', ...backendArgs],
    env: buildDesktopBackendEnv({
      ditingHome: DiTing_HOME,
      pythonPathEntries: [ACTIVE_DiTing_ROOT, ...getVenvSitePackagesEntries(VENV_ROOT)],
      venvRoot: VENV_ROOT
    }),
    root: ACTIVE_DiTing_ROOT,
    bootstrap: true,
    shell: false
  }
}

function resolveDiTingBackend(backendArgs) {
  // 1. Explicit override -- DiTing_DESKTOP_DiTing_ROOT points at a developer
  //    checkout. Honour it as-is (no bootstrap; the user is driving).
  const overrideRoot = process.env.DiTing_DESKTOP_DiTing_ROOT && path.resolve(process.env.DiTing_DESKTOP_DiTing_ROOT)

  if (overrideRoot && isDiTingSourceRoot(overrideRoot)) {
    const backend = createPythonBackend(overrideRoot, `DiTing source at ${overrideRoot}`, backendArgs)
  ...
  //    installed `diting` on PATH so local Python edits are actually exercised.
  //    (In dev with no checkout, SOURCE_REPO_ROOT won't pass isDiTingSourceRoot.)
  if (!IS_PACKAGED && isDiTingSourceRoot(SOURCE_REPO_ROOT)) {
    const backend = createPythonBackend(SOURCE_REPO_ROOT, `DiTing source at ${SOURCE_REPO_ROOT}`, backendArgs)
  ...
  // 3. Bootstrap-complete ACTIVE_DiTing_ROOT -- the canonical install at
  //    %LOCALAPPDATA%\diting\diting-agent (Windows) or ~/.diting/diting-agent.
  ...
  //    to spawning diting. Updates flow through the in-app update path
  //    (applyUpdates -> git pull) or `diting update` from the CLI.
  if (isBootstrapComplete()) {
    return createActiveBackend(backendArgs)
  }

  // 4. Existing `diting` on PATH -- installed via install.ps1 / install.sh from
  //    a previous tool-only setup, or pip-installed system-wide. Use it but
  //    do NOT write a bootstrap marker; the user did this themselves and we
  //    don't want to take ownership of an install we didn't perform.
  //    DiTing_DESKTOP_IGNORE_EXISTING=1 forces the bootstrap path for testing.
  if (process.env.DiTing_DESKTOP_IGNORE_EXISTING !== '1') {
    let ditingCommand = null
    const ditingOverride = process.env.DiTing_DESKTOP_DiTing

    if (ditingOverride) {
      const resolvedOverride = findOnPath(ditingOverride)

      if (resolvedOverride) {
        ditingCommand = resolvedOverride
      } else if (!isWindowsBinaryPathInWsl(ditingOverride, { isWsl: IS_WSL })) {
        ditingCommand = ditingOverride
      } else {
        rememberLog(`Ignoring Windows DiTing override under WSL: ${ditingOverride}`)
      }
    } else {
      ditingCommand = findOnPath('diting')
    }

    if (ditingCommand) {
      if (looksLikeDesktopAppBinary(ditingCommand)) {
        rememberLog(`Ignoring desktop app executable on PATH while resolving DiTing CLI: ${ditingCommand}`)
        ditingCommand = null
      }
    }

    if (ditingCommand) {
      const unwrapped = unwrapWindowsVenvDiTingCommand(ditingCommand, backendArgs)

      if (unwrapped) {
        return unwrapped
      }

      // Smoke-test the candidate before trusting it. A `diting` shim
      ...
      const shellForProbe = isCommandScript(ditingCommand)

      if (verifyDiTingCli(ditingCommand, { shell: shellForProbe })) {
        return (
          unwrapWindowsVenvDiTingCommand(ditingCommand, backendArgs) || {
            label: `existing DiTing CLI at ${ditingCommand}`,
            command: ditingCommand,
            args: backendArgs,
            bootstrap: false,
            env: {},
            kind: 'command',
      ...
      rememberLog(
        `Ignoring existing DiTing CLI at ${ditingCommand}: --version probe failed; falling through to bootstrap.`
      )
    }
  }

  // 5. Last-ditch: pip-installed diting_cli module via system Python.
  ...
  if (python) {
    ...
    // having diting_cli installed -- common on dev boxes that have
    ...
    // a uv-managed 3.11 into %LOCALAPPDATA%\diting\diting-agent\venv.
    if (canImportDiTingCli(python)) {
      return {
        kind: 'python',
        label: `installed diting_cli module via ${python}`,
        command: python,
        args: ['-m', 'diting_cli.main', ...backendArgs],
        bootstrap: false,
        env: {},
    ...
    rememberLog(`Ignoring system Python ${python}: diting_cli is not importable; falling through to bootstrap.`)
  }
  ...
  //    resolveDiTingBackend was the old "no payload" path and forced the
  //    user into a dead end. With the bootstrap protocol, "no install yet"
  //    is a recoverable state the GUI can drive through.
  return {
    kind: 'bootstrap-needed',
    label: 'DiTing Agent not installed yet; bootstrap required',
    command: null,
    args: backendArgs,
    bootstrap: true,
    env: {},
...
async function ensureRuntime(backend) {
  ...
  // backend.kind === 'bootstrap-needed' means resolveDiTingBackend couldn't
  ...
  if (backend.kind === 'bootstrap-needed') {
    rememberLog('[bootstrap] no DiTing install found; starting first-launch bootstrap')

    if (await handOffWindowsBootstrapRecovery('bootstrap-needed')) {
      const handoffError: Error & { isBootstrapFailure?: boolean; bootstrapHandedOff?: boolean } = new Error(
        'DiTing recovery was handed off to DiTing Setup. The desktop will restart when recovery completes.'
      )
    ...
    const bootstrapResult = await runBootstrap({
      ...
      ditingHome: DiTing_HOME,
      logRoot: path.join(DiTing_HOME, 'logs'),
    ...
    if (bootstrapResult.cancelled) {
      const cancelledError = new Error('DiTing install was cancelled.') as any
    ...
    if (!bootstrapResult.ok) {
      const bootstrapError = new Error(
        `DiTing bootstrap failed${bootstrapResult.failedStage ? ` at stage '${bootstrapResult.failedStage}'` : ''}: ` +
          `${bootstrapResult.error || 'unknown error'}. ` +
          `Check ${path.join(DiTing_HOME, 'logs', 'desktop.log')} for the full transcript.`
      ) as any

      bootstrapError.isBootstrapFailure = true
      bootstrapError.failedStage = bootstrapResult.failedStage || null
      // Latch the failure so subsequent startDiTing() calls return this
      // same error without re-running install.ps1.  Cleared by the
      // diting:bootstrap:reset IPC (renderer's "Reload and retry").
    ...
    return ensureRuntime(resolveDiTingBackend(backend.args))
  }
  ...
  if (!isDiTingSourceRoot(ACTIVE_DiTing_ROOT)) {
    throw new Error(
      `DiTing install at ${ACTIVE_DiTing_ROOT} is missing or incomplete. ` +
        'Reinstall via the desktop installer or scripts/install.ps1.'
    )
  }

  // On Windows, preflight Git Bash. DiTing' terminal tool calls bash.exe
  ...
  // %LOCALAPPDATA%\diting\git\, which findGitBash() picks up, so for any
  // user who completed the bootstrap this is a no-op. For users who got
  // here via an external `diting` on PATH, this check still helps.
  if (IS_WINDOWS && !findGitBash()) {
    throw new Error(
      'Git for Windows is required for DiTing on Windows (provides Git Bash, ' +
        "which the agent's terminal tool uses). Install it from " +
        'https://git-scm.com/download/win or run `winget install -e --id Git.Git`, ' +
        'then relaunch DiTing.'
    )
  }
  ...
  if (!fileExists(venvPython)) {
    ...
    // isDiTingSourceRoot() and the bootstrap writes the marker only after
    // install.ps1 succeeds. If we hit this, the user (or a deleted venv)
    // broke the invariant; tell them to re-run the install.
    throw new Error(
      `DiTing venv missing at ${VENV_ROOT}. Re-run the desktop installer or ` + '`scripts/install.ps1` to rebuild it.'
    )
  }

  backend.command = getVenvPython(VENV_ROOT)
  backend.label = `DiTing at ${ACTIVE_DiTing_ROOT} (venv: ${VENV_ROOT})`
  updateBootProgress({
    phase: 'runtime.ready',
    message: 'DiTing runtime is ready',
    progress: 82,
    running: true,
    error: null
  })
...
function fetchJson(url, token, options: any = {}) {
  return new Promise((resolve, reject) => {
    ...
    if (parsed.protocol !== 'http:' && parsed.protocol !== 'https:') {
      reject(new Error(`Unsupported DiTing backend URL protocol: ${parsed.protocol}`))
            ...
            reject(
              new Error(
                `Expected JSON from ${url} but got HTML (status ${res.statusCode}). ` +
                  'The endpoint is likely missing on the DiTing backend.'
              )
            )
    ...
    req.setTimeout(timeoutMs, () => {
      req.destroy(new Error(`Timed out connecting to DiTing backend after ${timeoutMs}ms`))
    })
...
function fetchPublicJson(url, options: any = {}) {
  ...
  // NO ``X-DiTing-Session-Token`` header — used by the auth-mode probe before
...
function getLinkTitleSession() {
  ...
  linkTitleSession = session.fromPartition('diting:link-titles', { cache: false })
...
async function previewFileTarget(rawTarget, baseDir) {
  ...
  const base = baseDir ? path.resolve(expandUserPath(baseDir)) : resolveDiTingCwd()
...
function sendPreviewFileChanged(payload) {
  ...
  webContents.send('diting:preview-file-changed', payload)
}
...
async function waitForDiTing(baseUrl, token) {
  ...
  throw new Error(`DiTing backend did not become ready: ${lastError?.message || 'timeout'}`)
}
...
function sendBackendExit(payload) {
  ...
  webContents.send('diting:backend-exit', payload)
}
...
function sendClosePreviewRequested() {
  ...
  webContents.send('diting:close-preview-requested')
}

// Tell the renderer the machine just woke. Sleep silently drops the
// renderer's WebSocket to the local backend; the renderer reconnects on this
// signal so the chat composer doesn't stay stuck on "Starting DiTing...".
function sendPowerResume() {
  ...
  webContents.send('diting:power-resume')
}
...
function sendOpenUpdatesRequested() {
  ...
  webContents.send('diting:open-updates')
...
function sendWindowStateChanged(nextIsFullscreen?: boolean) {
  ...
  webContents.send('diting:window-state-changed', state)
}
...
//
// Hosted DiTing gateways gate the dashboard behind an OAuth provider (e.g.
// Nous Research) instead of a static session token. The auth model is
// fundamentally different from the token path:
//
//   * REST is authed by HttpOnly session cookies (``diting_session_at``),
...
//     alongside the ~15-min access token (Portal NAS #293 / diting #37247).
//     Both are set as HttpOnly cookies (``diting_session_at`` ~15 min,
//     ``diting_session_rt`` 24h). When the AT cookie lapses but the RT cookie
...
const OAUTH_SESSION_PARTITION = 'persist:diting-remote-oauth'
  ...
  return new Promise((resolve, reject) => {
    ...
    try {
      win = new BrowserWindow({
        width: 520,
        height: 720,
        title: silent ? 'Connecting to DiTing Cloud agent…' : 'Sign in to DiTing gateway',
...
function fetchJsonViaOauthSession(url, options: any = {}) {
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      ...
      reject(new Error(`Timed out connecting to DiTing backend after ${timeoutMs}ms`))
    }, timeoutMs)
...
// --- DiTing Cloud discovery + silent per-agent sign-in (cloud-auto-discovery
// Phase 3) ---------------------------------------------------------------
//
...
// portal session cookie living in `persist:diting-remote-oauth`:
...
// convention (diting_cli/auth.py DEFAULT_NOUS_PORTAL_URL + the same env names)
// so a single override flips every DiTing surface to the same portal.
const DEFAULT_NOUS_PORTAL_URL = 'https://portal.nousresearch.com'

function resolvePortalBaseUrl() {
  const raw = process.env.DiTing_PORTAL_BASE_URL || process.env.NOUS_PORTAL_BASE_URL || DEFAULT_NOUS_PORTAL_URL
...
// authenticates via PRIVY, not the DiTing gateway session cookies, so this
// checks for the `privy-token` cookie on the portal host (NOT
// hasLiveOauthSession, which looks for diting_session_at/rt that the portal
...
function openPortalLoginWindow() {
  ...
  return new Promise((resolve, reject) => {
    if (!app.isReady()) {
      reject(new Error('Desktop is not ready to start a DiTing Cloud sign-in.'))
...
// Discover the hosted (DiTing Cloud) agents the signed-in user can see. Calls
...
async function discoverCloudAgents(org?: string) {
  ...
  if (!(await hasLivePortalSession())) {
    const err = new Error(
      'You are not signed in to DiTing Cloud. Open Settings → Gateway, choose DiTing Cloud, and sign in.'
    ) as any
  ...
  try {
    ...
    if (error && error.statusCode === 401) {
      const err = new Error('Your DiTing Cloud session has expired. Open Settings → Gateway and sign in again.') as any
      err.needsCloudLogin = true
      err.cause = error
      throw err
    }
...
async function cloudAgentSilentSignIn(dashboardUrl) {
  ...
  if (!(await hasLivePortalSession())) {
    const err = new Error('Your DiTing Cloud session has expired. Sign in to DiTing Cloud again.') as any
    err.needsCloudLogin = true
    throw err
  }
...
function sanitizeConnectionProfiles(raw: Record<string, any>) {
  ...
  for (const [name, entry] of Object.entries(raw)) {
    ...
    // Preserve the DiTing Cloud org tag on cloud-mode entries so Settings can
...
// a valid stored value (pins the root DiTing_HOME explicitly); null means "no
...
async function sanitizeDesktopConnectionConfig(config = readDesktopConnectionConfig(), profile = null) {
  ...
  const envOverride = key ? false : Boolean(process.env.DiTing_DESKTOP_REMOTE_URL)

  const remoteToken = decryptDesktopSecret(block.token)
  const authMode = normAuthMode(block.authMode)
  const remoteUrl = envOverride ? String(process.env.DiTing_DESKTOP_REMOTE_URL || '') : String(block.url || '')
  // The env override forces a plain remote connection. Otherwise reflect the
  // saved mode, preserving 'cloud' (a DiTing Cloud connection — Q6) so the UI
  ...
  return {
    ...
    remoteUrl,
    // The persisted DiTing Cloud org (slug/id) for a cloud connection, or '' for
    ...
    // scope is never overridden by DiTing_DESKTOP_REMOTE_URL.
    envOverride
  }
}
...
// `org` (optional) is the DiTing Cloud org slug/id the instance was discovered
...
function coerceDesktopConnectionConfig(input: any = {}, existing = readDesktopConnectionConfig(), options: any = {}) {
  ...
  // describe a discovered DiTing Cloud instance, NOT a user-owned remote gateway,
...
async function buildRemoteConnection(rawUrl, authMode, token, source) {
  ...
  if (authMode === 'oauth') {
    ...
    // Portal issues a 24h rotating refresh token (diting #37247), and the
    ...
    if (!(await hasLiveOauthSession(baseUrl))) {
      const err = new Error(
        'Remote DiTing gateway uses OAuth, but you are not signed in. ' +
  ...
  if (!token) {
    throw new Error(
      'Remote DiTing gateway is selected, but no session token is saved. ' +
...
//   2. env override (DiTing_DESKTOP_REMOTE_URL/_TOKEN) — applies app-wide
...
async function resolveRemoteBackend(profile) {
  ...
  const rawEnvUrl = process.env.DiTing_DESKTOP_REMOTE_URL
  const rawEnvToken = process.env.DiTing_DESKTOP_REMOTE_TOKEN

  if (rawEnvUrl) {
    if (!rawEnvToken) {
      throw new Error(
        'DiTing_DESKTOP_REMOTE_URL is set but DiTing_DESKTOP_REMOTE_TOKEN is not. ' +
          'Both must be provided to connect to a remote DiTing backend.'
      )
    }
...
function globalRemoteActive() {
  if (process.env.DiTing_DESKTOP_REMOTE_URL) {
    return true
  }
...
async function probeRemoteAuthMode(rawUrl) {
  ...
  // sending any credentials. ``/api/status`` is public on every DiTing
...
async function testDesktopConnectionConfig(input: any = {}) {
  ...
  if (wantRemote && block?.url) {
    ...
    const remote = (await resolveRemoteBackend(key)) || (await startDiTing())
  ...
  // connect to DiTing gateway". Mirror the renderer's connect here so the test
...
function resetDiTingConnection({ soft = false } = {}) {
  connectionPromise = null
  backendStartFailure = null

  stopBackendChild(ditingProcess)
...
// startDiTing() spawns fresh instead of racing the dying one. Shared by the
// connection-config and profile switch flows.
async function teardownPrimaryBackendAndWait({ soft = false } = {}) {
  // Capture the reference before resetDiTingConnection() nulls ditingProcess.
  const dying = ditingProcess && !ditingProcess.killed ? ditingProcess : null
...
function sendConnectionApplied() {
  ...
  webContents.send('diting:connection:applied')
}
...
// profile to startDiTing() (the window backend: boot UI, bootstrap, remote
...
// local-spawn portion of startDiTing() but without the boot-progress UI,
...
async function spawnPoolBackend(profile, entry) {
  ...
  if (remote) {
    await waitForDiTing(remote.baseUrl, remote.token)

    return {
      ...remote,
      profile,
      logs: ditingLog.slice(-80),
      ...getWindowState()
    }
  }

  const token = crypto.randomBytes(32).toString('base64url')
  // --profile wins over the inherited DiTing_HOME env (see _apply_profile_override
  // step 3 in diting_cli/main.py), so the child re-homes to this profile.
  // --port 0: the OS assigns an ephemeral port; the child announces it on stdout.
  const backendArgs = ['--profile', profile, 'serve', '--host', '127.0.0.1', '--port', '0']
  const backend = await ensureRuntime(resolveDiTingBackend(backendArgs))
  // Route old runtimes (no `serve`) through the legacy `dashboard --no-open`.
  backend.args = getBackendArgsForRuntime(backend)
  const ditingCwd = resolveDiTingCwd()
  const webDist = resolveWebDist()
  const readyFile = backend.readyFile ? makeDashboardReadyFile() : null

  rememberLog(`Starting DiTing backend for profile "${profile}" via ${backend.label}`)
  ...
  const child = spawn(
    ...
    hiddenWindowsChildOptions({
      ...
      env: {
        ...
        TERMINAL_CWD: ditingCwd,
        DiTing_DASHBOARD_SESSION_TOKEN: token,
        // Marks this dashboard backend as desktop-spawned so it runs the cron
        // scheduler tick loop (the gateway isn't running under the app).
        DiTing_DESKTOP: '1',
        DiTing_WEB_DIST: webDist,
        ...(readyFile ? { DiTing_DESKTOP_READY_FILE: readyFile } : {})
      },
      shell: backend.shell,
      stdio: ['ignore', 'pipe', 'pipe']
    })
  ...
  child.once('error', error => {
    rememberLog(`DiTing backend for profile "${profile}" failed to start: ${error.message}`)
    backendPool.delete(profile)
    rejectStart?.(error)
  })
  child.once('exit', (code, signal) => {
    rememberLog(`DiTing backend for profile "${profile}" exited (${signal || code})`)
    backendPool.delete(profile)

    if (!ready) {
      rejectStart?.(
        new Error(`DiTing backend for profile "${profile}" exited before it became ready (${signal || code}).`)
      )
    }
  })
  ...
  await Promise.race([waitForDiTing(baseUrl, token), startFailed])
  ready = true

  const authToken = await adoptServedDashboardToken(baseUrl, token, {
    childAlive: () => child.exitCode === null && !child.killed,
    label: `DiTing backend for profile "${profile}"`,
    rememberLog
  })
...
// backend whose ensure_diting_home() recreates the deleted profile directory.
//
...
async function startDiTing() {
  // Latched-failure short-circuit: once bootstrap has failed in this
  // process, every subsequent startDiTing() call re-throws the same error
  ...
  connectionPromise = (async () => {
    await advanceBootProgress('backend.resolve', 'Resolving DiTing backend', 8)
    // Resolve for the desktop's primary profile so a per-profile remote
    // override on the active profile is honored (falls back to env / global).
    const remote = await resolveRemoteBackend(primaryProfileKey())

    if (remote) {
      await advanceBootProgress('backend.remote', `Connecting to remote DiTing backend at ${remote.baseUrl}`, 24)
      await waitForDiTing(remote.baseUrl, remote.token)
      updateBootProgress({
        phase: 'backend.ready',
        message: 'Remote DiTing backend is ready',
        progress: 94,
        running: true,
        error: null
      })
    ...
    // deterministic (it wins over the sticky ~/.diting/active_profile file) and
    // resolves DiTing_HOME the same way `diting -p <name>` does on the CLI. An
    ...
    await advanceBootProgress('backend.runtime', 'Resolving DiTing runtime', 28)
    const backend = await ensureRuntime(resolveDiTingBackend(backendArgs))
    // Route old runtimes (no `serve`) through the legacy `dashboard --no-open`.
    backend.args = getBackendArgsForRuntime(backend)
    const ditingCwd = resolveDiTingCwd()
    ...
    await advanceBootProgress('backend.spawn', `Starting DiTing backend via ${backend.label}`, 84)
    rememberLog(`Starting DiTing backend via ${backend.label}`)
    ...
    ditingProcess = spawn(
      ...
      hiddenWindowsChildOptions({
        ...
        env: {
          ...process.env,
          // Explicitly pin DiTing_HOME for the child so Python's get_diting_home()
          // resolves to the SAME location our resolveDiTingHome() picked. Without
          // this pin, Python falls back to ~/.diting on every platform — fine on
          // mac/linux (where our default matches), but on Windows our default is
          // %LOCALAPPDATA%\diting, which differs from C:\Users\<u>\.diting.
          // Mismatch would split config / sessions / .env / logs across two
          // directories. install.ps1 sets DiTing_HOME via setx; the desktop
    ...
    ditingProcess.stdout.on('data', rememberLog)
    ditingProcess.stderr.on('data', rememberLog)
    ...
    ditingProcess.once('error', error => {
      rememberLog(`DiTing backend failed to start: ${error.message}`)
      updateBootProgress(
        {
          error: error.message,
          message: `DiTing backend failed to start: ${error.message}`,
          phase: 'backend.error',
          running: false
        },
        { allowDecrease: true }
    ...
    ditingProcess.once('exit', (code, signal) => {
      rememberLog(`DiTing backend exited (${signal || code})`)
      ditingProcess = null
      connectionPromise = null
      sendBackendExit({ code, signal })

      if (!backendReady) {
        const message = `DiTing backend exited before it became ready (${signal || code}).`
        updateBootProgress(
          {
            error: message,
            message,
        ...
        rejectBackendStart?.(
          new Error(
            `DiTing backend exited before it became ready (${signal || code}). Log: ${DESKTOP_LOG_PATH}\n${recentDiTingLog()}`
          )
        )
      }
    })

    await advanceBootProgress('backend.port', 'Waiting for DiTing backend to launch', 86)

    // Discover the ephemeral port the child bound to
    const port = await Promise.race([
      waitForDashboardPortAnnouncement(ditingProcess, { readyFile }),
      backendStartFailed
    ])
    ...
    const baseUrl = `http://127.0.0.1:${port}`
    await advanceBootProgress('backend.wait', 'Waiting for DiTing backend to become ready', 90)
    await Promise.race([waitForDiTing(baseUrl, token), backendStartFailed])
    backendReady = true
    backendStartFailure = null

    const authToken = await adoptServedDashboardToken(baseUrl, token, {
      // The exit/error handlers null ditingProcess when the child dies.
      childAlive: () => ditingProcess !== null && ditingProcess.exitCode === null && !ditingProcess.killed,
      rememberLog
    })

    updateBootProgress({
      phase: 'backend.ready',
      message: 'DiTing backend is ready. Finalizing desktop startup',
      progress: 94,
      running: true,
      error: null
    })
...
// here so it can leave the app's bounds and stay visible while DiTing is
...
// pushes pet state over IPC (diting:pet-overlay:state); the overlay just renders
// it. Control flows back (pop-in, composer submit) via diting:pet-overlay:control.
...
function spawnPetOverlayWindow(bounds) {
  const win = new BrowserWindow({
    ...
    // instead, leaving the main DiTing app as the Dock/cmd-tab anchor.
    skipTaskbar: !IS_MAC,
    hasShadow: false,
    alwaysOnTop: true,
    ...
    // anchor and the DiTing icon drops out of cmd/alt-tab — especially when the
    // main window is minimized. We flip this on only while the composer needs
    // the keyboard (see diting:pet-overlay:set-focusable).
    focusable: false,
    show: false,
  ...
  try {
    ...
    // which briefly hides the Dock/cmd-tab presence. Keep DiTing in the normal
  ...
  win.on('closed', () => {
    ...
    if (mainWindow && !mainWindow.isDestroyed()) {
      mainWindow.webContents.send('diting:pet-overlay:control', { type: 'pop-in' })
    }
  })
...
function createWindow() {
  ...
  mainWindow.webContents.once('did-finish-load', () => {
    ...
    startDiTing().catch(error => rememberLog(error.stack || error.message))
  })
}

ipcMain.handle('diting:connection', async (_event, profile) => ensureBackend(profile))
...
// "Starting DiTing…". Before the renderer's backoff loop reconnects, it asks us
// to confirm the cached PRIMARY backend is still reachable; if a remote one is
// not, we drop the cache so the next getConnection() rebuilds it. Local backends
// self-heal via their child 'exit' handler, so we never touch them here.
ipcMain.handle('diting:connection:revalidate', async () => {
  if (!connectionPromise) {
  ...
  try {
    ...
    // tick rebuilds a fresh, reachable descriptor. resetDiTingConnection only
    // nulls connectionPromise for a remote (no child to SIGTERM).
    rememberLog('Cached remote DiTing backend failed liveness probe; dropping stale connection.')
    resetDiTingConnection()

    return { ok: true, rebuilt: true }
  }
})
ipcMain.handle('diting:backend:touch', async (_event, profile) => {
  touchPoolBackend(profile)

  return { ok: true }
})
ipcMain.handle('diting:gateway:ws-url', async (_event, profile) => freshGatewayWsUrl(profile))
ipcMain.handle('diting:window:openSession', async (_event, sessionId, opts) => {
...
ipcMain.handle('diting:window:openNewSession', async () => {
...
ipcMain.handle('diting:zoom:get', event => {
...
ipcMain.on('diting:zoom:set-percent', (event, percent) => {
  const window = BrowserWindow.fromWebContents(event.sender)
...
ipcMain.handle('diting:pet-overlay:open', async (_event, request) => {
...
ipcMain.handle('diting:pet-overlay:close', async () => {
  closePetOverlay()
...
ipcMain.on('diting:pet-overlay:set-bounds', (_event, bounds) => {
...
ipcMain.on('diting:pet-overlay:ignore-mouse', (_event, ignore) => {
  if (petOverlayWindow && !petOverlayWindow.isDestroyed()) {
...
ipcMain.on('diting:pet-overlay:set-focusable', (_event, focusable) => {
  if (!petOverlayWindow || petOverlayWindow.isDestroyed()) {
    return
  }
...
ipcMain.on('diting:pet-overlay:state', (_event, payload) => {
  if (petOverlayWindow && !petOverlayWindow.isDestroyed()) {
    petOverlayWindow.webContents.send('diting:pet-overlay:state', payload)
  }
})
// Overlay → main renderer: control messages (pop back in, composer submit).
ipcMain.on('diting:pet-overlay:control', (_event, payload) => {
  if (!mainWindow || mainWindow.isDestroyed()) {
    return
  }
  ...
  mainWindow.webContents.send('diting:pet-overlay:control', payload)
})
ipcMain.handle('diting:bootstrap:reset', async () => {
  // Renderer's "Reload and retry" path. Clear the latched failure and
  // reset connection state so the next startDiTing() call restarts the
...
ipcMain.handle('diting:bootstrap:repair', async () => {
  // Forceful repair: drop the bootstrap-complete marker so the next
  // startDiTing() re-runs the full installer (refreshing a broken/partial
...
ipcMain.handle('diting:bootstrap:cancel', async () => {
...
ipcMain.handle('diting:boot-progress:get', async () => bootProgressState)
ipcMain.handle('diting:bootstrap:get', async () => getBootstrapState())
ipcMain.handle('diting:connection-config:get', async (_event, profile) =>
  sanitizeDesktopConnectionConfig(readDesktopConnectionConfig(), profile)
)
ipcMain.handle('diting:connection-config:test', async (_event, payload) => testDesktopConnectionConfig(payload))
ipcMain.handle('diting:connection-config:probe', async (_event, rawUrl) => probeRemoteAuthMode(rawUrl))
ipcMain.handle('diting:connection-config:oauth-login', async (_event, rawUrl) => {
...
ipcMain.handle('diting:connection-config:oauth-logout', async (_event, rawUrl) => {
...
// --- DiTing Cloud (cloud-auto-discovery Phase 3) ---
...
ipcMain.handle('diting:cloud:agent-sign-in', async (_event, dashboardUrl) => {
...
ipcMain.handle('diting:connection-config:save', async (_event, payload) => {
  const config = coerceDesktopConnectionConfig(payload)
  writeDesktopConnectionConfig(config)

  return sanitizeDesktopConnectionConfig(config, payload?.profile)
})
ipcMain.handle('diting:connection-config:apply', async (_event, payload) => {
  const config = coerceDesktopConnectionConfig(payload)
  writeDesktopConnectionConfig(config)
  ...
  if (key && key !== primaryProfileKey()) {
    ...
    // wipes session lists (skeletons) and re-dials on diting:connection:applied.
...
ipcMain.handle('diting:profile:get', async () => ({ profile: readActiveDesktopProfile() }))
ipcMain.handle('diting:profile:set', async (_event, name) => {
  const next = writeActiveDesktopProfile(name)

  // Switching profiles is a backend re-home: relaunch the dashboard under the
  // new DiTing_HOME. Pool backends keep their own homes, so only the primary
  // is torn down.
  await teardownPrimaryBackendAndWait()
  mainWindow?.reload()
...
ipcMain.on('diting:previewShortcutActive', (_event, active) => {
  previewShortcutActive = Boolean(active)
})

ipcMain.handle('diting:requestMicrophoneAccess', async () => {
...
ipcMain.handle('diting:api', async (_event, request) => {
  ...
  // backend calls ensure_diting_home() which recreates the profile directory,
...
ipcMain.handle('diting:notify', (_event, payload) => {
  ...
  notification.on('click', () => {
    ...
    if (payload?.sessionId) {
      mainWindow.webContents.send('diting:focus-session', payload.sessionId)
    }
  })
  notification.on('action', (_actionEvent, index) => {
    ...
    if (action?.id) {
      mainWindow.webContents.send('diting:notification-action', { sessionId: payload?.sessionId, actionId: action.id })
    }
  })
  notification.show()
...
ipcMain.handle('diting:readFileDataUrl', async (_event, filePath) => {
...
ipcMain.handle('diting:readFileText', async (_event, filePath) => {
...
ipcMain.handle('diting:selectPaths', async (_event, options: any = {}) => {
...
ipcMain.handle('diting:writeClipboard', (_event, text) => {
  clipboard.writeText(String(text || ''))

  return true
})

ipcMain.handle('diting:saveImageFromUrl', (_event, url) => saveImageFromUrl(String(url || '')))

ipcMain.handle('diting:saveImageBuffer', async (_event, payload) => {
...
ipcMain.handle('diting:saveClipboardImage', async () => {
...
ipcMain.handle('diting:normalizePreviewTarget', (_event, target, baseDir) =>
  normalizePreviewTarget(String(target || ''), baseDir ? String(baseDir) : '')
)

ipcMain.handle('diting:watchPreviewFile', (_event, url) => watchPreviewFile(String(url || '')))

ipcMain.handle('diting:stopPreviewFileWatch', (_event, id) => stopPreviewFileWatch(String(id || '')))

ipcMain.on('diting:titlebar-theme', (_event, payload) => {
...
ipcMain.on('diting:native-theme', (_event, mode) => {
...
ipcMain.on('diting:translucency', (_event, payload) => {
...
ipcMain.handle('diting:openExternal', (_event, url) => {
  if (!openExternalUrl(url)) {
    throw new Error('Invalid external URL')
  }
})

ipcMain.handle('diting:openPreviewInBrowser', async (_event, url) => {
...
// it via writeDefaultProjectDir so resolveDiTingCwd picks it up on the next
// session spawn (no app restart needed).
ipcMain.handle('diting:setting:defaultProjectDir:get', async () => ({
  dir: readDefaultProjectDir(),
  defaultLabel: app.getPath('home'),
  resolvedCwd: resolveDiTingCwd()
}))

ipcMain.handle('diting:workspace:sanitize', async (_event, cwd) => sanitizeWorkspaceCwd(cwd))

ipcMain.handle('diting:setting:defaultProjectDir:set', async (_event, dir) => {
...
ipcMain.handle('diting:setting:defaultProjectDir:pick', async () => {
...
ipcMain.handle('diting:fetchLinkTitle', (_event, url) => fetchLinkTitle(url))

ipcMain.handle('diting:logs:reveal', async () => {
  try {
    await fs.promises.mkdir(path.dirname(DESKTOP_LOG_PATH), { recursive: true })
...
ipcMain.handle('diting:logs:recent', async () => ({ path: DESKTOP_LOG_PATH, lines: ditingLog.slice(-200) }))
...
function terminalShellCommand() {
  // DiTing_DESKTOP_SHELL is the cross-platform escape hatch (a path or a bare
  ...
  const override = (process.env.DiTing_DESKTOP_SHELL || (IS_WINDOWS ? '' : process.env.SHELL) || '').trim()
...
function terminalShellEnv() {
  ...
  // /TERM=dumb; some terminals set COLORFGBG which would flip DiTing' TUI into
  ...
  env.TERM_PROGRAM = 'DiTing'
  env.TERM_PROGRAM_VERSION = app.getVersion()

  // Let a diting/--tui launched in this pane know it's embedded in the desktop
  // GUI (build_environment_hints surfaces this). Distinct from DiTing_DESKTOP,
  // which marks the agent *backend* and gates cron/gateway behavior.
  env.DiTing_DESKTOP_TERMINAL = '1'

  return env
}

function terminalChannel(id, suffix) {
  return `diting:terminal:${id}:${suffix}`
}
...
ipcMain.handle('diting:fs:readDir', async (_event, dirPath) => readDirForIpc(dirPath))

ipcMain.handle('diting:fs:gitRoot', async (_event, startPath) => gitRootForIpc(startPath))

// Reveal a path in the OS file manager (Finder / Explorer / Files).
ipcMain.handle('diting:fs:reveal', async (_event, targetPath) => {
...
ipcMain.handle('diting:fs:rename', async (_event, targetPath, newName) => {
...
ipcMain.handle('diting:fs:writeText', async (_event, filePath, content) => {
...
ipcMain.handle('diting:fs:trash', async (_event, targetPath) => {
  const target = String(targetPath || '').trim()
...
ipcMain.handle('diting:git:worktreeList', async (_event, repoPath) => listWorktrees(repoPath, resolveGitBinary()))

ipcMain.handle('diting:git:worktreeAdd', async (_event, repoPath, options) =>
  addWorktree(repoPath, options || {}, resolveGitBinary())
)

ipcMain.handle('diting:git:worktreeRemove', async (_event, repoPath, worktreePath, options) =>
  removeWorktree(repoPath, worktreePath, options || {}, resolveGitBinary())
)

ipcMain.handle('diting:git:branchSwitch', async (_event, repoPath, branch) =>
  switchBranch(repoPath, branch, resolveGitBinary())
)

ipcMain.handle('diting:git:branchList', async (_event, repoPath) => listBranches(repoPath, resolveGitBinary()))

ipcMain.handle('diting:git:baseBranchList', async (_event, repoPath) =>
...
ipcMain.handle('diting:git:repoStatus', async (_event, repoPath) => repoStatus(repoPath, resolveGitBinary()))
...
ipcMain.handle('diting:git:review:list', async (_event, repoPath, scope, baseRef) =>
  reviewList(repoPath, scope, baseRef, resolveGitBinary())
)
ipcMain.handle('diting:git:review:diff', async (_event, repoPath, filePath, scope, baseRef, staged) =>
...
ipcMain.handle('diting:git:fileDiff', async (_event, repoPath, filePath) =>
  fileDiffVsHead(repoPath, filePath, resolveGitBinary())
)
ipcMain.handle('diting:git:review:stage', async (_event, repoPath, filePath) =>
  reviewStage(repoPath, filePath ?? null, resolveGitBinary())
)
ipcMain.handle('diting:git:review:unstage', async (_event, repoPath, filePath) =>
  reviewUnstage(repoPath, filePath ?? null, resolveGitBinary())
)
ipcMain.handle('diting:git:review:revert', async (_event, repoPath, filePath) =>
  reviewRevert(repoPath, filePath ?? null, resolveGitBinary())
)
ipcMain.handle('diting:git:review:revParse', async (_event, repoPath, ref) =>
  reviewRevParse(repoPath, ref, resolveGitBinary())
)
ipcMain.handle('diting:git:review:commit', async (_event, repoPath, message, push) =>
  reviewCommit(repoPath, message, Boolean(push), resolveGitBinary())
)
ipcMain.handle('diting:git:review:commitContext', async (_event, repoPath) =>
  reviewCommitContext(repoPath, resolveGitBinary())
)
ipcMain.handle('diting:git:review:push', async (_event, repoPath) => reviewPush(repoPath, resolveGitBinary()))
ipcMain.handle('diting:git:review:shipInfo', async (_event, repoPath) => reviewShipInfo(repoPath, resolveGhBinary()))
ipcMain.handle('diting:git:review:createPr', async (_event, repoPath) =>
...
ipcMain.handle('diting:git:scanRepos', async (_event, roots, options) => {
  try {
...
ipcMain.handle('diting:terminal:start', async (event, payload = {}) => {
...
ipcMain.handle('diting:terminal:write', (_event, id, data) => {
...
ipcMain.handle('diting:terminal:resize', (_event, id, size = {}) => {
  const sessionInfo = terminalSessions.get(String(id || ''))
...
ipcMain.handle('diting:terminal:cwd', async (_event, id) => {
  const sessionInfo = terminalSessions.get(String(id || ''))
...
ipcMain.handle('diting:terminal:dispose', (_event, id) => disposeTerminalSession(String(id || '')))

ipcMain.handle('diting:updates:check', async () =>
...
ipcMain.handle('diting:updates:apply', async (_event, payload) =>
...
ipcMain.handle('diting:updates:branch:get', async () => readDesktopUpdateConfig())

ipcMain.handle('diting:updates:branch:set', async (_event, name) => {
...
// Resolve the canonical DiTing version (the one `release.py` bumps in
// diting_cli/__init__.py + pyproject.toml) so the desktop About panel shows the
// real DiTing version instead of the Electron app's own package.json version,
// which historically drifted (stuck at 0.0.2). Falls back to app.getVersion()
// when the source tree can't be read (e.g. a packaged build without the repo).
function resolveDiTingVersion() {
  try {
    const root = resolveUpdateRoot()
    const initPath = path.join(root, 'diting_cli', '__init__.py')
...
// Re-resolve the live DiTing version and push it into the native About panel
// just before showing it, so an in-place `diting update` is reflected without
...
ipcMain.handle('diting:version', async () => ({
  appVersion: resolveDiTingVersion(),
  electronVersion: process.versions.electron,
  nodeVersion: process.versions.node,
  platform: process.platform,
  ditingRoot: resolveUpdateRoot()
}))
...
// the actual removal via `diting uninstall …` so the cross-platform PATH /
// registry / service / node-symlink cleanup all lives in one place
// (diting_cli/uninstall.py + diting_cli/gui_uninstall.py).
//
...
async function getUninstallSummary() {
  ...
  const agentRoot = ACTIVE_DiTing_ROOT
  ...
  const fallback = () => ({
    diting_home: DiTing_HOME,
    agent_installed: isDiTingSourceRoot(agentRoot) && fileExists(py),
    gui_installed: true,
  ...
  return new Promise(resolve => {
    ...
    try {
      const child = spawn(
        py,
        ['-m', 'diting_cli.main', 'uninstall', '--gui-summary'],
        hiddenWindowsChildOptions({
          cwd: agentRoot,
          env: { ...process.env, DiTing_HOME, NO_COLOR: '1' },
          stdio: ['ignore', 'pipe', 'ignore']
        })
      )
...
async function runDesktopUninstall(mode) {
  ...
  if (!fileExists(venvPy)) {
    return {
      ok: false,
      error: 'agent-missing',
      message: `Can't run the uninstaller: no DiTing agent venv at ${VENV_ROOT}.`
    }
  }
  ...
  // Python with PYTHONPATH=<agentRoot> so `import diting_cli` resolves from
  ...
  try {
    await releaseBackendLock(ACTIVE_DiTing_ROOT, 'uninstall')
  } catch (error) {
  ...
  try {
    if (IS_WINDOWS) {
      scriptPath = path.join(app.getPath('temp'), `diting-uninstall-${Date.now()}.cmd`)
      ...
      scriptPath = path.join(app.getPath('temp'), `diting-uninstall-${Date.now()}.sh`)
...
ipcMain.handle('diting:uninstall:summary', async () => getUninstallSummary())
ipcMain.handle('diting:uninstall:run', async (_event, payload) => {
...
ipcMain.handle('diting:vscode-theme:fetch', async (_event, id) => fetchMarketplaceThemes(String(id || '')))

// Search the Marketplace for color-theme extensions (empty query = top installs).
ipcMain.handle('diting:vscode-theme:search', async (_event, query) => searchMarketplaceThemes(String(query || ''), 20))

// ---------------------------------------------------------------------------
// diting:// deep links (e.g. diting://blueprint/morning-brief?time=08:00).
// A docs/dashboard "Send to App" button opens this URL; we route it into the
// running app's chat composer. Three delivery paths: macOS 'open-url',
// Win/Linux running-app 'second-instance' (argv), Win/Linux cold-start argv.
// ---------------------------------------------------------------------------
const DiTing_PROTOCOL = 'diting'
...
function _extractDeepLink(argv) {
  ...
  return argv.find(a => typeof a === 'string' && a.startsWith(`${DiTing_PROTOCOL}://`)) || null
}
...
function handleDeepLink(url) {
  ...
  // diting://blueprint/<key>?slot=val  -> host="blueprint", path="/<key>"
  ...
  try {
    ...
    mainWindow.focus()
    mainWindow.webContents.send('diting:deep-link', payload)
...
ipcMain.handle('diting:deep-link-ready', () => {
  ...
  if (_pendingDeepLink) {
    ...
    handleDeepLink(
      `${DiTing_PROTOCOL}://${queued.kind}/${encodeURIComponent(queued.name)}` +
...
function registerDeepLinkProtocol() {
  try {
    if (process.defaultApp && process.argv.length >= 2) {
      ...
      app.setAsDefaultProtocolClient(DiTing_PROTOCOL, process.execPath, [path.resolve(process.argv[1])])
    } else {
      app.setAsDefaultProtocolClient(DiTing_PROTOCOL)
    }
  } catch (err) {
...
// second-instance argv. Without the lock a second `diting://` launch spawns a
...
app.whenReady().then(() => {
  ...
  // Win/Linux cold start: the launching diting:// URL is in our own argv.
