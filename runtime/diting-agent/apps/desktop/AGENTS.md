...
How to build DiTing Desktop well. This is a judgment guide, not an inventory —
...
- The **backend** is authoritative for anything another DiTing surface can also
...
- A **runtime home change** (switching the underlying `DiTing_HOME` profile) is
...
its shape. "Plugin" means several unrelated things across DiTing — do not assume
