...
//!
//! Mirrors `diting_constants.get_diting_home()` from the Python CLI:
//!   Windows: %LOCALAPPDATA%\diting
//!   macOS:   ~/.diting
//!   Linux:   ~/.diting  (override via $DiTing_HOME)
//!
//! NOTE (macOS): Python's get_diting_home(), scripts/install.sh, and the
//! Electron desktop's resolveDiTingHome() ALL use ~/.diting on macOS — there
...
/// Returns the canonical DiTing home directory, respecting $DiTing_HOME if set.
pub fn diting_home() -> PathBuf {
    if let Ok(override_path) = std::env::var("DiTing_HOME") {
    ...
    {
        // %LOCALAPPDATA%\diting — matches scripts/install.ps1's $DiTingHome.
        if let Some(local_app_data) = dirs::data_local_dir() {
            return local_app_data.join("diting");
        }
    }

    // macOS + Linux + fallback: ~/.diting (matches Python get_diting_home(),
    // install.sh, and the Electron desktop's resolveDiTingHome()).
    if let Some(home) = dirs::home_dir() {
        return home.join(".diting");
    }
...
pub fn bootstrap_cache_dir() -> PathBuf {
    diting_home().join("bootstrap-cache")
}
...
/// DiTing_HOME so it survives repo checkout deletion (unlike anything under
/// diting-agent/).
///
/// On Windows this is `%LOCALAPPDATA%\diting\diting-setup.exe`; on other
...
/// mid-update re-locks the venv shim and triggers `force_kill_other_diting`,
/// which then kills that legitimate backend in a respawn loop (#50238).
///
/// Lives directly under DiTing_HOME (same rationale as `installer_dest`) so the
/// Electron desktop — which resolves DiTing_HOME identically and pins it into
/// the updater's env — agrees on the exact path.
pub fn update_in_progress_marker() -> PathBuf {
    diting_home().join(".diting-update-in-progress")
}
...
pub fn copy_self_to_diting_home() -> std::io::Result<()> {
    ...
    tracing::info!(?src, ?dest, "copied installer to DiTing_HOME");
    Ok(())
}
...
/// We don't always know ACTIVE_DiTing_ROOT until install.ps1 reports it, so
/// this is a probe helper, not a definitive path.
pub fn likely_bootstrap_marker(install_root: &Path) -> PathBuf {
    install_root.join(".diting-bootstrap-complete")
}

/// Initializes tracing to bootstrap-installer.log under DiTing_HOME/logs/.
...
pub fn init_logging() -> Option<WorkerGuard> {
    ...
    if let Err(err) = std::fs::create_dir_all(&dir) {
        ...
        eprintln!("[diting-setup] could not create log dir {dir:?}: {err}");
        return None;
    }
    ...
    let env_filter = tracing_subscriber::EnvFilter::try_from_env("DiTing_BOOTSTRAP_LOG")
...
#[tauri::command]
pub fn get_diting_home() -> String {
    diting_home().to_string_lossy().into_owned()
}
