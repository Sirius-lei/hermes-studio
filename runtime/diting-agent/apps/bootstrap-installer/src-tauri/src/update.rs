...
//!
//! Driven when the installer is launched as `DiTing-Setup.exe --update` (see
//! `AppMode` in lib.rs). The desktop app hands off to us — it exits, then we:
//!
//!   1. wait for the old DiTing desktop process to fully exit (so both the
//!      venv shim and packaged app.asar are free; otherwise `diting update`
//!      or repair bootstrap can race locked files),
//!   2. run `diting update --yes --gateway` (Python/repo update; this does NOT
//!      rebuild apps/desktop by design — see cmd_update in diting_cli/main.py),
//!   3. run `diting desktop --build-only` (the rebuild step update skips),
...
//!
//! Cross-platform note: `diting update` already handles macOS/Linux (git/pip).
//! The only OS-specific bits here are the venv shim path (resolve_diting) and
...
/// `diting update` exit code meaning "another diting process is holding the
/// venv shim open / dirty precondition" — see _cmd_update_impl in
/// diting_cli/main.py (sys.exit(2)). We surface a targeted message for this.
const UPDATE_EXIT_CONCURRENT: i32 = 2;

/// How long to wait for the old desktop process to release files under the
/// install tree before giving up and letting `diting update`'s own guard decide.
...
async fn run_update(app: AppHandle) -> Result<()> {
    let diting_home = crate::paths::diting_home();
    let install_root = diting_home.join("diting-agent");
    ...
    // it, that backend re-locks the venv shim, our `force_kill_other_diting`
    ...
    let diting = resolve_diting(&install_root).ok_or_else(|| {
        let msg = format!(
            "Could not find the diting CLI under {}. Is DiTing installed? \
    ...
    // async on Windows. If it still holds the venv shim, `diting update`
    ...
    // ---- stage 2: diting update -----------------------------------------
    // Pass --branch so `diting update` targets the branch this installer was
    ...
    // without --branch, `diting update` switches the checkout to `main` (a
    ...
    // --force skips `diting update`'s Windows running-exe guard (which would
    ...
    // time `diting update` runs there is no legitimate diting.exe to protect,
    // and the guard would only produce a false "DiTing is still running" stop.
    //
    ...
    // probe only checks the diting.exe shim and app.asar, so an external venv
    ...
    // below surfaces the correct "close all DiTing windows" message.
    ...
    // Retry-once for the update-boundary crash. `diting update` lazily imports
    ...
    // second `diting update` runs clean because the now-current module is loaded
    ...
    // for the concurrent-instance guard (exit 2) — that's a "close DiTing" state
    // a retry can't fix.
    ...
    match update.exit_code {
        ...
        Some(code) if code == UPDATE_EXIT_CONCURRENT => {
            let msg = "DiTing is still running. Close all DiTing windows and try \
                       the update again."
                .to_string();
            emit_stage(
                &app,
        ...
        other => {
            let msg = format!(
                "diting update failed (exit {:?}). See {} for details.",
                other,
                crate::paths::diting_home()
                    .join("logs")
                    .join("update.log")
                    .display()
            );
    ...
    // ---- stage 3: diting desktop --build-only ----------------------------
    // `diting update` deliberately does NOT build apps/desktop (it installs
    ...
    // retry-once `diting update` already does above, and `diting update`'s own
    ...
    if rebuild.exit_code != Some(0) {
        let msg = format!(
            "Rebuilding the desktop app failed (exit {:?}). The update was \
             applied but the app could not be rebuilt; run `diting desktop` \
    ...
    if let Some(target_app) = launch_target {
        if let Err(err) = launch_macos_app_and_exit(&app, &target_app).await {
            emit_log(
                ...
                None,
                LogStream::Stderr,
                &format!("[update] could not auto-launch desktop: {err}. Launch DiTing manually."),
            );
        }
    } else if let Err(err) =
        crate::bootstrap::launch_diting_desktop(app.clone(), install_root.to_string_lossy().into_owned()).await
    {
...
pub(crate) async fn wait_for_install_locks_free(install_root: &Path, app: &AppHandle, stage: &str) {
    ...
    emit_log(app, Some(stage), LogStream::Stdout, "[handoff] waiting for DiTing to exit…");
    ...
    loop {
        ...
        if Instant::now() >= deadline {
            // Last resort: a backend diting.exe (or the desktop DiTing.exe
            ...
            // force-kill every DiTing.exe except ourselves, then give the OS a
            ...
            emit_log(
                ...
                LogStream::Stdout,
                &format!(
                    "[handoff] DiTing still holding install files ({}); force-killing stragglers…",
                    format_locked_paths(&locked)
                ),
            );
            force_kill_other_diting();
...
fn install_lock_probe_paths(install_root: &Path) -> Vec<PathBuf> {
    let mut paths = vec![venv_diting(install_root)];
...
fn desktop_app_payload_paths(install_root: &Path) -> Vec<PathBuf> {
    ...
    if cfg!(target_os = "windows") {
        ...
        vec![
            release.join("mac").join("DiTing.app").join("Contents").join("Resources").join("app.asar"),
            release.join("mac-arm64").join("DiTing.app").join("Contents").join("Resources").join("app.asar"),
        ]
    } else {
...
/// Force-kill any `diting.exe` other than this process. Windows-only; a no-op
...
/// knew its children — so we kill the whole `diting.exe` image tree via
/// taskkill, excluding our own PID.
///
/// Safe w.r.t. our own update child: this runs inside the install-lock wait,
/// which completes BEFORE we spawn `venv\Scripts\diting.exe update`. And a
/// desktop the user relaunches mid-update will NOT have spawned a backend —
/// `startDiTing()` in the desktop gates local-backend startup on our
/// update-in-progress marker and parks until we finish (#50238). So the only
/// diting.exe images here are stragglers from the old desktop — exactly what
/// we want gone. (`/FI PID ne <self>` also spares this Tauri process, though it
/// isn't named diting.exe.)
fn force_kill_other_diting() {
    if !cfg!(target_os = "windows") {
        return;
    }
...
/// Spawn `diting <args>` from `cwd`, stream stdout/stderr as Log events on the
...
/// Path to the venv diting shim under an install root, regardless of existence.
fn venv_diting(install_root: &Path) -> PathBuf {
    if cfg!(target_os = "windows") {
        install_root.join("venv").join("Scripts").join("diting.exe")
    } else {
        install_root.join("venv").join("bin").join("diting")
    }
}

/// Resolve the diting CLI to drive. Prefer the venv shim in the install we
/// just updated; fall back to `diting` on PATH.
fn resolve_diting(install_root: &Path) -> Option<PathBuf> {
    let shim = venv_diting(install_root);
    if shim.exists() {
        return Some(shim);
    }
    // PATH fallback. which-style probe via env, kept dependency-free.
    let exe = if cfg!(target_os = "windows") { "diting.exe" } else { "diting" };
...
fn update_child_env(install_root: &Path) -> Vec<(String, OsString)> {
    let diting_home = crate::paths::diting_home();
    let mut envs = vec![(
        "DiTing_HOME".to_string(),
        diting_home.as_os_str().to_os_string(),
    )];
    if let Some(path) = path_with_prepended_entries(&[
        diting_home.join("node").join("bin"),
...
async fn install_macos_app_update(
    ...
    let rebuilt_app = crate::bootstrap::resolve_diting_desktop_app(install_root).ok_or_else(|| {
        anyhow!(
            "desktop rebuild succeeded but no DiTing.app was found under {}",
    ...
    let tmp = PathBuf::from(format!("{}.diting-update-new", target_app.display()));
    let old = PathBuf::from(format!("{}.diting-update-old", target_app.display()));
    ...
    let ditto = Command::new("/usr/bin/ditto")
        .arg(&rebuilt_app)
        .arg(&tmp)
        .current_dir(crate::paths::diting_home())
        .status()
        .await
...
mod tests {
    ...
    #[test]
    fn venv_diting_is_under_install_root() {
        let root = Path::new("/x/diting-agent");
        let shim = venv_diting(root);
    ...
    fn lock_probe_paths_include_desktop_app_payload() {
        ...
        assert!(
            probes.iter().any(|p| p == &venv_diting(root)),
    ...
    fn locked_paths_ignores_missing_payloads() {
        let root = Path::new("/nonexistent/diting-agent");
        let probes = install_lock_probe_paths(root);
    ...
    fn update_marker_guard_writes_then_removes_on_drop() {
        ...
        let marker = dir.join(".diting-update-in-progress");
    ...
    fn parses_only_app_targets() {
        assert_eq!(
            target_app_from_args(["--update", "--target-app", "/Applications/DiTing.app"]),
            Some(PathBuf::from("/Applications/DiTing.app"))
        );
    ...
    async fn swap_installs_new_bundle_and_cleans_up() {
        ...
        let target = base.join("DiTing.app");
        let tmp = base.join("DiTing.app.diting-update-new");
        let old = base.join("DiTing.app.diting-update-old");
    ...
    async fn swap_failure_never_leaves_target_missing() {
        ...
        let target = base.join("DiTing.app");
        let tmp = base.join("DiTing.app.diting-update-new"); // intentionally absent
        let old = base.join("DiTing.app.diting-update-old");
        write_marker(&target, "OLD");
    ...
    async fn swap_rolls_back_when_install_step_fails() {
        ...
        let target = base.join("DiTing.app");
        let tmp = base.join("DiTing.app.diting-update-new"); // absent
        let old = base.join("DiTing.app.diting-update-old");
        write_marker(&target, "OLD");
