...
export const $ditingHome = atom<string | null>(null)
...
export async function initialize(): Promise<void> {
  ...
  if (fake) {
    unlisten = () => {}
    $logPath.set('~/.diting/logs/bootstrap-installer.log')
    $ditingHome.set('~/.diting')
  ...
  try {
    const [logPath, ditingHome, mode] = await Promise.all([
      invoke<string>('get_log_path'),
      invoke<string>('get_diting_home'),
      invoke<AppMode>('get_mode')
    ])
    $logPath.set(logPath)
    $ditingHome.set(ditingHome)
    $mode.set(mode)
  } catch (err) {
  ...
  unlisten = await listen<BootstrapEvent>('bootstrap', (event) => {
    ...
    switch (payload.type) {
      ...
      case 'complete':
        ...
        // Install: show the "launch DiTing" success screen. Update: this is a
...
export async function startUpdate(): Promise<void> {
  ...
  // Update is driven by the desktop handing off (DiTing-Setup.exe --update);
...
export async function launchDiTingDesktop(): Promise<void> {
  ...
  await invoke('launch_diting_desktop', { installRoot })
}
...
const FAKE_INSTALL_STAGES: FakeStage[] = [
  ...
  { name: 'repo', title: 'DiTing repository' },
