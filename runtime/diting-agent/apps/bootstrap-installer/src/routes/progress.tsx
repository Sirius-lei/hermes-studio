...
export default function ProgressScreen({ bootstrap }: ProgressProps) {
  ...
  const title = bootstrap.status === 'completed' ? 'Done' : isUpdate ? 'Updating DiTing' : 'Setting up DiTing Agent'
  const description = isUpdate
    ? 'DiTing is updating to the latest version — this only takes a moment.'
    : 'This is a one-time setup. The DiTing installer is downloading dependencies and configuring your machine. Subsequent launches will skip this step.'
  const pct = Math.round(progress.fraction * 100)

  return (
    <div className="diting-fade-in flex h-full flex-col">
