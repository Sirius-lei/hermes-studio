# OpenAI-Compatible API Server for DiTing Agent
...
SSE streaming. By exposing this endpoint, diting-agent becomes instantly
...
│  AnythingLLM     │  │    {"messages": [...]}             │  diting-agent   │
│  NextChat        │  │                                    │  gateway        │
...
A user would:
1. Set `API_SERVER_ENABLED=true` in `~/.diting/.env`
2. Run `diting gateway` (API server starts alongside Telegram/Discord/etc.)
3. Point Open WebUI (or any frontend) at `http://localhost:8642/v1`
4. Chat with diting-agent through any OpenAI-compatible UI
...
| GET | `/v1/models` | List available "models" (returns diting-agent as a model) |
...
Authorization: Bearer diting-api-key-here
...
But diting-agent sessions have persistent state (memory, skills, tool context).
...
Frontends send `"model": "diting-agent"` (or whatever). The actual LLM model
used is configured server-side in config.yaml. The API server maps any
requested model name to the configured diting-agent model.
...
1. `gateway/platforms/api_server.py` — new adapter
   ...
   - Frontend system prompt layered on top of diting-agent's core prompt
...
Once implemented, diting-agent works as a drop-in backend for:
