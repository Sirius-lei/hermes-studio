...
def rewrite_relative_links(body: str, meta: dict[str, Any]) -> str:
    ...
    base = f"https://github.com/NousResearch/diting-agent/blob/main/{source_dir}/{meta['rel_path']}"
...
def render_skill_page(
    ...
    diting_meta = (fm.get("metadata") or {}).get("diting") or {}
    tags = diting_meta.get("tags") or []
    related = diting_meta.get("related_skills") or []
    ...
    if meta["source_kind"] == "bundled":
        ...
        info_rows.append(
            (
                "Source",
                "Optional — install with `diting skills install official/"
                + meta["category"]
                + "/"
                + meta["slug"]
                + "`",
...
def build_catalog_md_bundled(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Catalog of bundled skills that ship with DiTing Agent"',
        "---",
        "",
        "# Bundled Skills Catalog",
        "",
        "DiTing ships with a large built-in skill library copied into `~/.diting/skills/` on install. Each skill below links to a dedicated page with its full definition, setup, and usage.",
        "",
        "DiTing also syncs bundled skills on `diting update`, but the sync manifest respects local deletions and user edits. If a skill listed here is missing from your profile's `~/.diting/skills/` tree, it is still shipped with DiTing; restore it with `diting skills reset <name> --restore`.",
        "",
