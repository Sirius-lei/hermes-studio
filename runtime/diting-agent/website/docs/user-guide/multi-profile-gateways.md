...
If you only run one DiTing agent, you don't need this page — see
[Profiles](./profiles.md) for the basics.

## When to use this

You want this setup when you have two or more DiTing agents that should all
...
(`ai.diting.gateway-<name>.plist`) or systemd user service
(`diting-gateway-<name>.service`). This guide adds the patterns for managing
them collectively.
...
diting profile create coder
diting profile create personal-bot
diting profile create research
...
```bash
diting config set gateway.multiplex_profiles true
diting gateway restart
```

Equivalently, in the default profile's `~/.diting/config.yaml`:
...
You do **not** run `diting gateway start` for the secondary profiles — the
...
With a multiplexer running, a named-profile `diting gateway start` / `run` is a
...
home). `diting status` reports the multiplexer and the profiles it serves;
`diting status -p <name>` slices to one profile. Each profile still writes its
...
`~/.local/bin/diting-gateways` and `chmod +x` it:
...
usage() {
  echo "Usage: diting-gateways {start|stop|restart|status|list}"
}
...
run_for_profile() {
  ...
  if [ "$profile" = "default" ]; then
    diting gateway "$action"
  else
    diting -p "$profile" gateway "$action"
  fi
}
...
```bash
diting-gateways start      # start every configured profile
diting-gateways stop       # stop every configured profile
diting-gateways restart    # restart all
diting-gateways status     # status across all
diting-gateways list       # delegates to `diting gateway list`
```

:::tip
The `default` profile is targeted with `diting gateway <action>` (no `-p`),
not `diting -p default gateway <action>`. The wrapper above handles both forms.
:::
...
These are equivalent to `diting -p coder gateway <action>` — useful if a
...
| Platform | Path                                                              |
| -------- | ----------------------------------------------------------------- |
| macOS    | `~/Library/LaunchAgents/ai.diting.gateway-<profile>.plist`        |
| Linux    | `~/.config/systemd/user/diting-gateway-<profile>.service`         |

The default profile keeps the historical names: `ai.diting.gateway.plist` /
`diting-gateway.service`.
...
```bash
# Default profile
tail -f ~/.diting/logs/gateway.log
tail -f ~/.diting/logs/gateway.error.log

# Named profile
tail -f ~/.diting/profiles/<name>/logs/gateway.log
tail -f ~/.diting/profiles/<name>/logs/gateway.error.log
```

Stream every profile's log simultaneously:

```bash
tail -f ~/.diting/logs/gateway.log ~/.diting/profiles/*/logs/gateway.log
```

The CLI also has a structured log viewer:

```bash
diting logs -f                  # follow default profile
diting -p coder logs -f         # follow one profile
diting logs --help              # filters, levels, JSON output
```

## Identify what's actually running

```bash
diting profile list             # profiles + model + gateway state
diting-gateways status          # full status across every profile
launchctl list | grep diting    # macOS — PIDs and labels
systemctl --user list-units 'diting-gateway-*'   # Linux — units
```
...
```
~/.diting/profiles/<name>/
├── .env              # API keys, bot tokens (chmod 600)
├── config.yaml       # model, provider, toolsets, gateway settings
└── SOUL.md           # personality / system prompt
```

The default profile uses `~/.diting/` directly with the same three files.

Edit them with any editor or via the CLI:

```bash
diting config set model.model anthropic/claude-sonnet-4    # default profile
...
caffeinate -i -w $(cat ~/.diting/gateway.pid) &   # awake while default gateway runs
...
systemd-inhibit --what=idle:sleep --who=diting --why="gateways running" \
  sleep infinity &
...
`diting-gateway-<profile>.service`) continue running across SSH disconnects
and reboots.
...
grep -H 'TELEGRAM_BOT_TOKEN\|DISCORD_BOT_TOKEN' \
     ~/.diting/.env ~/.diting/profiles/*/.env
```

## Updating the code

`diting update` pulls the latest code once and syncs new bundled skills into
every profile:
...
You ran `diting gateway start` after a previous `diting gateway stop`. The
...
```bash
ps -ef | grep "diting_cli.*-p <profile>"
cat ~/.diting/profiles/<profile>/gateway.pid
...
```bash
# macOS
launchctl unload ~/Library/LaunchAgents/ai.diting.gateway-<profile>.plist
launchctl load   ~/Library/LaunchAgents/ai.diting.gateway-<profile>.plist

# Linux
systemctl --user restart diting-gateway-<profile>.service
```

### Health check

```bash
diting doctor                  # default profile
diting -p <profile> doctor     # one profile
```
