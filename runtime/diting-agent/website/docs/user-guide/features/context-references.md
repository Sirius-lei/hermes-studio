...
Type `@` followed by a reference to inject content directly into your message. DiTing expands the reference inline and appends the content under an `--- Attached Context ---` section.
...
- DiTing env: `$DiTing_HOME/.env`

These directories are fully blocked (any file inside):
- `~/.ssh/`, `~/.aws/`, `~/.gnupg/`, `~/.kube/`, `$DiTing_HOME/skills/.hub/`
