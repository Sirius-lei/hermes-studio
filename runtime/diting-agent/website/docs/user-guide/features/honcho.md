...
[Honcho](https://github.com/plastic-labs/honcho) is an AI-native memory backend that adds dialectic reasoning and deep user modeling on top of DiTing's built-in memory system. Instead of simple key-value storage, Honcho maintains a running model of who the user is — their preferences, communication style, goals, and patterns — by reasoning about conversations after they happen.
...
**Multi-agent profiles**: When multiple DiTing instances talk to the same user (e.g., a coding assistant and a personal assistant), Honcho maintains separate "peer" profiles. Each peer sees only its own observations and conclusions, preventing cross-contamination of context.

## Setup

```bash
diting memory setup    # select "honcho" from the provider list
```
...
- `per-session` — each `diting` run gets a fresh session. Clean starts, memory via tools. Recommended for new users.
...
Server-side toggles set via the [Honcho dashboard](https://app.honcho.dev) win over local defaults — DiTing syncs them back at session init.
...
The `diting honcho` subcommand is **only registered when Honcho is the active memory provider** (`memory.provider: honcho` in `config.yaml`). On a fresh install, configure Honcho directly with `diting memory setup honcho` (or run `diting memory setup` and pick it from the list); the `diting honcho` subcommand then appears on the next invocation.

```bash
diting memory setup honcho    # Configure Honcho directly (works before activation)
diting honcho status          # Connection status, config, and key settings
diting honcho setup           # Redirects to `diting memory setup` (post-activation alias)
diting honcho strategy        # Show or set session strategy (per-session/per-directory/per-repo/global)
diting honcho peer            # Show or update peer names + dialectic reasoning level
diting honcho mode            # Show or set recall mode (hybrid/context/tools)
diting honcho tokens          # Show or set token budget for context and dialectic
diting honcho identity        # Seed or show the AI peer's Honcho identity
diting honcho sync            # Sync Honcho config to all existing profiles
diting honcho peers           # Show peer identities across all profiles
diting honcho sessions        # List known Honcho session mappings
diting honcho map             # Map current directory to a Honcho session name
diting honcho enable          # Enable Honcho for the active profile
diting honcho disable         # Disable Honcho for the active profile
diting honcho migrate         # Step-by-step migration guide from openclaw-honcho
```

## Migrating from `diting honcho`

If you previously used the standalone `diting honcho setup`:
...
No re-login or re-setup needed. Run `diting memory setup` and select "honcho" — the wizard detects your existing config.
