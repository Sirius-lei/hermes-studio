...
DiTing runs full language servers — pyright, gopls, rust-analyzer,
...
This is the same architecture top-tier coding agents use. DiTing
...
1. DiTing captures a baseline of current diagnostics for the file.
...
…). DiTing auto-detects the binary on PATH or in
`<DiTing_HOME>/lsp/bin/`.
...
2. Download the latest release zip from
   ...
   and extract it.
3. Point DiTing at the extracted bundle — the directory that contains
   `PowerShellEditorServices/Start-EditorServices.ps1`. Either:
   - set `lsp.servers.powershell.command: ["/path/to/bundle"]` in
     `config.yaml`, or
   - extract it to `<DiTing_HOME>/lsp/PowerShellEditorServices`, or
   - export `PSES_BUNDLE_PATH=/path/to/bundle`.

`diting lsp status` reports `installed` once `pwsh` is found; if the
...
`node_modules` tree — DiTing installs both packages together when you
run `diting lsp install typescript` or auto-install fires on first
use.

## CLI

```
diting lsp status          # service state + per-server install status
diting lsp list            # registry, optionally --installed-only
diting lsp install <id>    # eagerly install one server
diting lsp install-all     # try every server with a known recipe
diting lsp restart         # tear down running clients
diting lsp which <id>      # print resolved binary path
```

`diting lsp status` is the best starting point — it shows which
...
lsp:
  ...
  #   auto    — install via npm/pip/go install into <DiTing_HOME>/lsp/bin
...
When `install_strategy: auto`, DiTing installs binaries into
`<DiTing_HOME>/lsp/bin/`. NPM packages land in
`<DiTing_HOME>/lsp/node_modules/` with bin symlinks one level up.
Go binaries come from `go install` with `GOBIN` pointed at the
staging dir.

Nothing is ever installed to `/usr/local/`, `~/.local/`, or any other
shared location — the staging dir is fully DiTing-owned and is
...
Servers are kept alive for the life of the DiTing process. There's
...
**`diting lsp status` shows a server as "missing"**

The binary isn't on PATH and isn't in `<DiTing_HOME>/lsp/bin/`. Run
`diting lsp install <server_id>` to attempt an auto-install, or
install the binary manually through the language's normal toolchain.

**`Backend warnings` section in `diting lsp status`**
...
When `diting lsp status` shows a `Backend warnings` section, install
...
`~/.diting/logs/agent.log`.

**Server starts but never returns diagnostics**

Check `~/.diting/logs/agent.log` for `[agent.lsp.client]` entries —
...
the rest of the session. Run `diting lsp restart` to clear the set;
