...
When DiTing Agent runs inside a messaging gateway (Slack, Discord, Telegram,
...
persona in `~/.diting/SOUL.md`, or as a named preset under
`agent.personalities` in `~/.diting/config.yaml` (switchable per session
via `/personality`).
...
If you use DiTing' kanban multi-agent workflow, workers can attach
...
Install MCP servers via `~/.diting/config.yaml` under the `mcp_servers`
...
back into the thread as a native attachment. DiTing Agent's deliverable
