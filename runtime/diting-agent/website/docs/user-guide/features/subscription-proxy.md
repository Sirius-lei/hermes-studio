...
chat completions — use your DiTing-managed provider subscription as their
...
| Use case | "Use DiTing as a chat backend" | "Use my Portal sub from another app" |
...
This opens your browser for the Nous Portal OAuth flow. DiTing stores
the refresh token in `~/.diting/auth.json` — the same place all DiTing
...
Model:      DiTing-4-70B    # or DiTing-4.3-36B, DiTing-4-405B
```
...
interface in `diting_cli/proxy/adapters/`.
...
If you see `not logged in`, run `diting portal`. If you see
...
`diting_cli/proxy/adapters/<provider>.py` and registering it in
