...
DiTing can show an animated **pet** — a small mascot sprite that reacts to what
...
- Pets are installed into your profile's `pets/` directory
  (`<DiTing_HOME>/pets/<slug>/`), so each [profile](../profiles.md) keeps its
  own set.
...
In the terminal (CLI/TUI), DiTing renders the sprite at full fidelity when your
...
diting pets list
diting pets list cat

# Install a pet and make it active in one step
diting pets install boba --select
...
## `diting pets` commands

| Goal | Command |
| --- | --- |
| Browse the gallery | `diting pets list [query] [--limit N]` |
| List installed pets | `diting pets list --installed` |
| Install a pet | `diting pets install <slug> [--select] [--force]` |
| Set the active pet | `diting pets select [slug]` (omit slug for a picker) |
| Resize the pet everywhere | `diting pets scale <factor>` (e.g. `0.5`, clamped 0.1–3.0) |
| Preview/animate | `diting pets show [slug] [--state <s>] [--cycle] [--once] [--mode <m>] [--scale <f>]` |
| Disable the pet | `diting pets off` |
| Remove an installed pet | `diting pets remove <slug>` |
| Diagnose setup | `diting pets doctor` |

`diting pets show` flags:
...
Beyond installing pre-made pets from the gallery, DiTing can **generate a brand-new pet** from a text description — its own AI sprite-generation pipeline.

- CLI/TUI: `/hatch <description>` (alias `/generate-pet`), or `diting pets` → the generate flow.
...
- If no reference-capable backend is configured, generation surfaces an actionable error pointing you to `diting tools` → Image Generation. (Installing/adopting existing gallery pets needs no image backend.)
- Override the backend with the `DiTing_PET_IMAGE_PROVIDER` env var (e.g. `DiTing_PET_IMAGE_PROVIDER=openrouter`).
...
always-on-top desktop window. Out there it stays visible while DiTing is
...
Run `diting pets doctor` — it reports:
...
- The petdex npm CLI installs to `~/.codex/pets`; DiTing uses its own
  profile-scoped `<DiTing_HOME>/pets/` instead — install through `diting pets`.
