...
DiTing Agent can drive your desktop — clicking, typing, scrolling,
...
```
diting computer-use install
```

This fetches and runs the upstream cua-driver installer — `install.sh`
on macOS/Linux, `install.ps1` on Windows. Use `diting computer-use
status` to verify the install.

**Option 2: enable the toolset interactively.**

1. Run `diting tools`, pick `🖱️  Computer Use (macOS/Windows/Linux)`.
...
| Platform | Prereqs |
|---|---|
| **macOS** | System Settings → Privacy & Security → **Accessibility** + **Screen Recording** → allow your terminal (or DiTing app). `diting computer-use doctor` will tell you which permission is missing. |
...
```
diting -t computer_use chat
```

or add `computer_use` to your enabled toolsets in `~/.diting/config.yaml`.

## `diting computer-use doctor` — your first triage stop

`diting computer-use doctor` runs cua-driver's structured
`health_report` MCP tool and prints a per-check matrix. It's the single
fastest way to find out *why* an action isn't working.

```
$ diting computer-use doctor
...
agent is acting here." Each DiTing run declares its own cua-driver
**session id** (something like `diting-3a7b9c14d2e8`); the cursor's
...
DiTing intentionally keeps its skill (`skills/computer-use/SKILL.md`)
focused on the DiTing-side `computer_use` action vocabulary — the
...
These are **platform deep dives, not duplicates of the DiTing skill** —
...
Code, Codex, OpenCode, OpenClaw, and Antigravity; **DiTing
...
DiTing applies multi-layer guardrails:
...
Pair with `approvals.mode: manual` in `~/.diting/config.yaml` if you
want every action confirmed.

## Token efficiency

Screenshots are expensive. DiTing applies four layers of optimisation:
...
- **Windows: elevated (admin) windows can't be driven from a normal
  ...
  DiTing agent) cannot enumerate the UIA tree of, or inject mouse input
  ...
  run the DiTing agent itself at High integrity (launch from an
...
- **Platform-specific deployment gotchas:**
  - **macOS** uses private SkyLight SPIs. Apple can change them in any
    OS update. DiTing warns when the installed cua-driver is older than
    the version it was tested against.
  - **Windows** SSH sessions run in **Session 0**, which has no
    interactive desktop. Drive DiTing from inside the RDP / console
...
```
DiTing_CUA_DRIVER_CMD=/path/to/your/cua-driver
```

Swap the backend entirely (for testing):

```
DiTing_COMPUTER_USE_BACKEND=noop   # records calls, no side effects
```

### Telemetry

cua-driver ships with anonymous usage telemetry (PostHog) enabled by default
upstream. **DiTing disables it for you** — on every cua-driver invocation
(the MCP backend, `status`, `doctor`, and install) DiTing sets
...
When it's on, `diting computer-use doctor` reports `telemetry: enabled`;
...
unreleased fix — point DiTing at a binary you built from source instead
of the published release. DiTing resolves the driver with
`shutil.which("cua-driver")` and **does not enforce
`DiTing_CUA_DRIVER_VERSION`**, so a local build (reported as
...
- `-NoAutoStart` skips registering the `cua-driver-serve` logon daemon
  — you don't need it for DiTing testing (see notes).
...
### Option B — point DiTing straight at the built binary (fastest loop)

Skip the install ceremony entirely: `cargo build` and set
`DiTing_CUA_DRIVER_CMD` to the resulting binary. Best for rapid
edit/build/test.
...
```
# Windows (.env)
DiTing_CUA_DRIVER_CMD=C:\path\to\cua\libs\cua-driver\rust\target\debug\cua-driver.exe
# macOS / Linux (.env)
DiTing_CUA_DRIVER_CMD=/path/to/cua/libs/cua-driver/rust/target/debug/cua-driver
```

### Confirm DiTing is using your build

- `diting computer-use status` prints the resolved binary path and
  version.
- `diting computer-use doctor` confirms the binary is reachable and
...
- **DiTing spawns its own `cua-driver mcp` child over stdio** — it does
...
- **Rebuild loop.** After editing cua-driver source, re-run
  ...
  for Option A, or just re-`cargo build` for Option B — no DiTing
  change needed either way.
- **Local builds skip the version check.** DiTing warns when the
...
**First action when anything's off: run `diting computer-use doctor`.**
...
Run `diting computer-use install` to fetch the cua-driver binary, or
run `diting tools` and enable the Computer Use toolset.
...
Wayland without an XWayland bridge. `diting computer-use doctor` will
...
- **DiTing-side skill** — `skills/computer-use/SKILL.md` — teaches the
  DiTing `computer_use` action vocabulary; this is what the agent loads.
- **cua-driver skill pack** — for platform-specific deep dives
  ...
  install` autodetects DiTing (planned follow-up), this happens
...
- **cua.ai/docs** — the cua-driver project's documentation:
  ...
  - [Connect your agent](https://cua.ai/docs/how-to-guides/driver/connect-your-agent) — register cua-driver with various harnesses (DiTing among them)
