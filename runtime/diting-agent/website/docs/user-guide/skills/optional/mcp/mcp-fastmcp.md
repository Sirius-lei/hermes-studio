...
| | |
|---|---|
| Source | Optional — install with `diting skills install official/mcp/fastmcp` |
...
- smoke-test a server with the FastMCP CLI before wiring it into DiTing or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to DiTing. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.diting/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.diting/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is DiTing integration, either:

- configure the server in `~/.diting/config.yaml` using the `native-mcp` skill, or
...
### DiTing cannot see the deployed server

The server-building part may be correct while the DiTing config is not. Load the `native-mcp` skill and configure the server in `~/.diting/config.yaml`, then restart DiTing.
