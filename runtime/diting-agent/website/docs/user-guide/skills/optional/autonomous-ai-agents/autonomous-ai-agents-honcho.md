...
description: "Configure and use Honcho memory with DiTing -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session su..."
---
...
Configure and use Honcho memory with DiTing -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session summaries, and context budget enforcement. Use when setting up Honcho, troubleshooting memory, managing profiles with Honcho peers, or tuning observation, recall, and dialectic settings.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `diting skills install official/autonomous-ai-agents/honcho` |
...
# Honcho Memory for DiTing

Honcho provides AI-native cross-session user modeling. It learns who the user is across conversations and gives every DiTing profile its own peer identity while sharing a unified view of the user.
...
See: https://docs.honcho.dev/v3/guides/integrations/diting#running-honcho-locally-with-diting

### Verify

```bash
diting honcho status    # shows resolved config, connection test, peer info
```
...
3. **AI peer card** -- the identity card for this DiTing profile's AI peer
...
Honcho models conversations as interactions between **peers**. DiTing creates two peers per session:

- **User peer** (`peerName`): represents the human. Honcho builds a user representation from observed messages.
- **AI peer** (`aiPeer`): represents this DiTing instance. Each profile gets its own AI peer so agents develop independent views.
...
| `per-session` | New Honcho session each DiTing run |
| `global` | Single session across all directories |

Manual override: `diting honcho map my-project-name`
...
Each DiTing profile gets its own Honcho AI peer while sharing the same workspace (user context). This means:
...
# creates host block diting.coder, AI peer "coder", inherits config from default
```

What `--clone` does for Honcho:
1. Creates a `diting.coder` host block in `honcho.json`
...
```bash
diting honcho sync    # creates host blocks for all profiles that don't have one yet
```
...
Guidelines for DiTing when Honcho memory is active.
...
Config file: `$DiTing_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global).
...
Run `diting honcho setup`. Ensure `memory.provider: honcho` is in `~/.diting/config.yaml`.

### Memory not persisting across sessions
Check `diting honcho status` -- verify `saveMessages: true` and `writeFrequency` isn't `session` (which only writes on exit).

### Profile not getting its own peer
Use `--clone` when creating: `diting profile create <name> --clone`. For existing profiles: `diting honcho sync`.
...
| Command | Description |
|---------|-------------|
| `diting honcho setup` | Interactive setup wizard (cloud/local, identity, observation, recall, sessions) |
| `diting honcho status` | Show resolved config, connection test, peer info for active profile |
| `diting honcho enable` | Enable Honcho for the active profile (creates host block if needed) |
| `diting honcho disable` | Disable Honcho for the active profile |
| `diting honcho peer` | Show or update peer names (`--user <name>`, `--ai <name>`, `--reasoning <level>`) |
| `diting honcho peers` | Show peer identities across all profiles |
| `diting honcho mode` | Show or set recall mode (`hybrid`, `context`, `tools`) |
| `diting honcho tokens` | Show or set token budgets (`--context <N>`, `--dialectic <N>`) |
| `diting honcho sessions` | List known directory-to-session-name mappings |
| `diting honcho map <name>` | Map current working directory to a Honcho session name |
| `diting honcho identity` | Seed AI peer identity or show both peer representations |
| `diting honcho sync` | Create host blocks for all DiTing profiles that don't have one yet |
| `diting honcho migrate` | Step-by-step migration guide from OpenClaw native memory to DiTing + Honcho |
| `diting memory setup` | Generic memory provider picker (selecting "honcho" runs the same wizard) |
| `diting memory status` | Show active memory provider and config |
| `diting memory off` | Disable external memory provider |
