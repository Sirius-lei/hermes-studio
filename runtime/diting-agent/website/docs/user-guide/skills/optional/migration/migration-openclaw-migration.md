---
title: "Openclaw Migration — Migrate a user's OpenClaw customization footprint into DiTing Agent"
sidebar_label: "Openclaw Migration"
description: "Migrate a user's OpenClaw customization footprint into DiTing Agent"
---
...
Migrate a user's OpenClaw customization footprint into DiTing Agent. Imports DiTing-compatible memories, SOUL.md, command allowlists, user skills, and selected workspace assets from ~/.openclaw, then reports exactly what could not be migrated and why.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `diting skills install official/migration/openclaw-migration` |
| Path | `optional-skills/migration/openclaw-migration` |
| Version | `1.0.0` |
| Author | DiTing Agent (Nous Research) |
| License | MIT |
| Platforms | linux, macos, windows |
| Tags | `Migration`, `OpenClaw`, `DiTing`, `Memory`, `Persona`, `Import` |
| Related skills | [`diting-agent`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-diting-agent) |
...
# OpenClaw -> DiTing Migration

Use this skill when a user wants to move their OpenClaw setup into DiTing Agent with minimal manual cleanup.
...
```bash
diting claw migrate              # Full interactive migration
...
diting claw migrate --source /custom/path/.openclaw  # Custom source
```
...
**First-time setup:** The `diting setup` wizard automatically detects `~/.openclaw` and offers migration before configuration begins.

## What this skill does

It uses `scripts/openclaw_to_diting.py` to:

- import `SOUL.md` into the DiTing home directory as `SOUL.md`
- transform OpenClaw `MEMORY.md` and `USER.md` into DiTing memory entries
- merge OpenClaw command approval patterns into DiTing `command_allowlist`
- migrate DiTing-compatible messaging settings such as `TELEGRAM_ALLOWED_USERS`, and map OpenClaw workspace settings to DiTing working-directory configuration
- copy OpenClaw skills into `~/.diting/skills/openclaw-imports/`
- optionally copy the OpenClaw workspace instructions file into a chosen DiTing workspace
- mirror compatible workspace assets such as `workspace/tts/` into `~/.diting/tts/`
- archive non-secret docs that do not have a direct DiTing destination
...
- `scripts/openclaw_to_diting.py`

When this skill is installed from the Skills Hub, the normal location is:

- `~/.diting/skills/migration/openclaw-migration/scripts/openclaw_to_diting.py`

Do not guess a shorter path like `~/.diting/skills/openclaw-migration/...`.

Before running the helper:

1. Prefer the installed path under `~/.diting/skills/migration/openclaw-migration/`.
...
With `--migrate-secrets`, it will also import a small allowlisted set of DiTing-compatible secrets, currently:
...
DiTing CLI supports the `clarify` tool for interactive prompts, but it is limited to:
...
- `{"question":"One or more imported OpenClaw skills already exist in DiTing. How should I handle those skill conflicts?","choices":["keep existing skills","overwrite conflicting skills with backup","import conflicting skills under renamed folders"]}`
...
- `{"question":"Do you want to copy the OpenClaw workspace instructions file into a DiTing workspace?","choices":["skip workspace instructions","copy to a workspace path","decide later"]}`
...
13. If a renamed imported skill has an empty `details.backup`, do not imply the existing DiTing skill was renamed or backed up. Say only that the imported copy was placed in the new destination and reference `details.renamed_from` as the pre-existing folder that remained in place.
...
```bash
python3 ~/.diting/skills/migration/openclaw-migration/scripts/openclaw_to_diting.py
```

When using the terminal tool, prefer an absolute invocation pattern such as:

```json
{"command":"python3 /home/USER/.diting/skills/migration/openclaw-migration/scripts/openclaw_to_diting.py","workdir":"/home/USER"}
```

Dry run with the user-data preset:

```bash
python3 ~/.diting/skills/migration/openclaw-migration/scripts/openclaw_to_diting.py --preset user-data
```

Execute a user-data migration:

```bash
python3 ~/.diting/skills/migration/openclaw-migration/scripts/openclaw_to_diting.py --execute --preset user-data --skill-conflict skip
```

Execute a full compatible migration:

```bash
python3 ~/.diting/skills/migration/openclaw-migration/scripts/openclaw_to_diting.py --execute --preset full --migrate-secrets --skill-conflict skip
```

Execute with workspace instructions included:

```bash
python3 ~/.diting/skills/migration/openclaw-migration/scripts/openclaw_to_diting.py --execute --preset user-data --skill-conflict rename --workspace-target "/absolute/workspace/path"
```
...
2. Do not migrate secrets by default. Tokens, auth blobs, device credentials, and raw gateway config should stay out of DiTing unless the user explicitly asks for secret migration.
3. Do not silently overwrite non-empty DiTing targets unless the user explicitly wants that. The helper script will preserve backups when overwriting is enabled.
...
6. Even in secret-migration mode, only migrate secrets with a clean DiTing destination. Unsupported auth blobs must still be reported as skipped.
...
- DiTing persona state imported
- DiTing memory files populated with converted OpenClaw knowledge
- OpenClaw skills available under `~/.diting/skills/openclaw-imports/`
