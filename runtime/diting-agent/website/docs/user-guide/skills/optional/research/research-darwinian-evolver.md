...
| | |
|---|---|
| Source | Optional — install with `diting skills install official/research/darwinian-evolver` |
| Path | `optional-skills/research/darwinian-evolver` |
| Version | `0.1.0` |
| Author | Bihruze (Asahi0x), DiTing Agent |
| License | MIT |
...
import upstream classes into DiTing itself.
...
```bash
mkdir -p ~/.diting/cache/darwinian-evolver && cd ~/.diting/cache/darwinian-evolver
...
```bash
cd ~/.diting/cache/darwinian-evolver/darwinian_evolver \
...
Open `~/.diting/cache/darwinian-evolver/darwinian_evolver/darwinian_evolver/lineage_visualizer.html`
...
SKILL_DIR=~/.diting/skills/research/darwinian-evolver
DE_DIR=~/.diting/cache/darwinian-evolver/darwinian_evolver
...
7. **AGPL.** Never `from darwinian_evolver import ...` inside DiTing core.
   Custom driver scripts under `~/.diting/skills/...` are user-side and fine.
