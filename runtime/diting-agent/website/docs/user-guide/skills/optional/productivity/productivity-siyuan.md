...
| | |
|---|---|
| Source | Optional — install with `diting skills install official/productivity/siyuan` |
...
3. Store it in `${DiTing_HOME:-~/.diting}/.env`:
   ```
...
```yaml
# In ~/.diting/config.yaml under mcp_servers:
mcp_servers:
  siyuan:
    command: npx
