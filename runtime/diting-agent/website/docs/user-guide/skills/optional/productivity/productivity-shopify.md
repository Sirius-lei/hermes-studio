...
| | |
|---|---|
| Source | Optional — install with `diting skills install official/productivity/shopify` |
...
4. Save to `${DiTing_HOME:-~/.diting}/.env`:
   ```
...
}' '{"input":{"title":"Test Hoodie","status":"DRAFT","vendor":"DiTing","productType":"Apparel","tags":["test"]}}'
```
