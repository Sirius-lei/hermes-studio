...
| | |
|---|---|
| Source | Optional — install with `diting skills install official/productivity/here-now` |
...
- Skill helper paths:
  - `${DiTing_SKILL_DIR}/scripts/publish.sh` for publishing sites
  - `${DiTing_SKILL_DIR}/scripts/drive.sh` for private Drive storage

## Create a site

```bash
PUBLISH="${DiTing_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --client diting
```
...
```bash
PUBLISH="${DiTing_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --slug {slug} --client diting
```
...
```bash
DRIVE="${DiTing_SKILL_DIR}/scripts/drive.sh"
bash "$DRIVE" default
...
| `--client {name}`      | Agent name for attribution (e.g. `diting`)    |
