...
| | |
|---|---|
| Source | Optional — install with `diting skills install official/productivity/canvas` |
...
4. Name the token (e.g., "DiTing Agent"), set an optional expiry, and click **Generate Token**
5. Copy the token and add to `${DiTing_HOME:-~/.diting}/.env`:
...
```bash
CANVAS="python $DiTing_HOME/skills/productivity/canvas/scripts/canvas_api.py"
