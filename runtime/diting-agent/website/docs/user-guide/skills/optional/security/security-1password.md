...
| | |
|---|---|
| Source | Optional — install with `diting skills install official/security/1password` |
| Path | `optional-skills/security/1password` |
| Version | `1.0.0` |
| Author | arceus77-7, enhanced by DiTing Agent |
| License | MIT |
...
- `tmux` available for stable authenticated sessions during DiTing terminal calls (desktop app flow only)
...
### Service Account (recommended for DiTing)

Set `OP_SERVICE_ACCOUNT_TOKEN` in `${DiTing_HOME:-~/.diting}/.env` (the skill will prompt for this on first load).
...
## DiTing Execution Pattern (desktop app flow)

DiTing terminal commands are non-interactive by default and can lose auth context between calls.
...
```bash
SOCKET_DIR="${TMPDIR:-/tmp}/diting-tmux-sockets"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/diting-op.sock"
