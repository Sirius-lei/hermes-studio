...
- Give it a name like "diting-agent"
- Select scopes:
...
- Give it a title like "diting-agent-&lt;machine-name>"
...
elif _diting_env="${DiTing_HOME:-$HOME/.diting}/.env"; [ -f "$_diting_env" ] && grep -q "^GITHUB_TOKEN=" "$_diting_env"; then
  export GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" "$_diting_env" | head -1 | cut -d= -f2 | tr -d '\n\r')
  echo "AUTH_METHOD=curl"
