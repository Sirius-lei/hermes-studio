...
if command -v gh &>/dev/null && gh auth status &>/dev/null; then
  ...
  if [ -z "$GITHUB_TOKEN" ]; then
    if _diting_env="${DiTing_HOME:-$HOME/.diting}/.env"; [ -f "$_diting_env" ] && grep -q "^GITHUB_TOKEN=" "$_diting_env"; then
      GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" "$_diting_env" | head -1 | cut -d= -f2 | tr -d '\n\r')
...
curl -s -X POST \
  ...
  -d "{
    \"commit_id\": \"$HEAD_SHA\",
    \"event\": \"COMMENT\",
    \"body\": \"Code review from DiTing Agent\",
    \"comments\": [
...
```bash
source "${DiTing_HOME:-$HOME/.diting}/skills/github/github-auth/scripts/gh-env.sh"
...
gh pr review $PR_NUMBER --approve --body "Reviewed by DiTing Agent. Code looks clean — good test coverage, no security concerns."
