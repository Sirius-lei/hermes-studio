...
title: "Git Worktrees"
description: "Run multiple DiTing agents safely on the same repository using git worktrees and isolated checkouts"
---

# Git Worktrees

DiTing Agent is often used on large, long‑lived repositories. When you want to:
...
This page shows how to combine worktrees with DiTing so each session has a clean, isolated working directory.

## Why Use Worktrees with DiTing?

DiTing treats the **current working directory** as the project root:

- CLI: the directory where you run `diting` or `diting chat`
- Messaging gateways: the directory set by `terminal.cwd` in `~/.diting/config.yaml`
...
git worktree add ../repo-feature feature/diting-experiment
```

This creates:

- A new directory: `../repo-feature`
- A new branch: `feature/diting-experiment` checked out in that directory

Now you can `cd` into the new worktree and run DiTing there:

```bash
cd ../repo-feature

# Start DiTing in the worktree
diting
```
...
git worktree add ../repo-experiment-a feature/diting-a
git worktree add ../repo-experiment-b feature/diting-b
```
...
- Works on its own branch (`feature/diting-a` vs `feature/diting-b`).
...
- DiTing checkpoint data under `~/.diting/checkpoints/` is not automatically pruned when you remove a worktree, but it is usually very small.

## Best Practices

- **One worktree per DiTing experiment**
...
- **Name branches after the experiment**
  - e.g. `feature/diting-checkpoints-docs`, `feature/diting-refactor-tests`.
- **Commit frequently**
...
- **Avoid running DiTing from the bare repo root when using worktrees**
  - Prefer the worktree directories instead, so each agent has a clear scope.

## Using `diting -w` (Automatic Worktree Mode)

DiTing has a built‑in `-w` flag that **automatically creates a disposable git worktree** with its own branch. You don't need to set up worktrees manually — just `cd` into your repo and run:
...
- Check out an isolated branch (e.g. `diting/diting-<hash>`).
...
```bash
diting -w -z "Fix issue #123"
```

For parallel agents, open multiple terminals and run `diting -w` in each — every invocation gets its own worktree and branch automatically.

## Putting It All Together

- Use **git worktrees** to give each DiTing session its own clean checkout.
