...
description: "Chat with DiTing from Telegram, Discord, Slack, WhatsApp, Signal, SMS, Email, Home Assistant, Mattermost, Matrix, DingTalk, Yuanbao, Microsoft Teams, LINE, Raft, Webhooks, or any OpenAI-compatible frontend via the API server — architecture and setup overview"
---

# Messaging Gateway

Chat with DiTing from Telegram, Discord, Slack, WhatsApp, Signal, SMS, Email, Home Assistant, Mattermost, Matrix, DingTalk, Feishu/Lark, WeCom, Weixin, BlueBubbles (iMessage), QQ, Yuanbao, Microsoft Teams, LINE, ntfy, or your browser. The gateway is a single background process that connects to all your configured platforms, handles sessions, runs cron jobs, and delivers voice messages.

For the full voice feature set — including CLI microphone mode, spoken replies in messaging, and Discord voice-channel conversations — see [Voice Mode](/user-guide/features/voice-mode) and [Use Voice Mode with DiTing](/guides/use-voice-mode-with-diting).
...
```mermaid
flowchart TB
    subgraph Gateway["DiTing Gateway"]
...
For group chats, hooks, and automation flows, DiTing supports explicit silence tokens. If the agent's final response is exactly one supported token, the gateway suppresses outbound delivery and sends nothing to the chat.
...
Silence is a delivery decision only. DiTing keeps the assistant silence turn in the session transcript, so the conversation still alternates normally:
...
Failed turns still surface as errors; DiTing does not hide failures just because the text resembles a silence token.
...
```bash
diting gateway setup        # Interactive setup for all messaging platforms
```
...
```bash
diting gateway              # Run in foreground
diting gateway setup        # Configure messaging platforms interactively
diting gateway install      # Install as a user service (Linux) / launchd service (macOS)
sudo diting gateway install --system   # Linux only: install a boot-time system service
diting gateway start        # Start the default service
diting gateway stop         # Stop the default service
diting gateway status       # Check default service status
diting gateway status --system         # Linux only: inspect the system service explicitly
```
...
| `/update` | Update DiTing Agent to the latest version |
...
with the `session_reset` section in `~/.diting/config.yaml`:
...
Configure per-platform overrides in `~/.diting/gateway.json`:
...
diting pairing approve telegram XKGH5N7P

# Other pairing commands:
diting pairing list          # View pending + approved users
diting pairing revoke telegram 123456789  # Remove access
```
...
The first time you message a busy agent on any platform, DiTing appends a one-line reminder to the busy-ack explaining the knob (`"💡 First-time tip — …"`). The reminder fires once per install — a flag under `onboarding.seen.busy_input_prompt` latches it. Delete that key to see the tip again.
...
Control how much tool activity is displayed in `~/.diting/config.yaml`:
...
Off by default. When enabled, DiTing prepends a human-readable timestamp
...
When the agent running a background session uses `terminal(background=true)` to start long-running processes (servers, builds, etc.), the gateway can push status updates to your chat. Control this with `display.background_process_notifications` in `~/.diting/config.yaml`:
...
```bash
DiTing_BACKGROUND_NOTIFICATIONS=result
```
...
```bash
diting gateway install               # Install as user service
diting gateway start                 # Start the service
diting gateway stop                  # Stop the service
diting gateway status                # Check status
journalctl --user -u diting-gateway -f  # View logs

# Enable lingering (keeps running after logout)
sudo loginctl enable-linger $USER

# Or install a boot-time system service that still runs as your user
sudo diting gateway install --system
sudo diting gateway start --system
sudo diting gateway status --system
journalctl -u diting-gateway -f
```

Use the user service on laptops and dev boxes. Use the system service on VPS or headless hosts that should come back at boot without relying on systemd linger.

:::danger Don't add a custom `ExecStopPost` kill drop-in
The unit DiTing installs already shuts the gateway down cleanly with `KillMode=mixed` + `KillSignal=SIGTERM`, and uses `Restart=always` with `RestartForceExitStatus` so updates and `/restart` respawn correctly. Do **not** add a systemd drop-in such as `ExecStopPost=/bin/kill -9 $MAINPID` — `ExecStopPost` fires on *every* stop, including clean restarts, so it `SIGKILL`s the freshly spawned instance before it stabilizes and `Restart=always` immediately respawns it. The result is an infinite restart loop (and, on Telegram, a flood of restart messages). If you've added such a drop-in, remove it: `systemctl --user edit diting-gateway` (or `sudo systemctl edit diting-gateway` for a system service) and delete the `ExecStopPost` line, then `systemctl --user daemon-reload`.
:::

:::tip Headless VMs: user service + linger avoids root prompts
A system service needs root for every restart — including the automatic gateway restart at the end of `diting update`. When `diting update` runs as a non-root user, it tries passwordless `sudo systemctl`; if that's unavailable, it skips the restart and prints the manual `sudo systemctl restart diting-gateway` command (it never blocks on an interactive password prompt).
...
```bash
diting gateway install          # user service
sudo loginctl enable-linger $USER   # one-time: start at boot, survive logout
```

After that, `diting update` can restart the gateway without any privileges. If you prefer to keep the system service, either run updates with `sudo diting update`, or grant the service account passwordless sudo for systemctl, e.g. in `sudo visudo -f /etc/sudoers.d/diting-gateway`:

```
diting ALL=(root) NOPASSWD: /usr/bin/systemctl --no-ask-password reset-failed diting-gateway*, /usr/bin/systemctl --no-ask-password start diting-gateway*, /usr/bin/systemctl --no-ask-password restart diting-gateway*
```
:::

Avoid keeping both the user and system gateway units installed at once unless you really mean to. DiTing will warn if it detects both because start/stop/status behavior gets ambiguous.

:::info Multiple installations
If you run multiple DiTing installations on the same machine (with different `DiTing_HOME` directories), each gets its own systemd service name. The default `~/.diting` uses `diting-gateway`; other installations use `diting-gateway-<hash>`. The `diting gateway` commands automatically target the correct service for your current `DiTing_HOME`.
:::

### macOS (launchd)

```bash
diting gateway install               # Install as launchd agent
diting gateway start                 # Start the service
diting gateway stop                  # Stop the service
diting gateway status                # Check status
tail -f ~/.diting/logs/gateway.log   # View logs
```

The generated plist lives at `~/Library/LaunchAgents/ai.diting.gateway.plist`. It includes three environment variables:
...
- **DiTing_HOME** — scopes the gateway to your DiTing installation.

:::tip PATH changes after install
launchd plists are static — if you install new tools (e.g. a new Node.js version via nvm, or ffmpeg via Homebrew) after setting up the gateway, run `diting gateway install` again to capture the updated PATH. The gateway will detect the stale plist and reload automatically.
:::

:::info Multiple installations
Like the Linux systemd service, each `DiTing_HOME` directory gets its own launchd label. The default `~/.diting` uses `ai.diting.gateway`; other installations use `ai.diting.gateway-<suffix>`.
:::
...
|----------|---------|--------------|
| CLI | `diting-cli` | Full access |
| Telegram | `diting-telegram` | Full tools including terminal |
| Discord | `diting-discord` | Full tools including terminal |
| WhatsApp | `diting-whatsapp` | Full tools including terminal |
| WhatsApp Cloud API | `diting-whatsapp` | Full tools including terminal (shares toolset with the Baileys bridge) |
| Slack | `diting-slack` | Full tools including terminal |
| Google Chat | `diting-google_chat` | Full tools including terminal |
| Signal | `diting-signal` | Full tools including terminal |
| SMS | `diting-sms` | Full tools including terminal |
| Email | `diting-email` | Full tools including terminal |
| Home Assistant | `diting-homeassistant` | Full tools + HA device control (ha_list_entities, ha_get_state, ha_call_service, ha_list_services) |
| Mattermost | `diting-mattermost` | Full tools including terminal |
| Matrix | `diting-matrix` | Full tools including terminal |
| DingTalk | `diting-dingtalk` | Full tools including terminal |
| Feishu/Lark | `diting-feishu` | Full tools including terminal |
| WeCom | `diting-wecom` | Full tools including terminal |
| WeCom Callback | `diting-wecom-callback` | Full tools including terminal |
| Weixin | `diting-weixin` | Full tools including terminal |
| BlueBubbles | `diting-bluebubbles` | Full tools including terminal |
| QQBot | `diting-qqbot` | Full tools including terminal |
| Yuanbao | `diting-yuanbao` | Full tools including terminal |
| Microsoft Teams | `diting-teams` | Full tools including terminal |
| API Server | `diting-api-server` | Full tools (drops `clarify`, `text_to_speech` — programmatic access doesn't have an interactive user) |
| Webhooks | `diting-webhook` | Full tools including terminal |
| Raft | `diting-raft` | Wake-only channel; agent uses Raft CLI for message I/O |
...
1. **Gateway log** (`~/.diting/logs/gateway.log` or the systemd / launchd unit log). Search for the platform name and `circuit breaker`, `paused`, or `disabled`. The trip event includes the failure count and the last error.
