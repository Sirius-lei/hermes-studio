...
Connect DiTing to WeCom (Enterprise WeChat) as a self-built enterprise application using the callback/webhook model.

:::info WeCom Bot vs WeCom Callback
DiTing supports two WeCom integration modes:
...
> Run `diting gateway setup` and pick **WeCom Callback** for a guided walk-through.
...
3. DiTing decrypts the message, queues it for the agent
...
(Use `diting gateway start` only after `diting gateway install` has registered the systemd/launchd service.)
...
console. A mismatch between the token configured in DiTing and the token the
...
in `~/.diting/.env` values around `=` will also break signature checks. After
fixing, restart `diting gateway run`.
...
Check `diting gateway run` logs for the bound host/port. If the adapter bound to
