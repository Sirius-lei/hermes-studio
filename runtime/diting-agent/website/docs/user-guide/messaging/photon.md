...
Connect DiTing to **iMessage** through [Photon][photon], a managed
...
your first iMessage from DiTing — just a phone number we can bind to
your account.
:::
...
both directions. Because the SDK is TypeScript-only, DiTing runs it in a
...
diting photon setup --phone +15551234567
```
...
2. **Finds or creates** the `DiTing Agent` project on your account.
...
Runtime credentials are written to `~/.diting/.env`
...
(device token, dashboard project id) lives in `~/.diting/auth.json` under
`credential_pool.photon` / `credential_pool.photon_project`.

## Authorizing users

Photon uses the same authorization model as every other DiTing
channel. Choose one approach:

**DM pairing (default).** When an unknown number messages your Photon
line, DiTing replies with a pairing code. Approve it with:

```bash
diting pairing approve photon <CODE>
```

Use `diting pairing list` to see pending codes and approved users.

**Pre-authorize specific numbers** (in `~/.diting/.env`):

```bash
PHOTON_ALLOWED_USERS=+15551234567,+15559876543
```

**Open access** (dev only, in `~/.diting/.env`):
...
By default DiTing responds to every authorized DM and group message.
...
they match a wake-word pattern. The defaults match `DiTing` and
`@DiTing agent` variants. For a custom agent name, set regex patterns:
...
Send an iMessage to your assigned number and DiTing will reply.

## Status & troubleshooting

```bash
diting photon status
```

Prints saved credentials, sidecar health, your registered number, and the
assigned iMessage line DiTing uses. When a Photon token and dashboard project
...
- **`sidecar deps : ✗ run diting photon install-sidecar`** — Node is
  installed but `spectrum-ts` isn't. Run the suggested command.
- **`device token : ✗ missing`** — run `diting photon setup` to log in.
- **`No iMessage line assigned yet`** — Spectrum is enabled but no line
  has been provisioned; re-run `diting photon setup` or check the
  [dashboard][app].
- **Sidecar won't start** — confirm `node --version` is 18.17+ and that
  `diting photon install-sidecar` completed without errors.
...
- **Outbound attachments are supported.** DiTing sends images, voice
...
| `PHOTON_MENTION_PATTERNS` | DiTing wake words  | JSON list / comma / newline regex patterns for group mentions |
