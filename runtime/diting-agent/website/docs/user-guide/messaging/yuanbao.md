...
sidebar_position: 16
title: "Yuanbao"
description: "Connect DiTing Agent to the Yuanbao enterprise messaging platform via WebSocket gateway"
---

# Yuanbao

Connect DiTing to [Yuanbao](https://yuanbao.tencent.com/), Tencent's enterprise messaging platform. The adapter uses a WebSocket gateway for real-time message delivery and supports both direct (C2C) and group conversations.
...
After initial setup, verify these variables in `~/.diting/.env`:
...
All standard DiTing commands work on Yuanbao:
...
4. Review gateway logs: `tail -f ~/.diting/logs/gateway.log`
...
4. Consider enabling verbose logging: `DiTing_LOG_LEVEL=debug`
...
Yuanbao has a maximum message size. DiTing automatically chunks large responses with Markdown-aware splitting (respects code fences, tables, and paragraph boundaries).
...
```bash
DiTing_LOG_LEVEL=debug diting gateway
```
...
```bash
diting chat -q "Send 'Hello from CLI' to yuanbao:group:group_code"
```
