...
`~/.diting/config.yaml`, `~/.diting/.env`, and even the shell environment — for
...
Managed scope is read from a system-level directory, default `/etc/diting`:

```text
/etc/diting/
├── config.yaml     # managed config layer (wins over ~/.diting/config.yaml)
└── .env            # managed env layer (wins over ~/.diting/.env + shell)
```
...
The location can be relocated with the `DiTing_MANAGED_DIR` environment variable
(for containers or non-`/etc` deployments). This is a deployment/bootstrap path
knob — like `DiTing_HOME` — set by the same administrator who owns the managed
files. It is **never persisted** to any `.env` by DiTing.

```bash
# Point managed scope at a custom directory (set by IT / the deployment, not the user)
export DiTing_MANAGED_DIR=/opt/org/diting-policy
```

:::warning
A user who can set `DiTing_MANAGED_DIR` can repoint managed scope at a directory
...
left user-settable. `diting doctor` reports the *resolved* managed directory so
...
|---|---|---|
| 1 | `/etc/diting/config.yaml` (managed) | `/etc/diting/.env` (managed) |
| 2 | `~/.diting/config.yaml` (user) | `~/.diting/.env` (user) |
...
```bash
diting config        # shows a header naming the managed source + the pinned keys
diting doctor        # reports the resolved managed dir + pinned key counts
```

If you try to change a managed value, DiTing refuses and names the source:

```bash
$ diting config set model.default my/model
Cannot set 'model.default': it is managed by your administrator
(/etc/diting/config.yaml) and cannot be changed.
```

The same applies to managed secrets — `diting config set` / setup will not write
a user value for an env key pinned by the managed `.env`.

## Setting up a managed scope (administrators)

```bash
sudo mkdir -p /etc/diting

# Pin some config values for every user on this machine
sudo tee /etc/diting/config.yaml >/dev/null <<'YAML'
model:
  provider: nous
security:
  redact_secrets: true
...
sudo tee /etc/diting/.env >/dev/null <<'ENV'
OPENAI_API_BASE=https://inference.example.com/v1
ENV

sudo chmod 0755 /etc/diting
sudo chmod 0644 /etc/diting/config.yaml /etc/diting/.env
```

Changes take effect on the next DiTing start (a malformed managed file is logged
loudly and ignored — it never blocks startup, but the admin should check
`diting doctor` to confirm the policy is being applied).

## Security model and limitations (v1)

- **Enforcement is filesystem permissions only.** If a user has write access to
  the managed directory (or runs DiTing as `root`), managed scope is advisory.
