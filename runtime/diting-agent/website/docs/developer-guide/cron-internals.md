...
description: "How DiTing stores, schedules, edits, pauses, skill-loads, and delivers cron jobs"
---
...
| `diting_cli/cron.py` | CLI `diting cron` subcommands |
...
Jobs are stored in `~/.diting/cron/jobs.json` with atomic write semantics (write to temp file, then rename). Each job record contains:
...
In CLI mode, cron jobs only fire when `diting cron` commands are run or during active CLI sessions.
...
```python
# ~/.diting/scripts/check_competitors.py
...
| Local file | `local` | Save to `~/.diting/cron/output/` |
...
The scheduler uses cross-process file-based locking (`fcntl.flock` on Unix, `msvcrt.locking` on Windows) to prevent overlapping ticks from executing the same due-job batch twice — even between the gateway's in-process ticker and a standalone `diting cron` / manual `tick()` call. If the lock cannot be acquired, `tick()` returns 0 immediately.

## CLI Interface

The `diting cron` CLI provides direct job management:

```bash
diting cron list                    # Show all jobs
diting cron create                  # Interactive job creation (alias: add)
diting cron edit <job_id>           # Edit job configuration
diting cron pause <job_id>          # Pause a running job
diting cron resume <job_id>         # Resume a paused job
diting cron run <job_id>            # Trigger immediate execution
diting cron remove <job_id>         # Delete a job
```
