...
sidebar_position: 1
title: "Architecture"
description: "DiTing Agent internals — major subsystems, execution paths, data flow, and where to read next"
---

# Architecture

This page is the top-level map of DiTing Agent internals. Use it to orient yourself in the codebase, then dive into subsystem-specific docs for implementation details.
...
│ diting_state.py   │              │ Browser (5 backends)  │
...
├── cli.py                    # DiTingCLI — interactive terminal UI (large file)
├── model_tools.py            # Tool discovery, schema collection, dispatch
├── toolsets.py               # Tool groupings and platform presets
├── diting_state.py           # SQLite session/state database with FTS5
├── diting_constants.py       # DiTing_HOME, profile-aware paths
...
│
├── diting_cli/               # CLI subcommands and setup
│   ├── main.py               # Entry point — all `diting` subcommands (large file)
...
│   ├── skills_config.py      # diting skills — enable/disable per platform
│   ├── skills_hub.py         # /skills slash command
│   ├── tools_config.py       # diting tools — enable/disable per platform
│   ├── plugins.py            # PluginManager — discovery, loading, hooks
│   ├── callbacks.py          # Terminal callbacks (clarify, sudo, approval)
│   └── gateway.py            # diting gateway start/stop
│
...
```text
User input → DiTingCLI.process_input()
...
Three discovery sources: `~/.diting/plugins/` (user), `.diting/plugins/` (project), and pip entry points. Plugins register tools, hooks, and CLI commands through a context API. Two specialized plugin types exist: memory providers (`plugins/memory/`) and context engines (`plugins/context_engine/`). Both are single-select — only one of each can be active at a time, configured via `diting plugins` or `config.yaml`.
...
Exposes DiTing as an editor-native agent over stdio/JSON-RPC for VS Code, Zed, and JetBrains.
...
| **Profile isolation** | Each profile (`diting -p <name>`) gets its own DiTing_HOME, config, memory, sessions, and gateway PID. Multiple profiles run concurrently. |
