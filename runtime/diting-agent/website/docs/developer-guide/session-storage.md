...
DiTing Agent uses a SQLite database (`~/.diting/state.db`) to persist session
metadata, full message history, and model configuration across CLI and gateway
sessions. This replaces the earlier per-session JSONL file approach.

Source file: `diting_state.py`


## Architecture Overview

```
~/.diting/state.db (SQLite, WAL mode)
...
Multiple diting processes (gateway + CLI sessions + worktree agents) share one
...
db = SessionDB()                           # Default: ~/.diting/state.db
...
Default path: `~/.diting/state.db`

This is derived from `diting_constants.get_diting_home()` which resolves to
`~/.diting/` by default, or the value of `DiTing_HOME` environment variable.
