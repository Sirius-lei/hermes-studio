...
sidebar_position: 4
title: "Contributing"
description: "How to contribute to DiTing Agent — dev setup, code style, PR process"
---

# Contributing

Thank you for contributing to DiTing Agent! This guide covers setting up your dev environment, understanding the codebase, and getting your PR merged.
...
- Building a custom/local tool without modifying DiTing core? Start with [Build a DiTing Plugin](../developer-guide/plugins/index.md)
- Building a new built-in core tool for DiTing itself? Start with [Adding Tools](./adding-tools.md)
...
You can also run a fully isolated DiTing instance (throwaway DiTing_HOME, separate Electron
userData, distinct Electron app name to avoid the single-instance lock):

```bash
scripts/dev-sandbox.sh python -m diting_cli.main
scripts/dev-sandbox.sh --persistent python -m diting_cli.main desktop  # state survives restarts, but lives in the worktree :)
```
...
echo 'OPENROUTER_API_KEY=sk-or-v1-your-key' >> ~/.diting/.env
```
...
- **Profile-safe paths**: Never hardcode `~/.diting` — use `get_diting_home()` from `diting_constants` for code paths and `display_diting_home()` for user-facing messages. See [AGENTS.md](https://github.com/NousResearch/diting-agent/blob/main/AGENTS.md#profiles-multi-instance-support) for full rules.
...
By contributing, you agree that your contributions will be licensed under the [MIT License](https://github.com/NousResearch/diting-agent/blob/main/LICENSE).
