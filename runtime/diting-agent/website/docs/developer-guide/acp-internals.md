...
The ACP adapter wraps DiTing' synchronous `AIAgent` in an async JSON-RPC stdio server.
...
```text
diting acp / diting-acp / python -m acp_adapter
  -> acp_adapter.entry.main()
  -> parse --version / --check / --setup before server startup
  -> load ~/.diting/.env
  -> configure stderr logging
  -> construct DiTingACPAgent
  -> acp.run_agent(agent, use_unstable_protocol=True)
```

The Zed ACP Registry path launches the same adapter through `uvx --from 'diting-agent[acp]==<version>' diting-acp`, pointed at the `diting-agent` PyPI release.
...
- `allow_once` -> DiTing `once`
- `allow_always` -> DiTing `always`
- reject options -> DiTing `deny`

Timeouts and bridge failures deny by default.

### Tool rendering helpers

`acp_adapter/tools.py` maps DiTing tools to ACP tool kinds and builds editor-facing content.
...
new_session(cwd)
  -> create SessionState
  -> create AIAgent(platform="acp", enabled_toolsets=["diting-acp"])
...
Instead it reuses DiTing' runtime resolver:

- `acp_adapter/auth.py`
- `diting_cli/runtime_provider.py`

So ACP advertises and uses the currently configured DiTing provider/credentials. It also always advertises a terminal setup auth method (`diting-setup`, args `--setup`) so first-run registry clients can open DiTing' interactive model/provider configuration before starting a normal ACP session.
...
- ACP sessions are persisted to the shared `~/.diting/state.db` (SessionDB) and transparently restored across process restarts; they appear in `session_search`
...
- `toolsets.py` — `diting-acp` toolset definition
- `diting_cli/main.py` — `diting acp` CLI subcommand
- `pyproject.toml` — `[acp]` optional dependency + `diting-acp` script
