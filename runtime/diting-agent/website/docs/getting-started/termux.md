...
description: "Run DiTing Agent directly on an Android phone with Termux"
---

# DiTing on Android with Termux
...
DiTing Agent can run directly on an Android phone through [Termux](https://termux.dev/).
...
That does not stop DiTing from working well as a phone-native CLI agent — it just means the recommended mobile install is intentionally narrower than the desktop/server install.

---

## Option 1: One-line installer

DiTing now ships a Termux-aware installer path:
...
- links `diting` into `$PREFIX/bin` so it stays on your Termux PATH
...
### 5. Put `diting` on your Termux PATH

```bash
ln -sf "$PWD/venv/bin/diting" "$PREFIX/bin/diting"
```

`$PREFIX/bin` is already on PATH in Termux, so this makes the `diting` command persist across new shells without re-activating the venv every time.
...
Or set keys directly in `~/.diting/.env`.
...
### `diting doctor` says ripgrep or Node is missing
