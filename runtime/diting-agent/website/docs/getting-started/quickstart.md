...
sidebar_position: 1
title: "Quickstart"
description: "Your first conversation with DiTing Agent — from install to chatting in under 5 minutes"
---

# Quickstart

This guide gets you from zero to a working DiTing setup that survives real use. Install, choose a provider, verify a working chat, and know exactly what to do when something breaks.

## Prefer to watch?

**Onchain AI Garage** put together a Masterclass walkthrough of installation, setup, and basic commands — a good companion to this page if you'd rather follow along on video. For more, see the full [DiTing Agent Tutorials & Use Cases](https://www.youtube.com/playlist?list=PLmpUb_PWAkDxewld5ZYyKifuHxgIbiq2d) playlist.

<div style={{position: 'relative', paddingBottom: '56.25%', height: 0, overflow: 'hidden', maxWidth: '100%', marginBottom: '1.5rem'}}>
  <iframe
    style={{position: 'absolute', top: 0, left: 0, width: '100%', height: '100%'}}
    src="https://www.youtube-nocookie.com/embed/R3YOGfTBcQg"
    title="DiTing Agent Masterclass: Installation, Setup, Basic Commands"
    frameBorder="0"
...
- Setting up DiTing for a team, bot, or always-on workflow
...
|---|---|---|
| I just want DiTing working on my machine | `diting setup` | Run a real chat and verify it responds |
| I already know my provider | `diting model` | Save the config, then start chatting |
| I want a bot or always-on setup | `diting gateway setup` after CLI works | Connect Telegram, Discord, Slack, or another platform |
| I want a local or self-hosted model | `diting model` → custom endpoint | Verify the endpoint, model name, and context length |
| I want multi-provider fallback | `diting model` first | Add routing and fallback only after the base chat works |

**Rule of thumb:** if DiTing cannot complete a normal chat, do not add more features yet. Get one clean conversation working first, then layer on gateway, cron, skills, voice, or routing.

---

## 1. Install DiTing Agent
### With the DiTing Desktop installer on macOS or Windows (recommended)
To easily install the command-line and desktop applications, [download the DiTing Desktop installer](https://diting-agent.nousresearch.com/) from our website and run it.

### Without DiTing Desktop:
For a command-line only install without DiTing Desktop, run:
...
The single most important setup step. Use `diting model` to walk through the choice interactively:
...
:::info Setup modes
On a fresh install, `diting setup` offers three modes:
...
Blank Slate writes an explicit `platform_toolsets.cli` list plus `agent.disabled_toolsets`, so nothing you didn't choose ever loads — not even after `diting update`. Re-enable anything later with `diting tools`, seed skills with `diting skills opt-in --sync`, or tune settings with `diting setup agent`.
:::
...
|----------|-----------|---------------|
| **Nous Portal** | Subscription-based, zero-config | OAuth login via `diting model` |
| **OpenAI Codex** | ChatGPT OAuth, uses Codex models | Device code auth via `diting model` |
| **Anthropic** | Claude models directly — Max plan + extra usage credits (OAuth), or API key for pay-per-token | `diting model` → OAuth login (requires Max + extra credits), or an Anthropic API key |
...
| **MiniMax (OAuth)** | MiniMax frontier model via browser OAuth — no API key needed (model name in `diting_cli/models.py` may change between releases) | `diting model` → MiniMax (OAuth) |
...
| **xAI Grok OAuth** | SuperGrok / Premium+ subscription, no API key needed | `diting model` → xAI Grok OAuth |
...
| **Qwen OAuth** | Qwen Portal browser OAuth — no API key needed | `diting model` → Qwen OAuth |
...
| **GitHub Copilot** | GitHub Copilot subscription (GPT-5.x, Claude, Gemini, etc.) | OAuth via `diting model`, or `COPILOT_GITHUB_TOKEN` / `GH_TOKEN` |
| **GitHub Copilot ACP** | Copilot ACP agent backend (spawns local `copilot` CLI) | `diting model` (requires `copilot` CLI + `copilot login`) |
...
:::tip
You can switch providers at any time with `diting model` — no lock-in. For a full list of all supported providers and setup details, see [AI Providers](../integrations/providers.md).
:::

### How settings are stored

DiTing separates secrets from normal config:

- **Secrets and tokens** → `~/.diting/.env`
- **Non-secret settings** → `~/.diting/config.yaml`

The easiest way to set values correctly is through the CLI:

```bash
diting config set model anthropic/claude-opus-4.6
diting config set terminal.backend docker
diting config set OPENROUTER_API_KEY sk-or-...
```
...
```bash
diting            # classic CLI
diting --tui      # modern TUI (recommended)
```

You'll see a welcome banner with your model, available tools, and skills. Use a prompt that's specific and easy to verify:

:::tip Pick your interface
DiTing ships with two terminal interfaces: the classic `prompt_toolkit` CLI and a newer [TUI](../user-guide/tui.md) with modal overlays, mouse selection, and non-blocking input. Both share the same sessions, slash commands, and config — try each with `diting` vs `diting --tui`.
:::
...
```bash
diting --continue    # Resume the most recent session
diting -c            # Short form
```
...
```bash
diting gateway setup    # Interactive platform configuration
```
...
- `diting tools` — tune tool access per platform
- `diting skills` — browse and install reusable workflows
...
```bash
diting config set terminal.backend docker    # Docker isolation
diting config set terminal.backend ssh       # Remote server
```

### Voice mode

```bash
# From the DiTing install directory (the curl installer placed it at
# ~/.diting/diting-agent on Linux/macOS or %LOCALAPPDATA%\diting\diting-agent on Windows):
cd ~/.diting/diting-agent
...
Skills are on-demand instruction documents that teach DiTing how to do a specific task — deploy to Kubernetes, open a GitHub PR, fine-tune a model, search for GIFs. Each is a `SKILL.md` file with a name, a description, and a step-by-step procedure. The agent reads the short descriptions for free and only loads a skill's full content when a task actually calls for it, so adding skills doesn't bloat every request.

DiTing ships with a catalog of bundled skills already installed in `~/.diting/skills/`. You can add more from the Skills Hub, or write your own.

**Browse and install from the hub:**

```bash
diting skills browse                      # list everything available
diting skills search kubernetes           # find skills by keyword
diting skills install openai/skills/k8s   # install one (runs a security scan first)
```

The install argument is a `source/path` slug from the hub — `openai/skills/k8s` means the `k8s` skill from OpenAI's catalog. `diting skills browse` shows the exact slugs to use.
...
/k8s                                       # load it and let DiTing ask what you need
```
...
(If you installed without `[all]`, run `cd ~/.diting/diting-agent && uv pip install -e ".[acp]"` first.)
...
|---|---|---|
| DiTing opens but gives empty or broken replies | Provider auth or model selection is wrong | Run `diting model` again and confirm provider, model, and auth |
...
| Gateway starts but nobody can message it | Bot token, allowlist, or platform setup is incomplete | Re-run `diting gateway setup` and check `diting gateway status` |
| `diting --continue` can't find old session | Switched profiles or session never saved | Check `diting sessions list` and confirm you're in the right profile |
...
| `diting doctor` flags config problems | Config values are missing or stale | Fix the config, retest a plain chat before adding features |
...
2. `diting model`
3. `diting setup`
4. `diting sessions list`
5. `diting --continue`
6. `diting gateway status`
...
| `diting tools` | Configure which tools are enabled per platform |
| `diting setup` | Full setup wizard (configures everything at once) |
| `diting doctor` | Diagnose issues |
| `diting update` | Update to latest version |
| `diting gateway` | Start the messaging gateway |
| `diting --continue` | Resume last session |
