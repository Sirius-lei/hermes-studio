...
description: "Diagnose and fix common DiTing cron issues — jobs not firing, delivery failures, skill loading errors, and performance problems"
---
...
If you're expecting jobs to fire automatically, you need a running gateway (`diting gateway` for foreground, or `diting gateway start` for the installed service). For one-off debugging, you can manually trigger a tick with `diting cron tick`.
...
```bash
date
diting cron list   # Compare next_run times with local time
```
...
| Target | Requires |
|--------|----------|
| `telegram` | `TELEGRAM_BOT_TOKEN` in `~/.diting/.env` |
| `discord` | `DISCORD_BOT_TOKEN` in `~/.diting/.env` |
| `slack` | `SLACK_BOT_TOKEN` in `~/.diting/.env` |
...
| `local` | Write access to `~/.diting/cron/output/` |
...
If delivery fails, the job still runs — it just won't send anywhere. Check `diting cron list` for updated `last_error` field (if available).
...
Skills must be installed before they can be attached to cron jobs. If a skill is missing, install it first with `diting skills install <skill-name>` or via `/skills` in the CLI.

### Check 2: Check skill name vs. skill folder name

Skill names are case-sensitive and must match the installed skill's folder name. If your job specifies `ai-funding-daily-report` but the skill folder is `ai-funding-daily-report`, confirm the exact name from `diting skills list`.
...
2. `~/.diting/logs/agent.log` for scheduler messages (or `errors.log` for warnings)
3. The job's `last_run` metadata via `diting cron list`

### Check 2: Common error patterns

**"No such file or directory" for scripts**
The `script` path must be an absolute path (or relative to the DiTing config directory). Verify:
```bash
ls ~/.diting/scripts/your-script.py   # Must exist
diting cron edit <job_id> --script ~/.diting/scripts/your-script.py
```

**"Skill not found" at job execution**
The skill must be installed on the machine running the scheduler. If you move between machines, skills don't automatically sync — reinstall them with `diting skills install <skill-name>`.
...
Jobs are stored in `~/.diting/cron/jobs.json`. If this file is not readable/writable by your user, the scheduler will fail silently:

```bash
ls -la ~/.diting/cron/jobs.json
chmod 600 ~/.diting/cron/jobs.json   # Your user should own it
```
...
```bash
diting cron list                    # Show all jobs, states, next_run times
diting cron run <job_id>            # Schedule for next tick (for testing)
diting cron edit <job_id>           # Fix configuration issues
diting logs                         # View recent DiTing logs
diting skills list                  # Verify installed skills
```
...
1. Run the job with `diting cron run <job_id>` (fires on next gateway tick) and watch for errors in the chat output
2. Check `~/.diting/logs/agent.log` for scheduler messages and `~/.diting/logs/errors.log` for warnings
3. Open an issue at [github.com/NousResearch/diting-agent](https://github.com/NousResearch/diting-agent) with:
