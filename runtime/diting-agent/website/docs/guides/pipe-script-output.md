...
description: "Send text from any shell script, cron job, CI hook, or monitoring daemon to Telegram, Discord, Slack, Signal, and other platforms using `diting send`."
---

# Pipe Script Output to Messaging Platforms

`diting send` is a small, scriptable CLI that pushes a message to any
messaging platform DiTing is already configured for. Think of it as a
...
- Piping any tool's output anywhere (`make | diting send --to slack:#builds`)

The command reuses the same credentials and platform adapters that `diting
...
diting send --to slack:#eng --subject "[CI] build.log" --file build.log

# Thread target (Telegram topic, Discord thread)
diting send --to telegram:-1001234567890:17585 "threaded reply"

# List every configured target
diting send --list

# Filter by platform
diting send --list telegram
```
...
Any platform DiTing ships adapters for works as a target:
...
`diting send` resolves the message body in this order:

1. **Positional argument** — `diting send --to telegram "hi"`
2. **`--file PATH`** — `diting send --to telegram --file msg.txt`
3. **Piped stdin** — `echo hi | diting send --to telegram`

When stdin is a TTY (no pipe), DiTing does **not** wait for input — you'll
...
if [ "$ram_pct" -ge 85 ]; then
  diting send --to telegram --subject "⚠ MEMORY WARNING" \
    "RAM ${ram_pct}% on $(hostname)"
fi
```

Because `diting send` reuses your DiTing config, the same script works on
any host where DiTing is installed — no need to export bot tokens into
...
`diting send`. If the Python interpreter can't load because the box is
...
if ./scripts/deploy.sh; then
  diting send --to slack:#deploys "✅ ${CI_COMMIT_SHA:0:7} deployed"
else
  tail -n 100 deploy.log | diting send \
...
0 9 * * * /usr/local/bin/generate-metrics.sh \
  | /home/me/.diting/bin/diting send \
...
```bash
./train.py --epochs 200 && \
  diting send --to telegram "training done" || \
  diting send --to telegram "training failed (exit $?)"
```
...
diting send --to telegram --quiet "keepalive" || {
  echo "Telegram delivery failed" >&2
  exit 1
}

# Capture the message ID for later editing / threading
msg_id=$(diting send --to discord:#ops --json "build started" \
  | jq -r .message_id)
```

---

## Does `diting send` Need the Gateway Running?

**Usually no.** For any bot-token platform — Telegram, Discord, Slack,
Signal, SMS, WhatsApp Cloud API, and most others — `diting send` calls
the platform's REST endpoint directly using credentials from
`~/.diting/.env` and `~/.diting/config.yaml`. It's a standalone subprocess
...
pointing at the gateway; start it with `diting gateway start` and retry.
...
# Machine-readable
diting send --list --json
```

The listing is built from `~/.diting/channel_directory.json`, which the
gateway refreshes every few minutes while it's running. If you see
"no channels discovered yet", start the gateway once (`diting gateway
...
| Approach | Multi-platform | Reuses DiTing creds | Needs gateway | Best for |
|----------|----------------|---------------------|---------------|----------|
| `diting send` | ✅ | ✅ | No (bot-token) | Everything below |
...
`diting send` is intentionally the simplest possible surface. If you need
...
If you just need to pipe a raw string, reach for `diting send`.
...
- [Gateway Internals](/developer-guide/gateway-internals) —
  the delivery router that `diting send` shares with cron delivery.
