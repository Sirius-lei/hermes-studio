...
```bash
diting teams-pipeline validate
```
...
```bash
diting teams-pipeline token-health
diting teams-pipeline token-health --force-refresh
```
...
```bash
diting teams-pipeline subscriptions
```

### Renew near-expiry subscriptions

```bash
diting teams-pipeline maintain-subscriptions
diting teams-pipeline maintain-subscriptions --dry-run
```
...
#### Option 1: DiTing cron (recommended if you already run the DiTing gateway)

DiTing ships a built-in cron scheduler. The `--no-agent` mode runs a script as the job (rather than using an LLM), and `--script` must point at a file under `~/.diting/scripts/`. First create the script:

```bash
mkdir -p ~/.diting/scripts
cat > ~/.diting/scripts/maintain-teams-subscriptions.sh <<'EOF'
#!/usr/bin/env bash
exec diting teams-pipeline maintain-subscriptions
EOF
chmod +x ~/.diting/scripts/maintain-teams-subscriptions.sh
```

Then register a script-only cron job that runs every 12 hours (gives 6x headroom against the 72h expiry window):

```bash
diting cron create "0 */12 * * *" \
...
Create `/etc/systemd/system/diting-teams-pipeline-maintain.service`:

```ini
[Unit]
Description=DiTing Teams pipeline subscription maintenance
After=network-online.target

[Service]
Type=oneshot
User=diting
EnvironmentFile=/etc/diting/env
ExecStart=/usr/local/bin/diting teams-pipeline maintain-subscriptions
```

And `/etc/systemd/system/diting-teams-pipeline-maintain.timer`:

```ini
[Unit]
Description=Run DiTing Teams pipeline subscription maintenance every 12 hours
...
sudo systemctl enable --now diting-teams-pipeline-maintain.timer
systemctl list-timers diting-teams-pipeline-maintain.timer
```

#### Option 3: Plain crontab

```cron
0 */12 * * * /usr/local/bin/diting teams-pipeline maintain-subscriptions >> /var/log/diting/teams-pipeline-maintain.log 2>&1
```

Make sure the cron environment has the `MSGRAPH_*` credentials. Simplest fix: source `~/.diting/.env` at the top of a wrapper script that crontab calls.
...
```bash
diting teams-pipeline subscriptions   # should show expirationDateTime advanced
diting teams-pipeline maintain-subscriptions --dry-run   # should show "0 expiring soon" most of the time
```
...
```bash
diting teams-pipeline list
diting teams-pipeline list --status failed
diting teams-pipeline show <job-id>
```

### Replay a stored job

```bash
diting teams-pipeline run <job-id>
```

### Dry-run meeting artifact fetches

```bash
diting teams-pipeline fetch --meeting-id <meeting-id>
diting teams-pipeline fetch --join-web-url "<join-url>"
```
...
- run `diting teams-pipeline maintain-subscriptions --dry-run`
- inspect `diting teams-pipeline list --status failed`
...
- run `diting teams-pipeline validate`
...
Check:
- whether you manually replayed a job with `diting teams-pipeline run`
...
- [ ] `diting teams-pipeline validate` returns an OK snapshot
- [ ] `diting teams-pipeline token-health --force-refresh` succeeds
- [ ] **`maintain-subscriptions` is scheduled** (DiTing cron, systemd timer, or crontab — see [Automating subscription renewal](#automating-subscription-renewal-required-for-production)). Without this, Graph subscriptions silently expire within 72 hours.
