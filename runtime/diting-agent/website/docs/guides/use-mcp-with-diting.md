---
sidebar_position: 6
title: "Use MCP with DiTing"
description: "A practical guide to connecting MCP servers to DiTing Agent, filtering their tools, and using them safely in real workflows"
---

# Use MCP with DiTing

This guide shows how to actually use MCP with DiTing Agent in day-to-day workflows.
...
Use MCP when:
- a tool already exists in MCP form and you do not want to build a native DiTing tool
- you want DiTing to operate against a local or remote system through a clean RPC layer
- you want fine-grained per-server exposure control
- you want to connect DiTing to internal APIs, databases, or company systems without modifying DiTing core

Do not use MCP when:
- a built-in DiTing tool already solves the job well
...
- DiTing remains the agent
- MCP servers contribute tools
- DiTing discovers those tools at startup or reload time
...
If you installed DiTing with the standard install script, MCP support is already included (the installer runs `uv pip install -e ".[all]"`).
...
- DiTing banner/status should show MCP integration when configured
- ask DiTing what tools it has available
...
## WSL2: bridge DiTing in WSL to Windows Chrome

This is the practical setup when:

- DiTing runs inside WSL2
...
In this setup, DiTing does **not** connect to Chrome directly. Instead:

- DiTing runs in WSL
- DiTing starts a local stdio MCP server
...
```text
DiTing (WSL) -> MCP stdio bridge -> Windows Chrome
```

### Why this mode is useful

- you keep your real Windows browser profile, cookies, and logins
- DiTing stays in its supported Unix environment (WSL2)
- browser control is exposed as MCP tools instead of relying on DiTing core browser transport
...
```bash
diting mcp add chrome-devtools-win --command cmd.exe --args /c npx -y chrome-devtools-mcp@latest --autoConnect --no-usage-statistics
```

After saving the server:

```bash
diting mcp test chrome-devtools-win
```

Then start a fresh DiTing session or run:
...
Once loaded, DiTing can use the MCP-prefixed browser tools directly. For example:
...
If DiTing runs in WSL and Chrome runs on Windows, `/browser connect` may fail even though Chrome is open and debuggable.
...
- Start DiTing from a Windows-mounted path like `/mnt/c/Users/<you>` or `/mnt/c/workspace/...` when using Windows stdio executables through MCP.
- If you start DiTing from `/root` or `/home/...`, Windows may emit a `UNC` current-directory warning before the MCP server starts.
...
There are two categories of MCP-exposed functionality in DiTing:
...
2. DiTing-added utility wrappers
- filtered with:
  - `tools.resources`
  - `tools.prompts`
...
So DiTing will not pretend a server has resources/prompts if it does not.

## Common patterns

### Pattern 1: local project assistant

Use MCP for a repo-local filesystem or git server when you want DiTing to reason over a bounded workspace.
...
Use [Open Scaffold](https://github.com/graphanov/open-scaffold) when you want DiTing to read a repository's durable AI-work record: mission, plans, evidence notes, handoff packets, and review/gate results. DiTing remains the agent; Open Scaffold remains the repo-local record.

Add the server for one scaffolded repository:

```bash
diting mcp add open_scaffold --command npx --args -y open-scaffold@latest mcp serve --repo /absolute/path/to/repo
diting mcp test open_scaffold
```

Then keep the exposed surface read-oriented. Choose `select` in the `diting mcp add` prompt, or edit `config.yaml` afterward:
...
- Its write tools require the server to be started with `--allow-write`; do not enable that until you explicitly want DiTing to mutate `.osc` files.
- Open Scaffold records and gates work; it does not authorize DiTing to merge, publish, deploy, or spawn runtimes.
...
Now DiTing can combine them:

```text
Inspect the local project files, then create a GitHub issue summarizing the bug you find.
```

That is where MCP gets powerful: multi-system workflows without changing DiTing core.
...
Because DiTing now respects your per-server policy and capability-aware registration. That is expected, and usually desirable.
