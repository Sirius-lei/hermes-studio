---
sidebar_position: 8
title: "Use Voice Mode with DiTing"
description: "A practical guide to setting up and using DiTing voice mode across CLI, Telegram, Discord, and Discord voice channels"
---

# Use Voice Mode with DiTing
...
- you want DiTing sitting in a Discord voice channel for live conversation
...
There are really three different voice experiences in DiTing.
...
## Step 1: make sure normal DiTing works first
...
DiTing supports both local and cloud speech stacks.
...
### If you use `diting setup`

If you choose NeuTTS in the setup wizard, DiTing checks whether `neutts` is already installed. If it is missing, the wizard tells you NeuTTS needs the Python package `neutts` and the system package `espeak-ng`, offers to install them for you, installs `espeak-ng` with your platform package manager, and then runs:
...
4. DiTing transcribes and responds
...
- asking DiTing to structure your thoughts in real time

#### Accessibility / low-typing sessions

If typing is inconvenient, voice mode is one of the fastest ways to stay in the full DiTing loop.

## Tuning CLI behavior

### Silence threshold

If DiTing starts/stops too aggressively, tune:
...
DiTing stays a normal chat bot, but can speak replies.
...
- you want DiTing to function like a portable research or ops assistant
...
DiTing joins a Discord VC, listens to user speech, transcribes it, runs the normal agent pipeline, and speaks replies back into the channel.
...
- DiTing detects speech boundaries
- transcripts are posted in the associated text channel
- DiTing responds in text and audio
...
1. get text DiTing working
2. install `diting-agent[voice]`
