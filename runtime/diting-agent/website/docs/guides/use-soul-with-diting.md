---
sidebar_position: 7
title: "Use SOUL.md with DiTing"
description: "How to use SOUL.md to shape DiTing Agent's default voice, what belongs there, and how it differs from AGENTS.md and /personality"
---

# Use SOUL.md with DiTing

`SOUL.md` is the **primary identity** for your DiTing instance. It's the first thing in the system prompt — it defines who the agent is, how it speaks, and what it avoids.

If you want DiTing to feel like the same assistant every time you talk to it — or if you want to replace the DiTing persona entirely with your own — this is the file to use.
...
- personality
- communication style
- how direct or warm DiTing should be
- what DiTing should avoid stylistically
- how DiTing should relate to uncertainty, disagreement, and ambiguity

In short:
- `SOUL.md` is about who DiTing is and how DiTing speaks
...
DiTing now uses only the global SOUL file for the current instance:

```text
~/.diting/SOUL.md
```

If you run DiTing with a custom home directory, it becomes:
...
DiTing automatically seeds a starter `SOUL.md` for you if one does not already exist.

That means most users now begin with a real file they can read and edit immediately.

Important:
- if you already have a `SOUL.md`, DiTing does not overwrite it
- if the file exists but is empty, DiTing adds nothing from it to the prompt

## How DiTing uses it

When DiTing starts a session, it reads `SOUL.md` from `DiTing_HOME`, scans it for prompt-injection patterns, truncates it if needed, and uses it as the **agent identity** — slot #1 in the system prompt. This means SOUL.md completely replaces the built-in default identity text.
...
That alone can noticeably change how DiTing feels.
...
DiTing already tries to be helpful and clear. `SOUL.md` should add real personality and style, not restate obvious defaults.
...
# Style
How DiTing should sound.

# Avoid
What DiTing should not do.

# Defaults
How DiTing should behave when ambiguity appears.
```
...
Then restart DiTing or start a new session.
...
### I edited SOUL.md but DiTing still sounds the same

Check:
- you edited `~/.diting/SOUL.md` or `$DiTing_HOME/SOUL.md`
...
### DiTing is ignoring parts of my SOUL.md
