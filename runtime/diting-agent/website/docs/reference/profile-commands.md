...
This page covers all commands related to [DiTing profiles](../user-guide/profiles.md). For general CLI commands, see [CLI Commands Reference](./cli-commands.md).
...
Top-level command for managing profiles. Running `diting profile` without a subcommand shows help.
...
```bash
diting profile use <name>
```

Sets `<name>` as the active profile. All subsequent `diting` commands (without `-p`) will use this profile.
...
diting profile use default
```

## `diting profile create`

```bash
diting profile create <name> [options]
```
...
| `--description "<text>"` | One- or two-sentence description of what this profile is good at. Used by the kanban orchestrator to route tasks based on role instead of profile name alone. Skip and add later via `diting profile describe`. Persisted in `<profile_dir>/profile.yaml`. |
| `--no-skills` | Create an **empty** profile with zero bundled skills enabled. Writes a `.no-bundled-skills` marker into the profile so future `diting update` runs won't re-seed the bundled set, and refuses to combine with `--clone`, `--clone-from`, or `--clone-all` (which would copy skills in anyway). Useful for narrow orchestrator profiles or sandbox profiles that should not inherit the full skill catalog. To toggle this on an already-created profile (including the default `~/.diting`), use `diting skills opt-out` / `diting skills opt-in`. |
...
diting profile create work2 --clone-from work

# Clone everything from a specific profile
diting profile create work2-backup --clone-from work --clone-all
```

## `diting profile describe`

```bash
diting profile describe [<name>] [options]
```
...
diting profile describe researcher

# Set it explicitly
diting profile describe researcher --text "Reads source code and writes findings."

# Let the LLM generate one
diting profile describe researcher --auto

# Fill in descriptions for every profile that doesn't have one
diting profile describe --all --auto
```

## `diting profile delete`

```bash
diting profile delete <name> [options]
```
...
```bash
diting profile delete mybot
diting profile delete mybot --yes
```
...
```bash
diting profile show <name>
```

Displays details about a profile including its home directory, configured model, gateway status, skills count, and configuration file status.

This shows the profile's DiTing home directory, not the terminal working directory. Terminal commands start from `terminal.cwd` (or the launch directory on the local backend when `cwd: "."`).
...
```bash
$ diting profile show work
Profile: work
Path:    ~/.diting/profiles/work
...
## `diting profile alias`

```bash
diting profile alias <name> [options]
```

Regenerates the shell alias script at `~/.local/bin/<name>`. Useful if the alias was accidentally deleted or if you need to update it after moving your DiTing installation.
...
```bash
diting profile alias work
# Creates/updates ~/.local/bin/work

diting profile alias work --name mywork
# Creates ~/.local/bin/mywork

diting profile alias work --remove
# Removes the wrapper script
```

## `diting profile rename`

```bash
diting profile rename <old-name> <new-name>
```
...
```bash
diting profile rename mybot assistant
# ~/.diting/profiles/mybot → ~/.diting/profiles/assistant
# ~/.local/bin/mybot → ~/.local/bin/assistant
```

## `diting profile export`

```bash
diting profile export <name> [options]
```
...
```bash
diting profile export work
# Creates work.tar.gz in the current directory

diting profile export work -o ./work-2026-03-29.tar.gz
```

## `diting profile import`

```bash
diting profile import <archive> [options]
```
...
```bash
diting profile import ./work-2026-03-29.tar.gz
# Infers profile name from the archive

diting profile import ./work-2026-03-29.tar.gz --name work-restored
```
...
:::info
`diting profile export` / `import` are still the right commands for
...
### `diting profile install`

```bash
diting profile install <source> [--name <name>] [--alias] [--force] [--yes]
```
...
| `--alias` | Also create a shell wrapper (e.g. `telemetry` → `diting -p telemetry`). |
...
diting profile install github.com/kyle/telemetry-distribution --alias

# Install from a full HTTPS git URL
diting profile install https://github.com/kyle/telemetry-distribution.git

# Install from SSH
diting profile install git@github.com:kyle/telemetry-distribution.git

# Install from a local directory during development
diting profile install ./telemetry/
```

### `diting profile update`

```bash
diting profile update <name> [--force-config] [--yes]
```
...
```bash
diting profile info <name>
```

Prints the profile's distribution manifest — name, version, required
DiTing version, author, env var requirements, the source URL/path, and
...
`diting profile list` also shows the distribution name and version in a
`Distribution` column, and `diting profile show <name>` / `delete <name>`
...
diting profile install git@github.com:your-org/internal-assistant.git

# Uses your git credential helper
diting profile install https://github.com/your-org/internal-assistant.git
```
...
`diting_requires` supports `>=`, `<=`, `==`, `!=`, `>`, `<`, or a bare
version (treated as `>=`). Install fails with a clear error if the current
DiTing version doesn't satisfy the spec.
...
2. Initialize a git repo (or use an existing one) and push to GitHub /
   GitLab / any host DiTing can clone from.
3. Tell recipients to run `diting profile install <your-repo-url>`.
...
## `diting -p` / `diting --profile`

```bash
diting -p <name> <command> [options]
diting --profile <name> <command> [options]
```

Global flag to run any DiTing command under a specific profile without changing the sticky default. This overrides the active profile for the duration of the command.
...
```bash
diting -p work chat -q "Check the server status"
diting --profile dev gateway start
diting -p personal skills list
diting -p work config edit
```

## `diting completion`

```bash
diting completion <shell>
```
...
- `diting profile <TAB>` — subcommands (list, use, create, etc.)
- `diting profile use <TAB>` — profile names
- `diting -p <TAB>` — profile names
