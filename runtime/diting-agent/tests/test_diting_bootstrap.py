"""Tests for diting_bootstrap — Windows UTF-8 stdio shim.

The bootstrap module is imported at the top of every DiTing entry point
(diting, diting-agent, diting-acp, gateway, batch_runner, cli.py).  It
...
Key invariants covered by these tests:
  ...
  5. Load order: every DiTing entry point imports diting_bootstrap as its
...
def _fresh_import():
    """Return a freshly-imported diting_bootstrap module.

    Drops any cached copy from sys.modules first so module-level code
    runs again and the platform check re-evaluates.
    """
    sys.modules.pop("diting_bootstrap", None)
    import diting_bootstrap  # noqa: WPS433
    return diting_bootstrap
...
class TestWindowsBehavior:
    ...
    def test_stdout_reconfigured_to_utf8_on_windows(self):
        # The live process's stdout should now be UTF-8 (the DiTing CLI
...
class TestEntryPointsImportBootstrap:
    """Every DiTing entry point must import diting_bootstrap as its
    ...
    # Entry points that invoke DiTing as a process.  Each one must
    # import diting_bootstrap before doing any file I/O or stdout writes.
    ENTRY_POINTS = [
        "diting_cli/main.py",   # diting CLI (console_script)
        "run_agent.py",          # diting-agent (console_script)
        "acp_adapter/entry.py",  # diting-acp (console_script)
    ...
    def test_entry_point_imports_bootstrap(self, path):
        """The file must contain 'import diting_bootstrap' and that
        ...
        half-finished ``diting update`` (git-reset landed new code but
        ``uv pip install -e .`` didn't finish re-registering
        ``diting_bootstrap`` as a top-level module) leaves diting
        ...
        """
        # Resolve relative to the diting-agent repo root.  Tests live
        # at tests/test_diting_bootstrap.py, so go up one dir.
        import pathlib
        ...
        for node in ast.iter_child_nodes(tree):
            ...
            # diting start even when diting_bootstrap hasn't been
        ...
        assert first_import_name == "diting_bootstrap", (
            f"{path}: first top-level import is {first_import_name!r}, "
            f"but it must be 'diting_bootstrap' so UTF-8 stdio is "
            f"configured before anything else initializes.  Move the "
            f"'import diting_bootstrap' line to be the first import."
        )


class TestHardenImportPath:
    """harden_import_path() must keep a same-named package in the launch
    directory from shadowing DiTing's own top-level modules — covering both
    the relative ('' / '.') and absolute-path forms the cwd can take on
    sys.path (issue #51286)."""

    def _run(self, hb, path_seed, env=None):
        original = sys.path[:]
        original_env = os.environ.get("DiTing_PYTHON_SRC_ROOT")
        try:
            sys.path[:] = path_seed
            if env is not None:
                os.environ["DiTing_PYTHON_SRC_ROOT"] = env
            elif "DiTing_PYTHON_SRC_ROOT" in os.environ:
                del os.environ["DiTing_PYTHON_SRC_ROOT"]
            hb.harden_import_path(src_root="/opt/diting")
            return sys.path[:]
        finally:
            sys.path[:] = original
            if original_env is None:
                os.environ.pop("DiTing_PYTHON_SRC_ROOT", None)
            else:
                os.environ["DiTing_PYTHON_SRC_ROOT"] = original_env

    def test_relative_cwd_forms_removed(self):
        hb = _fresh_import()
        result = self._run(hb, ["", ".", "/opt/diting", "/usr/lib/python"])
    ...
    def test_absolute_cwd_path_loses_to_src_root(self):
        ...
        # DiTing root.  The guard must relocate DiTing to the front.
        hb = _fresh_import()
        result = self._run(hb, ["/home/user/tg-ws-proxy", "/opt/diting"])
        assert result[0] == "/opt/diting"
        # The cwd absolute path may still appear (it can hold legit deps),
        # but only AFTER the DiTing root.
        assert result.index("/opt/diting") < result.index("/home/user/tg-ws-proxy")

    def test_src_root_not_duplicated(self):
        hb = _fresh_import()
        result = self._run(hb, ["/opt/diting", "/opt/diting", ""])
        assert result.count("/opt/diting") == 1
    ...
    def test_env_var_used_when_no_arg(self):
        ...
        try:
            ...
            os.environ["DiTing_PYTHON_SRC_ROOT"] = "/env/diting"
            hb.harden_import_path()
            assert sys.path[0] == "/env/diting"
        finally:
            sys.path[:] = original
            if original_env is None:
                os.environ.pop("DiTing_PYTHON_SRC_ROOT", None)
