...
def test_root_session_no_compression(db):
    ...
    assert prov["currentDiTingSessionId"] == "root1"
    assert prov["rootDiTingSessionId"] == "root1"
    assert prov["parentDiTingSessionId"] is None
...
def test_compression_split_continuation(db):
    ...
    prov = build_session_provenance(
        db, "acp-1", "new", previous_diting_session_id="old"
    )
    assert prov["sessionKind"] == "continuation"
    assert prov["parentDiTingSessionId"] == "old"
    assert prov["rootDiTingSessionId"] == "old"
    assert prov["compressionDepth"] == 1
    assert prov["previousDiTingSessionId"] == "old"
...
def test_multi_depth_chain(db):
    ...
    assert prov["rootDiTingSessionId"] == "s0"
...
def test_non_compression_parent_is_root_not_continuation(db):
    ...
    assert prov["sessionKind"] == "root"
    assert prov["compressionDepth"] == 0
    assert prov["rootDiTingSessionId"] == "p"  # lineage root still walked
...
def test_no_false_rotation_when_head_unchanged(db):
    ...
    prov = build_session_provenance(
        db, "acp-1", "s", previous_diting_session_id="s"
    )
    assert "reason" not in prov
    assert "creatorKind" not in prov
    assert prov["previousDiTingSessionId"] == "s"
...
def test_meta_wrapper_shape(db):
    ...
    assert set(meta.keys()) == {"diting"}
    assert "sessionProvenance" in meta["diting"]
    assert meta["diting"]["sessionProvenance"]["currentDiTingSessionId"] == "root1"
