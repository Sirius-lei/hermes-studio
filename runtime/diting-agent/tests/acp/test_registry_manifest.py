"""Tests for ACP Registry metadata shipped with DiTing."""
...
def test_agent_json_matches_official_registry_required_fields():
    ...
    assert data["id"] == "diting-agent"
    assert re.fullmatch(r"[a-z][a-z0-9-]*", data["id"])
    assert data["name"] == "DiTing Agent"
    assert data["description"]
    assert data["repository"] == "https://github.com/NousResearch/diting-agent"
    assert data["website"].startswith("https://diting-agent.nousresearch.com/")
...
def test_agent_json_uses_uvx_distribution_without_local_command_fields():
    ...
    assert uvx["package"] == f"diting-agent[acp]=={data['version']}"
    assert uvx["args"] == ["diting-acp"]
...
def test_agent_json_pins_uvx_package_to_pyproject_version():
    ...
    assert _manifest()["distribution"]["uvx"]["package"] == (
        f"diting-agent[acp]=={_pyproject_version()}"
    )
