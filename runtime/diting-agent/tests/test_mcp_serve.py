"""
Tests for mcp_serve — DiTing MCP server.
...
def _isolate_diting_home(tmp_path, monkeypatch):
    """Redirect DiTing_HOME to a temp directory."""
    monkeypatch.setenv("DiTing_HOME", str(tmp_path))
    try:
        import diting_constants
        monkeypatch.setattr(diting_constants, "get_diting_home", lambda: tmp_path)
...
def _create_test_db(db_path, session_id, messages):
    """Create a minimal SQLite DB mimicking diting_state schema."""
...
class TestEventBridgePollE2E:
    ...
    def test_poll_picks_up_new_conversation_on_db_change(
        ...
        # _poll_once reads <DiTing_HOME>/state.db for its mtime gate; the autouse
        # fixture points DiTing_HOME at tmp_path.
        db_path = tmp_path / "state.db"
