...
class TestEnvFileReadBlocking:
    ...
    def test_allowed_diting_env(self):
        """DiTing' own .env inside DiTing_HOME is NOT blocked by this rule
        (it's handled by other mechanisms). Only project-local .env is blocked."""
        # Note: diting internal .env is in ~/.diting/.env which is NOT a project-local
        # path, but the basename check applies to ANY .env. This is intentional —
        # even ~/.diting/.env should not be readable via read_file.
        error = get_read_block_error(os.path.expanduser("~/.diting/.env"))
        assert error is not None
...
class TestCacheFileReadBlocking:
    """Internal DiTing cache files must remain blocked."""

    def test_hub_index_cache_blocked(self, tmp_path):
        """Hub index-cache reads are blocked."""
        diting_home = tmp_path / ".diting"
        cache = diting_home / "skills" / ".hub" / "index-cache" / "data.json"
        cache.parent.mkdir(parents=True)
        cache.write_text("{}")

        with patch("agent.file_safety._diting_home_path", return_value=diting_home):
            error = get_read_block_error(str(cache))
            assert error is not None
            assert "internal DiTing cache" in error

    def test_hub_directory_blocked(self, tmp_path):
        """Hub directory reads are blocked."""
        diting_home = tmp_path / ".diting"
        hub = diting_home / "skills" / ".hub" / "metadata.json"
...
class TestCombinedGuards:
    ...
    def test_env_guard_works_regardless_of_diting_home(self, tmp_path):
        """The env basename guard does not depend on DiTing_HOME resolution."""
        diting_home = tmp_path / ".diting"
        diting_home.mkdir()
    ...
    def test_cache_guard_still_works_with_env_guard(self, tmp_path):
        ...
        diting_home = tmp_path / ".diting"
        cache = diting_home / "skills" / ".hub" / "index-cache" / "x"
        cache.parent.mkdir(parents=True)
        cache.write_text("")
