...
""",
        ...
        from diting_cli import config as _config_mod
    ...
    def test_missing_plugin_entry_yields_default_deny(self, tmp_path, monkeypatch):
        ...
        diting_home = tmp_path / ".diting"
        diting_home.mkdir()
        (diting_home / "config.yaml").write_text("plugins: {}\n", encoding="utf-8")
        monkeypatch.setenv("DiTing_HOME", str(diting_home))
        from diting_cli import config as _config_mod
        _config_mod._config_cache = None  # type: ignore[attr-defined]
...
class TestPluginContextIntegration:
    def test_ctx_llm_is_lazy_singleton(self):
        from diting_cli.plugins import PluginContext, PluginManifest, PluginManager
