...
def isolated_home(monkeypatch):
    """Temp DiTing_HOME with config + clean credential env vars."""
    test_home = tempfile.mkdtemp(prefix="diting_test_31179_")
    diting_home = os.path.join(test_home, ".diting")
    os.makedirs(diting_home)
...
def _fresh_modules():
    """Drop cached diting modules so each test reloads against current env."""
