...
path via ``diting chat --query``) couldn't see what the aggregator actually
produced without joining to the session DB by hand.

These exercise the real ``consume_and_save_trace`` → ``save_moa_turn`` path
with real file I/O against a temp DiTing_HOME — no mocks on the write path.
"""
...
def _enable_traces(tmp_path, monkeypatch):
    """Point DiTing_HOME at a temp dir and turn moa.save_traces on."""
    diting_home = tmp_path / ".diting"
    diting_home.mkdir()
    monkeypatch.setenv("DiTing_HOME", str(diting_home))

    # save_moa_turn reads config via diting_cli.config.load_config; stub it to
