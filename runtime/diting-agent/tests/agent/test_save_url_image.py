...
def http_server(tmp_path, monkeypatch):
    """Spin up a localhost HTTP server and isolate DiTing_HOME under tmp_path."""
    monkeypatch.setenv("DiTing_HOME", str(tmp_path / ".diting"))
    (tmp_path / ".diting").mkdir()

    # Force the constants/image cache helpers to re-read DiTing_HOME.
    import sys
    for mod in list(sys.modules):
        if mod.startswith("diting_constants") or mod.startswith("agent.image_gen_provider"):
...
class TestSaveUrlImage:
    def test_writes_real_bytes_to_diting_home_cache(self, http_server):
        base, _ = http_server
        ...
        # The cache directory must be under DiTing_HOME — gateway cleanup
