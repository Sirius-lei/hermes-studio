...
engine plugins (e.g. diting-lcm) rely on:
...
def test_discover_context_engines_includes_plugin_registered_engines(monkeypatch):
    """Plugin-registered context engines appear in the ``diting plugins`` picker."""
    from diting_cli import plugins_cmd
    ...
    monkeypatch.setattr(
        "diting_cli.plugins.discover_plugins",
        lambda *_a, **_kw: None,
    )
    monkeypatch.setattr(
        "diting_cli.plugins.get_plugin_context_engine",
