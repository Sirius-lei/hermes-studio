...
def test_translate_native_response_surfaces_reasoning_and_tool_calls():
        ...
        "candidates": [
            {
                "content": {
                    "parts": [
                        ...
                        {"functionCall": {"name": "search", "args": {"q": "diting"}}},
                    ]
                },
                "finishReason": "STOP",
            }
    ...
    assert json.loads(choice.message.tool_calls[0].function.arguments) == {"q": "diting"}
...
def test_max_tokens_none_defaults_to_gemini_output_ceiling():
    ...
    maxOutputTokens is absent, truncating tool calls mid-stream. DiTing passes
...
def test_x_goog_api_client_header_is_set():
    ...
    assert "diting-agent/" in headers["X-Goog-Api-Client"], (
        "diting-agent not found in X-Goog-Api-Client header"
    )


def test_x_goog_api_client_header_format():
    """Header value should be 'diting-agent/<version>' matching the package version."""
    from agent.gemini_native_adapter import GeminiNativeClient, _DiTing_VERSION

    client = GeminiNativeClient(api_key="fake-key", model="gemini-2.0-flash")
    headers = client._headers()

    expected = f"diting-agent/{_DiTing_VERSION}"
    assert headers["X-Goog-Api-Client"] == expected


def test_user_agent_contains_version():
    """User-Agent should include the diting-agent version."""
    from agent.gemini_native_adapter import GeminiNativeClient, _DiTing_VERSION

    client = GeminiNativeClient(api_key="fake-key", model="gemini-2.0-flash")
    headers = client._headers()

    assert f"diting-agent/{_DiTing_VERSION}" in headers["User-Agent"]


def test_diting_version_is_valid():
    """_DiTing_VERSION should be a non-empty string."""
    from agent.gemini_native_adapter import _DiTing_VERSION

    assert isinstance(_DiTing_VERSION, str)
    assert len(_DiTing_VERSION) > 0
    assert _DiTing_VERSION != "0.0.0", (
        "Version should resolve from diting_cli.__version__, not the fallback"
    )
