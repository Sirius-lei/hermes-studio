...
@pytest.fixture()
def agent_env():
    """Spin up the mock provider + an isolated DiTing_HOME, yield (agent, helpers)."""
    ...
    test_home = tempfile.mkdtemp(prefix="diting_e2e_47967_")
    os.makedirs(os.path.join(test_home, ".diting"))
    prev_home = os.environ.get("DiTing_HOME")
    os.environ["DiTing_HOME"] = os.path.join(test_home, ".diting")
    ...
    for mod in list(sys.modules):
        if mod == "run_agent" or mod.startswith("agent.") or mod.startswith("tools.") or mod.startswith("diting_"):
            del sys.modules[mod]
    ...
    try:
        ...
        if prev_home is None:
            ...
            os.environ["DiTing_HOME"] = prev_home
