...
Damien's incident (Discord, 2026-05-28): a long DiTing session in a Discord
...
def _make_legacy_session_db_class() -> type:
    ...
    imports the same still-loaded ``diting_state`` module, whose ``SessionDB``
    ...
    namespace = {"__name__": "diting_state"}
    source = '''
class SessionDB:
...
_NominalSessionDBImpostor.__module__ = "diting_state"
...
def test_missing_lock_subsystem_fails_open_not_infinite_loop(tmp_path: Path, monkeypatch) -> None:
    ...
    ``diting_state.SessionDB`` class remains pre-lock. The compatibility path is
    ...
    real_session_db_type = diting_state.SessionDB
    monkeypatch.setattr(diting_state, "SessionDB", legacy_type)
    try:
        ...
        monkeypatch.setattr(diting_state, "SessionDB", real_session_db_type)
