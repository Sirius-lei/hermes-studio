...
def test_diting_client_tag_includes_current_version():
    """The client tag must reflect diting_cli.__version__ verbatim."""
    from diting_cli import __version__
    from agent.portal_tags import diting_client_tag

    assert diting_client_tag() == f"client=diting-client-v{__version__}"


def test_diting_client_tag_format():
    """The client tag has the exact shape Nous Portal expects."""
    from agent.portal_tags import diting_client_tag

    tag = diting_client_tag()
    assert tag.startswith("client=diting-client-v")
...
def test_nous_portal_tags_contains_product_and_client():
    ...
    from agent.portal_tags import diting_client_tag, nous_portal_tags

    tags = nous_portal_tags()
    assert "product=diting-agent" in tags
    assert diting_client_tag() in tags
    assert len(tags) == 2
