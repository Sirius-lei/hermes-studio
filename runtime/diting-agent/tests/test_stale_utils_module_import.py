...
Real incident (gateway session 1518671026962174144)::
    ...
    cannot import name 'env_float' from 'utils' (~/.diting/diting-agent/utils.py)
...
2. ``diting update`` ran ``git pull``, updating ``utils.py`` (now defining
...
``diting_cli/main.py`` (the ``diting update`` flow, ~line 9326) already
acknowledges this exact hazard -- "source files on disk are newer than cached
Python modules in this process" -- and reloads ``diting_constants`` after the
