...
Run from the PR worktree:
    ...
    cd ~/.diting/diting-agent/.worktrees/browser-providers-plugin
...
# checkout lives two levels up at ``~/.diting/diting-agent``.
MAIN_DIR = REPO_ROOT.parent.parent  # ~/.diting/diting-agent
PR_DIR = REPO_ROOT  # the worktree we're in
assert (MAIN_DIR / "tools" / "browser_tool.py").exists(), (
    f"MAIN_DIR={MAIN_DIR} doesn't look like a diting-agent checkout"
)
...
# Isolated DiTing_HOME for the config write.
