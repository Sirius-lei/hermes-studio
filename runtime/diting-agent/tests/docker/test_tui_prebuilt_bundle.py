...
package-lock.json. Without DiTing_TUI_DIR set, ``_make_tui_argv`` falls
...
The fix is ``ENV DiTing_TUI_DIR=/opt/diting/ui-tui`` in the Dockerfile, which
...
def _exec_py(image: str, py: str) -> str:
    """Run a Python snippet inside the image as the diting user, return stdout."""
    inner = (
        "source /opt/diting/.venv/bin/activate && "
        "cd /opt/diting && "
        f"python3 -c {shlex.quote(py)}"
    )
    # Drop to the diting user (UID 10000) so we exercise the same path the
    # dashboard PTY child runs as — not root.
    cmd = [
        "docker", "run", "--rm", "--entrypoint", "su", image,
        "diting", "-s", "/bin/bash", "-c", inner,
    ]
...
def test_diting_tui_dir_env_is_set(built_image: str) -> None:
    """DiTing_TUI_DIR must point at the prebuilt bundle dir in the image."""
    r = subprocess.run(
        ["docker", "run", "--rm", "--entrypoint", "sh", built_image,
         "-c", 'printf "%s" "$DiTing_TUI_DIR"'],
        capture_output=True, text=True, timeout=60,
    )
    assert r.returncode == 0, r.stderr[-2000:]
    assert r.stdout.strip() == "/opt/diting/ui-tui", (
        f"DiTing_TUI_DIR={r.stdout.strip()!r} (expected /opt/diting/ui-tui)"
    )
...
def test_prebuilt_bundle_present_and_no_runtime_install(built_image: str) -> None:
    ...
    py = (
        ...
        "from diting_cli.main import _tui_need_npm_install, _find_bundled_tui, _make_tui_argv\n"
        "ui = Path('/opt/diting/ui-tui')\n"
    ...
    # With DiTing_TUI_DIR set, _make_tui_argv returns the prebuilt path BEFORE
