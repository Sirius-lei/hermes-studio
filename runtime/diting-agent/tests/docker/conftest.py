...
Override the image with ``DiTing_TEST_IMAGE`` env var to point at a pre-built
...
IMAGE_TAG = os.environ.get("DiTing_TEST_IMAGE", "diting-agent-harness:latest")
...
def built_image() -> str:
    ...
    Override with ``DiTing_TEST_IMAGE`` env var to point at a pre-built
    image (faster local iteration).
    """
    if os.environ.get("DiTing_TEST_IMAGE"):
        return IMAGE_TAG
...
# ---------------------------------------------------------------------------
# docker_exec — default to the unprivileged diting user
# ---------------------------------------------------------------------------
#
# Background: every DiTing runtime path inside the container drops to UID
# 10000 (the ``diting`` user) via ``s6-setuidgid diting``. ``docker exec``
...
# fails (PermissionError swallowed) for diting, so a test that ran as root
...
# helpers below run every probe under ``-u diting`` unless a specific
...
def docker_exec(
    ...
    *args: str,
    ...
    """Run a command inside ``container`` as ``user`` (default: diting).
