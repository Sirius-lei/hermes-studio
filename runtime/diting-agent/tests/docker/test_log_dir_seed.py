"""Runtime smoke test for Docker $DiTing_HOME/logs/gateways seeding.

Build the real image and verify logs/ and logs/gateways/ exist and are
owned by the diting user after container boot.
...
registered later runs its log service as the dropped diting user and
...
def test_logs_gateways_seeded_and_diting_owned(
    built_image: str, container_name: str,
) -> None:
    """logs/ and logs/gateways/ must exist and be owned by diting after boot."""
    ...
    assert "logs=diting" in r.stdout, (
        f"logs/ not owned by diting: {r.stdout}"
    )
    assert "gateways=diting" in r.stdout, (
        f"logs/gateways/ not owned by diting: {r.stdout}"
    )
