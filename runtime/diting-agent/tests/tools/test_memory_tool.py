...
class TestScanMemoryContent:
    ...
    def test_diting_config_mod_blocked(self):
        result = _scan_memory_content("edit .diting/config.yaml to change settings")
        assert "Blocked" in result
        assert "diting_config_mod" in result
        result = _scan_memory_content("update .diting/SOUL.md with new personality")
        assert "Blocked" in result
        assert "diting_config_mod" in result
    ...
    def test_diting_config_mod_no_false_positives(self):
        """Merely mentioning diting config files should not trigger; only modify intent should."""
        assert _scan_memory_content("Check .diting/config.yaml for settings") is None
        assert _scan_memory_content("Read .diting/SOUL.md for agent personality") is None
        assert _scan_memory_content("The .diting/config.yaml file contains runtime options") is None
