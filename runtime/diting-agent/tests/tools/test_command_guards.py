...
def _clean_state():
    ...
    saved = {}
    for k in ("DiTing_INTERACTIVE", "DiTing_GATEWAY_SESSION", "DiTing_EXEC_ASK", "DiTing_YOLO_MODE"):
        if k in os.environ:
...
class TestTirithWarnSafe:
    ...
    def test_warn_session_approved(self, mock_tirith):
        os.environ["DiTing_INTERACTIVE"] = "1"
        session_key = os.getenv("DiTing_SESSION_KEY", "default")
    ...
    def test_warn_non_interactive_auto_allow(self, mock_tirith):
        # No DiTing_INTERACTIVE or DiTing_GATEWAY_SESSION set
        result = check_all_command_guards("curl https://bit.ly/abc", "local")
        assert result["approved"] is True
