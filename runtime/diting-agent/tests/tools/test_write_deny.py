...
class TestWriteDenyExactPaths:
    ...
    def test_diting_env(self):
        # ``.env`` under the active DiTing_HOME (profile-aware, not just
        # ``~/.diting``) must be write-denied. The hermetic test conftest
        # points DiTing_HOME at a tempdir — resolve via get_diting_home()
        # to match the denylist.
        from diting_constants import get_diting_home
        path = str(get_diting_home() / ".env")
        assert _is_write_denied(path) is True

    def test_diting_root_env_when_running_under_profile(self, tmp_path, monkeypatch):
        ...
        ``~/.diting/.env`` (whose credentials are inherited by every profile)
        could be silently overwritten by ``write_file`` while a profile was
        active.
        """
        root = tmp_path / "diting_root"
        ...
        # Sanity check: DiTing_HOME does point to the profile dir, not the root.
        from diting_constants import get_diting_home, get_default_diting_root
        assert get_diting_home() == profile_home
        assert get_default_diting_root() == root
...
class TestWriteAllowed:
    ...
    def test_diting_control_files_requested_writable(self):
        from diting_constants import get_diting_home
