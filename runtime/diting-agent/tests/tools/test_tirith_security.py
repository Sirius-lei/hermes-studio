...
class TestInstallArchiveMemberValidation:
    ...
    def test_install_extracts_regular_tirith_member(self, mock_target, mock_which,
        ...
        assert reason == ""
        assert path == str(diting_home / "bin" / "tirith")
    ...
    def test_install_rejects_non_regular_tirith_member(self, mock_target, mock_which,
        ...
        assert not os.path.lexists(diting_home / "bin" / "tirith")
...
class TestDiskFailureMarker:
    ...
    def test_install_failed_recovers_from_diting_bin(self):
        """After _INSTALL_FAILED, manual install in DiTing_HOME/bin is picked up."""
        from tools.tirith_security import _resolve_tirith_path, _INSTALL_FAILED
        import tempfile
        tmpdir = tempfile.mkdtemp()
        diting_bin = os.path.join(tmpdir, "tirith")
        # Create a fake executable
        with open(diting_bin, "w") as f:
            f.write("#!/bin/sh\n")
        os.chmod(diting_bin, 0o755)

        _tirith_mod._resolved_path = _INSTALL_FAILED

        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._diting_bin_dir", return_value=tmpdir), \
             patch("tools.tirith_security._clear_install_failed") as mock_clear:
            result = _resolve_tirith_path("tirith")
            assert result == diting_bin
            assert _tirith_mod._resolved_path == diting_bin
            mock_clear.assert_called_once()
...
# ---------------------------------------------------------------------------
# DiTing_HOME isolation
# ---------------------------------------------------------------------------

class TestDiTingHomeIsolation:
    def test_diting_bin_dir_respects_diting_home(self):
        """_diting_bin_dir must use DiTing_HOME, not hardcoded ~/.diting."""
        from tools.tirith_security import _diting_bin_dir
        import tempfile
        tmpdir = tempfile.mkdtemp()
        with patch.dict(os.environ, {"DiTing_HOME": tmpdir}):
            result = _diting_bin_dir()
        assert result == os.path.join(tmpdir, "bin")
        assert os.path.isdir(result)

    def test_failure_marker_respects_diting_home(self):
        """_failure_marker_path must use DiTing_HOME, not hardcoded ~/.diting."""
        from tools.tirith_security import _failure_marker_path
        with patch.dict(os.environ, {"DiTing_HOME": "/custom/diting"}):
            result = _failure_marker_path()
        assert result == "/custom/diting/.tirith-install-failed"

    def test_conftest_isolation_prevents_real_home_writes(self):
        """The conftest autouse fixture sets DiTing_HOME; verify it's active."""
        diting_home = os.getenv("DiTing_HOME")
        assert diting_home is not None, "DiTing_HOME should be set by conftest"
        assert "diting_test" in diting_home, "Should point to test temp dir"

    def test_get_diting_home_fallback(self):
        """Without DiTing_HOME set, falls back to the active OS home."""
        from tools.tirith_security import _get_diting_home
        with patch.dict(os.environ, {}, clear=True):
            # Remove DiTing_HOME entirely. With HOME also absent, expanduser
            ...
            expected = os.path.join(os.path.expanduser("~"), ".diting")
            result = _get_diting_home()
    ...
    def test_mkdtemp_oserror_propagates_to_ensure_installed(self):
        ...
        with patch("tools.tirith_security.tempfile.mkdtemp",
             ...
             patch("tools.tirith_security.shutil.which",
                   return_value=None), \
             patch("tools.tirith_security._diting_bin_dir",
