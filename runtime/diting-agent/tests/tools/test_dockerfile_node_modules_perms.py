...
Older images made /opt/diting/ui-tui and /opt/diting/node_modules writable so a
...
def test_dockerfile_uses_prebuilt_tui_instead_of_writable_node_modules() -> None:
    ...
    assert "ENV DiTing_TUI_DIR=/opt/diting/ui-tui" in text
    assert "cd ../ui-tui && npm run build" in text
    assert "chown -R diting:diting /opt/diting/ui-tui" not in text
    assert "chown -R diting:diting /opt/diting/node_modules" not in text
