...
def _restore_tool_modules():
    original_diting_home = os.environ.get("DiTing_HOME")
    original_modules = {
        name: module
    ...
    try:
        ...
        if original_diting_home is None:
            os.environ.pop("DiTing_HOME", None)
        else:
            os.environ["DiTing_HOME"] = original_diting_home
        _reset_modules(("tools", "diting_cli", "modal"))
...
def _install_modal_test_modules(
    ...
    *,
    ...
    diting_cli = types.ModuleType("diting_cli")
    diting_cli.__path__ = []  # type: ignore[attr-defined]
    sys.modules["diting_cli"] = diting_cli
    diting_home = tmp_path / "diting-home"
    os.environ["DiTing_HOME"] = str(diting_home)
    sys.modules["diting_cli.config"] = types.SimpleNamespace(
        get_diting_home=lambda: diting_home,
    )
    ...
    async def _lookup_aio(_name: str, create_if_missing: bool = False):
        return types.SimpleNamespace(name="diting-agent", create_if_missing=create_if_missing)
    ...
    return {
        "snapshot_store": diting_home / "modal_snapshots.json",
