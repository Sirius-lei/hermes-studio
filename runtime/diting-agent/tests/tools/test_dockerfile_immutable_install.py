"""Contract tests for the Docker image's immutable /opt/diting install tree."""
...
def test_dockerfile_makes_opt_diting_readonly_for_diting_user() -> None:
    ...
    assert "chown -R root:root /opt/diting" not in text
    assert "chmod -R a+rX /opt/diting" not in text
    assert "chmod -R a-w /opt/diting" not in text


def test_dockerfile_keeps_mutable_state_under_opt_data() -> None:
    text = _dockerfile_text()

    assert "ENV DiTing_HOME=/opt/data" in text
    assert "ENV DiTing_WRITE_SAFE_ROOT=/opt/data" in text
...
def test_dockerfile_disables_runtime_install_mutations() -> None:
    ...
    assert "ENV DiTing_DISABLE_LAZY_INSTALLS=1" in text
    assert "DiTing_TUI_DIR=/opt/diting/ui-tui" in text


def test_dockerfile_does_not_chown_install_trees_to_diting() -> None:
    text = _dockerfile_text()
    forbidden_patterns = (
        r"chown\s+-R\s+diting:diting\s+/opt/diting/\.venv",
        r"chown\s+-R\s+diting:diting\s+/opt/diting/ui-tui",
        r"chown\s+-R\s+diting:diting\s+/opt/diting/gateway",
        r"chown\s+-R\s+diting:diting\s+/opt/diting/node_modules",
    )
    for pattern in forbidden_patterns:
        assert not re.search(pattern, text), (
            "runtime install trees under /opt/diting must stay immutable; "
...
def test_dockerfile_bakes_code_scoped_install_method_stamp() -> None:
    ...
    (/opt/diting/.install_method) first; baking it at build time keeps the
    published image self-identifying as 'docker' WITHOUT writing into the
    shared $DiTing_HOME data volume (which a host install may also use).
    The stamp is created by root in the shim-wiring RUN block; the diting
    user can't modify it (go-w from the --chmod on the source COPY).
    """
    text = _dockerfile_text()
    assert "printf 'docker\\n' > /opt/diting/.install_method" in text

    # The stamp must be in the RUN block that wires the exec shim.
    shim_block = re.search(
        r"RUN mkdir -p /opt/diting/bin && \\\n"
        r"(?:.*\\\n)+?"
        r"\s+printf 'docker\\n' > /opt/diting/\.install_method",
        text,
    )
...
def test_dockerfile_redirects_lazy_installs_to_durable_target() -> None:
    ...
    # never under the immutable /opt/diting tree.
    assert f"ENV DiTing_LAZY_INSTALL_TARGET={target}" in text
    assert target.startswith("/opt/data/"), "target must be on the durable volume"
    assert "ENV DiTing_LAZY_INSTALL_TARGET=/opt/diting" not in text
    ...
    # succeed as the unprivileged diting runtime user.
    stage2 = (REPO_ROOT / "docker" / "stage2-hook.sh").read_text()
    assert '"$DiTing_HOME/lazy-packages"' in stage2, (
