...
def test_diting_provider_subclass_exists():
    """DiTingMCPOAuthProvider is defined and subclasses OAuthClientProvider."""
    from tools.mcp_oauth_manager import _DiTing_PROVIDER_CLS
    from mcp.client.auth.oauth2 import OAuthClientProvider

    assert _DiTing_PROVIDER_CLS is not None
    assert issubclass(_DiTing_PROVIDER_CLS, OAuthClientProvider)
...
def test_manager_builds_diting_provider_subclass(tmp_path, monkeypatch):
    """get_or_build_provider returns DiTingMCPOAuthProvider, not plain OAuthClientProvider."""
    from tools.mcp_oauth_manager import (
        MCPOAuthManager, _DiTing_PROVIDER_CLS, reset_manager_for_tests,
    )
    reset_manager_for_tests()
    monkeypatch.setenv("DiTing_HOME", str(tmp_path))
    _set_interactive_stdin(monkeypatch)
    ...
    assert _DiTing_PROVIDER_CLS is not None
    assert isinstance(provider, _DiTing_PROVIDER_CLS)
    assert provider._diting_server_name == "srv"
