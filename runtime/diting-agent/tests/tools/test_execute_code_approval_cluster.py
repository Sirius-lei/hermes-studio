...
Covers the canonical fix for issues #4146, #27303, #30882, #33057:
  ...
  4. tools.code_execution_tool._scrub_child_env — broad DiTing_ prefix dropped,
...
def gw_session(monkeypatch):
    """A clean gateway session: DiTing_GATEWAY_SESSION set, a bound session
...
def test_guard_cron_deny_blocks(monkeypatch):
    monkeypatch.setattr(A, "_YOLO_MODE_FROZEN", False)
    monkeypatch.setenv("DiTing_CRON_SESSION", "1")
    monkeypatch.delenv("DiTing_GATEWAY_SESSION", raising=False)
    monkeypatch.setattr(A, "_get_approval_mode", lambda: "manual")
...
def test_env_scrub_diting_allowlist_and_secret_blocks():
    from tools.code_execution_tool import _scrub_child_env

    env = {
        # operational allowlist → kept
        "DiTing_HOME": "/h", "DiTing_PROFILE": "p",
        "DiTing_CONFIG": "/c.yaml", "DiTing_ENV": "/e",
        # other DiTing_* → dropped (broad prefix removed)
        "DiTing_BASE_URL": "https://x", "DiTing_INTERACTIVE": "1",
        "DiTing_KANBAN_DB": "postgres://u:p@h/db",
    ...
    for kept in ("DiTing_HOME", "DiTing_PROFILE", "DiTing_CONFIG", "DiTing_ENV", "PATH"):
        assert kept in out, f"{kept} should be kept"
    for dropped in (
        "DiTing_BASE_URL", "DiTing_INTERACTIVE", "DiTing_KANBAN_DB",
...
def test_env_scrub_logs_dropped_diting_vars(caplog):
    """Dropping a non-allowlisted, non-secret DiTing_* var must be diagnosable:
    ...
    leave users guessing why a sandbox script sees an unset DiTing_* var."""
    import logging

    from tools.code_execution_tool import _scrub_child_env

    env = {
        "DiTing_HOME": "/h",          # allowlisted → kept, not logged
        "DiTing_BASE_URL": "https://x",   # dropped → logged
        "DiTing_KANBAN_DB": "postgres://u:p@h/db",  # dropped → logged
        "DiTing_API_KEY": "sk",       # secret → dropped silently (not logged)
        "PATH": "/usr/bin",           # safe prefix → kept
    }
    with caplog.at_level(logging.DEBUG, logger="tools.code_execution_tool"):
        out = _scrub_child_env(env, is_passthrough=lambda _: False, is_windows=False)

    assert "DiTing_HOME" in out and "PATH" in out
    assert "DiTing_BASE_URL" not in out and "DiTing_KANBAN_DB" not in out

    msgs = "\n".join(r.getMessage() for r in caplog.records)
    assert "DiTing_BASE_URL" in msgs and "DiTing_KANBAN_DB" in msgs
    assert "env_passthrough" in msgs
    # Secret vars are dropped but must NOT be named in the diagnostic log.
    assert "DiTing_API_KEY" not in msgs


def test_env_scrub_no_log_when_nothing_dropped(caplog):
    """No diagnostic noise when there are no dropped DiTing_* vars."""
    import logging
