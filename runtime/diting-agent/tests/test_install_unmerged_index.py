...
The ``diting update`` Python path already clears the conflict with ``git reset``
...
def test_install_ps1_stops_venv_resident_processes_before_removing_venv() -> None:
    ...
    ``venv\\Scripts\\pythonw.exe -m diting_cli.main gateway run`` — image name
    ``pythonw``, not ``diting.exe`` — so the ``taskkill /IM diting.exe`` guard
    ...
    # The diting.exe tree-kill is preserved (kills spawned child processes too).
    assert 'taskkill /F /T /IM diting.exe' in text
