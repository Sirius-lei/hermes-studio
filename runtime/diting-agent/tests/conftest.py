"""Shared fixtures for the diting-agent test suite.
...
2. **Isolated DiTing_HOME.** DiTing_HOME points to a per-test tempdir so
   code reading ``~/.diting/*`` via ``get_diting_home()`` can't see the
   real one. (We do NOT also redirect HOME — that broke subprocesses in
   CI. Code using ``Path.home() / ".diting"`` instead of the canonical
   ``get_diting_home()`` is a bug to fix at the callsite.)
3. **Deterministic runtime.** TZ=UTC, LANG=C.UTF-8, PYTHONHASHSEED=0.
4. **No DiTing_SESSION_* inheritance** — the agent's current gateway
...
# DiTing_* vars that change test behavior by being set. Unset all of these
# unconditionally — individual tests that need them set do so explicitly.
_DiTing_BEHAVIORAL_VARS = frozenset({
    "DiTing_YOLO_MODE",
    "DiTing_INTERACTIVE",
    "DiTing_QUIET",
    "DiTing_TOOL_PROGRESS",
    "DiTing_TOOL_PROGRESS_MODE",
    "DiTing_MAX_ITERATIONS",
    "DiTing_SESSION_PLATFORM",
    "DiTing_SESSION_CHAT_ID",
    "DiTing_SESSION_CHAT_NAME",
    "DiTing_SESSION_THREAD_ID",
    "DiTing_SESSION_SOURCE",
    "DiTing_SESSION_KEY",
    "DiTing_GATEWAY_SESSION",
    "DiTing_CRON_SESSION",
    "_DiTing_GATEWAY",
    "DiTing_PLATFORM",
    "DiTing_MODEL",
    "DiTing_INFERENCE_MODEL",
    "DiTing_INFERENCE_PROVIDER",
    "DiTing_TUI_PROVIDER",
    "DiTing_MANAGED",
    "DiTing_MANAGED_DIR",
    "DiTing_DEV",
    ...
    "DiTing_TIMEZONE",
    "DiTing_REDACT_SECRETS",
    "DiTing_BACKGROUND_NOTIFICATIONS",
    "DiTing_EXEC_ASK",
    "DiTing_HOME_MODE",
    "DiTing_AGENT_USE_LEGACY_SESSION_KEYS",
    # Kanban path/board pins must never leak from a developer shell or
    # dispatched worker into tests; otherwise tests can write fake tasks to
    # the real ~/.diting/kanban.db instead of the per-test DiTing_HOME.
    "DiTing_KANBAN_DB",
    "DiTing_KANBAN_BOARD",
    "DiTing_KANBAN_HOME",
    "DiTing_KANBAN_WORKSPACES_ROOT",
    "DiTing_KANBAN_LOGS_ROOT",
    "DiTing_KANBAN_TASK",
    "DiTing_KANBAN_WORKSPACE",
    "DiTing_KANBAN_RUN_ID",
    "DiTing_KANBAN_CLAIM_LOCK",
    "DiTing_KANBAN_DISPATCH_IN_GATEWAY",
    "DiTing_TENANT",
    ...
    "DiTing_DASHBOARD_OAUTH_CLIENT_ID",
    "DiTing_DASHBOARD_PORTAL_URL",
    "TERMINAL_CWD",
    "TERMINAL_ENV",
...
def _hermetic_environment(tmp_path, monkeypatch):
    ...
    Also redirects HOME and DiTing_HOME to per-test tempdirs so code that
    reads ``~/.diting/*`` can't touch the real one, and pins TZ/LANG so
    ...
    # 2. Blank behavioral DiTing_* vars that could change test semantics.
    for name in _DiTing_BEHAVIORAL_VARS:
        monkeypatch.delenv(name, raising=False)

    # 3. Redirect DiTing_HOME to a per-test tempdir. Code that reads
    #    ``~/.diting/*`` via ``get_diting_home()`` now gets the tempdir.
    #
    #    NOTE: We do NOT also redirect HOME. Doing so broke CI because
    #    some tests (and their transitive deps) spawn subprocesses that
    #    inherit HOME and expect it to be stable. If a test genuinely
    #    needs HOME isolated, it should set it explicitly in its own
    #    fixture. Any code in the codebase reading ``~/.diting/*`` via
    #    ``Path.home() / ".diting"`` instead of ``get_diting_home()``
    #    is a bug to fix at the callsite.
    fake_diting_home = tmp_path / "diting_test"
    fake_diting_home.mkdir()
    (fake_diting_home / "sessions").mkdir()
    (fake_diting_home / "cron").mkdir()
    (fake_diting_home / "memories").mkdir()
    (fake_diting_home / "skills").mkdir()
    monkeypatch.setenv("DiTing_HOME", str(fake_diting_home))
    ...
    #    ~/.diting/plugins/ (which, per step 3, is now empty — but the
    #    singleton might still be cached from a previous test).
    try:
        import diting_cli.plugins as _plugins_mod
...
@pytest.fixture(autouse=True)
def _isolate_diting_home(_hermetic_environment):
...
@pytest.fixture()
def mock_config():
    """Return a minimal diting config dict suitable for unit tests."""
    return {
...
# environment and finds the developer's live ``diting-gateway`` process
...
# correlated with a ``tests/diting_cli/`` pytest run starting up.
#
...
#    ``check_output`` reject any ``systemctl ... <verb> diting-gateway``
...
def _live_system_guard(request, monkeypatch):
    ...
    tokens[0]), so ``bash -c "systemctl restart diting-gateway"``,
    ``sudo systemctl ...``, ``env systemctl ...``, ``setsid systemctl ...``
    are all caught. ``pkill``/``killall``/``taskkill`` invocations
    targeting diting/python patterns are also blocked.
    """
    ...
    def _matches_diting_gateway(cmd_str: str) -> bool:
        low = cmd_str.lower()
        return any(tok in low for tok in _DiTing_TOKENS)

    def _is_blocked_systemctl(cmd) -> bool:
        cmd_str = _cmd_to_string(cmd)
        if "systemctl" not in cmd_str:
            return False
        if not _matches_diting_gateway(cmd_str):
            return False
        try:
    ...
    def _is_process_killer(cmd) -> bool:
        ...
        for tok in tokens:
            ...
            if head in _PROCESS_KILLERS:
                low = cmd_str.lower()
                # pkill -f pattern: catch diting-themed patterns + a
                # plain "python" -f which would catch the live gateway
                # whose cmdline contains "python -m diting_cli.main".
                if (
                    "diting" in low
                    or "gateway" in low
    ...
    def _check_subprocess_cmd(name, cmd):
        if _is_blocked_systemctl(cmd):
            raise RuntimeError(
                ...
                "live diting-gateway systemd unit. Mock "
        ...
        if _is_process_killer(cmd):
            raise RuntimeError(
                ...
                "targeting diting/python could hit the live gateway. "
                "Mark with @pytest.mark.live_system_guard_bypass if "
                "intentional."
            )
        # Block any subprocess that would run `diting update` (or the
        # equivalent `python -m diting_cli.main update`).  These commands
        ...
        if "update" in low and (
            # diting update / diting update --gateway / setsid bash -c ... diting update
            ("diting" in low and "update" in low.split())
            or
            # python -m diting_cli.main update --gateway
            ("diting_cli" in low and "update" in low.split())
            or
            # venv/bin/diting update  (absolute path variant used in tests)
            (".venv/bin/diting" in low and "update" in low)
        ):
            raise RuntimeError(
                f"tests/conftest.py live-system guard: blocked "
                f"subprocess.{name}({cmd!r}) — this command would run "
                "`diting update` against the real checkout, fetching "
