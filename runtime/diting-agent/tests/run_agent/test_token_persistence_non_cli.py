...
def test_session_search_lazily_opens_db_when_entrypoint_did_not_pass_one(monkeypatch):
    ...
    diting_state = ModuleType("diting_state")
    diting_state.SessionDB = FakeSessionDB
    monkeypatch.setitem(sys.modules, "diting_state", diting_state)
    ...
    result = json.loads(agent._invoke_tool("session_search", {"query": "DiTing"}, "task-id"))

    assert result["success"] is True
    assert captured["db"] is sentinel_db
    assert captured["query"] == "DiTing"
