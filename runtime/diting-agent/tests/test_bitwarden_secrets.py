...
binary download are exercised manually by `diting secrets bitwarden
...
def diting_home(tmp_path, monkeypatch):
    """Point DiTing at an isolated home directory."""
    home = tmp_path / ".diting"
    home.mkdir()
    monkeypatch.setenv("DiTing_HOME", str(home))
    # Some modules cache get_diting_home; clear if needed.
    import diting_constants
    if hasattr(diting_constants, "_DiTing_HOME_CACHE"):
        diting_constants._DiTing_HOME_CACHE = None  # type: ignore[attr-defined]
    return home
...
def test_install_bws_rejects_malicious_member(diting_home, monkeypatch):
...
def test_install_bws_happy_path(diting_home, monkeypatch):
...
def test_install_bws_checksum_mismatch(diting_home, monkeypatch):
...
def test_install_bws_missing_checksum_entry(diting_home, monkeypatch):
...
def test_env_loader_skips_when_disabled(tmp_path, monkeypatch):
    ...
    from diting_cli.env_loader import _apply_external_secret_sources
...
def test_disk_cache_written_after_first_fetch(monkeypatch, tmp_path):
    """First fetch hits bws AND writes a 0600 file under diting_home/cache/."""
    home = tmp_path / ".diting"
    home.mkdir()
    fake_binary = tmp_path / "bws"
    fake_binary.write_text("")
