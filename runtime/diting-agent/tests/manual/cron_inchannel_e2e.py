...
Uses a throwaway DiTing_HOME so it never touches ~/.diting. No real names.
"""
...
def _fresh_home():
    """Point DiTing_HOME at a throwaway dir BEFORE importing gateway modules
    (mirror.py binds _SESSIONS_INDEX from get_diting_home() at import time)."""
...
# Import AFTER DiTing_HOME is set.
...
# a different get_diting_home() at import if something imported it earlier).
...
def main():
    ...
    print(f"DiTing_HOME (throwaway): {HOME}")
