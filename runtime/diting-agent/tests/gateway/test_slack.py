...
def _redirect_cache(tmp_path, monkeypatch):
    """Point document cache to tmp_path so tests don't touch ~/.diting."""
    monkeypatch.setattr(
...
class TestAppMentionHandler:
    ...
    def test_app_mention_registered_on_connect(self):
        ...
        # covering every COMMAND_REGISTRY entry (e.g. /diting, /btw, /stop,
        ...
        for expected in ("/diting", "/btw", "/stop", "/model", "/help"):
...
class TestSlackProxyBehavior:
    ...
    async def test_connect_uses_proxy_when_not_bypassed(self):
        ...
        assert "diting_feedback" in created_apps[0].registered_actions
...
class TestBangPrefixCommands:
    ...
    async def test_bang_with_bot_suffix_resolves(self, adapter):
        """``!stop@diting`` matches the get_command() ``@suffix`` stripping."""
        await adapter._handle_slack_message(self._make_event("!stop@diting"))

        msg_event = adapter.handle_message.call_args[0][0]
        assert msg_event.text.startswith("/stop@diting")
        assert msg_event.message_type == MessageType.COMMAND
...
class TestSlashCommands:
    ...
    # instead of as /diting subcommands. This is the Discord/Telegram parity
    # fix: the slash name itself becomes the command.
    # ------------------------------------------------------------------

    @pytest.mark.asyncio
    async def test_native_btw_slash(self, adapter):
        """/btw with args must dispatch to /background, not /diting btw."""
        command = {
            "command": "/btw",
    ...
    @pytest.mark.asyncio
    async def test_legacy_diting_prefix_still_works(self, adapter):
        """Backward compat: /diting btw foo must still route to /btw foo.

        Old workspace manifests only declared /diting as the single slash.
    ...
    @pytest.mark.asyncio
    async def test_legacy_diting_freeform_question(self, adapter):
        """/diting <free-form text> must stay as the raw text (non-command)."""
        command = {
            "command": "/diting",
...
class TestSlashEphemeralAck:
    ...
    @pytest.mark.asyncio
    async def test_legacy_diting_slash_stashes_context(self, adapter):
        """Legacy /diting <subcommand> also stashes context."""
        command = {
            "command": "/diting",
            "text": "help",
            "user_id": "U_H",
    ...
    @pytest.mark.asyncio
    async def test_freeform_diting_question_does_not_stash_context(self, adapter):
        """Free-form /diting <question> must NOT route agent reply ephemeral."""
        command = {
            "command": "/diting",
