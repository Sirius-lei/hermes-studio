...
class TestSplitPairingDirMigration:
    def test_merges_new_approved_into_active_legacy_dir(self, tmp_path):
        ...
        with patch("gateway.pairing.PAIRING_DIR", legacy), patch("gateway.pairing.get_diting_home", return_value=home):
            store = PairingStore()
...
class TestProfileScopedStorage:
    ...
    under <DiTing_HOME>/profiles/<name>/pairing/ so a multiplexing gateway
    ...
    def test_default_store_uses_global_dir(self, tmp_path, monkeypatch):
        ...
        ``diting pairing`` CLI continues to work without a profile context."""
    ...
    def test_profile_store_uses_profiles_subdir(self, tmp_path, monkeypatch):
        ...
        <DiTing_HOME>/profiles/yangyang/pairing/."""
        from diting_constants import get_diting_home
        monkeypatch.setattr("diting_constants.get_diting_home", lambda: tmp_path)
