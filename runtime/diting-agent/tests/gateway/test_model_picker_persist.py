...
closure the PR changed, against a real temp ``DiTing_HOME``.
"""
...
def _fake_switch_result():
    ...
    from diting_cli.model_switch import ModelSwitchResult
...
def _setup_isolated_home(tmp_path, monkeypatch, model_yaml_value):
    ...
    monkeypatch.setattr(
        "diting_cli.model_switch.resolve_display_context_length",
        lambda *a, **k: 272000,
    )
    # save_config writes to ``get_diting_home() / config.yaml`` — point it here.
    monkeypatch.setattr("diting_constants.get_diting_home", lambda: diting_home)
    monkeypatch.setattr("diting_cli.config.get_diting_home", lambda: diting_home)
    return cfg_path
