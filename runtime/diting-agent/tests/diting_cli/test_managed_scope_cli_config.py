...
diting_cli.config._load_config_impl, so the managed-scope merge has to be
applied in BOTH places or the interactive CLI/TUI surface (skin, display prefs)
silently ignores administrator-pinned values while `diting config`/`doctor`
...
def _load_cli_config(home):
    ...
    cli.py binds ``_diting_home = get_diting_home()`` at import time (module
    singleton), so monkeypatching DiTing_HOME after import doesn't move it.
    ...
    this only matters for tests that swap DiTing_HOME.)
    """
    import cli

    cli._diting_home = home
