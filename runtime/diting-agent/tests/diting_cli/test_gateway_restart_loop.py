...
Covers:
- Defense 1: gateway stop/restart refuse when _DiTing_GATEWAY=1
...
class TestGatewayLifecyclePattern:
    ...
    @pytest.mark.parametrize("text", [
        ...
        "diting gateway stop",
        "diting  gateway  restart",         # double spaces
        "Hermez Gateway Restart".lower().replace("z", "s"),  # case handled
        "DiTing GATEWAY RESTART",           # uppercase
    ])
    def test_diting_gateway_commands(self, text):
        assert _contains_gateway_lifecycle_command(text), f"Should match: {text!r}"

    @pytest.mark.parametrize("text", [
        "launchctl kickstart gui/501/ai.diting.gateway",
        "launchctl unload ~/Library/LaunchAgents/ai.diting.gateway.plist",
        "launchctl stop ai.diting.gateway",
        "systemctl restart diting-gateway",
        "systemctl stop diting-gateway.service",
        "systemctl start diting-gateway",
    ])
    def test_service_manager_commands(self, text):
        assert _contains_gateway_lifecycle_command(text), f"Should match: {text!r}"

    @pytest.mark.parametrize("text", [
        "kill diting gateway process",
        "pkill -f diting.*gateway",
        "pkill -f gateway.*diting",          # inverse token order
    ])
    ...
    @pytest.mark.parametrize("text", [
        ...
        "diting cron list",
        "diting update",
        "diting config set model claude",
        "echo 'just a normal cron job'",
        "run the backup script",
        "gateway is running fine",
        # `diting gateway start` is benign — starting a gateway from inside a
        ...
        "diting gateway start --all",
        # Tightened launchctl/systemctl branches: ops on NON-gateway diting
        # services must not be falsely blocked (the old `.*diting` matched any
        # diting token).
        "launchctl unload ai.diting.update-checker.plist",
        "launchctl restart ai.diting.daemon",
        "systemctl restart diting-meta.service",
        "systemctl restart diting-cron-helper",
...
class TestCronCreateLifecycleBlock:
    ...
    def test_block_diting_gateway_restart(self, capsys):
        args = Namespace(
            cron_command="create",
            schedule="30m",
            prompt="Upgrade diting then run diting gateway restart",
            name=None,
            deliver=None,
            repeat=None,
            skill=None,
    ...
    def test_block_launchctl_kickstart(self, capsys):
        args = Namespace(
            cron_command="create",
            schedule="0 9 * * *",
            prompt="Run launchctl kickstart -k gui/501/ai.diting.gateway",
            name=None,
            deliver=None,
            repeat=None,
            skill=None,
    ...
    def test_block_script_with_lifecycle_command(self, tmp_path, capsys, monkeypatch):
        ...
        # restart_diting_gateway_once.sh). The script must live under
        # DiTing_HOME/scripts so the scheduler — and the guard — resolve it.
        monkeypatch.setenv("DiTing_HOME", str(tmp_path / ".diting"))
        scripts_dir = tmp_path / ".diting" / "scripts"
...
class TestGatewaySelfTargetingGuard:
    """Verify diting gateway stop/restart refuse when _DiTing_GATEWAY=1."""

    def test_stop_refuses_inside_gateway(self, monkeypatch):
        monkeypatch.setenv("_DiTing_GATEWAY", "1")
    ...
    def test_stop_allows_outside_gateway(self, monkeypatch):
        ...
        monkeypatch.delenv("_DiTing_GATEWAY", raising=False)
...
class TestTerminalToolGatewayLifecycleGuard:
    """terminal_tool must refuse gateway lifecycle commands when _DiTing_GATEWAY=1.

    Issue #37453: systemctl --user restart diting-gateway runs as a child of the
    ...
    @pytest.mark.parametrize("cmd", [
        "systemctl restart diting-gateway",
        "systemctl --user restart diting-gateway",
        "systemctl stop diting-gateway.service",
        "diting gateway restart",
        "launchctl kickstart gui/501/ai.diting.gateway",
        "pkill -f diting.*gateway",
    ...
    def test_force_true_cannot_bypass_block(self, monkeypatch):
        ...
        result = json.loads(tt.terminal_tool(
            command="systemctl restart diting-gateway", force=True
        ))
    ...
    def test_safe_systemctl_commands_pass_through(self, monkeypatch):
        """Non-diting systemctl commands must not be blocked by this guard."""
        import tools.terminal_tool as tt
    ...
    def test_guard_inactive_outside_gateway(self, monkeypatch):
        """Without _DiTing_GATEWAY=1 the lifecycle guard must not fire."""
        import tools.terminal_tool as tt
        ...
        result = json.loads(tt.terminal_tool(command="systemctl restart diting-gateway"))

        # Outside the gateway the lifecycle guard doesn't block — the normal
        # approval flow handles it (here mocked as approved).
        assert result["exit_code"] == 0
        assert calls == ["systemctl restart diting-gateway"]
...
class TestLifecycleGuardModule:
    ...
    def test_prompt_with_command_raises(self):
        ...
        with pytest.raises(GatewayLifecycleBlocked) as exc:
            check_gateway_lifecycle("please run diting gateway restart", None)
    ...
    def test_split_across_prompt_and_script_still_blocks(self, tmp_path):
        ...
        script.write_text("diting gateway stop\n")
        with pytest.raises(GatewayLifecycleBlocked):
    ...
    def test_binary_script_does_not_silently_bypass(self, tmp_path):
        ...
        script.write_bytes(b"\xfediting gateway restart\xff")
        with pytest.raises(GatewayLifecycleBlocked):
    ...
    def test_relative_script_resolved_under_scripts_dir(self, tmp_path, monkeypatch):
        """A bare/relative script name resolves under DiTing_HOME/scripts (the
        ...
        (scripts_dir / "restart.sh").write_text(
            "launchctl kickstart -k gui/501/ai.diting.gateway\n"
        )
        with pytest.raises(GatewayLifecycleBlocked):
...
class TestCreateJobBlocksLifecycleCommands:
    ...
    diting_cli.cron.cron_create. Enforcing at create_job covers both."""
    ...
    def test_create_job_blocks_prompt_command(self):
        ...
        with pytest.raises(GatewayLifecycleBlocked):
            create_job(prompt="then run diting gateway restart", schedule="30m")
    ...
    def test_cronjob_tool_surfaces_block_as_error(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("DiTing_HOME", str(tmp_path / ".diting"))
        (tmp_path / ".diting").mkdir(parents=True)
        from tools.cronjob_tools import cronjob
        result = json.loads(cronjob(
            action="create", schedule="0 9 * * *",
            prompt="please run diting gateway restart nightly",
        ))
