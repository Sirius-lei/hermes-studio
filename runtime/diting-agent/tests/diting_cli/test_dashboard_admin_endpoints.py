...
def _client():
    ...
    # Keep the state DB under the isolated DiTing_HOME for any handler that
    # touches it.
...
class TestMcpEndpoints:
    ...
    def _setup(self, _isolate_diting_home):
...
class TestMemoryEndpoints:
    ...
    def _setup(self, _isolate_diting_home):
        ...
        (get_diting_home() / "memories").mkdir(parents=True, exist_ok=True)
...
class TestOpsEndpoints:
    ...
    def test_backup_output_uses_output_flag(self, monkeypatch):
        ...
        r = self.client.post(
            "/api/ops/backup",
            json={"output": "  /tmp/diting-test.zip  "},
        )

        assert r.status_code == 200
        assert captured == {
            "subcommand": ["backup", "-o", "/tmp/diting-test.zip"],
            "name": "backup",
        }
...
class TestSystemStatsEndpoint:
    ...
    def test_stats_shape(self):
        ...
        for key in ("os", "arch", "hostname", "python_version", "diting_version"):
    ...
    def test_sources_lists_configured_hubs(self, monkeypatch):
        ...
        class _Src:
            ...
            def search(self, q, limit=10):
                return [_FakeMeta("diting-index/featured-skill", "trusted")]

        def _fake_router():
            srcs = [_Src("official"), _Src("github")]
            # diting-index source advertises availability + featured search.
            idx = _Src("diting-index")
        ...
        assert {"official", "github", "diting-index"} <= ids
...
class TestSkillsHubPreviewEndpoint:
    ...
    def test_preview_returns_skill_md_text(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "diting_cli.skills_hub._resolve_source_meta_and_bundle",
...
class TestUpdateCheckEndpoint:
    """``GET /api/diting/update/check`` reports availability without applying.

    Powers the dashboard's check-before-you-update flow: the System page
    shows the commit-behind count and asks the user to confirm before
    ``POST /api/diting/update`` runs ``diting update``.
    """
    ...
    def test_git_install_reports_behind_count(self, monkeypatch):
        ...
        import diting_cli.banner as banner

        monkeypatch.setattr(banner, "check_for_updates", lambda: 5)

        r = self.client.get("/api/diting/update/check")
        assert r.status_code == 200
        body = r.json()
        assert {
            "install_method",
    ...
    def test_up_to_date(self, monkeypatch):
        ...
        body = self.client.get("/api/diting/update/check").json()
...
class TestDebugShareEndpoint:
    ...
    def test_returns_structured_urls(self, monkeypatch):
        import diting_cli.debug as dbg
        ...
        monkeypatch.setattr("diting_cli.dump.run_dump", lambda a: None)
...
class TestToolsConfigEndpoints:
    ...
    the dashboard surface that replicates the `diting tools` configurator."""
    ...
    def test_post_setup_spawns_action(self, monkeypatch):
        ...
        monkeypatch.setattr(ws, "_spawn_diting_action", _fake_spawn)
        r = self.client.post(
