"""Tests for the top-level `./diting` launcher script."""
...
def test_launcher_delegates_to_argparse_entrypoint(monkeypatch):
    """`./diting` should use `diting_cli.main`, not the legacy Fire wrapper."""
    launcher_path = Path(__file__).resolve().parents[2] / "diting"
    called = []

    fake_main_module = types.ModuleType("diting_cli.main")

    def fake_main():
        called.append("diting_cli.main")

    fake_main_module.main = fake_main
    monkeypatch.setitem(sys.modules, "diting_cli.main", fake_main_module)
