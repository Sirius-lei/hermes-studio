...
the SAME provider universe as ``diting model`` (the CLI/TUI picker).
...
def test_every_diting_model_provider_is_configurable_in_desktop():
    """PARITY CONTRACT: GUI (keys ∪ accounts) ⊇ `diting model` universe."""
    ...
    assert not missing, (
        "providers shown in `diting model` but not configurable in the desktop "
