"""Tests for the per-profile Projects store (diting_cli/projects_db)."""
...
def test_create_get_list(conn):
    pid = pdb.create_project(conn, name="DiTing Agent", folders=["/tmp/diting"])
    proj = pdb.get_project(conn, pid)

    assert proj is not None
    assert proj.slug == "diting-agent"
    assert proj.name == "DiTing Agent"
    # First folder becomes primary.
    assert proj.primary_path == "/tmp/diting"
    assert [f.path for f in proj.folders] == ["/tmp/diting"]
    assert proj.folders[0].is_primary is True

    # Lookup by slug too.
    assert pdb.get_project(conn, "diting-agent").id == pid
    assert len(pdb.list_projects(conn)) == 1


def test_slug_collision_disambiguates(conn):
    pdb.create_project(conn, name="DiTing Agent")
    pdb.create_project(conn, name="DiTing Agent")
    slugs = sorted(p.slug for p in pdb.list_projects(conn))

    assert slugs == ["diting-agent", "diting-agent-2"]
...
def test_per_profile_isolation(tmp_path):
    # Two distinct DB paths stand in for two profiles' DiTing_HOME.
...
def test_db_path_under_diting_home():
    # Resolves under DiTing_HOME (set by the autouse isolation fixture).
