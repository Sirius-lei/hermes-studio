"""Tests for IRC gateway configuration via `diting setup gateway` UI.

Covers the full plugin-platform discovery → status → configure flow so that
a fresh DiTing install (no state, no env vars) can set up IRC through the
...
class TestIRCFreshInstallDiscovery:
    """IRC appears in the setup menu on a brand-new DiTing install."""
    ...
    def test_irc_status_configured_when_env_set(self, monkeypatch):
        ...
        try:
            ...
            monkeypatch.setenv("IRC_CHANNEL", "#diting")
            monkeypatch.setenv("IRC_NICKNAME", "diting-bot")
...
class TestIRCGatewaySetupFreshInstall:
    """Simulate the full `diting setup gateway` experience with IRC present."""
