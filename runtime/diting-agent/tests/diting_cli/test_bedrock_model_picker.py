...
Covers the three paths changed by fix/bedrock-provider-model-ids-live-discovery:
  ...
  2. list_authenticated_providers() Section 2 (DiTing_OVERLAYS) — bedrock
...
# ---------------------------------------------------------------------------
# 2. list_authenticated_providers() — bedrock via DiTing_OVERLAYS (Section 2)
# ---------------------------------------------------------------------------
...
class TestBedrockOverlayRegistration:
    """bedrock entry in DiTing_OVERLAYS is correctly configured."""

    def test_bedrock_overlay_exists(self):
        from diting_cli.providers import DiTing_OVERLAYS
        assert "bedrock" in DiTing_OVERLAYS

    def test_bedrock_overlay_transport(self):
        from diting_cli.providers import DiTing_OVERLAYS
        assert DiTing_OVERLAYS["bedrock"].transport == "bedrock_converse"

    def test_bedrock_overlay_auth_type(self):
        from diting_cli.providers import DiTing_OVERLAYS
        assert DiTing_OVERLAYS["bedrock"].auth_type == "aws_sdk"
