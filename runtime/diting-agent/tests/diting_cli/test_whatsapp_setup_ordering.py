...
Before the fix, ``diting whatsapp`` wrote ``WHATSAPP_ENABLED=true`` at
...
``diting gateway`` then paid a 30s bridge-bootstrap timeout and queued
...
def isolated_home(tmp_path, monkeypatch):
    home = tmp_path / "home"
    diting = home / ".diting"
    diting.mkdir(parents=True)
    monkeypatch.setattr(Path, "home", lambda: home)
    monkeypatch.setenv("DiTing_HOME", str(diting))
...
def _env_value(diting_home: Path, key: str) -> str | None:
...
def test_aborted_setup_does_not_enable_whatsapp(isolated_home, monkeypatch):
    ...
    """
    from diting_cli.main import cmd_whatsapp
    ...
    monkeypatch.setattr("diting_cli.main._require_tty", lambda *_a, **_kw: None)
...
def test_existing_pairing_skip_branch_enables_whatsapp(isolated_home, monkeypatch):
    """User runs ``diting whatsapp`` with an existing paired session and
