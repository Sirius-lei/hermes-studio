...
def diting_home(tmp_path, monkeypatch):
    ...
    # /etc/diting on the dev/CI box can't influence the test.
    monkeypatch.setenv("DiTing_MANAGED_DIR", str(tmp_path / "no_such_managed_dir"))
...
def test_user_config_overrides_default(diting_home, monkeypatch):
    from diting_cli.config import load_config, cfg_get
...
def test_env_expansion_in_user_config(diting_home, monkeypatch):
    from diting_cli.config import load_config, cfg_get
...
def test_no_managed_dir_means_user_value_wins(diting_home):
...
def test_user_env_overrides_shell(tmp_path, monkeypatch):
    ...
    load_diting_dotenv(diting_home=str(home))
