"""Tests for the `diting memory reset` CLI command.
...
- Profile-scoped reset (uses DiTing_HOME)
"""
...
def memory_env(tmp_path, monkeypatch):
    """Set up a fake DiTing_HOME with memory files."""
    diting_home = tmp_path / ".diting"
    memories = diting_home / "memories"
    memories.mkdir(parents=True)
    monkeypatch.setenv("DiTing_HOME", str(diting_home))

    # Create sample memory files
    (memories / "MEMORY.md").write_text(
        "§\nDiTing repo is at ~/.diting/diting-agent\n§\nUser prefers dark themes",
        encoding="utf-8",
    )
...
def _run_memory_reset(target="all", yes=False, monkeypatch=None, confirm_input="no"):
    ...
    Simulates what happens when `diting memory reset` is run.
    """
...
class TestMemoryReset:
    ...
    def test_reset_no_files_exist(self, tmp_path, monkeypatch):
        ...
        diting_home = tmp_path / ".diting"
        (diting_home / "memories").mkdir(parents=True)
        monkeypatch.setenv("DiTing_HOME", str(diting_home))
    ...
    def test_reset_profile_scoped(self, tmp_path, monkeypatch):
        """Reset should work on the active profile's DiTing_HOME."""
    ...
    def test_reset_empty_memories_dir(self, tmp_path, monkeypatch):
        ...
        # The memories dir won't exist; get_diting_home() / "memories" won't have files
        result = _run_memory_reset(target="all", yes=True)
        assert result == "nothing"
