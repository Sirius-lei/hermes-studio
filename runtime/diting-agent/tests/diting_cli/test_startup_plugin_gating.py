...
``diting_cli.main`` skips eager plugin discovery at argparse-setup time
when the invocation is clearly targeting a known built-in subcommand.
This saves 500-650ms on ``diting --help``, ``diting version``,
``diting logs``, etc., by not importing ``google.cloud.pubsub_v1``,
...
def _live_subcommand_names() -> set[str]:
    """Run ``diting --help`` in-process and parse the subcommand block.
    ...
    built-in-only set.
    """
    from diting_cli import main as _main

    argv_backup = sys.argv[:]
    sys.argv = ["diting", "--help"]
    buf = io.StringIO()
    try:
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (["diting", "-w", "--tui"], None),
        (["diting", "version"], "version"),
        (["diting", "--tui", "chat"], "chat"),
        (["diting", "-w", "logs"], "logs"),
        (["diting", "chat", "hello world"], "chat"),
        (["diting", "gateway", "run"], "gateway"),
        # Top-level value-taking flags: the value should be skipped.
        (["diting", "-m", "gpt5", "chat"], "chat"),
        (["diting", "--model", "gpt5", "chat", "hi"], "chat"),
        (["diting", "-m", "gpt5", "--provider", "openai", "chat"], "chat"),
        (["diting", "-z", "hello world"], None),
        (["diting", "-z", "hello", "chat"], "chat"),
        (["diting", "--model=gpt5", "chat"], "chat"),     # inline form
        (["diting", "--", "chat"], "chat"),               # -- terminator
        (["diting", "-w", "--"], None),
        # Unknown positional after skipped flags → plugin-cmd candidate.
        (["diting", "some-plugin-cmd"], "some-plugin-cmd"),
        (["diting", "-m", "gpt5", "some-plugin-cmd"], "some-plugin-cmd"),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ["diting"],                          # bare → chat
        ["diting", "--help"],                # top-level help
        ["diting", "-h"],
        ["diting", "version"],               # known built-in
        ["diting", "logs"],
        ["diting", "gateway", "run"],
        ["diting", "--tui"],
        ["diting", "-w", "--tui"],
        ["diting", "chat", "hi"],
        ["diting", "help"],                  # accepted built-in-ish
        ["diting", "-m", "gpt5", "chat"],    # flag-value-skipping
    ],
)
...
@pytest.mark.parametrize(
    "argv",
    [
        ["diting", "meet", "join"],          # potential google_meet plugin
        ["diting", "honcho", "status"],      # potential memory plugin
        ["diting", "unknown-subcmd"],
    ],
)
...
def test_builtin_set_covers_every_registered_subcommand():
    ...
    # in the set so ``diting help <cmd>`` skips discovery; it won't show
    ...
    assert not missing_from_declaration, (
        ...
        f"diting_cli/main.py::_BUILTIN_SUBCOMMANDS so plugin discovery "
