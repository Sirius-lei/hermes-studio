...
"""
    ...
    assert "Tyler Williams > Active Projects: DiTing Agent" in entries
...
def test_cron_store_is_archived_without_config_cron_section(tmp_path: Path):
    ...
    assert "Run `diting cron` to recreate scheduled tasks" in notes_text
...
def test_skill_installs_cleanly_under_skills_guard():
    ...
    # diting_config_mod  — print statements in the post-migration summary that
    #                      tell the user to *review* ~/.diting/config.yaml;
    ...
    KNOWN_FALSE_POSITIVES = {"agent_config_mod", "python_os_environ", "diting_config_mod"}
...
def test_rebrand_text_replaces_openclaw_variants():
    mod = load_module()
    # Mixed-case / capitalized matches → capital-H ``DiTing``.
    assert mod.rebrand_text("OpenClaw prefers Python 3.11") == "DiTing prefers Python 3.11"
    assert mod.rebrand_text("I told Open Claw to use dark mode") == "I told DiTing to use dark mode"
    assert mod.rebrand_text("Open-Claw config is great") == "DiTing config is great"
    assert mod.rebrand_text("OPENCLAW uses tools well") == "DiTing uses tools well"
    # All-lowercase matches → lowercase ``diting``; this preserves the
    # real filesystem path ``~/.diting`` (DiTing home) when rebranding
    # memory entries that reference ``~/.openclaw`` or ``openclaw`` prose.
    assert mod.rebrand_text("openclaw should always respond concisely") == "diting should always respond concisely"


def test_rebrand_text_replaces_legacy_bot_names():
    mod = load_module()
    # Same case-preservation rule as above.
    assert mod.rebrand_text("ClawdBot remembers my timezone") == "DiTing remembers my timezone"
    assert mod.rebrand_text("clawdbot prefers tabs") == "diting prefers tabs"
    assert mod.rebrand_text("MoltBot was configured for Spanish") == "DiTing was configured for Spanish"
    assert mod.rebrand_text("moltbot uses Python") == "diting uses Python"
...
def test_rebrand_text_handles_multiple_replacements():
    ...
    assert mod.rebrand_text(text) == "DiTing said to ask DiTing about DiTing settings"


def test_rebrand_text_preserves_filesystem_path_casing():
    """Lowercase matches — especially ``.openclaw`` filesystem paths — must
    rewrite to lowercase ``.diting`` (the real DiTing home), not the broken
    ``.diting``.
    ...
    rewritten to ``~/.diting/config.yaml`` — a path that doesn't exist —
    and the agent kept trying to read it.
    """
    mod = load_module()
    assert mod.rebrand_text("config is at ~/.openclaw/config.yaml") == \
        "config is at ~/.diting/config.yaml"
    assert mod.rebrand_text("use .openclaw directory") == "use .diting directory"
    assert mod.rebrand_text("Path.home() / '.openclaw'") == "Path.home() / '.diting'"
    # Sentence with both lowercase path and capitalized prose.
    assert mod.rebrand_text("openclaw config path: ~/.openclaw/") == \
        "diting config path: ~/.diting/"
...
def test_migrate_soul_rebrands_content(tmp_path):
    ...
    assert "OpenClaw" not in result
    assert "You are DiTing" in result
