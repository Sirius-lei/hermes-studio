...
class TestToolsetConsistency:
    ...
    def test_diting_platforms_share_core_tools(self):
        """All diting-* platform toolsets share the same core tools.

        Platform-specific additions (e.g. ``discord`` / ``discord_admin``
        on diting-discord, gated on DISCORD_BOT_TOKEN) are allowed on top —
        the invariant is that the core set is identical across platforms.
        """
        platforms = ["diting-cli", "diting-telegram", "diting-discord", "diting-whatsapp", "diting-slack", "diting-signal", "diting-homeassistant"]
...
class TestDefaultPlatformWebSearchCoverage:
    def test_diting_whatsapp_toolset_includes_web_search(self):
        assert "web_search" in resolve_toolset("diting-whatsapp")

    def test_diting_api_server_toolset_includes_web_search(self):
        assert "web_search" in resolve_toolset("diting-api-server")
