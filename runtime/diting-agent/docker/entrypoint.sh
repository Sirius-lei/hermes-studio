...
# by /etc/cont-init.d/01-diting-setup (installed by the Dockerfile). This
...
# on the pre-s6 contract "entrypoint.sh sets up state then execs diting"
...
echo "[diting] WARNING: docker/entrypoint.sh is a deprecated shim under " \
...
exec /opt/diting/docker/stage2-hook.sh "$@"
