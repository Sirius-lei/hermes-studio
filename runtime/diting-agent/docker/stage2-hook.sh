...
#
# Wired into the image as /etc/cont-init.d/01-diting-setup by the
...
DiTing_HOME="${DiTing_HOME:-/opt/data}"
INSTALL_DIR="/opt/diting"

# Drop to diting via s6-setuidgid, but skip it when already non-root.
as_diting() { [ "$(id -u)" = 0 ] || { "$@"; return; }; s6-setuidgid diting "$@"; }
...
# starts non-root. The baked install tree under /opt/diting is intentionally
# root-owned and non-writable; mutable runtime state must live under
# $DiTing_HOME. An arbitrary `--user` UID therefore cannot repair or populate
...
# default) and pass DiTing_UID/DiTing_GID — or the PUID/PGID aliases — which the
...
# `id -u diting` is the unremapped build UID (10000) because no root-only remap
# could run. root starts (id -u = 0) and the normal supervised drop to the
# diting UID are both unaffected.
cur_uid="$(id -u)"
if [ "$cur_uid" != 0 ] && [ "$cur_uid" != "$(id -u diting)" ]; then
    cat >&2 <<EOF
[stage2] ERROR: container started with --user $cur_uid (an arbitrary, non-diting UID).
...
and the baked /opt/diting install tree is intentionally root-owned and
...
Start the container as root (the default) and pass your host UID/GID instead:
    ...
    docker run -e DiTing_UID=\$(id -u) -e DiTing_GID=\$(id -g) ...

NAS users (Synology / unRAID / UGOS) can use the PUID/PGID aliases:

    docker run -e PUID=\$(id -u) -e PGID=\$(id -g) ...

The image remaps the diting user to that UID/GID at boot and chowns the data
...
# --- Bootstrap DiTing_HOME as root ---
# Create the directory (and any missing parents) while we still have root
# privileges so the chown checks below see real metadata and the later
# `s6-setuidgid diting mkdir -p` block doesn't EACCES on root-owned
# ancestors. Without this, custom DiTing_HOME paths whose parents only
# root can create (e.g. `DiTing_HOME=/home/diting/.diting` in a Compose
...
# --- UID/GID remap ---
# Accept PUID/PGID as aliases for DiTing_UID/DiTing_GID.  NAS users (UGOS,
...
# UID 10000 leaves the runtime unable to read the volume.  DiTing_UID/
# DiTing_GID still win when both are set.  See #15290, salvages #25872.
DiTing_UID="${DiTing_UID:-${PUID:-}}"
DiTing_GID="${DiTing_GID:-${PGID:-}}"

if [ -n "${DiTing_UID:-}" ] && validate_uid_gid "$DiTing_UID" && [ "$DiTing_UID" != "$(id -u diting)" ]; then
    echo "[stage2] Changing diting UID to $DiTing_UID"
    usermod -u "$DiTing_UID" diting
fi
if [ -n "${DiTing_GID:-}" ] && validate_uid_gid "$DiTing_GID" && [ "$DiTing_GID" != "$(id -g diting)" ]; then
    echo "[stage2] Changing diting GID to $DiTing_GID"
    # -o allows non-unique GID (e.g. macOS GID 20 "staff" may already
    # exist as "dialout" in the Debian-based container image).
    groupmod -o -g "$DiTing_GID" diting 2>/dev/null || true
fi
...
# host's `docker` group (or root). The supervised diting user (UID 10000)
...
# diting process with `Groups: 10000` (998 gone); after this hook adds
...
# matching entry that includes diting. Idempotent across container
...
#   - diting is already a member of the right group (idempotent restart)
...
for sock in /var/run/docker.sock /run/docker.sock; do
    ...
    if id -G diting 2>/dev/null | tr ' ' '\n' | grep -qx "$sock_gid"; then
        echo "[stage2] diting already in group $sock_gid for $sock"
        break
    fi
    ...
    if usermod -aG "$sock_group" diting 2>/dev/null; then
        echo "[stage2] Added diting to group $sock_group (GID $sock_gid) for $sock"
    else
        echo "[stage2] Warning: usermod -aG $sock_group diting failed; docker backend may fail with EACCES"
    fi
    break
done

# --- Fix ownership of data volume ---
# When DiTing_UID is remapped or the top-level $DiTing_HOME isn't owned by
# the runtime diting UID, restore ownership to diting — but ONLY for the
# directories diting actually writes to. The full $DiTing_HOME may be a
# host-mounted bind containing unrelated user files; `chown -R` would
# silently destroy host ownership of those (see issue #19788).
#
# The canonical list of diting-owned subdirs is the same one the s6-setuidgid
# mkdir -p block below seeds. Keep them in sync if the seed list changes.
actual_diting_uid=$(id -u diting)
...
chown_diting_tree() {
    ...
    chown -R diting:diting "$target" 2>/dev/null || \
        echo "[stage2] Warning: chown $target failed (rootless container?) — continuing"
}

needs_chown=false
if [ "$(stat -c %u "$DiTing_HOME" 2>/dev/null)" != "$actual_diting_uid" ]; then
    needs_chown=true
fi
if [ "$needs_chown" = true ]; then
    echo "[stage2] Fixing ownership of $DiTing_HOME (targeted) to diting ($actual_diting_uid)"
    # In rootless Podman the container's "root" is mapped to an
    # unprivileged host UID — chown will fail. That's fine: the volume
    # is already owned by the mapped user on the host side.
    #
    # Top-level $DiTing_HOME: chown the directory itself (not its contents)
    # so diting can mkdir new subdirs but bind-mounted host files keep
    # their existing ownership.
    if refuse_symlinked_path "chown" "$DiTing_HOME"; then
        :
    else
        chown diting:diting "$DiTing_HOME" 2>/dev/null || \
            echo "[stage2] Warning: chown $DiTing_HOME failed (rootless container?) — continuing"
    fi
    # DiTing-owned subdirs: recursive chown is safe here because these are
    # created and managed exclusively by diting (see the s6-setuidgid mkdir
    ...
    for sub in cron sessions logs hooks memories skills skins plans workspace home profiles pairing platforms/pairing lazy-packages; do
        if [ -e "$DiTing_HOME/$sub" ]; then
            chown_diting_tree "$DiTing_HOME/$sub"
        fi
    done
fi
...
# diting user. Hosted/container instances keep mutable state under
# $DiTing_HOME (/opt/data) and run with PYTHONDONTWRITEBYTECODE plus
# DiTing_DISABLE_LAZY_INSTALLS=1. Keeping /opt/diting root-owned and
...
# $DiTing_HOME/lazy-packages dir on the data volume (Dockerfile sets
# DiTing_LAZY_INSTALL_TARGET). That dir is appended to the END of sys.path,
...
# even though installs are re-enabled. The dir is seeded + chowned to diting
...
# Always reset ownership of $DiTing_HOME/profiles to diting on every
# boot. Profile dirs and files can land owned by root when commands
# are invoked via `docker exec <container> diting …` (which defaults
# to root unless `-u` is passed), and that breaks the cont-init
# reconciler (02-reconcile-profiles) which runs as diting and walks
# the profiles dir. Idempotent; skipped on rootless containers where
# chown would fail.
if [ -d "$DiTing_HOME/profiles" ]; then
    chown_diting_tree "$DiTing_HOME/profiles"
fi

# Always reset ownership of $DiTing_HOME/cron on every boot for the same
# docker-exec/root-write reason as profiles/. The cron scheduler state
# (jobs.json) must stay readable by the unprivileged diting runtime even
# after root-context maintenance commands or scheduler writes.
if [ -d "$DiTing_HOME/cron" ]; then
    chown_diting_tree "$DiTing_HOME/cron"
fi
...
# diting pairing approve …` defaults to uid=0 and writes 0600 root-owned
# approval files that the unprivileged diting gateway cannot read,
# silently leaving the approved user unauthorized (#10270). The targeted
# data-volume chown above only runs when the top-level $DiTing_HOME is
...
# is negligible.
if [ -d "$DiTing_HOME/platforms/pairing" ]; then
    chown_diting_tree "$DiTing_HOME/platforms/pairing"
fi
# Legacy location (pre-consolidated layout).
if [ -d "$DiTing_HOME/pairing" ]; then
    chown_diting_tree "$DiTing_HOME/pairing"
fi

# Reset ownership of diting-owned top-level state files on every boot.
# The targeted data-volume chown above only covers diting-owned
# *subdirectories*; loose state files living directly under $DiTing_HOME
# are missed. When those files are created or rewritten by
# `docker exec <container> diting …` (root unless `-u` is passed) they
# land root-owned, and the unprivileged diting runtime then hits
...
# sweep so host-owned files in a bind-mounted $DiTing_HOME are never
...
# entries of diting_cli.profile_distribution.USER_OWNED_EXCLUDE plus the
...
for f in \
    ...
    active_profile; do
    if [ -e "$DiTing_HOME/$f" ]; then
        if refuse_symlinked_path "chown" "$DiTing_HOME/$f"; then
            :
        else
            chown diting:diting "$DiTing_HOME/$f" 2>/dev/null || true
        fi
    fi
done

# --- config.yaml permissions ---
# Ensure config.yaml is readable by the diting runtime user even if it
# was edited on the host after initial ownership setup.
if [ -f "$DiTing_HOME/config.yaml" ]; then
    if refuse_symlinked_path "chown/chmod" "$DiTing_HOME/config.yaml"; then
        :
    else
        chown diting:diting "$DiTing_HOME/config.yaml" 2>/dev/null || true
        chmod 640 "$DiTing_HOME/config.yaml" 2>/dev/null || true
    fi
fi

# --- Seed directory structure as diting user ---
# Run as diting via s6-setuidgid so dirs end up owned correctly (matters
# under rootless Podman where chown back to root would fail).
#
# Use direct `mkdir -p` invocation (no `sh -c "..."` wrapper) so the
# shell isn't a second interpreter — defends against $DiTing_HOME values
# containing shell metacharacters. PR #30136 review item O2.
as_diting mkdir -p \
    "$DiTing_HOME/backups" \
    "$DiTing_HOME/cron" \
    "$DiTing_HOME/sessions" \
    "$DiTing_HOME/logs" \
    "$DiTing_HOME/logs/gateways" \
    "$DiTing_HOME/hooks" \
    "$DiTing_HOME/memories" \
    "$DiTing_HOME/skills" \
    "$DiTing_HOME/skins" \
    "$DiTing_HOME/plans" \
    "$DiTing_HOME/workspace" \
    "$DiTing_HOME/home" \
    "$DiTing_HOME/pairing" \
    "$DiTing_HOME/platforms/pairing" \
    "$DiTing_HOME/lazy-packages"

# --- Install-method stamp ---
# The 'docker' stamp is baked into the immutable install tree at
# /opt/diting/.install_method (see Dockerfile), NOT written here into
# $DiTing_HOME. detect_install_method() reads the code-scoped stamp first.
#
# Why we no longer stamp $DiTing_HOME: it is a shared DATA volume, commonly
# bind-mounted from the host (~/.diting:/opt/data) and sometimes shared with a
...
# 'diting update'. To heal homes already poisoned by older images, remove a
# stale 'docker' stamp from $DiTing_HOME if one is present (the host install's
# own installer re-creates its code-scoped stamp; a genuine container relies on
# the baked /opt/diting stamp, so deleting the data-dir copy is safe).
if [ -f "$DiTing_HOME/.install_method" ]; then
    stamped="$(tr -d '[:space:]' < "$DiTing_HOME/.install_method" 2>/dev/null || true)"
    if [ "$stamped" = "docker" ]; then
        rm -f "$DiTing_HOME/.install_method" 2>/dev/null || true
    fi
fi
...
seed_one() {
    dest=$1
    src=$2
    if [ ! -f "$DiTing_HOME/$dest" ] && [ -f "$INSTALL_DIR/$src" ]; then
        if refuse_symlinked_path "seed" "$DiTing_HOME/$dest"; then
            :
        else
            as_diting cp "$INSTALL_DIR/$src" "$DiTing_HOME/$dest"
        fi
    fi
}
...
if [ -f "$DiTing_HOME/.env" ]; then
    if refuse_symlinked_path "chown/chmod" "$DiTing_HOME/.env"; then
        :
    else
        chown diting:diting "$DiTing_HOME/.env" 2>/dev/null || true
        chmod 600 "$DiTing_HOME/.env" 2>/dev/null || true
    fi
fi

# --- Migrate persisted config schema ---
# Docker image upgrades replace the code under $INSTALL_DIR but preserve
# $DiTing_HOME on the mounted volume. Run the same safe, non-interactive
# config-schema migrations that `diting update` runs for non-Docker installs,
# after first-boot seeding and before supervised gateway services start.
# Set DiTing_SKIP_CONFIG_MIGRATION=1 for controlled/manual migrations.
if [ -f "$DiTing_HOME/config.yaml" ]; then
    s6-setuidgid diting "$INSTALL_DIR/.venv/bin/python" "$INSTALL_DIR/scripts/docker_config_migrate.py" \
...
if [ ! -f "$DiTing_HOME/auth.json" ] && [ -n "${DiTing_AUTH_JSON_BOOTSTRAP:-}" ]; then
    if refuse_symlinked_path "seed" "$DiTing_HOME/auth.json"; then
        :
    else
        printf '%s' "$DiTing_AUTH_JSON_BOOTSTRAP" > "$DiTing_HOME/auth.json"
        chown diting:diting "$DiTing_HOME/auth.json" 2>/dev/null || true
        chmod 600 "$DiTing_HOME/auth.json"
    fi
fi
...
# supply a freshly-issued session via DiTing_AUTH_JSON_REBOOTSTRAP (distinct
...
if [ -f "$DiTing_HOME/auth.json" ] && [ -n "${DiTing_AUTH_JSON_REBOOTSTRAP:-}" ]; then
    if refuse_symlinked_path "reseed" "$DiTing_HOME/auth.json"; then
        :
    else
        s6-setuidgid diting "$INSTALL_DIR/.venv/bin/python" \
            "$INSTALL_DIR/scripts/docker_rebootstrap_nous_session.py" \
            "$DiTing_HOME/auth.json" \
...
# can set DiTing_GATEWAY_BOOTSTRAP_STATE=running; we seed the state file
...
# DiTing_AUTH_JSON_BOOTSTRAP seeds auth.json. The [ ! -f ] guard is the
...
if [ ! -f "$DiTing_HOME/gateway_state.json" ] && \
        [ "${DiTing_GATEWAY_BOOTSTRAP_STATE:-}" = "running" ]; then
    if refuse_symlinked_path "seed" "$DiTing_HOME/gateway_state.json"; then
        :
    else
        printf '{"gateway_state":"running"}\n' > "$DiTing_HOME/gateway_state.json"
        chown diting:diting "$DiTing_HOME/gateway_state.json" 2>/dev/null || true
        chmod 644 "$DiTing_HOME/gateway_state.json"
    fi
fi
...
if [ -d "$INSTALL_DIR/skills" ]; then
    as_diting "$INSTALL_DIR/.venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" \
        || echo "[stage2] Warning: skills_sync.py failed; continuing"
fi

# --- Discover agent-browser's Chromium binary ---
# The image's Dockerfile runs `npx playwright install chromium`, which
# populates ``$PLAYWRIGHT_BROWSERS_PATH`` (=/opt/diting/.playwright) with
# a ``chromium_headless_shell-<build>/chrome-headless-shell-linux64/``
# directory. agent-browser (the runtime CLI DiTing spawns for the
...
# main-wrapper.sh propagates it into the supervised ``diting`` process
...
if [ -z "${AGENT_BROWSER_EXECUTABLE_PATH:-}" ] && \
    ...
    if [ -n "$browser_bin" ]; then
        ...
        # up for all supervised services (main-diting, dashboard, etc.).
