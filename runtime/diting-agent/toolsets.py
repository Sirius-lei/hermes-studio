...
_DiTing_CORE_TOOLS = [
    ...
    # read-only terminal tab (both gated on DiTing_DESKTOP via check_fn —
    ...
    # spawned as a kanban worker (DiTing_KANBAN_TASK env set) or the current
...
TOOLSETS = {
    ...
    "x_search": {
        "description": (
            ...
            "enable in `diting tools` → X (Twitter) Search."
        ),
        "tools": ["x_search"],
        "includes": []
    },
    ...
    "kanban": {
        "description": (
            ...
            "is spawned by the kanban dispatcher (DiTing_KANBAN_TASK env "
    ...
    # Coding posture (base DiTing — CLI/TUI/desktop/ACP). Auto-selected in a
    ...
    "coding": {
        ...
        # non-configurable-toolset recovery loop in diting_cli/tools_config.py).
        "posture": True,
    },
    
    # ==========================================================================
    # Full DiTing toolsets (CLI + messaging platforms)
    #
    ...
    # the `diting send` CLI), not by the model deciding to send on its own.
    # ==========================================================================
    ...
    "diting-cli": {
        ...
        "tools": _DiTing_CORE_TOOLS,
        "includes": []
    },

    "diting-cron": {
        # Mirrors diting-cli so cron's "default" toolset is the same set of
        # core tools users see interactively — then `diting tools` filters
        # them down per the platform config. _DEFAULT_OFF_TOOLSETS (moa,
        # homeassistant) are excluded by _get_platform_tools() unless
        # the user explicitly enables them.
        "description": "Default cron toolset - same core tools as diting-cli; gated by `diting tools`",
        "tools": _DiTing_CORE_TOOLS,
        "includes": []
    },
    ...
    "diting-email": {
        "description": "Email bot toolset - interact with DiTing via email (IMAP/SMTP)",
        "tools": _DiTing_CORE_TOOLS,
        "includes": []
    },
    ...
    "diting-sms": {
        "description": "SMS bot toolset - interact with DiTing via SMS (Twilio)",
        "tools": _DiTing_CORE_TOOLS,
        "includes": []
    },
    ...
    "diting-webhook": {
        ...
        "tools": _DiTing_WEBHOOK_SAFE_TOOLS,
        "includes": []
    },
    ...
    "diting-gateway": {
        ...
        "tools": [],
        "includes": ["diting-telegram", "diting-discord", "diting-whatsapp", "diting-slack", "diting-signal", "diting-bluebubbles", "diting-homeassistant", "diting-email", "diting-sms", "diting-mattermost", "diting-matrix", "diting-dingtalk", "diting-feishu", "diting-wecom", "diting-wecom-callback", "diting-weixin", "diting-qqbot", "diting-webhook", "diting-yuanbao"]
    }
}
...
def bundle_non_core_tools(toolset_name: str) -> Set[str]:
    """Return a ``diting-*`` bundle's platform-specific tools, excluding core.

    Platform bundles are defined as ``_DiTing_CORE_TOOLS + [platform extras]``.
    ...
    Bundle nesting is one level deep in practice (only ``diting-gateway``
...
def resolve_toolset(name: str, visited: Set[str] = None, *, include_registry: bool = True) -> List[str]:
    ...
    if not toolset:
        # Auto-generate a toolset for plugin platforms (diting-<name>).
        # Gives them _DiTing_CORE_TOOLS plus any tools the plugin registered
        ...
        if include_registry and name.startswith("diting-"):
            platform_name = name[len("diting-"):]
            try:
                from gateway.platform_registry import platform_registry
                if platform_registry.is_registered(platform_name):
                    plugin_tools = set(_DiTing_CORE_TOOLS)
                    try:
                        from tools.registry import registry
                        plugin_tools.update(
