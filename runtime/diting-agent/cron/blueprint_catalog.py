...
renders natively:
  ...
  * Docs catalog          -> a copy-paste command + a ``diting://`` deep-link
...
def blueprint_deeplink(blueprint: AutomationBlueprint, values: Optional[Dict[str, Any]] = None) -> str:
    """Build the ``diting://blueprint/<key>?slot=val`` deep-link URL."""
    ...
    return f"diting://blueprint/{quote(blueprint.key)}{qs}"
