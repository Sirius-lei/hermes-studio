...
``diting gateway restart`` (or ``launchctl kickstart ai.diting.gateway``
or ``systemctl restart diting-gateway``).  When the cron fires, the
...
``diting cron create`` CLI subcommand AND the agent's ``cronjob`` model
...
command identifier (``diting gateway``, ``launchctl ... diting-gateway``,
``systemctl ... diting-gateway``, ``pkill`` against the gateway) so it
...
blocks these commands at *execution* time when ``_DiTing_GATEWAY=1``, and
``diting gateway stop|restart`` refuse to self-target from inside the
...
_GATEWAY_LIFECYCLE_PATTERN = re.compile(
    r"(?i)"
    # Branch A: `diting gateway restart|stop` — the canonical foot-gun.
    # `start` is intentionally excluded: starting a gateway from inside a
    # gateway is benign (a no-op or "already running" error), and a
    # legitimate cron job might start a sibling profile's gateway.
    r"(?:diting\s+gateway\s+(?:restart|stop))"
    # Branch B: launchctl ops on a diting-gateway label. macOS launchd
    # labels look like `ai.diting.gateway` / `diting-gateway`. Requiring the
    # gateway identifier prevents blocking unrelated diting services (e.g.
    # `launchctl unload ai.diting.update-checker.plist`).
    r"|(?:launchctl\s+(?:kickstart|unload|load|stop|restart)\b[^\n]*\bditing[.\-]?gateway)"
    # Branch C: systemctl ops on a diting-gateway unit.
    r"|(?:systemctl\s+(?:-\S+\s+)*(?:restart|stop|start)\b[^\n]*\bditing[.\-]?gateway)"
    # Branch D: pkill / kill targeting the diting gateway process. Both
...
def _resolve_script_path(script_path: str) -> Path:
    ...
    under ``<DiTing_HOME>/scripts/`` and only accepts absolute paths as-is.
    ...
    (``~/.diting/scripts/restart.sh``) but is passed as the bare name
    ...
    if raw.is_absolute():
        return raw
    return get_diting_home() / "scripts" / raw
...
def check_gateway_lifecycle(
    ...
    if contains_gateway_lifecycle_command(combined):
        raise GatewayLifecycleBlocked(
            ...
            "(#30719). Run `diting gateway restart` from a shell outside "
