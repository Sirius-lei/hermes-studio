#!/usr/bin/env python3
"""Live test harness for DiTing Agent's Tool Search feature.
...
# Force-isolate the test environment BEFORE any diting imports.
ORIGINAL_HOME = os.environ.get("DiTing_HOME")
ORIGINAL_AUTH = Path.home() / ".diting" / "auth.json"
...
def setup_isolated_home(enabled: bool) -> Path:
    """Create a fresh ~/.diting/ for one test, copying minimal credentials.

    Also reads OPENROUTER_API_KEY from the user's real ``~/.diting/.env`` so
    the agent can authenticate against OpenRouter inside the isolated home.
    """
    home_dir = Path(tempfile.mkdtemp(prefix="diting_ts_live_"))
    diting_home = home_dir / ".diting"
    diting_home.mkdir(parents=True)

    if ORIGINAL_AUTH.exists():
        shutil.copy(ORIGINAL_AUTH, diting_home / "auth.json")

    # Copy .env so OPENROUTER_API_KEY (or others) are visible to the agent
    # running inside the isolated home.
    real_env_file = Path.home() / ".diting" / ".env"
    if real_env_file.exists():
        shutil.copy(real_env_file, diting_home / ".env")
        ...
        load_diting_dotenv(diting_home=str(Path.home() / ".diting"))
    ...
    (diting_home / "config.yaml").write_text(_yaml_dump(cfg), encoding="utf-8")
    return diting_home
...
def reset_module_state():
    """Drop cached modules so the new DiTing_HOME takes effect."""
    keys = [k for k in sys.modules.keys()
            if k.startswith(("tools.", "model_tools", "toolsets",
                             "diting_cli", "agent.", "run_agent"))]
    for k in keys:
        del sys.modules[k]
...
def run_one_scenario(scenario: Dict[str, Any], enabled: bool, out_dir: Path) -> Dict[str, Any]:
    ...
    os.environ["DiTing_HOME"] = str(home)
...
def main():
    ...
    # Restore original DiTing_HOME
    if ORIGINAL_HOME is not None:
        os.environ["DiTing_HOME"] = ORIGINAL_HOME
    else:
