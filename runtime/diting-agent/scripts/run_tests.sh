#!/usr/bin/env bash
# Canonical test runner for diting-agent. Run this instead of calling
...
#   * Proper venv activation (probes .venv, venv, then ~/.diting/...)
#
# Usage:
...
# ── Activate venv ───────────────────────────────────────────────────────────
VENV=""
for candidate in "$REPO_ROOT/.venv" "$REPO_ROOT/venv" "$HOME/.diting/diting-agent/venv"; do
...
if [ -f "$HOME/.diting/pytest_live_guard.py" ]; then
  EXTRA_PYTHONPATH="$HOME/.diting"
...
exec env -i \
  ...
  LC_ALL=C.UTF-8 \
  PYTHONHASHSEED=0 \
  ${DiTing_RUN_SLOW_PET_TESTS:+DiTing_RUN_SLOW_PET_TESTS="$DiTing_RUN_SLOW_PET_TESTS"} \
