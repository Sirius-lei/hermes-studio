# ============================================================================
# DiTing Agent Installer for Windows
# ============================================================================
# Installation script for Windows (PowerShell).
# Uses uv for fast Python provisioning and package management.
#
# Usage:
#   iex (irm https://diting-agent.nousresearch.com/install.ps1)
#
...
param(
    ...
    [string]$Commit = "",
    [string]$Tag = "",
    [string]$DiTingHome = $(if ($env:DiTing_HOME) { $env:DiTing_HOME } else { "$env:LOCALAPPDATA\diting" }),
    [string]$InstallDir = $(if ($env:DiTing_HOME) { "$env:DiTing_HOME\diting-agent" } else { "$env:LOCALAPPDATA\diting\diting-agent" }),
    ...
    # builds apps/desktop into a launchable DiTing.exe.
    #
    # Why opt-in:
    #   * DiTing-Setup.exe (the signed Tauri bootstrap installer) passes
    ...
    #     from inside an already-launched DiTing.exe; if THAT recursively
    #     built apps/desktop it would try to overwrite the live DiTing.exe
    ...
    #     `diting desktop` already builds on demand.
...
$RepoUrlSsh = "git@github.com:NousResearch/diting-agent.git"
$RepoUrlHttps = "https://github.com/NousResearch/diting-agent.git"
$PythonVersion = "3.11"
...
function Write-Banner {
    ...
    Write-Host "|             * DiTing Agent Installer                    |" -ForegroundColor Magenta
    Write-Host "+---------------------------------------------------------+" -ForegroundColor Magenta
...
function Write-BrowserEnv {
    ...
    if (-not (Test-Path $DiTingHome)) {
        New-Item -ItemType Directory -Force -Path $DiTingHome | Out-Null
    }
    $envFile = Join-Path $DiTingHome ".env"
...
function Install-AgentBrowser {
    ...
    $prefixDir = Join-Path $DiTingHome "node"
...
function Install-Uv {
    # DiTing owns its own uv at $DiTingHome\bin\uv.exe.  Always install there —
    # no PATH probing, no conda guards, no multi-location resolution chains.
    # The runtime update path (diting_cli/managed_uv.py) looks in the same
    # place, so install.ps1 and `diting update` stay in sync.
    $managedUv = Join-Path $DiTingHome "bin\uv.exe"
    ...
    Write-Info "Installing managed uv into $DiTingHome\bin ..."
    New-Item -ItemType Directory -Path (Join-Path $DiTingHome "bin") -Force | Out-Null

    # UV_INSTALL_DIR tells the astral installer to place the binary
    # directly into $DiTingHome\bin instead of ~/.local/bin.
    $prevEAP = $ErrorActionPreference
    try {
        $ErrorActionPreference = "Continue"
        $env:UV_INSTALL_DIR = Join-Path $DiTingHome "bin"
...
function Resolve-UvCmd {
    ...
    # sibling process and DiTing_HOME wasn't propagated).
...
function Resolve-AvailablePythonVersion {
    ...
    # ``$script:PythonVersion = $fallbackVer`` mutation.  Under DiTing-Setup.exe
...
function Install-Git {
    ...
    which DiTing uses to run shell commands.
    ...
    package manager):
      ...
      2. Download **PortableGit** from the official git-for-windows GitHub
         ...
         ``%LOCALAPPDATA%\diting\git`` -- never touches system Git, never
    ...
    **Why PortableGit, not MinGit:**  MinGit is the minimal-automation
    ...
    DiTing needs ``bash.exe`` to run shell commands.  PortableGit is the
    ...
    blocked).  Owning the DiTing copy of Git ourselves is predictable and
    recoverable: if it ever breaks, ``Remove-Item %LOCALAPPDATA%\diting\git``
    and re-running this installer fully recovers.

    After install we locate ``bash.exe`` and persist the path in
    ``DiTing_GIT_BASH_PATH`` (User scope) so DiTing can find it in a fresh
    ...
    # Download PortableGit into $DiTingHome\git.  Always works as long as
    # we can reach github.com -- no admin, no winget, no reliance on the
    # user's possibly-broken system Git install.
    Write-Info "Git not found -- downloading PortableGit to $DiTingHome\git\ ..."
    ...
    try {
        ...
        if ($arch -eq "32-bit-mingit") {
            Write-Warn "32-bit Windows detected -- PortableGit is 64-bit only.  Installing MinGit 32-bit as a last resort; bash-dependent DiTing features (terminal tool, agent-browser) will not work on this machine."
        ...
        Write-Info "then re-run this installer.  DiTing needs Git Bash on Windows to run"
...
function Set-GitBashEnvVar {
    ...
    ``DiTing_GIT_BASH_PATH`` (User env scope) so DiTing can find it even before
    ...
    #
    # Layouts:
    #   PortableGit (our default): $DiTingHome\git\bin\bash.exe
    #   MinGit (32-bit fallback):  $DiTingHome\git\usr\bin\bash.exe
    $candidates += "$DiTingHome\git\bin\bash.exe"       # PortableGit layout (primary)
    $candidates += "$DiTingHome\git\usr\bin\bash.exe"   # MinGit / PortableGit usr\bin fallback
    ...
    foreach ($candidate in $candidates) {
        if ($candidate -and (Test-Path $candidate)) {
            [Environment]::SetEnvironmentVariable("DiTing_GIT_BASH_PATH", $candidate, "User")
            $env:DiTing_GIT_BASH_PATH = $candidate
            Write-Info "Set DiTing_GIT_BASH_PATH=$candidate"
            return
        }
    }

    Write-Warn "Could not locate bash.exe -- DiTing may not find Git Bash."
    Write-Info "If needed, set DiTing_GIT_BASH_PATH manually to your bash.exe path."
}
...
function Test-Node {
    ...
    $managedNode = "$DiTingHome\node\node.exe"
    if ((Test-Path $managedNode) -and (Test-NodeVersionOk (& $managedNode --version))) {
        $version = & $managedNode --version
        $env:Path = "$DiTingHome\node;$env:Path"
        Write-Success "Node.js $version found (DiTing-managed)"
        $script:HasNode = $true
        return $true
    }

    Write-Info "Installing DiTing-managed Node.js $NodeVersion LTS..."
    ...
    # The portable zip path drops node.exe + npm into $DiTingHome\node\
    ...
    Write-Info "Downloading portable Node.js $NodeVersion to $DiTingHome\node\ ..."
    ...
    try {
        ...
        if ($zipName) {
            ...
            $tmpDir = "$env:TEMP\diting-node-extract"
            ...
            if ($extractedDir) {
                if (Test-Path "$DiTingHome\node") { Remove-Item -Recurse -Force "$DiTingHome\node" }
                Move-Item $extractedDir.FullName "$DiTingHome\node"
                ...
                $version = & "$DiTingHome\node\node.exe" --version
                Write-Success "Node.js $version installed to $DiTingHome\node\ (portable, user-scoped)"
                $script:HasNode = $true
...
function Install-SystemPackages {
    ...
    if ($hasWinget) {
        ...
        foreach ($pkg in $wingetPkgs) {
            $log = "$env:TEMP\diting-winget-$($pkg -replace '[^A-Za-z0-9]','_')-$(Get-Random).log"
            $pkgLogs[$pkg] = $log
...
function Install-Repository {
        ...
        if ($repoValid) {
            ...
            try {
                ...
                if (-not [string]::IsNullOrWhiteSpace(($statusOut -join "`n"))) {
                    ...
                    # the index conflict state is dropped. Mirrors the `diting
                    # update` path (#4735).
                    ...
                    $stashName = "diting-install-autostash-" + (Get-Date -Format "yyyyMMdd-HHmmss")
                ...
                if ($Commit) {
                    ...
                    # ff-only pull cannot succeed — mirror ``diting update`` and
                ...
                if ($autostashRef) {
                    ...
                    if ($restoreNow) {
                        ...
                        if ($LASTEXITCODE -eq 0) {
                            ...
                            Write-Warn "Review git diff / git status if DiTing behaves unexpectedly."
                        } else {
            ...
            try {
                ...
                Write-Info "terminals, running diting processes) and try again."
                throw
            }
        }
    }
    ...
    if (-not $didUpdate) {
        ...
        if (-not $cloneSuccess) {
            ...
            try {
                ...
                if ($Commit) {
                    $zipUrl = "https://github.com/NousResearch/diting-agent/archive/$Commit.zip"
                    $zipLabel = $Commit
                } elseif ($Tag) {
                    $zipUrl = "https://github.com/NousResearch/diting-agent/archive/refs/tags/$Tag.zip"
                    $zipLabel = $Tag
                } else {
                    $zipUrl = "https://github.com/NousResearch/diting-agent/archive/refs/heads/$Branch.zip"
                    $zipLabel = $Branch
                }
                $zipPath = "$env:TEMP\diting-agent-$zipLabel.zip"
                $extractPath = "$env:TEMP\diting-agent-extract"
    ...
    # next `diting update` checkout aborts on a "dirty" tree the user never
...
function Install-Venv {
    ...
    # Re-resolve the interpreter before creating the venv.  Under DiTing-Setup.exe
    ...
    if (Test-Path "venv") {
        ...
        # speedups.pyd) are loaded as DLLs by any running diting process.
        ...
        if ($env:OS -eq "Windows_NT") {
            $myPid = $PID
            Write-Info "Stopping any running diting processes before recreating venv..."
            ...
            try {
                schtasks /Query /FO CSV 2>$null | ConvertFrom-Csv | Where-Object { $_.TaskName -like '*DiTing_Gateway*' } | ForEach-Object {
                    $tn = $_.TaskName
            ...
            # The launcher CLI (diting.exe) plus its child tree.
            & taskkill /F /T /IM diting.exe /FI "PID ne $myPid" 2>$null | Out-Null
            # taskkill /IM diting.exe is NOT enough: the gateway/agent that a
            # scheduled task or watchdog autostarts runs as
            # `pythonw.exe -m diting_cli.main gateway run` straight out of
            # venv\Scripts\, so its image name is python/pythonw, not diting.exe.
            ...
            # user (see diting_cli/gateway_windows.py), so the installer always
    ...
    if (Test-Path $venvPythonExe) {
        ...
        # the task fires normally on next logon and `diting update` / the
...
function Install-Dependencies {
    ...
    if (Test-Path "uv.lock") {
        ...
        # empty and producing the broken state where `diting.exe` exists
...
"@ 2>$null
    ...
    if (-not $installed) {
        throw "Failed to install diting-agent package even with no extras. Inspect the uv pip install output above."
    }
    ...
    # isn't set, leaving venv\ empty and diting.exe broken with
    ...
    # here, not 30 seconds later when the user runs `diting`.
    if (-not $NoVenv) {
    ...
    if (-not $NoVenv) {
        # uv on Windows can register diting.exe in dist-info/RECORD but fail to
        ...
        # `diting` command while diting-agent.exe / diting-acp.exe exist
...
"@ 2>$null
            if ($LASTEXITCODE -eq 0 -and $scriptNames) {
                ...
                if ($missing.Count -gt 0) {
                    ...
                    if ($stillMissing.Count -gt 0) {
                        ...
                        Write-Info "Workaround: `"$pythonExe`" -m diting_cli.main <command>"
                    } else {
    ...
    # users hit and lazy-import errors from `diting dashboard` are confusing.
    ...
    if (Test-Path $pythonExe) {
        ...
        try {
            & $pythonExe -m py_compile "$InstallDir\diting_cli\web_server.py" 2>&1 | Out-Null
            if ($LASTEXITCODE -eq 0) { $webServerSyntaxOk = $true }
        } catch { }
        $ErrorActionPreference = $prevEAP
        if (-not $webOk) {
            Write-Warn "fastapi/uvicorn not importable -- `diting dashboard` will not work."
            Write-Info "Attempting targeted install of [web] extra as last resort..."
            & $UvCmd pip install -e ".[web]"
            if ($LASTEXITCODE -eq 0) {
                Write-Success "[web] extra installed; `diting dashboard` should now work."
            } else {
        ...
        if (-not $webServerSyntaxOk) {
            throw "dashboard backend source failed syntax check: diting_cli/web_server.py"
        }
    }
...
function Set-PathVariable {
    Write-Info "Setting up diting command..."
    
    if ($NoVenv) {
        $ditingBin = "$InstallDir"
    } else {
        $ditingBin = "$InstallDir\venv\Scripts"
    }
    
    # Add the venv Scripts dir to user PATH so diting is globally available
    # On Windows, the diting.exe in venv\Scripts\ has the venv Python baked in
    $currentPath = [Environment]::GetEnvironmentVariable("Path", "User")
    
    if ($currentPath -notlike "*$ditingBin*") {
        [Environment]::SetEnvironmentVariable(
            "Path",
            "$ditingBin;$currentPath",
            "User"
        )
        Write-Success "Added to user PATH: $ditingBin"
    } else {
        Write-Info "PATH already configured"
    }
    
    # Set DiTing_HOME so the Python code finds config/data in the right place.
    # Only needed on Windows where we install to %LOCALAPPDATA%\diting instead
    # of the Unix default ~/.diting
    $currentDiTingHome = [Environment]::GetEnvironmentVariable("DiTing_HOME", "User")
    if (-not $currentDiTingHome -or $currentDiTingHome -ne $DiTingHome) {
        [Environment]::SetEnvironmentVariable("DiTing_HOME", $DiTingHome, "User")
        Write-Success "Set DiTing_HOME=$DiTingHome"
    }
    $env:DiTing_HOME = $DiTingHome
    
    # Update current session
    $env:Path = "$ditingBin;$env:Path"
    
    Write-Success "diting command ready"
}

function Write-BootstrapMarker {
    # Writes $InstallDir\.diting-bootstrap-complete which tells the DiTing
    ...
    # DiTing-Setup.exe) or fall back to whatever git resolves in the
    # checkout. The desktop validates schemaVersion + pinnedCommit
    # length but doesn't enforce that HEAD matches the pin (users
    # update via `diting update` which moves HEAD legitimately).
    ...
    $markerPath = Join-Path $InstallDir ".diting-bootstrap-complete"
    $marker = [ordered]@{
        schemaVersion = 1
...
function Copy-ConfigTemplates {
    ...
    # Create the DiTing_HOME directory structure ($DiTingHome, default %LOCALAPPDATA%\diting)
    New-Item -ItemType Directory -Force -Path "$DiTingHome\cron" | Out-Null
    New-Item -ItemType Directory -Force -Path "$DiTingHome\sessions" | Out-Null
    New-Item -ItemType Directory -Force -Path "$DiTingHome\logs" | Out-Null
    New-Item -ItemType Directory -Force -Path "$DiTingHome\pairing" | Out-Null
    New-Item -ItemType Directory -Force -Path "$DiTingHome\hooks" | Out-Null
    New-Item -ItemType Directory -Force -Path "$DiTingHome\image_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$DiTingHome\audio_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$DiTingHome\memories" | Out-Null
    New-Item -ItemType Directory -Force -Path "$DiTingHome\skills" | Out-Null

    
    # Create .env
    $envPath = "$DiTingHome\.env"
    ...
    # Create config.yaml
    $configPath = "$DiTingHome\config.yaml"
    ...
    # (the default PS5 behaviour), and DiTing's prompt-injection scanner
    ...
    # PowerShell version.
    $soulPath = "$DiTingHome\SOUL.md"
    if (-not (Test-Path $soulPath)) {
        # MUST match DEFAULT_SOUL_MD in diting_cli/default_soul.py. The runtime
...
"@
    ...
    Write-Success "Configuration directory ready: $DiTingHome"
    
    # Seed bundled skills into $DiTingHome\skills (manifest-based, one-time per skill)
    Write-Info "Syncing bundled skills to $DiTingHome\skills ..."
    $pythonExe = "$InstallDir\venv\Scripts\python.exe"
    if (Test-Path $pythonExe) {
        try {
            & $pythonExe "$InstallDir\tools\skills_sync.py" 2>$null
            Write-Success "Skills synced to $DiTingHome\skills"
        } catch {
            # Fallback: simple directory copy
            $bundledSkills = "$InstallDir\skills"
            $userSkills = "$DiTingHome\skills"
            if ((Test-Path $bundledSkills) -and -not (Get-ChildItem $userSkills -Exclude '.bundled_manifest' -ErrorAction SilentlyContinue)) {
                Copy-Item -Path "$bundledSkills\*" -Destination $userSkills -Recurse -Force -ErrorAction SilentlyContinue
                Write-Success "Skills copied to $DiTingHome\skills"
            }
        }
    }
}
...
function Install-NodeDeps {
    if (-not $HasNode) {
        # Cross-process driver mode (DiTing-Setup.exe runs each -Stage NAME
    ...
    if (-not $npmCmd) {
        ...
        Write-Info "Open a new PowerShell window and re-run 'diting setup tools' later."
        return
    }
    $npmExe = $npmCmd.Source
    if ($npmExe -like "*.ps1") {
    ...
    if (Test-Path "$InstallDir\package.json") {
        ...
        $browserLog = "$env:TEMP\diting-npm-browser-$(Get-Random).log"
        ...
        if ($browserNpmOk) {
            ...
            if (-not $npxExe) {
                ...
                $pwLog = "$env:TEMP\diting-playwright-install-$(Get-Random).log"
                Push-Location $InstallDir
    ...
    if (Test-Path "$tuiDir\package.json") {
        ...
        $tuiLog = "$env:TEMP\diting-npm-tui-$(Get-Random).log"
...
# the final `electron` -> `DiTing` rename dies with ENOENT and every re-run
...
function Install-Desktop {
    # Build apps/desktop into a launchable DiTing.exe. Only called from
    ...
    #
    # The Tauri bootstrap installer's launch_diting_desktop command
    # resolves apps/desktop/release/win-unpacked/DiTing.exe directly,
    ...
    # enforces the build floor (^20.19 || >=22.12) and prepends the DiTing-managed
    ...
    # The --dir mode produces an unpacked DiTing.exe in
    ...
    # are NOT trying to work around). The DiTing icon + product name are
    # stamped onto DiTing.exe by our own rcedit step (Set-DesktopExeIdentity)
    ...
    $buildLog = "$env:TEMP\diting-desktop-build-$(Get-Random).log"
    ...
    $exeCandidates = @(
        "$desktopDir\release\win-unpacked\DiTing.exe",
        "$desktopDir\release\win-arm64-unpacked\DiTing.exe"
    )
    $found = $false
    $desktopExe = $null
    ...
    if (-not $found) {
        throw "Desktop build completed but no DiTing.exe was found under $desktopDir\release\*-unpacked\"
    }

    # 3b. The DiTing icon + identity are stamped onto DiTing.exe by the
    ...
    #    DiTing.exe. We deliberately do NOT point them at `diting desktop`: that
...
function New-DesktopShortcuts {
    ...
    try {
        ...
        $targets = @(
            (Join-Path ([Environment]::GetFolderPath('Programs')) 'DiTing.lnk'),
            (Join-Path ([Environment]::GetFolderPath('Desktop')) 'DiTing.lnk')
        )
        ...
        # with the DiTing icon, but without this the shortcut can keep drawing
...
function Install-PlatformSdks {
    ...
    # ~/.diting/.env are importable.  Two problems this solves:
    #
...
function Invoke-SetupWizard {
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Skipping setup wizard (non-interactive). Configure via the GUI or 'diting setup'."
        return
    }
    ...
    if (-not $NoVenv) {
        & ".\venv\Scripts\python.exe" -m diting_cli.main setup
    } else {
        python -m diting_cli.main setup
    }
...
function Start-GatewayIfConfigured {
    ...
    $ditingCmd = "$InstallDir\venv\Scripts\diting.exe"
    if (-not (Test-Path $ditingCmd)) {
        $ditingCmd = "diting"
    }

    # If WhatsApp is enabled but not yet paired, run foreground for QR scan
    $whatsappEnabled = $content | Where-Object { $_ -match "^WHATSAPP_ENABLED=true" }
    $whatsappSession = "$DiTingHome\whatsapp\session\creds.json"
    if ($whatsappEnabled -and -not (Test-Path $whatsappSession)) {
        Write-Host ""
        Write-Info "WhatsApp is enabled but not yet paired."
        Write-Info "Running 'diting whatsapp' to pair via QR code..."
        Write-Host ""
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Start the gateway later with: diting gateway"
        return
    }
    ...
    if ($response -eq "" -or $response -match "^[Yy]") {
        ...
        try {
            $logFile = "$DiTingHome\logs\gateway.log"
            Start-Process -FilePath $ditingCmd -ArgumentList "gateway" `
                -RedirectStandardOutput $logFile `
                -RedirectStandardError "$DiTingHome\logs\gateway-error.log" `
                -WindowStyle Hidden
            Write-Success "Gateway started! Your bot is now online."
            Write-Info "Logs: $logFile"
            Write-Info "To stop: close the gateway process from Task Manager"
        } catch {
            Write-Warn "Failed to start gateway. Run manually: diting gateway"
        }
    } else {
        Write-Info "Skipped. Start the gateway later with: diting gateway"
    }
}
...
function Write-Completion {
    ...
    Write-Host "$DiTingHome\config.yaml"
    Write-Host "   API Keys:  " -NoNewline -ForegroundColor Yellow
    Write-Host "$DiTingHome\.env"
    Write-Host "   Data:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$DiTingHome\cron\, sessions\, logs\"
    Write-Host "   Code:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$DiTingHome\diting-agent\"
    Write-Host ""
    ...
    Write-Host ""
    Write-Host "   diting              " -NoNewline -ForegroundColor Green
    Write-Host "Start chatting"
    Write-Host "   diting setup        " -NoNewline -ForegroundColor Green
    Write-Host "Configure API keys & settings"
    Write-Host "   diting config       " -NoNewline -ForegroundColor Green
    Write-Host "View/edit configuration"
    Write-Host "   diting config edit  " -NoNewline -ForegroundColor Green
    Write-Host "Open config in editor"
    Write-Host "   diting gateway      " -NoNewline -ForegroundColor Green
    Write-Host "Start messaging gateway (Telegram, Discord, etc.)"
    Write-Host "   diting update       " -NoNewline -ForegroundColor Green
...
$InstallStages = @(
    ...
    @{ Name = "repository";       Title = "Cloning DiTing repository";            Category = "install";      NeedsUserInput = $false; Worker = "Stage-Repository" }
...
if ($IncludeDesktop) {
    ...
    # (DiTing-Setup.exe), never via the irm|iex CLI one-liner.
    $InstallStages += @{ Name = "desktop"; Title = "Building desktop app"; Category = "install"; NeedsUserInput = $false; Worker = "Stage-Desktop" }
}
$InstallStages += @(
    @{ Name = "path";             Title = "Adding DiTing to PATH";                Category = "finalize";     NeedsUserInput = $false; Worker = "Stage-Path" }
...
try {
    ...
    Write-Host "  Invoke-WebRequest -Uri 'https://diting-agent.nousresearch.com/install.ps1' -OutFile install.ps1" -ForegroundColor Yellow
