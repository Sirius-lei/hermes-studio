...
row taxonomy is inferred from the atlas shape so DiTing can render both legacy
...
# rows (1536x1872), while older DiTing/petdex sheets used 9 columns x 8 rows
...
class PetState(str, Enum):
    ...
    These are DiTing' activity state names. They are not always identical to the
...
# Legacy DiTing/petdex row order (top -> bottom) used by the older 8-row,
...
# Canonical DiTing activity names -> accepted row-name aliases in descending
