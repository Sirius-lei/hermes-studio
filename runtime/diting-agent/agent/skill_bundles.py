...
Storage
-------
Bundles live in ``~/.diting/skill-bundles/*.yaml`` (and the equivalent
profile-aware directory under ``DiTing_HOME``). Each file looks like::
...
- :func:`list_bundles` — return rich info for display (``diting bundles``)
...
def _bundles_dir() -> Path:
    """Return the canonical bundles directory under DiTing_HOME.

    Honors ``DiTing_BUNDLES_DIR`` for tests; falls back to
    ``<DiTing_HOME>/skill-bundles``.
    """
    override = os.environ.get("DiTing_BUNDLES_DIR")
    if override:
        return Path(override).expanduser()
    return get_diting_home() / "skill-bundles"
...
# ---------------------------------------------------------------------------
# File-level CRUD helpers — used by `diting bundles` CLI subcommand.
# ---------------------------------------------------------------------------
