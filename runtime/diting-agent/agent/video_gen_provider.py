...
as ``kind: backend``) or ``~/.diting/plugins/video_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
class VideoGenProvider(abc.ABC):
    ...
    def capabilities(self) -> Dict[str, Any]:
        ...
        Used by the tool layer for soft validation and by ``diting tools``
...
def _videos_cache_dir() -> Path:
    """Return ``$DiTing_HOME/cache/videos/``, creating parents as needed."""
    from diting_constants import get_diting_home

    path = get_diting_home() / "cache" / "videos"
...
def save_b64_video(
    ...
    *,
    ...
    """Decode base64 video data and write under ``$DiTing_HOME/cache/videos/``.
...
def save_url_video(
    ...
    *,
    ...
    """Download a video URL and write it under ``$DiTing_HOME/cache/videos/``.
