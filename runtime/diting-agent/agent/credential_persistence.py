...
``auth.json``.  They intentionally have no dependency on ``diting_cli.auth`` so
...
# Sources DiTing owns and can intentionally persist in auth.json.  Everything
...
def sanitize_borrowed_credential_payload(
    ...
    Owned sources (manual entries and DiTing-owned OAuth/device-code state)
