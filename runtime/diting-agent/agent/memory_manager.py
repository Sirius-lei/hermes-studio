...
class MemoryManager:
    ...
    def add_provider(self, provider: MemoryProvider) -> None:
        ...
        _core_tool_names = set(_DiTing_CORE_TOOLS)
    ...
    def _strip_skill_scaffolding(text: str) -> Optional[str]:
        ...
        When a user invokes a /skill or /bundle, DiTing expands the turn into
    ...
    def initialize_all(self, session_id: str, **kwargs) -> None:
        ...
        Automatically injects ``diting_home`` into *kwargs* so that every
        provider can resolve profile-scoped storage paths without importing
        ``get_diting_home()`` themselves.
        """
        if "diting_home" not in kwargs:
            from diting_constants import get_diting_home
            kwargs["diting_home"] = str(get_diting_home())
        for provider in self._providers:
            try:
