...
upstream prefix cache warm.  See ``diting-agent-dev``'s
...
def _tui_embedded_pane_clarifier(hint: str) -> str:
    ...
    Triggered by ``DiTing_DESKTOP_TERMINAL=1`` (set by ``main.cjs`` only on the
    ...
    if _TUI_EMBEDDED_PANE_CLARIFIER in hint:
        return hint
    if not is_truthy_value(os.getenv("DiTing_DESKTOP_TERMINAL")):
        return hint
...
def build_system_prompt_parts(agent: Any, system_message: Optional[str] = None) -> Dict[str, str]:
    ...
    AIAgent.  DiTing never re-renders parts of this string mid-
    ...
    # Some execution modes (cron) still want DiTing_HOME persona while keeping
    ...
    # Pointer to the diting-agent skill + docs for user questions about DiTing itself.
    stable_parts.append(DiTing_AGENT_HELP_GUIDANCE)
    ...
    # DiTing_KANBAN_TASK env var). Normal chat sessions never see
    ...
    # Coding posture (base DiTing, any interactive coding surface in a code
    ...
    # Active-profile hint — names the DiTing profile the agent is running
    # under so it doesn't conflate ~/.diting/skills/ (default profile) with
    # ~/.diting/profiles/<active>/skills/ (this profile's). Deterministic
    ...
    if active_profile == "default":
        stable_parts.append(
            "Active DiTing profile: default. Other profiles (if any) live "
            "under ~/.diting/profiles/<name>/. Each profile has its own "
        ...
        stable_parts.append(
            f"Active DiTing profile: {active_profile}. This session reads "
            f"and writes ~/.diting/profiles/{active_profile}/. The default "
            f"profile's data lives at ~/.diting/skills/, ~/.diting/plugins/, "
            f"~/.diting/cron/, ~/.diting/memories/ — those belong to a "
    ...
    if platform_key == "telegram" and _default_hint:
        try:
            from diting_cli.config import load_config_readonly
...
def build_system_prompt(agent: Any, system_message: Optional[str] = None) -> str:
    ...
    one cached block — DiTing never rebuilds or reinjects parts of it
