...
def openclaw_residue_hint_cli() -> str:
    """Banner shown the first time DiTing starts and finds ``~/.openclaw/``.

    Points users at ``diting claw migrate`` (non-destructive port of config,
    memory, and skills) first. ``diting claw cleanup`` is mentioned as the
    ...
    return (
        ...
        "To port your config, memory, and skills over to DiTing, run "
        "`diting claw migrate`.\n"
        "If you've already migrated and want to archive the old directory, "
        "run `diting claw cleanup` (renames it to ~/.openclaw.pre-migration — "
