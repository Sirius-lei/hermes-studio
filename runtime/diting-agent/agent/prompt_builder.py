...
from diting_constants import get_diting_home, get_skills_dir, is_wsl
...
_DiTing_MD_NAMES = (".diting.md", "DiTing.md")


def _find_diting_md(cwd: Path) -> Optional[Path]:
    """Discover the nearest ``.diting.md`` or ``DiTing.md``.
    ...
    # could pick up a .diting.md planted in /tmp, /home, etc.
    search_dirs = [current, *current.parents] if stop_at else [current]

    for directory in search_dirs:
        for name in _DiTing_MD_NAMES:
...
DEFAULT_AGENT_IDENTITY = (
    "You are DiTing Agent, an intelligent AI assistant created by Nous Research. "
...
DiTing_AGENT_HELP_GUIDANCE = (
    "You run on DiTing Agent (by Nous Research). When the user needs help with "
    "DiTing itself — configuring, setting up, using, extending, or troubleshooting "
    "it — or when you need to understand your own features, tools, or capabilities, "
    "the documentation at https://diting-agent.nousresearch.com/docs is your "
    "authoritative reference and always holds the latest, most up-to-date "
    "information. Load the `diting-agent` skill with skill_view(name='diting-agent') "
...
KANBAN_GUIDANCE = (
    ...
    "the shared board at `~/.diting/kanban.db`. Your task id is in "
    "`$DiTing_KANBAN_TASK`; your workspace is `$DiTing_KANBAN_WORKSPACE`. "
    ...
    "ground truth.\n"
    "2. **Work inside the workspace.** `cd $DiTing_KANBAN_WORKSPACE` before "
    ...
    "\n"
    "- **Workspace.** `cd $DiTing_KANBAN_WORKSPACE` first. For a `worktree` kind "
    "with no `.git`, `git worktree add <path> "
    "${DiTing_KANBAN_BRANCH:-wt/$DiTing_KANBAN_TASK}` from the main repo, then "
    "cd there. For a project-linked task the workspace is a fresh "
    "`<repo>/.worktrees/<task-id>` and `$DiTing_KANBAN_BRANCH` a deterministic "
    ...
    "every assignee in a real profile (`diting profile list`, or ask the user), "
    "and express dependencies via `parents=[...]` on `kanban_create`, not prose.\n"
    "\n"
    "## Do NOT\n"
    "\n"
    "- Do not shell out to `diting kanban <verb>` for board operations. Use "
...
#
# The diting-agent runtime already executes a batch of tool calls
...
# from Cline's TypeScript tool-surface guidance to diting-agent's Python
...
def computer_use_guidance(platform_name: Optional[str] = None) -> str:
    ...
    return (
        ...
        "run `diting computer-use doctor` and share the output. That command "
...
STEER_CHANNEL_NOTE = (
    ...
    "While you work, the user can send an out-of-band message that DiTing "
...
PLATFORM_HINTS = {
    ...
    "tui": (
        "You are running in the DiTing terminal UI (TUI). "
        "Cron jobs scheduled from this session are LOCAL-ONLY: their output is "
        "saved (viewable via cronjob action='list') but is NOT delivered back "
    ...
    "desktop": (
        "You are chatting inside the DiTing desktop app — a graphical chat "
    ...
    "webui": (
        "You are in the DiTing WebUI, a browser-based chat interface. "
...
# container / remote host rather than on the machine where DiTing itself
...
def _probe_remote_backend(env_type: str) -> str | None:
    ...
    operate on a different machine than the host DiTing runs on.
    """
...
def build_environment_hints() -> str:
    ...
    if not is_remote_backend:
        ...
        if probe:
            hints.append(
                ...
                f"where DiTing itself is running. The host OS, home, and cwd "
                f"of the DiTing process are irrelevant; only the following "
            ...
            hints.append(
                f"Terminal backend: {backend}. Your `terminal`, `read_file`, "
                f"`write_file`, `patch`, and `search_files` tools all operate "
                f"inside {description} — NOT on the machine where DiTing "
    ...
    # Embedder-supplied environment description. Lets a host that wraps DiTing
    ...
    extra = (os.getenv("DiTing_ENVIRONMENT_HINT") or "").strip()
    if not extra:
        try:
...
def _skills_prompt_snapshot_path() -> Path:
    return get_diting_home() / ".skills_prompt_snapshot.json"
...
def _current_session_platform_hint() -> str:
    ...
    platform = os.environ.get("DiTing_PLATFORM") or os.environ.get("DiTing_SESSION_PLATFORM")
    if platform:
        return platform
...
def build_skills_system_prompt(
    ...
    scanned alongside the local ``~/.diting/skills/`` directory.  External dirs
    ...
    if not skills_by_category:
        ...
        result = (
            ...
            "or troubleshoot DiTing Agent itself — its CLI, config, models, providers, tools, "
            "skills, voice, gateway, plugins, or any feature — load the `diting-agent` skill "
            "first. It has the actual commands (e.g. `diting config set …`, `diting tools`, "
            "`diting setup`) so you don't have to guess or invent workarounds.\n"
...
def build_nous_subscription_prompt(valid_tool_names: "set[str] | None" = None) -> str:
    ...
    lines.extend(
        [
            ...
            "Useful commands: diting setup, diting setup tools, diting setup terminal, diting status.",
        ]
    )
    return "\n".join(lines)
...
def load_soul_md(context_length: Optional[int] = None) -> Optional[str]:
    """Load SOUL.md from DiTing_HOME and return its content, or None.
    ...
    """
    try:
        from diting_cli.config import ensure_diting_home
        ensure_diting_home()
    except Exception as e:
        logger.debug("Could not ensure DiTing_HOME before loading SOUL.md: %s", e)

    soul_path = get_diting_home() / "SOUL.md"
...
def _load_diting_md(cwd_path: Path, context_length: Optional[int] = None) -> str:
    """.diting.md / DiTing.md — walk to git root."""
    diting_md_path = _find_diting_md(cwd_path)
    if not diting_md_path:
        return ""
    try:
        content = diting_md_path.read_text(encoding="utf-8").strip()
        if not content:
            return ""
        content = _strip_yaml_frontmatter(content)
        rel = diting_md_path.name
        try:
            rel = str(diting_md_path.relative_to(cwd_path))
        except ValueError:
            pass
        content = _scan_context_content(content, rel)
        result = f"## {rel}\n\n{content}"
        return _truncate_content(
            result, ".diting.md", context_length=context_length,
            read_path=str(diting_md_path),
        )
    except Exception as e:
        logger.debug("Could not read %s: %s", diting_md_path, e)
        return ""
...
def build_context_files_prompt(
    ...
    Priority (first found wins — only ONE project context type is loaded):
      1. .diting.md / DiTing.md  (walk to git root)
      2. AGENTS.md / agents.md   (cwd only)
      3. CLAUDE.md / claude.md   (cwd only)
      4. .cursorrules / .cursor/rules/*.mdc  (cwd only)

    SOUL.md from DiTing_HOME is independent and always included when present.
    ...
    project_context = (
        _load_diting_md(cwd_path, context_length)
    ...
    # SOUL.md from DiTing_HOME only — skip when already loaded as identity
    if not skip_soul:
