...
Plugins built on DiTing Agent often need to make their own LLM calls
...
Today the only stable plugin surfaces extend an existing DiTing
...
:class:`~diting_cli.plugins.PluginContext`:
...
embedded slugs, no shorthands. This mirrors DiTing' main config
...
handles every provider, fallback chain, and per-task override DiTing
supports.
"""
...
class PluginLlm:
    ...
    Instances are constructed by :class:`diting_cli.plugins.PluginContext`
