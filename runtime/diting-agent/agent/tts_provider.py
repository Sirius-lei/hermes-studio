...
2. **Command-type providers** declared under ``tts.providers.<name>:
   ...
   CLI into DiTing with shell-template placeholders. **Wins over a
...
shipped today) or ``~/.diting/plugins/tts/<name>/`` (user-installed).
...
``{success: False, error: …}`` JSON envelope the rest of DiTing
expects.
"""
...
class TTSProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``diting tools``.
    ...
    def is_available(self) -> bool:
        ...
        Must NOT raise — used by the picker and ``diting setup`` for
