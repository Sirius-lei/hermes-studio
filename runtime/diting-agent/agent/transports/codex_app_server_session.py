...
Owns one Codex thread per DiTing session. Drives `turn/start`, consumes
...
# DiTing' tools.terminal.security_mode → Codex's permissions profile id.
# Defaults if config is missing → workspace-write (matches Codex's own default).
_DiTing_TO_CODEX_PERMISSION_PROFILE = {
...
def _coerce_turn_input_text(user_input: Any) -> str:
    """Collapse DiTing/OpenAI rich content into app-server text input.
...
class CodexAppServerSession:
    """One Codex thread per DiTing session, lifetime owned by AIAgent.
    ...
    def __init__(
        ...
        *,
        ...
        self._permission_profile = (
            permission_profile or _DiTing_TO_CODEX_PERMISSION_PROFILE.get(
                os.environ.get("DiTing_TERMINAL_SECURITY_MODE", "auto"),
                "workspace-write",
            )
        )
    ...
    def ensure_started(self) -> str:
        ...
        self._client.initialize(
            client_name="diting",
            client_title="DiTing Agent",
            client_version=_get_diting_version(),
        )
    ...
    def run_turn(
        ...
        *,
        ...
        # supports rich content but DiTing' text path is the common case).
        try:
    ...
    def _handle_server_request(self, req: dict) -> None:
        """Translate a codex server request (approval) into DiTing' approval
        ...
        if method == "item/commandExecution/requestApproval":
            ...
            # OAuth, form data). For our own diting-tools callback we
            # auto-accept — the user already approved DiTing' tools
...
def _approval_choice_to_codex_decision(choice: str) -> str:
    """Map DiTing approval choices onto codex's CommandExecutionApprovalDecision
    / FileChangeApprovalDecision wire values.

    DiTing returns 'once', 'session', 'always', or 'deny'.
...
def _get_diting_version() -> str:
    """Best-effort DiTing version string for codex's userAgent line."""
    try:
