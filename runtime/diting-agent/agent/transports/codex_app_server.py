...
projection into DiTing' display, approval bridging, transcript projection into
...
"codex_app_server"`. DiTing' default tool dispatch is unchanged when this
...
class CodexAppServerClient:
    ...
    def __init__(
        ...
        # `os.environ.copy()` also handed it every Tier-1 DiTing secret — gateway
        ...
        spawn_env = diting_subprocess_env(inherit_credentials=True)
        if env:
        ...
        if spawn_env.get("DiTing_KANBAN_TASK"):
            kanban_db = spawn_env.get("DiTing_KANBAN_DB")
            kanban_root = (
                ...
                if kanban_db
                    ...
                    "DiTing_KANBAN_ROOT",
                    os.path.join(
                        spawn_env.get("DiTing_HOME", os.path.expanduser("~/.diting")),
                        "kanban",
                    ),
                )
            )
    ...
    def initialize(
        self,
        client_name: str = "diting",
        client_title: str = "DiTing Agent",
