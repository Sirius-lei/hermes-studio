...
cache problem must never block DiTing startup.
"""
...
def resolve_cache_home(home_path: Optional[Path] = None) -> Path:
    """Resolve the DiTing home used for cache paths.

    ``home_path`` is whatever ``load_diting_dotenv()`` already resolved;
    falling back to ``$DiTing_HOME`` / ``~/.diting`` keeps direct callers
    (and tests that don't thread a home through) working.
    """
    if home_path is None:
        home_path = Path(os.getenv("DiTing_HOME", Path.home() / ".diting"))
    return home_path
...
class DiskCache(Generic[K]):
    ...
    One JSON object per backend lives at ``<diting_home>/cache/<basename>``::
    ...
    def write(
        ...
        try:
            ...
            # metadata isn't exposed if DiTing_HOME is ever made traversable.
            try:
