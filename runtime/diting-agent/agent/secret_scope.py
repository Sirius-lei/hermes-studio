...
- ``set_secret_scope(mapping)`` installs the active profile's secrets for the
  ...
  via ``copy_context()`` exactly like the DiTing_HOME override).
...
_GLOBAL_ENV_EXACT = frozenset({
    # DiTing runtime / deployment
    "DiTing_HOME", "DiTing_PROFILE", "DiTing_GATEWAY_LOCK_DIR",
    "DiTing_MAX_ITERATIONS", "DiTing_MAX_TOKENS", "DiTing_API_TIMEOUT",
    "DiTing_REDACT_SECRETS", "DiTing_NOUS_TIMEOUT_SECONDS",
    "_DiTing_GATEWAY",
    # OS / interpreter
    "PATH", "HOME", "USER", "LANG", "LC_ALL", "TZ", "PWD", "SHELL", "TMPDIR",
    "VIRTUAL_ENV", "PYTHONPATH", "SSL_CERT_FILE",
    # Kanban paths (per-board, not per-profile-secret)
    "DiTing_KANBAN_DB", "DiTing_KANBAN_WORKSPACES_ROOT", "DiTing_KANBAN_BOARD",
})
_GLOBAL_ENV_PREFIXES = (
    "DiTing_KANBAN_",
    "DiTing_TELEGRAM_",   # tuning knobs (batch delays, fallback toggles) — NOT the token
...
def build_profile_secret_scope(diting_home: Path) -> Dict[str, str]:
    ...
    """
    return load_env_file(Path(diting_home) / ".env")
