...
Every DiTing request that hits the Nous Portal — main agent loop, auxiliary
client (compression / titles / vision / web_extract / session_search / etc.),
and any future code path — must carry the same product-attribution tags so
Nous can attribute usage to DiTing Agent and bucket it by client release.

Tag shape (sent in OpenAI-compatible ``extra_body['tags']``):

    [
        "product=diting-agent",
        "client=diting-client-v<__version__>",
    ]

The version is sourced live from ``diting_cli.__version__`` so it auto-aligns
...
``diting_cli.__version__`` is the canonical source of truth.
"""
...
def _diting_version() -> str:
    """Return the current DiTing release version, e.g. ``"0.13.0"``.

    Falls back to ``"unknown"`` if ``diting_cli`` cannot be imported (should
...
def diting_client_tag() -> str:
    """Return the ``client=...`` tag for Nous Portal requests.

    Format: ``client=diting-client-v<MAJOR>.<MINOR>.<PATCH>``.
    """
    return f"client=diting-client-v{_diting_version()}"
...
def nous_portal_tags() -> List[str]:
    ...
    """
    return ["product=diting-agent", diting_client_tag()]
