...
Optional hooks (override to opt in):
  ...
  backup_paths() -> list[str]            — extra on-disk paths to include in `diting backup`
"""
...
class MemoryProvider(ABC):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs always include:
          - diting_home (str): The active DiTing_HOME directory path. Use this
            for profile-scoped storage instead of hardcoding ``~/.diting``.
        ...
        kwargs may also include:
          ...
          - agent_workspace (str): Shared workspace name (e.g. "diting").
    ...
    def get_config_schema(self) -> List[Dict[str, Any]]:
        ...
        Used by 'diting memory setup' to walk the user through configuration.
    ...
    def save_config(self, values: Dict[str, Any], diting_home: str) -> None:
        """Write non-secret config to the provider's native location.

        Called by 'diting memory setup' after collecting user inputs.
        ``values`` contains only non-secret fields (secrets go to .env).
        ``diting_home`` is the active DiTing_HOME directory path.
    ...
    def backup_paths(self) -> List[str]:
        """Return extra on-disk paths this provider stores OUTSIDE DiTing_HOME.

        ``diting backup`` only walks DiTing_HOME, so any provider state kept
        ...
        of the archive, and ``diting import`` restores them to their original
