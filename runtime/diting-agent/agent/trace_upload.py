"""Upload a DiTing session transcript to Hugging Face as an agent trace.

DiTing stores sessions in its own SQLite store (``diting_state.SessionDB``),
...
* **Zero LLM turn.** This is a deterministic export — it never spends a
  model call. The ``diting trace upload`` subcommand calls
...
* **Private by default.** Traces can contain prompts, tool output, local
  ...
  is passed through DiTing' secret redactor (``force=True``) unless the
...
DEFAULT_DATASET_NAME = "diting-traces"
_DiTing_VERSION = "diting-agent"
...
# ---------------------------------------------------------------------------
# Conversion: DiTing OpenAI-format messages -> Claude Code JSONL
# ---------------------------------------------------------------------------
...
def _redact(text: Any, enabled: bool) -> Any:
    ...
    Non-strings pass through untouched. Uses DiTing' shared redactor with
...
def build_trace_jsonl(
    ...
    *,
    ...
    """Render DiTing conversation messages as Claude Code JSONL text.
...
_NO_TOKEN_MESSAGE = (
    ...
    "2. Add it to your environment as HF_TOKEN (e.g. in ~/.diting/.env):\n"
    "     HF_TOKEN=hf_xxxxxxxxxxxxxxxxxxxx\n"
    "3. Run /upload-trace again (or `diting trace upload`)."
)
...
def upload_session_trace(
    ...
    *,
    ...
    the user's private ``{user}/diting-traces`` dataset. Returns a
