...
def _diting_home_path() -> Path:
    """Resolve the active DiTing_HOME (profile-aware) without circular imports."""
    try:
        from diting_constants import get_diting_home  # local import to avoid cycles
        return get_diting_home()
    except Exception:
        return Path(os.path.expanduser("~/.diting"))


def _diting_root_path() -> Path:
    """Resolve the DiTing root dir (always the parent of any profile, never per-profile)."""
    try:
        from diting_constants import get_default_diting_root  # local import to avoid cycles
...
def build_write_denied_paths(home: str) -> set[str]:
    ...
    diting_home = _diting_home_path()
    diting_root = _diting_root_path()
    return {
        os.path.realpath(p)
        for p in [
            ...
            str(diting_home / ".anthropic_oauth.json"),
            # Top-level Anthropic PKCE credential store remains sensitive even
            # when a profile is active; default/non-profile sessions still read it.
            str(diting_root / ".anthropic_oauth.json"),
...
def get_safe_write_roots() -> set[str]:
    """Return resolved DiTing_WRITE_SAFE_ROOT paths. Supports multiple directories
    separated by ``os.pathsep`` (``:`` on Unix, ``;`` on Windows).
    E.g., ``/opt/data:/var/www/html`` on Unix, ``C:\\data;D:\\www`` on Windows."""
    env = os.getenv("DiTing_WRITE_SAFE_ROOT", "")
    if not env:
        return set()
    roots: set[str] = set()
...
def _classify_write_denial(path: str) -> Optional[str]:
    ...
    diting_dirs = []
    for base in (_diting_home_path(), _diting_root_path()):
        try:
            real = os.path.realpath(base)
            if real not in diting_dirs:
                diting_dirs.append(real)
        except Exception:
            continue

    for base_real in diting_dirs:
        try:
...
def get_write_denied_error(path: str, *, verb: str = "Write") -> Optional[str]:
    ...
    if denial == "safe_root":
        ...
        return (
            f"{verb} denied: '{path}' is outside DiTing_WRITE_SAFE_ROOT "
...
def get_read_block_error(path: str) -> Optional[str]:
    """Return an error message when a read targets a denied DiTing path.

    Three categories are blocked:

      * Internal DiTing cache files under ``DiTing_HOME/skills/.hub`` —
        readable metadata that an attacker could use as a prompt-injection
        carrier.
      * Credential / secret stores under DiTing_HOME and the global DiTing
    ...
    **This is NOT a security boundary.** The terminal tool runs as the
    ...
    or ``cat ~/.diting/.env`` and exfiltrate the file. The read-deny exists
    ...
    # Resolve BOTH the active DiTing_HOME (profile-aware) AND the global
    # DiTing root so credential stores at <root>/auth.json etc. are also
    # blocked when running under a profile (DiTing_HOME points at
    # <root>/profiles/<name> in profile mode). Same shape as the write
    # deny widening (#15981, #14157).
    diting_dirs: list[Path] = []
    for base in (_diting_home_path(), _diting_root_path()):
        try:
            real = base.resolve()
            if real not in diting_dirs:
    ...
    for hd in diting_dirs:
        ...
        for blocked in blocked_dirs:
            try:
                ...
                continue
            return (
                f"Access denied: {path} is an internal DiTing cache file "
    ...
    for hd in diting_dirs:
        for name in credential_file_names:
            ...
            if resolved == blocked:
                return (
                    f"Access denied: {path} is a DiTing credential store "
    ...
    for hd in diting_dirs:
        ...
        if resolved == mcp_tokens:
            return (
                f"Access denied: {path} is the DiTing MCP token directory "
        ...
        try:
            ...
            continue
        return (
            f"Access denied: {path} is a DiTing MCP token file "
            "and cannot be read directly. (Defense-in-depth — not a "
            "security boundary; the terminal tool can still bypass.)"
        )
...
def raise_if_read_blocked(path: str) -> None:
    """Raise ``ValueError`` if ``path`` is a denied DiTing read (see
...
#
# DiTing profiles are separate DiTing_HOME dirs under
...
#
# Reference: May 2026 incident where a diting-security profile session
# edited skills under both ``~/.diting/profiles/diting-security/skills/``
# AND ``~/.diting/skills/`` (the default profile's skills) without realizing
# the second path belonged to a different profile.
# ---------------------------------------------------------------------------

# Profile-scoped directories under DiTing_HOME / <root> / <root>/profiles/<X>/
...
def _resolve_active_profile_name() -> str:
    """Return the active profile name derived from DiTing_HOME.
    ...
    """
    try:
        home_real = _diting_home_path().resolve()
        root_real = _diting_root_path().resolve()
...
def classify_cross_profile_target(path: str) -> Optional[dict]:
    ...
    Returns ``None`` when the target is outside DiTing scope, or is inside
...
def get_cross_profile_warning(path: str) -> Optional[str]:
    ...
    DiTing entirely. Caller is expected to surface the warning to the
    ...
    return (
        ...
        f"belongs to DiTing profile {info['target_profile']!r}, but the "
...
#
#   <DiTing_HOME>/profiles/<name>/sandboxes/<backend>/<task>/home/.diting/...
#
...
# ``…/sandboxes/<backend>/<task>/home/.diting/…`` segment and warns
# regardless of which DiTing profile is active. It does NOT cover the
# inner-container case where the bind mount strips the ``sandboxes/`` prefix
# (the agent's view inside the container is plain ``/root/.diting/...``);
...
def _find_sandbox_mirror_segments(parts: tuple) -> Optional[int]:
    """Return the index of the inner ``.diting`` part in a sandbox-mirror path.

    Matches ``…/sandboxes/<backend>/<task>/home/.diting/…`` and returns the
    index where the inner DiTing-state portion starts. Returns ``None`` for
    paths that do not contain the sandbox-mirror shape.
    """
    for i, part in enumerate(parts):
        if part != "sandboxes":
            continue
        # Need at least: sandboxes / <backend> / <task> / home / .diting / <thing>
        if i + 5 >= len(parts):
            continue
        if parts[i + 3] == "home" and parts[i + 4] == ".diting":
            return i + 4
    return None


def classify_sandbox_mirror_target(path: str) -> Optional[dict]:
    """Classify a write target as a sandbox-mirror of authoritative DiTing state.

    Returns ``None`` when the path does not match the sandbox-mirror shape.
    Otherwise returns a dict with:

      * ``target_path``: the resolved path string
      * ``mirror_root``: the ``…/sandboxes/<backend>/<task>/home/.diting``
        prefix (so callers can show users which sandbox owns the mirror)
      * ``inner_path``: the portion under the mirror's ``.diting`` (what the
        agent likely meant to address on the host)

    Detection is path-shape-only — does not require any DiTing resolver to
    succeed, so it works correctly even when called from contexts where
    DiTing_HOME resolution would be ambiguous.
    """
    try:
        target = Path(os.path.expanduser(str(path))).resolve()
    except (OSError, RuntimeError):
...
def get_sandbox_mirror_warning(path: str) -> Optional[str]:
    ...
    return (
        ...
        f"Writes here land on a copy that the host DiTing process never "
        f"reads — the authoritative file is likely {info['inner_path']!r} "
        f"under the real DiTing_HOME. Use the host-side tool for "
...
# full ``…/sandboxes/<backend>/<task>/home/.diting/…`` prefix on the host.
# But when file tools execute *inside* the container the bind-mount strips
# that prefix: the agent sees plain ``/root/.diting/…``.  The root:root
...
def classify_container_mirror_target(
    ...
    under the mirror prefix. Otherwise returns:
      ...
      * ``inner_path``: portion under the mirror root (what the agent
        likely meant to address in the host DiTing_HOME)
    """
...
def get_container_mirror_warning(
    ...
    sandbox mirror of authoritative DiTing state.
    ...
    return (
        ...
        f"bind-mounted home — a per-task mirror that the host DiTing "
        f"process never reads. The authoritative file is "
        f"{info['inner_path']!r} under the real DiTing_HOME. Use the "
