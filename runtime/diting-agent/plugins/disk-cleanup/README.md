...
Auto-tracks and cleans up ephemeral files created during DiTing Agent
sessions — test scripts, temp outputs, cron logs, stale chrome profiles.
Scoped strictly to `$DiTing_HOME` and `/tmp/diting-*`.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `DiTing_HOME`, track it silently as `test` / `temp` / `cron-output`. |
...
- `is_safe_path()` rejects anything outside `DiTing_HOME` or `/tmp/diting-*`
- Windows mounts (`/mnt/c` etc.) are rejected
- The state directory `$DiTing_HOME/disk-cleanup/` is itself excluded
- `$DiTing_HOME/logs/`, `memories/`, `sessions/`, `skills/`, `plugins/`,
