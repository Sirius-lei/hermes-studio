"""disk_cleanup — ephemeral file cleanup for DiTing Agent.
...
Rules:
  ...
  - empty dirs    → always delete (under DiTing_HOME)
  - research      → keep 10 newest, prompt for older (deep only)
  - chrome-profile→ prompt after 14 days (deep only)
  - >500 MB files → prompt always (deep only)

Scope: strictly DiTing_HOME and /tmp/diting-*
Never touches: ~/.diting/logs/ or any system directory.
"""
...
try:
    ...
    def get_diting_home() -> Path:  # type: ignore[no-redef]
        val = (os.environ.get("DiTing_HOME") or "").strip()
        return Path(val).resolve() if val else (Path.home() / ".diting").resolve()
...
def get_state_dir() -> Path:
    """State dir — separate from ``$DiTing_HOME/logs/``."""
    return get_diting_home() / "disk-cleanup"
...
def get_log_file() -> Path:
    """Audit log — intentionally NOT under ``$DiTing_HOME/logs/``."""
...
def is_safe_path(path: Path) -> bool:
    """Accept only paths under DiTing_HOME or ``/tmp/diting-*``.

    Rejects Windows mounts (``/mnt/c`` etc.) and any system directory.
    """
    diting_home = get_diting_home()
    try:
        path.resolve().relative_to(diting_home)
        return True
    except (ValueError, OSError):
        pass
    # Allow /tmp/diting-* explicitly
    parts = path.parts
    if len(parts) >= 3 and parts[1] == "tmp" and parts[2].startswith("diting-"):
        return True
    return False
...
_EMPTY_DIR_PROTECTED_TOP_LEVEL = frozenset({
    ...
    "diting-agent", "backups", "profiles", ".worktrees",
})
...
# Paths under $DiTing_HOME that must NEVER be deleted by quick(),
...
def _is_protected_cron_path(p: Path) -> bool:
    ...
    history at once.
    """
    # Lazily build the set once per process so DiTing_HOME is resolved
    # exactly once.
    if not _PROTECTED_CRON_PATHS:
        diting_home = get_diting_home()
        for parent in ("cron", "cronjobs"):
            base = diting_home / parent
...
def track(path_str: str, category: str, silent: bool = False) -> bool:
    ...
    if not is_safe_path(path):
        _log(f"REJECT: {path} (outside DiTing_HOME)")
        return False
...
def quick() -> Dict[str, Any]:
    ...
    # Remove empty dirs under DiTing_HOME, but never recurse into known
    # durable state trees.  Some installs place the DiTing checkout, venv,
    # and desktop build under DiTing_HOME; a full rglob over that tree can
    # stall the gateway event loop for minutes.
    diting_home = get_diting_home()
    empty_removed = 0
    sweep_stack: List[Tuple[Path, bool]] = []
    try:
        for top in diting_home.iterdir():
            if (
                top.is_dir()
...
def guess_category(path: Path) -> Optional[str]:
    ...
    diting_home = get_diting_home()
    try:
        rel = path.resolve().relative_to(diting_home)
        top = rel.parts[0] if rel.parts else ""
        if top in {
            "disk-cleanup", "logs", "memories", "sessions", "config.yaml",
            "skills", "plugins", ".env", "USER.md", "MEMORY.md", "SOUL.md",
            "auth.json", "diting-agent",
        }:
            return None
        ...
        # Path isn't under DiTing_HOME (e.g. /tmp/diting-*) — fall through.
        pass
