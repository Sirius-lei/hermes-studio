# DiTing Achievements Performance Spec (Post-Hackathon)

Status: Draft (no code changes yet)
Owner: diting-achievements plugin
...
### `GET /api/plugins/diting-achievements/achievements`
...
### `POST /api/plugins/diting-achievements/rescan` (optional)
Manual refresh trigger.
Prefer async trigger + immediate status response.

### `GET /api/plugins/diting-achievements/scan-status` (optional new)
Reports scan state for UX/ops.

### Removed
- `GET /api/plugins/diting-achievements/overview`
...
- Persist plugin data under:
  - `~/.diting/plugins/diting-achievements/scan_snapshot.json`

### Phase 2
- Incremental scan checkpoints with per-session fingerprints.
- Persist checkpoint data under:
  - `~/.diting/plugins/diting-achievements/scan_checkpoint.json`
...
- `~/.diting/plugins/diting-achievements/`
