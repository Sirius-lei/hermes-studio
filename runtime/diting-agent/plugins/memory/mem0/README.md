...
```bash
diting memory setup    # select "mem0"
```
...
Behavioral settings live in `$DiTing_HOME/mem0.json` (set them via `diting memory setup`). Only the secret `MEM0_API_KEY` belongs in `~/.diting/.env`.
...
| `user_id` | `diting-user` | User identifier on Mem0 |
...
2. Point the plugin at it — via the setup wizard:
   ...
   ```
3. Start a fresh DiTing session and call `mem0_search` — it connects to your server.
...
```bash
diting memory setup mem0 --mode oss \
...
```bash
diting memory setup mem0 --mode oss --oss-llm-key sk-...
```

Or edit `$DiTing_HOME/mem0.json` directly:
```json
{
  "mode": "oss",
  "oss": {
    ...
    "vector_store": {"provider": "qdrant", "config": {"path": "~/.diting/mem0_qdrant"}}
  }
}
```
...
```bash
diting memory setup mem0 --mode platform --api-key sk-...
```
...
- Check `user_id` matches between sessions (`$DiTing_HOME/mem0.json`).
