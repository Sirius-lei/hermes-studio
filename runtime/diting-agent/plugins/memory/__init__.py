...
1. Bundled providers: ``plugins/memory/<name>/`` (shipped with diting-agent)
2. User-installed providers: ``$DiTing_HOME/plugins/<name>/``
...
_USER_NAMESPACE = "_diting_user_memory"


def _register_synthetic_package(name: str, search_locations: List[str]) -> None:
    """Register an empty package shell in sys.modules.

    User-installed providers import as ``_diting_user_memory.<name>``, a
    ...
    ``ModuleNotFoundError: No module named '_diting_user_memory'`` — the
...
def _get_user_plugins_dir() -> Optional[Path]:
    """Return ``$DiTing_HOME/plugins/`` or None if unavailable."""
    try:
        from diting_constants import get_diting_home
        d = get_diting_home() / "plugins"
...
def load_memory_provider(name: str) -> Optional["MemoryProvider"]:
    ...
    (``$DiTing_HOME/plugins/<name>/``) directories.  Bundled takes
...
def discover_plugin_cli_commands() -> List[dict]:
    ...
    try:
        ...
        if module_name in sys.modules:
            ...
            if not _is_bundled:
                # cli.py imports as _diting_user_memory.<name>.cli, usually
