...
- Supermemory API key from [app.supermemory.ai/integrations?connect=diting](http://app.supermemory.ai/integrations?connect=diting)
...
Config file: `$DiTing_HOME/supermemory.json`

| Key | Default | Description |
|-----|---------|-------------|
| `container_tag` | `diting` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags (e.g. `diting-{identity}` → `diting-coder`). |
...
All Supermemory API calls send `x-sm-source: diting`, and document writes stamp
`metadata.sm_source: diting`. This is a **functional routing key, not telemetry**:
it groups DiTing-written memories into a dedicated "DiTing" Space in the
...
Use `{identity}` in the `container_tag` to scope memories per DiTing profile:

```json
{
  "container_tag": "diting-{identity}"
}
```

For a profile named `coder`, this resolves to `diting-coder`. The default profile resolves to `diting-default`. Without `{identity}`, all profiles share the same container.
