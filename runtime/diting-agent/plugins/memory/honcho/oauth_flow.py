...
``diting://`` handler. Endpoints are env-overridable with local-dev defaults
...
# The loopback redirect registered for the DiTing OAuth client. IP-literal so
...
_DEFAULT_CLIENT_ID = "diting-agent"
...
def begin_authorization(
    ...
    *,
    ...
    (``diting-desktop`` / ``diting-cli``) so the consent side can attribute
...
_CALLBACK_HTML = (
    ...
    b"<div>Connected to Honcho. You can close this tab and return to DiTing.</div>"
)
...
def start_loopback_flow_background(
    *,
    ...
    # the request, where a context-local DiTing_HOME override can't reach.
