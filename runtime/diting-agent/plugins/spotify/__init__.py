...
``_check_spotify_available()`` — when the user has not run ``diting auth
spotify``, the tools remain registered (so they appear in ``diting
...
The Spotify auth flow (``diting auth spotify``), CLI plumbing, and docs
