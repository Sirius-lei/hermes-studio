...
Optional DiTing observability plugin that maps DiTing observer hooks to
...
not replace DiTing Agent's planner, tools, memory, model provider routing, or
CLI UX. Instead, this plugin lets DiTing emit NeMo Relay lifecycle events for
the work DiTing already owns: sessions, turns, provider/API calls, tool calls,
approval prompts, and delegated subagents.

With this plugin enabled, DiTing Agent can:

- Preserve DiTing execution as NeMo Relay scopes, LLM spans, tool spans, and
  mark events.
...
```bash
diting plugins enable observability/nemo_relay
```

The `DiTing_NEMO_RELAY_*` environment variables below only configure an
already-enabled plugin. They do not enable plugin discovery by themselves.

For isolated test homes, enable the plugin in the same `DiTing_HOME` that the
agent run will use:

```bash
env DiTing_HOME=/tmp/diting-nemo-relay-test \
  diting plugins enable observability/nemo_relay
```

Runs started with `--ignore_user_config` skip the enabled-plugin state from
`DiTing_HOME`, so local E2E tests should omit that flag unless the test harness
loads `observability/nemo_relay` explicitly another way.

`DiTing_HOME` is the DiTing profile/config home used by both
`diting plugins enable ...` and the later `diting chat ...` run. If unset,
DiTing uses the user's default home, usually `~/.diting`. For isolated smoke
tests, choose any writable temporary directory and use the same value for every
command in that test:

```bash
export DiTing_HOME=/tmp/diting-nemo-relay-test
diting plugins enable observability/nemo_relay
diting chat --query 'Reply exactly ok' --provider custom --model qwen3.6:35b
```

For source checkouts, make sure the `diting` command you run is built from the
...
uv run diting plugins enable observability/nemo_relay
uv run diting chat --query 'Reply exactly ok' --provider custom --model qwen3.6:35b
```
...
```bash
uv build --wheel
python -m pip install --force-reinstall dist/diting_agent-*.whl
...
The plugin can configure exporters directly from `DiTing_NEMO_RELAY_*`
...
```bash
export DiTing_NEMO_RELAY_ATOF_ENABLED=1
export DiTing_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=.nemo-relay/atof
export DiTing_NEMO_RELAY_ATIF_ENABLED=1
export DiTing_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=.nemo-relay/atif
```

Optional overrides:

- `DiTing_NEMO_RELAY_ATOF_FILENAME`
- `DiTing_NEMO_RELAY_ATOF_MODE` (`append` or `overwrite`)
- `DiTing_NEMO_RELAY_ATIF_FILENAME_TEMPLATE`
- `DiTing_NEMO_RELAY_ATIF_AGENT_NAME`
- `DiTing_NEMO_RELAY_ATIF_AGENT_VERSION`
- `DiTing_NEMO_RELAY_ATIF_MODEL_NAME`
- `DiTing_NEMO_RELAY_ATIF_SUBAGENT_EXPORT_MODE` (`embedded` by default; set `all` to also write standalone child files)

### NeMo Relay Component Config

To initialize NeMo Relay from a component config, create a `plugins.toml` file
and point DiTing at it:

```bash
export DiTing_NEMO_RELAY_PLUGINS_TOML=.nemo-relay/plugins.toml
```
...
agent_name = "DiTing Agent"
agent_version = "local"
```

When `DiTing_NEMO_RELAY_PLUGINS_TOML` is set and initializes successfully, NeMo
Relay owns exporter lifecycle through that config. The direct
`DiTing_NEMO_RELAY_ATOF_*` fallback setup is skipped. If the same
`plugins.toml` observability config enables `atif`, the direct
`DiTing_NEMO_RELAY_ATIF_*` fallback setup is also skipped so DiTing does not
double-export trajectories on teardown. If `plugins.toml` initialization fails,
DiTing keeps the direct env-var fallbacks active for that run.
...
exposes `llm.execute(...)` / `tools.execute(...)`, DiTing routes LLM and tool
...
DiTing feature-detects the dynamic-plugin activation API available in NeMo Relay
0.6 and later. Configure native or worker plugins with DiTing-owned
...
with this DiTing-owned section. The gateway combines those records with
separate lifecycle state for enablement, trust policy, and worker environments;
the Python binding does not yet expose that resolver. DiTing rejects
...
DiTing activates these plugins before registering its managed LLM and tool
...
activation API, DiTing logs an actionable warning and continues with the
...
For the full generic DiTing middleware contract, see
...
export DiTing_HOME=/tmp/diting-nemo-relay-docs/diting-home
mkdir -p "$DiTing_HOME"

cat > "$DiTing_HOME/config.yaml" <<'YAML'
model:
  provider: custom
  default: qwen3.6:35b
...
This run starts a parent DiTing session, delegates to a child subagent, has the
child call `terminal`, and writes both ATOF and ATIF.

```bash
export DiTing_NEMO_RELAY_ATOF_ENABLED=1
export DiTing_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=/tmp/diting-nemo-relay-docs/subagent/atof
export DiTing_NEMO_RELAY_ATOF_FILENAME=nested-subagent-atof.jsonl
export DiTing_NEMO_RELAY_ATOF_MODE=overwrite
export DiTing_NEMO_RELAY_ATIF_ENABLED=1
export DiTing_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=/tmp/diting-nemo-relay-docs/subagent/atif
export DiTing_NEMO_RELAY_ATIF_FILENAME_TEMPLATE='nested-subagent-atif-{session_id}.json'
export DiTing_NEMO_RELAY_ATIF_AGENT_NAME='DiTing Agent E2E'
export DiTing_NEMO_RELAY_ATIF_AGENT_VERSION=docs-example
export DiTing_NEMO_RELAY_ATIF_SUBAGENT_EXPORT_MODE=all
...
{"kind":"mark","name":"diting.subagent.start","metadata":{"parent_session_id":"docs-parent-session","session_id":"docs-child-session","subagent_id":"sa-0-docs","child_role":"leaf"}}
...
```json
{
  ...
  "session_id": "docs-parent-session",
  "agent": {"name": "DiTing Agent E2E", "version": "docs-example", "model_name": "qwen3.6:35b"},
  "steps": [
    {
      "source": "agent",
...
message. DiTing dispatches the read-only tools as one batch, and NeMo Relay
records both tool invocations.

```bash
mkdir -p /tmp/diting-nemo-relay-docs/workdir
printf 'docs_parallel_alpha_function\n' > /tmp/diting-nemo-relay-docs/workdir/alpha.txt
printf 'docs_parallel_beta_function\n' > /tmp/diting-nemo-relay-docs/workdir/beta.txt
cd /tmp/diting-nemo-relay-docs/workdir

export DiTing_NEMO_RELAY_ATOF_ENABLED=1
export DiTing_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=/tmp/diting-nemo-relay-docs/parallel/atof
export DiTing_NEMO_RELAY_ATOF_FILENAME=parallel-tools-atof.jsonl
export DiTing_NEMO_RELAY_ATOF_MODE=overwrite
export DiTing_NEMO_RELAY_ATIF_ENABLED=1
export DiTing_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=/tmp/diting-nemo-relay-docs/parallel/atif
export DiTing_NEMO_RELAY_ATIF_FILENAME_TEMPLATE='parallel-tools-atif-{session_id}.json'
export DiTing_NEMO_RELAY_ATIF_AGENT_NAME='DiTing Agent E2E'
export DiTing_NEMO_RELAY_ATIF_AGENT_VERSION=docs-example
...
- DiTing sessions map to `agent` scopes.
- DiTing API request hooks map to `llm` scope start/end events.
- DiTing tool hooks map to `tool` scope start/end events.
...
The `observability/nemo_relay` plugin uses DiTing execution middleware to hand
...
Enable it for DiTing:

```bash
export DiTing_NEMO_RELAY_PLUGINS_TOML=/tmp/diting-middleware-test/plugins.toml
```

When the adaptive component is enabled and the installed NeMo Relay runtime
exposes `llm.execute(...)` and `tools.execute(...)`, DiTing routes execution
through these boundaries:

```text
DiTing provider call
  -> llm_execution middleware
    -> nemo_relay.llm.execute(...)
      -> DiTing provider adapter next_call(...)

DiTing tool call
  -> tool_execution middleware
    -> nemo_relay.tools.execute(...)
      -> DiTing tool dispatcher next_call(...)
```
...
middleware for a local DiTing run. This path requires a NeMo Relay runtime that
supports `[components.config.tool_parallelism]`, as provided by the supported
0.x release range beginning with 0.5.

```bash
export DiTing_HOME=/tmp/diting-middleware-test/diting-home
mkdir -p "$DiTing_HOME" /tmp/diting-middleware-test/nemo-relay
...
cat > /tmp/diting-middleware-test/nemo-relay/plugins.toml <<'TOML'
version = 1
...
[components.config.atof]
enabled = true
output_directory = "/tmp/diting-middleware-test/atof"
filename = "middleware-events.jsonl"
mode = "overwrite"

[components.config.atif]
enabled = true
output_directory = "/tmp/diting-middleware-test/atif"
filename_template = "middleware-trajectory-{session_id}.json"
agent_name = "DiTing Middleware E2E"
agent_version = "local"
...
export DiTing_NEMO_RELAY_PLUGINS_TOML=/tmp/diting-middleware-test/nemo-relay/plugins.toml
