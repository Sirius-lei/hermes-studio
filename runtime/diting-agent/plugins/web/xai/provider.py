...
``{title, url, description, position}`` rows every other DiTing web
provider produces.
...
prefers DiTing-managed xAI Grok OAuth (via ``diting auth``) and falls back
to ``XAI_API_KEY`` (resolved through ``~/.diting/.env``, then
``os.environ``).
"""
...
class XAIWebSearchProvider(WebSearchProvider):
    ...
    def is_available(self) -> bool:
        ...
        every ``diting tools`` repaint and at tool-registration time.
    ...
    def search(self, query: str, limit: int = 5) -> Dict[str, Any]:
        ...
        if not api_key:
            return {
                "success": False,
                "error": (
                    "No xAI credentials found. Run `diting auth` to sign in with "
