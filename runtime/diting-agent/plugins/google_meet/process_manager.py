...
session-scoped state file under ``$DiTing_HOME/workspace/meetings/.active.json``
...
# File + directory layout (under $DiTing_HOME):
#
#   workspace/meetings/
...
def _root() -> Path:
    return Path(get_diting_home()) / "workspace" / "meetings"
...
def start(
    ...
    *,
    ...
    If a bot is already running for this diting install, leave it first —
    ...
    env = os.environ.copy()
    env["DiTing_MEET_URL"] = url
    env["DiTing_MEET_OUT_DIR"] = str(out)
    env["DiTing_MEET_GUEST_NAME"] = guest_name
    if headed:
        env["DiTing_MEET_HEADED"] = "1"
    if auth_state:
        env["DiTing_MEET_AUTH_STATE"] = auth_state
    if duration:
        env["DiTing_MEET_DURATION"] = duration
    # v2: realtime mode + passthroughs. The bot defaults to transcribe
    # mode if DiTing_MEET_MODE isn't set, matching v1 behavior.
    if mode:
        env["DiTing_MEET_MODE"] = mode
    if realtime_model:
        env["DiTing_MEET_REALTIME_MODEL"] = realtime_model
    if realtime_voice:
        env["DiTing_MEET_REALTIME_VOICE"] = realtime_voice
    if realtime_instructions:
        env["DiTing_MEET_REALTIME_INSTRUCTIONS"] = realtime_instructions
    if realtime_api_key:
        env["DiTing_MEET_REALTIME_KEY"] = realtime_api_key
