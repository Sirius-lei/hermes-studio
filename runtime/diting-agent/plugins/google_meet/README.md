...
Let the diting agent join a Google Meet call, transcribe it, optionally speak
...
```
┌─ gateway (Linux box, where diting runs) ────────────────────────────┐
│                                                                      │
...
│                                                                      │
│   NodeServer (from `diting meet node run`)                           │
│     │                                                                │
...
| `__init__.py` | `register(ctx)` — registers 5 tools + `on_session_end` hook + `diting meet` CLI |
...
| `cli.py` | `diting meet setup / auth / join / status / transcript / say / stop / node ...` |
...
| `node/registry.py` | v3: `$DiTing_HOME/workspace/meetings/nodes.json` |
| `node/server.py` | v3: `NodeServer` (runs on host machine) |
| `node/client.py` | v3: `NodeClient` (used by tool handlers + CLI on gateway) |
| `node/cli.py` | v3: `diting meet node {run,list,approve,remove,status,ping}` |
...
diting meet install                                      # pip + Chromium
diting meet setup                                        # preflight
diting meet auth                                         # optional
diting meet join https://meet.google.com/abc-defg-hij    # transcribe
```
...
```bash
diting meet install --realtime                     # installs pulseaudio-utils
echo 'OPENAI_API_KEY=sk-...' >> ~/.diting/.env
diting meet join https://meet.google.com/abc-defg-hij --mode realtime
# then from the agent or CLI:
diting meet say "Good morning everyone, I'm the note-taker bot."
```

macOS:
```bash
diting meet install --realtime     # runs: brew install blackhole-2ch ffmpeg
# then — manually! — open System Settings → Sound → Input → BlackHole 2ch
echo 'OPENAI_API_KEY=sk-...' >> ~/.diting/.env
diting meet join https://meet.google.com/abc-defg-hij --mode realtime
```

On macOS, diting will **not** switch your system audio input automatically — the
...
diting plugins enable google_meet
diting meet node run --display-name my-mac --host 0.0.0.0 --port 18789
