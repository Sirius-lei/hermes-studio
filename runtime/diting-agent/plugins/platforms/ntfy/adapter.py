"""ntfy platform adapter (DiTing plugin).
...
``plugins/platforms/ntfy/``. The DiTing plugin loader scans the
    ...
    platforms:
      ntfy:
        ...
        extra:
          ...
          topic: "diting-in"              # subscribe topic (incoming)
          publish_topic: "diting-out"     # optional — defaults to topic
...
_ECHO_TAG = "diting-agent"  # tag added to outgoing messages for echo-loop prevention
...
async def _standalone_send(
    ...
    *,
    ...
    ``diting cron`` running standalone). Without this hook,
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="pip install httpx   # already a DiTing dependency",
        # Env-driven auto-configuration: seeds PlatformConfig.extra so
        # env-only setups show up in `diting gateway status` without
