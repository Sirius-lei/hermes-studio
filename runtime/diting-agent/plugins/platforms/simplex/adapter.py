"""SimpleX Chat platform adapter (DiTing plugin).
...
This adapter ships as a DiTing platform plugin under
``plugins/platforms/simplex/``. The DiTing plugin loader scans the
...
Optional environment variables:
    ...
    DiTing_SIMPLEX_TEXT_BATCH_DELAY
...
discoverable and ``diting setup`` can describe it even when websockets is
...
# Imported at module top because they're stdlib-only inside DiTing — no
...
class SimplexAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig, **kwargs):
        ...
        self._text_batch_delay = float(
            os.getenv("DiTing_SIMPLEX_TEXT_BATCH_DELAY", "0.8")
        )
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``diting setup gateway`` → SimpleX.

    Prompts for the WebSocket URL and the optional allowlist / groups /
    auto-accept / home channel. Writes to ``~/.diting/.env`` via
    ``diting_cli.config``.
    """
    print()
    ...
    try:
        ...
        print(
            "diting_cli.config not available; set SIMPLEX_* vars manually in "
            "~/.diting/.env"
        )
        return
