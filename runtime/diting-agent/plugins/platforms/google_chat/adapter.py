...
# ``model_tools`` import time, so ``diting status``, ``diting chat``, etc.
...
# ``diting_plugins.platforms__google_chat.adapter`` once the plugin
...
#     fresh "DiTing is thinking…" card during the small window between
...
def _mime_for_message_type(mime: str) -> MessageType:
    """Map a MIME string to a diting MessageType.
...
class GoogleChatAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Persisted to ${DiTing_HOME}/google_chat_thread_counts.json so
        ...
        try:
            ...
            _diting_home = _get_diting_home()
        except (ModuleNotFoundError, ImportError):
            _diting_home = _Path.home() / ".diting"
        self._thread_count_store = _ThreadCountStore(
            _diting_home / "google_chat_thread_counts.json"
        )
        ...
        # they don't sit in the chat forever as "DiTing is thinking…".
    ...
    def _bot_id_cache_path(self) -> _Path:
        ...
        base = os.getenv("DiTing_HOME", str(_Path.home() / ".diting"))
    ...
    async def _dispatch_message(self, msg: Dict[str, Any], envelope: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if text.startswith("/setup-files") and event.source is not None:
                ...
                # ${DiTing_HOME}/google_chat_user_tokens/<sanitized>.json
    ...
    async def _build_message_event(
        ...
        """Parse a Chat API message into a diting MessageEvent."""
        space = envelope.get("space") or msg.get("space") or {}
    ...
    async def send_typing(self, chat_id: str, metadata: Any = None) -> None:
        """Post a visible 'DiTing is thinking…' marker message.
        ...
        "DiTing is thinking…" stuck in chat forever, plus one card that
        ...
        body: Dict[str, Any] = {"text": "DiTing is thinking…"}
        if thread_id:
            body["thread"] = {"name": thread_id}
    ...
    async def stop_typing(self, chat_id: str) -> None:
        ...
        Google Chat has no separate typing API: the "DiTing is thinking…"
    ...
    async def on_processing_complete(
        ...
        see "DiTing is thinking…" stuck forever.
        """
    ...
    async def _consume_typing_card_with_text(
        ...
        creating a fresh "DiTing is thinking…" card during any subsequent
...
def interactive_setup() -> None:
    """Walk the user through Google Chat configuration via ``diting setup``.

    The setup wizard at ``diting_cli/gateway.py`` calls this for plugin
    ...
    prompt for env vars, persist them to ``~/.diting/.env`` so the next
    ...
    print_info("  3. Create a Pub/Sub topic (e.g. diting-chat-events) and a Pull subscription.")
    ...
    print()
    print_success("Google Chat configuration saved to ~/.diting/.env")
    print_info("Restart the gateway: diting gateway restart")
...
async def _standalone_send(
    ...
    *,
    ...
    runner is not in this process (e.g. ``diting cron`` running as a
    separate process from ``diting gateway``).  Without this hook,
...
def register(ctx) -> None:
    """Plugin entry point — called by the DiTing plugin system at startup.
    ...
    ctx.register_platform(
        ...
        install_hint="pip install 'diting-agent[google_chat]'",
        ...
        # the "DiTing is thinking..." marker patches and edit overhead.
        ...
        platform_hint=(
            ...
            "are generating a response, a 'DiTing is thinking…' marker message "
