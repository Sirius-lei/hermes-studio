...
Small Node helper that bridges DiTing Agent to Photon's Spectrum SDK
(`spectrum-ts`).  DiTing is Python; Photon has no public HTTP
...
- exposes a loopback-only HTTP control channel for the Python adapter
  to push send/typing requests (auth via `X-DiTing-Sidecar-Token`)
...
The DiTing plugin's `diting photon setup` command runs `npm install`
here automatically.
