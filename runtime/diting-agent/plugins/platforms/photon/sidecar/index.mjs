// DiTing Agent — Photon Spectrum sidecar
//
...
//
// Inbound  (gRPC -> DiTing): the SDK's `app.messages` async iterator is a
...
//   gateway connects.
// Outbound (DiTing -> gRPC): `/send` drives `space.send(...)`; `/typing`
//   sends the documented `typing("start" | "stop")` content builder.
//
// Protocol (all requests require `X-DiTing-Sidecar-Token: ${TOKEN}`):
...
//                          default off — toggle with `diting photon telemetry`)
  ...
  streamRestartTimer = setTimeout(() => {
    ...
    console.error(
      `photon-sidecar: upstream stream degraded for ${degradedForMs}ms; ` +
        "exiting so DiTing can restart the Photon adapter"
    );
    process.exit(75);
...
// instead of a cryptic module-resolution error during import. Apply DiTing'
...
function inboundStreamErrorMessage(e) {
  ...
  // iMessage internal error. Local DiTing allowlists cannot cause or fix this:
  ...
  if (
    ...
    out +=
      ...
      "this is upstream of DiTing, so inbound iMessages may not be delivered " +
...
const server = http.createServer(async (req, res) => {
  if (!tokenOk(req.headers["x-diting-sidecar-token"])) {
  ...
  try {
    ...
    if (req.url === "/send-attachment") {
      ...
      // overrides only when DiTing supplied them so a known-good
