...
Credential storage mirrors every other DiTing channel:

    * runtime SDK creds  -> ``~/.diting/.env``  (``PHOTON_PROJECT_ID`` =
      project id, ``PHOTON_PROJECT_SECRET``) via ``save_env_value``
    * management metadata -> ``~/.diting/auth.json`` under
...
try:
    import httpx
except ImportError:  # pragma: no cover - httpx is a diting dependency
...
# registers DiTing as its own client_id.
...
# Default name of the project DiTing provisions for the operator.
DEFAULT_PROJECT_NAME = "DiTing Agent"
...
# ---------------------------------------------------------------------------
# auth.json helpers — share the file with the rest of diting-agent.

def _auth_json_path() -> Path:
    """Resolve ``~/.diting/auth.json`` honouring the active DiTing profile."""
    try:
        from diting_constants import get_diting_home
        return Path(get_diting_home()) / "auth.json"
    except Exception:
        return Path(os.path.expanduser("~/.diting")) / "auth.json"
...
def load_project_credentials() -> Tuple[Optional[str], Optional[str]]:
    ...
    Precedence: process env (``~/.diting/.env`` is loaded into the gateway's
...
def store_project_credentials(
    *,
    ...
    The runtime SDK creds land in ``~/.diting/.env`` via the same
...
def _persist_runtime_env(spectrum_project_id: str, project_secret: str) -> None:
    """Write the SDK creds to ``~/.diting/.env`` (canonical runtime store).
    ...
    try:
        ...
        logger.warning("photon: diting_cli.config unavailable — skipping .env write")
        return
    try:
...
def print_credential_summary(emit: Any = print) -> None:
    ...
    labels["device_token"] = (
        ...
        else "✗ missing (run `diting photon setup`)"
    )
    ...
    labels["phone_number"] = (
        phone if phone else "✗ missing (run `diting photon setup --phone ...`)"
    )
    labels["assigned_phone_number"] = (
        assigned if assigned else "✗ missing (run `diting photon setup`)"
    )
...
def credential_summary() -> Dict[str, str]:
    ...
    def _present_phone() -> str:
        ...
        return phone or "✗ missing (run `diting photon setup --phone ...`)"

    def _present_assigned_phone() -> str:
        _phone, assigned = load_user_numbers()
        return assigned or "✗ missing (run `diting photon setup`)"
