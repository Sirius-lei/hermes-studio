...
This plugin connects DiTing Agent to iMessage (and other Spectrum
...
TypeScript-only, DiTing runs it inside a small supervised Node sidecar and
...
- **Outbound**: `send` / `send_typing` / reaction tapbacks are loopback POSTs
  ...
  `/unreact`), authenticated with a shared `X-DiTing-Sidecar-Token`.
...
There is no separate `login` command; like every other DiTing channel,
...
Run `diting photon status` to see what's configured.

## Credentials

Runtime SDK credentials live in `~/.diting/.env` (the same place every other
...
Management metadata lives in `~/.diting/auth.json` under `credential_pool`:
...
| `PHOTON_TELEMETRY`        | false                      | Spectrum SDK telemetry — toggle with `diting photon telemetry on\|off` (restart the gateway to apply) |
...
6. Verify end-to-end: `diting photon status`, a DM and a group roundtrip,
