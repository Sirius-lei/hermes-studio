"""
IRC Platform Adapter for DiTing Agent.

A plugin-based gateway adapter that connects to an IRC server and relays
messages to/from the DiTing agent.  Zero external dependencies — uses
...
class IRCAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config, **kwargs):
        ...
        try:
            ...
            self.port = 6697
        self.nickname = os.getenv("IRC_NICKNAME") or extra.get("nickname", "diting-bot")
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        await self._send_raw(f"USER {self.nickname} 0 * :DiTing Agent")
    ...
    async def disconnect(self) -> None:
        ...
        if self._writer and not self._writer.is_closing():
            try:
                await self._send_raw("QUIT :DiTing Agent shutting down")
    ...
    async def _handle_line(self, raw: str) -> None:
        ...
        if command == "433":
            # Retry with incrementing suffix: diting_, diting_1, diting_2...
...
def check_requirements() -> bool:
    ...
    # diting setup / requirements check path.
...
def interactive_setup() -> None:
    """Interactive `diting gateway setup` flow for the IRC platform.

    Lazy-imports ``diting_cli.setup`` helpers so the plugin stays importable
    ...
    print_info("Connect DiTing to an IRC network. Uses Python stdlib — no extra packages needed.")
    ...
    channel = prompt(
        "Channel to join (e.g. #diting — comma-separate for multiple)",
    ...
    print()
    print_success("IRC configuration saved to ~/.diting/.env")
    print_info("Restart the gateway for changes to take effect: diting gateway restart")
...
async def _standalone_send(
    ...
    *,
    ...
    nickname = os.getenv("IRC_NICKNAME") or extra.get("nickname", "diting-bot")
    ...
    nick_base = nickname.rstrip("_0123456789-")[:24] or "diting-bot"
    ...
    try:
        ...
        await _raw(f"USER {standalone_nick} 0 * :DiTing Agent (cron)")
