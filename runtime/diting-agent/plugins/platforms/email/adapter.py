"""
Email platform adapter for the DiTing gateway.

Allows users to interact with DiTing by sending emails.
...
def _send_imap_id(imap: "imaplib.IMAP4") -> None:
    ...
    try:
        ...
        imap.xatom(
            "ID",
            f'("name" "diting-agent" "version" "{_diting_version}" '
            '"vendor" "NousResearch" '
...
class EmailAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # send-helper, and `diting config show` already read. Without the
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        if missing:
            message = (
                ...
                + ", ".join(missing)
                + ". Set it via `diting gateway setup` (env) or platforms.email "
                "in config.yaml."
            )
    ...
    def _send_email(
        ...
        subject = ctx.get("subject", "DiTing Agent")
        ...
        msg_id = f"<diting-{uuid.uuid4().hex[:12]}@{self._message_id_domain()}>"
...
# diting_cli/gateway.py, and the _send_email dispatch in
