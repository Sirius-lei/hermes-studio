"""
LINE Messaging API platform adapter for DiTing Agent.
...
to DiTing Agent. It deliberately ports the *strongest* idea from each into
...
class LineAdapter(BasePlatformAdapter):
    ...
    async def _handle_media(self, request) -> Any:
        ...
        ``/private/tmp`` on macOS), and ``DiTing_HOME``. PR #8398.
        """
        from aiohttp import web
        ...
        try:
            ...
            diting_home = Path(get_diting_home()).resolve()
        except Exception:
            diting_home = Path.home().joinpath(".diting").resolve()
...
def is_connected(config) -> bool:
    """Surface in ``diting status`` even before the adapter is instantiated."""
...
def _env_enablement() -> Optional[Dict[str, Any]]:
    ...
    Lets ``diting status`` reflect a LINE configuration that lives entirely
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``diting setup line``.

    Mirrors the irc/teams style: prompts for the two required vars, plus
    one optional public URL. Writes to ``~/.diting/.env`` via ``diting_cli.config``.
    """
    print()
    ...
    try:
        from diting_cli.config import get_env_var, set_env_var
    except ImportError:
        print("diting_cli.config not available; set LINE_* vars manually in ~/.diting/.env")
        return
