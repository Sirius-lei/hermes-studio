"""
DiTing MCP Server — expose messaging conversations as MCP tools.
...
Plus: channels_list (DiTing-specific extra)

Usage:
    diting mcp serve
    diting mcp serve --verbose
...
logger = logging.getLogger("diting.mcp_serve")
...
def _get_sessions_dir() -> Path:
    """Return the sessions directory using DiTing_HOME."""
    try:
        from diting_constants import get_diting_home
        return get_diting_home() / "sessions"
    except ImportError:
        return Path(os.environ.get("DiTing_HOME", Path.home() / ".diting")) / "sessions"
...
def _load_channel_directory() -> dict:
    ...
    try:
        from diting_constants import get_diting_home
        directory_file = get_diting_home() / "channel_directory.json"
    except ImportError:
        directory_file = Path(
            os.environ.get("DiTing_HOME", Path.home() / ".diting")
...
class EventBridge:
    ...
    This is the DiTing equivalent of OpenClaw's WebSocket gateway bridge.
    ...
    def _poll_once(self, db):
        ...
        try:
            from diting_constants import get_diting_home
            db_file = get_diting_home() / "state.db"
        except ImportError:
            db_file = Path(os.environ.get("DiTing_HOME", Path.home() / ".diting")) / "state.db"
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "FastMCP":
    """Create and return the DiTing MCP server with all tools registered."""
    ...
    mcp = FastMCP(
        "diting",
        instructions=(
            "DiTing Agent messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the DiTing MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
        print(
