# DiTing Agent Has Had "Routines" Since March
...
DiTing equivalent — works today:
```bash
diting webhook subscribe auth-watch \
  --events "pull_request" \
...
Every use case in their blog post — backlog triage, docs drift, deploy verification, alert correlation, library porting, bespoke PR review — has a working DiTing implementation. No new features needed. It's been shipping since March 2026.

---

## What's Different

| | Claude Code Routines | DiTing Agent |
|---|---|---|
...
## Things DiTing Does That Routines Can't
...
DiTing has no daily limit. Run 500 automations a day if you want. The only constraint is your API budget, and you choose which models to use for which tasks.
...
DiTing Agent is open source and free. The automation infrastructure — cron scheduler, webhook platform, skill system, multi-platform delivery — is built in.
...
```bash
diting gateway setup    # enable webhooks
diting webhook subscribe pr-review \
  --events "pull_request" \
...
Full automation blueprints gallery: [diting-agent.nousresearch.com/docs/reference/automation-blueprints-catalog](https://diting-agent.nousresearch.com/docs/reference/automation-blueprints-catalog)

Documentation: [diting-agent.nousresearch.com](https://diting-agent.nousresearch.com)

GitHub: [github.com/NousResearch/diting-agent](https://github.com/NousResearch/diting-agent)

---

*DiTing Agent is built by [Nous Research](https://nousresearch.com). Open source, model-agnostic, runs on your infrastructure.*
