# DiTing Agent Security Policy

This document describes DiTing Agent's trust model, names the one
security boundary the project treats as load-bearing, and defines the
scope for vulnerability reports.

## 1. Reporting a Vulnerability

Report privately via [GitHub Security Advisories](https://github.com/NousResearch/diting-agent/security/advisories/new)
or **security@nousresearch.com**. Do not open public issues for
security vulnerabilities. **DiTing Agent does not operate a bug
bounty program.**
...
- Environment details (`diting version`, commit SHA, OS, Python
  version).
...
DiTing Agent is a single-tenant personal agent. Its posture is
layered, and the layers are not equally load-bearing. Reporters and
operators should reason about them in the same terms.

### 2.1 Definitions

- **Agent process.** The Python interpreter running DiTing Agent,
...
- **Trust envelope.** The set of resources an operator has implicitly
  granted DiTing Agent access to by running it — typically, whatever
  the operator's own user account can reach on the host.
- **Stance.** An explicit statement in DiTing Agent's documentation
...
DiTing Agent supports two OS-level isolation postures. They address
...
DiTing Agent supports this in two ways:

- **DiTing Agent's own Docker image and Compose setup.** Lighter-
...
Under a whole-process wrapper, DiTing Agent's in-process heuristics
...
DiTing Agent filters the environment it passes to its lower-trust
...
A malicious or buggy plugin is not a vulnerability in DiTing Agent
itself. Bugs in DiTing Agent's plugin-install or plugin-discovery
...
4. **Within the authorized set, all callers are equally trusted.**
   DiTing Agent does not model per-caller capabilities inside a
...
- Trust-model documentation violations: code behaving contrary to
  what this policy, DiTing Agent's own documentation, or reasonable
  operator expectations would predict — including cases where
  DiTing Agent has documented a stance about how its output should
...
- **Documented break-glass settings.** Operator-selected trade-offs
  ...
  diting-home security, and similar. Reports against those
...
- **Community-contributed skills and plugins.** Third-party skills
  ...
  plugins are in the operator's review surface, not DiTing Agent's
  trust surface (§2.4, §2.5). A skill or plugin doing something
  malicious is the expected failure mode of one that wasn't
  reviewed, not a vulnerability in DiTing Agent. Bugs in DiTing
...
- DiTing Agent includes supply-chain guards for MCP server
