...
const SIDEBAR_COLLAPSED_KEY = "diting-sidebar-collapsed";
          ...
          >
            ...
            >
              ...
              {sidebarNav.pluginItems.length > 0 && (
                <div
                  aria-labelledby="diting-sidebar-plugin-nav-heading"
                ...
                >
                  <span
                    ...
                    id="diting-sidebar-plugin-nav-heading"
                  >
...
function SidebarSystemActions({
  ...
  const { activeAction, isBusy, isRunning, pendingAction, runAction } =
    useSystemActions();
  const canUpdateDiTing = status?.can_update_diting === true;
  ...
  const updateConfirmDescription = useMemo(() => {
    if (updateConfirmInfo?.behind && updateConfirmInfo.behind > 0) {
      ...
      return `This will run 'diting update' (${cmd}) and pull ${n} new commit${n === 1 ? "" : "s"}. The gateway restarts when the update finishes; the current session keeps its prompt cache until then.`;
    }
    const cmd = updateConfirmInfo?.update_command ?? "diting update";
    return (
      t.status.updateDiTingConfirmMessage ??
      `This will run 'diting update' (${cmd}) and restart the gateway when it finishes.`
    );
  }, [t.status.updateDiTingConfirmMessage, updateConfirmInfo]);
  ...
  if (canUpdateDiTing) {
    items.push({
      action: "update",
      icon: Download,
      label: t.status.updateDiTing,
      runningLabel: t.status.updatingDiTing,
      spin: false,
    });
  }
  ...
  return (
    ...
    <ConfirmDialog
      ...
      description={
        ...
        "This restarts the DiTing gateway process. Connected channels and active sessions will reconnect afterward."
      }
    ...
    <ConfirmDialog
      cancelLabel={t.common.cancel}
      confirmLabel={t.status.updateDiTingConfirmNow ?? "Update now"}
      description={
      ...
      title={t.status.updateDiTingConfirmTitle ?? `${t.status.updateDiTing}?`}
    />
    </>
  );
}
