...
interface DiTingConsoleModalProps {
  open: boolean;
  onClose: () => void;
}
...
export function DiTingConsoleModal({ open, onClose }: DiTingConsoleModalProps) {
  const modalRef = useModalBehavior({ open, onClose });
  const hostRef = useRef<HTMLDivElement | null>(null);
  const termRef = useRef<XtermTerminal | null>(null);
  const wsRef = useRef<WebSocket | null>(null);
  const lineRef = useRef("");
  const promptRef = useRef("diting> ");
  const inputPromptRef = useRef("diting> ");
  ...
  const handleFrame = useCallback(
    (frame: ConsoleFrame) => {
      ...
      if (frame.type === "ready") {
        const nextPrompt = frame.prompt || "diting> ";
  ...
  useEffect(() => {
    ...
    writeLine(term, "\x1b[2mConnecting to DiTing Console...\x1b[0m");
  ...
  return createPortal(
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="diting-console-title"
    >
      <div
        className={cn(
          themedBody,
