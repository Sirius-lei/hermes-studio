# nix/nixosModules.nix — NixOS module for diting-agent
#
# Two modes:
#   container.enable = false (default) → native systemd service
#   container.enable = true            → OCI container (persistent writable layer)
#
# Container mode: diting runs from /nix/store bind-mounted read-only into a
...
# container recreation. Environment variables are written to $DiTing_HOME/.env
# and read by diting at startup — no container recreation needed for env changes.
#
# Tool resolution: the diting wrapper uses --suffix PATH for nix store tools,
...
{ inputs, ... }: {
  ...
  let
    cfg = config.services.diting-agent;
    effectivePackage =
    ...
    diting-agent = inputs.self.packages.${pkgs.stdenv.hostPlatform.system}.default;

    # Deep-merge config type (from 0xrsydn/nix-diting-agent)
    deepConfigType = lib.types.mkOptionType {
      name = "diting-config-attrs";
      description = "DiTing YAML config (attrset), merged deeply via lib.recursiveUpdate.";
    ...
    generatedConfigFile = pkgs.writeText "diting-config.yaml" configJson;
    configFile = if cfg.configFile != null then cfg.configFile else generatedConfigFile;

    configMergeScript = pkgs.callPackage ./configMergeScript.nix { };

    # config.yaml mode: group-writable (0660) when interactive users share this
    # DiTing_HOME via addToSystemPackages, so they can save settings through the
    ...
    documentDerivation = pkgs.runCommand "diting-documents" { } (
      ''
        mkdir -p $out
      '' + lib.concatStringsSep "\n" (
        lib.mapAttrsToList (name: value:
          ...
          else "cat > $out/${name} <<'DiTing_DOC_EOF'\n${value}\nDiTing_DOC_EOF"
        ) cfg.documents
      )
    );

    containerName = "diting-agent";
    containerDataDir = "/data";     # stateDir mount point inside container
    containerHomeDir = "/home/diting";
    ...
    # diting user + sudo on first boot (writable layer persists), then
    # drops privileges. Supports arbitrary base images (Debian, Alpine, etc).
    containerEntrypoint = pkgs.writeShellScript "diting-container-entrypoint" ''
      set -eu

      DiTing_UID="''${DiTing_UID:?DiTing_UID must be set}"
      DiTing_GID="''${DiTing_GID:?DiTing_GID must be set}"

      # ── Group: ensure a group with GID=$DiTing_GID exists ──
      # Check by GID (not name) to avoid collisions with pre-existing groups
      # (e.g. GID 100 = "users" on Ubuntu)
      EXISTING_GROUP=$(getent group "$DiTing_GID" 2>/dev/null | cut -d: -f1 || true)
      if [ -n "$EXISTING_GROUP" ]; then
        GROUP_NAME="$EXISTING_GROUP"
      else
        GROUP_NAME="diting"
        if command -v groupadd >/dev/null 2>&1; then
          groupadd -g "$DiTing_GID" "$GROUP_NAME"
        elif command -v addgroup >/dev/null 2>&1; then
          addgroup -g "$DiTing_GID" "$GROUP_NAME" 2>/dev/null || true
        fi
      fi

      # ── User: ensure a user with UID=$DiTing_UID exists ──
      PASSWD_ENTRY=$(getent passwd "$DiTing_UID" 2>/dev/null || true)
      if [ -n "$PASSWD_ENTRY" ]; then
        TARGET_USER=$(echo "$PASSWD_ENTRY" | cut -d: -f1)
        TARGET_HOME=$(echo "$PASSWD_ENTRY" | cut -d: -f6)
      else
        TARGET_USER="diting"
        TARGET_HOME="/home/diting"
        if command -v useradd >/dev/null 2>&1; then
          useradd -u "$DiTing_UID" -g "$DiTing_GID" -m -d "$TARGET_HOME" -s /bin/bash "$TARGET_USER"
        elif command -v adduser >/dev/null 2>&1; then
          adduser -u "$DiTing_UID" -D -h "$TARGET_HOME" -s /bin/sh -G "$GROUP_NAME" "$TARGET_USER" 2>/dev/null || true
        fi
      fi
      mkdir -p "$TARGET_HOME"
      chown "$DiTing_UID:$DiTing_GID" "$TARGET_HOME"
      chmod 0750 "$TARGET_HOME"

      # Ensure DiTing_HOME is owned by the target user.
      ...
      if [ -n "''${DiTing_HOME:-}" ] && [ -d "$DiTing_HOME" ]; then
        find "$DiTing_HOME" \! -user "$DiTing_UID" -exec chown "$DiTing_UID:$DiTing_GID" {} +
      fi
      ...
      if [ ! -f /var/lib/diting-tools-provisioned ] && command -v apt-get >/dev/null 2>&1; then
        ...
        touch /var/lib/diting-tools-provisioned
      fi

      if command -v sudo >/dev/null 2>&1 && [ ! -f /etc/sudoers.d/diting ]; then
        mkdir -p /etc/sudoers.d
        echo "$TARGET_USER ALL=(ALL) NOPASSWD:ALL" > /etc/sudoers.d/diting
        chmod 0440 /etc/sudoers.d/diting
      fi
      ...
      if command -v setpriv >/dev/null 2>&1; then
        exec setpriv --reuid="$DiTing_UID" --regid="$DiTing_GID" --init-groups "$@"
    ...
    # so they can update without recreation. Env vars go through $DiTing_HOME/.env.
    ...
    # Default: /var/lib/diting/workspace → /data/workspace.
  ...
  in {
    options.services.diting-agent = with lib; {
      enable = mkEnableOption "DiTing Agent gateway service";

      # ── Package ──────────────────────────────────────────────────────────
      package = mkOption {
        type = types.package;
        default = diting-agent;
        description = "The diting-agent package to use.";
      };
      ...
      stateDir = mkOption {
        type = types.str;
        default = "/var/lib/diting";
        description = "State directory. Contains .diting/ subdir (DiTing_HOME).";
      };
      ...
      settings = mkOption {
        ...
        default = { };
        description = ''
          Declarative DiTing config (attrset). Deep-merged across module
      ...
      environmentFiles = mkOption {
        ...
        description = ''
          ...
          Contents are merged into $DiTing_HOME/.env at activation time.
          DiTing reads this file on every startup via load_diting_dotenv().
        '';
      };
      ...
      environment = mkOption {
        ...
        default = { };
        description = ''
          Non-secret environment variables. Merged into $DiTing_HOME/.env
          ...
          options = {
            ...
            auth = mkOption {
              ...
              description = ''
                ...
                (remote MCP servers). Tokens are stored in $DiTing_HOME/mcp-tokens/.
              '';
            };
      ...
      extraArgs = mkOption {
        type = types.listOf types.str;
        default = [ ];
        description = "Extra command-line arguments for `diting gateway`.";
      };
      ...
      extraPackages = mkOption {
        ...
        description = ''
          ...
          Implemented via the diting user's per-user profile
      ...
      extraPlugins = mkOption {
        ...
        default = [ ];
        description = ''
          Directory-based plugin packages to symlink into the diting plugins
          directory. Each package should contain a plugin.yaml and __init__.py
          at its root. DiTing discovers these automatically on startup.
        '';
        example = literalExpression ''
          [
      ...
      extraPythonPackages = mkOption {
        ...
        description = ''
          ...
          diting_agent.plugins entry-point group. Each package must be built
          with the same Python interpreter as diting (python312).
        '';
        example = literalExpression ''
          [
      ...
      extraDependencyGroups = mkOption {
        ...
        description = ''
          ...
          Use this for optional extras already declared in diting-agent's
      ...
      addToSystemPackages = mkOption {
        ...
        default = false;
        description = ''
          Add the diting CLI to environment.systemPackages and export
          DiTing_HOME system-wide (via environment.variables) so interactive
      ...
      container = {
        ...
        hostUsers = mkOption {
          ...
          default = [ ];
          description = ''
            Interactive users who get a ~/.diting symlink to the service
            stateDir. These users are automatically added to the diting group.
          '';
          example = [ "sidbin" ];
        };
      };
    ...
    config = lib.mkIf cfg.enable (lib.mkMerge [
      ...
      (lib.mkIf (cfg.mcpServers != { }) {
        services.diting-agent.settings.mcp_servers = lib.mapAttrs (_name: srv:
          # Stdio transport
      ...
      # ── Host CLI ──────────────────────────────────────────────────────
      # Add the diting CLI to system PATH and export DiTing_HOME system-wide
      # so interactive shells share state (sessions, skills, cron) with the
      # gateway service instead of creating a separate ~/.diting/.
      (lib.mkIf cfg.addToSystemPackages {
        environment.systemPackages = [ effectivePackage ];
        environment.variables.DiTing_HOME = "${cfg.stateDir}/.diting";
      })
      ...
      # ── Assertions ─────────────────────────────────────────────────────
      {
        ...
        in [{
          ...
          message = "services.diting-agent.extraPlugins: duplicate plugin names detected: ${toString names}. If using fetchFromGitHub, set name = \"plugin-name\" to disambiguate.";
        }];
      }
      ...
      # Wire extraPackages into the diting user's per-user profile so the
      ...
      (lib.mkIf (cfg.container.enable && !cfg.addToSystemPackages && cfg.container.hostUsers != []) {
        warnings = [
          ''
            services.diting-agent: container.enable is true and container.hostUsers
            is set, but addToSystemPackages is false. Without a host-installed diting
            binary, container routing will not work for interactive users.
            Set addToSystemPackages = true or ensure diting is on PATH.
          ''
        ];
      })
      ...
      # ── Directories ───────────────────────────────────────────────────
      {
        systemd.tmpfiles.rules = [
          ...
          "d ${cfg.stateDir}/.diting        2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.diting/cron   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.diting/sessions 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.diting/logs   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.diting/memories 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.diting/plugins 2770 ${cfg.user} ${cfg.group} - -"
      ...
      {
        system.activationScripts."diting-agent-setup" = lib.stringAfter ([ "users" ] ++ lib.optional (config.system.activationScripts ? setupSecrets) "setupSecrets") ''
          # Ensure directories exist (activation runs before tmpfiles)
          mkdir -p ${cfg.stateDir}/.diting
          mkdir -p ${cfg.stateDir}/home
          mkdir -p ${cfg.workingDirectory}
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir} ${cfg.stateDir}/.diting ${cfg.stateDir}/home ${cfg.workingDirectory}
          chmod 2770 ${cfg.stateDir} ${cfg.stateDir}/.diting ${cfg.workingDirectory}
          ...
          find ${cfg.stateDir}/.diting -maxdepth 1 \
            \( -name "*.db" -o -name "*.db-wal" -o -name "*.db-shm" -o -name "SOUL.md" \) \
            -exec chmod g+rw {} + 2>/dev/null || true
          for _subdir in cron sessions logs memories plugins; do
            mkdir -p "${cfg.stateDir}/.diting/$_subdir"
            chown ${cfg.user}:${cfg.group} "${cfg.stateDir}/.diting/$_subdir"
            chmod 2770 "${cfg.stateDir}/.diting/$_subdir"
            find "${cfg.stateDir}/.diting/$_subdir" -type f \
              -exec chmod g+rw {} + 2>/dev/null || true
          done
          ...
          # diting-group users can save settings via the CLI/TUI, else 0640).
          ${if cfg.configFile != null then ''
            install -o ${cfg.user} -g ${cfg.group} -m ${configYamlMode} -D ${configFile} ${cfg.stateDir}/.diting/config.yaml
          '' else ''
            ${configMergeScript} ${generatedConfigFile} ${cfg.stateDir}/.diting/config.yaml
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.diting/config.yaml
            chmod ${configYamlMode} ${cfg.stateDir}/.diting/config.yaml
          ''}

          # Managed mode marker (so interactive shells also detect NixOS management)
          touch ${cfg.stateDir}/.diting/.managed
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.diting/.managed
          chmod 0644 ${cfg.stateDir}/.diting/.managed
          ...
          ${if cfg.container.enable then ''
            cat > ${cfg.stateDir}/.diting/.container-mode <<'DiTing_CONTAINER_MODE_EOF'
    # Written by NixOS activation script. Do not edit manually.
    backend=${cfg.container.backend}
    container_name=${containerName}
    exec_user=${cfg.user}
    diting_bin=${containerDataDir}/current-package/bin/diting
    DiTing_CONTAINER_MODE_EOF
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.diting/.container-mode
            chmod 0644 ${cfg.stateDir}/.diting/.container-mode
          '' else ''
            rm -f ${cfg.stateDir}/.diting/.container-mode

            # Remove symlink bridge for hostUsers
            ${lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.diting";
              in ''
                if [ -L "${symlinkPath}" ] && [ "$(readlink "${symlinkPath}")" = "${cfg.stateDir}/.diting" ]; then
                  rm -f "${symlinkPath}"
                  echo "diting-agent: removed symlink ${symlinkPath}"
                fi
              '') cfg.container.hostUsers)}
          ''}

          # ── Symlink bridge for interactive users ───────────────────────
          # Create ~/.diting -> stateDir/.diting for each hostUser so the
          ...
          ${lib.optionalString cfg.container.enable
            (lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.diting";
                target = "${cfg.stateDir}/.diting";
              in ''
                if [ -d "${symlinkPath}" ] && [ ! -L "${symlinkPath}" ]; then
                  # Real directory — back it up, then create symlink.
                  # (ln -sfn cannot atomically replace a directory.)
                  _backup="${symlinkPath}.bak.$(date +%s)"
                  echo "diting-agent: backing up existing ${symlinkPath} to $_backup"
          ...
          ${lib.optionalString (cfg.authFile != null) ''
            ${if cfg.authFileForceOverwrite then ''
              install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.diting/auth.json
            '' else ''
              if [ ! -f ${cfg.stateDir}/.diting/auth.json ]; then
                install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.diting/auth.json
              fi
            ''}
          ''}
          ...
          # DiTing reads $DiTing_HOME/.env at startup via load_diting_dotenv(),
          # so this is the single source of truth for both native and container mode.
          ${lib.optionalString (cfg.environment != {} || cfg.environmentFiles != []) ''
            ENV_FILE="${cfg.stateDir}/.diting/.env"
            install -o ${cfg.user} -g ${cfg.group} -m 0640 /dev/null "$ENV_FILE"
            cat > "$ENV_FILE" <<'DiTing_NIX_ENV_EOF'
    ${envFileContent}
    DiTing_NIX_ENV_EOF
            ${lib.concatStringsSep "\n" (map (f: ''
              if [ -f "${f}" ]; then
        ...
        find ${cfg.stateDir}/.diting/plugins -maxdepth 1 -type l -name 'nix-managed-*' -delete 2>/dev/null || true
        ...
        ${lib.concatStringsSep "\n" (map (plugin:
          ...
          in ''
            ...
            ln -sfn ${plugin} ${cfg.stateDir}/.diting/plugins/nix-managed-${name}
            chown -h ${cfg.user}:${cfg.group} ${cfg.stateDir}/.diting/plugins/nix-managed-${name}
          '') cfg.extraPlugins)}
        '';
      }
      ...
      (lib.mkIf (!cfg.container.enable) {
        systemd.services.diting-agent = {
          description = "DiTing Agent Gateway";
          ...
          environment = {
            HOME = cfg.stateDir;
            DiTing_HOME = "${cfg.stateDir}/.diting";
            DiTing_MANAGED = "true";
          ...
          serviceConfig = {
            ...
            # $DiTing_HOME/.env by the activation script. load_diting_dotenv()
            # reads them at Python startup — no systemd EnvironmentFile needed.

            ExecStart = lib.concatStringsSep " " ([
              "${effectivePackage}/bin/diting"
              "gateway"
            ] ++ cfg.extraArgs);
            ...
            # so interactive users in the diting group can read/write them.
            UMask = "0007";
      ...
      (lib.mkIf cfg.container.enable {
        ...
        systemd.services.diting-agent = {
          description = "DiTing Agent Gateway (container)";
          wantedBy = [ "multi-user.target" ];
          after = [ "network-online.target" ]
          ...
          preStart = ''
            ...
            if [ "$NEED_CREATE" = "true" ]; then
              ...
              DiTing_UID=$(${pkgs.coreutils}/bin/id -u ${cfg.user})
              DiTing_GID=$(${pkgs.coreutils}/bin/id -g ${cfg.user})
              ...
              ${containerBin} create \
                ...
                --env DiTing_UID="$DiTing_UID" \
                --env DiTing_GID="$DiTing_GID" \
                --env DiTing_HOME=${containerDataDir}/.diting \
                --env DiTing_MANAGED=true \
                --env HOME=${containerHomeDir} \
                --env MESSAGING_CWD=${containerWorkDir} \
                ${lib.concatStringsSep " " cfg.container.extraOptions} \
                ${cfg.container.image} \
                ${containerDataDir}/current-package/bin/diting gateway run --replace ${lib.concatStringsSep " " cfg.extraArgs}
