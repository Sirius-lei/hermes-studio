...
{ ... }:
{
  perSystem =
    ...
    let
      ...
      ditingNpmLib = self'.packages.default.passthru.DiTingNpmLib;
      ...
      # Non-npm packages may have their own devShellHook (e.g. diting-agent
    ...
    in
    {
      devShells.default = pkgs.mkShell {
        packages =
          with pkgs;
          [
            (pkgs.runCommand "diting" { } ''
              mkdir -p $out/bin
              install -Dm755 ${../diting} $out/bin/diting
            '')
        ...
        shellHook = ''
          ${combinedNonNpm}
          ${ditingNpmLib.mkNpmDevShellHook npmPackageJsonPaths}

          # for the devshell to pick up the src
          export DiTing_PYTHON_SRC_ROOT=$(git rev-parse --show-toplevel)
          echo "DiTing Agent dev shell in $DiTing_PYTHON_SRC_ROOT"
          echo "Ready. Run 'diting' or 'sandbox diting' to start."
        '';
      };
    };
}
