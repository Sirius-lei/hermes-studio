# nix/desktop.nix — DiTing Desktop (Electron) app build + wrapper
#
# `ditingAgent` is the fully-built `.#default` package — it ships the
# `diting` binary with the venv, runtime PATH, bundled skills/plugins, etc.
# already wired up.  We point the desktop at it via the existing
# `DiTing_DESKTOP_DiTing` override env var, so the desktop's resolver
# uses our fully wrapped binary at step 4 ("existing DiTing CLI").
...
let
  npm = ditingNpmLib.mkNpmPassthru {
    folder = "apps/desktop";
    attr = "desktop";
    pname = "diting-desktop";
  };
  ...
  targetPlatform =
    if stdenv.hostPlatform.isDarwin then
      ...
      throw "diting-desktop: unsupported host platform for node-pty staging";
  ...
  targetArch =
    if stdenv.hostPlatform.isAarch64 then
      ...
      throw "diting-desktop: unsupported host arch for node-pty staging";

  # Build the renderer (dist/ + electron/ + package.json).
  renderer = pkgs.buildNpmPackage (
    npm
    // {
      pname = "diting-desktop-renderer";
      inherit version;
      doCheck = true;
...
stdenv.mkDerivation {
  ...
  installPhase = ''
    ...
    mkdir -p $out/share/diting-desktop $out/bin
    cp -r ${renderer}/* $out/share/diting-desktop/
    ...
    substituteInPlace $out/share/diting-desktop/dist/electron-main.mjs \
      --replace-fail "process.resourcesPath" "'$out/share/diting-desktop'"

    # Wrap the nixpkgs electron binary to launch our app.  Set
    # DiTing_DESKTOP_DiTing to the absolute path of the nix-built `diting`
    # binary so the desktop's resolver step 4 ("existing DiTing CLI on
    # PATH") uses our fully wrapped binary — venv with all deps,
    # bundled skills/plugins, runtime PATH (ripgrep/git/ffmpeg/etc).
    # No reimplementation of the agent resolver in the wrapper.
    makeWrapper ${lib.getExe electron} $out/bin/diting-desktop \
      --add-flags "$out/share/diting-desktop" \
      --set DiTing_DESKTOP_DiTing "${lib.getExe ditingAgent}" \
  ...
  meta = with lib; {
    description = "Native Electron desktop shell for DiTing Agent";
    homepage = "https://github.com/NousResearch/diting-agent";
    license = licenses.mit;
    platforms = platforms.unix;
    mainProgram = "diting-desktop";
  };
}
