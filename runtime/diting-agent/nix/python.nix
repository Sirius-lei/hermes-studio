...
let
  ...
  editableOverlay = workspace.mkEditablePyprojectOverlay {
    root = "$DiTing_PYTHON_SRC_ROOT"; # resolved at shellHook time
  };
  ...
  editableSet = pythonSet.overrideScope (
    lib.composeManyExtensions [
      editableOverlay
      (final: prev: {
        diting-agent = prev.diting-agent.overrideAttrs (old: {
...
in
{
  venv = pythonSet.mkVirtualEnv "diting-agent-env" {
    diting-agent = dependency-groups;
  };
  editableVenv = editableSet.mkVirtualEnv "diting-agent-editable-env" {
    diting-agent = dependency-groups;
  };
}
