...
{ inputs, ... }: {
  perSystem = { pkgs, lib, self', ... }:
    let
      diting-agent = self'.packages.default;
      ditingVenv = diting-agent.DiTingVenv;

      configMergeScript = pkgs.callPackage ./configMergeScript.nix { };

      # Auto-generated config key reference — always in sync with Python
      configKeys = pkgs.runCommand "diting-config-keys" {} ''
        set -euo pipefail
        export HOME=$TMPDIR
        ${ditingVenv}/bin/python3 -c '
import json, sys
...
' > $out
    ...
    in {
      ...
      checks = {
        ...
        in pkgs.runCommand "diting-cross-eval" { } (
          if failures != [] then
        ...
        build-package = pkgs.runCommand "diting-build-package" { } ''
          echo "PASS: package built at ${diting-agent}"
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify the devShell builds successfully (cross-platform).
        build-devshell = pkgs.runCommand "diting-build-devshell" { } ''
        ...
        package-contents = pkgs.runCommand "diting-package-contents" { } ''
          set -e
          echo "=== Checking binaries ==="
          test -x ${diting-agent}/bin/diting || (echo "FAIL: diting binary missing"; exit 1)
          test -x ${diting-agent}/bin/diting-agent || (echo "FAIL: diting-agent binary missing"; exit 1)
          echo "PASS: All binaries present"

          echo "=== Checking version ==="
          ${diting-agent}/bin/diting version 2>&1 | grep -qi "diting" || (echo "FAIL: version check"; exit 1)
        ...
        entry-points-sync = pkgs.runCommand "diting-entry-points-sync" { } ''
          set -e
          echo "=== Checking entry points match pyproject.toml [project.scripts] ==="
          for bin in diting diting-agent diting-acp; do
            test -x ${diting-agent}/bin/$bin || (echo "FAIL: $bin binary missing from Nix package"; exit 1)
        ...
        cli-commands = pkgs.runCommand "diting-cli-commands" { } ''
          set -e
          export HOME=$(mktemp -d)

          echo "=== Checking diting --help ==="
          ${diting-agent}/bin/diting --help 2>&1 | grep -q "gateway" || (echo "FAIL: gateway subcommand missing"; exit 1)
          ${diting-agent}/bin/diting --help 2>&1 | grep -q "config" || (echo "FAIL: config subcommand missing"; exit 1)
        ...
        bundled-skills = pkgs.runCommand "diting-bundled-skills" { } ''
          set -e
          echo "=== Checking bundled skills ==="
          test -d ${diting-agent}/share/diting-agent/skills || (echo "FAIL: skills directory missing"; exit 1)
          echo "PASS: skills directory exists"

          SKILL_COUNT=$(find ${diting-agent}/share/diting-agent/skills -name "SKILL.md" | wc -l)
          test "$SKILL_COUNT" -gt 0 || (echo "FAIL: no SKILL.md files found in skills directory"; exit 1)
          echo "PASS: $SKILL_COUNT bundled skills found"

          grep -q "DiTing_BUNDLED_SKILLS" ${diting-agent}/bin/diting || \
            (echo "FAIL: DiTing_BUNDLED_SKILLS not in wrapper"; exit 1)
          echo "PASS: DiTing_BUNDLED_SKILLS set in wrapper"
        ...
        bundled-plugins = pkgs.runCommand "diting-bundled-plugins" { } ''
          set -e
          echo "=== Checking bundled plugins ==="
          test -d ${diting-agent}/share/diting-agent/plugins || (echo "FAIL: plugins directory missing"; exit 1)
          echo "PASS: plugins directory exists"

          test -f ${diting-agent}/share/diting-agent/plugins/platforms/irc/plugin.yaml || \
            (echo "FAIL: irc plugin manifest missing"; exit 1)
          echo "PASS: irc plugin manifest present"

          grep -q "DiTing_BUNDLED_PLUGINS" ${diting-agent}/bin/diting || \
            (echo "FAIL: DiTing_BUNDLED_PLUGINS not in wrapper"; exit 1)
          echo "PASS: DiTing_BUNDLED_PLUGINS set in wrapper"
        ...
        bundled-locales = pkgs.runCommand "diting-bundled-locales" { } ''
          set -e
          echo "=== Checking bundled locales ==="
          test -d ${diting-agent}/share/diting-agent/locales || (echo "FAIL: locales directory missing"; exit 1)
          echo "PASS: locales directory exists"

          LOC_COUNT=$(find ${diting-agent}/share/diting-agent/locales -name "*.yaml" | wc -l)
          test "$LOC_COUNT" -ge 16 || (echo "FAIL: expected >=16 catalogs, found $LOC_COUNT"; exit 1)
          echo "PASS: $LOC_COUNT locale catalogs found"

          test -f ${diting-agent}/share/diting-agent/locales/en.yaml || (echo "FAIL: en.yaml missing"; exit 1)
          echo "PASS: en.yaml present"

          grep -q "DiTing_BUNDLED_LOCALES" ${diting-agent}/bin/diting || \
            (echo "FAIL: DiTing_BUNDLED_LOCALES not in wrapper"; exit 1)
          echo "PASS: DiTing_BUNDLED_LOCALES set in wrapper"

          echo "=== Rendering via the wrapper override (DiTing_BUNDLED_LOCALES) ==="
          export HOME=$(mktemp -d)
          RENDERED=$(cd "$HOME" && DiTing_BUNDLED_LOCALES=${diting-agent}/share/diting-agent/locales \
            ${ditingVenv}/bin/python3 -c "from agent import i18n; print(i18n.t('gateway.reset.header_default', lang='en'))")
          echo "rendered: $RENDERED"
          test "$RENDERED" != "gateway.reset.header_default" || (echo "FAIL: i18n returned the raw key with DiTing_BUNDLED_LOCALES set"; exit 1)
          ...
          BARE_DIR=$(cd "$HOME" && ${ditingVenv}/bin/python3 -c "from agent import i18n; print(i18n._locales_dir())")
          BARE=$(cd "$HOME" && ${ditingVenv}/bin/python3 -c "from agent import i18n; print(i18n.t('gateway.reset.header_default', lang='en'))")
          echo "resolved dir (no env var): $BARE_DIR"
          echo "rendered: $BARE"
          test "$BARE" != "gateway.reset.header_default" || \
            (echo "FAIL: sealed venv could not resolve locales without DiTing_BUNDLED_LOCALES — data-files materialization regressed"; exit 1)
        ...
        bundled-tui = pkgs.runCommand "diting-bundled-tui" { } ''
          set -e
          echo "=== Checking bundled TUI ==="
          test -d ${diting-agent}/ui-tui || (echo "FAIL: ui-tui directory missing"; exit 1)
          echo "PASS: ui-tui directory exists"

          test -f ${diting-agent}/ui-tui/dist/entry.js || (echo "FAIL: compiled entry.js missing"; exit 1)
          echo "PASS: compiled entry.js present"

          # self-contained bundle; no runtime node_modules expected

          grep -q "DiTing_TUI_DIR" ${diting-agent}/bin/diting || \
            (echo "FAIL: DiTing_TUI_DIR not in wrapper"; exit 1)
          echo "PASS: DiTing_TUI_DIR set in wrapper"
        ...
        # Verify DiTing_NODE is set in wrapper and points to Node 20+
        # (string-width uses the /v regex flag which requires Node 20+)
        diting-node = pkgs.runCommand "diting-node-version" { } ''
          set -e
          echo "=== Checking DiTing_NODE in wrapper ==="
          grep -q "DiTing_NODE" ${diting-agent}/bin/diting || \
            (echo "FAIL: DiTing_NODE not set in wrapper"; exit 1)
          echo "PASS: DiTing_NODE present in wrapper"

          DiTing_NODE=$(sed -n "s/^export DiTing_NODE='\(.*\)'/\1/p" ${diting-agent}/bin/diting)
          test -x "$DiTing_NODE" || (echo "FAIL: DiTing_NODE=$DiTing_NODE not executable"; exit 1)
          echo "PASS: DiTing_NODE executable at $DiTing_NODE"

          NODE_MAJOR=$("$DiTing_NODE" --version | sed 's/^v//' | cut -d. -f1)
          test "$NODE_MAJOR" -ge 20 || \
            (echo "FAIL: Node v$NODE_MAJOR < 20, TUI needs /v regex flag support"; exit 1)
          echo "PASS: Node v$NODE_MAJOR >= 20"

          echo "=== All DiTing_NODE checks passed ==="
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify DiTing_MANAGED guard works on all mutation commands
        managed-guard = pkgs.runCommand "diting-managed-guard" { } ''
          set -e
          export HOME=$(mktemp -d)
          ...
          check_blocked() {
            local label="$1"
            shift
            OUTPUT=$(DiTing_MANAGED=true "$@" 2>&1 || true)
          ...
          echo "=== Checking DiTing_MANAGED guards ==="
          check_blocked "config set" ${diting-agent}/bin/diting config set model foo
          check_blocked "config edit" ${diting-agent}/bin/diting config edit
        ...
        extra-python-packages = let
          ...
          ditingWithExtra = diting-agent.override {
            extraPythonPackages = [ testPkg ];
          };
        in pkgs.runCommand "diting-extra-python-packages" { } ''
          set -e
          echo "=== Checking extraPythonPackages PYTHONPATH injection ==="

          grep -q "PYTHONPATH" ${ditingWithExtra}/bin/diting || \
            (echo "FAIL: PYTHONPATH not in wrapper"; exit 1)
          echo "PASS: PYTHONPATH present in wrapper"

          grep -q "${testPkg}" ${ditingWithExtra}/bin/diting || \
          ...
          if grep -q "PYTHONPATH" ${diting-agent}/bin/diting; then
        ...
        extra-dependency-groups = let
          ditingWithGroups = diting-agent.override {
            extraDependencyGroups = [ "honcho" ];
          };
        in pkgs.runCommand "diting-extra-dependency-groups" { } ''
          set -e
          ...
          echo "derivation: ${ditingWithGroups}"
          echo "venv: ${ditingWithGroups.DiTingVenv}"
        ...
        messaging-variant = pkgs.runCommand "diting-messaging-variant" { } ''
          set -e
          echo "=== Checking discord.py importable from messaging variant ==="
          ${self'.packages.messaging.DiTingVenv}/bin/python3 -c \
        ...
        in pkgs.runCommand "diting-config-roundtrip" {
          ...
          merge_and_load() {
            local diting_home="$1"
            export DiTing_HOME="$diting_home"
            ${configMergeScript} ${nixSettings} "$diting_home/config.yaml"
            ${ditingVenv}/bin/python3 -c '
import json, sys
