...
React + Ink terminal UI for DiTing. TypeScript owns the screen. Python owns sessions, tools, model calls, and most command logic.
...
Interpreter resolution order is: `DiTing_PYTHON` → `PYTHON` → `$VIRTUAL_ENV/bin/python` → `./.venv/bin/python` → `./venv/bin/python` → `python3` (or `python` on Windows).
...
- `src/app/setupHandoff.ts` — launches external `diting setup`, suspends Ink while it runs, opens a new session on success
...
- Input history is stored in `~/.diting/.diting_history` or under `DiTing_HOME`.
...
### Setup (`setup.ts`)
`/setup` — launches external `diting setup` wizard, suspends Ink while it runs
...
```text
ui-tui/
  packages/diting-ink/   forked Ink renderer (local dep)
  src/
    ...
    app/
      ...
      setupHandoff.ts               launches external diting setup, suspends Ink while it runs
    ...
    lib/
      ...
      parentLog.ts               append-only log to ~/.diting/tui-parent.log
    ...
    types/
      diting-ink.d.ts            type declarations for @diting/ink
...
tui_gateway/
  ...
  slash_worker.py        persistent DiTingCLI subprocess for slash commands
```
