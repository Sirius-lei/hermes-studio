...
const logDir = join(process.env.DiTing_HOME?.trim() || join(homedir(), '.diting'), 'logs')
...
// a real ~/.diting (tests must stay hermetic — see AGENTS.md).
...
export function recordParentLifecycle(line: string): void {
  ...
  try {
    ...
    if (!warned) {
      warned = true
      process.stderr.write('diting-tui: parent lifecycle log unavailable\n')
    }
  }
}
