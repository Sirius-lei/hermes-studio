...
export function shouldForceTruecolor(env: NodeJS.ProcessEnv = process.env): boolean {
  const override = (env.DiTing_TUI_TRUECOLOR ?? '').trim()
...
if (shouldForceTruecolor()) {
  ...
  // incorrectly. Keep DiTing on the safer TERM-driven 256-color path unless
  // users explicitly opt back in via DiTing_TUI_TRUECOLOR=1.
