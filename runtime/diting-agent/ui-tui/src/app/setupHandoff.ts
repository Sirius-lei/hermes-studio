import type { RunExternalProcess } from '@diting/ink'
...
export async function runExternalSetup({ args, ctx, done, launcher, suspend }: RunExternalSetupOptions) {
  ...
  transcript.sys(`launching \`diting ${args.join(' ')}\`…`)
  ...
  if (result.error) {
    transcript.sys(`error launching diting: ${result.error}`)
  ...
  if (result.code !== 0) {
    transcript.sys(`diting ${args[0]} exited with code ${result.code}`)
    patchUiState({ status: 'setup required' })
