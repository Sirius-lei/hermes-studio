"""Windows UTF-8 bootstrap for DiTing entry points.
...
should be imported at the very top of every DiTing entry point
(``diting``, ``diting-agent``, ``diting-acp``, ``python -m gateway.run``,
...
def harden_import_path(src_root: str | None = None) -> None:
    """Stop a package in the current directory from shadowing DiTing modules.

    DiTing ships top-level modules with common names (``utils``, ``proxy``,
    ...
    We drop the relative forms outright, then force the real DiTing source root
    ...
    """
    root = src_root or os.environ.get("DiTing_PYTHON_SRC_ROOT") or os.path.dirname(
...
def activate_durable_lazy_target() -> None:
    ...
    (``DiTing_LAZY_INSTALL_TARGET``, e.g. ``/opt/data/lazy-packages``).
    ...
    """
    if not os.environ.get("DiTing_LAZY_INSTALL_TARGET", "").strip():
        return
    try:
...
# Apply on import — entry points just need ``import diting_bootstrap``
# (or ``from diting_bootstrap import apply_windows_utf8_bootstrap``) at
