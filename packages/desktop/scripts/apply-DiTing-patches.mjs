#!/usr/bin/env node
// Apply locally-curated patches to DiTing-agent inside the bundled venv.
// Each patch is idempotent: a marker string is searched for first, and the
// edit is skipped if the patch is already in place.
//
// Run after `install-DiTing.mjs`. Designed to be safe to re-run.
...
const sitePkgs = process.env.DiTing_AGENT_SITE_PACKAGES ?? (
  TARGET_OS === 'win32'
...
if (!existsSync(dtPath)) {
  console.error(`dingtalk.py not found at ${dtPath} — is DiTing-agent installed?`)
  process.exit(1)
}
...
function validateSitecustomizePatches(text) {
  if (text.includes('# patch:brotlicffi-error-compat') && !text.includes('_DiTing_brotlicffi.error')) {
  ...
  if (
    ...
    && !text.includes('_DiTing_apply_hidden_process_options')
  ) {
...
// NOTE: the former `dt-pre-start` patch was retired — DiTing-agent now ships
...
# patch:browser-stdout-decode-fallback
def _DiTing_read_browser_output(path: str) -> str:
...
`,
  ...
  for (const [id, find, replace] of [
    [
      ...
      `            # patch:browser-fallback-stdout-read
            stdout = _DiTing_read_browser_output(stdout_path).strip()`,
    ],
    [
      ...
      `            # patch:browser-command-stdout-read
            stdout = _DiTing_read_browser_output(stdout_path)
            stderr = _DiTing_read_browser_output(stderr_path)`,
    ],
  ]) {
    browserSrc = patchText(
      browserSrc,
  ...
  const readsBrowserOutput = browserSrc.includes('_DiTing_read_browser_output(')
  const definesBrowserOutputReader = browserSrc.includes('def _DiTing_read_browser_output')
  if (readsBrowserOutput && !definesBrowserOutputReader) {
    console.error(
      '  ✗ browser stdout decode fallback is incomplete: browser_tool.py calls _DiTing_read_browser_output but does not define it',
    )
    process.exit(1)
  }
...
try:
    import brotlicffi as _DiTing_brotlicffi
    if not hasattr(_DiTing_brotlicffi, "error"):
        _DiTing_brotlicffi.error = (
            getattr(_DiTing_brotlicffi, "Error", None)
            or getattr(_DiTing_brotlicffi, "BrotliError", None)
            or Exception
        )
except Exception:
    pass
...
try:
    import os as _DiTing_os
    if _DiTing_os.name == "nt" and _DiTing_os.environ.get("DiTing_DESKTOP", "").strip().lower() == "true":
        import asyncio as _DiTing_asyncio
        import subprocess as _DiTing_subprocess
        if not getattr(_DiTing_subprocess, "_DiTing_desktop_hidden_defaults_installed", False):
            _DiTing_create_no_window = getattr(_DiTing_subprocess, "CREATE_NO_WINDOW", 0) or 0x08000000

            def _DiTing_apply_hidden_process_options(kwargs):
                flags = kwargs.get("creationflags", 0) or 0
                try:
                    kwargs["creationflags"] = int(flags) | _DiTing_create_no_window
                except Exception:
                    kwargs["creationflags"] = _DiTing_create_no_window

                startupinfo = kwargs.get("startupinfo")
                if startupinfo is None:
                    try:
                        startupinfo = _DiTing_subprocess.STARTUPINFO()
                    except Exception:
                        return
                    kwargs["startupinfo"] = startupinfo
                try:
                    startupinfo.dwFlags |= getattr(_DiTing_subprocess, "STARTF_USESHOWWINDOW", 1)
                    startupinfo.wShowWindow = getattr(_DiTing_subprocess, "SW_HIDE", 0)
                except Exception:
                    pass

            _DiTing_original_popen = _DiTing_subprocess.Popen
            _DiTing_original_create_subprocess_exec = _DiTing_asyncio.create_subprocess_exec
            _DiTing_original_create_subprocess_shell = _DiTing_asyncio.create_subprocess_shell

            class _DiTingHiddenPopen(_DiTing_original_popen):
                def __init__(self, *args, **kwargs):
                    _DiTing_apply_hidden_process_options(kwargs)
                    super().__init__(*args, **kwargs)

            async def _DiTing_hidden_create_subprocess_exec(*args, **kwargs):
                _DiTing_apply_hidden_process_options(kwargs)
                return await _DiTing_original_create_subprocess_exec(*args, **kwargs)

            async def _DiTing_hidden_create_subprocess_shell(*args, **kwargs):
                _DiTing_apply_hidden_process_options(kwargs)
                return await _DiTing_original_create_subprocess_shell(*args, **kwargs)

            _DiTing_subprocess.Popen = _DiTingHiddenPopen
            _DiTing_asyncio.create_subprocess_exec = _DiTing_hidden_create_subprocess_exec
            _DiTing_asyncio.create_subprocess_shell = _DiTing_hidden_create_subprocess_shell
            _DiTing_subprocess._DiTing_desktop_hidden_defaults_installed = True
except Exception:
    pass
`
