#!/usr/bin/env node
// Install diting-agent into the bundled Python at resources/python/<os>-<arch>/.
...
import { DiTingVersion } from './runtime-config.mjs'
...
// Match the packaged runtime to the channel list exposed at /DiTing/channels.
...
// not expose dedicated diting-agent extras; their deps are covered by base or
...
const DiTing_PACKAGE = process.env.DiTing_PACKAGE || `diting-agent[${DiTing_EXTRAS}]==${DiTing_VERSION}`
const EXTRA_PYTHON_PACKAGES = splitPackageList(
  process.env.DiTing_EXTRA_PYTHON_PACKAGES || [
    'websockets',
    'mautrix==0.21.0',
    'Markdown==3.10.2',
    'aiosqlite==0.22.1',
...
const BROWSER_PACKAGES = splitPackageList(
  process.env.DiTing_BROWSER_PACKAGES || 'agent-browser@^0.26.0 @askjo/camofox-browser@^1.5.2',
)
const CHROME_FOR_TESTING_VERSION = (
  process.env.DiTing_CHROME_FOR_TESTING_VERSION
  || process.env.DiTing_WINDOWS_CHROME_FOR_TESTING_VERSION
  || '149.0.7827.55'
).trim()
const SKIP_BROWSER_RUNTIME = process.env.DiTing_SKIP_BROWSER_RUNTIME === '1'
  || process.env.DiTing_SKIP_BROWSER_RUNTIME?.toLowerCase() === 'true'
...
function browserRuntimeEnv(includeAgentBrowser = true) {
  ...
  const env = {
    ...
    DiTing_AGENT_NODE: TARGET_OS === 'win32' ? resolve(NODE_DIR, 'node.exe') : resolve(NODE_DIR, 'bin', 'node'),
    DiTing_AGENT_NODE_ROOT: NODE_DIR,
    AGENT_BROWSER_HOME,
...
function chromeForTestingUrl() {
  if (process.env.DiTing_CHROME_FOR_TESTING_URL?.trim()) {
    return process.env.DiTing_CHROME_FOR_TESTING_URL.trim()
  }
  if (process.env.DiTing_WINDOWS_CHROME_FOR_TESTING_URL?.trim()) {
    return process.env.DiTing_WINDOWS_CHROME_FOR_TESTING_URL.trim()
  }
...
function pinChromeForTestingBundle() {
  ...
  const tmpRoot = mkdtempSync(join(tmpdir(), 'DiTing-cft-'))
...
function removeBrokenDashboardAuthPlugin() {
  if (pythonModuleExists('diting_cli.dashboard_auth')) return
  ...
  console.warn(
    '! Removed bundled dashboard_auth/nous plugin because diting_cli.dashboard_auth is missing from the diting-agent package',
  )
}

function installBrowserRuntime() {
  if (SKIP_BROWSER_RUNTIME) {
    console.warn('! Skipping bundled browser runtime because DiTing_SKIP_BROWSER_RUNTIME is set')
    return
  }
  if (BROWSER_PACKAGES.length === 0) {
    console.warn('! Skipping bundled browser runtime because DiTing_BROWSER_PACKAGES is empty')
    return
  }
...
installPythonPackages([DiTing_PACKAGE], 'diting-agent')
...
const DiTingBin = TARGET_OS === 'win32'
  ? resolve(PY_DIR, 'Scripts', 'DiTing.exe')
  : resolve(PY_DIR, 'bin', 'DiTing')
const DiTingCheckCommand = TARGET_OS === 'win32' ? pyBin : DiTingBin
const DiTingCheckArgs = TARGET_OS === 'win32' ? ['-m', 'diting_cli.main', '--version'] : ['--version']

if (!existsSync(DiTingBin)) {
  console.error(`DiTing binary not found at ${DiTingBin} after install`)
  process.exit(1)
}

// diting-web-ui's agent-bridge searches for `run_agent.py` at <python_root>/run_agent.py
...
if (TARGET_OS === 'win32') {
  ...
  const cmdPath = resolve(PY_DIR, 'Scripts', 'DiTing.cmd')
  writeFileSync(
    cmdPath,
    [
      '@echo off',
      'set "PY=%~dp0..\\python.exe"',
      '"%PY%" -m diting_cli.main %*',
    ].join('\r\n'),
  )
} else {
  const launcher = [
    ...
    'exec "$DIR/python3" -m diting_cli.main "$@"',
    '',
  ].join('\n')
  writeFileSync(DiTingBin, launcher, { mode: 0o755 })
  chmodSync(DiTingBin, 0o755)
  // Same for diting-agent / DiTing-acp (they all just dispatch into modules)
  for (const [name, mod] of [
    ['diting-agent', 'run_agent'],
    ['DiTing-acp', 'acp_adapter.entry'],
  ]) {
    const p = resolve(PY_DIR, 'bin', name)
    if (existsSync(p)) {
      writeFileSync(p, launcher.replace('diting_cli.main', mod), { mode: 0o755 })
      chmodSync(p, 0o755)
    }
  }
}

console.log(`✓ DiTing installed at ${DiTingBin} (relocatable launcher)`)

run(DiTingCheckCommand, DiTingCheckArgs)
...
if (SKIP_BROWSER_RUNTIME) {
  console.log('✓ DiTing Python, MCP, and websockets checks passed; browser runtime skipped')
} else {
  console.log('✓ DiTing Python, MCP, websockets, agent-browser, and Chromium checks passed')
}
