export default {
  ...
  sidebar: {
    ...
    connected: '연결됨',
    disconnected: '연결 끊김',
    updateTip: '터미널에서 "diting-web-ui update"를 실행하여 업데이트하세요',
  ...
  runtimeVersions: {
    ...
    deleteVersion: '삭제',
    deleteRuntimeConfirm: 'DiTing Runtime {version}을 삭제할까요?',
    ...
    activateSuccess: '버전 선택이 저장되었습니다. DiTing Studio를 다시 시작하면 적용됩니다.',
    jobStatus: {
      queued: '대기 중',
      running: '실행 중',
      completed: '완료',
  ...
  chat: {
    ...
    openHistory: '기록 열기',
    DiTingHistory: 'DiTing 기록',
    historyScopeHint: '현재 profile의 DiTing 기록 세션을 소스별로 읽기 전용으로 봅니다.',
    noSessions: '세션 없음',
    newChat: '새 채팅',
    agent: 'Agent',
    ...
    searchScope: '검색 범위: Web UI 로컬 세션 DB만 포함하며 읽기 전용 DiTing 기록 세션은 포함하지 않습니다.',
  ...
  skills: {
    ...
    writeApprovalDescription: 'DiTing Agent가 보류한 메모리와 스킬 쓰기를 승인하거나 거부합니다.',
    writeApprovalRefresh: '새로고침',
    writeApprovalEmpty: '승인 대기 중인 쓰기가 없습니다.',
    writeApprovalUnsupported: '이 DiTing Agent 버전은 쓰기 승인을 지원하지 않습니다. 이 기능을 사용하려면 DiTing Agent를 업그레이드하세요.',
  ...
  plugins: {
    title: '플러그인',
    refresh: '새로고침',
    notice: '탐색 가능한 DiTing 플러그인 manifest의 읽기 전용 인벤토리입니다. 탐색 메타데이터는 플러그인 코드를 로드하지 않고 읽습니다. v1의 관리 작업은 CLI에 유지되며, 변경 사항은 새 DiTing 세션에서 적용됩니다.',
  ...
  models: {
    ...
    copilotAddDetected: '이 컴퓨터에서 GitHub Copilot OAuth 토큰이 감지되었습니다. 추가를 클릭하여 DiTing에서 Copilot을 활성화하세요.',
    copilotAddSourceEnv: '출처: ~/.diting/.env (COPILOT_GITHUB_TOKEN)',
    copilotAddSourceGhCli: '출처: gh CLI (gh auth token)',
    copilotAddSourceAppsJson: '출처: VS Code Copilot 확장 (apps.json)',
    copilotDeleteHintEnv: '이 작업은 ~/.diting/.env의 COPILOT_GITHUB_TOKEN을 지웁니다. 다른 도구에는 영향이 없습니다.',
    copilotDeleteHintGhCli: 'Copilot이 DiTing에서 숨겨집니다. gh CLI 로그인에는 영향이 없으며 `gh auth status`는 여전히 로그인 상태를 표시합니다.',
    copilotDeleteHintAppsJson: 'Copilot이 DiTing에서 숨겨집니다. VS Code Copilot 확장 로그인에는 영향이 없습니다.',
    customBadge: '커스텀',
    previewBadge: '프리뷰',
    disabledBadge: '사용 불가',
    ...
    visibilityHint: 'Web UI의 모델 선택기와 모델 페이지 표시에만 영향을 줍니다. DiTing CLI의 provider/model 설정은 변경되지 않으며 실제 호출은 원본 모델 ID를 사용합니다.',
    ...
    aliasManage: '표시 이름',
    aliasHint: 'Web UI 표시 이름만 변경합니다. DiTing에는 여전히 원본 모델 ID가 전달됩니다.',
    aliasEdit: '이름 변경',
  ...
  profiles: {
    ...
    rename: '이름 변경',
    delete: '삭제',
    switchTo: 'DiTing Profile 전환',
    switchConfirm: '`DiTing profile use {name}`를 실행하고 DiTing CLI active profile을 변경합니다. 계속하시겠습니까?',
    switchSuccess: 'DiTing active profile이 "{name}"(으)로 전환되었습니다',
    switchFailed: 'DiTing Profile 전환 실패. 게이트웨이를 수동으로 재시작해야 할 수 있습니다.',
  ...
  settings: {
    ...
    gatewayAutoStart: {
      title: 'Gateway 자동 시작',
      description: 'Web UI 시작 시 자동으로 실행할 로컬 DiTing profile gateway를 제어합니다. 이 값은 profile config.yaml이 아니라 Web UI 설정에 저장됩니다.',
      enabled: 'Gateway 자동 시작',
      enabledHint: '끄면 시작 시 어떤 profile gateway도 자동으로 실행하지 않습니다.',
      management: '통합 gateway',
      managementHint: 'DiTing Agent 0.17.0 이상이 필요합니다. 켜면 Web UI가 multiplex_profiles를 기록해 default gateway가 모든 profiles를 처리합니다. 다른 profile은 Feishu, Weixin, QQBot 같은 메시징 진입점을 별도로 설정할 수 없습니다. 끄면 해당 설정을 제거합니다.',
      managementAuto: '자동',
    ...
    session: {
      ...
      memoryWriteApprovalHint: 'DiTing Agent의 메모리 쓰기를 저장하기 전에 승인 대기 상태로 둡니다.',
      skillsWriteApproval: '스킬 쓰기 검토',
      skillsWriteApprovalHint: 'DiTing Agent의 스킬 변경을 저장하기 전에 승인 대기 상태로 둡니다.',
      writeApprovalTitle: '쓰기 승인 대기',
      writeApprovalDescription: 'DiTing Agent가 보류한 메모리와 스킬 쓰기를 승인하거나 거부합니다.',
      writeApprovalRefresh: '새로고침',
      writeApprovalEmpty: '승인 대기 중인 쓰기가 없습니다.',
  ...
  githubPreview: {
    ...
    start: "미리보기 시작",
    stop: "중지",
    note: "미리보기 코드는 Web UI 데이터 홈 아래에 저장됩니다. 프로덕션은 18648을 사용하고, 미리보기 개발 환경은 프론트엔드 18651, 백엔드 18647에서 실행됩니다.",
    path: "미리보기 경로",
  ...
  codingAgents: {
    ...
    codexDescription: "저장소 작업을 위한 OpenAI CLI 및 DiTing openai-codex 제공업체 흐름입니다.",
    copyCommand: "복사",
    ...
    installSuccess: "설치됨",
    installFailed: "설치 실패",
    installFailedDiTingHint: "설치에 실패했습니다. DiTing에게 설치를 요청하세요.",
    ...
    notes: {
      ...
      codexAuth: "DiTing가 관리하는 OpenAI Codex OAuth 자격 증명을 추가합니다.",
  ...
  platform: {
    ...
    weixinTokenHint: 'weixin CLI QR 로그인에서 가져오기 (DiTing weixin)',
    accountId: '계정 ID',
  ...
  skillsUsage: {
    title: '스킬 사용량',
    subtitle: 'DiTing 세션의 스킬 로드와 편집을 추적합니다',
    refresh: '새로고침',
  ...
  changelog: {
    ...
    new_0_6_18_2: 'DiTing Studio MCP가 api, devices, use toolset으로 나뉘고 session, model, chat-run, ops 작업이 추가되었습니다',
    ...
    new_0_6_18_6: 'Desktop 및 runtime setup은 Unicode-safe Windows PATH 등록, DiTing Agent 0.17.0 runtime publish 지원, mobile PWA metadata, 여러 profile/auth recovery 수정으로 더 안전해졌습니다',
    ...
    new_0_6_13_5: '데스크톱 업데이트 안정성을 개선했습니다. Windows 업그레이드는 남아 있는 DiTing Studio 프로세스를 닫고, Cloudflare를 먼저 확인하며, GitHub 업데이트 피드를 fallback으로 유지합니다',
