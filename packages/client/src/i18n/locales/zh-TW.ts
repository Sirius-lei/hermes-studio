export default {
  ...
  sidebar: {
    ...
    collapse: '收起選單',
    expand: '展開選單',
    updateTip: '在終端機執行 "diting-web-ui update" 即可更新',
  ...
  runtimeVersions: {
    ...
    useVersion: '下次啟動使用',
    deleteVersion: '刪除',
    deleteRuntimeConfirm: '刪除 DiTing Runtime {version}？',
    ...
    activateSuccess: '版本選擇已儲存，重啟 DiTing Studio 後生效。',
    jobStatus: {
      queued: '排隊中',
      running: '下載中',
      completed: '已完成',
  ...
  chat: {
    ...
    contextEditHint: '常見值：256k (DiTing 預設), 128k (GPT-4), 32k (GPT-3.5)',
    contextEditSave: '儲存',
    ...
    openHistory: '開啟歷史',
    DiTingHistory: 'DiTing 歷史',
    historyScopeHint: '這裡按來源以唯讀方式查看目前 profile 的 DiTing 歷史工作階段。',
    noSessions: '目前無工作階段',
    searchTitle: '搜尋工作階段',
    searchSubtitle: '依標題或訊息內容搜尋',
    searchScope: '搜尋範圍：僅 Web UI 本地工作階段資料庫；不包含唯讀 DiTing 歷史工作階段。',
  ...
  skills: {
    ...
    writeApprovalDescription: '批准或拒絕 DiTing Agent 暫存的記憶和技能寫入。',
    writeApprovalRefresh: '重新整理',
    writeApprovalEmpty: '暫無待審核寫入。',
    writeApprovalUnsupported: '目前 DiTing Agent 版本不支援寫入審核。請升級 DiTing Agent 後再使用。',
  ...
  plugins: {
    title: '插件',
    refresh: '重新整理',
    notice: '唯讀顯示可發現的 DiTing 插件 manifest。發現元資料讀取不會載入插件程式碼。v1 管理動作仍保留在 CLI，新 DiTing 工作階段生效。',
    loadFailed: '載入插件失敗',
    commandCopied: '指令已複製',
  ...
  models: {
    ...
    copilotAddDetected: '已在本機偵測到 GitHub Copilot OAuth 憑證，點擊「新增」即可在 DiTing 中啟用 Copilot。',
    copilotAddSourceEnv: '來源：~/.diting/.env（COPILOT_GITHUB_TOKEN）',
    copilotAddSourceGhCli: '來源：gh CLI（gh auth token）',
    copilotAddSourceAppsJson: '來源：VS Code Copilot 插件（apps.json）',
    copilotDeleteHintEnv: '此操作會清除 ~/.diting/.env 中的 COPILOT_GITHUB_TOKEN，不影響其他工具。',
    copilotDeleteHintGhCli: 'Copilot 將從 DiTing 清單移除。不會影響 gh CLI —— `gh auth status` 仍顯示已登入。',
    copilotDeleteHintAppsJson: 'Copilot 將從 DiTing 清單移除。不會影響 VS Code Copilot 插件的登入。',
    customBadge: '自訂',
    previewBadge: '預覽',
    disabledBadge: '不可用',
    ...
    visibilityHint: '僅影響 Web UI 的模型選擇器和模型頁展示，不會改寫 DiTing CLI 的 provider/model 配置。實際呼叫仍使用原始模型 ID。',
    ...
    aliasHint: '僅修改 Web UI 顯示名，傳送給 DiTing 的仍是原始模型 ID。',
  ...
  profiles: {
    ...
    rename: '重新命名',
    delete: '刪除',
    switchTo: '切換 DiTing Profile',
    switchConfirm: '將執行 `DiTing profile use {name}` 並切換 DiTing CLI 的 active profile，是否繼續？',
    switchSuccess: 'DiTing active profile 已切換為「{name}」',
    switchFailed: '切換 DiTing Profile 失敗，閘道可能需要手動重新啟動',
  ...
  settings: {
    ...
    gatewayAutoStart: {
      title: 'Gateway 自動啟動',
      description: '控制 Web UI 啟動時自動拉起哪些本機 DiTing profile gateway。此項寫入 Web UI 設定，不寫入 profile config.yaml。',
      enabled: '自動啟動 gateway',
      enabledHint: '關閉後，啟動期不會自動拉起任何 profile gateway。',
      management: '統一 gateway',
      managementHint: '需要 DiTing Agent 0.17.0 及以上版本。開啟後會寫入 multiplex_profiles，讓 default gateway 服務所有 profiles；其他 profile 不能單獨設定飛書、微信、QQBot 等通訊工具入口。關閉後會移除此設定。',
      managementAuto: '自動',
    ...
    session: {
      ...
      memoryWriteApprovalHint: 'DiTing Agent 寫入 MEMORY.md / USER.md 前先進入待審核佇列。',
      skillsWriteApproval: '技能寫入審核',
      skillsWriteApprovalHint: 'DiTing Agent 建立或修改技能前先進入待審核佇列。',
      writeApprovalTitle: '待審核寫入',
      writeApprovalDescription: '批准或拒絕 DiTing Agent 暫存的記憶和技能寫入。',
      writeApprovalRefresh: '重新整理',
      writeApprovalEmpty: '暫無待審核寫入。',
  ...
  githubPreview: {
    ...
    start: "開啟預覽",
    stop: "停止",
    note: "預覽程式碼存放在 Web UI 資料目錄下。正式環境使用 18648，預覽開發環境使用前端 18651、後端 18647。",
    path: "預覽路徑",
    webuiHome: "預覽資料目錄",
    currentTag: "目前 Tag",
    repoReady: "倉庫就緒",
  ...
  codingAgents: {
    ...
    codexDescription: "OpenAI CLI，以及 DiTing openai-codex 提供商的倉庫任務流程。",
    copyCommand: "複製",
    commandCopied: "命令已複製",
    ...
    installSuccess: "安裝完成",
    installFailed: "安裝失敗",
    installFailedDiTingHint: "安裝失敗，請讓 DiTing 幫你安裝。",
    ...
    notes: {
      ...
      codexAuth: "新增 DiTing 管理的 OpenAI Codex OAuth 憑證。",
  ...
  platform: {
    ...
    weixinTokenHint: '透過 weixin CLI 掃碼登入取得 (DiTing weixin)',
  ...
  skillsUsage: {
    title: '技能用量',
    subtitle: '追蹤 DiTing 工作階段中的技能載入與編輯',
    refresh: '重新整理',
  ...
  changelog: {
    ...
    new_0_6_18_2: 'DiTing Studio MCP 拆分為 api、devices 和 use 工具集，新增 session、model、chat-run 與 ops 相關操作',
    ...
    new_0_6_18_6: '桌面與 runtime 設定更安全：Windows PATH 註冊支援 Unicode，支援發布 DiTing Agent 0.17.0 runtime，並補齊行動端 PWA metadata 和多項 profile/auth 恢復修復',
    ...
    new_0_6_13_5: '桌面更新更可靠：Windows 升級會關閉殘留 DiTing Studio 行程，優先檢查 Cloudflare，並保留 GitHub 更新源兜底',
