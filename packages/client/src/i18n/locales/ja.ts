export default {
  ...
  sidebar: {
    ...
    connected: '接続済み',
    disconnected: '未接続',
    updateTip: 'ターミナルで "diting-web-ui update" を実行して更新してください',
  ...
  runtimeVersions: {
    ...
    useVersion: '次回起動時に使用',
    deleteVersion: '削除',
    deleteRuntimeConfirm: 'DiTing Runtime {version} を削除しますか？',
    ...
    activateSuccess: 'バージョン選択を保存しました。DiTing Studio を再起動すると反映されます。',
    jobStatus: {
      queued: '待機中',
      running: '実行中',
      completed: '完了',
  ...
  chat: {
    ...
    contextEditHint: '一般的な値：256k (DiTing 既定), 128k (GPT-4), 32k (GPT-3.5)',
    contextEditSave: '保存',
    ...
    openHistory: '履歴を開く',
    DiTingHistory: 'DiTing 履歴',
    historyScopeHint: '現在の profile の DiTing 履歴セッションをソース別に読み取り専用で表示します。',
    ...
    searchScope: '検索範囲: Web UI のローカルセッション DB のみ。読み取り専用の DiTing 履歴セッションは含まれません。',
  ...
  skills: {
    ...
    writeApprovalDescription: 'DiTing Agent が保留したメモリとスキルの書き込みを承認または拒否します。',
    writeApprovalRefresh: '更新',
    writeApprovalEmpty: '承認待ちの書き込みはありません。',
    writeApprovalUnsupported: 'この DiTing Agent バージョンは書き込み承認に対応していません。この機能を使うには DiTing Agent を更新してください。',
  ...
  plugins: {
    title: 'プラグイン',
    refresh: '更新',
    notice: '検出可能な DiTing プラグイン manifest の読み取り専用インベントリです。検出メタデータはプラグインコードを読み込まずに取得します。v1 の管理操作は CLI のままで、変更は新しい DiTing セッションで有効になります。',
  ...
  models: {
    ...
    copilotAddDetected: 'このマシンで GitHub Copilot OAuth トークンを検出しました。「追加」をクリックして DiTing で Copilot を有効化します。',
    copilotAddSourceEnv: 'ソース: ~/.diting/.env (COPILOT_GITHUB_TOKEN)',
    copilotAddSourceGhCli: 'ソース: gh CLI (gh auth token)',
    copilotAddSourceAppsJson: 'ソース: VS Code Copilot 拡張機能 (apps.json)',
    copilotDeleteHintEnv: 'この操作で ~/.diting/.env の COPILOT_GITHUB_TOKEN を消去します。他のツールには影響しません。',
    copilotDeleteHintGhCli: 'Copilot は DiTing 上で非表示になります。gh CLI のログインには影響しません — `gh auth status` は引き続きログイン状態を表示します。',
    copilotDeleteHintAppsJson: 'Copilot は DiTing 上で非表示になります。VS Code Copilot 拡張機能のログインには影響しません。',
    customBadge: 'カスタム',
    previewBadge: 'プレビュー',
    disabledBadge: '利用不可',
    ...
    visibilityHint: 'Web UI のモデル選択とモデルページの表示だけに影響します。DiTing CLI の provider/model 設定は変更されず、実際の呼び出しは元のモデル ID を使います。',
    ...
    aliasManage: '表示名',
    aliasHint: 'Web UI の表示名だけを変更します。DiTing には元のモデル ID が送信されます。',
    aliasEdit: '名前変更',
  ...
  profiles: {
    ...
    rename: '名前変更',
    delete: '削除',
    switchTo: 'DiTing Profile を切り替え',
    switchConfirm: '`DiTing profile use {name}` を実行し、DiTing CLI の active profile を切り替えます。続行しますか？',
    switchSuccess: 'DiTing active profile を「{name}」に切り替えました',
    switchFailed: 'DiTing Profile の切り替えに失敗しました。ゲートウェイの手動再起動が必要な場合があります。',
  ...
  settings: {
    ...
    gatewayAutoStart: {
      title: 'Gateway 自動起動',
      description: 'Web UI 起動時に自動起動するローカル DiTing profile gateway を制御します。この設定は Web UI 設定に保存され、profile config.yaml には保存されません。',
      enabled: 'Gateway を自動起動',
      enabledHint: '無効にすると、起動時に profile gateway を自動起動しません。',
      management: '統一 gateway',
      managementHint: 'DiTing Agent 0.17.0 以降が必要です。有効にすると Web UI が multiplex_profiles を書き込み、default gateway がすべての profiles を処理します。他の profile では Feishu、Weixin、QQBot などのメッセージング入口を個別設定できません。無効にするとその設定を削除します。',
      managementAuto: '自動',
    ...
    session: {
      ...
      memoryWriteApprovalHint: 'DiTing Agent のメモリ書き込みを保存前に承認待ちにします。',
      skillsWriteApproval: 'スキル書き込みを確認',
      skillsWriteApprovalHint: 'DiTing Agent のスキル変更を保存前に承認待ちにします。',
      writeApprovalTitle: '書き込み承認待ち',
      writeApprovalDescription: 'DiTing Agent が保留したメモリとスキルの書き込みを承認または拒否します。',
      writeApprovalRefresh: '更新',
      writeApprovalEmpty: '承認待ちの書き込みはありません。',
  ...
  githubPreview: {
    ...
    start: "プレビューを開始",
    stop: "停止",
    note: "プレビューコードは Web UI データホーム配下に保存されます。本番は 18648、プレビュー開発環境はフロントエンド 18651、バックエンド 18647 で実行されます。",
    path: "プレビューパス",
  ...
  codingAgents: {
    ...
    codexDescription: "リポジトリ作業向けの OpenAI CLI と DiTing openai-codex プロバイダーフローです。",
    copyCommand: "コピー",
    commandCopied: "コマンドをコピーしました",
    ...
    installFailedDiTingHint: "インストールに失敗しました。DiTing にインストールを依頼してください。",
    ...
    notes: {
      ...
      codexAuth: "DiTing 管理の OpenAI Codex OAuth 認証情報を追加します。",
  ...
  platform: {
    ...
    weixinTokenHint: 'weixin CLI の QR ログインから取得 (DiTing weixin)',
    accountId: 'アカウント ID',
  ...
  skillsUsage: {
    title: 'スキル使用状況',
    subtitle: 'DiTing セッションでのスキル読み込みと編集を追跡します',
    refresh: '更新',
  ...
  changelog: {
    ...
    new_0_6_18_2: 'DiTing Studio MCP を api、devices、use の toolset に分割し、session、model、chat-run、ops 操作を追加しました',
    ...
    new_0_6_18_6: 'Desktop と runtime setup は、Unicode-safe な Windows PATH 登録、DiTing Agent 0.17.0 runtime publish 対応、mobile PWA metadata、profile/auth recovery 修正で安全になりました',
    ...
    new_0_6_13_5: 'デスクトップ更新の信頼性を改善しました。Windows アップグレード時に残留 DiTing Studio プロセスを閉じ、Cloudflare を先に確認し、GitHub をフォールバック更新元として保持します',
