...
const STORAGE_KEY_THEME = "DiTing_terminal_theme";
...
function buildWsUrl(): string {
  ...
  if (base) {
    return `${wsProtocol}//${new URL(base).host}/api/DiTing/terminal${token ? `?token=${encodeURIComponent(token)}` : ""}`;
  }
  ...
  const host = import.meta.env.DEV && directDevPort
    ...
    : location.host;
  return `${wsProtocol}//${host}/api/DiTing/terminal${token ? `?token=${encodeURIComponent(token)}` : ""}`;
}
