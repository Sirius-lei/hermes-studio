...
import { fetchPlugins, type DiTingPluginInfo, type DiTingPluginsMetadata } from '@/api/DiTing/plugins'
...
const plugins = ref<DiTingPluginInfo[]>([])
const warnings = ref<string[]>([])
const metadata = ref<DiTingPluginsMetadata | null>(null)
...
function pluginCommand(plugin: DiTingPluginInfo) {
  const escapedKey = plugin.key.replace(/'/g, `'\\''`)
  if (plugin.effectiveStatus === 'disabled' || plugin.effectiveStatus === 'inactive') {
    return `DiTing plugins enable '${escapedKey}'`
  }
  if (plugin.effectiveStatus === 'enabled') {
    return `DiTing plugins disable '${escapedKey}'`
  }
  return ''
}

async function copyCommand(plugin: DiTingPluginInfo) {
    ...
    <div class="plugins-content" :class="{ 'is-loading': loading && plugins.length === 0 }">
      ...
      <template v-else>
        ...
        <div v-if="metadata" class="metadata-panel">
          <span>{{ t('plugins.metadata.agentRoot') }}: <code>{{ metadata.DiTingAgentRoot }}</code></span>
