...
import WorkflowAgentNode from '@/components/DiTing/workflow/WorkflowAgentNode.vue'
import FolderPicker from '@/components/DiTing/chat/FolderPicker.vue'
import ChatInput from '@/components/DiTing/chat/ChatInput.vue'
import MessageList from '@/components/DiTing/chat/MessageList.vue'
import ProfileAvatar from '@/components/DiTing/profiles/ProfileAvatar.vue'
...
import { uploadRuntimeFiles } from '@/api/DiTing/files'
import {
...
} from '@/api/DiTing/workflow-socket'
import { fetchSkills } from '@/api/DiTing/skills'
import { fetchSession } from '@/api/DiTing/sessions'
...
} from '@/components/DiTing/workflow/types'
...
const { screenToFlowCoordinate, getViewport, setViewport } = useVueFlow('DiTing-workflow')
...
const WORKFLOW_CHAT_PANEL_STORAGE_KEY = 'DiTing.workflow.chatPanelWidth'
...
function skillOptionsCacheKey(agent: string, profile = activeWorkflowProfile.value): string {
  ...
  return target === 'DiTing' ? `${target}:${profile || 'default'}` : target
}
...
async function ensureSkillOptionsForAgent(agent: string, profile = activeWorkflowProfile.value): Promise<void> {
  ...
  const request = fetchSkills(target === 'DiTing' ? profile : undefined, target)
    .then((data) => {
...
function makeNode(
  ...
  return {
    ...
    data: {
      title,
      agent: data.agent || agentOptions.value[0]?.value || 'DiTing',
      ...
      agentOptions: agentOptions.value,
      skillOptions: skillOptionsForAgent(data.agent || agentOptions.value[0]?.value || 'DiTing'),
      skillsLoading: skillsLoadingForAgent(data.agent || agentOptions.value[0]?.value || 'DiTing'),
      modelGroups: modelGroups.value,
      onUpdate: updateNodeData,
      onUploadImages: uploadNodeImages,
    },
...
function workflowValidationError(): string | null {
  ...
  for (const node of nodes.value) {
    ...
    if (node.data.agent !== 'DiTing' && !node.data.apiMode) {
