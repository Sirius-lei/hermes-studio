...
import { type Session } from '@/stores/DiTing/chat'
...
import OutlinePanel from '@/components/DiTing/chat/OutlinePanel.vue'
import PageSidebarNav from '@/components/layout/PageSidebarNav.vue'
import PageSidebarFooter from '@/components/layout/PageSidebarFooter.vue'
import { batchDeleteSessions, deleteSession, fetchDiTingSessions, fetchDiTingSession, fetchSessionMessagesPage, importDiTingSession, type DiTingMessage, type SessionSummary } from '@/api/DiTing/sessions'
...
// DiTing history sessions (exclude api_server)
const DiTingSessions = ref<SessionSummary[]>([])
const DiTingSessionsLoading = ref(false)
const DiTingSessionsLoaded = ref(false)
...
async function loadDiTingSessions() {
  const requestId = ++DiTingSessionsRequestId
  DiTingSessionsLoading.value = true
  try {
    const sessions = await fetchDiTingSessions(undefined, undefined, effectiveHistoryProfile.value)
    if (requestId !== DiTingSessionsRequestId) return
    DiTingSessions.value = sessions
    DiTingSessionsLoaded.value = true
  } catch (err) {
    console.error('Failed to load DiTing sessions:', err)
  } finally {
    if (requestId === DiTingSessionsRequestId) {
      DiTingSessionsLoading.value = false
    }
  }
}
...
function findHistorySession(sessionId: string): SessionSummary | undefined {
  return DiTingSessions.value.find(session => session.id === sessionId)
}
...
function mapHistoryMessages(messages: DiTingMessage[]): Session['messages'] {
...
async function loadHistorySession(sessionId: string, profile?: string | null) {
  ...
  if (page) {
    ...
    // Some imported/legacy DiTing sessions may only exist in DiTing state.db.
    // Keep the old full-detail path as a compatibility fallback.
    const sessionDetail = await fetchDiTingSession(sessionId, sessionProfile)
    if (!sessionDetail) {
...
async function openDefaultHistorySession(replace = false) {
  const firstCliSession = DiTingSessions.value.find(s => s.source === 'cli')
  const firstSession = firstCliSession || DiTingSessions.value[0]
  if (!firstSession) {
    historySessionId.value = null
    historySession.value = null
    if (routeSessionId.value) await router.replace({ name: 'DiTing.history' })
    return
  }
...
async function syncRouteSession() {
  ...
  if (!DiTingSessions.value.some(s => s.id === sessionId)) {
    historySessionId.value = null
    historySession.value = null
    await router.replace({ name: 'DiTing.history' })
    return
  }
...
onMounted(async () => {
  ...
  await loadDiTingSessions()
  await syncRouteSession()

  mobileQuery = window.matchMedia('(max-width: 768px)')
  handleMobileChange(mobileQuery)
  mobileQuery.addEventListener('change', handleMobileChange)
  window.addEventListener('DiTing:open-page-sidebar', openPageSidebar)
})

onUnmounted(() => {
  mobileQuery?.removeEventListener('change', handleMobileChange)
  window.removeEventListener('DiTing:open-page-sidebar', openPageSidebar)
})
...
watch([routeSessionId, routeProfile, routeUserId], async ([sessionId, _profile, userId], [_prevSessionId, _prevProfile, prevUserId]) => {
  ...
  if (userChanged && DiTingSessionsLoaded.value) {
    historySessionId.value = null
    historySession.value = null
    await loadDiTingSessions()
  }
  if (!sessionId) {
    historySessionId.value = null
    historySession.value = null
    if (userChanged && DiTingSessionsLoaded.value && DiTingSessions.value.length > 0) {
      await openDefaultHistorySession(true)
    }
    return
  }
  if (!DiTingSessionsLoaded.value) return
  if (routeProfile.value && !DiTingSessions.value.some(s => s.profile === routeProfile.value)) {
    await loadDiTingSessions()
  }
  await syncRouteSession()
})
...
const collapsedGroups = ref<Set<string>>(new Set(JSON.parse(localStorage.getItem('DiTing_collapsed_groups') || '[]')))
...
// Computed sessions from DiTing API
const historySessions = computed<Session[]>(() =>
  DiTingSessions.value.map(sessionSummaryToSession)
)
...
function toggleGroup(source: string) {
  ...
  localStorage.setItem('DiTing_collapsed_groups', JSON.stringify([...collapsedGroups.value]))
}

watch(groupedSessions, groups => {
  if (localStorage.getItem('DiTing_collapsed_groups') !== null) {
...
// Auto-load the first CLI session when DiTing sessions are loaded.
watch(DiTingSessionsLoaded, (loaded) => {
  if (loaded && DiTingSessions.value.length > 0 && !routeSessionId.value) {
...
async function handleImportToWebUi(sessionId: string) {
  const summary = findHistorySession(sessionId)
  try {
    const result = await importDiTingSession(sessionId, summary?.profile || null)
    if (result.ok) {
...
async function handleDeleteSession(id: string, profile?: string | null) {
  ...
  DiTingSessions.value = DiTingSessions.value.filter(s => s.id !== id)
  ...
  if (historySessionId.value === id) {
    ...
    else await router.replace({ name: 'DiTing.history' })
  }
    ...
    <aside class="session-list" :class="{ collapsed: !showSessions }">
      <div v-if="showSessions" class="page-sidebar-top">
        ...
        <div class="session-list-toolbar">
          <span class="session-list-title">{{ t('chat.DiTingHistory') }}</span>
          <div class="session-list-actions">
            ...
            <NButton
              ...
              quaternary
              size="tiny"
              :disabled="DiTingSessions.length === 0"
      ...
      <div v-if="showSessions" class="session-items">
        <div v-if="DiTingSessionsLoading && DiTingSessions.length === 0" class="session-loading">{{ t('common.loading') }}</div>
        <div v-else-if="DiTingSessions.length === 0" class="session-empty">{{ t('chat.noSessions') }}</div>
