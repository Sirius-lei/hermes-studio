...
import GroupChatPanel from '@/components/DiTing/group-chat/GroupChatPanel.vue'
...
async function syncRouteRoom() {
    ...
    if (!roomId) {
        if (!store.currentRoomId && store.rooms.length > 0) {
            await router.replace({ name: 'DiTing.groupChatRoom', params: { roomId: store.rooms[0].id } })
        }
        return
    }
