...
import type { Message, ContentBlock } from "@/stores/DiTing/chat";
import { computed, onBeforeUnmount, onMounted, ref } from "vue";
import { useI18n } from "vue-i18n";
import { useMessage } from "naive-ui";
import { downloadFile, getDownloadUrl } from "@/api/DiTing/download";
...
function parseContentBlocks(content: string): Array<ContentBlock | Record<string, unknown>> | null {
  ...
  try {
    ...
    // DiTing Agent stored some multimodal user messages via Python str(list),
...
const copyableContent = computed(() => {
  ...
  if (isWorkflowMessage.value) return null
  const content = props.message.content || ''
...
function normalizeWorkflowActorName(value?: string) {
  ...
  if (!text) return "主智能体"
  if (/^DiTing$/i.test(text)) return "主智能体"
  return text
}
 ...
 * Upload format in content: [File: name.txt](/tmp/DiTing-uploads/abc123.txt)
 */
      ...
      <div class="msg-body">
        ...
        <div class="msg-content" :class="messageLayoutRole">
          ...
          >
            ...
            <MarkdownRenderer
              v-if="parsedThinking.hasThinking && parsedThinking.body && message.role === 'assistant' && !isContentBlockArray"
            ...
            <MarkdownRenderer
              v-if="message.role === 'system' && message.content && !isCommandMessage && !isWorkflowMessage"
              :content="message.content"
            />
