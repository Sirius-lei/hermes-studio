...
import { downloadFile, getDownloadUrl, fetchFileText } from '@/api/DiTing/download'
...
const componentId = `DiTing-mermaid-${Math.random().toString(36).slice(2)}`
...
async function handleMarkdownClick(event: MouseEvent): Promise<void> {
  ...
  // Full download URL: open directly (already has /api/DiTing/download?path=...)
  if (href.startsWith('/api/DiTing/download?')) {
    event.preventDefault()
    event.stopPropagation()
