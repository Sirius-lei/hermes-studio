...
async function loadFolders(subPath = ''): Promise<FolderListResponse | null> {
  try {
    ...
    return await request<FolderListResponse>(`/api/DiTing/workspace/folders${query}`)
  } catch {
    return null
  }
}
    ...
    case 'delete':
      ...
      dialog.warning({
        ...
        onPositiveClick: async () => {
          try {
            await request('/api/DiTing/workspace/folders', {
              method: 'DELETE',
...
async function submitRenameModal() {
  ...
  try {
    if (renameMode.value === 'create') {
      ...
      await request('/api/DiTing/workspace/folders/rename', {
        method: 'POST',
