...
import { LIVE_CHAT_MAX_LOADED_MESSAGES, useChatStore, type Message } from "@/stores/DiTing/chat";
...
const displayMessagesWithForkDivider = computed<Message[]>(() => {
  ...
  if (workflowIndex >= 0) {
    ...
    const otDiTingsages = messages.filter((_, index) => index !== workflowIndex);
    let lastUserIndex = -1;
    for (let index = otDiTingsages.length - 1; index >= 0; index -= 1) {
      if (otDiTingsages[index].role === "user") {
        lastUserIndex = index;
        break;
      }
    }
    if (lastUserIndex >= 0) {
      ...
      for (let index = lastUserIndex + 1; index < otDiTingsages.length; index += 1) {
        const candidate = otDiTingsages[index];
      ...
      messages = [
        ...otDiTingsages.slice(0, insertionIndex),
        workflowMessage,
        ...otDiTingsages.slice(insertionIndex),
      ];
    } else {
      messages = [workflowMessage, ...otDiTingsages];
    }
  }
...
const historyArchiveHref = computed(() => {
  const session = chatStore.activeSession;
  if (!session?.id) return "#/DiTing/history";
  const profileQuery = session.profile ? `?profile=${encodeURIComponent(session.profile)}` : "";
  return `#/DiTing/history/session/${encodeURIComponent(session.id)}${profileQuery}`;
});
...
const forkLineage = computed(() => {
  ...
  return {
    ...
    parentHref: `#/DiTing/history/session/${encodeURIComponent(session.parentSessionId)}${session.profile ? `?profile=${encodeURIComponent(session.profile)}` : ""}`,
