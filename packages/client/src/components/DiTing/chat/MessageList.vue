...
const displayMessagesWithForkDivider = computed<Message[]>(() => {
  ...
  if (workflowIndex >= 0) {
    ...
    const remainingMessages = messages.filter((_, index) => index !== workflowIndex);
    let lastUserIndex = -1;
    for (let index = remainingMessages.length - 1; index >= 0; index -= 1) {
      if (remainingMessages[index].role === "user") {
        lastUserIndex = index;
        break;
      }
    }
    if (lastUserIndex >= 0) {
      ...
      for (let index = lastUserIndex + 1; index < remainingMessages.length; index += 1) {
        const candidate = remainingMessages[index];
      ...
      messages = [workflowMessage, ...remainingMessages];
    }
  }
