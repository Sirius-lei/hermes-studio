...
import { fetchSessions, searchSessions, type SessionSearchResult, type SessionSummary } from '@/api/DiTing/sessions'
...
async function openItem(item: SearchItem) {
  ...
  const routeName = chatStore.runtimeMode === 'global_agent' ? 'DiTing.globalAgentSession' : 'DiTing.session'
