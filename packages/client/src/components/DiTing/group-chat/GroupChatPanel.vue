...
import { updateRoomConfig, forceCompress } from '@/api/DiTing/group-chat'
import GroupMessageList from './GroupMessageList.vue'
import GroupChatInput from './GroupChatInput.vue'
import ProfileAvatar from '@/components/DiTing/profiles/ProfileAvatar.vue'
import PageSidebarNav from '@/components/layout/PageSidebarNav.vue'
import { copyToClipboard } from '@/utils/clipboard'
import type { Attachment } from '@/stores/DiTing/chat'
import type { RoomAgent } from '@/api/DiTing/group-chat'
...
async function handleCreateRoom(name: string, inviteCode: string, userName: string, description: string, compression: { triggerTokens: number; maxHistoryTokens: number; tailMessageCount: number }) {
    try {
        ...
        await router.push({ name: 'DiTing.groupChatRoom', params: { roomId: res.room.id } })
    } catch {
...
async function handleDeleteRoom(roomId: string) {
    try {
        ...
        if (store.currentRoomId === roomId) {
            await router.replace({ name: 'DiTing.groupChat' })
        }
...
function buildRoomUrl(roomId: string) {
    const href = router.resolve({ name: 'DiTing.groupChatRoom', params: { roomId } }).href
...
async function handleSelectRoom(roomId: string) {
    try {
        await router.push({ name: 'DiTing.groupChatRoom', params: { roomId } })
