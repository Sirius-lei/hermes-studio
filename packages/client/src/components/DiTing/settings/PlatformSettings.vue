...
import { saveCredentials as saveCredsApi, fetchWeixinQrCode, pollWeixinQrStatus, saveWeixinCredentials } from '@/api/DiTing/config'
    ...
    >
      ...
      <template v-if="p.key === 'matrix'">
        ...
        <SettingRow :label="t('platform.matrixUserId')" :hint="t('platform.matrixUserIdHint')">
          <NInput :value="credentialDraft('matrix').extra?.user_id || ''" :loading="isSavingPlatform('matrix')" clearable size="small" class="input-lg" placeholder="@DiTing:example.org" @update:value="v => setCredentialDraft('matrix', { extra: { ...credentialDraft('matrix').extra, user_id: v } })" />
        </SettingRow>
