...
import { getTask } from '@/api/DiTing/kanban'
import { useKanbanStore } from '@/stores/DiTing/kanban'
import { withDefaultAssignee } from '@/utils/DiTing/kanban-assignees'
import HistoryMessageList from '@/components/DiTing/chat/HistoryMessageList.vue'
import type { Session, Message } from '@/stores/DiTing/chat'
import type { KanbanTaskDetail } from '@/api/DiTing/kanban'
...
async function searchTaskSessions() {
  ...
  try {
    const res = await request<{ results: any[] }>(
      `/api/DiTing/kanban/search-sessions?task_id=${encodeURIComponent(taskId)}&profile=${encodeURIComponent(profile)}&board=${encodeURIComponent(board)}`
    )
        ...
        <template v-if="detail">
          ...
          <div v-if="detail.runs.length > 0" class="detail-section">
            ...
            <div v-if="showSessions && sessionResults.length > 0" class="session-list">
              <div v-for="session in sessionResults" :key="session.id" class="session-item" @click="router.push({ name: 'DiTing.chat', query: { session: session.id } })">
