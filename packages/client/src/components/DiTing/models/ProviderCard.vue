...
import { checkCopilotToken, disableCopilot } from '@/api/DiTing/copilot-auth'
    ...
    onPositiveClick: async () => {
      ...
      try {
        if (isCopilot.value) {
          ...
          // 仅当 token 来自 ~/.DiTing/.env 时才清掉，gh-cli / apps.json 不动。
        ...
        // "无默认模型"的尴尬态。与 DiTing CLI `model` 子命令的隐含行为对齐。
