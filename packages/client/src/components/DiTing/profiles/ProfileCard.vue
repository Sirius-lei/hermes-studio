...
const props = defineProps<{ profile: DiTingProfile }>()
...
const detail = ref<DiTingProfileDetail | null>(null)
...
async function handleSwitch() {
  dialog.warning({
    ...
    onPositiveClick: performDiTingSwitch,
  })
}

async function performDiTingSwitch() {
  switching.value = true
  try {
    const ok = await profilesStore.switchDiTingProfile(props.profile.name)
    if (ok) {
