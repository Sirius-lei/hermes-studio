...
import { fetchSkillContent, fetchSkillFiles, pinSkillApi, type SkillFileEntry, type SkillTarget } from '@/api/DiTing/skills'
...
async function loadSkill() {
  ...
  try {
    ...
    const [skillContent, skillFiles] = await Promise.all([
      fetchSkillContent(skillPath, props.target || 'DiTing'),
      fetchSkillFiles(props.category, props.skill, props.target || 'DiTing'),
    ])
...
async function viewFile(filePath: string) {
  ...
  try {
    ...
    if (filePath.startsWith('/') || /^[a-zA-Z]:[\\/]/.test(filePath)) {
      ...
      const segments = normalizedPath.split(/(?:^|\/)(?:\.DiTing|DiTing)\/skills\//)[1]
      if (segments) {
        const afterSkillDir = segments.split('/').slice(2).join('/')
        relPath = afterSkillDir
      }
    }
    fileContent.value = await fetchSkillContent(`${base}${relPath}`, props.target || 'DiTing')
  } catch (err: any) {
...
watch(() => `${props.target || 'DiTing'}/${props.category}/${props.skill}`, loadSkill, { immediate: true })
</script>
