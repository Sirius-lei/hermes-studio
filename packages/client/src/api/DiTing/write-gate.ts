...
export async function fetchPendingWrites(): Promise<PendingWritesResponse> {
  return request<PendingWritesResponse>('/api/DiTing/write-gate/pending')
}

export async function fetchPendingWriteDiff(subsystem: WriteGateSubsystem, id: string): Promise<string> {
  const res = await request<{ diff: string; review?: PendingWriteReview }>(`/api/DiTing/write-gate/pending/${encodeURIComponent(subsystem)}/${encodeURIComponent(id)}/diff`)
  return res.diff
}
...
export async function approvePendingWrite(subsystem: WriteGateSubsystem, id: string): Promise<{ output: string }> {
  return request<{ success: boolean; output: string }>(
    `/api/DiTing/write-gate/pending/${encodeURIComponent(subsystem)}/${encodeURIComponent(id)}/approve`,
    { method: 'POST' },
  )
}
...
export async function rejectPendingWrite(subsystem: WriteGateSubsystem, id: string): Promise<{ output: string }> {
  return request<{ success: boolean; output: string }>(
    `/api/DiTing/write-gate/pending/${encodeURIComponent(subsystem)}/${encodeURIComponent(id)}/reject`,
    { method: 'POST' },
  )
}
