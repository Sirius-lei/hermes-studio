...
export interface DiTingMessage {
  id: number
  session_id: string
...
export async function fetchSessions(source?: string, limit?: number, profile?: string): Promise<SessionSummary[]> {
  ...
  const res = await request<{ sessions: SessionSummary[] }>(`/api/DiTing/sessions${query ? `?${query}` : ''}`)
  return res.sessions
}

/**
 * Fetch DiTing sessions only (exclude api_server source)
 */
export async function fetchDiTingSessions(source?: string, limit?: number, profile?: string | null): Promise<SessionSummary[]> {
  const params = new URLSearchParams()
  if (source) params.set('source', source)
  if (limit) params.set('limit', String(limit))
  if (profile) params.set('profile', profile)
  const query = params.toString()
  const res = await request<{ sessions: SessionSummary[] }>(`/api/DiTing/sessions/DiTing${query ? `?${query}` : ''}`)
  return res.sessions
}
...
export async function searchSessions(q: string, source?: string, limit?: number, profile?: string): Promise<SessionSearchResult[]> {
  ...
  if (profile) params.set('profile', profile)
  const query = params.toString()
  const res = await request<{ results: SessionSearchResult[] }>(`/api/DiTing/search/sessions?${query}`)
  return res.results
}
...
export async function fetchSession(id: string, profile?: string | null): Promise<SessionDetail | null> {
  try {
    ...
    if (profile) params.set('profile', profile)
    const query = params.toString()
    const res = await request<{ session: SessionDetail }>(`/api/DiTing/sessions/${id}${query ? `?${query}` : ''}`)
    return res.session
  } catch {
    return null
  }
...
export async function fetchSessionContext(id: string, profile?: string | null): Promise<SessionContext | null> {
  try {
    ...
    if (profile) params.set('profile', profile)
    const query = params.toString()
    return await request<SessionContext>(`/api/DiTing/sessions/${encodeURIComponent(id)}/context${query ? `?${query}` : ''}`)
  } catch {
    return null
  }
}
...
export async function fetchSessionMessagesPage(
  ...
  try {
    ...
    const res = await request<PaginatedSessionMessages>(
      `/api/DiTing/sessions/conversations/${encodeURIComponent(id)}/messages/paginated?${params}`,
    )
    return res
  } catch {
    return null
...
/**
 * Fetch DiTing session detail only (exclude api_server source)
 */
export async function fetchDiTingSession(id: string, profile?: string | null): Promise<SessionDetail | null> {
  try {
    const params = new URLSearchParams()
    if (profile) params.set('profile', profile)
    const query = params.toString()
    const res = await request<{ session: SessionDetail }>(`/api/DiTing/sessions/DiTing/${id}${query ? `?${query}` : ''}`)
    return res.session
  } catch {
    return null
  }
...
export async function deleteSession(id: string, profile?: string | null): Promise<boolean> {
  try {
    ...
    if (profile) params.set('profile', profile)
    const query = params.toString()
    await request(`/api/DiTing/sessions/${id}${query ? `?${query}` : ''}`, { method: 'DELETE' })
    return true
  } catch {
    return false
  }
...
export async function importDiTingSession(id: string, profile?: string | null): Promise<{ ok: boolean; imported: boolean; session?: SessionDetail }> {
  const params = new URLSearchParams()
  if (profile) params.set('profile', profile)
  const query = params.toString()
  return request<{ ok: boolean; imported: boolean; session?: SessionDetail }>(
    `/api/DiTing/sessions/DiTing/${encodeURIComponent(id)}/import${query ? `?${query}` : ''}`,
    { method: 'POST' },
  )
}
...
export async function batchDeleteSessions(targets: Array<string | BatchDeleteSessionTarget>): Promise<{ deleted: number; failed: number; errors: Array<{ id: string; error: string }> }> {
  try {
    ...
    const res = await request<{ deleted: number; failed: number; errors: Array<{ id: string; error: string }> }>(
      '/api/DiTing/sessions/batch-delete',
      {
        method: 'POST',
        body: JSON.stringify({
...
export async function renameSession(id: string, title: string): Promise<boolean> {
  try {
    await request(`/api/DiTing/sessions/${id}/rename`, {
      method: 'POST',
...
export async function setSessionWorkspace(id: string, workspace: string | null): Promise<boolean> {
  try {
    await request(`/api/DiTing/sessions/${id}/workspace`, {
      method: 'POST',
...
export async function setSessionModel(id: string, model: string, provider: string, apiMode?: ProviderApiMode): Promise<boolean> {
  try {
    await request(`/api/DiTing/sessions/${id}/model`, {
      method: 'POST',
...
export async function exportSession(id: string, mode: 'full' | 'compressed' = 'full', ext: 'json' | 'txt' = 'json'): Promise<void> {
  ...
  const url = `${baseUrl}/api/DiTing/sessions/${id}/export?mode=${mode}&ext=${ext}&token=${encodeURIComponent(token)}`
...
export async function fetchUsageStats(days = 30): Promise<UsageStatsResponse> {
  ...
  return request<UsageStatsResponse>(`/api/DiTing/usage/stats?${params}`)
}
...
export async function fetchSessionUsage(ids: string[]): Promise<Record<string, { input_tokens: number; output_tokens: number }>> {
  ...
  return request(`/api/DiTing/sessions/usage?${params}`)
}

export async function fetchSessionUsageSingle(id: string): Promise<{ input_tokens: number; output_tokens: number } | null> {
  try {
    return await request<{ input_tokens: number; output_tokens: number }>(`/api/DiTing/sessions/${id}/usage`)
  } catch {
    return null
  }
}
...
export async function fetchContextLength(profile?: string, provider?: string, model?: string): Promise<number> {
  ...
  const query = params.toString()
  const res = await request<{ context_length: number }>(`/api/DiTing/sessions/context-length${query ? `?${query}` : ''}`)
