...
export async function listTaskPlans(): Promise<TaskPlanRecord[]> {
  const res = await request<{ plans: TaskPlanRecord[] }>('/api/DiTing/task-plans')
  return res.plans
}

export async function getTaskPlan(id: string): Promise<TaskPlanRecord> {
  const res = await request<{ plan: TaskPlanRecord }>(`/api/DiTing/task-plans/${encodeURIComponent(id)}`)
  return res.plan
}

export async function generateTaskPlan(data: GenerateTaskPlanRequest): Promise<TaskPlanRecord> {
  const res = await request<{ plan: TaskPlanRecord }>('/api/DiTing/task-plans/generate', {
    method: 'POST',
...
export async function deleteTaskPlan(id: string): Promise<{ ok: boolean }> {
  return request<{ ok: boolean }>(`/api/DiTing/task-plans/${encodeURIComponent(id)}`, {
    method: 'DELETE',
  })
}
...
export async function exportTaskPlanToKanban(id: string, data: ExportTaskPlanRequest = {}): Promise<{
  ...
  return request(`/api/DiTing/task-plans/${encodeURIComponent(id)}/export-kanban`, {
    method: 'POST',
    body: JSON.stringify(data),
  })
}
