...
export async function listWorkflows(profile?: string | null): Promise<WorkflowRecord[]> {
  const path = appendProfile('/api/DiTing/workflows', profile)
...
export async function fetchWorkflow(id: string): Promise<WorkflowRecord> {
  const res = await request<{ workflow: WorkflowRecord }>(`/api/DiTing/workflows/${encodeURIComponent(id)}`)
  return res.workflow
}

export async function createWorkflow(input: WorkflowCreateRequest): Promise<WorkflowRecord> {
  const res = await request<{ workflow: WorkflowRecord }>('/api/DiTing/workflows', {
    method: 'POST',
...
export async function deleteWorkflow(id: string): Promise<void> {
  await request<{ ok: true }>(`/api/DiTing/workflows/${encodeURIComponent(id)}`, { method: 'DELETE' })
}

export async function batchDeleteWorkflows(ids: string[]): Promise<WorkflowBatchDeleteResult> {
  return request<WorkflowBatchDeleteResult>('/api/DiTing/workflows/batch-delete', {
    method: 'POST',
...
export async function listWorkflowRuns(id: string, limit = 100): Promise<WorkflowRunRecord[]> {
  ...
  const res = await request<{ runs: WorkflowRunRecord[] }>(`/api/DiTing/workflows/${encodeURIComponent(id)}/runs?${params}`)
  return res.runs
}
...
export async function stopWorkflowRun(id: string, runId: string): Promise<WorkflowRunRecord> {
  const res = await request<{ ok: true; run: WorkflowRunRecord }>(
    `/api/DiTing/workflows/${encodeURIComponent(id)}/runs/${encodeURIComponent(runId)}/stop`,
    { method: 'POST' },
  )
  return res.run
}
...
export async function deleteWorkflowRun(id: string, runId: string): Promise<void> {
  await request<{ ok: true }>(
    `/api/DiTing/workflows/${encodeURIComponent(id)}/runs/${encodeURIComponent(runId)}`,
    { method: 'DELETE' },
  )
}

export async function runWorkflowNow(id: string, input: WorkflowRunNowRequest = {}): Promise<WorkflowRunStartResult> {
  return request<WorkflowRunStartResult>(`/api/DiTing/workflows/${encodeURIComponent(id)}/run`, {
    method: 'POST',
    body: JSON.stringify(input),
  })
}
...
export async function rerunWorkflowRunFromNode(
  ...
  return request<WorkflowRunStartResult>(
    `/api/DiTing/workflows/${encodeURIComponent(id)}/runs/${encodeURIComponent(runId)}/rerun-from-node`,
    {
      method: 'POST',
      body: JSON.stringify({
        ...input,
