...
export type SkillTarget = 'DiTing' | 'claude' | 'codex'
...
export async function fetchSkills(profile?: string, target: SkillTarget = 'DiTing'): Promise<SkillsData> {
  const params = new URLSearchParams()
  if (profile) params.set('profile', profile)
  if (target !== 'DiTing') params.set('target', target)
  const query = params.toString() ? `?${params.toString()}` : ''
  const res = await request<SkillListResponse>(`/api/DiTing/skills${query}`)
...
export async function fetchSkillUsageStats(days = 7): Promise<SkillUsageStats> {
  ...
  return request<SkillUsageStats>(`/api/DiTing/skills/usage/stats?${params}`)
}

function targetQuery(target: SkillTarget = 'DiTing'): string {
  if (target === 'DiTing') return ''
  return `?target=${encodeURIComponent(target)}`
}

export async function fetchSkillContent(skillPath: string, target: SkillTarget = 'DiTing'): Promise<string> {
  const res = await request<{ content: string }>(`/api/DiTing/skills/${skillPath}${targetQuery(target)}`)
  return res.content
}

export async function fetchSkillFiles(category: string, skill: string, target: SkillTarget = 'DiTing'): Promise<SkillFileEntry[]> {
  const res = await request<{ files: SkillFileEntry[] }>(`/api/DiTing/skills/${category}/${skill}/files${targetQuery(target)}`)
  return res.files
}

export async function fetchMemory(): Promise<MemoryData> {
  return request<MemoryData>('/api/DiTing/memory')
}

export async function saveMemory(section: 'memory' | 'user' | 'soul', content: string): Promise<void> {
  await request('/api/DiTing/memory', {
    method: 'POST',
...
export async function toggleSkill(name: string, enabled: boolean): Promise<void> {
  await request('/api/DiTing/skills/toggle', {
    method: 'PUT',
...
export async function pinSkillApi(name: string, pinned: boolean): Promise<void> {
  await request('/api/DiTing/skills/pin', {
    method: 'PUT',
...
export async function fetchExternalDirs(): Promise<ExternalDirEntry[]> {
  const res = await request<{ dirs: ExternalDirEntry[] }>('/api/DiTing/skills/external-dirs')
  return res.dirs ?? []
}

export async function saveExternalDirs(dirs: string[]): Promise<void> {
  await request('/api/DiTing/skills/external-dirs', {
    method: 'PUT',
...
export async function deleteSkillApi(category: string, name: string): Promise<void> {
  ...
  await request(`/api/DiTing/skills/${c}/${n}`, { method: 'DELETE' })
}
...
export async function importSkill(files: File[], category?: string): Promise<{ name: string }> {
  ...
  const res = await fetch(`${baseUrl}/api/DiTing/skills/import`, {
    method: 'POST',
    headers,
    body: formData,
  })
