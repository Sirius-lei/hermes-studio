...
export interface AvailableModelGroup {
  ...
  builtin?: boolean
  /** Env var used by DiTing to override this provider's base URL. If present, the preset URL is editable. */
  base_url_env?: string
...
export async function triggerUpdate(): Promise<{ success: boolean; message: string }> {
  return request<{ success: boolean; message: string }>('/api/DiTing/update', { method: 'POST' })
}

export async function fetchPreviewStatus(): Promise<PreviewStatus> {
  return request<PreviewStatus>('/api/DiTing/update/preview')
}

export async function fetchPreviewTags(): Promise<{ tags: PreviewTag[] }> {
  return request<{ tags: PreviewTag[] }>('/api/DiTing/update/preview/tags')
}

export async function preparePreview(tag: string): Promise<PreviewActionResponse> {
  return request<PreviewActionResponse>('/api/DiTing/update/preview/prepare', {
    method: 'POST',
...
export async function installPreview(): Promise<PreviewActionResponse> {
  return request<PreviewActionResponse>('/api/DiTing/update/preview/install', { method: 'POST' })
}

export async function startPreview(tag?: string): Promise<PreviewActionResponse> {
  return request<PreviewActionResponse>('/api/DiTing/update/preview/start', {
    method: 'POST',
    body: JSON.stringify({ tag }),
  })
}
...
export async function stopPreview(): Promise<PreviewActionResponse> {
  return request<PreviewActionResponse>('/api/DiTing/update/preview/stop', { method: 'POST' })
}

export async function fetchConfigModels(): Promise<ConfigModelsResponse> {
  return request<ConfigModelsResponse>('/api/DiTing/config/models')
}

export async function fetchAvailableModels(): Promise<AvailableModelsResponse> {
  return request<AvailableModelsResponse>('/api/DiTing/available-models')
}
...
export async function fetchAvailableModelsForProfile(profile: string): Promise<AvailableModelsResponse> {
  ...
  return request<AvailableModelsResponse>(`/api/DiTing/available-models?${params.toString()}`)
}
...
export async function fetchProviderModels(data: {
  ...
  return request<{ models: string[] }>('/api/DiTing/provider-models', {
    method: 'POST',
...
export async function refreshProviderModelCache(): Promise<{ success: boolean }> {
  return request<{ success: boolean }>('/api/DiTing/provider-models/cache/refresh', {
    method: 'POST',
  })
}
...
export async function updateDefaultModel(data: {
  ...
  await request('/api/DiTing/config/model', {
    method: 'PUT',
    body: JSON.stringify(data),
  })
}
...
export async function updateModelAlias(data: {
  ...
  await request('/api/DiTing/model-alias', {
    method: 'PUT',
    body: JSON.stringify(data),
  })
}
...
export async function addCustomProvider(data: CustomProvider): Promise<void> {
  await request('/api/DiTing/config/providers', {
    method: 'POST',
    body: JSON.stringify(data),
  })
}
...
export async function removeCustomProvider(name: string, options: { source?: 'custom_providers' | 'providers'; providerKey?: string } = {}): Promise<void> {
  ...
  await request(`/api/DiTing/config/providers/${encodeURIComponent(name)}${query.size ? `?${query}` : ''}`, {
    method: 'DELETE',
  })
}
...
export async function updateProvider(poolKey: string, data: {
  ...
  await request(`/api/DiTing/config/providers/${encodeURIComponent(poolKey)}`, {
    method: 'PUT',
    body: JSON.stringify(data),
  })
}
...
export async function updateModelVisibility(data: {
  ...
  return request<{ success: boolean; model_visibility: ModelVisibility }>('/api/DiTing/model-visibility', {
    method: 'PUT',
    body: JSON.stringify(data),
  })
}
...
export async function addCustomModel(data: {
  ...
  return request<{ success: boolean; custom_models: CustomModels }>('/api/DiTing/custom-model', {
    method: 'PUT',
    body: JSON.stringify(data),
  })
}
...
export async function removeCustomModel(data: {
  ...
  return request<{ success: boolean; custom_models: CustomModels }>(`/api/DiTing/custom-model?${params.toString()}`, {
    method: 'DELETE',
  })
}
