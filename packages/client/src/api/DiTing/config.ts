...
export async function fetchConfig(sections?: string[]): Promise<AppConfig> {
  ...
  return request<AppConfig>(`/api/DiTing/config${query}`)
}
...
export async function updateConfigSection(
  ...
  await request('/api/DiTing/config', {
    method: 'PUT',
...
export async function fetchAuxiliaryModels(): Promise<AuxiliaryModelsResponse> {
  return request<AuxiliaryModelsResponse>('/api/DiTing/config/auxiliary-models')
}
...
export async function saveAuxiliaryModels(auxiliary: AuxiliaryModelsConfig): Promise<{
  ...
  return request<{ success: boolean; auxiliary: AuxiliaryModelsConfig }>('/api/DiTing/config/auxiliary-models', {
    method: 'PUT',
...
export async function saveCredentials(
  ...
  await request('/api/DiTing/config/credentials', {
    method: 'PUT',
...
export async function fetchWeixinQrCode(): Promise<WeixinQrCode> {
  return request<WeixinQrCode>('/api/DiTing/weixin/qrcode')
}

export async function pollWeixinQrStatus(qrcode: string): Promise<WeixinQrStatus> {
  return request<WeixinQrStatus>(`/api/DiTing/weixin/qrcode/status?qrcode=${encodeURIComponent(qrcode)}`)
}
...
export async function saveWeixinCredentials(data: {
  ...
  await request('/api/DiTing/weixin/save', {
    method: 'POST',
