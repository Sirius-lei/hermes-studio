...
export async function fetchProfiles(): Promise<DiTingProfile[]> {
  const res = await request<{ profiles: DiTingProfile[] }>('/api/DiTing/profiles')
  return res.profiles
}

export async function fetchProfileDetail(name: string): Promise<DiTingProfileDetail> {
  const res = await request<{ profile: DiTingProfileDetail }>(`/api/DiTing/profiles/${encodeURIComponent(name)}`)
  return res.profile
}

export async function fetchProfileRuntimeStatus(name: string): Promise<ProfileRuntimeStatus> {
  return request<ProfileRuntimeStatus>(`/api/DiTing/profiles/${encodeURIComponent(name)}/runtime-status`)
}
...
export async function fetchProfileRuntimeStatusesWithMeta(options: { refresh?: boolean } = {}): Promise<ProfileRuntimeStatusesResponse> {
  ...
  return request<ProfileRuntimeStatusesResponse>(`/api/DiTing/profiles/runtime-statuses${query}`)
}
...
export async function updateProfileAvatar(name: string, avatar: ProfileAvatar): Promise<ProfileAvatar> {
  const res = await request<{ avatar: ProfileAvatar }>(`/api/DiTing/profiles/${encodeURIComponent(name)}/avatar`, {
    method: 'PUT',
...
export async function deleteProfileAvatar(name: string): Promise<void> {
  await request(`/api/DiTing/profiles/${encodeURIComponent(name)}/avatar`, { method: 'DELETE' })
}
...
export async function restartProfileGateway(name: string): Promise<ProfileRuntimeStatus['gateway']> {
  const res = await request<{ success: boolean; gateway: ProfileRuntimeStatus['gateway'] }>(
    `/api/DiTing/profiles/${encodeURIComponent(name)}/gateway/restart`,
    { method: 'POST' },
  )
  return res.gateway
}
...
export async function restartProfileRuntime(name: string): Promise<ProfileRuntimeStatus> {
  const res = await request<{ success: boolean; status: ProfileRuntimeStatus }>(
    `/api/DiTing/profiles/${encodeURIComponent(name)}/restart`,
    { method: 'POST' },
  )
  return res.status
}
...
export async function deleteProfile(name: string): Promise<boolean> {
  try {
    await request(`/api/DiTing/profiles/${encodeURIComponent(name)}`, { method: 'DELETE' })
    return true
  } catch {
    return false
  }
...
export async function renameProfile(name: string, newName: string): Promise<boolean> {
  try {
    await request(`/api/DiTing/profiles/${encodeURIComponent(name)}/rename`, {
      method: 'POST',
...
export async function switchDiTingProfile(name: string): Promise<boolean> {
  try {
    await request('/api/DiTing/profiles/active', {
      method: 'PUT',
...
export async function exportProfile(name: string): Promise<boolean> {
  try {
    ...
    const res = await fetch(`${baseUrl}/api/DiTing/profiles/${encodeURIComponent(name)}/export`, {
      method: 'POST',
      headers,
    })
    ...
    a.href = url
    a.download = `DiTing-profile-${name}.tar.gz`
    a.click()
...
export async function importProfile(file: File): Promise<boolean> {
  try {
    ...
    const res = await fetch(`${baseUrl}/api/DiTing/profiles/import`, {
      method: 'POST',
      headers,
      body: formData,
    })
