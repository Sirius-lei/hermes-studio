...
function activeProfileName(): string | null {
  try {
    return localStorage.getItem('DiTing_active_profile_name')
  } catch {
    return null
  }
}
...
export function buildKanbanEventsWebSocketUrl(opts?: KanbanBoardOptions): string {
  ...
  const path = `/api/DiTing/kanban/events?${params.toString()}`
...
export async function listBoards(opts?: { includeArchived?: boolean }): Promise<KanbanBoard[]> {
  ...
  const res = await request<{ boards: KanbanBoard[] }>(appendQuery('/api/DiTing/kanban/boards', params))
  return res.boards
}

export async function createBoard(data: KanbanBoardCreateRequest): Promise<KanbanBoard> {
  const res = await request<{ board: KanbanBoard }>('/api/DiTing/kanban/boards', {
    method: 'POST',
...
export async function archiveBoard(slug: string): Promise<{ ok: boolean }> {
  return request<{ ok: boolean }>(`/api/DiTing/kanban/boards/${encodeURIComponent(slug)}`, {
    method: 'DELETE',
  })
}

export async function getCapabilities(): Promise<KanbanCapabilities> {
  const res = await request<{ capabilities: KanbanCapabilities }>('/api/DiTing/kanban/capabilities')
...
export async function listTasks(opts?: KanbanListOptions): Promise<KanbanTask[]> {
  ...
  if (opts?.includeArchived) params.set('includeArchived', 'true')
  const res = await request<{ tasks: KanbanTask[] }>(appendQuery('/api/DiTing/kanban', params))
  return res.tasks
}

export async function getTask(id: string, opts?: KanbanBoardOptions): Promise<KanbanTaskDetail> {
  return request<KanbanTaskDetail>(appendQuery(`/api/DiTing/kanban/${encodeURIComponent(id)}`, boardParams(opts?.board)))
}

export async function createTask(data: KanbanCreateRequest, opts?: KanbanBoardOptions): Promise<KanbanTask> {
  const res = await request<{ task: KanbanTask }>(appendQuery('/api/DiTing/kanban', boardParams(opts?.board)), {
    method: 'POST',
    body: JSON.stringify(data),
  })
  return res.task
...
export async function completeTasks(taskIds: string[], summary?: string, opts?: KanbanBoardOptions): Promise<{ ok: boolean }> {
  return request<{ ok: boolean }>(appendQuery('/api/DiTing/kanban/complete', boardParams(opts?.board)), {
    method: 'POST',
...
export async function blockTask(taskId: string, reason: string, opts?: KanbanBoardOptions): Promise<{ ok: boolean }> {
  return request<{ ok: boolean }>(appendQuery(`/api/DiTing/kanban/${encodeURIComponent(taskId)}/block`, boardParams(opts?.board)), {
    method: 'POST',
...
export async function unblockTasks(taskIds: string[], opts?: KanbanBoardOptions): Promise<{ ok: boolean }> {
  return request<{ ok: boolean }>(appendQuery('/api/DiTing/kanban/unblock', boardParams(opts?.board)), {
    method: 'POST',
...
export async function assignTask(taskId: string, profile: string, opts?: KanbanBoardOptions): Promise<{ ok: boolean }> {
  return request<{ ok: boolean }>(appendQuery(`/api/DiTing/kanban/${encodeURIComponent(taskId)}/assign`, boardParams(opts?.board)), {
    method: 'POST',
...
export async function addComment(taskId: string, data: KanbanCommentCreateRequest, opts?: KanbanBoardOptions): Promise<{ ok: boolean; output?: string }> {
  return request<{ ok: boolean; output?: string }>(appendQuery(`/api/DiTing/kanban/${encodeURIComponent(taskId)}/comments`, boardParams(opts?.board)), {
    method: 'POST',
    body: JSON.stringify(data),
  })
}
...
export async function linkTasks(data: KanbanLinkRequest, opts?: KanbanBoardOptions): Promise<{ ok: boolean; output?: string }> {
  return request<{ ok: boolean; output?: string }>(appendQuery('/api/DiTing/kanban/links', boardParams(opts?.board)), {
    method: 'POST',
    body: JSON.stringify(data),
  })
}
...
export async function unlinkTasks(data: KanbanLinkRequest, opts?: KanbanBoardOptions): Promise<{ ok: boolean; output?: string }> {
  ...
  return request<{ ok: boolean; output?: string }>(appendQuery('/api/DiTing/kanban/links', params), {
    method: 'DELETE',
  })
}
...
export async function bulkUpdateTasks(data: KanbanBulkUpdateRequest, opts?: KanbanBoardOptions): Promise<{ results: KanbanBulkTaskResult[] }> {
  return request<{ results: KanbanBulkTaskResult[] }>(appendQuery('/api/DiTing/kanban/tasks/bulk', boardParams(opts?.board)), {
    method: 'POST',
    body: JSON.stringify(data),
  })
}
...
export async function getTaskLog(taskId: string, opts?: KanbanTaskLogOptions): Promise<KanbanTaskLog> {
  ...
  return request<KanbanTaskLog>(appendQuery(`/api/DiTing/kanban/${encodeURIComponent(taskId)}/log`, params))
}
...
export async function getDiagnostics(opts?: KanbanDiagnosticsOptions): Promise<unknown[]> {
  ...
  const res = await request<{ diagnostics: unknown[] }>(appendQuery('/api/DiTing/kanban/diagnostics', params))
  return res.diagnostics
}

export async function reclaimTask(taskId: string, opts?: KanbanReclaimOptions): Promise<{ ok: boolean; output?: string }> {
  return request<{ ok: boolean; output?: string }>(appendQuery(`/api/DiTing/kanban/${encodeURIComponent(taskId)}/reclaim`, boardParams(opts?.board)), {
    method: 'POST',
...
export async function reassignTask(taskId: string, profile: string, opts?: KanbanReassignOptions): Promise<{ ok: boolean; output?: string }> {
  return request<{ ok: boolean; output?: string }>(appendQuery(`/api/DiTing/kanban/${encodeURIComponent(taskId)}/reassign`, boardParams(opts?.board)), {
    method: 'POST',
...
export async function specifyTask(taskId: string, opts?: KanbanSpecifyOptions): Promise<unknown[]> {
  const res = await request<{ results: unknown[] }>(appendQuery(`/api/DiTing/kanban/${encodeURIComponent(taskId)}/specify`, boardParams(opts?.board)), {
    method: 'POST',
...
export async function dispatch(opts?: KanbanDispatchOptions): Promise<unknown> {
  const params = boardParams(opts?.board)
  const res = await request<{ result: unknown }>(appendQuery('/api/DiTing/kanban/dispatch', params), {
    method: 'POST',
...
export async function getStats(opts?: KanbanBoardOptions): Promise<KanbanStats> {
  const res = await request<{ stats: KanbanStats }>(appendQuery('/api/DiTing/kanban/stats', boardParams(opts?.board)))
  return res.stats
}

export async function getAssignees(opts?: KanbanBoardOptions): Promise<KanbanAssignee[]> {
  const res = await request<{ assignees: KanbanAssignee[] }>(appendQuery('/api/DiTing/kanban/assignees', boardParams(opts?.board)))
