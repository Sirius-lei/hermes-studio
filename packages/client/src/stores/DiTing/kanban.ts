...
import * as kanbanApi from '@/api/DiTing/kanban'
import type { KanbanTask, KanbanStats, KanbanAssignee, KanbanBoard, KanbanCapabilities, KanbanDiagnosticsOptions, KanbanDispatchOptions, KanbanBulkUpdateRequest, KanbanCreateRequest } from '@/api/DiTing/kanban'

export const KANBAN_SELECTED_BOARD_STORAGE_KEY = 'DiTing.kanban.selectedBoard'
...
export const useKanbanStore = defineStore('kanban', () => {
  ...
  function assertCapability(key: string): void {
    if (!isCapabilitySupported(key)) {
      throw new Error(`Kanban capability "${key}" is not supported by the current DiTing backend`)
    }
  }
