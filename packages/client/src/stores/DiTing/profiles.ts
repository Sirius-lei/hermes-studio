...
import { ref } from 'vue'
import * as profilesApi from '@/api/DiTing/profiles'
import type { DiTingProfile, DiTingProfileDetail } from '@/api/DiTing/profiles'
import { useAppStore } from './app'

const ACTIVE_PROFILE_STORAGE_KEY = 'DiTing_active_profile_name'

export const useProfilesStore = defineStore('profiles', () => {
  const profiles = ref<DiTingProfile[]>([])
  ...
  const activeProfile = ref<DiTingProfile | null>(null)
  const detailMap = ref<Record<string, DiTingProfileDetail>>({})
  ...
  async function fetchDiTingProfiles() {
    loading.value = true
    try {
      profiles.value = await profilesApi.fetchProfiles()
      ...
      console.error('Failed to fetch DiTing profiles:', err)
    } finally {
      loading.value = false
    }
  }
  ...
  async function switchDiTingProfile(name: string) {
    switching.value = true
    try {
      const ok = await profilesApi.switchDiTingProfile(name)
      if (ok) await fetchDiTingProfiles()
      return ok
    } finally {
      switching.value = false
    }
