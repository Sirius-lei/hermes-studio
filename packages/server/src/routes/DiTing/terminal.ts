...
import { getActiveProfileDir } from '../../services/DiTing/DiTing-profile'
import { getTerminalConfig, type TerminalConfig } from '../../services/DiTing/file-provider'
...
export function resolveTerminalCwd(
  ...
  if (!existsSync(cwd)) {
    logger.warn({ cwd }, 'Configured terminal cwd does not exist; falling back to DiTing profile directory')
    return fallback
  }
  return cwd
}
