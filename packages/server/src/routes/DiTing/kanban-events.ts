...
import * as kanbanCli from '../../services/DiTing/DiTing-kanban'
...
export function setupKanbanEventsWebSocket(httpServers: HttpServer | HttpServer[]) {
  ...
  servers.forEach((httpServer) => {
    httpServer.on('upgrade', async (req: KanbanEventsRequest, socket, head) => {
      ...
      if (url.pathname !== '/api/DiTing/kanban/events') return
  ...
  wss.on('connection', (ws, req: KanbanEventsRequest) => {
    ...
    child.on('error', (err) => {
      logger.error(err, 'DiTing CLI: kanban watch failed')
  ...
  logger.info('WebSocket ready at /api/DiTing/kanban/events (kanban watch bridge)')
}
