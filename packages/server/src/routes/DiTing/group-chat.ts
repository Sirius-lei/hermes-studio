...
import type { GroupChatServer } from '../../services/DiTing/group-chat'
import { isReservedMentionName } from '../../services/DiTing/group-chat/mention-routing'
...
// Create room
groupChatRoutes.post('/api/DiTing/group-chat/rooms', async (ctx) => {
    if (!chatServer) {
        ctx.status = 503
...
groupChatRoutes.post('/api/DiTing/group-chat/rooms/:roomId/clone', async (ctx) => {
    if (!chatServer) {
        ctx.status = 503
        ctx.body = { error: 'Group chat not initialized' }
        return
...
groupChatRoutes.get('/api/DiTing/group-chat/rooms/:roomId', async (ctx) => {
    if (!chatServer) {
        ctx.status = 503
        ctx.body = { error: 'Group chat not initialized' }
        return
...
// List rooms
groupChatRoutes.get('/api/DiTing/group-chat/rooms', async (ctx) => {
    if (!chatServer) {
        ctx.status = 503
        ctx.body = { error: 'Group chat not initialized' }
        return
...
groupChatRoutes.get('/api/DiTing/group-chat/rooms/join/:code', async (ctx) => {
    if (!chatServer) {
        ctx.status = 503
        ctx.body = { error: 'Group chat not initialized' }
        return
...
groupChatRoutes.put('/api/DiTing/group-chat/rooms/:roomId/invite-code', async (ctx) => {
    if (!chatServer) {
        ctx.status = 503
        ctx.body = { error: 'Group chat not initialized' }
        return
...
// Add agent to room
groupChatRoutes.post('/api/DiTing/group-chat/rooms/:roomId/agents', async (ctx) => {
    if (!chatServer) {
        ctx.status = 503
        ctx.body = { error: 'Group chat not initialized' }
        return
...
// List agents in room
groupChatRoutes.get('/api/DiTing/group-chat/rooms/:roomId/agents', async (ctx) => {
    if (!chatServer) {
        ctx.status = 503
        ctx.body = { error: 'Group chat not initialized' }
        return
...
groupChatRoutes.delete('/api/DiTing/group-chat/rooms/:roomId/agents/:agentId', async (ctx) => {
    if (!chatServer) {
        ctx.status = 503
        ctx.body = { error: 'Group chat not initialized' }
        return
...
// Delete room
groupChatRoutes.delete('/api/DiTing/group-chat/rooms/:roomId', async (ctx) => {
    if (!chatServer) {
        ctx.status = 503
        ctx.body = { error: 'Group chat not initialized' }
        return
...
groupChatRoutes.post('/api/DiTing/group-chat/rooms/:roomId/clear-context', async (ctx) => {
    if (!chatServer) {
        ctx.status = 503
        ctx.body = { error: 'Group chat not initialized' }
        return
...
groupChatRoutes.put('/api/DiTing/group-chat/rooms/:roomId/config', async (ctx) => {
    if (!chatServer) {
        ctx.status = 503
        ctx.body = { error: 'Group chat not initialized' }
        return
...
groupChatRoutes.post('/api/DiTing/group-chat/rooms/:roomId/compress', async (ctx) => {
    if (!chatServer) {
        ctx.status = 503
        ctx.body = { error: 'Group chat not initialized' }
        return
