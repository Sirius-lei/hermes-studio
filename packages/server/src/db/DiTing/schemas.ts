/**
 * Centralized schema definitions for all DiTing SQLite tables.
...
export const DEVICES_SCHEMA: Record<string, string> = {
  ...
  DiTing_agent_version: "TEXT NOT NULL DEFAULT ''",
  DiTing_web_ui_version: "TEXT NOT NULL DEFAULT ''",
...
// ============================================================================
// Group Chat (services/DiTing/group-chat/index.ts)
// ============================================================================
...
/**
 * Initialize missing DiTing SQLite tables with proper schemas.
 * Existing tables only receive safe additive columns.
 * Call this once at application bootstrap.
 */
export function initAllDiTingTables(): void {
  const db = getDb()
  if (!db) return
  ...
  try {
    ...
    console.error('Error initializing DiTing SQLite tables:', e)
