import { getActiveProfileDir, getProfileDir } from '../../services/DiTing/DiTing-profile'
...
export interface DiTingSessionRow {
  id: string
  source: string
  user_id: string | null
  model: string
...
export interface DiTingSessionSearchRow extends DiTingSessionRow {
  matched_message_id: number | null
  snippet: string
  rank: number
}

export interface DiTingMessageRow {
  id: number | string
  session_id: string
  role: string
  content: string
...
export interface DiTingSessionDetailRow extends DiTingSessionRow {
  messages: DiTingMessageRow[]
  thread_session_count: number
}

export interface PaginatedDiTingSessionDetailResult {
  session: DiTingSessionDetailRow
  messages: DiTingMessageRow[]
  total: number
  offset: number
  limit: number
...
interface DiTingSessionInternalRow extends DiTingSessionRow {
...
function mapRow(row: Record<string, unknown>): DiTingSessionRow {
...
function mapSearchRow(row: Record<string, unknown>): DiTingSessionSearchRow {
  return {
    ...mapRow(row),
...
function mapInternalSessionRow(row: Record<string, unknown>): DiTingSessionInternalRow {
  return {
    ...mapRow(row),
...
function mapMessageRow(row: Record<string, unknown>): DiTingMessageRow {
...
function isCompressionEnded(session: DiTingSessionInternalRow | undefined): boolean {
  return !!session && COMPRESSION_END_REASONS.has(String(session.end_reason || ''))
}

function isCompressionContinuation(parent: DiTingSessionInternalRow | undefined, child: DiTingSessionInternalRow | undefined): boolean {
...
function latestSessionInChain(chain: DiTingSessionInternalRow[]): DiTingSessionInternalRow {
...
function projectSessionSummary(root: DiTingSessionInternalRow, chain: DiTingSessionInternalRow[]): DiTingSessionRow {
...
interface SessionIndex {
  byId: Map<string, DiTingSessionInternalRow>
...
function getLatestContinuationChild(
  parent: DiTingSessionInternalRow,
  idx: SessionIndex,
): DiTingSessionInternalRow | null {
  ...
  const candidates = (idx.childrenByParent.get(parent.id) || [])
    ...
    .filter((c): c is DiTingSessionInternalRow => !!c)
...
function collectCompressionPath(
  session: DiTingSessionInternalRow,
  idx: SessionIndex,
): DiTingSessionInternalRow[] {
  const reversed: DiTingSessionInternalRow[] = [session]
  const seen = new Set<string>()
  let current: DiTingSessionInternalRow | null = session
...
function extendCompressionChain(
  chain: DiTingSessionInternalRow[],
  idx: SessionIndex,
): DiTingSessionInternalRow[] {
  const result = [...chain]
  const seen = new Set(result.map(s => s.id))
  let current: DiTingSessionInternalRow | null = result[result.length - 1] || null
 ...
 * without chain traversal. Used by syncFromDiTing for ephemeral sessions.
 */
export async function getSessionMessagesFromDb(sessionId: string): Promise<{
  messages: DiTingMessageRow[]
  session: DiTingSessionRow | null
} | null> {
...
export async function getSessionDetailFromDb(sessionId: string): Promise<DiTingSessionDetailRow | null> {
  const db = await openSessionDb()
  try {
...
export async function getSessionDetailFromDbWithProfile(sessionId: string, profile: string): Promise<DiTingSessionDetailRow | null> {
  const { DatabaseSync } = await import('node:sqlite')
...
export async function getSessionDetailPaginatedFromDbWithProfile(
  ...
  offset = 0,
  limit = 150,
): Promise<PaginatedDiTingSessionDetailResult | null> {
...
export async function getExactSessionDetailFromDbWithProfile(sessionId: string, profile: string): Promise<DiTingSessionDetailRow | null> {
  const { DatabaseSync } = await import('node:sqlite')
  const dbPath = join(getProfileDir(profile), 'state.db')
  const db = new DatabaseSync(dbPath, { open: true, readOnly: true })
  try {
...
export interface DiTingUsageStats extends LocalUsageStats {
  cost: number
  total_api_calls: number
}

export interface DiTingSkillUsageRow {
  skill: string
  view_count: number
  manage_count: number
  total_count: number
...
export interface DiTingSkillUsageDailySkillRow {
  skill: string
  view_count: number
  manage_count: number
  total_count: number
...
export interface DiTingSkillUsageDailyRow {
  date: string
  view_count: number
  manage_count: number
  total_count: number
  skills: DiTingSkillUsageDailySkillRow[]
}

export interface DiTingSkillUsageStats {
  period_days: number
  summary: {
  ...
  by_day: DiTingSkillUsageDailyRow[]
  top_skills: DiTingSkillUsageRow[]
}
...
export async function getUsageStatsFromDb(
  ...
  nowSeconds = Math.floor(Date.now() / 1000),
  profile?: string,
): Promise<DiTingUsageStats> {
  const empty: DiTingUsageStats = {
    input_tokens: 0,
    output_tokens: 0,
    cache_read_tokens: 0,
...
export async function listSessionSummaries(source?: string, limit = 2000, profile?: string): Promise<DiTingSessionRow[]> {
  if (!SQLITE_AVAILABLE) {
    throw new Error(`node:sqlite requires Node >= 22.5, current: ${process.versions.node}`)
  }
...
export async function searchSessionSummariesWithProfile(
  ...
  source?: string,
  limit = 20,
): Promise<DiTingSessionSearchRow[]> {
  if (!SQLITE_AVAILABLE) {
    throw new Error(`node:sqlite requires Node >= 22.5, current: ${process.versions.node}`)
  }
  ...
  try {
    ...
    const idx = loadAllSessions(db)
    const merged = new Map<string, DiTingSessionSearchRow>()
