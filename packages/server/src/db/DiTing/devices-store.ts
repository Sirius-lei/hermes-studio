...
function rowToRecord(row: StoredDeviceRow | Record<string, any>): DeviceRelationRecord {
  ...
  return {
    ...
    url: String(row.url || ''),
    os,
    DiTing_agent_version: String(row.DiTing_agent_version || ''),
    DiTing_web_ui_version: String(row.DiTing_web_ui_version || ''),
...
function deviceToRow(device: LanDeviceInfo | DeviceRelationRecord, existing: DeviceRelationRecord | null, now: number): StoredDeviceRow {
  return {
    ...
    DiTing_agent_version: device.DiTing_agent_version,
    DiTing_web_ui_version: device.DiTing_web_ui_version,
...
function saveRow(row: StoredDeviceRow): DeviceRelationRecord {
  ...
  db.prepare(`
    INSERT INTO ${DEVICES_TABLE} (
      ...
      ip, http_port, url, os_json, DiTing_agent_version, DiTing_web_ui_version,
    ...
    ON CONFLICT(id) DO UPDATE SET
      ...
      DiTing_agent_version = excluded.DiTing_agent_version,
      DiTing_web_ui_version = excluded.DiTing_web_ui_version,
  ...
  `).run(
    ...
    row.url,
    row.os_json,
    row.DiTing_agent_version,
    row.DiTing_web_ui_version,
    row.response_ms,
