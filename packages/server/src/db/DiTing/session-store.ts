/**
 * Self-built session database — completely replaces DiTing CLI dependency.
...
// Tables are created automatically on bootstrap via initAllDiTingTables()
...
function mapSessionRow(row: Record<string, unknown>): DiTingSessionRow {
...
export function createSession(data: {
  ...
  const agent = data.agent || (source === 'cli' ? 'DiTing' : '')
...
export function getSession(id: string): DiTingSessionRow | null {
  if (!isSqliteAvailable()) return null
  const db = getDb()!
...
export function updateSession(id: string, data: Partial<Omit<DiTingSessionRow, 'id' | 'profile'>>): void {
...
export function getSessionDetail(id: string): DiTingSessionDetailRow | null {
  if (!isSqliteAvailable()) return null
  const db = getDb()!
