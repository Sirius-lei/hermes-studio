...
function startRuntimeServicesAfterListen(): void {
  ...
  void (async () => {
    try {
      ...
      console.log(`[bootstrap] agent bridge started in ${Date.now() - startedAt}ms`)
      const warmed = await warmAgentBridgeProfilesOnStartup(agentBridgeManager.getRuntimeState?.().endpoint)
      if (warmed.length > 0) {
        console.log(`[bootstrap] warmed agent bridge profiles in ${Date.now() - startedAt}ms: ${warmed.join(', ')}`)
      }
    } catch (err) {
      ...
      if (agentBridgeRequired()) {
        console.error('[bootstrap] DiTing_WEB_UI_REQUIRE_AGENT_BRIDGE is enabled; stopping server')
        servers.forEach(httpServer => httpServer.close())
        process.exitCode = 1
      }
      return
    }
...
export async function bootstrap() {
  ...
  if (isDesktopRuntime()) agentBridgeManager = getAgentBridgeManager()
