...
const MANIFEST_OWNER = 'diting-web-ui'
...
export class DiTingSkillInjector {
  private readonly targetDirs: string[]

  constructor(
    private readonly sourceDir = DiTingSkillInjector.resolveSourceDir(),
    targetDirOrDirs: string | string[] = DiTingSkillInjector.resolveTargetDirs(),
  ) {
  ...
  static resolveSourceDir(env: NodeJS.ProcessEnv = process.env, baseDir = __dirname): string {
    const override = env.DiTing_WEB_UI_SKILLS_DIR?.trim()
    if (override) return resolve(override)

    const candidates = [
      // Production bundle: dist/server/index.js with dist/skills copied by build.
      resolve(baseDir, '../skills'),
      // Development/test: packages/server/src/services/DiTing -> packages/skills.
      resolve(baseDir, '../../../../skills'),
  ...
  static resolveTargetDirs(rootDir = detectDiTingRootHome()): string[] {
  ...
  static resolveTargetDirForProfile(profile: string, rootDir = detectDiTingRootHome()): string {
  ...
  private static profileForTargetDir(targetDir: string, rootDir = detectDiTingRootHome()): string {
    const root = resolve(rootDir)
  ...
  private async injectIntoTarget(targetDir: string, entries: import('fs').Dirent[]): Promise<SkillInjectionTargetResult> {
    const profile = DiTingSkillInjector.profileForTargetDir(targetDir)
