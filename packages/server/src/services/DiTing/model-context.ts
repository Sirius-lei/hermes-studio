...
const DiTing_BASE = detectDiTingHome()
const MODELS_DEV_CACHE = resolve(DiTing_BASE, 'models_dev_cache.json')
...
function getProfileDir(profile?: string): string {
  if (!profile || profile === 'default') return DiTing_BASE
  const dir = join(DiTing_BASE, 'profiles', profile)
  return existsSync(dir) ? dir : DiTing_BASE
}
