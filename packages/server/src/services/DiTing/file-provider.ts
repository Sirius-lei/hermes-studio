...
import { getActiveProfileDir, getActiveEnvPath, getProfileDir } from './DiTing-profile'
import { isPathWithin, relativePathFromBase } from './DiTing-path'
...
export interface FileEntry {
  name: string
  path: string       // relative to DiTing home
  isDir: boolean
  size: number
...
/**
 * Resolve a relative path to an absolute path under the DiTing home directory.
 * Validates path safety (no traversal).
 */
export function resolveDiTingPath(relativePath: string, profile?: string): string {
...
/**
 * Read terminal config from DiTing config.yaml.
 */
...
/**
 * Read SSH env vars from DiTing .env file.
 */
...
async function resolveDockerContainer(cfg: TerminalConfig): Promise<string> {
  ...
  throw Object.assign(
    new Error('Cannot determine Docker container. Set terminal.docker_container_name in DiTing config.'),
    { code: 'backend_error' },
  )
}
...
/**
 * Create a FileProvider based on the active DiTing terminal config.
