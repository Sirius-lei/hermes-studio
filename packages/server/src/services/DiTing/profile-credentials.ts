 ...
 * 智能克隆 Profile 凭据管理
 *
 * 背景：`DiTing profile create --clone` 会完整复制源 profile 的 .env + config.yaml，
 * 包括各平台的独占凭据（Weixin / Telegram / Slack / ...）。
 * 这导致多个 profile 同时持有同一个 bot token，diting-agent 内部的 token 互斥机制
...
import { detectDiTingHome } from './DiTing-path'

function DiTingBase(): string {
  return detectDiTingHome()
}

function profileDir(profileName: string): string {
  const base = DiTingBase()
...
function activeProfileName(): string {
  try {
    return readFileSync(join(DiTingBase(), 'active_profile'), 'utf-8').trim() || 'default'
  } catch {
    return 'default'
  }
}
 ...
 * 多个 profile 共享同一凭据会触发 diting-agent 的 token 互斥机制 → 启动失败。
 *
 * 不在此列表的（模型 provider API key、工具调试开关等）视为可安全共享。
 *
 * **来源（不要凭主观推测扩展）**：与 diting-agent `gateway/platforms/` 中实际调用
 ...
 * provider only.
 *
 * DiTing CLI `profile create --clone` copies config/.env/SOUL/skills, but not
 ...
 *
 * @param profileName profile 名称（'default' → ~/.diting/，其他 → ~/.diting/profiles/<name>/）
 */
