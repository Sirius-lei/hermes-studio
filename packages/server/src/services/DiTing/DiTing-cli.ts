...
import { getActiveProfileDir, getActiveProfileName, getProfileDir, listProfileNamesFromDisk } from './DiTing-profile'
...
import { execDiTingWithBin, resolveDiTingBin as resolveRuntimeDiTingBin, spawnDiTingWithBin } from './DiTing-process'
...
async function stopGatewayForActiveProfile(): Promise<void> {
  try {
    await execDiTingWithBin(DiTing_BIN, ['gateway', 'stop'], {
      timeout: 30000,
      ...activeGatewayExecOpts(),
    })
  } catch (err) {
    logger.warn(err, 'DiTing gateway stop before restart failed; continuing with run --replace')
  }
}
...
function activeGatewayExecOpts() {
  return {
    ...
    env: {
      ...process.env,
      DiTing_HOME: getActiveProfileDir(),
    },
  }
}

export interface DiTingSession {
  id: string
  source: string
  user_id: string | null
  model: string
...
export interface DiTingSessionFull {
  id: string
  source: string
  user_id: string | null
  model: string
...
function parseSessionExport(stdout: string): DiTingSessionFull[] {
  const lines = stdout.trim().split('\n').filter(Boolean)
  const sessions: DiTingSessionFull[] = []
  for (const line of lines) {
    try {
      const raw: DiTingSessionFull = JSON.parse(line)
      sessions.push(raw)
    } catch {
...
export async function exportSessionsRaw(source?: string): Promise<DiTingSessionFull[]> {
  const args = ['sessions', 'export', '-']
  if (source) args.push('--source', source)

  try {
    const { stdout } = await execDiTingWithBin(DiTing_BIN, args, {
    ...
    logger.error(err, 'DiTing CLI: sessions export failed')
    throw new Error(`Failed to list sessions: ${err.message}`)
  }
}

/**
 * List sessions from DiTing CLI (without messages)
 */
export async function listSessions(source?: string, limit?: number): Promise<DiTingSession[]> {
  const raws = await exportSessionsRaw(source)
  const sessions: DiTingSession[] = []
...
/**
 * Get a single session with messages from DiTing CLI
 */
export async function getSession(id: string): Promise<DiTingSession | null> {
  ...
  try {
    ...
    const raw: DiTingSessionFull = raws[0]
    return {
      id: raw.id,
      source: raw.source,
      user_id: raw.user_id,
    ...
    logger.error(err, 'DiTing CLI: session export failed')
    throw new Error(`Failed to get session: ${err.message}`)
  }
}

/**
 * Delete a session from DiTing CLI
 */
export async function deleteSession(id: string): Promise<boolean> {
  try {
    await execDiTingWithBin(DiTing_BIN, ['sessions', 'delete', id, '--yes'], {
      timeout: 10000,
      ...execOpts,
    })
    return true
  } catch (err: any) {
    logger.error(err, 'DiTing CLI: session delete failed')
    return false
  }
}

/**
 * Delete a session from a specific DiTing profile.
 */
export async function deleteSessionForProfile(id: string, profile: string): Promise<boolean> {
  try {
    await execDiTingWithBin(DiTing_BIN, ['sessions', 'delete', id, '--yes'], {
      ...
      env: {
        ...process.env,
        DiTing_HOME: getProfileDir(profile),
      },
    })
    return true
  } catch (err: any) {
    logger.error({ err, sessionId: id, profile }, 'DiTing CLI: profile session delete failed')
    return false
  }
}

/**
 * Rename a session title via DiTing CLI
 */
export async function renameSession(id: string, title: string): Promise<boolean> {
  try {
    await execDiTingWithBin(DiTing_BIN, ['sessions', 'rename', id, title], {
      timeout: 10000,
      ...execOpts,
    })
    return true
  } catch (err: any) {
    logger.error(err, 'DiTing CLI: session rename failed')
    return false
  }
}
...
export async function getVersion(): Promise<string> {
  try {
    const { stdout } = await execDiTingWithBin(DiTing_BIN, ['--version'], { timeout: 5000, ...execOpts })
    return stdout.trim()
  } catch {
    return ''
  }
...
/**
 * Start DiTing gateway (uses launchd/systemd)
 */
export async function startGateway(): Promise<string> {
  ...
  const { stdout, stderr } = await execDiTingWithBin(DiTing_BIN, ['gateway', 'start'], {
    timeout: 30000,
    ...activeGatewayExecOpts(),
  })
...
/**
 * Start DiTing gateway in background (for WSL where launchd/systemd is unavailable)
 * Uses "DiTing gateway run" as a detached background process
 */
export async function startGatewayBackground(): Promise<number | null> {
  const child = spawnDiTingWithBin(DiTing_BIN, ['gateway', 'run'], {
    detached: true,
    stdio: 'ignore',
    windowsHide: true,
    env: {
...
/**
 * Restart DiTing gateway through DiTing CLI, falling back to detached
...
export async function restartGateway(): Promise<string> {
  ...
  if (isDocker || isTermux || process.platform === 'win32') {
    ...
    const result = startGatewayRunManaged(DiTing_BIN, { profileDir })
  ...
  try {
    const { stdout, stderr } = await execDiTingWithBin(DiTing_BIN, ['gateway', 'restart'], {
      timeout: 30000,
      ...activeGatewayExecOpts(),
    })
    const ready = await waitForGatewayRunning(profileDir)
    if (!ready) throw new Error('DiTing gateway restart completed but gateway did not report running within timeout')
    return stdout || stderr
  } catch (err: any) {
    logger.warn(err, 'DiTing gateway restart failed; falling back to gateway run')
    await stopGatewayForActiveProfile()
    const lockReleased = await waitForGatewayLockReleasedAfterStop(profileDir)
...
export async function stopGateway(): Promise<string> {
  const { stdout, stderr } = await execDiTingWithBin(DiTing_BIN, ['gateway', 'stop'], {
    timeout: 30000,
    ...activeGatewayExecOpts(),
  })
  return stdout || stderr
...
export async function listLogFiles(): Promise<LogFileInfo[]> {
  try {
    const { stdout } = await execDiTingWithBin(DiTing_BIN, ['logs', 'list'], {
      timeout: 10000,
      ...execOpts,
    })
    ...
    logger.error(err, 'DiTing CLI: logs list failed')
    return []
  }
}
...
export async function readLogs(
  ...
  try {
    ...
    logger.error(err, 'DiTing CLI: logs read failed')
...
export interface DiTingProfile {
  name: string
  active: boolean
  model: string
...
export interface DiTingProfileDetail {
  name: string
  path: string
  model: string
  provider: string
...
function readProfileDefaultModel(name: string): string {
  ...
  try {
    ...
    logger.warn(err, 'DiTing CLI: failed to read profile config model for %s', name)
  }
  return '—'
}
 ...
 * List all profiles
 */
export async function listProfiles(): Promise<DiTingProfile[]> {
  const profileNames = listProfileNamesFromDisk()
  const activeProfileName = getActiveProfileName()
  let runtimeInfo = new Map<string, ProfileListRuntimeInfo>()
  try {
    const { stdout } = await execDiTingWithBin(DiTing_BIN, ['profile', 'list'], {
      timeout: 10000,
      ...execOpts,
    })
    runtimeInfo = parseProfileListRuntimeInfo(stdout, profileNames)
  } catch (err: any) {
    logger.warn(err, 'DiTing CLI: profile list failed; falling back to disk profile list')
  }
 ...
 * Get profile details
 */
export async function getProfile(name: string): Promise<DiTingProfileDetail> {
  try {
    const { stdout } = await execDiTingWithBin(DiTing_BIN, ['profile', 'show', name], {
      timeout: 10000,
      ...execOpts,
    })
    ...
    logger.error(err, 'DiTing CLI: profile show failed')
...
export async function createProfile(name: string, clone?: boolean): Promise<string> {
  ...
  try {
    const { stdout, stderr } = await execDiTingWithBin(DiTing_BIN, args, {
      timeout: 15000,
      ...execOpts,
    })
    return stdout || stderr
  } catch (err: any) {
    logger.error(err, 'DiTing CLI: profile create failed')
...
export async function deleteProfile(name: string): Promise<boolean> {
  try {
    await execDiTingWithBin(DiTing_BIN, ['profile', 'delete', name, '--yes'], {
      timeout: 10000,
      ...execOpts,
    })
    return true
  } catch (err: any) {
    logger.error(err, 'DiTing CLI: profile delete failed')
    return false
  }
}
...
export async function renameProfile(oldName: string, newName: string): Promise<boolean> {
  try {
    await execDiTingWithBin(DiTing_BIN, ['profile', 'rename', oldName, newName], {
      timeout: 10000,
      ...execOpts,
    })
    return true
  } catch (err: any) {
    logger.error(err, 'DiTing CLI: profile rename failed')
    return false
  }
}
...
export async function useProfile(name: string): Promise<string> {
  try {
    const { stdout, stderr } = await execDiTingWithBin(DiTing_BIN, ['profile', 'use', name], {
      timeout: 10000,
      ...execOpts,
    })
    return stdout || stderr
  } catch (err: any) {
    logger.error(err, 'DiTing CLI: profile use failed')
...
export async function exportProfile(name: string, outputPath?: string): Promise<string> {
  ...
  try {
    ...
    logger.error(err, 'DiTing CLI: profile export failed')
    throw new Error(`Failed to export profile: ${err.message}`)
  }
}

/**
 * Run DiTing setup --non-interactive --reset to generate default config for current profile
 */
export async function setupReset(): Promise<string> {
  try {
    const { stdout, stderr } = await execDiTingWithBin(DiTing_BIN, ['setup', '--non-interactive', '--reset'], {
      timeout: 30000,
      ...execOpts,
    })
    return stdout || stderr
  } catch (err: any) {
    logger.error(err, 'DiTing CLI: setup reset failed')
...
export async function importProfile(archivePath: string, name?: string): Promise<string> {
  ...
  try {
    ...
    logger.error(err, 'DiTing CLI: profile import failed')
    throw new Error(`Failed to import profile: ${err.message}`)
  }
}

/**
 * Pin or unpin a skill via DiTing curator
 */
export async function pinSkill(name: string, pinned: boolean): Promise<string> {
  const subcmd = pinned ? 'pin' : 'unpin'
  try {
    const { stdout, stderr } = await execDiTingWithBin(DiTing_BIN, ['curator', subcmd, name], {
      timeout: 15000,
      ...execOpts,
    })
    return stdout || stderr
  } catch (err: any) {
    logger.error(err, `DiTing CLI: curator ${subcmd} failed`)
