...
import { listProfileNamesFromDisk } from './DiTing-profile'

const LEGACY_SERVER_NAME = 'DiTing-studio'
...
const MANAGED_ENV_KEY = 'DiTing_WEB_UI_MANAGED_MCP'
...
function isDisabled(): boolean {
  return isEnabledEnv(process.env.DiTing_WEB_UI_DISABLE_MCP_AUTOINJECT)
}

function allowTransientAutoinject(): boolean {
  return isEnabledEnv(process.env.DiTing_WEB_UI_ALLOW_TRANSIENT_MCP_AUTOINJECT)
}
...
function managedConfig(profile: string, serverName: string, toolset: string): Record<string, unknown> {
  const env: Record<string, string> = {
    DiTing_WEB_UI_URL: `http://127.0.0.1:${config.port}`,
    DiTing_WEB_UI_HOME: config.appHome,
    DiTing_WEBUI_STATE_DIR: config.appHome,
...
function sameConfig(existing: Record<string, any>, desired: Record<string, unknown>): boolean {
  ...
  return existing.command === desired.command &&
    ...
    existing.env.DiTing_WEB_UI_URL === desiredEnv.DiTing_WEB_UI_URL &&
    existing.env.DiTing_WEB_UI_HOME === desiredEnv.DiTing_WEB_UI_HOME &&
    existing.env.DiTing_WEBUI_STATE_DIR === desiredEnv.DiTing_WEBUI_STATE_DIR &&
    existing.env.DiTing_WEB_UI_PROFILE === desiredEnv.DiTing_WEB_UI_PROFILE &&
    existing.env.DiTing_MCP_SERVER_NAME === desiredEnv.DiTing_MCP_SERVER_NAME &&
    existing.env.DiTing_MCP_TOOLSET === desiredEnv.DiTing_MCP_TOOLSET &&
    existing.env.DiTing_WEB_UI_TOKEN === undefined &&
      ...
      if (LEGACY_SERVER_NAMES.has(name) && !isManagedServer(server)) {
        return {
          ...
          result: {
            profile,
            status: 'skipped',
            reason: `existing ${name} MCP server is not managed by DiTing Web UI`,
      ...
      if (!isManagedServer(existing)) {
        return {
          ...
          result: {
            profile,
            status: 'skipped',
            reason: `existing ${server.name} MCP server is not managed by DiTing Web UI`,
          } satisfies BundledMcpInjectionTargetResult,
        }
      }
...
export async function injectBundledMcpServer(): Promise<BundledMcpInjectionResult> {
  ...
  if (isDisabled()) {
    logger.info('[mcp-autoinject] disabled by DiTing_WEB_UI_DISABLE_MCP_AUTOINJECT')
    return result
  }
