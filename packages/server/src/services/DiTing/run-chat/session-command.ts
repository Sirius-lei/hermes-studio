...
import { addMessage, clearSessionMessages, createBranchedSession, createSession, getSession, getSessionDetail, renameSession, updateSessionStats } from '../../../db/DiTing/session-store'
...
function createBranchSession(parentSessionId: string, requestedTitle: string, ctx: SessionCommandContext): BranchSessionSummary | null {
  ...
  const persisted = createBranchedSession({
    ...
    source,
    agent: parent.agent || (source === 'cli' ? 'DiTing' : ''),
