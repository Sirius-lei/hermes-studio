 ...
 * to communicate with DiTing CLI agent.
 */

import type { Server, Socket } from 'socket.io'
import { getSystemPrompt } from '../../../lib/llm-prompt'
import { getSession, getSessionDetail, createSession, addMessage, updateSession, updateSessionStats } from '../../../db/DiTing/session-store'
...
function syncBridgeGeneratedTitle(sessionId: string, title: unknown, emit: (event: string, payload: any) => void): boolean {
  ...
  if (!isReplaceableLocalTitle(sessionId)) {
    logger.info('[chat-run-socket] skipped DiTing generated title for manually titled session %s', sessionId)
    return false
  }
...
export async function handleBridgeRun(
  ...
  const runPrompt = [
    ...
    'When calling DiTing Web UI endpoints from tools or skills, include the current DiTing profile as the X-DiTing-Profile header if the endpoint supports profile-scoped behavior.',
  ].filter(Boolean).join('\n')
  fullInstructions = `\n${runPrompt}\n${fullInstructions}`
...
async function maybeEnqueueGoalContinuation(args: {
  ...
  if (isGoalJudgeUnavailable(decision.reason)) {
    emitGoalStatus(
      ...
      args.state,
      'judge_unavailable',
      'Goal judge is not configured; automatic goal continuation was skipped. The goal remains active, but DiTing cannot mark it done automatically.',
    )
    return
  }
