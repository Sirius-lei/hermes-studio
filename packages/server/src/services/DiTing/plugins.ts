...
import { getActiveProfileDir, getProfileDir } from './DiTing-profile'
import { resolveAgentBridgeCommand } from './agent-bridge/manager'

const execFileAsync = promisify(execFile)

export type DiTingPluginSource = 'bundled' | 'user' | 'project' | 'entrypoint'
export type DiTingPluginKind = 'standalone' | 'backend' | 'exclusive' | 'platform' | 'model-provider'
export type DiTingPluginConfigStatus = 'enabled' | 'disabled' | 'not-enabled' | 'auto' | 'provider-managed'
export type DiTingPluginEffectiveStatus = 'enabled' | 'disabled' | 'inactive' | 'auto-active' | 'provider-managed'

export interface DiTingPluginInfo {
  key: string
  name: string
  kind: DiTingPluginKind | string
  source: DiTingPluginSource | string
  configStatus: DiTingPluginConfigStatus
  effectiveStatus: DiTingPluginEffectiveStatus
  version: string
  description: string
  author: string
  path: string
...
warnings = []
agent_root = os.environ.get("DiTing_AGENT_ROOT_RESOLVED", "")

# python -c normally prepends the process cwd to sys.path. Remove it before any
# DiTing imports so an arbitrary WUI launch directory cannot shadow modules like
# diting_cli, diting_constants, utils, or yaml. The process cwd is still preserved
...
try:
    ...
    print(json.dumps({
        "error": "Failed to import DiTing Agent plugin modules",
        "detail": str(exc),
...
project_plugins_enabled = env_enabled("DiTing_ENABLE_PROJECT_PLUGINS")
...
print(json.dumps({
    ...
    "warnings": warnings,
    "metadata": {
        "DiTingAgentRoot": os.environ.get("DiTing_AGENT_ROOT_RESOLVED", ""),
...
export async function listDiTingPlugins(profile?: string): Promise<DiTingPluginsResponse> {
  const command = resolveAgentBridgeCommand()
  const agentRoot = command.agentRoot || ''
  const DiTingHome = profile ? getProfileDir(profile) : getActiveProfileDir()
  const env: NodeJS.ProcessEnv = {
    ...process.env,
    DiTing_AGENT_ROOT_RESOLVED: agentRoot,
  ...
  try {
    ...
    const parsed = JSON.parse(stdout) as DiTingPluginsResponse & { error?: string; detail?: string }
  ...
  throw new Error(`Failed to discover DiTing plugins.\n${errors.join('\n')}`)
}
