...
def _set_path_env(agent_root: str | None, DiTing_home: str | None) -> None:
    _sync_runtime_patches()
    _ORIGINAL_SET_PATH_ENV(agent_root, DiTing_home)
...
class BridgeBroker(_broker.BridgeBroker):
    def __init__(self, endpoint: str, agent_root: str | None = None, DiTing_home: str | None = None) -> None:
        _sync_broker_patches()
        super().__init__(endpoint, agent_root, DiTing_home)
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="DiTing AIAgent in-process bridge")
    parser.add_argument("--endpoint", default=os.environ.get("DiTing_AGENT_BRIDGE_ENDPOINT", DEFAULT_ENDPOINT))
    parser.add_argument("--agent-root", default=os.environ.get("DiTing_AGENT_ROOT"))
    parser.add_argument("--diting-home", default=os.environ.get("DiTing_HOME"))
    parser.add_argument("--worker-profile", default=os.environ.get("DiTing_AGENT_BRIDGE_WORKER_PROFILE"))
    args = parser.parse_args(argv)

    _set_path_env(args.agent_root, args.diting_home)
    _ensure_agent_imports()
    if args.worker_profile:
        ...
        BridgeBroker(args.endpoint, args.agent_root, args.diting_home).serve_forever()
    return 0
