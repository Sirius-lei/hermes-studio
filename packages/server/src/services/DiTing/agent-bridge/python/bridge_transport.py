...
class WorkerProcess:
    ...
    def __init__(self, key: str, profile: str, endpoint: str, agent_root: str | None, DiTing_home: str | None) -> None:
        self.key = key or profile or "default"
        self.profile = profile or "default"
        self.endpoint = endpoint
        self.agent_root = agent_root
        self.DiTing_home = DiTing_home
    ...
    def start(self) -> None:
        with self._lock:
            ...
            args = [
                sys.executable,
                str(Path(__file__).with_name("DiTing_bridge.py").resolve()),
                "--endpoint",
                self.endpoint,
                "--worker-profile",
                self.profile,
            ...
            if self.DiTing_home:
                args.extend(["--DiTing-home", self.DiTing_home])

            env = {
                **os.environ,
                "DiTing_AGENT_BRIDGE_ENDPOINT": self.endpoint,
                "DiTing_AGENT_BRIDGE_WORKER_PROFILE": self.profile,
                "DiTing_AGENT_BRIDGE_BROKER_PID": str(os.getpid()),
            }
    ...
    def _pipe_stderr(self) -> None:
        ...
        def run() -> None:
            ...
            for line in proc.stderr:
                text = line.rstrip()
                if text:
                    print(f"[DiTing-bridge-worker:{self.key}] {text}", file=sys.stderr, flush=True)

        threading.Thread(target=run, daemon=True, name=f"DiTing-bridge-worker-stderr-{self.key}").start()
    ...
    def _wait_ready(self) -> None:
        ...
        threading.Thread(target=read_stdout, daemon=True, name=f"DiTing-bridge-worker-stdout-{self.key}").start()
...
def _worker_endpoint(key: str, namespace: str | None = None) -> str:
    ...
    transport = (
        os.environ.get("DiTing_AGENT_BRIDGE_WORKER_TRANSPORT", "")
    ).strip().lower()
    use_tcp = transport == "tcp" or (transport not in {"ipc", "unix"} and os.name == "nt")
    if use_tcp:
        port_base = int(os.environ.get("DiTing_AGENT_BRIDGE_WORKER_PORT_BASE") or "18780")
        return f"tcp://127.0.0.1:{port_base + int(safe[:4], 16) % 1000}"
    root = Path(tempfile.gettempdir()) / "DiTing-agent-bridge-workers"
...
def _kill_windows_endpoint_occupants(endpoint: str) -> None:
    ...
    for pid in _windows_listening_pids_on_port(port):
        try:
            print(
                f"[DiTing-bridge] killing stale process tree pid={pid} port={port}",
                file=sys.stderr,
                flush=True,
            )
            subprocess.run(
            ...
            print(
                f"[DiTing-bridge] failed to kill stale process pid={pid}: {exc}",
                file=sys.stderr,
                flush=True,
            )
