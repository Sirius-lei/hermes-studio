...
DEFAULT_ENDPOINT = "tcp://127.0.0.1:28765" if os.name == "nt" else "ipc:///tmp/diting-agent-bridge.sock"
DEFAULT_AGENT_ROOT = "~/.diting/diting-agent"
DEFAULT_DiTing_HOME = "~/.diting"
APPROVAL_TIMEOUT_SECONDS = 120
APPROVAL_TIMEOUT_MS = APPROVAL_TIMEOUT_SECONDS * 1000
PARENT_WATCHDOG_INTERVAL_SECONDS = 2.0
OPENROUTER_ATTRIBUTION_ENV = {
    "referer": "DiTing_OPENROUTER_APP_REFERER",
    "title": "DiTing_OPENROUTER_APP_TITLE",
    "categories": "DiTing_OPENROUTER_APP_CATEGORIES",
}
_SURROGATE_RE = re.compile("[\ud800-\udfff]")


def _bridge_platform() -> str:
    return os.environ.get("DiTing_AGENT_BRIDGE_PLATFORM", "cli").strip() or "cli"
...
def _hidden_subprocess_kwargs() -> dict[str, Any]:
    if os.name != "nt":
        return {}
    if os.environ.get("DiTing_DESKTOP", "").strip().lower() != "true":
        return {}
...
def _install_windows_hidden_subprocess_defaults() -> None:
    ...
    if os.environ.get("DiTing_DESKTOP", "").strip().lower() != "true":
        return
    if getattr(subprocess, "_DiTing_hidden_defaults_installed", False):
        return
    ...
    subprocess._DiTing_hidden_defaults_installed = True  # type: ignore[attr-defined]
...
def _start_parent_process_watchdog(
    ...
    def run() -> None:
        while not stop_event.wait(interval):
            if _process_exists(parent_pid):
                continue
            print(
                f"[DiTing-bridge] parent pid {parent_pid} exited; stopping {label}",
                file=sys.stderr,
                flush=True,
            )
            stop_event.set()
    ...
    threading.Thread(target=run, daemon=True, name=f"DiTing-bridge-parent-watchdog-{label}").start()
...
def _suppress_bridge_platform_hint() -> None:
    raw = os.environ.get("DiTing_BRIDGE_SUPPRESS_PLATFORM_HINT", "cli").strip()
...
def _agent_roots_near_path(path: Path) -> list[Path]:
    roots: list[Path] = []
    try:
...
def _candidate_agent_roots_from_bin(command: str | None) -> list[Path]:
    if not command:
        return []
    ...
    candidates: list[Path] = [
        ...
        bin_path.parent.parent / "diting-agent",
        bin_path.parent.parent / "lib" / "diting-agent",
    ]
    try:
        script = bin_path.read_text(encoding="utf-8")
        for match in re.findall(r"""["']?((?:[A-Za-z]:)?[^"'\s`]*diting-agent[^"'\s`]*)["']?""", script, re.IGNORECASE):
            candidates.extend(_agent_roots_near_path(Path(match).expanduser()))
    except Exception:
        pass
    return candidates
...
def _candidate_agent_roots(raw: str | None = None) -> list[Path]:
    ...
    env_root = os.environ.get("DiTing_AGENT_ROOT")
    if env_root:
        candidates.append(Path(env_root).expanduser())

    for command in (
        os.environ.get("DiTing_BIN"),
        "diting",
    ):
        candidates.extend(_candidate_agent_roots_from_bin(command))

    script_path = Path(__file__).resolve()
    candidates.extend([
        Path.cwd(),
        Path.cwd() / "runtime" / "diting-agent",
        Path.cwd() / ".diting" / "diting-agent",
        Path.cwd() / "diting-agent",
        script_path.parent,
        script_path.parent.parent,
        script_path.parent.parent.parent,
        script_path.parent.parent.parent / ".diting" / "diting-agent",
    ])
    for parent in script_path.parents:
        candidates.extend([
            parent / ".diting" / "diting-agent",
            parent / "diting-agent",
        ])

    candidates.extend([
        Path.home() / ".diting" / "diting-agent",
        Path.home() / "diting-agent",
        Path("/opt/diting/diting-agent"),
        Path("/opt/diting-agent"),
        Path("/usr/local/lib/diting-agent"),
        Path("/usr/local/diting-agent"),
    ])
...
def _discover_agent_root(raw: str | None = None) -> Path:
    ...
    raise RuntimeError(
        "diting-agent runtime run_agent.py not found. Pass --agent-root or set "
        f"DiTing_AGENT_ROOT. Tried: {attempted}"
    )


def _discover_DiTing_home(raw: str | None = None) -> Path:
    if raw:
        return Path(raw).expanduser().resolve()
    env_home = os.environ.get("DiTing_HOME")
    if env_home:
        return Path(env_home).expanduser().resolve()
    DiTing_home = Path(DEFAULT_DiTing_HOME).expanduser()
    if DiTing_home.exists():
        return DiTing_home.resolve()
    return Path(DEFAULT_DiTing_HOME).expanduser().resolve()
...
def _bridge_log(event: str, payload: dict[str, Any]) -> None:
    try:
        ...
        print(
            "[DiTing_bridge] " + json.dumps(_sanitize_surrogates(body), ensure_ascii=False, default=_json_default),
            file=sys.stderr,
            flush=True,
        )
    except Exception:
        print(f"[DiTing_bridge] {event}", file=sys.stderr, flush=True)
...
def _agent_root() -> Path | None:
    return _find_agent_root(os.environ.get("DiTing_AGENT_ROOT"))


def _DiTing_home() -> Path:
    return _discover_DiTing_home(os.environ.get("DiTing_HOME"))


def _base_DiTing_home() -> Path:
    return _normalize_base_home(_discover_DiTing_home(
        os.environ.get("DiTing_AGENT_BRIDGE_BASE_HOME") or None
    ))


def _worker_profile() -> str | None:
    raw = os.environ.get("DiTing_AGENT_BRIDGE_WORKER_PROFILE", "").strip()
    return raw or None


def _profile_home(profile: str | None) -> Path:
    base = _base_DiTing_home()
...
def _set_path_env(agent_root: str | None = None, DiTing_home: str | None = None) -> None:
    resolved_root = _discover_agent_root(agent_root) if agent_root else _find_agent_root()
    if resolved_root is not None:
        os.environ["DiTing_AGENT_ROOT"] = str(resolved_root)
    else:
        os.environ.pop("DiTing_AGENT_ROOT", None)
    resolved_home = _discover_DiTing_home(DiTing_home)
    os.environ["DiTing_HOME"] = str(resolved_home)
    base_home = str(_normalize_base_home(resolved_home))
    os.environ["DiTing_AGENT_BRIDGE_BASE_HOME"] = base_home
...
def _ensure_agent_imports() -> None:
    ...
    if root is not None:
        ...
        raise RuntimeError(
            "diting-agent run_agent.py not found in source locations and the "
            "current Python environment cannot import run_agent. Install "
            "diting-agent or pass --agent-root/DiTing_AGENT_ROOT."
        )
    os.environ.setdefault("DiTing_HOME", str(_DiTing_home()))
    os.environ.setdefault("DiTing_AGENT_BRIDGE_BASE_HOME", str(_DiTing_home()))
    _apply_openrouter_attribution_override()


def _apply_openrouter_attribution_override() -> None:
    """Override diting-agent OpenRouter attribution at bridge runtime only."""
...
def _apply_profile_env(profile: str | None) -> str | None:
    """Temporarily set DiTing_HOME to the profile directory.
    Returns the original DiTing_HOME value to restore later.
    """
    profile_home = _profile_home(profile)
    original = os.environ.get("DiTing_HOME")
    if not (profile_home / "config.yaml").exists():
        return original
    os.environ["DiTing_HOME"] = str(profile_home)
    return original


def _restore_profile_env(original: str | None) -> None:
    """Restore DiTing_HOME after profile-scoped agent creation."""
    if original is not None:
        os.environ["DiTing_HOME"] = original
    else:
        os.environ.pop("DiTing_HOME", None)
...
def _set_worker_profile_env(profile: str | None) -> None:
    profile_home = _profile_home(profile)
    os.environ["DiTing_HOME"] = str(profile_home)
    os.environ["DiTing_AGENT_BRIDGE_WORKER_PROFILE"] = profile or "default"
...
def _refresh_terminal_env() -> None:
    """Bridge current worker DiTing_HOME/config.yaml terminal config to TERMINAL_* env vars.
    ...
    """
    DiTing_home = os.environ.get("DiTing_HOME", "")
    if not DiTing_home:
        return
    config_path = Path(DiTing_home) / "config.yaml"
    ...
    try:
        ...
        print(
            f"[DiTing-bridge] Failed to refresh terminal env from {config_path}",
            file=sys.stderr,
            flush=True,
        )
...
def _install_execute_code_approval_memory_patch() -> None:
    ...
    DiTing Agent intentionally treats execute_code approvals as one-shot in
    gateway/ask mode.  Web UI keeps DiTing_EXEC_ASK enabled so dangerous
    ...
    try:
        ...
        if not callable(original) or getattr(original, "_DiTing_web_ui_memory_patch", False):
            return
        ...
        setattr(patched_check_execute_code_guard, "_DiTing_web_ui_memory_patch", True)
        setattr(patched_check_execute_code_guard, "_DiTing_web_ui_original", original)
...
def _resolve_runtime(model: str, provider: str | None = None) -> dict[str, Any]:
    ...
    requested = provider or os.environ.get("DiTing_BRIDGE_PROVIDER", "").strip() or None
    return resolve_runtime_provider(requested=requested, target_model=model or None)


def _load_enabled_toolsets() -> list[str] | None:
    _ensure_agent_imports()
    raw = os.environ.get("DiTing_BRIDGE_TOOLSETS", "").strip()
    if raw:
...
def _discover_bridge_mcp_tools() -> list[str]:
    ...
    try:
        ...
        print(
            f"[DiTing_bridge] MCP tool discovery failed: {exc}",
            file=sys.stderr,
            flush=True,
        )
        return []
...
def _log_worker_startup_context(profile: str | None) -> None:
    ...
    try:
        ...
        _bridge_log("bridge.worker.initialized", {
            ...
            "platform": _bridge_platform(),
            "DiTing_home": str(_DiTing_home()),
            "base_DiTing_home": str(_base_DiTing_home()),
            "config_path": str(_DiTing_home() / "config.yaml"),
...
def _cfg_max_turns(cfg: dict[str, Any], default: int = 90) -> int:
    try:
        env_max = int(os.environ.get("DiTing_BRIDGE_MAX_TURNS", "") or 0)
        if env_max > 0:
            return env_max
    except ValueError:
        pass
