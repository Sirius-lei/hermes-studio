...
class BridgeBroker:
    ...
    def __init__(self, endpoint: str, agent_root: str | None = None, DiTing_home: str | None = None) -> None:
        ...
        self._worker_idle_timeout_seconds = _positive_int(
            os.environ.get("DiTing_AGENT_BRIDGE_WORKER_IDLE_TIMEOUT_SECONDS")
        ) or self.IDLE_TIMEOUT_SECONDS
        self._gc_interval_seconds = _positive_int(
            os.environ.get("DiTing_AGENT_BRIDGE_GC_INTERVAL_SECONDS")
        ) or self.GC_INTERVAL_SECONDS
        self._disable_worker_idle_gc = str(
            os.environ.get("DiTing_AGENT_BRIDGE_DISABLE_WORKER_IDLE_GC", "")
    ...
    def _worker_for_profile(self, profile: str, worker_key: str | None = None) -> WorkerProcess:
        ...
        with self._lock:
            ...
            if worker is None:
                worker = WorkerProcess(key, profile, _worker_endpoint(key, self.endpoint), self.agent_root, self.DiTing_home)
    ...
    def _handle_connection(self, conn: socket.socket) -> None:
        try:
            ...
            print(f"[DiTing-bridge-broker] connection error: {exc}", file=sys.stderr, flush=True)
        finally:
            try:
                conn.close()
            except OSError:
            ...
            while not self._stop.is_set():
                try:
                    ...
                    threading.Thread(
                        ...
                        args=(conn,),
                        daemon=True,
                        name="DiTing-bridge-broker-connection",
                    ).start()
                except KeyboardInterrupt:
                    break
                except Exception as exc:
                    print(f"[DiTing-bridge-broker] server loop error: {exc}", file=sys.stderr, flush=True)
        finally:
            restore_signals()
            try:
