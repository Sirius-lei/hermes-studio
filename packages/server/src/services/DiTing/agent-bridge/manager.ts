...
import { detectDiTingHome, getDiTingBin } from '../DiTing-path'
...
const OPENROUTER_WEB_UI_ATTRIBUTION_ENV = {
  DiTing_OPENROUTER_APP_REFERER: 'https://DiTing-studio.ai',
  DiTing_OPENROUTER_APP_TITLE: 'DiTing Studio',
  DiTing_OPENROUTER_APP_CATEGORIES: 'cli-agent,personal-agent',
} as const
...
function isLegacyGlobalDefaultEndpoint(endpoint: string): boolean {
  ...
  return normalized === 'ipc:///tmp/diting-agent-bridge.sock' ||
    normalized === 'tcp://127.0.0.1:28765' ||
    normalized === 'tcp://localhost:28765'
}

export function buildAgentBridgeProcessEnv(endpoint: string, DiTingHome: string | undefined, agentRoot: string | undefined): NodeJS.ProcessEnv {
  const home = DiTingHome || process.env.DiTing_HOME
  const referer = process.env.DiTing_OPENROUTER_APP_REFERER || OPENROUTER_WEB_UI_ATTRIBUTION_ENV.DiTing_OPENROUTER_APP_REFERER
  const title = process.env.DiTing_OPENROUTER_APP_TITLE || OPENROUTER_WEB_UI_ATTRIBUTION_ENV.DiTing_OPENROUTER_APP_TITLE
  const categories = process.env.DiTing_OPENROUTER_APP_CATEGORIES || OPENROUTER_WEB_UI_ATTRIBUTION_ENV.DiTing_OPENROUTER_APP_CATEGORIES
  const env: NodeJS.ProcessEnv = {
    ...process.env,
    PYTHONDONTWRITEBYTECODE: '1',
    DiTing_AGENT_BRIDGE_ENDPOINT: endpoint,
    DiTing_OPENROUTER_APP_REFERER: referer,
    DiTing_OPENROUTER_APP_TITLE: title,
    DiTing_OPENROUTER_APP_CATEGORIES: categories,
  }
  if (home) {
    env.DiTing_HOME = home
  }
  if (agentRoot) {
    env.DiTing_AGENT_ROOT = agentRoot
  }
...
function uvCandidates(agentRoot?: string): string[] {
  if (!agentRoot) {
    return [
      process.env.DiTing_AGENT_BRIDGE_UV,
      process.env.UV,
...
function agentRootFromPath(path: string): string | undefined {
  let current = path
  try {
    if (!existsSync(join(current, 'run_agent.py'))) {
  ...
  for (let i = 0; i < 8; i++) {
    if (existsSync(join(current, 'run_agent.py'))) return current
...
function agentRootFromDiTingBin(): string | undefined {
  const DiTingBin = resolveExecutable(getDiTingBin())
  if (!DiTingBin) return undefined

  const binDir = dirname(DiTingBin)
  const rootCandidates = [
    resolve(binDir, '..'),
    resolve(binDir, '..', '..'),
    resolve(binDir, '..', 'diting-agent'),
    resolve(binDir, '..', 'lib', 'diting-agent'),
    resolve(binDir, '..', '..', 'diting-agent'),
  ]
  ...
  try {
    const script = readFileSync(DiTingBin, 'utf-8')
    const scriptRoots = script.match(/["']?((?:[A-Za-z]:)?[^"'\s`]*diting-agent[^"'\s`]*)["']?/gi) || []
    for (const match of scriptRoots) {
      const candidate = match.replace(/^["']|["']$/g, '')
      const found = agentRootFromPath(candidate)
    ...
    if (python) {
      ...
      const shebangRootCandidates = [
        resolve(pyDir, '..', '..'),
        resolve(pyDir, '..', '..', 'diting-agent'),
        resolve(pyDir, '..', '..', 'lib', 'diting-agent'),
      ]
...
function DiTingBinPython(): string | undefined {
  const DiTingBin = resolveExecutable(getDiTingBin())
  if (!DiTingBin) return undefined
  try {
    const first = readFileSync(DiTingBin, 'utf-8').split(/\r?\n/, 1)[0]
    const match = first.match(/^#!\s*(.+)$/)
    const command = match?.[1]?.trim().split(/\s+/) || []
    const raw = command[0]?.endsWith('/env') && command[1] ? command[1] : command[0]
    if (!raw || !/^python(?:\d+(?:\.\d+)?)?(?:\.exe)?$/i.test(basename(raw))) return undefined
    const python = isAbsolute(raw) ? raw : resolveExecutable(raw)
...
function resolveAgentRoot(explicit?: string, DiTingHome = detectDiTingHome()): string | undefined {
  const workspaceRuntime = process.env.VITEST
    ? undefined
    : join(process.cwd(), 'runtime', 'diting-agent')
  const candidates = [
    explicit,
    process.env.DiTing_AGENT_ROOT,
    join(DiTingHome, 'diting-agent'),
    agentRootFromDiTingBin(),
    workspaceRuntime,
    process.cwd(),
    join(process.cwd(), 'diting-agent'),
    '/usr/local/lib/diting-agent',
    '/usr/local/diting-agent',
...
export function resolveAgentBridgeCommand(options: AgentBridgeManagerOptions = {}): BridgeCommand {
  const DiTingHome = options.DiTingHome || detectDiTingHome()
  const agentRoot = resolveAgentRoot(options.agentRoot, DiTingHome)
  const explicitPython = options.python || process.env.DiTing_AGENT_BRIDGE_PYTHON
  if (explicitPython) {
    return { command: explicitPython, argsPrefix: [], agentRoot, DiTingHome }
  }

  const venvPython = firstExistingExecutable(pathCandidates(agentRoot))
  if (venvPython) {
    return { command: venvPython, argsPrefix: [], agentRoot, DiTingHome }
  }

  const shebangPython = DiTingBinPython()
  if (shebangPython && existsSync(shebangPython)) {
    return { command: shebangPython, argsPrefix: [], agentRoot, DiTingHome }
  }
  ...
  if (uv) {
    ...
    prefix.push('python')
    return { command: uv, argsPrefix: prefix, agentRoot, DiTingHome }
  }
  ...
  return { command: fallback, argsPrefix: [], agentRoot, DiTingHome }
}

function bridgeScriptPath(): string {
  const candidates = [
    // Built server: dist/server/index.js -> dist/server/agent-bridge/python/DiTing_bridge.py
    resolve(__dirname, 'agent-bridge', 'python', 'DiTing_bridge.py'),
    // ts-node/dev source tree.
    resolve(__dirname, 'services/DiTing/agent-bridge/python/DiTing_bridge.py'),
    resolve(process.cwd(), 'packages/server/src/services/DiTing/agent-bridge/python/DiTing_bridge.py'),
  ]
...
function shouldKillStaleIpcBridgeProcesses(): boolean {
  const raw = String(process.env.DiTing_AGENT_BRIDGE_KILL_STALE_IPC || '').trim().toLowerCase()
...
export class AgentBridgeManager {
  ...
  constructor(options: AgentBridgeManagerOptions = {}) {
    ...
    this.explicitEndpoint = Boolean(options.endpoint || process.env.DiTing_AGENT_BRIDGE_ENDPOINT)
    this.endpoint = options.endpoint || process.env.DiTing_AGENT_BRIDGE_ENDPOINT || DEFAULT_AGENT_BRIDGE_ENDPOINT
  }
  ...
  private async waitForManagedChildExit(child: ChildProcess): Promise<boolean> {
    return new Promise<boolean>((resolve) => {
      const gracefulTimeoutMs = envPositiveInt('DiTing_AGENT_BRIDGE_RECOVERY_EXIT_TIMEOUT_MS')
        ?? DEFAULT_AGENT_BRIDGE_RECOVERY_EXIT_TIMEOUT_MS
      const sigkillWaitMs = envPositiveInt('DiTing_AGENT_BRIDGE_RECOVERY_SIGKILL_WAIT_MS')
  ...
  private async startProcess(): Promise<void> {
    ...
    const DiTingHome = command.DiTingHome
    if (agentRoot) args.push('--agent-root', agentRoot)
    if (DiTingHome) args.push('--diting-home', DiTingHome)

    const env = buildAgentBridgeProcessEnv(this.endpoint, DiTingHome, agentRoot)
    ...
    await new Promise<void>((resolveReady, rejectReady) => {
      const startupTimeoutMs = this.options.startupTimeoutMs
        ?? envPositiveInt('DiTing_AGENT_BRIDGE_STARTUP_TIMEOUT_MS')
  ...
  private async prepareEndpoint(): Promise<void> {
    ...
    // known-stale sockets with DiTing_AGENT_BRIDGE_KILL_STALE_IPC=1.
    ...
    process.env.DiTing_AGENT_BRIDGE_ENDPOINT = this.endpoint
  }

  private autoRestartEnabled(): boolean {
    const raw = String(process.env.DiTing_AGENT_BRIDGE_AUTO_RESTART || '').trim().toLowerCase()
  ...
  private scheduleRestart(code: number | null, signal: NodeJS.Signals | null): void {
    ...
    const envDelay = envPositiveInt('DiTing_AGENT_BRIDGE_RESTART_DELAY_MS') ?? DEFAULT_AGENT_BRIDGE_RESTART_DELAY_MS
    ...
    if (!child) {
      if (this.attached || this.ready) {
        try {
          const client = new AgentBridgeClient({
            endpoint: this.endpoint,
            timeoutMs: envPositiveInt('DiTing_AGENT_BRIDGE_SHUTDOWN_TIMEOUT_MS') ?? 5000,
            connectRetryMs: 0,
          })
