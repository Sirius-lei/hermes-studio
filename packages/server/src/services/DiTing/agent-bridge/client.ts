...
import { getActiveProfileName, getProfileDir } from '../DiTing-profile'
...
function resolveDefaultAgentBridgeEndpoint(): string {
  if (process.env.VITEST) {
    return process.platform === 'win32'
      ...
      : `ipc://${join(tmpdir(), `DiTing-agent-bridge-test-${process.pid}.sock`)}`
  }
  return process.platform === 'win32'
    ? 'tcp://127.0.0.1:18765'
...
export class AgentBridgeClient {
  ...
  constructor(options: AgentBridgeOptions = {}) {
    ...
    this.timeoutMs = options.timeoutMs ?? envPositiveInt('DiTing_AGENT_BRIDGE_TIMEOUT_MS') ?? DEFAULT_AGENT_BRIDGE_TIMEOUT_MS
    this.connectRetryMs = options.connectRetryMs ?? envPositiveInt('DiTing_AGENT_BRIDGE_CONNECT_RETRY_MS') ?? 5000
  }
