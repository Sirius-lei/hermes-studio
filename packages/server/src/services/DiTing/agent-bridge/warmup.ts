...
import { getActiveProfileName } from '../DiTing-profile'
...
export function resolveWarmProfiles(
  raw = process.env.DiTing_AGENT_BRIDGE_WARM_PROFILES,
...
export async function warmAgentBridgeProfilesOnStartup(endpoint?: string): Promise<string[]> {
  ...
  const client = new AgentBridgeClient({
    endpoint,
    timeoutMs: envPositiveInt('DiTing_AGENT_BRIDGE_WARM_TIMEOUT_MS') ?? 30000,
    connectRetryMs: 0,
  })
