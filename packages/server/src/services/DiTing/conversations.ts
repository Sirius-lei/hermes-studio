import { exportSessionsRaw, type DiTingSessionFull } from './DiTing-cli'
...
type ConversationSession = DiTingSessionFull & {
...
function visibleHumanMessage(message: DiTingMessageLike): boolean {
...
function firstVisibleHumanText(messages: DiTingMessageLike[]): string {
  const firstVisible = messages.find(visibleHumanMessage)
  return firstVisible ? textFromContent(firstVisible.content).trim() : ''
}

function maxMessageTimestamp(messages: DiTingMessageLike[]): number {
...
function enrichSession(session: DiTingSessionFull, nowSeconds: number): ConversationSession {
...
function sessionMessages(session: DiTingSessionFull): DiTingMessageLike[] {
  return Array.isArray(session.messages) ? session.messages as DiTingMessageLike[] : []
}

function normalizeVisibleMessage(message: DiTingMessageLike, session: DiTingSessionFull, index: number): ConversationMessage | null {
...
function visibleMessagesForSessions(sessions: DiTingSessionFull[]): ConversationMessage[] {
  return sessions
...
function hasVisibleHumanMessages(sessions: DiTingSessionFull[]): boolean {
