...
import { getDiTingBaseDir, getProfileDir } from './DiTing-profile'
...
const WRITE_GATE_REQUIRED_FILES = [
  ...
  join('DiTing_cli', 'write_approval_commands.py'),
]
const WRITE_GATE_IMPORT_CHECK = 'import tools.write_approval; import DiTing_cli.write_approval_commands'
...
async function listPendingFor(profile: string, subsystem: WriteGateSubsystem): Promise<PendingWriteRecord[]> {
  ...
  for (const entry of entries) {
    ...
    try {
      ...
      // Skip unreadable records, matching DiTing Agent's pending list behavior.
    }
  }
...
function DiTingShebangPython(): string | null {
  for (const candidate of pathCandidates(getDiTingBin())) {
...
function agentRootFromDiTingBin(): string {
  for (const candidate of pathCandidates(getDiTingBin())) {
    if (!existsSync(candidate)) continue
    try {
...
function agentRootCandidates(): string[] {
  const shebangPython = DiTingShebangPython()
  return [
    process.env.DiTing_AGENT_ROOT?.trim() || '',
    join(getDiTingBaseDir(), 'DiTing-agent'),
    join(homedir(), '.DiTing', 'DiTing-agent'),
...
function resolveAgentRoot(): string {
  ...
  throw new Error('DiTing Agent source not found. Set DiTing_AGENT_ROOT to enable write approval actions.')
}

function resolveDiTingPython(agentRoot?: string): string {
...
function hasWriteGatePythonImports(): boolean {
  const agentRoot = tryResolveAgentRoot()
  const python = resolveDiTingPython(agentRoot)
...
async function runPythonAction(
  ...
  if (!isWriteGateSupported()) {
    throw new Error('DiTing Agent write approval is not supported by the installed DiTing Agent version.')
  }
  const agentRoot = tryResolveAgentRoot()
  const python = resolveDiTingPython(agentRoot)
