...
import { normalizeMessageContentForStorage, normalizeMessageContentForStorageRole } from '../../../db/DiTing/message-content'
...
import { findUserByUsername, getUserAvatar } from '../../../db/DiTing/users-store'
...
function maxAgentMentionDepth(): number {
    const value = Number(process.env.DiTing_GROUP_CHAT_MAX_AGENT_MENTION_DEPTH)
...
class ChatStorage {
    ...
    init(): void {
        ...
        const db = this.db()
        if (!db) return
        // Tables are now created centrally in initAllDiTingTables()
