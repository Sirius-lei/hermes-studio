...
class AgentClient {
    ...
    async connect(port?: number): Promise<void> {
        const actualPort = port ?? parseInt(process.env.PORT || '18648', 10)
    ...
    // ─── DiTing Agent Bridge Integration ───────────────────────
    ...
    async replyToMention(
        ...
        try {
            ...
            const runPrompt = 'When calling DiTing Web UI endpoints from tools or skills, include the current DiTing profile as the X-DiTing-Profile header if the endpoint supports profile-scoped behavior.'
