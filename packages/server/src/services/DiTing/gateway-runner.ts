...
import { getActiveProfileDir } from './DiTing-profile'
import { spawnDiTingWithBin } from './DiTing-process'

interface SupervisedGateway {
  pid: number
  child: ReturnType<typeof spawnDiTingWithBin>
  DiTingBin: string
  profileDir: string
  startedAt: number
}
 ...
 *   1. `/restart` calls `DiTing gateway stop` (CLI) which kills our child.
...
export function startGatewayRunManaged(
  ...
  return startGatewayRunManagedInternal(DiTingBin, {
...
function startGatewayRunManagedInternal(
  ...
  const child = spawnDiTingWithBin(DiTingBin, ['gateway', 'run', '--replace'], {
    detached: true,
    stdio: 'ignore',
    windowsHide: true,
    env: {
  ...
  if (pid) {
    const entry: SupervisedGateway = { pid, child, DiTingBin, profileDir, startedAt: Date.now() }
    state.current = entry
    ...
    child.on('exit', (code, signal) => {
      ...
      state.respawnTimer = setTimeout(() => {
        state.respawnTimer = null
        try {
          const next = startGatewayRunManagedInternal(DiTingBin, {
            profileDir,
