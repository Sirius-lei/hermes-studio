 ...
 * DiTing CLI, tracks failures (max 3 attempts), and auto-drains on
 * a timer + profile switch.
 */
import { getDb } from '../../db/index'
import { deleteSession as DiTingDeleteSession } from './DiTing-cli'
...
export class SessionDeleter {
  ...
  async drain(profile: string): Promise<{ deleted: string[]; skipped: string[]; failed: string[] }> {
    ...
    for (const row of rows) {
      try {
        const ok = await DiTingDeleteSession(row.session_id)
        if (ok) {
