...
export interface GatewayAutoStartConfig {
  ...
  include?: string[]
  exclude?: string[]
  // Derived from DiTing Agent default config.yaml when returned by the config
...
export interface AppConfig {
  ...
  // These aliases never replace the canonical model ID sent back to DiTing.
  ...
  // provider catalog response without changing DiTing Agent config.yaml.
  customModels?: Record<string, string[]>

  // Web UI-only model picker visibility. This filters what the WUI exposes in
  // its sidebar/model pages and never renames or rewrites DiTing canonical
  // provider/model IDs. DiTing CLI config remains the upstream source of truth.
  modelVisibility?: Record<string, ModelVisibilityRule>

  // Web UI startup policy for automatically starting DiTing API gateways.
  // Defaults to legacy behavior: all local profiles are eligible. This is a
  // Web UI-level setting, not the active DiTing profile's config.yaml.
