...
export function defaultHermesWorkspace(
  profile: string,
  options: {
...
export async function ensureHermesRunWorkspace(
  profile: string,
  ...
  const resolved = String(workspace || '').trim() || defaultHermesWorkspace(profile, options)
