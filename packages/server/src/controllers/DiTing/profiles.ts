...
import { SessionDeleter } from '../../services/DiTing/session-deleter'
...
} from '../../services/DiTing/gateway-autostart'
import { logger } from '../../services/logger'
import { smartCloneCleanup, copyModelProviderAuthForClone } from '../../services/DiTing/profile-credentials'
import { detectDiTingRootHome } from '../../services/DiTing/DiTing-path'
...
import type { DiTingProfile } from '../../services/DiTing/DiTing-cli'
...
function getActiveProfileFile(): string {
  return join(detectDiTingRootHome(), 'active_profile')
}

function listProfilesFromDisk(activeProfileName: string): DiTingProfile[] {
  const base = detectDiTingRootHome()
  const profiles: DiTingProfile[] = [{
    name: 'default',
...
async function injectBundledSkillsForProfile(name: string): Promise<void> {
  try {
    const targetDir = DiTingSkillInjector.resolveTargetDirForProfile(name)
    const result = await new DiTingSkillInjector(undefined, targetDir).injectMissingSkills()
...
function deleteForbiddenProfileFromDisk(name: string): boolean {
  ...
  logger.warn('[deleteProfile] removed reserved profile "%s" from disk after DiTing CLI rejected deletion', name)
  return true
}
...
function filterVisibleProfiles(profiles: DiTingProfile[]): DiTingProfile[] {
  return profiles.filter(profile => !isForbiddenProfileName(profile.name))
}

function requestedProfileName(ctx: any): string {
  return ctx.state?.profile?.name || ctx.get?.('x-DiTing-profile') || getActiveProfileName()
}

function filterProfilesForUser(ctx: any, profiles: DiTingProfile[]): DiTingProfile[] {
...
function attachProfileAvatars<T extends DiTingProfile>(profiles: T[]): Array<T & { avatar: ProfileAvatarResponse | null }> {
...
async function useProfileWithFallback(name: string): Promise<string> {
  ...
  try {
    return await DiTingCli.useProfile(name)
  } catch (err: any) {
    if (!profileExistsForManualSwitch(name)) throw err

    const base = detectDiTingRootHome()
    writeFileSync(join(base, 'active_profile'), `${name}\n`, 'utf-8')
    logger.warn(err, '[switchProfile] DiTing profile use failed; wrote active_profile directly for existing profile "%s"', name)
...
async function buildRuntimeStatus(profile: DiTingProfile | string, bridgeState?: Awaited<ReturnType<typeof readBridgeWorkers>>) {
...
function listProfilesForStatusFast(): DiTingProfile[] {
...
export async function list(ctx: any) {
  try {
    let profiles: DiTingProfile[]
    try {
      profiles = await DiTingCli.listProfiles()
    } catch (err: any) {
      const { getActiveProfileName } = await import('../../services/DiTing/DiTing-profile')
    ...
    // Web UI active profile is request-scoped and comes from X-DiTing-Profile.
    profiles.forEach(p => {
...
export async function create(ctx: any) {
  ...
  try {
    const output = await DiTingCli.createProfile(name, clone)
...
export async function get(ctx: any) {
  ...
  try {
    const profile = await DiTingCli.getProfile(name)
...
async function listProfilesForStatus(): Promise<DiTingProfile[]> {
  let profiles: DiTingProfile[]
  try {
    profiles = await DiTingCli.listProfiles()
  } catch {
...
export async function remove(ctx: any) {
  ...
  try {
    ...
    const ok = await DiTingCli.deleteProfile(name)
...
export async function rename(ctx: any) {
  ...
  try {
    const ok = await DiTingCli.renameProfile(ctx.params.name, new_name)
    if (ok) {
...
export async function switchProfile(ctx: any) {
  ...
  try {
    ...
    try {
      const result = await bridgeCleanupClient().destroyProfile(name)
      logger.info('[switchProfile] destroyed bridge sessions for DiTing profile "%s" destroyed=%s', name, result.destroyed)
    } catch (err: any) {
      logger.warn(err, '[switchProfile] failed to destroy bridge sessions for profile "%s"', name)
    }

    try {
      const detail = await DiTingCli.getProfile(name)
      ...
      if (!existsSync(profileConfig)) {
        writeFileSync(profileConfig, '# DiTing Agent Configuration\n', 'utf-8')
        logger.info('Created config.yaml for: %s', detail.path)
      }

      const profileEnv = join(detail.path, '.env')
      if (!existsSync(profileEnv)) {
        writeFileSync(profileEnv, '# DiTing Agent Environment Configuration\n', 'utf-8')
    ...
    logger.info('[switchProfile] switched session deleter to DiTing profile "%s"', name)
...
export async function exportProfile(ctx: any) {
  const { name } = ctx.params
  if (denyProfile(ctx, name)) return
  const outputPath = join(tmpdir(), `DiTing-profile-${name}.tar.gz`)
  try {
    await DiTingCli.exportProfile(name, outputPath)
...
export async function importProfile(ctx: any) {
  ...
  const tmpDir = join(tmpdir(), 'DiTing-import')
  ...
  try {
    const result = await DiTingCli.importProfile(archivePath)
