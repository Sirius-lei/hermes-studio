...
} from '../../db/DiTing/stt-settings-store'
import { config } from '../../config'
import { SttProviderConfigError, transcribeWithProvider } from '../../services/DiTing/stt-providers'
import { SttNoSpeechDetectedError } from '../../services/DiTing/stt-providers/types'
...
const STT_TRANSCRIBE_FAILED_PROMPT_PCM_URL =
  'https://ekko-DiTing-studio.oss-cn-beijing.aliyuncs.com/stt-transcribe-failed-xiaohe.s16le.pcm'
...
export async function missingProfileAudio(ctx: Context) {
  ...
  ctx.set('X-DiTing-STT-Configured', 'false')
  ctx.set('X-DiTing-STT-Reason', status.reason || 'stt_not_configured')
...
export async function mcuVoiceTurn(ctx: Context) {
  ...
  const interactionId = ctx.get('x-DiTing-mcu-interaction-id') || `mcu-voice-${Date.now()}`
  const token = bearerToken(ctx)
  const clientId = ctx.get('x-DiTing-mcu-device-id') || undefined
  let debugAudioPath = ''
