...
const GEMINI_CALLBACK_BIND_HOST = process.env.DiTing_WEB_UI_GEMINI_CALLBACK_BIND_HOST?.trim() || GEMINI_REDIRECT_HOST
...
export function resolveGeminiOAuthClientCredentials(): { clientId: string; clientSecret: string } {
  return {
    clientId: process.env.DiTing_GEMINI_CLIENT_ID?.trim() || DEFAULT_GOOGLE_CLIENT_ID,
    clientSecret: process.env.DiTing_GEMINI_CLIENT_SECRET?.trim() || DEFAULT_GOOGLE_CLIENT_SECRET,
  }
}
...
function html(status: 'success' | 'error', message: string): string {
  ...
  return `<!doctype html><html><head><meta charset="utf-8"><title>DiTing</title><style>body{font:16px/1.5 system-ui,sans-serif;margin:10vh auto;max-width:32rem;text-align:center;color:#222}h1{color:${color}}p{color:#555}</style></head><body><h1>${title}</h1><p>${safeMessage}</p></body></html>`
}
    ...
    const server = createServer((req, res) => {
      void (async () => {
        ...
        try {
          ...
          closeServer(session)
          const body = html('success', 'You can close this tab and return to DiTing Web UI.')
...
export async function start(ctx: any) {
  try {
    ...
    const authorizeUrl = `${GOOGLE_AUTH_ENDPOINT}?${params.toString()}#DiTing`
