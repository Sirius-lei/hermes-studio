...
import { getActiveProfileName, getProfileDir, listProfileNamesFromDisk } from '../../services/DiTing/DiTing-profile'
...
async function resolveApiKeyImageProvider(profile: string, providerName = APIKEY_IMAGE_PROVIDER): Promise<ApiKeyImageProvider | null> {
  ...
  const DiTingConfig = await readConfigYamlForProfile(profile)
  const customProviders = getCompatibleCustomProviders(DiTingConfig)
