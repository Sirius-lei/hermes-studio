...
import { gatewayAutostartDisabledByEnv, reconcileGatewayManagementTransition, restartGatewayForProfile } from '../../services/DiTing/gateway-autostart'
...
import { EXCLUSIVE_PLATFORM_CREDENTIAL_KEYS } from '../../services/DiTing/profile-credentials'
...
function gatewayManagementFromDiTingConfig(config: Record<string, any>): 'per_profile' | 'unified' {
...
async function readGatewayAutoStartForResponse(): Promise<ReturnType<typeof normalizeGatewayAutoStartConfig>> {
  ...
  gatewayAutoStart.management = gatewayManagementFromDiTingConfig(defaultConfig)
  return gatewayAutoStart
}

async function writeDiTingGatewayManagement(mode: unknown): Promise<'per_profile' | 'unified' | null> {
  ...
  try {
    if (APP_CONFIG_SECTIONS.has(section)) {
      if (section === 'gatewayAutoStart') {
        ...
        const writtenManagement = await writeDiTingGatewayManagement(values.management)
    ...
    // Platform adapters run through DiTing gateway; restart it so channel
