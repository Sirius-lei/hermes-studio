...
import { getActiveProfileName, getProfileDir } from '../../services/DiTing/DiTing-profile'
import { getSkillUsageStatsFromDb } from '../../db/DiTing/sessions-db'
...
type SkillTarget = 'DiTing' | 'claude' | 'codex'

function requestSkillTarget(ctx: any): SkillTarget {
  const target = String(ctx.query?.target || 'DiTing').trim().toLowerCase()
  return target === 'claude' || target === 'codex' ? target : 'DiTing'
}

function globalSkillsDir(target: Exclude<SkillTarget, 'DiTing'>): string {
...
function requestTargetSkillsDir(ctx: any): string {
  ...
  return target === 'DiTing' ? requestSkillsDir(ctx) : globalSkillsDir(target)
}
...
/** Read bundled manifest as a name→hash map from ~/.diting/skills/.bundled_manifest */
...
/** Read hub-installed skill names from ~/.diting/skills/.hub/lock.json */
...
/** Compute md5 hash of all files in a directory (mirrors DiTing _dir_hash), with in-memory cache */
...
/** GET /api/DiTing/skills/external-dirs — return the raw list with existence flags */
...
/** PUT /api/DiTing/skills/external-dirs — replace the list verbatim. */
...
export async function importSkill(ctx: any) {
  ...
  try {
    if (isSingleZip) {
      ...
      stagingDir = join(tmpdir(), `DiTing-skill-import-${randomBytes(6).toString('hex')}`)
