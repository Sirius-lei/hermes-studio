...
import { getActiveEnvPath, getActiveAuthPath, getActiveProfileName, getProfileDir, listProfileNamesFromDisk } from '../../services/DiTing/DiTing-profile'
...
import { getCompatibleCustomProviders } from '../../services/DiTing/custom-providers-compat'
import { buildProviderModelMap, PROVIDER_PRESETS } from '../../shared/providers'
import { getCopilotModelsDetailed, resolveCopilotOAuthToken, type CopilotModelMeta } from '../../services/DiTing/copilot-models'
import { readAppConfig, writeAppConfig, type ModelVisibilityRule } from '../../services/app-config'
import { getDb } from '../../db'
import { MODEL_CONTEXT_TABLE } from '../../db/DiTing/schemas'
...
} from '../../services/DiTing/model-catalog-cache'
...
function requestScopedProfileName(ctx: any): string {
  const headerProfile = typeof ctx.get === 'function' ? ctx.get('x-DiTing-profile') : ''
...
export async function resolveAvailableModelForProfile(input: {
  ...
  if (!provider || !model) throw new Error('No DiTing model is available for planning')
...
export async function getAvailable(ctx: any) {
  try {
    ...
    if (typeof modelSection === 'object' && modelSection !== null) {
      currentDefault = String(modelSection.default || '').trim()
      currentDefaultProvider = String(modelSection.provider || '').trim()
      // When DiTing CLI sets provider: custom, resolve to custom:name
    ...
    const isOAuthAuthorized = (providerKey: string): boolean => {
      try {
        ...
        const provider = auth.providers?.[providerKey]
        const pool = auth.credential_pool?.[providerKey]
        // Legacy OAuth providers are stored under providers.*; newer DiTing
    ...
    // 时也不返回。避免误把 VS Code/gh CLI 用户的全局凭证当作 DiTing provider。
    const appConfig = await readAppConfig()
    const copilotEnabled = appConfig.copilotEnabled === true
    const modelAliases = normalizeAliases(appConfig.modelAliases)
    const modelVisibility = normalizeModelVisibility(appConfig.modelVisibility)
...
export async function updateModelContext(ctx: any) {
  // 支持两种方式：
  // 1. 路径参数: /api/DiTing/model-context/:provider/:model
  // 2. 查询参数: /api/DiTing/model-context?provider=xxx&model=xxx
