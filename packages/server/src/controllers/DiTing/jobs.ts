...
import { getDiTingBin } from '../../services/DiTing/DiTing-path'
import { getActiveProfileName, getProfileDir } from '../../services/DiTing/DiTing-profile'
import { execDiTingWithBin } from '../../services/DiTing/DiTing-process'
...
async function runDiTingCron(profile: string, args: string[]): Promise<void> {
  const profileDir = resolveProfileDir(profile)
  try {
    await execDiTingWithBin(getDiTingBin(), args, {
      cwd: process.cwd(),
      env: { ...process.env, DiTing_HOME: profileDir },
      timeout: TIMEOUT_MS,
      maxBuffer: 1024 * 1024,
      windowsHide: true,
    })
    ...
    throw new Error(stderr || stdout || error?.message || 'DiTing cron command failed')
  }
}
...
function sendCommandError(ctx: Context, error: any): void {
  ctx.status = 500
  ctx.body = { error: { message: error?.message || 'DiTing cron command failed' } }
}
...
export async function create(ctx: Context) {
  ...
  if (repeat != null) {
    ...
    // DiTing CLI normalizes repeat <= 0 to an unbounded/null repeat.
    args.push('--repeat', '0')
  }
  ...
  try {
    await runDiTingCron(profile, args)
...
export async function remove(ctx: Context) {
  ...
  try {
    await runDiTingCron(profile, getCronArgs(profile, 'remove', ctx.params.id))
    ctx.body = { ok: true }
  } catch (error: any) {
    sendCommandError(ctx, error)
  }
...
export async function pause(ctx: Context) {
  ...
  try {
    await runDiTingCron(profile, getCronArgs(profile, 'pause', ctx.params.id))
    const job = findJob(profile, ctx.params.id)
    ctx.body = { job }
  } catch (error: any) {
    sendCommandError(ctx, error)
...
export async function resume(ctx: Context) {
  ...
  try {
    await runDiTingCron(profile, getCronArgs(profile, 'resume', ctx.params.id))
    const job = findJob(profile, ctx.params.id)
    ctx.body = { job }
  } catch (error: any) {
    sendCommandError(ctx, error)
...
export async function run(ctx: Context) {
  ...
  try {
    await runDiTingCron(profile, getCronArgs(profile, 'run', ctx.params.id))
    const job = findJob(profile, ctx.params.id)
    ctx.body = { job }
  } catch (error: any) {
    sendCommandError(ctx, error)
