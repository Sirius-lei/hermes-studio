...
import { listSessionSummaries, getUsageStatsFromDb, getSessionDetailFromDb, getSessionDetailFromDbWithProfile, getSessionDetailPaginatedFromDbWithProfile, getExactSessionDetailFromDbWithProfile } from '../../db/DiTing/sessions-db'
import {
...
} from '../../db/DiTing/session-store'
import { ExportCompressor } from '../../lib/context-compressor/export-compressor'
import { deleteUsage, getUsage, getUsageBatch } from '../../db/DiTing/usage-store'
import type { UsageStatsModelRow, UsageStatsDailyRow } from '../../db/DiTing/usage-store'
import {
  getCollaborationRun as getStoredCollaborationRun,
  listSessionCollaborationRuns,
} from '../../db/DiTing/collaboration-run-store'
import { getModelContextLength } from '../../services/DiTing/model-context'
import { getActiveProfileName, listProfileNamesFromDisk } from '../../services/DiTing/DiTing-profile'
import { isPathWithin } from '../../services/DiTing/DiTing-path'
import { getGroupChatServer } from '../../routes/DiTing/group-chat'
import { logger } from '../../services/logger'
import type { ConversationSummary } from '../../services/DiTing/conversations'
...
import { AgentBridgeClient, getAgentBridgeManager } from '../../services/DiTing/agent-bridge'
...
function isDiTingHistorySessionSource(source?: string | null): boolean {
...
async function deleteDiTingSessionIfPresent(sessionId: string, profile?: string | null): Promise<DiTingDeleteResult> {
  ...
  try {
    ...
    const deleted = await DiTingCli.deleteSessionForProfile(sessionId, targetProfile)
    return {
      attempted: true,
      deleted,
      profile: targetProfile,
      error: deleted ? undefined : 'Failed to delete DiTing session',
    }
  } catch (err: any) {
    const message = err?.message || 'Failed to inspect DiTing session'
    logger.warn({ err, sessionId, profile: targetProfile }, 'DiTing Session: profile delete skipped')
...
async function getProfileDefaultModel(profile: string): Promise<ProfileDefaultModel> {
  try {
    ...
    logger.warn({ err, profile }, 'DiTing Session: failed to read profile default model for import')
  }
...
/**
 * List DiTing sessions only (exclude api_server source)
 * GET /api/DiTing/sessions/DiTing?source=&limit=
 */
export async function listDiTingSessions(ctx: any) {
  const source = (ctx.query.source as string) || undefined
  const limit = ctx.query.limit ? parseInt(ctx.query.limit as string, 10) : undefined
  ...
  ctx.body = { sessions: filterPendingDeletedSessions(filterBySessionAccess(ctx, filterByAllowedProfiles(ctx, allSessions)).filter(s => isDiTingHistorySessionSource(s.source))) }
}
...
/**
 * Get DiTing session detail only (exclude api_server source)
 * GET /api/DiTing/sessions/DiTing/:id
 */
export async function getDiTingSession(ctx: any) {
  const profile = requestedProfile(ctx)

  // Prefer the Web UI local session store. DiTing state.db can lag behind or
  ...
  if (localSession && isDiTingHistorySessionSource(localSession.source) && (!profile || localSessionProfile === profile)) {
    if (denySessionAccess(ctx, localSession)) return
    ctx.body = { session: localSession }
    return
  }

  // Try DiTing state.db next (consistent with listDiTingSessions)
  try {
    const session = profile
      ? await getSessionDetailFromDbWithProfile(ctx.params.id, profile)
      : await getSessionDetailFromDb(ctx.params.id)
    if (session && isDiTingHistorySessionSource(session.source)) {
    ...
    logger.warn(err, 'DiTing Session DB: detail query failed, falling back to CLI')
  }

  // Fallback to CLI
  const session = await DiTingCli.getSession(ctx.params.id)
  if (!session) {
    ctx.status = 404
    ctx.body = { error: 'Session not found' }
    return
  ...
  if (!isDiTingHistorySessionSource(session.source)) {
    ctx.status = 404
    ctx.body = { error: 'Session not found' }
    return
  }
...
export async function importDiTingSession(ctx: any) {
  ...
  try {
    ...
    logger.warn({ err, sessionId, profile }, 'DiTing Session: import query failed')
    ctx.status = 500
    ctx.body = { error: 'Failed to read DiTing session' }
    return
  }
...
export async function remove(ctx: any) {
  ...
  const DiTingProfile = requestedProfile(ctx) || existing?.profile || getActiveProfileName()
  ...
  const DiTing = codingAgentSession
    ? { attempted: false, deleted: false, profile: DiTingProfile }
    : await deleteDiTingSessionIfPresent(sessionId, DiTingProfile)
  ...
  ctx.body = { ok: true, deleted: Boolean(existing), DiTing }
}
...
export async function batchRemove(ctx: any) {
  ...
  const results = {
    ...
    DiTingErrors: [] as Array<{ id: string; profile?: string; error: string }>
  }
  ...
  for (const target of targets) {
    ...
    const DiTing = codingAgentSession
      ...
      : await deleteDiTingSessionIfPresent(id, targetProfile)
    if (DiTing.deleted) {
      results.DiTingDeleted++
    } else if (DiTing.attempted && DiTing.error) {
      results.DiTingFailed++
      results.DiTingErrors.push({ id, profile: DiTing.profile, error: DiTing.error })
    }
...
export async function setWorkspace(ctx: any) {
  ...
  const { updateSession, getSession, createSession } = await import('../../db/DiTing/session-store')
...
export async function usageStats(ctx: any) {
  ...
  try {
    DiTing = profile ? await getUsageStatsFromDb(days, undefined, profile) : await getUsageStatsFromDb(days)
  } catch (err) {
    logger.warn(err, 'usageStats: failed to load DiTing usage analytics from state.db')
  }
  ...
  ctx.body = {
    total_input_tokens: DiTing.input_tokens,
    total_output_tokens: DiTing.output_tokens,
    total_cache_read_tokens: DiTing.cache_read_tokens,
    total_cache_write_tokens: DiTing.cache_write_tokens,
    total_reasoning_tokens: DiTing.reasoning_tokens,
    total_sessions: DiTing.sessions,
    total_cost: DiTing.cost,
    total_api_calls: DiTing.total_api_calls,
    period_days: days,
    model_usage: DiTing.by_model.sort((a, b) => (b.input_tokens + b.output_tokens) - (a.input_tokens + a.output_tokens)),
 ...
 * GET /api/DiTing/workspace/folders?path=<relative_path>
...
export async function getConversationMessagesPaginated(ctx: any) {
  ...
  const { getSessionDetailPaginated } = await import('../../db/DiTing/session-store')
